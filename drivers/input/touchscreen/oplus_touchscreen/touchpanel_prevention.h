...
struct kernel_grip_info {
	...
	struct grip_point_info first_point
		[TOUCH_MAX_NUM]; /*store the fist frame point of each ID*/
	struct grip_point_info second_point
		[TOUCH_MAX_NUM]; /*store the second frame point of each ID*/
	struct grip_point_info latest_points
		[TOUCH_MAX_NUM]
		[POINT_DIFF_CNT]; /*store the latest different 5 points of each ID*/
	bool sync_up_makeup
		[TOUCH_MAX_NUM]; /*shows this point need make up while report up*/
	...
	uint8_t large_finger_status
		[TOUCH_MAX_NUM]; /*show large area finger status for grip strategy*/
	uint8_t large_point_status
		[TOUCH_MAX_NUM]; /*parameter for judge if ponits in large area need make up or not*/
	...
	bool condition_out_status
		[TOUCH_MAX_NUM]; /*show if exit the conditional rejection*/
	...
	struct hrtimer
		grip_up_timer[TOUCH_MAX_NUM]; /*using for report touch up event*/
	bool grip_hold_status[TOUCH_MAX_NUM]; /*show if this id is in hold status*/
	struct work_struct
		grip_up_work[TOUCH_MAX_NUM]; /*using for report touch up*/
	struct workqueue_struct
		*grip_up_handle_wq; /*just for handle report up event*/
	...
	uint8_t exit_match_times
		[TOUCH_MAX_NUM]; /*record actual match the exit condition times*/
	struct grip_point_info txMax_frame_point
		[TOUCH_MAX_NUM]; /*record the max tx frame point info*/
	struct grip_point_info rxMax_frame_point
		[TOUCH_MAX_NUM]; /*record the max rx frame point info*/
	struct grip_point_info last_frame_point
		[TOUCH_MAX_NUM]; /*record the last frame point info*/
	struct grip_point_info rxChanged_frame_point
		[TOUCH_MAX_NUM]; /*record the rx changed frame point info*/
	struct grip_point_info txChanged_frame_point
		[TOUCH_MAX_NUM]; /*record the tx changed frame point info*/
	uint32_t fsr_stable_time
		[TOUCH_MAX_NUM]; /*record fsr stable time under detect period*/
	uint8_t points_pos[TOUCH_MAX_NUM]; /*record point area distribution*/
	uint8_t points_center_down
		[TOUCH_MAX_NUM]; /*record whether points down after center point down*/
