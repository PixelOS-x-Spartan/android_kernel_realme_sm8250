...
static int sec_read_mutual(struct chip_data_s6sy792 *chip_info, u8 type,
			   char *data, int len);
static bool check_calibration(struct chip_data_s6sy792 *chip_info);
static int ver_bottom_large_handle_func(struct grip_zone_area *grip_zone,
					bool enable);
static int hor_corner_large_handle_func(struct grip_zone_area *grip_zone,
					bool enable);
static int long_size_dead_zone_handle_func(struct grip_zone_area *grip_zone,
					   bool enable);
static int short_size_dead_zone_handle_func(struct grip_zone_area *grip_zone,
					    bool enable);
static int long_size_condtion_zone_handle_func(struct grip_zone_area *grip_zone,
					       bool enable);
static int
short_size_condtion_zone_handle_func(struct grip_zone_area *grip_zone,
				     bool enable);
static int long_size_large_zone_handle_func(struct grip_zone_area *grip_zone,
					    bool enable);
static int short_size_large_zone_handle_func(struct grip_zone_area *grip_zone,
					     bool enable);
...
static int sec_enable_black_gesture(struct chip_data_s6sy792 *chip_info,
				    bool enable)
{
	int ret = 0;
	int i = 0;
...
static void sec_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	struct chip_data_s6sy792 *chip_info =
		(struct chip_data_s6sy792 *)chip_data;
	int ret = -1;
	int i = 0;
...
static int sec_enable_charge_mode(struct chip_data_s6sy792 *chip_info,
				  bool enable)
{
	int ret = -1;
...
static int sec_enable_earsense_mode(struct chip_data_s6sy792 *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int sec_enable_palm_reject(struct chip_data_s6sy792 *chip_info,
				  bool enable)
{
	int ret = -1;
...
static int sec_enable_game_mode(struct chip_data_s6sy792 *chip_info,
				bool enable)
{
	int ret = -1;
...
static int sec_enable_headset_mode(struct chip_data_s6sy792 *chip_info,
				   bool enable)
{
	int ret = -1;
...
static int sec_wait_for_ready(struct chip_data_s6sy792 *chip_info,
			      unsigned int ack)
{
	int rc = -1;
...
static int sec_flash_page_erase(struct chip_data_s6sy792 *chip_info,
...
static int sec_flash_page_write(struct chip_data_s6sy792 *chip_info,
...
static bool sec_limited_flash_page_write(struct chip_data_s6sy792 *chip_info,
					 u32 page_idx, u8 *page_data)
{
	int ret = -1;
	u8 *tCmd = NULL;
...
static int sec_flash_write(struct chip_data_s6sy792 *chip_info, u32 mem_addr,
...
static int sec_block_read(struct chip_data_s6sy792 *chip_info, u32 mem_addr,
			  int mem_size, u8 *buf)
{
	int ret;
	u8 cmd[5];
...
static int sec_memory_read(struct chip_data_s6sy792 *chip_info, u32 mem_addr,
			   u8 *mem_data, u32 mem_size)
{
	int ret;
	int retry = 3;
...
static int sec_chunk_update(struct chip_data_s6sy792 *chip_info, u32 addr,
			    u32 size, u8 *data)
{
	int ii = 0, ret = 0;
	u8 *mem_rb = NULL;
...
static void sec_set_kernel_grip_para(int para)
{
	...
	ret = touch_i2c_write_block(g_chip_info->client, SEC_CMD_GRIP_CONSTANT,
				    sizeof(grip_buf), grip_buf);
	if (ret < 0) {
		TPD_INFO("%s: failed to set grip constant value\n", __func__);
	} else {
		TPD_INFO("%s: set grip constant value : %d.\n", __func__,
			 g_chip_info->kernel_grip_para);
	}
			      ...
			      int is_suspended)
{
	...
	TPD_DEBUG(
		"first event: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
		...
		chip_info->first_event[8], chip_info->first_event[9],
		chip_info->first_event[10], chip_info->first_event[11]);
    ...
    */
	...
	if (event_id == SEC_STATUS_EVENT) {
		...
		if (left_event_cnt > MAX_EVENT_COUNT - 1) {
			TPD_INFO(
				"%s: event buffer overflow, do clear the buffer\n",
				__func__);
			ret = touch_i2c_write_block(chip_info->client,
						    SEC_CMD_CLEAR_EVENT_STACK,
						    0, NULL);
			if (ret < 0) {
				TPD_INFO("%s: clear event buffer failed\n",
					 __func__);
				...
				int max_num)
{
	...
	if ((t_id < max_num) &&
		...
		points[t_id].x =
			(p_event_coord->x_15_8 << 8) | (p_event_coord->x_7_0);
		points[t_id].y =
			(p_event_coord->y_15_8 << 8) | (p_event_coord->y_7_0);
	...
	for (i = 0; i < left_event; i++) {
		...
		if ((t_id < max_num) &&
		    ((p_event_coord->tchsta == SEC_COORDINATE_ACTION_PRESS) ||
		     (p_event_coord->tchsta == SEC_COORDINATE_ACTION_MOVE))) {
			points[t_id].x = (p_event_coord->x_15_8 << 8) |
					 (p_event_coord->x_7_0);
			points[t_id].y = (p_event_coord->y_15_8 << 8) |
					 (p_event_coord->y_7_0);
			points[t_id].z = p_event_coord->z & 0x3F;
			points[t_id].width_major = p_event_coord->major;
			points[t_id].touch_major = p_event_coord->major;
...
static int sec_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if (LEVEL_BASIC != tp_debug) {
		...
		for (i = 0; i < p_event_gesture->coordLen / 4; i++) {
			printk("(%d, %d) ",
			       (coord[4 * i + 1] << 8) | coord[4 * i],
			       (coord[4 * i + 3] << 8) | coord[4 * i + 2]);
		}
	}
				   ...
				   struct kernel_grip_info *grip_info)
{
	...
	list_for_each (pos, &grip_info->large_zone_list) {
		...
		for (i = 0; i < ARRAY_SIZE(sec_grip); i++) {
			if (!strncmp(grip_zone->name, sec_grip[i].name,
				     GRIP_TAG_SIZE)) {
				if (sec_grip[i].handle_func) {
					sec_grip[i].handle_func(grip_zone,
								true);
				}
			}
		}
...
static int sec_stop_filter_set(void *chip_data, int level, int usb_state)
{
	...
	ret = touch_i2c_write_block(chip_info->client,
				    SEC_CMD_REG_STOP_LOCK_POINT, sizeof(buf),
				    buf);
...
static int sec_smooth_lv_set(void *chip_data, int level)
{
	...
	if (chip_info->control_smooth_flag) {
		ret = touch_i2c_write_byte(chip_info->client, 0x90, level);
	} else {
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_REG_SMOOTH_LEVEL, level);
	}
...
static int sec_sensitive_lv_set(void *chip_data, int level)
{
	...
	buf[3] = level;
	if (chip_info->control_smooth_flag) {
		ret = touch_i2c_write_block(chip_info->client, 0x3F,
					    sizeof(buf), buf);
	} else {
		ret = touch_i2c_write_block(chip_info->client,
					    SEC_CMD_SENSETIVE_CTRL, sizeof(buf),
					    buf);
	}
	TPD_INFO("%s: set value: %d %s!\n", __func__, level,
		 ret < 0 ? "failed" : "success");
...
static void sec_set_gesture_state(void *chip_data, int state)
{
	...
	if (CHK_BIT(state, (1 << Up2DownSwip)) ||
	    CHK_BIT(state, (1 << DouSwip))) {
...
static int sec_read_self(struct chip_data_s6sy792 *chip_info, u8 type,
			 char *data, int len)
{
	int ret = 0;
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	if (chip_info->control_smooth_flag) {
		state = touch_i2c_read_byte(chip_info->client, 0x90);
		seq_printf(s, "smooth level state: 0x%02x\n", state);

		memset(buf, 0, 4);
		touch_i2c_read_block(chip_info->client, 0x3F, 4, buf);
		seq_printf(
			s,
			"sensetive level state: 0x%02x 0x%02x 0x%02x 0x%02x.\n",
			buf[0], buf[1], buf[2], buf[3]);
	} else {
		state = touch_i2c_read_byte(chip_info->client,
					    SEC_CMD_REG_SMOOTH_LEVEL);
		seq_printf(s, "smooth level state: 0x%02x\n", state);

		memset(buf, 0, 4);
		touch_i2c_read_block(chip_info->client, SEC_CMD_SENSETIVE_CTRL,
				     4, buf);
		seq_printf(
			s,
			"sensetive level state: 0x%02x 0x%02x 0x%02x 0x%02x.\n",
					...
					bool enable)
{
	...
	if (enable) {
		...
		} else if ((grip_zone->grip_side >> TYPE_SHORT_CORNER_SIDE) &
			   0x01) {
	...
	ret = touch_i2c_write_byte(g_chip_info->client, SEC_CMD_GRIP_DIRECTION,
				   0x00);
	ret |= touch_i2c_write_block(g_chip_info->client, SEC_CMD_GRIP_AREA,
				     sizeof(area_size), area_size);
	ret |= touch_i2c_write_byte(g_chip_info->client, SEC_CMD_GRIP_DIRECTION,
				    g_chip_info->touch_direction);
	TPD_DETAIL(
		"%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		__func__, SEC_CMD_GRIP_AREA, area_size[0], area_size[1],
		area_size[2], area_size[3], area_size[4]);
	TPD_DETAIL("%s: %s area size is %s %s in fw : [%d, %d] [%d %d].\n",
		   __func__, grip_zone->name, ret < 0 ? "failed" : "success",
		   enable ? "change" : "remove", grip_zone->start_x,
		   grip_zone->start_y, grip_zone->x_width, grip_zone->y_width);
					       ...
					       bool enable)
{
	...
	if (enable) {
		...
		ret = touch_i2c_write_block(g_chip_info->client,
					    SEC_CMD_GRIP_PARA, sizeof(exit_thd),
					    exit_thd);
		TPD_DETAIL("%s: cmd is : 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
			   __func__, SEC_CMD_GRIP_PARA, exit_thd[0],
			   exit_thd[1], exit_thd[2]);
		TPD_DETAIL("%s: %s exit thd is %s change in fw : %d.\n",
			   __func__, grip_zone->name,
			   ret < 0 ? "failed" : "success", grip_zone->exit_thd);
	}
...
static int sec_set_fw_grip_area(struct grip_zone_area *grip_zone, bool enable)
{
	...
	if (i == ARRAY_SIZE(sec_grip)) {
		...
		TPD_INFO("%s: %s %s in fw : [%d, %d] [%d %d] %d %d %d.\n",
			 __func__, grip_zone->name,
			 enable ? "modify" : "remove", grip_zone->start_x,
			 grip_zone->start_y, grip_zone->x_width,
			 grip_zone->y_width, grip_zone->exit_thd,
...
static int sec_set_no_handle_area(struct kernel_grip_info *grip_info)
{
	...
	ret = touch_i2c_write_block(g_chip_info->client, SEC_CMD_EDGE_SCREEN,
				    sizeof(edge_range), edge_range);
	TPD_DETAIL(
		"%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		__func__, SEC_CMD_EDGE_SCREEN, edge_range[0], edge_range[1],
		edge_range[2], edge_range[3]);
...
static int sec_set_condition_frame_limit(int frame_limit)
{
	...
	ret = touch_i2c_write_block(g_chip_info->client, SEC_CMD_GRIP_PARA,
				    sizeof(press_time), press_time);
	TPD_DETAIL("%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		   __func__, SEC_CMD_GRIP_PARA, press_time[0], press_time[1],
		   press_time[2]);
	TPD_DETAIL(
		"%s: Condition frame limit is %s change in fw : frame_limit = %d.\n",
...
static int sec_set_large_frame_limit(int frame_limit)
{
	...
	TPD_DETAIL("%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		   ...
		   press_time[2]);
	TPD_DETAIL(
		"%s: large frame limit is %s change in fw : frame_limit = %d.\n",
		__func__, ret < 0 ? "failed" : "success", frame_limit);
...
static int sec_set_large_corner_frame_limit(int frame_limit)
{
	...
	TPD_DETAIL("%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		   ...
		   press_time[2]);
	TPD_DETAIL(
		"%s: large corner frame limit is %s change in fw : frame_limit = %d.\n",
		__func__, ret < 0 ? "failed" : "success", frame_limit);
...
static int sec_set_large_thd(int large_thd)
{
	...
	TPD_DETAIL("%s: cmd write is : 0x%02x, 0x%02x, 0x%02x, 0x%02x.\n",
		   __func__, SEC_CMD_GRIP_PARA, touch_size[0], touch_size[1],
		   touch_size[2]);
...
static void sec_set_grip_touch_direction(uint8_t dir)
{
	...
	ret |= touch_i2c_write_byte(g_chip_info->client, SEC_CMD_WET_SWITCH,
				    !!g_chip_info->touch_direction);

	TPD_INFO("%s: set touch_direction: %d %s!\n", __func__,
		 g_chip_info->touch_direction, ret < 0 ? "failed" : "success");
}
...
static int sec_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:S6SY%03X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       0x792, fw_ver, chip_info->hw_res->TX_NUM,
			...
			const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(ts->dev->of_node, "sec_kernel_grip_para",
				   &chip_info->kernel_grip_para);
	if (ret) {
		TPD_INFO(
			"kernel_grip_para is not specified, using default value.\n");
	...
	chip_info->control_smooth_flag =
		of_property_read_bool(ts->dev->of_node, "control_smooth_flag");
