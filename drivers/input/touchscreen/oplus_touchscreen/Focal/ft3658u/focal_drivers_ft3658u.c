...
static int focal_dump_reg_state(void *chip_data, char *buf)
{
	...
	// 3 charge in
	regvalue =
		touch_i2c_read_byte(ts_data->client, FTS_REG_CHARGER_MODE_EN);
					 ...
					 int retries_delay)
{
	...
	for (i = 0; i < retries; i++) {
		...
		TPD_DEBUG("flash status fail,ok:%04x read:%04x, retries:%d",
...
static int fts_fwupg_enter_into_boot(struct fts_ts_data *ts_data)
{
	...
	do {
		/*reset to boot*/
		ret = touch_i2c_write_byte(ts_data->client, FTS_REG_UPGRADE,
					   FTS_UPGRADE_AA);
		if (ret < 0) {
...
static int fts_fwupg_erase(struct fts_ts_data *ts_data, u32 delay)
{
	...
	flag = fts_fwupg_check_flash_status(ts_data,
			       ...
			       u32 len, u32 delay)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		for (j = 0; j < FTS_RETRIES_WRITE; j++) {
			cmd = FTS_CMD_FLASH_STATUS;
			ret = touch_i2c_read_block(ts_data->client, cmd, 2,
						   val);
			read_status = (((u16)val[0]) << 8) + val[1];
...
int fts_fwupg_ecc_cal_tp(struct fts_ts_data *ts_data, u32 saddr, u32 len)
{
	...
	ret = touch_i2c_write_block(ts_data->client, wbuf[0] & 0xff, 6,
				    &wbuf[1]);
	if (ret < 0) {
	...
	bflag = fts_fwupg_check_flash_status(ts_data,
...
static int fts_enter_factory_work_mode(struct fts_ts_data *ts_data, u8 mode_val)
{
	...
	TPD_INFO("%s:enter %s mode", __func__,
		 (mode_val == 0x40) ? "factory" : "work");
			   ...
			   int data_type)
{
	...
	if (data_type == FTS_MUTUAL_CAP_DATA_DELTA) {
		byte_num =
			ts_data->hw_res->TX_NUM * ts_data->hw_res->RX_NUM * 2;
	} else {
		byte_num =
			(ts_data->hw_res->TX_NUM + ts_data->hw_res->RX_NUM) * 2;
	}
	...
	if (is_diff) {
		regval = touch_i2c_read_byte(ts_data->client,
					     FACTORY_REG_DATA_SELECT);
	...
	ret = touch_i2c_read_block(ts_data->client, raw_addr, MAX_PACKET_SIZE,
				   buf);
	...
	while (size > 0) {
		...
		ret = touch_i2c_read(ts_data->client, NULL, 0, buf + offset,
				     packet_len);
		if (ret < 0) {
...
reg_restore:
	if (is_diff) {
		ret = touch_i2c_write_byte(ts_data->client,
					   FACTORY_REG_DATA_SELECT, regval);
		if (ret < 0) {
...
static int fts_enable_black_gesture(struct fts_ts_data *ts_data, bool enable)
{
	...
	if (enable) {
		for (i = 0; i < 5; i++) {
			ret = touch_i2c_write_byte(ts_data->client,
						   FTS_REG_GESTURE_CONFIG1,
						   config1);
			ret = touch_i2c_write_byte(ts_data->client,
						   FTS_REG_GESTURE_CONFIG2,
						   config2);
			ret = touch_i2c_write_byte(ts_data->client,
						   FTS_REG_GESTURE_CONFIG4,
						   config4);
			ret = touch_i2c_write_byte(ts_data->client,
						   FTS_REG_GESTURE_EN, enable);
			msleep(1);
			ret = touch_i2c_read_byte(ts_data->client,
						  FTS_REG_GESTURE_EN);
			if (1 == ret)
				break;
		}
	} else {
		ret = touch_i2c_write_byte(ts_data->client, FTS_REG_GESTURE_EN,
					   enable);
	}
	if (i >= 5)
...
static int fts_enable_edge_limit(struct fts_ts_data *ts_data, bool enable)
{
	...
	return touch_i2c_write_byte(ts_data->client, FTS_REG_EDGE_LIMIT,
				    edge_mode);
}
...
static int fts_enable_charge_mode(struct fts_ts_data *ts_data, bool enable)
{
	...
	return touch_i2c_write_byte(ts_data->client, FTS_REG_CHARGER_MODE_EN,
				    enable);
}
...
static int fts_enable_headset_mode(struct fts_ts_data *ts_data, bool enable)
{
	...
	return touch_i2c_write_byte(ts_data->client, FTS_REG_HEADSET_MODE_EN,
				    enable);
}
...
static int fts_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_SLEEP:
		...
		ret = touch_i2c_write_byte(ts_data->client, FTS_REG_POWER_MODE,
					   0x03);
		if (ret < 0) {
	...
	case MODE_GESTURE:
		...
		if (ts_data->ts
			    ->is_suspended) { // do not pull up reset when doing resume
				     ...
				     bool force)
{
	...
	if (force || (buf[OFFSET_FW_DATA_FW_VER] != ts_data->fwver)) {
		TPD_INFO("Need update, force(%d)/fwver:Host(0x%02x),TP(0x%02x)",
			 force, buf[OFFSET_FW_DATA_FW_VER], ts_data->fwver);
		focal_esd_check_enable(ts_data, false);
...
static void fts_read_fod_info(struct fts_ts_data *ts_data)
{
	...
	ret = touch_i2c_read_block(ts_data->client, cmd, FTS_REG_FOD_INFO_LEN,
				   val);
	if (ret < 0) {
	...
	TPD_DEBUG("%s:FOD info buffer:%x %x %x %x %x %x %x %x %x", __func__,
				  ...
				  int is_suspended)
{
	...
	ret = touch_i2c_read_block(ts_data->client, cmd, FTS_POINTS_ONE,
				   &buf[0]);
	if (ret < 0) {
	...
	TPD_DEBUG(
		"%s, fgerprint, touched = %d, event_type = %d, fp_down = %d, fp_down_report = %d, ",
		__func__, ts_data->ts->view_area_touched,
		ts_data->fod_info.event_type, ts_data->fod_info.fp_down,
	...
	if (!is_suspended && ts_data->fp_en) {
		...
		if ((ts_data->fod_info.event_type == FTS_EVENT_FOD) &&
			...
			if (!ts_data->fod_info.fp_down_report) { // 38, 1, 0
				...
				TPD_DEBUG(
					"%s, fgerprint, set IRQ_FINGERPRINT when fger down but not reported! \n",
					__func__);
			}
			...
			if (ts_data->fod_info.fp_down_report) { // 38, 0, 1
				...
				SET_BIT(result_event, IRQ_FINGERPRINT);
				TPD_DEBUG(
					"%s, fgerprint, set IRQ_FINGERPRINT when fger up but still reported! \n",
					__func__);
			}
				...
				int max_num)
{
	...
	if (point_num > max_num) {
		TPD_INFO("invalid point_num(%d),max_num(%d)", point_num,
			 max_num);
		return -EIO;
	}
	...
	for (i = 0; i < max_num; i++) {
		...
		if (!ts_data->high_resolution_support &&
			...
			points[pointid].x = ((buf[2 + base] & 0x0F) << 8) +
					    (buf[3 + base] & 0xFF);
			points[pointid].y = ((buf[4 + base] & 0x0F) << 8) +
					    (buf[5 + base] & 0xFF);
...
static int fts_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(ts_data->client, cmd,
				   FTS_GESTURE_DATA_LEN - 2, &buf[2]);
	if (ret < 0) {
	...
	case GESTURE_FINGER_PRINT:
		...
		TPD_DEBUG(
			"%s, fgerprint, touched = %d, fp_down = %d, fp_down_report = %d, \n",
			__func__, ts_data->ts->view_area_touched,
			ts_data->fod_info.fp_down,
			ts_data->fod_info.fp_down_report);
		if (ts_data->fod_info.event_type == FTS_EVENT_FOD) {
...
static void fts_enable_fingerprint_underscreen(void *chip_data, uint32_t enable)
{
	...
	if (ret < 0) {
		TPD_INFO("%s: read FOD enable(%x) fail", __func__,
			 FTS_REG_FOD_EN);
		return;
	}
	...
	if (enable) {
		...
		if ((!ts_data->ts->view_area_touched) &&
			...
			ts_data->fod_info.fp_down_report = 0;
			TPD_DEBUG(
				"%s, fgerprint, fp_down_report status abnormal (notouch, 38!, 0, 1), needed to be reseted! \n",
				__func__);
		}
	} else {
		val &= 0xFD;
	...
	if (ret < 0) {
		TPD_INFO("%s: write FOD enable(%x=%x) fail", __func__,
			 FTS_REG_FOD_EN, val);
	}
}
...
static void fts_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	ret = touch_i2c_write_byte(ts_data->client, FTS_REG_GESTURE_CONFIG1,
				   config1);
	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG1 enable(%x=%x) fail",
			 __func__, FTS_REG_GESTURE_CONFIG1, config1);
	}
	ret = touch_i2c_write_byte(ts_data->client, FTS_REG_GESTURE_CONFIG2,
				   config2);
	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG2 enable(%x=%x) fail",
			 __func__, FTS_REG_GESTURE_CONFIG2, config2);
	}
	ret = touch_i2c_write_byte(ts_data->client, FTS_REG_GESTURE_CONFIG4,
				   config4);
	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG4 enable(%x=%x) fail",
			 __func__, FTS_REG_GESTURE_CONFIG4, config4);
	}

	msleep(1);
	TPD_INFO(
		"%s, enable[%d] register[FTS_REG_GESTURE_CONFIG1. FTS_REG_GESTURE_CONFIG2. FTS_REG_GESTURE_CONFIG4]",
		__func__, enable);
}
...
static int fts_smooth_lv_set(void *chip_data, int level)
{
	...
	return touch_i2c_write_byte(ts_data->client, FTS_REG_SMOOTH_LEVEL,
				    level);
}
...
static int fts_sensitive_lv_set(void *chip_data, int level)
{
	...
	return touch_i2c_write_byte(ts_data->client, FTS_REG_SENSITIVE_LEVEL,
				    level);
}
...
static int fts_set_high_frame_rate(void *chip_data, int level, int time)
{
	...
	if (level > 0) {
		TPD_INFO(
			"Enter high_frame mode, MODE_GAME, write 0xC3=%d, MODE_HIGH_FRAME write 0x8E=%d, HIGH_FRAME_TIME write 0x8A=%d",
			true, true, time);
		ret = touch_i2c_write_byte(ts_data->client,
					   FTS_REG_GAME_MODE_EN, true);
		if (ret < 0) {
			return ret;
		}
		ret = touch_i2c_write_byte(ts_data->client,
					   FTS_REG_HIGH_FRAME_EN, true);
		if (ret < 0) {
			return ret;
		}
		ret = touch_i2c_write_byte(ts_data->client,
					   FTS_REG_HIGH_FRAME_TIME, time);
		if (ret < 0) {
			return ret;
		}
	} else {
...
static void ft3658u_start_aging_test(void *chip_data)
{
	...
	if (ret < 0) {
		TPD_INFO("%s: enable(%x=%x) fail", __func__,
			 FTS_REG_GAME_MODE_EN, 2);
	}
	ret = touch_i2c_write_byte(ts_data->client, FTS_REG_POWER_MODE, 0);
	if (ret < 0) {
		TPD_INFO("%s: enable(%x=%x) fail", __func__, FTS_REG_POWER_MODE,
			 0);
	}
}
...
static int ft3658u_parse_dts(struct fts_ts_data *ts_data,
			     struct i2c_client *client)
{
	...
	ts_data->high_resolution_support =
		of_property_read_bool(np, "high_resolution_support");
	ts_data->high_resolution_support_x8 =
		of_property_read_bool(np, "high_resolution_support_x8");
