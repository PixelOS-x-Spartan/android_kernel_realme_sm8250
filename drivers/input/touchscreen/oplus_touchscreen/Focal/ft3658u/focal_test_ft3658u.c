...
static int fts_special_operation_for_samsung(struct fts_ts_data *ts_data)
{
	...
	if (true ==
		...
		if (ret < 0) {
			FTS_TEST_SAVE_ERR(
				"write FTS_REG_SAMSUNG_SPECIFAL fail at %s,ret=%d\n",
				__func__, ret);
			return -EIO;
		}
	}
		...
		if (ret >= 0) {
			...
			for (j = 0; j < 20; j++) {
				...
				if ((ret >= 0) && (FTS_FACTORY_MODE == mode)) {
					...
					sys_delay(200);
					fts_special_operation_for_samsung(
						ts_data);
					return 0;
				} else
					sys_delay(FACTORY_TEST_DELAY);
...
static int get_channel_num(struct fts_ts_data *ts_data)
{
	...
	if ((tx_num != ts_data->hw_res->TX_NUM) ||
		...
		FTS_TEST_SAVE_ERR(
			"channel num check fail, tx_num:%d-%d, rx_num:%d-%d\n",
			tx_num, ts_data->hw_res->TX_NUM, rx_num,
			ts_data->hw_res->RX_NUM);
		return -EIO;
	}
			  ...
			  bool rx_check, int *data, int *min, int *max)
{
	...
	if (tx_check) {
		for (i = rx_num; i < channel_num; i++) {
			...
			if ((data[i] < min[i]) || (data[i] > max[i])) {
				TPD_INFO(
					"tx check ERR [%d]: [%d] > [%d] > [%d] \n",
					i, max[i], data[i], min[i]);
				FTS_TEST_SAVE_INFO(
					"test fail,tx%d=%5d,range=(%5d,%5d)\n",
					i - rx_num + 1, data[i], min[i],
					max[i]);
				result = false;
			}
		}
...
static int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		ret = fts_test_read_reg(FACTROY_REG_SHORT2_TEST_STATE,
					&short_state);
		if ((ret >= 0) && (retval == short_state)) {
			break;
		} else
			TPD_DEBUG("reg%x=%x,retry:%d",
				  FACTROY_REG_SHORT2_TEST_STATE, short_state,
				  i);
	}
...
static int short_test_ch_to_all(struct fts_ts_data *ts_data, int *adc,
				u8 *ab_ch, int offset, bool *result)
{
	int ret = 0;
	int i = 0;
	int short_res[256] = { 0 };
...
static int fts_auto_preoperation(struct fts_ts_data *ts_data)
{
	...
	ts_data->noise_rawdata =
		(int *)kzalloc(node_num * sizeof(int), GFP_KERNEL);
	...
	ts_data->scap_rawdata = (int *)kzalloc(
		channel_num * NUM_MODE * sizeof(int), GFP_KERNEL);
	if (!ts_data->scap_rawdata) {
		FTS_TEST_SAVE_ERR("kzalloc for scap_rawdata fail\n");
		goto alloc_err;
	}

	ts_data->panel_differ_raw =
		(int *)kzalloc(node_num * sizeof(int), GFP_KERNEL);
	...
	ts_data->rawdata_linearity =
		kzalloc(node_num * 2 * sizeof(int), GFP_KERNEL);
	if (!ts_data->rawdata_linearity) {
		FTS_TEST_SAVE_ERR(
			"ts_data->rawdata_linearity buffer malloc fail\n");
		goto alloc_err;
	}
...
static int fts_rawdata_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	for (i = 0; i < 3; i++) {
		...
		rawdata_addr = FACTORY_REG_RAWDATA_ADDR_MC_SC;
		byte_num = node_num * 2;
		ret = read_rawdata(FACTORY_REG_LINE_ADDR, 0xAA, rawdata_addr,
				   byte_num, ts_data->rawdata);
		if (ret < 0) {
			FTS_TEST_SAVE_ERR("read rawdata fail\n");
		}
	}
				   ...
				   bool *test_result)
{
	...
	if (!ts_data->fts_autotest_offset->fts_uniformity_data_P ||
		...
		TPD_INFO(
			"fts_uniformity_data_P || fts_uniformity_data_N is NULL");
		return 0;
	}
	...
	if (!thr || !thr->node_valid || !ts_data->rawdata ||
	    !ts_data->rawdata_linearity) {
		FTS_TEST_SAVE_ERR(
			"node_valid/rawdata/rawdata_linearity is null\n");
		ret = -EINVAL;
		goto test_err;
	}
	...
	for (row = 0; row < tx_num; row++) {
		for (col = 0; col < rx_num - 1; col++) {
			...
			deviation = abs(ts_data->rawdata[offset] -
					ts_data->rawdata[offset2]);
			max = max(ts_data->rawdata[offset],
				  ts_data->rawdata[offset2]);
			max = max ? max : 1;
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((rl_tmp[i] <
			...
			TPD_INFO(
				"uniformity data ERR [%d]: [%d] > [%d] > [%d] \n",
				i,
				ts_data->fts_autotest_offset
					->fts_uniformity_data_P[i],
				rl_tmp[i],
				ts_data->fts_autotest_offset
					->fts_uniformity_data_N[i]);
			FTS_TEST_SAVE_ERR(
				"test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
...
static int fts_scap_cb_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->fts_autotest_offset->fts_scap_cb_data_P ||
		...
		TPD_INFO(
			"fts_scap_cb_data_P || fts_scap_cb_data_N || fts_scap_cb_data_waterproof_N || fts_scap_cb_data_waterproof_P is NULL");
		return 0;
	}
	...
	if (fw_wp_check) {
		...
		tmp_result =
			compare_mc_sc(ts_data, tx_check, rx_check, scb_tmp,
				      ts_data->fts_autotest_offset
					      ->fts_scap_cb_data_waterproof_N,
				      ts_data->fts_autotest_offset
					      ->fts_scap_cb_data_waterproof_P);
	...
	if (fw_wp_check) {
		...
		tmp2_result = compare_mc_sc(
			ts_data, tx_check, rx_check, scb_tmp,
			ts_data->fts_autotest_offset->fts_scap_cb_data_N,
...
test_err:
	if (tmp_result && tmp2_result) {
		*test_result = true;
		FTS_TEST_SAVE_INFO(
			"------Scap CB (normal && waterproof) Test PASS\n");
	} else {
		*test_result = false;
		if (tmp_result)
			FTS_TEST_SAVE_ERR(
				"------Scap CB Test (waterproof) NG\n");
		if (tmp2_result)
				     ...
				     bool *test_result)
{
	...
	if (!ts_data->fts_autotest_offset->fts_scap_raw_data_P ||
		...
		TPD_INFO(
			"fts_scap_raw_data_P || fts_scap_raw_data_N || fts_scap_raw_waterproof_data_N || fts_scap_raw_waterproof_data_P is NULL");
		return 0;
	}
	...
	if (fw_wp_check) {
		...
		rx_check = get_fw_wp(wc_sel, WATER_PROOF_ON_RX);
		tmp_result =
			compare_mc_sc(ts_data, tx_check, rx_check, srawdata_tmp,
				      ts_data->fts_autotest_offset
					      ->fts_scap_raw_waterproof_data_N,
				      ts_data->fts_autotest_offset
					      ->fts_scap_raw_waterproof_data_P);
	...
	if (fw_wp_check) {
		srawdata_tmp = ts_data->scap_rawdata + ts_data->srawdata_cnt;
		ret = get_rawdata_mc_sc(WATER_PROOF_OFF, byte_num,
					srawdata_tmp);
		if (ret < 0) {
		...
		rx_check = get_fw_wp(wc_sel, WATER_PROOF_OFF_RX);
		tmp2_result = compare_mc_sc(
			ts_data, tx_check, rx_check, srawdata_tmp,
			ts_data->fts_autotest_offset->fts_scap_raw_data_N,
...
static int fts_noise_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	ret = fts_test_write_reg(FACTORY_REG_FRAME_NUM,
				 (frame_num >> 8) & 0xFF);
	...
	if (ts_data->fts_autotest_offset->fts_noise_data_P &&
		...
		for (i = 0; i < node_num; i++) {
			...
			if (ts_data->noise_rawdata[i] >
				...
				TPD_INFO(
					"noise data ERR [%d]: [%d] > [%d] > [%d] \n",
					i,
					ts_data->fts_autotest_offset
						->fts_noise_data_P[i],
					ts_data->noise_rawdata[i],
					ts_data->fts_autotest_offset
						->fts_noise_data_N[i]);
				FTS_TEST_SAVE_ERR(
					"test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
...
static void fts_auto_write_result(struct fts_ts_data *ts_data, int failed_count)
{
	...
	ts_data->csv_fd =
		ksys_open(file_data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);
#else
	...
	ts_data->csv_fd =
		sys_open(file_data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);
	...
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Rawdata Test", 7, tx_num, rx_num, line_num, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);

	line_num += tx_num;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Rawdata Uniformity Test", 16, tx_num, rx_num,
			  line_num, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	line_num += tx_num;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
	...
	line_num += tx_num;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, rx_num, line_num, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, rx_num, line_num, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	...
	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Noise Test", 14, tx_num, rx_num, line_num, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);

	line_num += tx_num;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Null Noise", 41, 1, 1, line_num, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	...
	fts_test_save_data("Rawdata Uniformity Test",
			   ts_data->rawdata_linearity, ts_data->rl_cnt, rx_num,
			   ts_data->csv_fd);
	...
	fts_test_save_data("SCAP CB Test", ts_data->scap_cb + channel_num,
			   ts_data->scb_cnt - channel_num, rx_num,
			   ts_data->csv_fd);
	//    fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata, ts_data->srawdata_cnt, channel_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata,
			   channel_num, rx_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test",
			   ts_data->scap_rawdata + channel_num,
			   ts_data->srawdata_cnt - channel_num, rx_num,
			   ts_data->csv_fd);
	fts_test_save_data("Noise Test", ts_data->noise_rawdata, node_num,
			   rx_num, ts_data->csv_fd);
	TPD_INFO("null_noise_max:%d", ts_data->null_noise_max);
	fts_test_save_data("Null Noise", &ts_data->null_noise_max, 1, 1,
			   ts_data->csv_fd);
...
static int fts_threshold_malloc(struct fts_ts_data *ts_data)
{
	...
	thr->node_valid_sc =
		(int *)kzalloc(channel_num * sizeof(int), GFP_KERNEL);
...
static int fts_get_threshold(struct fts_ts_data *ts_data, char *data)
{
	...
	if (!data) {
		...
		TPD_INFO("scap_cb_on_max = [%d] scap_cb_on_min = [%d] \n", 490,
			 0);
		TPD_INFO(
			"scap_rawdata_off_max = [%d] scap_rawdata_off_min = [%d] \n",
			15000, 3000);
		TPD_INFO(
			"scap_rawdata_on_max = [%d] scap_rawdata_on_min = [%d] \n",
			15000, 3000);
	}
				      ...
				      const struct firmware *limit_fw)
{
	...
	ts_data->fts_autotest_offset =
		kzalloc(sizeof(struct fts_autotest_offset), GFP_KERNEL);

	ret = request_firmware(&limit_fw, ts->panel_data.test_limit_name,
			       &ts_data->client->dev);
	...
	for (i = 0; i < item_cnt; i++) {
		TPD_INFO("%s: i[%d] \n", __func__, i);
		item_head = (struct auto_test_item_header *)(limit_fw->data +
							     p_item_offset[i]);
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			TPD_INFO(
				"[%d] incorrect item type: LIMIT_TYPE_NO_DATA\n",
				item_head->item_bit);
		} else if (item_head->item_limit_type ==
			   LIMIT_TYPE_TOP_FLOOR_DATA) {
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_NOISE_DATA) {
					...
					ts_data->fts_autotest_offset
					   ...
					   TYPE_UNIFORMITY_DATA) {
					ts_data->fts_autotest_offset
						->fts_uniformity_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->fts_autotest_offset
						->fts_uniformity_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
				} else if (item_head->item_bit ==
					   TYPE_PANEL_DIFFER_DATA) {
					ts_data->fts_autotest_offset
						->fts_panel_differ_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->fts_autotest_offset
						->fts_panel_differ_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
				}
				...
				if (item_head->item_bit ==
					...
					ts_data->fts_autotest_offset
					   ...
					   TYPE_FACTORY_PANEL_DIFFER_DATA) {
					ts_data->fts_autotest_offset
						->fts_panel_differ_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
			   ...
			   LIMIT_TYPE_TOP_FLOOR_RX_TX_DATA) {
			if (false == ts_data->use_panelfactory_limit) {
				TPD_INFO("test item bit [%d] \n",
					 item_head->item_bit);
				...
				if (item_head->item_bit ==
					...
					ts_data->fts_autotest_offset
					   ...
					   TYPE_FACTORY_SCAP_CB_WATERPROOF_DATA) {
					ts_data->fts_autotest_offset
						->fts_scap_cb_data_waterproof_P =
						(int32_t *)(limit_fw->data +
							    item_head
					...
					ts_data->fts_autotest_offset
					   ...
					   TYPE_FACTORY_SCAP_RAW_WATERPROOF_DATA) {
					ts_data->fts_autotest_offset
						->fts_scap_raw_waterproof_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
...
static void fts_print_threshold(struct fts_ts_data *ts_data)
{
	...
	print_buffer(ts_data->fts_autotest_offset->fts_noise_data_P, node_num,
		     rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_noise_data_N, node_num,
		     rx_num);

	TPD_INFO("rawdata threshold max/min:");
	print_buffer(ts_data->fts_autotest_offset->fts_raw_data_P, node_num,
		     rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_raw_data_N, node_num,
		     rx_num);

	TPD_INFO("uniformity threshold max/min:");
	print_buffer(ts_data->fts_autotest_offset->fts_uniformity_data_P,
		     node_num, rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_uniformity_data_N,
		     node_num, rx_num);

	TPD_INFO("scap cb normal threshold max/min:");
	print_buffer(ts_data->fts_autotest_offset->fts_scap_cb_data_P,
		     channel_num, channel_num);
	print_buffer(ts_data->fts_autotest_offset->fts_scap_cb_data_N,
		     channel_num, channel_num);
	...
	print_buffer(ts_data->fts_autotest_offset->fts_scap_raw_data_P,
		     channel_num, channel_num);
	print_buffer(ts_data->fts_autotest_offset->fts_scap_raw_data_N,
		     channel_num, channel_num);
...
int fts_test_entry(struct fts_ts_data *ts_data)
{
	...
	FTS_TEST_SAVE_ERR("FW_VER:0x%02x, TX_NUM:%d, RX_NUM:%d\n",
			  ts_data->fwver, ts_data->hw_res->TX_NUM,
			  ts_data->hw_res->RX_NUM);
