				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	struct focal_debug_func *focal_debug_ops =
		(struct focal_debug_func *)ts->private_data;
				  ...
				  const char *buf, size_t count)
{
	...
	if (2 == num_read_chars) {
		...
		g_rwreg_result.value = regvalue;
		g_rwreg_result.result =
			touch_i2c_write_byte(ts->client, regaddr, regvalue);
		if (g_rwreg_result.result < 0) {
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (ts->esd_handle_support) {
		if (focal_debug_ops && focal_debug_ops->get_esd_check_flag) {
			count = sprintf(
				buf,
				"esd_running_flag: %d,  esd_check_need_stop: %d\n",
				ts->esd_info.esd_running_flag,
				focal_debug_ops->get_esd_check_flag(
					ts->chip_data));
		} else {
...
static struct attribute *focal_attributes[] = { &dev_attr_fts_fw_version.attr,
				 ...
				 size_t count, loff_t *ppos)
{
	...
	case PROC_WRITE_DATA:
		writelen = count - 1;
		if (writelen > 0) {
			ret = touch_i2c_write(ts->client, writebuf + 1,
					      writelen);
			if (ret < 0) {
				TPD_INFO("[APK]: write iic error!\n");
			}
				...
				size_t count, loff_t *ppos)
{
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			if (ts->esd_handle_support) {
				if (focal_debug_ops &&
				    focal_debug_ops->esd_check_enable) {
					focal_debug_ops->esd_check_enable(
						ts->chip_data, true);
				}
			}
			       ...
			       size_t count, loff_t *ppos)
{
	...
	case PROC_WRITE_REGISTER:
		ret = touch_i2c_write_byte(ts->client, writebuf[1],
					   writebuf[2]);
		if (ret < 0) {
	...
	case PROC_HW_RESET:
		...
		if (strncmp(tmp, "focal_driver", 12) == 0) {
			...
			if (ts->ts_ops->reset_gpio_control) {
				ts->ts_ops->reset_gpio_control(ts->chip_data,
							       0);
				msleep(1);
				ts->ts_ops->reset_gpio_control(ts->chip_data,
							       1);
			      ...
			      size_t count, loff_t *ppos)
{
	...
	case PROC_READ_REGISTER:
		num_read_chars = 1;
		ret = touch_i2c_read(ts->client, &proc.cmd[0], 1, &readbuf[0],
				     num_read_chars);
		if (ret < 0) {
	...
	case PROC_READ_DATA:
		...
		ret = touch_i2c_read(ts->client, NULL, 0, readbuf,
				     num_read_chars);
		if (ret < 0) {
		    ...
		    struct fts_proc_operations *syna_ops)
{
	...
	proc.proc_entry =
		proc_create_data(PROC_NAME, 0777, NULL, &ftxxxx_proc_fops, ts);
...
void ft_limit_read_std(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	for (m = 0; m < item_cnt; m++) {
		TPD_INFO("common debug d: p_item_offset[%d] = 0x%x \n", m,
			 p_item_offset[m]);
		item_head = (struct auto_test_item_header *)(fw->data +
							     p_item_offset[m]);
		...
		if (item_head->item_limit_type ==
			...
			TPD_INFO("size 1: %d * %d = %d \n", NUM_MODE,
				 num_panel_node, (NUM_MODE * num_panel_node));
			if (p_data32) {
