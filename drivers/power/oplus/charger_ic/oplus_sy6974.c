...
static int sy6974_input_current_limit_write(int value)
{
	...
	if (value <= 0) {
		...
		rc = sy6974_config_interface(chip, REG00_SY6974_ADDRESS, reg_val,
...
static int sy6974_charging_current_write_fast(int chg_cur)
{
	...
	if (chg_cur > REG02_SY6974_MAX_FAST_CHG_CURRENT_MA || chg_cur < REG02_SY6974_MIN_FAST_CHG_CURRENT_MA)
	...
	rc = sy6974_config_interface(chip, REG02_SY6974_ADDRESS, value, REG02_SY6974_FAST_CHG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
static int sy6974_set_vindpm_vol(int vol)
{
	...
	rc = sy6974_config_interface(chip, REG06_SY6974_ADDRESS, value, REG06_SY6974_VINDPM_MASK);
	return rc;
}
...
static int sy6974_set_prechg_current(int ipre_ma)
{
	...
	return sy6974_config_interface(chip, REG03_SY6974_ADDRESS, value, REG03_SY6974_PRE_CHG_CURRENT_LIMIT_MASK);
}
...
static int sy6974_set_termchg_current(int term_curr)
{
	...
	rc = sy6974_config_interface(chip, REG03_SY6974_ADDRESS, value, REG03_SY6974_TERM_CHG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
static int sy6974_set_wdt_timer(int reg)
{
	...
	rc = sy6974_config_interface(chip, REG05_SY6974_ADDRESS, reg, REG05_SY6974_WATCHDOG_TIMER_MASK);
...
static int sy6974_set_chging_term_disable(void)
{
	...
	rc = sy6974_config_interface(chip, REG05_SY6974_ADDRESS, REG05_SY6974_TERMINATION_DISABLE,
...
static int sy6974_kick_wdt(void)
{
	...
	rc = sy6974_config_interface(chip, REG01_SY6974_ADDRESS, REG01_SY6974_WDT_TIMER_RESET,
...
static int sy6974_enable_charging(void)
{
	...
	rc = sy6974_config_interface(chip, REG01_SY6974_ADDRESS, REG01_SY6974_CHARGING_ENABLE,
...
static int sy6974_disable_charging(void)
{
	...
	rc = sy6974_config_interface(chip, REG01_SY6974_ADDRESS, REG01_SY6974_CHARGING_DISABLE,
				     REG01_SY6974_CHARGING_MASK);
	if (rc < 0) {
...
static bool sy6974_check_charger_suspend_enable(void)
{
	...
	charger_suspend_enable =
		((reg_val & REG00_SY6974_SUSPEND_MODE_MASK) == REG00_SY6974_SUSPEND_MODE_ENABLE) ? true : false;
...
int sy6974_otg_enable(void)
{
	...
	sy6974_config_interface(chip, REG00_SY6974_ADDRESS, REG00_SY6974_SUSPEND_MODE_DISABLE,
				REG00_SY6974_SUSPEND_MODE_MASK);

	rc = sy6974_config_interface(chip, REG01_SY6974_ADDRESS, REG01_SY6974_OTG_ENABLE, REG01_SY6974_OTG_MASK);
	if (rc) {
...
int sy6974_otg_disable(void)
{
	...
	rc = sy6974_config_interface(chip, REG01_SY6974_ADDRESS, REG01_SY6974_OTG_DISABLE, REG01_SY6974_OTG_MASK);
	if (rc)
...
static int sy6974_reset_charger(void)
{
	...
	rc = sy6974_config_interface(chip, REG0B_SY6974_ADDRESS, REG0B_SY6974_REG_RST_RESET, REG0B_SY6974_REG_RST_MASK);
	if (rc < 0) {
...
enum charger_type mt_charger_type_detection_sy6974(void)
{
	...
	sy6974_config_interface(charger_ic, REG07_SY6974_ADDRESS, REG07_SY6974_IINDET_EN_FORCE_DET,
	...
	if (val_buf == REG08_SY6974_VBUS_STAT_UNKNOWN) {
		MTK_CHR_Type_num = CHARGER_UNKNOWN;
	} else if (val_buf == REG08_SY6974_VBUS_STAT_SDP || val_buf == REG08_SY6974_VBUS_STAT_CDP) {
		MTK_CHR_Type_num = STANDARD_HOST;
	} else if (val_buf == REG08_SY6974_VBUS_STAT_DCP || val_buf == REG08_SY6974_VBUS_STAT_OCP ||
...
static int sy6974_irq_registration(void)
{
	...
	ret = request_threaded_irq(gpio_to_irq(chip->irq_gpio), NULL, sy6974_irq_handler_fn,
				   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "SY6974-eint", chip);
	if (ret < 0) {
