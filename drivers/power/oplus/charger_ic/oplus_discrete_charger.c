...
static int oplus_get_iio_channel(struct smb_charger *chg, const char *propname, struct iio_channel **chan)
{
	int rc = 0;
...
static void oplus_parse_dt_adc_channels(struct work_struct *work)
{
	...
	int rc = 0;
	struct smb_charger *chg = container_of(work, struct smb_charger, parse_dt_adc_channels_work.work);
...
int oplus_chg_get_shutdown_soc(void)
{
	...
	if (chg->soc_backup_nvmem) {
		...
		if (((soc & SOC_VALID_MASK) == 0) || (soc & SOC_VALUE_MASK) > FULL_SOC) {
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, keep_vbus_work.work);
...
bool oplus_usbtemp_condition(void)
{
	...
	if (oplus_ccdetect_check_is_gpio(g_oplus_chip)) {
		level = gpio_get_value(chg->ccdetect_gpio);
		if (level == 1 || tcpm_inquire_typec_attach_state(chg->tcpc) == TYPEC_ATTACHED_AUDIO ||
...
static void oplus_pdo_select(int vbus_mv, int ibus_ma)
{
	...
	if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		while (1) {
			ret = tcpm_inquire_pd_source_apdo(chg->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS, &cap_i, &apdo_cap);
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->dual_charger_support) {
		if ((chip->charger_volt > VBUS_9V_THR_MV && chip->soc >= SOC_9V_THR &&
		...
		} else if (chip->charger_volt < VBUS_5V_THR_MV && chip->soc < SOC_9V_THR &&
			...
			printk(KERN_ERR "%s: vbus[%d], ibus[%d], ret[%d]\n", __func__, PDO_9V_VBUS_MV, PDO_9V_IBUS_MA,
			       ret);
		} else {
...
static int oplus_discrete_usb_get_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int oplus_discrete_usb_set_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int oplus_discrete_usb_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
	default:
		break;
...
static int oplus_discrete_ac_get_property(struct power_supply *psy, enum power_supply_property psp,
					  union power_supply_propval *val)
{
	int rc = 0;
...
static int oplus_discrete_batt_get_prop(struct power_supply *psy, enum power_supply_property psp,
					union power_supply_propval *val)
{
	int rc = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static int oplus_discrete_batt_set_prop(struct power_supply *psy, enum power_supply_property psp,
					const union power_supply_propval *val)
{
	return oplus_battery_set_property(psy, psp, val);
}

static int oplus_discrete_batt_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
...
static int oplus_discrete_iio_init(struct smb5 *chip, struct platform_device *pdev, struct iio_dev *indio_dev)
{
...
{
	if (g_oplus_chip && g_oplus_chip->sub_chg_ops && g_oplus_chip->sub_chg_ops->get_charger_current) {
...
static int discrete_charger_probe(struct platform_device *pdev)
{
	...
	chip->iio_chans = devm_kcalloc(&pdev->dev, chip->nchannels, sizeof(*chip->iio_chans), GFP_KERNEL);
	if (!chip->iio_chans)
		return -ENOMEM;

	chip->iio_chan_ids = devm_kcalloc(&pdev->dev, chip->nchannels, sizeof(*chip->iio_chan_ids), GFP_KERNEL);
	...
	if (of_find_property(chg->dev->of_node, "nvmem-cells", NULL)) {
		chg->soc_backup_nvmem = devm_nvmem_cell_get(chg->dev, "oplus_soc_backup");
	...
	if (!chg->tcpc) {
		if (get_tcpc_count < GET_TCPC_CNT_MAX) {
			g_oplus_chip =
				NULL; /*Set g_oplus_chip to NULL after  discrete_charger Probe fail to avoid wild pointer */
