...
static int oplus_mos_ctrl_parse_dt(struct oplus_mos_dev *dev)
{
	...
	rc = of_property_read_u32(node, "qcom,parallel_vbat_gap_abnormal",
				  &dev->switching_chip->parallel_vbat_gap_abnormal);
	if (rc) {
...
static int oplus_mos_ctrl_set_enable(int enable)
{
	...
	if (!g_mos_dev->mos_en_pinctrl || !g_mos_dev->mos_en_gpio_active || !g_mos_dev->mos_en_gpio_sleep) {
...
static int oplus_mos_ctrl_probe(struct platform_device *pdev)
{
	...
	g_mos_dev->switching_chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_switch_chip), GFP_KERNEL);
