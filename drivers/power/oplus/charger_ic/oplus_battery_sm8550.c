...
static const char *const qc_power_supply_wls_type_text[] = { "Unknown", "BPP", "EPP", "HPP" };
...
static int oem_pmic_buffer_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	rc = pmic_glink_write(bcdev->client, data, len);
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->read_pmic_buffer_ack, msecs_to_jiffies(OEM_READ_WAIT_TIME_MS));
		if (!rc) {
			pr_err("Error, timed out sending message\n");
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_OEM_GLINK_ABNORMAL);
...
static void handle_oem_read_pmic_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg,
					size_t len)
{
	u32 buf_len;

	if (len > sizeof(bcdev->read_pmic_buffer_dump)) {
		pr_err("Incorrect length received: %zu expected: %u\n", len, sizeof(bcdev->read_pmic_buffer_dump));
		goto out;
	}
...
static int wls_bcc_get_max_curr(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_WLS_BCC_MAX, &pval);
	if (rc < 0)
...
static int wls_bcc_get_min_curr(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_WLS_BCC_MIN, &pval);
	if (rc < 0)
...
static int wls_bcc_get_stop_curr(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_WLS_BCC_STOP, &pval);
	if (rc < 0)
...
static void handle_bcc_read_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg, size_t len)
{
	...
	if (oplus_chg_wls_support_bcc(g_oplus_chip) && oplus_chg_is_wls_present() &&
	    oplus_chg_is_wls_fastchg_started()) {
		bcdev->bcc_read_buffer_dump.data_buffer[15] = 1;
...
static void oplus_adsp_voocphy_enable_check_func(struct work_struct *work)
{
	...
	if (chip->mmi_chg == 0 || chip->charger_exist == false || chip->charger_type != POWER_SUPPLY_TYPE_USB_DCP ||
	    bcdev->qc_enable_status == 1 || oplus_get_quirks_plug_status(QUIRKS_STOP_ADSP_VOOCPHY)) {
...
static bool oplus_chg_wls_support_bcc(struct oplus_chg_chip *chip)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_BCC_SUPPORT, &temp_val);
	if (rc < 0)
		support = false;
	else
...
static ssize_t wireless_type_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_WLS];
	int rc;
...
out:
	schedule_delayed_work(&bcdev->adspvoocphy_plugin_cnt_check_work, OPLUS_PLUGIN_CNT_DETECT_INTERVAL);
}
	...
	while (!kthread_should_stop()) {
		...
		if (curr_range_change == false && chip->usbtemp_batt_current < 5000 &&
			...
			chg_err("reconnected when hi_current, need keep %d seconds",
				now_time.tv_sec - pre_hi_current_time.tv_sec);
	...
	dischg:
		...
		if (log_count == 40) {
			...
			chg_err("usbtemp current status = %d [%d, %d, %d, %d]\n", chip->usbtemp_curr_status,
				usbtemp_dbg_curr_status, curr_range_change, chip->usbtemp_batt_current,
				chip->usbtemp_pre_batt_current);
			log_count = 0;
		}
	}
...
static int smbchg_kick_wdt(void)
{
	...
	if (chip->fg_info_package_read_support || !smbchg_get_charge_enable())
		return 0;
...
static void oplus_plugin_irq_work(struct work_struct *work)
{
	...
	chg_err("!!!prop[%d], usb_online[%d] abnormal_adapter_info [%d %d] cid_status[%d]\n", pst->prop[USB_IN_STATUS],
		bcdev->usb_in_status, chip->is_abnormal_adapter, chip->support_abnormal_adapter, cid_status);
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		if (oplus_chg_get_adsp_pre_is_abnormal_adapter()) {
			bcdev->abnormal_adapter_disconnect_cnt = oplus_chg_get_adsp_abnormal_adapter_disconnect_cnt();
			chg_err("adspvoocphy abnormal_adapter_info [%d]", bcdev->abnormal_adapter_disconnect_cnt);
		}
	}
	...
	if (bcdev->usb_ocm) {
		if (bcdev->usb_in_status == 1) {
			...
			if (is_charger_boot_mode() && g_oplus_chip->prop_status == POWER_SUPPLY_STATUS_CHARGING) {
	...
	if (is_wls_ocm_available(chip) && usb_pre_plugin_status != usb_plugin_status) {
		(void)oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_TRX_ONLINE, &temp_val);
		if (!!temp_val.intval) {
...
int oplus_chg_set_pd_config(void)
{
	...
	if ((chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > HIGH_CHARGER_VOL_THR &&
...
static int oplus_chg_track_debugfs_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->debug_pmic_glink_err = 0;
	debugfs_create_u32("debug_force_icl_err", 0644, debugfs_pm8550b, &(bcdev->debug_force_icl_err));
	debugfs_create_u32("debug_pmic_glink_err", 0644, debugfs_pm8550b, &(bcdev->debug_pmic_glink_err));
