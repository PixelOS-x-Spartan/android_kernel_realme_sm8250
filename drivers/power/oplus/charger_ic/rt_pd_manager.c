...
static const char *const iio_channel_map[] = {
	"pd_active",	  "pd_usb_suspend_supported", "pd_in_hard_reset", "pd_current_max",
	"pd_voltage_min", "pd_voltage_max",	      "real_type",	  "pd_sdp",
};
...
static int smblib_get_prop(struct rt_pd_manager_data *rpmd, enum iio_psy_property ipp, union power_supply_propval *val)
{
	int ret = 0, value = 0;
...
static int smblib_set_prop(struct rt_pd_manager_data *rpmd, enum iio_psy_property ipp,
...
#else
static inline int smblib_get_prop(struct rt_pd_manager_data *rpmd, enum power_supply_property psp,
				  union power_supply_propval *val)
{
	return power_supply_get_property(rpmd->usb_psy, psp, val);
}

static inline int smblib_set_prop(struct rt_pd_manager_data *rpmd, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
...
static int extcon_init(struct rt_pd_manager_data *rpmd)
{
	...
	extcon_set_property_capability(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_SS);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
	extcon_set_property_capability(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT);
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) */
	extcon_set_property_capability(rpmd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(rpmd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS);
out:
	return ret;
}
...
static inline void start_usb_host(struct rt_pd_manager_data *rpmd)
{
	...
	extcon_set_property(rpmd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY, val);

	val.intval = 1;
	extcon_set_property(rpmd->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS, val);
...
static inline void start_usb_peripheral(struct rt_pd_manager_data *rpmd)
{
	...
	dev_err(rpmd->dev, "%s\n", __func__);
	val.intval = tcpm_inquire_cc_polarity(rpmd->tcpc);
	extcon_set_property(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY, val);
	...
	extcon_set_property(rpmd->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT, val);
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) */
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	struct rt_pd_manager_data *rpmd = container_of(dwork, struct rt_pd_manager_data, usb_dwork);
	...
	case DR_DEVICE:
		...
		if (val.intval == POWER_SUPPLY_TYPE_UNKNOWN) {
			if (rpmd->usb_type_polling_cnt < USB_TYPE_POLLING_CNT_MAX)
				schedule_delayed_work(&rpmd->usb_dwork, msecs_to_jiffies(USB_TYPE_POLLING_INTERVAL));
			break;
		} else if (val.intval == POWER_SUPPLY_TYPE_USB || val.intval == POWER_SUPPLY_TYPE_USB_CDP) {
			pr_err("%s start usb\n", __func__);
			stop_usb_host(rpmd);
...
static void pd_sink_set_vol_and_cur(struct rt_pd_manager_data *rpmd, int mv, int ma, uint8_t type)
{
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct rt_pd_manager_data *rpmd = container_of(nb, struct rt_pd_manager_data, pd_nb);
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if ((rpmd->sink_mv_new != rpmd->sink_mv_old) || (rpmd->sink_ma_new != rpmd->sink_ma_old)) {
		...
		if (noti->vbus_state.type & TCP_VBUS_CTRL_PD_DETECT)
			pd_sink_set_vol_and_cur(rpmd, rpmd->sink_mv_new, rpmd->sink_ma_new, noti->vbus_state.type);
		break;
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			smblib_set_prop(rpmd, POWER_SUPPLY_PROP_PD_USB_SUSPEND_SUPPORTED, &val);
			smblib_set_prop(rpmd, POWER_SUPPLY_PROP_PD_ACTIVE, &val);
			...
			memset(&rpmd->partner_identity, 0, sizeof(rpmd->partner_identity));
			rpmd->partner_desc.usb_pd = false;
			switch (new_state) {
			case TYPEC_ATTACHED_AUDIO:
				rpmd->partner_desc.accessory = TYPEC_ACCESSORY_AUDIO;
				break;
			case TYPEC_ATTACHED_DEBUG:
			case TYPEC_ATTACHED_DBGACC_SNK:
			case TYPEC_ATTACHED_CUSTOM_SRC:
				rpmd->partner_desc.accessory = TYPEC_ACCESSORY_DEBUG;
				break;
			default:
				rpmd->partner_desc.accessory = TYPEC_ACCESSORY_NONE;
				break;
			}
			rpmd->partner = typec_register_partner(rpmd->typec_port, &rpmd->partner_desc);
	...
	case TCP_NOTIFY_PR_SWAP:
		...
		if (noti->swap_state.new_role == PD_ROLE_SINK) {
			...
			if (g_oplus_chip && g_oplus_chip->chg_ops && g_oplus_chip->chg_ops->set_prswap)
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			if (g_oplus_chip && g_oplus_chip->chg_ops && g_oplus_chip->chg_ops->get_charger_type &&
...
{
	struct rt_pd_manager_data *rpmd = container_of(cap, struct rt_pd_manager_data, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	rpmd->iio_channels =
		devm_kcalloc(rpmd->dev, POWER_SUPPLY_IIO_PROP_MAX, sizeof(rpmd->iio_channels[0]), GFP_KERNEL);
	...
	for (i = 0; i < POWER_SUPPLY_IIO_PROP_MAX; i++) {
		rpmd->iio_channels[i] = devm_iio_channel_get(rpmd->dev, iio_channel_map[i]);
	...
	ret = register_tcp_dev_notifier(rpmd->tcpc, &rpmd->pd_nb, TCP_NOTIFY_TYPE_ALL);
	if (ret < 0) {
...
out:
	...
	dev_info(rpmd->dev, "%s %s!!\n", __func__, ret == -EPROBE_DEFER ? "Over probe cnt max" : "OK");
	return 0;
...
static int rt_pd_manager_remove(struct platform_device *pdev)
{
	...
	ret = unregister_tcp_dev_notifier(rpmd->tcpc, &rpmd->pd_nb, TCP_NOTIFY_TYPE_ALL);
	if (ret < 0)
...
static const struct of_device_id rt_pd_manager_of_match[] = { { .compatible = "richtek,rt-pd-manager" }, {} };
