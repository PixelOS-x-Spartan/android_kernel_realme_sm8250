...
module_param_named(weak_chg_icl_ua, __weak_chg_icl_ua, int, 0600);

static int __try_sink_enabled = 1;
module_param_named(try_sink_enabled, __try_sink_enabled, int, 0600);

static int __audio_headset_drp_wait_ms = 100;
module_param_named(audio_headset_drp_wait_ms, __audio_headset_drp_wait_ms, int, 0600);
...
int smblib_icl_override(struct smb_charger *chg, bool override)
{
	...
	rc = smblib_masked_write(chg, USBIN_LOAD_CFG_REG, ICL_OVERRIDE_AFTER_APSD_BIT,
...
int smblib_stat_sw_override_cfg(struct smb_charger *chg, bool override)
{
	...
	rc = smblib_masked_write(chg, STAT_CFG_REG, STAT_SW_OVERRIDE_CFG_BIT, override ? STAT_SW_OVERRIDE_CFG_BIT : 0);
	if (rc < 0) {
...
static int chg_freq_list[] = {
	9600, 9600, 6400, 4800, 3800, 3200, 2700, 2400, 2100, 1900, 1700, 1600, 1500, 1400, 1300, 1200,
};
...
static int smblib_set_adapter_allowance(struct smb_charger *chg, u8 allowed_voltage)
{
	int rc = 0;
...
static void smblib_rerun_apsd(struct smb_charger *chg)
{
	...
	if (chg->wa_flags & QC_AUTH_INTERRUPT_WA_BIT) {
		rc = smblib_masked_write(chg, USBIN_SOURCE_CHANGE_INTRPT_ENB_REG, AUTH_IRQ_EN_CFG_BIT,
					 AUTH_IRQ_EN_CFG_BIT);
		if (rc < 0)
...
static void smblib_uusb_removal(struct smb_charger *chg)
{
	...
	rc = smblib_set_adapter_allowance(chg, USBIN_ADAPTER_ALLOW_5V_OR_9V_TO_12V);
	if (rc < 0)
...
void smblib_suspend_on_debug_battery(struct smb_charger *chg)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_DEBUG_BATTERY, &val);
	if (rc < 0) {
...
static int smblib_get_hw_pulse_cnt(struct smb_charger *chg, int *count)
{
	...
	case PMI8998_SUBTYPE:
		...
		*count = val[0] & QC_PULSE_COUNT_MASK;
		break;
	case PM660_SUBTYPE:
		rc = smblib_multibyte_read(chg, QC_PULSE_COUNT_STATUS_1_REG, val, 2);
		if (rc) {
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	/* configure current */
	if (chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT && (chg->real_charger_type == POWER_SUPPLY_TYPE_USB)) {
...
override_suspend_config:
	...
	if (icl_ua == INT_MAX) {
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB)
			...
			override = false;
		else if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_CDP && icl_ua == 1500000)
			/*
	...
	/* enforce override */
	rc = smblib_masked_write(chg, USBIN_ICL_OPTIONS_REG, USBIN_MODE_CHG_BIT, override ? USBIN_MODE_CHG_BIT : 0);
...
int smblib_toggle_stat(struct smb_charger *chg, int reset)
{
	...
	if (reset) {
		rc = smblib_masked_write(chg, STAT_CFG_REG, STAT_SW_OVERRIDE_CFG_BIT | STAT_SW_OVERRIDE_VALUE_BIT,
		...
		rc = smblib_masked_write(chg, STAT_CFG_REG, STAT_SW_OVERRIDE_CFG_BIT | STAT_SW_OVERRIDE_VALUE_BIT, 0);
		if (rc < 0) {
...
static int smblib_micro_usb_disable_power_role_switch(struct smb_charger *chg, bool disable)
{
	int rc = 0;
	u8 power_role;
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, (uint8_t)TYPEC_POWER_ROLE_CMD_MASK,
				 power_role);
	if (rc < 0) {
...
static int __smblib_set_prop_typec_power_role(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;
	u8 power_role;
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_POWER_ROLE_CMD_MASK, power_role);
	if (rc < 0) {
		smblib_err(chg, "Couldn't write 0x%02x to TYPE_C_INTRPT_ENB_SOFTWARE_CTRL rc=%d\n", power_role, rc);
		return rc;
	}
...
static int smblib_dc_icl_vote_callback(struct votable *votable, void *data, int icl_ua, const char *client)
{
	struct smb_charger *chg = data;
	int rc = 0;
	bool suspend;
...
static int smblib_pd_disallowed_votable_indirect_callback(struct votable *votable, void *data, int disallowed,
							  const char *client)
{
	struct smb_charger *chg = data;
	int rc;

	rc = vote(chg->pd_allowed_votable, PD_DISALLOWED_INDIRECT_VOTER, !disallowed, 0);
...
static int smblib_hvdcp_enable_vote_callback(struct votable *votable, void *data, int hvdcp_enable, const char *client)
{
	struct smb_charger *chg = data;
	int rc;
	...
	rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, HVDCP_EN_BIT | HVDCP_AUTH_ALG_EN_CFG_BIT, val);
	if (rc < 0) {
...
static int smblib_hvdcp_disable_indirect_vote_callback(struct votable *votable, void *data, int hvdcp_disable,
						       const char *client)
{
	struct smb_charger *chg = data;

	vote(chg->hvdcp_enable_votable, HVDCP_INDIRECT_VOTER, !hvdcp_disable, 0);

	return 0;
}

static int smblib_apsd_disable_vote_callback(struct votable *votable, void *data, int apsd_disable, const char *client)
{
	struct smb_charger *chg = data;
	int rc;

	if (apsd_disable) {
		rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, AUTO_SRC_DETECT_BIT, 0);
		if (rc < 0) {
			smblib_err(chg, "Couldn't disable APSD rc=%d\n", rc);
			return rc;
		}
	} else {
		rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, AUTO_SRC_DETECT_BIT, AUTO_SRC_DETECT_BIT);
		if (rc < 0) {
...
static int smblib_hvdcp_hw_inov_dis_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct smb_charger *chg = data;
	int rc;
...
static int smblib_usb_irq_enable_vote_callback(struct votable *votable, void *data, int enable, const char *client)
{
	struct smb_charger *chg = data;

	if (!chg->irq_info[INPUT_CURRENT_LIMIT_IRQ].irq || !chg->irq_info[HIGH_DUTY_CYCLE_IRQ].irq)
		return 0;
...
static int smblib_typec_irq_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_disable_power_role_switch_callback(struct votable *votable, void *data, int disable,
						     const char *client)
{
	struct smb_charger *chg = data;
	union power_supply_propval pval;
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		...
		pval.intval = disable ? POWER_SUPPLY_TYPEC_PR_SINK : POWER_SUPPLY_TYPEC_PR_DUAL;
...
static int _smblib_vconn_regulator_enable(struct regulator_dev *rdev)
{
	...
	val = chg->typec_status[3] & CC_ORIENTATION_BIT ? 0 : VCONN_EN_ORIENTATION_BIT;
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG,
				 VCONN_EN_VALUE_BIT | VCONN_EN_ORIENTATION_BIT, VCONN_EN_VALUE_BIT | val);
	if (rc < 0) {
...
static int _smblib_vconn_regulator_disable(struct regulator_dev *rdev)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, VCONN_EN_VALUE_BIT, 0);
	if (rc < 0)
...
static int smblib_enable_otg_wa(struct smb_charger *chg)
{
	...
	for (i = 0; i < ARRAY_SIZE(otg_current); i++) {
		...
		rc = smblib_set_charge_param(chg, &chg->param.otg_cl, otg_current[i]);
		if (rc < 0) {
...
static int _smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	...
	rc = smblib_masked_write(chg, OTG_ENG_OTG_CFG_REG, ENG_BUCKBOOST_HALT1_8_MODE_BIT,
...
static int _smblib_vbus_regulator_disable(struct regulator_dev *rdev)
{
	...
	if (chg->wa_flags & OTG_WA) {
		...
		rc = smblib_set_charge_param(chg, &chg->param.otg_cl, chg->param.otg_cl.min_u);
		if (rc < 0) {
	...
	rc = smblib_masked_write(chg, OTG_ENG_OTG_CFG_REG, ENG_BUCKBOOST_HALT1_8_MODE_BIT, 0);
	if (rc < 0) {
		smblib_err(chg, "Couldn't set OTG_ENG_OTG_CFG_REG rc=%d\n", rc);
		return rc;
	}
...
int smblib_get_prop_batt_capacity(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if (chg->bms_psy)
		rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CAPACITY, val);
	return rc;
}
...
int smblib_get_prop_batt_status(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	stat &= ENABLE_TRICKLE_BIT | ENABLE_PRE_CHARGING_BIT | ENABLE_FAST_CHARGING_BIT | ENABLE_FULLON_MODE_BIT;
...
int smblib_get_prop_charge_qnovo_enable(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
int smblib_set_prop_charge_qnovo_enable(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;

	rc = smblib_masked_write(chg, QNOVO_PT_ENABLE_CMD_REG, QNOVO_PT_ENABLE_CMD_BIT,
...
int smblib_rerun_aicl(struct smb_charger *chg)
{
	...
	rc = smblib_get_charge_param(chg, &chg->param.icl_stat, &settled_icl_ua);
	if (rc < 0) {
	...
	vote(chg->usb_icl_votable, AICL_RERUN_VOTER, true,
	     max(settled_icl_ua - chg->param.usb_icl.step_u, chg->param.usb_icl.step_u));
...
int smblib_dp_dm(struct smb_charger *chg, int val)
{
	...
	case POWER_SUPPLY_DP_DM_ICL_DOWN:
		...
		if (target_icl_ua < 0) {
			...
			if (rc < 0) {
				smblib_err(chg, "Couldn't get max current rc=%d\n", rc);
				return rc;
			}
...
int smblib_disable_hw_jeita(struct smb_charger *chg, bool disable)
{
	...
	return 0;
	//#endif
	mask = JEITA_EN_COLD_SL_FCV_BIT | JEITA_EN_HOT_SL_FCV_BIT | JEITA_EN_HOT_SL_CCC_BIT | JEITA_EN_COLD_SL_CCC_BIT,
...
int smblib_get_prop_usb_current_now(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if (!chg->iio.usbin_i_chan || PTR_ERR(chg->iio.usbin_i_chan) == -EPROBE_DEFER)
...
int smblib_get_prop_charger_temp_max(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;

	if (!chg->iio.temp_max_chan || PTR_ERR(chg->iio.temp_max_chan) == -EPROBE_DEFER)
...
int smblib_get_prop_typec_cc_orientation(struct smb_charger *chg, union power_supply_propval *val)
{
	if (chg->typec_status[3] & CC_ATTACHED_BIT)
		val->intval = (bool)(chg->typec_status[3] & CC_ORIENTATION_BIT) + 1;
	else
		val->intval = 0;
...
int smblib_get_prop_pd_allowed(struct smb_charger *chg, union power_supply_propval *val)
{
...
int smblib_get_pe_start(struct smb_charger *chg, union power_supply_propval *val)
{
	 ...
	 */
	val->intval = !get_client_vote_locked(chg->pd_disallowed_votable_indirect, HVDCP_TIMEOUT_VOTER);
	return 0;
}

int smblib_get_prop_die_health(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
static int smblib_handle_usb_current(struct smb_charger *chg, int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			rc = vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, rp_ua);
			if (rc < 0)
				return rc;
		} else {
			/*
			...
			if (rc < 0)
				return rc;
			rc = vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
			if (rc < 0)
				return rc;
		}
	} else {
...
static int __smblib_set_prop_pd_active(struct smb_charger *chg, bool pd_active)
{
	...
	if (chg->pd_active) {
		...
		rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, VCONN_EN_ORIENTATION_BIT,
					 orientation ? 0 : VCONN_EN_ORIENTATION_BIT);
		if (rc < 0)
			smblib_err(chg, "Couldn't enable vconn on CC line rc=%d\n", rc);

		/* SW controlled CC_OUT */
		rc = smblib_masked_write(chg, TAPER_TIMER_SEL_CFG_REG, TYPEC_SPARE_CFG_BIT, TYPEC_SPARE_CFG_BIT);
		if (rc < 0)
		...
		rc = smblib_masked_write(chg, TAPER_TIMER_SEL_CFG_REG, TYPEC_SPARE_CFG_BIT, 0);
		if (rc < 0)
		...
		if ((chg->connector_type != POWER_SUPPLY_CONNECTOR_MICRO_USB) && !chg->typec_legacy_valid &&
		    !sink_attached && hvdcp)
...
int smblib_reg_block_update(struct smb_charger *chg, struct reg_info *entry)
{
	...
	while (entry && entry->reg) {
		...
		rc = smblib_masked_write(chg, entry->reg, entry->mask, entry->val);
		if (rc < 0) {
...
int smblib_reg_block_restore(struct smb_charger *chg, struct reg_info *entry)
{
	...
	while (entry && entry->reg) {
		rc = smblib_masked_write(chg, entry->reg, entry->mask, entry->bak);
		if (rc < 0) {
			dev_err(chg->dev, "Error in writing %s rc=%d\n", entry->desc, rc);
			break;
		}
...
static struct reg_info cc2_detach_settings[] = {
	...
	{ .reg = MISC_CFG_REG, .mask = TCC_DEBOUNCE_20MS_BIT, .val = TCC_DEBOUNCE_20MS_BIT, .desc = "Tccdebounce time" },
	{},
};
...
static int smblib_cc2_sink_removal_enter(struct smb_charger *chg)
{
	...
	ccout = (stat & CC_ATTACHED_BIT) ? (!!(stat & CC_ORIENTATION_BIT) + 1) : 0;
	ufp_mode = (stat & TYPEC_DEBOUNCE_DONE_STATUS_BIT) ? !(stat & UFP_DFP_MODE_STATUS_BIT) : 0;
...
int smblib_set_prop_pd_in_hard_reset(struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, EXIT_SNK_BASED_ON_CC_BIT,
				 (chg->pd_hard_reset) ? EXIT_SNK_BASED_ON_CC_BIT : 0);
	if (rc < 0)
		smblib_err(chg, "Couldn't set EXIT_SNK_BASED_ON_CC rc=%d\n", rc);

	vote(chg->apsd_disable_votable, PD_HARD_RESET_VOTER, chg->pd_hard_reset, 0);
...
int smblib_get_prop_slave_current_now(struct smb_charger *chg, union power_supply_propval *pval)
{
...
irqreturn_t smblib_handle_usbin_uv(int irq, void *data)
{
	...
	if (!chg->non_compliant_chg_detected && apsd->pst == POWER_SUPPLY_TYPE_USB_HVDCP) {
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (get_effective_result(chg->apsd_disable_votable) && !chg->pd_active)
...
irqreturn_t smblib_handle_icl_change(int irq, void *data)
{
	...
	if (chg->mode == PARALLEL_MASTER) {
		...
		if ((settled_ua == get_effective_result(chg->usb_icl_votable)) || (stat & AICL_DONE_BIT))
			delay = 0;
...
static void smblib_check_ov_condition(struct smb_charger *chg)
{
	...
	if (chg->wa_flags & OV_IRQ_WA_BIT) {
		rc = power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		if (rc < 0) {
		...
		if (pval.intval > MICRO_10P3V) {
			...
			rc = power_supply_set_property(chg->batt_psy, POWER_SUPPLY_PROP_DP_DM, &pval);
			return;
		}
	}
}
...
static void smblib_handle_hvdcp_3p0_auth_done(struct smb_charger *chg, bool rising)
{
	...
	if (chg->wa_flags & QC_AUTH_INTERRUPT_WA_BIT) {
		 ...
		 * change interrupt.
		 */
		rc = smblib_masked_write(chg, USBIN_SOURCE_CHANGE_INTRPT_ENB_REG, AUTH_IRQ_EN_CFG_BIT, 0);
		if (rc < 0)
			smblib_err(chg, "Couldn't enable QC auth setting rc=%d\n", rc);
	}
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	case CDP_CHARGER_BIT:
		...
		vote(chg->pd_disallowed_votable_indirect, HVDCP_TIMEOUT_VOTER, false, 0);
		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB || chg->use_extcon)
	...
	case DCP_CHARGER_BIT:
		if (chg->wa_flags & QC_CHARGER_DETECTION_WA_BIT)
			schedule_delayed_work(&chg->hvdcp_detect_work, msecs_to_jiffies(HVDCP_DET_MS));
		break;
	default:
		break;
	}
...
static int typec_try_sink(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, MISC_CFG_REG, TCC_DEBOUNCE_20MS_BIT, TCC_DEBOUNCE_20MS_BIT);
	if (rc < 0) {
...
{
	smblib_set_charge_param(chg, &chg->param.freq_boost, chg->chg_freq.freq_above_otg_threshold);
...
static void smblib_handle_typec_removal(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_CFG_REG, APSD_START_ON_CC_BIT, APSD_START_ON_CC_BIT);
	if (rc < 0)
	...
	smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, VCONN_EN_VALUE_BIT, 0);
	chg->vconn_en = false;
...
unlock:
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, EXIT_SNK_BASED_ON_CC_BIT, 0);
	if (rc < 0)
...
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode)
{
	...
	if ((apsd->pst != POWER_SUPPLY_TYPE_USB_DCP) && (apsd->pst != POWER_SUPPLY_TYPE_USB_FLOAT))
		return;
	...
	if (apsd->pst == POWER_SUPPLY_TYPE_USB_FLOAT &&
	    get_client_vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER) <= 100000)
		return;
...
static void smblib_handle_typec_cc_state_change(struct smb_charger *chg)
{
	...
	} else if (chg->typec_present && chg->typec_mode == POWER_SUPPLY_TYPEC_NONE) {
	...
	/* suspend usb if sink */
	if ((chg->typec_status[3] & UFP_DFP_MODE_STATUS_BIT) && chg->typec_present)
...
void smblib_usb_typec_change(struct smb_charger *chg)
{
	...
	rc = smblib_multibyte_read(chg, TYPE_C_STATUS_1_REG, chg->typec_status, 5);
	if (rc < 0) {
...
irqreturn_t smblib_handle_usb_typec_change(int irq, void *data)
{
	...
	if (chg->cc2_detach_wa_active || chg->typec_en_dis_active || chg->try_sink_active) {
		smblib_dbg(chg, PR_MISC | PR_INTERRUPT, "Ignoring since %s active\n",
			   chg->cc2_detach_wa_active ? "cc2_detach_wa" : "typec_en_dis");
		return IRQ_HANDLED;
	}

	if (chg->pr_swap_in_progress) {
		smblib_dbg(chg, PR_INTERRUPT, "Ignoring since pr_swap_in_progress\n");
		return IRQ_HANDLED;
	}
...
int smblib_set_prop_pr_swap_in_progress(struct smb_charger *chg, const union power_supply_propval *val)
{
	 ...
	 */
	smblib_usb_typec_change(chg);
	rc = smblib_masked_write(chg, MISC_CFG_REG, TCC_DEBOUNCE_20MS_BIT, val->intval ? TCC_DEBOUNCE_20MS_BIT : 0);
	if (rc < 0)
...
static void oplus_divider_set_work(struct work_struct *work)
{
	...
	u8 stat;
	bool vbus_rising;
	struct smb_charger *chg = container_of(work, struct smb_charger, divider_set_work.work);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, hvdcp_detect_work.work);
...
static void rdstd_cc2_detach_work(struct work_struct *work)
{
	int rc;
	u8 stat4, stat5;
	struct smb_charger *chg = container_of(work, struct smb_charger, rdstd_cc2_detach_work);
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, UFP_EN_CMD_BIT | DFP_EN_CMD_BIT,
	...
	if ((stat4 & TYPEC_DEBOUNCE_DONE_STATUS_BIT) || (stat5 & TIMER_STAGE_2_BIT)) {
...
static void smblib_otg_oc_exit(struct smb_charger *chg, bool success)
{
	...
	rc = smblib_masked_write(chg, OTG_CFG_REG, QUICKSTART_OTG_FASTROLESWAP_BIT, 0);
	if (rc < 0)
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, otg_oc_work);
	int rc, i;
	u8 stat;
	...
	smblib_masked_write(chg, OTG_CFG_REG, QUICKSTART_OTG_FASTROLESWAP_BIT, QUICKSTART_OTG_FASTROLESWAP_BIT);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, vconn_oc_work);
	int rc, i;
	u8 stat;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, otg_ss_done_work.work);
	int rc;
	bool success = false;
	u8 stat;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, legacy_detection_work);
	int rc;
	u8 stat;
	bool legacy, rp_high;
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_DISABLE_CMD_BIT,
...
static int smblib_create_votables(struct smb_charger *chg)
{
	...
	chg->dc_icl_votable = create_votable("DC_ICL", VOTE_MIN, smblib_dc_icl_vote_callback, chg);
	if (IS_ERR(chg->dc_icl_votable)) {
		rc = PTR_ERR(chg->dc_icl_votable);
		return rc;
	}

	chg->pd_disallowed_votable_indirect = create_votable("PD_DISALLOWED_INDIRECT", VOTE_SET_ANY,
	...
	chg->pd_allowed_votable = create_votable("PD_ALLOWED", VOTE_SET_ANY, NULL, NULL);
	...
	chg->hvdcp_disable_votable_indirect = create_votable("HVDCP_DISABLE_INDIRECT", VOTE_SET_ANY,
							     smblib_hvdcp_disable_indirect_vote_callback, chg);
	...
	chg->hvdcp_enable_votable =
		create_votable("HVDCP_ENABLE", VOTE_SET_ANY, smblib_hvdcp_enable_vote_callback, chg);
	...
	chg->apsd_disable_votable =
		create_votable("APSD_DISABLE", VOTE_SET_ANY, smblib_apsd_disable_vote_callback, chg);
	...
	chg->hvdcp_hw_inov_dis_votable =
		create_votable("HVDCP_HW_INOV_DIS", VOTE_SET_ANY, smblib_hvdcp_hw_inov_dis_vote_callback, chg);
	...
	chg->usb_irq_enable_votable =
		create_votable("USB_IRQ_DISABLE", VOTE_SET_ANY, smblib_usb_irq_enable_vote_callback, chg);
	...
	chg->typec_irq_disable_votable =
		create_votable("TYPEC_IRQ_DISABLE", VOTE_SET_ANY, smblib_typec_irq_disable_vote_callback, chg);
	if (IS_ERR(chg->typec_irq_disable_votable)) {
		rc = PTR_ERR(chg->typec_irq_disable_votable);
		return rc;
	}

	chg->disable_power_role_switch = create_votable("DISABLE_POWER_ROLE_SWITCH", VOTE_SET_ANY,
...
int smblib_init(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		rc = qcom_step_chg_init(chg->dev, chg->step_chg_enabled, chg->sw_jeita_enabled);
		if (rc < 0) {
		...
		if (chg->pl.psy) {
			...
			if (rc < 0) {
				smblib_err(chg, "Couldn't config stat sw rc=%d\n", rc);
				return rc;
			}
		}
		break;
...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	chg->reddragon_ipc_wa = of_property_read_bool(node, "qcom,qcs605-ipc-wa");
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = of_property_read_u32(node, "qcom,charger_id_num", &chg->charger_id_num);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,fcc-max-ua", &chg->batt_profile_fcc_ua);
	if (rc < 0)
		chg->batt_profile_fcc_ua = -EINVAL;

	rc = of_property_read_u32(node, "qcom,fv-max-uv", &chg->batt_profile_fv_uv);
	if (rc < 0)
	...
	rc = of_property_read_u32(node, "qcom,boost-threshold-ua", &chip->dt.boost_threshold_ua);
	if (rc < 0)
		chip->dt.boost_threshold_ua = MICRO_P1A;

	rc = of_property_read_u32(node, "qcom,min-freq-khz", &chip->dt.min_freq_khz);
	if (rc < 0)
		chip->dt.min_freq_khz = -EINVAL;

	rc = of_property_read_u32(node, "qcom,max-freq-khz", &chip->dt.max_freq_khz);
	if (rc < 0)
		chip->dt.max_freq_khz = -EINVAL;

	rc = of_property_read_u32(node, "qcom,wipower-max-uw", &chip->dt.wipower_max_uw);
	if (rc < 0)
	...
	of_property_read_u32(node, "qcom,chg-inhibit-threshold-mv", &chip->dt.chg_inhibit_thr_mv);
	if ((chip->dt.chg_inhibit_thr_mv < 0 || chip->dt.chg_inhibit_thr_mv > 300)) {
		pr_err("qcom,chg-inhibit-threshold-mv is incorrect\n");
		return -EINVAL;
	}

	chip->dt.auto_recharge_soc = of_property_read_bool(node, "qcom,auto-recharge-soc");
	...
	chg->disable_stat_sw_override = of_property_read_bool(node, "qcom,disable-stat-sw-override");
 ...
 ************************/
int smblib_get_prop_batt_charge_counter(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CHARGE_COUNTER, val);
	return rc;
}
...
static int smb2_usb_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		val->intval = get_client_vote(chg->fcc_votable, FG_ESR_VOTER);
		break;
	...
	case POWER_SUPPLY_PROP_MOISTURE_DETECTED:
		val->intval = get_client_vote(chg->disable_power_role_switch, MOISTURE_VOTER);
		break;
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		if (((oplus_vooc_get_fastchg_started() == true) || (oplus_vooc_get_fastchg_dummy_started() == true)) &&
...
static int smb2_usb_set_prop(struct power_supply *psy, enum power_supply_property psp,
			     const union power_supply_propval *val)
{
	...
	if (!chg->typec_present) {
		...
		switch (psp) {
		case POWER_SUPPLY_PROP_MOISTURE_DETECTED:
			vote(chg->disable_power_role_switch, MOISTURE_VOTER, val->intval > 0, 0);
			break;
		default:
			rc = -EINVAL;
			break;
...
static int smb2_usb_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CTM_CURRENT_MAX:
		return 1;
...
static int smb2_init_usb_psy(struct smb2 *chip)
{
	...
	chg->usb_psy = power_supply_register(chg->dev, &chg->usb_psy_desc, &usb_cfg);
...
static int smb2_usb_port_get_prop(struct power_supply *psy, enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct smb2 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb2_usb_port_set_prop(struct power_supply *psy, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	int rc = 0;
...
static int smb2_init_usb_port_psy(struct smb2 *chip)
{
	...
	chg->usb_port_psy = power_supply_register(chg->dev, &usb_port_psy_desc, &usb_port_cfg);
...
static int smb2_usb_main_get_prop(struct power_supply *psy, enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct smb2 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb2_usb_main_set_prop(struct power_supply *psy, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	struct smb2 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb2_usb_main_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc;
...
static int smb2_init_usb_main_psy(struct smb2 *chip)
{
	...
	chg->usb_main_psy = power_supply_register(chg->dev, &usb_main_psy_desc, &usb_main_cfg);
...
static enum power_supply_property smb2_dc_props[] = {
	POWER_SUPPLY_PROP_INPUT_SUSPEND, POWER_SUPPLY_PROP_PRESENT,   POWER_SUPPLY_PROP_ONLINE,
	POWER_SUPPLY_PROP_CURRENT_MAX,	 POWER_SUPPLY_PROP_REAL_TYPE,
};

static int smb2_dc_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	struct smb2 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
	int rc = 0;
...
static int smb2_dc_set_prop(struct power_supply *psy, enum power_supply_property psp,
			    const union power_supply_propval *val)
{
	struct smb2 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb2_dc_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc;
...
static int smb2_init_dc_psy(struct smb2 *chip)
{
	...
	chg->dc_psy = power_supply_register(chg->dev, &dc_psy_desc, &dc_cfg);
...
static int smb2_ac_get_property(struct power_supply *psy, enum power_supply_property psp,
				union power_supply_propval *val)
{
	int rc = 0;
...
static int smb2_batt_set_prop(struct power_supply *psy, enum power_supply_property prop,
			      const union power_supply_propval *val)
{
	int rc = 0;
	struct smb_charger *chg = power_supply_get_drvdata(psy);
...
static int smb2_batt_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc = 0;
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
...
static int smb2_init_batt_psy(struct smb2 *chip)
{
	...
	chg->batt_psy = power_supply_register(chg->dev, &batt_psy_desc, &batt_cfg);
...
static int smb2_configure_typec(struct smb_charger *chg)
{
	 ...
	 * changes
	 */
	rc = smblib_write(chg, TYPE_C_INTRPT_ENB_REG, TYPEC_CCSTATE_CHANGE_INT_EN_BIT);
	if (rc < 0) {
	 ...
	 */
	rc = smblib_masked_write(chg, TYPE_C_CFG_REG, FACTORY_MODE_DETECTION_EN_BIT | VCONN_OC_CFG_BIT, 0);
	if (rc < 0) {
	...
	rc = smblib_masked_write(chg, TYPE_C_CFG_2_REG, VCONN_SOFTSTART_CFG_MASK, VCONN_SOFTSTART_CFG_MASK);
	if (rc < 0) {
	...
	rc = smblib_masked_write(
		chg, TYPE_C_CFG_3_REG,
		EN_TRYSINK_MODE_BIT | TYPEC_NONCOMPLIANT_LEGACY_CABLE_INT_EN_BIT | TYPEC_LEGACY_CABLE_INT_EN_BIT, 0);
	if (rc < 0) {
	...
	rc = smblib_masked_write(chg, TYPE_C_CFG_2_REG, DFP_CC_1P4V_OR_1P6V_BIT, DFP_CC_1P4V_OR_1P6V_BIT);
	if (rc < 0)
...
static int smb2_disable_typec(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_DISABLE_CMD_BIT | UFP_EN_CMD_BIT,
				 TYPEC_DISABLE_CMD_BIT);
	if (rc < 0) {
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	if (chip->dt.dc_icl_ua < 0)
		smblib_get_charge_param(chg, &chg->param.dc_icl, &chip->dt.dc_icl_ua);
	...
	rc = smblib_masked_write(chg, DC_ENG_SSUPPLY_CFG2_REG, ENG_SSUPPLY_IVREF_OTG_SS_MASK, OTG_SS_SLOW);
	if (rc < 0) {
	...
	rc = smblib_set_charge_param(chg, &chg->param.otg_cl,
				     (chg->wa_flags & OTG_WA) ? chg->param.otg_cl.min_u : chg->otg_cl_ua);
	if (rc < 0) {
	...
	vote(chg->dc_icl_votable, DEFAULT_VOTER, true, chip->dt.dc_icl_ua);
	vote(chg->hvdcp_disable_votable_indirect, DEFAULT_VOTER, chip->dt.hvdcp_disable, 0);
	vote(chg->pd_disallowed_votable_indirect, CC_DETACHED_VOTER, true, 0);
	vote(chg->pd_disallowed_votable_indirect, HVDCP_TIMEOUT_VOTER, true, 0);
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG,
				 SUSPEND_ON_COLLAPSE_USBIN_BIT | USBIN_AICL_START_AT_MAX_BIT | USBIN_AICL_ADC_EN_BIT |
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG, USBIN_AICL_START_AT_MAX_BIT | USBIN_AICL_ADC_EN_BIT,
				 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure AICL rc=%d\n", rc);
		return rc;
	...
	rc = smblib_masked_write(chg, CHGR_CFG2_REG, CHG_EN_POLARITY_BIT | CHG_EN_SRC_BIT, 0);
	if (rc < 0) {
	...
	chg->connector_type =
		(val & EN_MICRO_USB_MODE_BIT) ? POWER_SUPPLY_CONNECTOR_MICRO_USB : POWER_SUPPLY_CONNECTOR_TYPEC;
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
...
#ifndef OPLUS_FEATURE_CHG_BASIC
	vote(chg->hvdcp_enable_votable, MICRO_USB_VOTER, (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB), 0);
#else
	vote(chg->hvdcp_enable_votable, MICRO_USB_VOTER, false, 0);
#endif

	/* configure VCONN for software control */
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, VCONN_EN_SRC_BIT | VCONN_EN_VALUE_BIT,
				 VCONN_EN_SRC_BIT);
	if (rc < 0) {
	...
	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG,
				 BITE_WDOG_DISABLE_CHARGING_CFG_BIT | BARK_WDOG_TIMEOUT_MASK | BITE_WDOG_TIMEOUT_MASK,
				 val);
	if (rc) {
	...
	rc = smblib_masked_write(chg, WD_CFG_REG,
				 WATCHDOG_TRIGGER_AFP_EN_BIT | WDOG_TIMER_EN_ON_PLUGIN_BIT | BARK_WDOG_INT_EN_BIT,
				 WDOG_TIMER_EN_ON_PLUGIN_BIT | BARK_WDOG_INT_EN_BIT);
	if (rc) {
		pr_err("Couldn't configue WD config rc=%d\n", rc);
		return rc;
	}
	...
	rc = smblib_masked_write(chg, MISC_CFG_REG, STAT_PARALLEL_1400MA_EN_CFG_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't disable h/w autonomous parallel control rc=%d\n", rc);
		return rc;
	}
	 ...
	 */
	rc = smblib_masked_write(chg, TAPER_TIMER_SEL_CFG_REG, TYPEC_DRP_DFP_TIME_CFG_BIT, TYPEC_DRP_DFP_TIME_CFG_BIT);
	if (rc < 0) {
	...
	case 1:
		rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FLOAT_OPTIONS_MASK, 0);
		break;
	case 2:
		rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FLOAT_OPTIONS_MASK, FORCE_FLOAT_SDP_CFG_BIT);
		break;
	case 3:
		rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FLOAT_OPTIONS_MASK, FLOAT_DIS_CHGING_CFG_BIT);
		break;
	case 4:
		rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FLOAT_OPTIONS_MASK, SUSPEND_FLOAT_CFG_BIT);
		break;
	default:
		rc = 0;
		break;
	...
	if (chip->dt.auto_recharge_soc) {
		rc = smblib_masked_write(chg, FG_UPDATE_CFG_2_SEL_REG,
					 SOC_LT_CHG_RECHARGE_THRESH_SEL_BIT | VBT_LT_CHG_RECHARGE_THRESH_SEL_BIT,
	...
	if (chg->disable_stat_sw_override) {
		rc = smblib_masked_write(chg, STAT_CFG_REG, STAT_SW_OVERRIDE_CFG_BIT, 0);
		if (rc < 0) {
...
static int smb2_post_init(struct smb2 *chip)
{
	...
	if (g_oplus_chip && oplus_ccdetect_check_is_gpio(g_oplus_chip) == true) {
		...
		rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_POWER_ROLE_CMD_MASK, 0);
		if (rc < 0) {
...
static int smb2_chg_config_init(struct smb2 *chip)
{
	...
	revid_dev_node = of_parse_phandle(chip->chg.dev->of_node, "qcom,pmic-revid", 0);
	if (!revid_dev_node) {
	...
	case PMI8998_SUBTYPE:
		...
		chip->chg.wa_flags |= BOOST_BACK_WA | QC_AUTH_INTERRUPT_WA_BIT | TYPEC_PBS_WA_BIT;
...
#else
		chip->chg.wa_flags |= BOOST_BACK_WA | OTG_WA | OV_IRQ_WA_BIT | TYPEC_PBS_WA_BIT;
#endif
...
static int smb2_request_interrupt(struct smb2 *chip, struct device_node *node, const char *irq_name)
{
	struct smb_charger *chg = &chip->chg;
	...
	rc = devm_request_threaded_irq(chg->dev, irq, NULL, smb2_irqs[irq_index].handler, IRQF_ONESHOT, irq_name,
				       irq_data);
	if (rc < 0) {
...
static void smb2_free_interrupts(struct smb_charger *chg)
{
	...
	for (i = 0; i < ARRAY_SIZE(smb2_irqs); i++) {
		if (smb2_irqs[i].irq > 0) {
			...
			devm_free_irq(chg->dev, smb2_irqs[i].irq, smb2_irqs[i].irq_data);
		}
	}
}
...
DEFINE_SIMPLE_ATTRIBUTE(force_usb_psy_update_ops, NULL, force_usb_psy_update_write, "0x%02llx\n");
...
DEFINE_SIMPLE_ATTRIBUTE(force_dc_psy_update_ops, NULL, force_dc_psy_update_write, "0x%02llx\n");
...
static int smbchg_set_fastchg_current_raw(int current_ma)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.fcc_votable, DEFAULT_VOTER, true, current_ma * 1000);
	if (rc < 0)
...
static void smbchg_aicl_enable(bool enable)
{
	...
	rc = smblib_masked_write(&chip->pmic_spmi.smb2_chip->chg, USBIN_AICL_OPTIONS_CFG_REG, USBIN_AICL_EN_BIT,
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (pre_current > current_ma) {
		...
		if (usb_icl[n] > 1200) {
			rc = vote(chip->pmic_spmi.smb2_chip->chg.usb_icl_votable, USB_PSY_VOTER, true,
				  usb_icl[n] * 1000);
			msleep(90);
			n--;
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.fv_votable, BATT_PROFILE_VOTER /*DEFAULT_VOTER*/, true,
		  vfloat_mv * 1000);
	if (rc < 0)
...
static int smbchg_charging_enble(void)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.chg_disable_votable, DEFAULT_VOTER, false, 0);
	if (rc < 0)
...
static int smbchg_charging_disble(void)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.chg_disable_votable, DEFAULT_VOTER, true, 0);
	if (rc < 0)
...
int pm660l_bob_regulator_get_mode(unsigned int *mode)
{
	...
	if (bob_mode != REGULATOR_MODE_FAST && bob_mode != REGULATOR_MODE_NORMAL && bob_mode != REGULATOR_MODE_IDLE &&
	    bob_mode != REGULATOR_MODE_STANDBY) {
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	pr_info("QPNP SMB2 probed successfully usb:present=%d type=%d batt:present = %d health = %d charge = %d\n",
		usb_present, chg->real_charger_type, batt_present, batt_health, batt_charge_type);
	return rc;
...
static void smb2_shutdown(struct platform_device *pdev)
{
	...
	smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_POWER_ROLE_CMD_MASK, UFP_EN_CMD_BIT);
	...
	/* force enable APSD */
	smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, AUTO_SRC_DETECT_BIT, AUTO_SRC_DETECT_BIT);
