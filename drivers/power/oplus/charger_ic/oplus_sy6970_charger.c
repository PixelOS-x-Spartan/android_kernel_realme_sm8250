...
static int bq2589x_switch_to_hvdcp(struct bq2589x *bq, enum hvdcp_type type)
{
	...
	case HVDCP_9V:
		...
		break;
	case HVDCP_12V:
		val = (BQ2589X_DP_0P6V << BQ2589X_DPDAC_SHIFT) | (BQ2589X_DM_0P6V << BQ2589X_DMDAC_SHIFT);
		break;
	case HVDCP_20V:
		val = (BQ2589X_DP_3P3V << BQ2589X_DPDAC_SHIFT) | (BQ2589X_DM_3P3V << BQ2589X_DMDAC_SHIFT);
		break;

	case HVDCP_CONTINOUS:
		val = (BQ2589X_DP_0P6V << BQ2589X_DPDAC_SHIFT) | (BQ2589X_DM_3P3V << BQ2589X_DMDAC_SHIFT);
		break;
	case HVDCP_DPF_DMF:
...
static int bq2589x_inform_charger_type(struct bq2589x *bq)
{
	...
	ret = power_supply_set_property(bq->psy, POWER_SUPPLY_PROP_CHARGE_TYPE, &propval);
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	} else if (!ret && (prev_chg_type == bq->chg_type) && (bq->vbus_type == BQ2589X_VBUS_TYPE_DCP) &&
...
int oplus_bq2589x_set_qc_config(void)
{
	...
	if (!chip->calling_on && !chip->camera_on &&
		...
		if (chip->charger_volt > OPLUS_CHG_QC_VOLT_LOW_THRESHOLD &&
		    (chip->calling_on || chip->camera_on || chip->soc >= OPLUS_CHG_QC_5VAND9V_SWITCH_THRESHOLD ||
...
int oplus_bq2589x_pd_setup(void)
{
	...
	if ((chip->charger_volt > 7500) && ((chip->temperature >= 450) || (chip->temperature < 0))) {
		vbus = VBUS_VOL_5V;
		ibus = IBUS_VOL_3A;
