...
int sgm41542_set_vindpm_vol(int vindpm)
{
	...
	/*set input os*/
	rc = sgm41542_config_interface(chip, REG0F_SGM41542_ADDRESS, offset,
				       REG0F_SGM41542_VINDPM_THRESHOLD_OFFSET_MASK);

	/*set input vindpm*/
	tmp = (vindpm - offset_val) / REG06_SGM41542_VINDPM_STEP_MV;
	rc = sgm41542_config_interface(chip, REG06_SGM41542_ADDRESS, tmp << REG06_SGM41542_VINDPM_SHIFT,
...
int sgm41542_input_current_limit_without_aicl(int current_ma)
{
	...
	rc = sgm41542_config_interface(chip, REG00_SGM41542_ADDRESS, tmp << REG00_SGM41542_INPUT_CURRENT_LIMIT_SHIFT,
...
aicl_pre_step:
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, i,
		  sgm41542_usb_icl[i], sw_aicl_point);
	goto aicl_rerun;
aicl_end:
...
static ssize_t sgm41542_access_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t sgm41542_access_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct chip_sgm41542 *chip = dev_get_drvdata(dev);
	int ret = 0;
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			pr_err("[%s] write sgm41542 reg 0x%02x with value 0x%02x !\n", __func__,
			       (unsigned int)chip->reg_access, reg_value);
...
int sgm41542_kick_wdt(void)
{
	...
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_WDT_TIMER_RESET,
...
int sgm41542_set_wdt_timer(int reg)
{
	...
	rc = sgm41542_config_interface(chip, REG05_SGM41542_ADDRESS, reg, REG05_SGM41542_WATCHDOG_TIMER_MASK);
	if (rc)
...
int sgm41542_set_int_mask(int val)
{
	...
	rc = sgm41542_config_interface(chip, REG0A_SGM41542_ADDRESS, val,
...
int sgm41542_set_chg_timer(bool enable)
{
	...
	rc = sgm41542_config_interface(chip, REG05_SGM41542_ADDRESS,
				       enable ? REG05_SGM41542_CHG_SAFETY_TIMER_ENABLE :
						REG05_SGM41542_CHG_SAFETY_TIMER_DISABLE,
...
int sgm41542_charging_current_write_fast(int chg_cur)
{
	...
	rc = sgm41542_config_interface(chip, REG02_SGM41542_ADDRESS, tmp << REG02_SGM41542_FAST_CHG_CURRENT_LIMIT_SHIFT,
...
int sgm41542_float_voltage_write(int vfloat_mv)
{
	...
	rc = sgm41542_config_interface(chip, REG04_SGM41542_ADDRESS, tmp << REG04_SGM41542_CHG_VOL_LIMIT_SHIFT,
...
int sgm41542_set_termchg_current(int term_curr)
{
	...
	rc = sgm41542_config_interface(chip, REG03_SGM41542_ADDRESS, tmp << REG03_SGM41542_PRE_CHG_CURRENT_LIMIT_SHIFT,
...
int sgm41542_otg_ilim_set(int ilim)
{
	...
	rc = sgm41542_config_interface(chip, REG02_SGM41542_ADDRESS, ilim, REG02_SGM41542_OTG_CURRENT_LIMIT_MASK);
	if (rc < 0)
...
int sgm41542_otg_enable(void)
{
	...
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_OTG_ENABLE,
...
int sgm41542_otg_disable(void)
{
	...
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_OTG_DISABLE,
				       REG01_SGM41542_OTG_MASK);
	if (rc < 0)
...
int sgm41542_enable_charging(void)
{
	...
	sgm41542_otg_disable();
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_CHARGING_ENABLE,
...
int sgm41542_disable_charging(void)
{
	...
	sgm41542_otg_disable();
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_CHARGING_DISABLE,
				       REG01_SGM41542_CHARGING_MASK);
	if (rc < 0)
...
int sgm41542_unsuspend_charger(void)
{
	...
	if ((chip->before_unsuspend_icl == 0) || (chip->before_suspend_icl == 0) ||
	    (chip->before_unsuspend_icl != 100) || (chip->before_unsuspend_icl == chip->before_suspend_icl)) {
...
int sgm41542_set_rechg_voltage(int recharge_mv)
{
	...
	rc = sgm41542_config_interface(chip, REG04_SGM41542_ADDRESS, recharge_mv,
...
int sgm41542_reset_charger(void)
{
	...
	rc = sgm41542_config_interface(chip, REG0B_SGM41542_ADDRESS, REG0B_SGM41542_REG_RST_RESET,
...
int sgm41542_set_chging_term_disable(void)
{
	...
	rc = sgm41542_config_interface(chip, REG05_SGM41542_ADDRESS, REG05_SGM41542_TERMINATION_DISABLE,
...
int sgm41542_set_prechg_voltage_threshold(void)
{
	...
	rc = sgm41542_config_interface(chip, REG01_SGM41542_ADDRESS, REG01_SGM41542_SYS_VOL_LIMIT_3400MV,
...
int sgm41542_set_otg_voltage(void)
{
	...
	rc = sgm41542_config_interface(chip, REG06_SGM41542_ADDRESS, REG06_SGM41542_OTG_VLIM_5000MV,
...
int sgm41542_set_ovp(int val)
{
	...
	rc = sgm41542_config_interface(chip, REG06_SGM41542_ADDRESS, val, REG06_SGM41542_OVP_MASK);
...
int sgm41542_set_iindet(void)
{
	...
	rc = sgm41542_config_interface(chip, REG07_SGM41542_ADDRESS, REG07_SGM41542_IINDET_EN_MASK,
...
int sgm41542_set_shipmode(bool enable)
{
	...
	if (enable) {
		rc = sgm41542_config_interface(chip, REG07_SGM41542_ADDRESS, REG07_SGM41542_BATFET_DIS_ON,
					       REG07_SGM41542_BATFET_DIS_MASK);
		if (rc < 0)
			chg_err("Couldn't set REG07_SGM41542_BATFET_DIS_ON rc = %d\n", rc);
	} else {
		rc = sgm41542_config_interface(chip, REG07_SGM41542_ADDRESS, REG07_SGM41542_BATFET_DIS_OFF,
					       REG07_SGM41542_BATFET_DIS_MASK);
		if (rc < 0)
...
static int sgm41542_parse_dt(struct chip_sgm41542 *chip)
{
	...
	if (!gpio_is_valid(chip->enable_gpio)) {
		...
		ret = gpio_request(chip->enable_gpio, "sgm41542-enable-gpio");
		if (ret) {
...
void sgm41542_force_dpdm(void)
{
	...
	val = 0;
	sgm41542_config_interface(chip, REG0D_SGM41542_ADDRESS, val, REG0D_SGM41542_DPDM_VSEL_MASK); /*dp/dm Hiz*/
}
...
int sgm41542_enable_qc20_hvdcp_9v(void)
{
	...
	ret = sgm41542_config_interface(chip, REG0D_SGM41542_ADDRESS, dp_val, REG0D_SGM41542_DP_VSEL_MASK); /*dp 0.6V*/
	if (ret)
		return ret;
	...
	ret = sgm41542_config_interface(chip, REG0D_SGM41542_ADDRESS, val,
					REG0D_SGM41542_DPDM_VSEL_MASK); /*dp 3.3v dm 0.6v*/
...
static int sgm41542_disable_hvdcp(void)
{
	...
	ret = sgm41542_config_interface(chip, REG0D_SGM41542_ADDRESS, val, REG0D_SGM41542_DPDM_VSEL_MASK);
...
static int sgm41542_set_qc_config(void)
{
	...
	if (!g_oplus_chip->calling_on && !g_oplus_chip->camera_on && g_oplus_chip->charger_volt < MT6377_VBUS_VOL_6V5 &&
	    g_oplus_chip->soc < SOC_90 && g_oplus_chip->temperature <= TEMP_42 &&
	    g_oplus_chip->cool_down_force_5v == false && !btb_detect_over) {
		...
		if (g_oplus_chip->charger_volt > MT6377_VBUS_VOL_7V5 &&
		    (g_oplus_chip->calling_on || g_oplus_chip->camera_on || g_oplus_chip->soc >= SOC_90 ||
		     g_oplus_chip->batt_volt >= BAT_VOLT_4V45 || g_oplus_chip->temperature > TEMP_42 ||
		     g_oplus_chip->temperature < TEMP_0 || g_oplus_chip->cool_down_force_5v == true ||
		     btb_detect_over)) {
...
static irqreturn_t sgm41542_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&chip->charger_suspended) == 1) {
		...
		wait_event_interruptible_timeout(chip->wait, atomic_read(&chip->charger_suspended) == 0,
		...
		if (g_oplus_chip) {
			if (oplus_vooc_get_fastchg_to_normal() == false && oplus_vooc_get_fastchg_to_warm() == false) {
				if (g_oplus_chip->authenticate && g_oplus_chip->mmi_chg &&
				    !g_oplus_chip->balancing_bat_stop_chg && oplus_vooc_get_allow_reading() &&
...
static int sgm41542_irq_register(struct chip_sgm41542 *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, gpio_to_irq(chip->irq_gpio), NULL, sgm41542_irq_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "sgm41542-eint", chip);
	if (ret < 0) {
...
__attribute__((unused)) static int sgm41542_get_ichg(struct charger_device *chg_dev, u32 *curr)
{
	...
	if (!ret) {
		ichg = (reg_val & REG02_SGM41542_FAST_CHG_CURRENT_LIMIT_MASK) >>
		       REG02_SGM41542_FAST_CHG_CURRENT_LIMIT_SHIFT;
...
__attribute__((unused)) static int sgm41542_set_chargevolt(u32 volt)
{
	...
	return sgm41542_config_interface(chip, REG04_SGM41542_ADDRESS, val << REG04_SGM41542_CHG_VOL_LIMIT_SHIFT,
					 REG04_SGM41542_CHG_VOL_LIMIT_MASK);
}
...
static int sgm41542_charger_get_property(struct power_supply *psy, enum power_supply_property psp,
...
static char *sgm41542_charger_supplied_to[] = { "battery", "mtk-master-charger" };
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct chip_sgm41542 *chip = (struct chip_sgm41542 *)container_of(nb, struct chip_sgm41542, pd_nb);
...
static int sgm41542_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip->chg_dev =
		charger_device_register(chip->chg_dev_name, &client->dev, chip, &sgm41542_chg_ops, &sgm41542_chg_props);
