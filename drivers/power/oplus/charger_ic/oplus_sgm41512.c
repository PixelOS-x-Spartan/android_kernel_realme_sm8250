...
int sgm41512_set_vindpm_vol(int vol)
{
	...
	rc = sgm41512_config_interface(chip, REG06_SGM41512_ADDRESS, tmp << REG06_SGM41512_VINDPM_SHIFT,
...
int sgm41512_input_current_limit_without_aicl(int current_ma)
{
	...
	rc = sgm41512_config_interface(chip, REG00_SGM41512_ADDRESS, tmp << REG00_SGM41512_INPUT_CURRENT_LIMIT_SHIFT,
...
aicl_pre_step:
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, i,
		  sgm41512_usb_icl[i], sw_aicl_point);
	goto aicl_rerun;
aicl_end:
...
static ssize_t sgm41512_access_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t sgm41512_access_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct chip_sgm41512 *chip = dev_get_drvdata(dev);
	int ret = 0;
...
int sgm41512_kick_wdt(void)
{
	...
	rc = sgm41512_config_interface(chip, REG01_SGM41512_ADDRESS, REG01_SGM41512_WDT_TIMER_RESET,
...
int sgm41512_set_wdt_timer(int reg)
{
	...
	rc = sgm41512_config_interface(chip, REG05_SGM41512_ADDRESS, reg, REG05_SGM41512_WATCHDOG_TIMER_MASK);
	if (rc) {
...
int sgm41512_set_int_mask(int val)
{
	...
	rc = sgm41512_config_interface(chip, REG0A_SGM41512_ADDRESS, val,
...
int sgm41512_set_chg_timer(bool enable)
{
	...
	rc = sgm41512_config_interface(chip, REG05_SGM41512_ADDRESS,
				       enable ? REG05_SGM41512_CHG_SAFETY_TIMER_ENABLE :
						REG05_SGM41512_CHG_SAFETY_TIMER_DISABLE,
...
static bool sgm41512_chg_is_usb_present(void)
{
	...
	if ((oplus_ccdetect_check_is_gpio(g_oplus_chip) == true &&
	    ...
	    (oplus_ccdetect_check_is_gpio(g_oplus_chip) == false && oplus_get_otg_online_status())) {
#else
...
int sgm41512_charging_current_write_fast(int chg_cur)
{
	...
	rc = sgm41512_config_interface(chip, REG02_SGM41512_ADDRESS, tmp << REG02_SGM41512_FAST_CHG_CURRENT_LIMIT_SHIFT,
...
int sgm41512_float_voltage_write(int vfloat_mv)
{
	...
	rc = sgm41512_config_interface(chip, REG04_SGM41512_ADDRESS, tmp << REG04_SGM41512_CHG_VOL_LIMIT_SHIFT,
...
int sgm41512_set_termchg_current(int term_curr)
{
	...
	rc = sgm41512_config_interface(chip, REG03_SGM41512_ADDRESS, tmp << REG03_SGM41512_PRE_CHG_CURRENT_LIMIT_SHIFT,
...
int sgm41512_otg_ilim_set(int ilim)
{
	...
	rc = sgm41512_config_interface(chip, REG02_SGM41512_ADDRESS, ilim, REG02_SGM41512_OTG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
int sgm41512_set_rechg_voltage(int recharge_mv)
{
	...
	rc = sgm41512_config_interface(chip, REG04_SGM41512_ADDRESS, recharge_mv,
...
int sgm41512_reset_charger(void)
{
	...
	rc = sgm41512_config_interface(chip, REG0B_SGM41512_ADDRESS, REG0B_SGM41512_REG_RST_RESET,
...
int sgm41512_set_prechg_voltage_threshold(void)
{
	...
	rc = sgm41512_config_interface(chip, REG01_SGM41512_ADDRESS, REG01_SGM41512_SYS_VOL_LIMIT_3400MV,
...
int sgm41512_set_otg_voltage(void)
{
	...
	rc = sgm41512_config_interface(chip, REG06_SGM41512_ADDRESS, REG06_SGM41512_OTG_VLIM_5000MV,
...
int sgm41512_set_ovp(int val)
{
	...
	rc = sgm41512_config_interface(chip, REG06_SGM41512_ADDRESS, val, REG06_SGM41512_OVP_MASK);
...
int sgm41512_set_iindet(void)
{
	...
	rc = sgm41512_config_interface(chip, REG07_SGM41512_ADDRESS, REG07_SGM41512_IINDET_EN_MASK,
...
int sgm41512_set_shipmode(bool enable)
{
	...
	if (enable) {
		rc = sgm41512_config_interface(chip, REG07_SGM41512_ADDRESS, REG07_SGM41512_BATFET_DIS_ON,
					       REG07_SGM41512_BATFET_DIS_MASK);
		if (rc < 0) {
			chg_err("Couldn't set REG07_SGM41512_BATFET_DIS_ON rc = %d\n", rc);
		}
	} else {
		rc = sgm41512_config_interface(chip, REG07_SGM41512_ADDRESS, REG07_SGM41512_BATFET_DIS_OFF,
					       REG07_SGM41512_BATFET_DIS_MASK);
		if (rc < 0) {
...
static int sgm41512_parse_dt(struct chip_sgm41512 *chip)
{
	...
	if (!gpio_is_valid(chip->enable_gpio)) {
		...
		ret = gpio_request(chip->enable_gpio, "msg41512-enable-gpio");
		if (ret) {
...
static int sgm41512_irq_register(struct chip_sgm41512 *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, gpio_to_irq(chip->irq_gpio), NULL, sgm41512_irq_handler,
...
static int sgm41512_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct chip_sgm41512 *chip = NULL;
	int ret = 0;
	...
	chip->chg_dev = charger_device_register(chip->chg_dev_name, &client->dev, chip, &sgm41512_charger_ops,
						&sgm41512_chg_props);
