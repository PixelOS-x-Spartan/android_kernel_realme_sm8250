...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
void __attribute__((weak)) set_lpm_disallowed_flag(bool disable)
{
	return;
}
#endif
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (g_oplus_chip->vbatt_num == 2) {
			...
			if (((chg->svooc_detect_time - chg->svooc_detach_time) <= 1000) &&
			    oplus_vooc_get_detach_unexpectly() && g_oplus_chip->temperature <= 440 &&
			    g_oplus_chip->temperature >= 45) {
	...
	if (g_oplus_chip->hiz_gnd_cable && !vbus_rising) {
		...
		if (!ccdetect_level && fastchging) {
			...
			schedule_delayed_work(&chg->connect_check_work,
					      msecs_to_jiffies(OPLUS_CONNECT_CHK_INTERVAL_MS));
		} else if (!fastchging) {
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->dual_charger_support) {
		if ((chip->charger_volt > 7500 && chip->soc >= 90 &&
		...
		} else if (chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500 &&
			...
			oplus_chg_unsuspend_charger();
			   ...
			   (chip->suport_pd_9v2a == false ||
			    (chip->suport_pd_9v2a == true && chip->charger_volt > 7500))) {
			chip->chg_ops->input_current_write(500);
			oplus_chg_suspend_charger();
			oplus_chg_config_charger_vsys_threshold(0x03); //set Vsys Skip threshold 101%
			mutex_lock(&pd_select_pdo_v);
...
int oplus_chg_enable_qc_detect(void)
{
	...
	if (is_vooc_support_single_batt_svooc() == false && oplus_charger_ic_chip_is_null()) {
		return 0;
	}
...
int oplus_chg_set_qc_config()
{
	...
	if (chip->dual_charger_support) {
		...
		if ((chip->soc >= 90 && chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr) ||
		    chip->charging_state != CHARGING_STATUS_CCCV ||
		    chip->temperature >= chip->limits.tbatt_pdqc_to_5v_thr || chip->cool_down_force_5v ||
		    chg->qc_abnormal_check_count > 3) {
	...
	} else if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500 &&
		   chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr) {
		chip->chg_ops->input_current_write(500);
		oplus_chg_suspend_charger();
		oplus_chg_config_charger_vsys_threshold(0x03); //set Vsys Skip threshold 101%
...
static int smb5_batt_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PARALLEL_CURRENT_NOW:
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_PARALLEL_CURRENT_NOW, val);
		break;
#endif
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = smblib_get_iio_channel(chg, "usb_temp_l", &chg->iio.usbtemp_v_chan_l);

	rc = smblib_get_iio_channel(chg, "usb_temp_r", &chg->iio.usbtemp_v_chan_r);
#endif
