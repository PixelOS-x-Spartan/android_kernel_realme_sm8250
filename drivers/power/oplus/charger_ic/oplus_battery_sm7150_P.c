...
static int smb5_chg_config_init(struct smb5 *chip)
{
	...
	case PMI632_SUBTYPE:
		...
		chg->wa_flags |= (CHG_TERMINATION_WA | WEAK_ADAPTER_WA | USBIN_OV_WA);
...
module_param_named(pmic_register, pmic_register, int, 0600);

static int charge_debug_mask;
module_param_named(charge_debug_mask, charge_debug_mask, int, 0600);

static int charge_debug_delay = 5000;
module_param_named(charge_debug_delay, charge_debug_delay, int, 0600);
...
static void opluschg_debug_func(struct work_struct *w)
{
	...
	for (j = 0; j < 7; j++) {
		...
		for (i = 0; i < 16; i++) {
			...
			chg_debug("0x%04x [0x%02x 0x%02x 0x%02x 0x%02x], [0x%02x 0x%02x 0x%02x 0x%02x ], "
				  ...
				  stat[7], stat[8], stat[9], stat[10], stat[11], stat[12], stat[13], stat[14],
				  stat[15]);
		}
	}
	...
	if (charge_debug_mask & DEBUG_MASK_VOTER) {
		for (i = 0; i < chg->usb_icl_votable->num_clients; i++) {
			if (chg->usb_icl_votable->client_strs[i])
				chg_debug("%s: %s:\t\t\ten=%d v=%d\n", chg->usb_icl_votable->name,
					  chg->usb_icl_votable->client_strs[i], chg->usb_icl_votable->votes[i].enabled,
					  chg->usb_icl_votable->votes[i].value);
		}

		for (i = 0; i < chg->fcc_votable->num_clients; i++) {
			if (chg->fcc_votable->client_strs[i])
				chg_debug("%s: %s:\t\t\ten=%d v=%d\n", chg->fcc_votable->name,
					  chg->fcc_votable->client_strs[i], chg->fcc_votable->votes[i].enabled,
					  chg->fcc_votable->votes[i].value);
		}

		for (i = 0; i < chg->fv_votable->num_clients; i++) {
			if (chg->fv_votable->client_strs[i])
				chg_debug("%s: %s:\t\t\ten=%d v=%d\n", chg->fv_votable->name,
					  chg->fv_votable->client_strs[i], chg->fv_votable->votes[i].enabled,
					  chg->fv_votable->votes[i].value);
		}
	}
	...
	if (pmic_register == 1) {
		...
		chg_debug(
			"FCC 0x1061 = 0x%x, FV 0x1070 = 0x%x, ICL 0x1370 = 0x%x, OCL 0x1152 = 0x%x, OTG_ENABLE 0x1140 = 0x%x\n",
			stat[0], stat[1], stat[2], stat[3], stat[4]);
	}

	schedule_delayed_work(&opluschg_debug_work,
			      msecs_to_jiffies(charge_debug_delay > 1000 ? charge_debug_delay : 1000));
}
...
static int opluschg_get_charger_type(void)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_CDP ||
	    chg->real_charger_type == POWER_SUPPLY_TYPE_USB_DCP || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD)
...
static void opluschg_monitor_func(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger, opluschg_monitor_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	int boot_mode = get_boot_mode();
	static int counts = 0;
	int rechg_vol;
	...
	if (counts >= (chip->batt_full ? 8 : 3)) { //because rechg counts=6
		...
		if (rc < 0) {
			printk(KERN_ERR
			       "opluschg_monitor_func: Couldn't get BATTERY_CHARGER_STATUS_8_REG status rc=%d\n",
			       rc);
			goto rerun_work;
		}
...
static int opluschg_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		if (opluschg_usbtemp_condition() == true) {
			...
			resistance1 =
				voltage_convert_resistance(voltage1, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_51KOHM);
			resistance2 =
				voltage_convert_resistance(voltage2, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_51KOHM);
			...
			check_dischg_enable(temperature1, temperature2, critical_rise1, critical_rise2, bat_temp,
					    dischg_enable);

			if (usbtemp_debug == 1)
				chg_debug("[%d %d][%d %d %d %d][%d %d %d %d]\n", bat_temp, dischg_enable, voltage1,
					  resistance1, temperature1, critical_rise1, voltage2, resistance2,
					  temperature2, critical_rise2);
...
static int opluschg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	/* uart gpio */
	chip->normalchg_gpio.uart_bias_disable =
		pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, "uart_bias_disable");
	...
	chg_debug("usb_switch_1, dischg, ship_mode, shortc, ccdetect] = [%d, %d, %d, %d, %d]\n",
		  ...
		  chip->normalchg_gpio.ship_gpio, chip->normalchg_gpio.shortc_gpio, chg->ccdetect_gpio);
...
static int opluschg_get_chargerid(void)
{
	...
	if (boot_with_console() == false) {
		...
		pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
				     g_oplus_chip->normalchg_gpio.uart_bias_disable);
		mutex_unlock(&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
		msleep(10);
	}
...
static void opluschg_set_otg_switch(bool value)
{
	...
	if (g_oplus_chip->otg_switch == true)
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK, 0);
	else
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK,
					 EN_SNK_ONLY_BIT);
	if (rc < 0)
	...
	chg_debug("stat 0x1544 = 0x%x, otg_switch = %d, otg_online = %d, ui_otg_switch = %d\n", stat,
		  g_oplus_chip->otg_switch, g_oplus_chip->otg_online, g_oplus_chip->ui_otg_switch);
}
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip && g_oplus_chip->enable_shipmode) {
		...
		for (i = 0; i < 5; i++) {
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
					     g_oplus_chip->normalchg_gpio.ship_active);
			mdelay(3);
			pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
					     g_oplus_chip->normalchg_gpio.ship_sleep);
			mdelay(3);
		}
		mutex_unlock(&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	}
