...
int oplus_mt6765_pd_setup_forsvooc(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < PD_CHG_9V2A_VOLT_THRESHOLD &&
	    chip->soc < PD_CHG_SOC_THRESHOLD && chip->temperature <= PD_SVOOC_CHG_TEMP_THRESHOLD &&
		...
		if (chip->charger_volt > PD_CHG_5V3A_VOLT_THRESHOLD &&
		    (chip->calling_on || chip->soc >= PD_CHG_SOC_THRESHOLD || chip->camera_on ||
		     chip->batt_volt >= chip->limits.vbatt_pdqc_to_5v_thr ||
		     chip->temperature > PD_CHG_TEMP_THRESHOLD || chip->cool_down_force_5v == true)) {
			vbus_mv = VBUS_5V;
			ibus_ma = IBUS_3A;
...
int oplus_mt6765_pd_setup_forvooc(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd for vooc30_project [%d]", __func__, pinfo->pd_type, chip->batt_volt);
...
int oplus_chg_pd_setup(void)
{
	...
	if (oplus_chg_get_voocphy_support()) {
		...
		printk(KERN_ERR "%s: [%d-%d-%d-%d-%d-%d]", __func__, chip->calling_on, chip->camera_on,
		       chip->charger_volt, chip->soc, chip->temperature, chip->cool_down_force_5v);
		if (!chip->calling_on && !chip->camera_on && chip->charger_volt < PD_CHG_9V2A_VOLT_THRESHOLD &&
		    chip->soc < PD_CHG_SOC_THRESHOLD && chip->temperature <= PD_CHG_TEMP_THRESHOLD &&
			...
			if (chip->charger_volt > PD_CHG_5V3A_VOLT_THRESHOLD &&
			    (chip->calling_on || chip->camera_on || chip->soc >= PD_CHG_SOC_THRESHOLD ||
...
struct charger_consumer *mtk_charger_get_by_name(struct device *dev, const char *name)
{
	struct charger_consumer *puser;
...
int mtk_charger_enable_high_voltage_charging(struct charger_consumer *consumer, bool en)
{
...
int mtk_charger_enable_power_path(struct charger_consumer *consumer, int idx, bool en)
{
	int ret = 0;
	bool is_en = true;
	struct mtk_charger *info = consumer->cm;
...
static int _mtk_charger_enable_charging(struct charger_consumer *consumer, int idx, bool en)
{
	struct mtk_charger *info = consumer->cm;
...
int mtk_charger_enable_charging(struct charger_consumer *consumer, int idx, bool en)
{
	struct mtk_charger *info = consumer->cm;
	int ret = 0;
...
int mtk_charger_set_input_current_limit(struct charger_consumer *consumer, int idx, int input_current)
{
	struct mtk_charger *info = consumer->cm;
...
int mtk_charger_set_charging_current_limit(struct charger_consumer *consumer, int idx, int charging_current)
{
	struct mtk_charger *info = consumer->cm;
...
int mtk_charger_get_charger_temperature(struct charger_consumer *consumer, int idx, int *tchg_min, int *tchg_max)
{
	struct mtk_charger *info = consumer->cm;
...
int mtk_charger_force_charging_current(struct charger_consumer *consumer, int idx, int charging_current)
{
	struct mtk_charger *info = consumer->cm;
...
int mtk_charger_enable_chg_type_det(struct charger_consumer *consumer, bool en)
{
	struct mtk_charger *info = consumer->cm;
	struct charger_device *chg_dev;
	int ret = 0;
...
int register_mtk_charger_notifier(struct charger_consumer *consumer, struct notifier_block *nb)
{
	int ret = 0;
	struct mtk_charger *info = consumer->cm;
...
int unregister_mtk_charger_notifier(struct charger_consumer *consumer, struct notifier_block *nb)
{
	int ret = 0;
	struct mtk_charger *info = consumer->cm;
...
static void mtk_charger_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		ret = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_TYPE, &type_prop);
	}

	pr_notice("%s event, name:%s online:%d usb_type:%d type:%d vbus:%d\n", __func__, psy->desc->name, prop.intval,
		  prop2.intval, type_prop.intval, get_vbus(info));
...
static int notify_adapter_event(struct notifier_block *notifier, unsigned long evt, void *unused)
{
...
int oplus_usb_switch_gpio_gpio_init(void)
{
	...
	g_oplus_chip->normalchg_gpio.charger_gpio_as_output1 =
		pinctrl_lookup_state(g_oplus_chip->normalchg_gpio.pinctrl, "charger_gpio_as_output_low");
	if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.charger_gpio_as_output1)) {
		chg_err("get charger_gpio_as_output_low fail\n");
		return -EINVAL;
	}
	g_oplus_chip->normalchg_gpio.charger_gpio_as_output2 =
		pinctrl_lookup_state(g_oplus_chip->normalchg_gpio.pinctrl, "charger_gpio_as_output_high");
...
struct USB_PORT_TEMPERATURE Usb_Port_Temperature_Table[USB_NTC_TABLE_SIZE] = {
	...
	{ -400, 4397119 }, { -350, 3088599 }, { -300, 2197225 }, { -250, 1581881 }, { -200, 1151037 }, { -150, 846579 },
	{ -100, 628988 },  { -50, 471632 },   { 0, 357012 },	 { 50, 272500 },    { 100, 209710 },   { 150, 162651 },
	{ 200, 127080 },   { 250, 100000 }, /* 100K */
	{ 300, 79222 },	   { 350, 63167 },    { 400, 50677 },	 { 410, 48528 },    { 420, 46482 },    { 430, 44533 },
	{ 440, 42675 },	   { 450, 40904 },    { 460, 39213 },	 { 470, 37601 },    { 480, 36063 },    { 490, 34595 },
	{ 500, 33195 },	   { 510, 31859 },    { 520, 30584 },	 { 530, 29366 },    { 540, 28203 },    { 550, 27091 },
	{ 560, 26028 },	   { 570, 25013 },    { 580, 24042 },	 { 590, 23113 },    { 600, 22224 },    { 610, 21374 },
	{ 620, 20560 },	   { 630, 19782 },    { 640, 19036 },	 { 650, 18322 },    { 660, 17640 },    { 670, 16986 },
	{ 680, 16360 },	   { 690, 15759 },    { 700, 15184 },	 { 710, 14631 },    { 720, 14100 },    { 730, 13591 },
	{ 740, 13103 },	   { 750, 12635 },    { 760, 12187 },	 { 770, 11756 },    { 780, 11343 },    { 790, 10946 },
	{ 800, 10565 },	   { 810, 10199 },    { 820, 9847 },	 { 830, 9509 },	    { 840, 9184 },     { 850, 8872 },
	{ 860, 8572 },	   { 870, 8283 },     { 880, 8005 },	 { 890, 7738 },	    { 900, 7481 },     { 950, 6337 },
	{ 1000, 5384 },	   { 1050, 4594 },    { 1100, 3934 },	 { 1150, 3380 },    { 1200, 2916 },    { 1250, 2522 }
};
...
int usb_port_volt_to_temp(int volt)
{
	...
	if (usb_r >= Usb_Port_Temperature_Table[0].temperature_r) {
		...
		usb_temp_value = (((usb_r - res2) * tmp1) + ((res1 - usb_r) * tmp2)) / (res1 - res2);
	}

	USBTEMP_LOG_PRINT("[%s] %d %d %d %d %d %d\n", __func__, res1, res2, usb_r, tmp1, tmp2, usb_temp_value);
...
{
	struct mtk_charger *info = container_of(nb, struct mtk_charger, psy_nb);
...
static void mtk_charger_start_timer(struct mtk_charger *info)
{
	...
	chg_err("%s: alarm timer start:%ld %ld\n", __func__, info->endtime.tv_sec, info->endtime.tv_nsec);
...
static int mtk_charger_parse_dt(struct mtk_charger *info, struct device *dev)
{
...
static ssize_t mtk_chg_current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
			...
			mtk_charger_notifier(info, CHARGER_NOTIFY_STOP_CHARGING);
		} else if (cmd_discharging == 0) {
			info->cmd_discharging = false;
			charger_dev_enable(info->chg1_dev, true);
			mtk_charger_notifier(info, CHARGER_NOTIFY_START_CHARGING);
		}
...
static enum alarmtimer_restart recover_charge_hrtimer_func(struct alarm *alarm, ktime_t now)
{
...
void oplus_wake_up_usbtemp_thread(void)
{
	if (oplus_usbtemp_check_is_support() == true) {
		...
		chg_debug("wake_up_usbtemp_thread, vbus:%d, otg:%d", oplus_chg_get_vbus_status(g_oplus_chip),
			  oplus_chg_get_otg_online());
	}
}
...
static int oplus_usb_set_property(struct power_supply *psy, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	int ret = 0;
...
static int oplus_usb_property_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
	default:
...
static int oplus_chg_parse_charger_dt_2nd_override(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,temp_cool_vfloat_mv_2nd", &chip->limits.temp_cool_vfloat_mv);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_vfloat_mv_2nd", &chip->limits.temp_normal_vfloat_mv);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,cool_vfloat_over_sw_limit_2nd", &chip->limits.cool_vfloat_over_sw_limit);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,default_iterm_ma_2nd", &chip->limits.default_iterm_ma);
	if (rc < 0) {
...
int mtk_pdc_get_max_watt(struct mtk_charger *info)
{
	...
	if (info->pdc.pdc_max_watt_setting != -1)
		...
		if (info->chg_data[CHG1_SETTING].thermal_charging_current_limit != -1)
			charging_current = info->chg_data[CHG1_SETTING].thermal_charging_current_limit / 1000;

		info->pdc.pdc_max_watt = vbat * charging_current;
	}
	chg_err("[%s]watt:%d:%d vbat:%d c:%d=>\n", __func__, info->pdc.pdc_max_watt_setting, info->pdc.pdc_max_watt,
		vbat, charging_current);
...
void pmic_set_register_value(struct regmap *map, unsigned int addr, unsigned int mask, unsigned int shift,
			     unsigned int val)
{
...
static ssize_t gsm_pmic_pwm_node_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (strncmp(upmu_vs1_mode, "1", 1) == 0) {
		pmic_set_register_value(pmic_chip->regmap, MT6357_VS1_ANA_CON2, PMIC_RG_VS1_MODESET_MASK,
					PMIC_RG_VS1_MODESET_SHIFT, FORCE_PWM);
	} else if (strncmp(upmu_vs1_mode, "0", 1) == 0) {
		pmic_set_register_value(pmic_chip->regmap, MT6357_VS1_ANA_CON2, PMIC_RG_VS1_MODESET_MASK,
					PMIC_RG_VS1_MODESET_SHIFT, AUTO_MODE);
	} else
		chg_err("gsm_pmic_pwm_node_write parameter error.\n");
...
{
	if (!proc_create("gsm_pmic_pwm", S_IWUSR | S_IWGRP | S_IWOTH, NULL, &gsm_pmic_pwm_ctrl)) {
