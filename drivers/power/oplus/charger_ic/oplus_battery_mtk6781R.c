...
extern int mt6370_pmu_reg_update_bits(struct mt6370_pmu_chip *chip, u8 addr, u8 mask, u8 data);
	...
	if (pinfo->data.dual_charger_support) {
		if (g_oplus_chip->dual_charger_support && g_oplus_chip->slave_charger_enable && (pdqc_flag == true)) {
			...
			chg_debug("usb input max current limit aicl: master and salve input current: %d, %d\n",
				  charger_current_set_pre * 1000 * (100 - g_oplus_chip->slave_pct) / 100,
				  charger_current_set_pre * 1000 * g_oplus_chip->slave_pct / 100);
			bq25601d_input_current_limit_write(charger_current_set_pre * 1000 * g_oplus_chip->slave_pct /
							   100);
			rc = charger_dev_set_input_current(chg, charger_current_set_pre * 1000 *
									(100 - g_oplus_chip->slave_pct) / 100);
		} else
			rc = charger_dev_set_input_current(chg, charger_current_set_pre * 1000);
	} else {
		rc = charger_dev_set_input_current(chg, charger_current_set_pre * 1000);
...
aicl_pre_step:
	...
	if (pinfo->data.dual_charger_support) {
		if (g_oplus_chip->dual_charger_support && g_oplus_chip->slave_charger_enable && (pdqc_flag == true)) {
			...
			msleep(500);
			chg_debug("usb input max current limit aicl: master and salve input current: %d, %d\n",
				  usb_icl[i] * 1000 * (100 - g_oplus_chip->slave_pct) / 100,
				  usb_icl[i] * 1000 * g_oplus_chip->slave_pct / 100);
			bq25601d_input_current_limit_write(usb_icl[i] * 1000 * g_oplus_chip->slave_pct / 100);
			rc = charger_dev_set_input_current(chg,
							   usb_icl[i] * 1000 * (100 - g_oplus_chip->slave_pct) / 100);
		} else
			rc = charger_dev_set_input_current(chg, usb_icl[i] * 1000);
	} else {
...
static int oplus_mt6370_pd_setup(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 &&
	    chip->soc < 90
	    /*&& chip->temperature <= 420 */
	    && chip->cool_down_force_5v == false && chip->batt_volt < 4550) {
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
...
int oplus_chg_set_qc_config(void)
{
	...
	chg_err("%s:charger_volt:%d,temperature:%d,cool_down_force_5v:%d,batt_volt:%d\n", __func__, chip->charger_volt,
		chip->temperature, chip->cool_down_force_5v, chip->batt_volt);
	if (!chip->calling_on && !chip->camera_on && chip->soc < 90 && chip->batt_volt < 4550 &&
	    chip->temperature <= 420 &&
