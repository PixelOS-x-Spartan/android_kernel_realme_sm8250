...
int sy6974b_set_vindpm_vol(int vol)
{
	...
	rc = sy6974b_config_interface(chip, REG06_SY6974B_ADDRESS, tmp << REG06_SY6974B_VINDPM_SHIFT,
...
int sy6974b_input_current_limit_without_aicl(int current_ma)
{
	...
	rc = sy6974b_config_interface(chip, REG00_SY6974B_ADDRESS, tmp << REG00_SY6974B_INPUT_CURRENT_LIMIT_SHIFT,
...
aicl_pre_step:
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, i,
		  sy6974b_usb_icl[i], sw_aicl_point);
	goto aicl_rerun;
aicl_end:
...
static ssize_t sy6974b_access_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t sy6974b_access_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct chip_sy6974b *chip = dev_get_drvdata(dev);
	int ret = 0;
...
void sy6974b_dump_registers_debug(void)
{
	...
	if (g_oplus_chip->charger_exist || g_oplus_chip->ac_online || (sy6974b_debug & ENABLE_DUMP_LOG))
...
int sy6974b_kick_wdt(void)
{
	...
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_WDT_TIMER_RESET,
...
int sy6974b_set_wdt_timer(int reg)
{
	...
	rc = sy6974b_config_interface(chip, REG05_SY6974B_ADDRESS, reg, REG05_SY6974B_WATCHDOG_TIMER_MASK);
	if (rc) {
...
int sy6974b_set_int_mask(int val)
{
	...
	rc = sy6974b_config_interface(chip, REG0A_SY6974B_ADDRESS, val,
...
bool oplus_chg_is_usb_present(void)
{
	...
	if (chip && atomic_read(&chip->driver_suspended) == 1 && g_oplus_chip && g_oplus_chip->unwakelock_chg == 1 &&
...
int sy6974b_charging_current_write_fast(int chg_cur)
{
	...
	rc = sy6974b_config_interface(chip, REG02_SY6974B_ADDRESS, tmp << REG02_SY6974B_FAST_CHG_CURRENT_LIMIT_SHIFT,
...
int sy6974b_float_voltage_write(int vfloat_mv)
{
	...
	rc = sy6974b_config_interface(chip, REG04_SY6974B_ADDRESS, tmp << REG04_SY6974B_CHG_VOL_LIMIT_SHIFT,
...
int sy6974b_set_termchg_current(int term_curr)
{
	...
	rc = sy6974b_config_interface(chip, REG03_SY6974B_ADDRESS, tmp << REG03_SY6974B_TERM_CHG_CURRENT_LIMIT_SHIFT,
...
int sy6974b_otg_ilim_set(int ilim)
{
	...
	rc = sy6974b_config_interface(chip, REG02_SY6974B_ADDRESS, ilim, REG02_SY6974B_OTG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
int sy6974b_otg_enable(void)
{
	...
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_OTG_ENABLE, REG01_SY6974B_OTG_MASK);
	if (rc < 0) {
...
int sy6974b_otg_disable(void)
{
	...
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_OTG_DISABLE, REG01_SY6974B_OTG_MASK);
	if (rc < 0) {
...
int sy6974b_enable_charging(void)
{
	...
	if (!g_oplus_chip || !g_oplus_chip->otg_online)
		sy6974b_otg_disable();
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_CHARGING_ENABLE,
...
int sy6974b_disable_charging(void)
{
	...
	if (!g_oplus_chip || !g_oplus_chip->otg_online)
		sy6974b_otg_disable();
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_CHARGING_DISABLE,
				      REG01_SY6974B_CHARGING_MASK);
	if (rc < 0) {
...
int sy6974b_unsuspend_charger(void)
{
	...
	rc = sy6974b_config_interface(chip, REG00_SY6974B_ADDRESS, REG00_SY6974B_SUSPEND_MODE_DISABLE,
				      REG00_SY6974B_SUSPEND_MODE_MASK);
	if (rc < 0) {
...
int sy6974b_set_rechg_voltage(int recharge_mv)
{
	...
	rc = sy6974b_config_interface(chip, REG04_SY6974B_ADDRESS, recharge_mv, REG04_SY6974B_RECHG_THRESHOLD_VOL_MASK);
...
int sy6974b_reset_charger(void)
{
	...
	rc = sy6974b_config_interface(chip, REG0B_SY6974B_ADDRESS, REG0B_SY6974B_REG_RST_RESET,
...
int sy6974b_set_chging_term_disable(void)
{
	...
	rc = sy6974b_config_interface(chip, REG05_SY6974B_ADDRESS, REG05_SY6974B_TERMINATION_DISABLE,
...
int sy6974b_set_prechg_voltage_threshold(void)
{
	...
	rc = sy6974b_config_interface(chip, REG01_SY6974B_ADDRESS, REG01_SY6974B_SYS_VOL_LIMIT_3400MV,
...
int sy6974b_set_otg_voltage(void)
{
	...
	rc = sy6974b_config_interface(chip, REG06_SY6974B_ADDRESS, REG06_SY6974B_OTG_VLIM_5150MV,
...
int sy6974b_set_ovp(int val)
{
	...
	rc = sy6974b_config_interface(chip, REG06_SY6974B_ADDRESS, val, REG06_SY6974B_OVP_MASK);
...
int sy6974b_set_iindet(void)
{
	...
	msleep(10);
#endif
	rc = sy6974b_config_interface(chip, REG07_SY6974B_ADDRESS, REG07_SY6974B_IINDET_EN_MASK,
...
__attribute__((unused)) static int sy6974b_set_chargevolt(u32 volt)
{
	...
	return sy6974b_config_interface(chip, REG04_SY6974B_ADDRESS, val << REG04_SY6974B_CHG_VOL_LIMIT_SHIFT,
					REG04_SY6974B_CHG_VOL_LIMIT_MASK);
}
...
static irqreturn_t sy6974b_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&chip->driver_suspended) == 1) {
		...
		wait_event_interruptible_timeout(chip->wait, atomic_read(&chip->driver_suspended) == 0,
...
static int sy6974b_irq_register(struct chip_sy6974b *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, gpio_to_irq(chip->sy6974b_irq_gpio), NULL, sy6974b_irq_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "sy6974b-eint", chip);
	if (ret < 0) {
...
static char *sy6974b_charger_supplied_to[] = { "battery", "mtk-master-charger" };
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct chip_sy6974b *chip = (struct chip_sy6974b *)container_of(nb, struct chip_sy6974b, pd_nb);
...
static int sy6974b_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct chip_sy6974b *chip = NULL;
	int ret = 0;
	...
	chip->bc12_retried = 0;
	chip->bc12_delay_cnt = 0;
	chip->chg_consumer = charger_manager_get_by_name(&client->dev, "sy6974b");
	...
	chip->chg_dev =
		charger_device_register(chip->chg_dev_name, &client->dev, chip, &sy6974b_chg_ops, &sy6974b_chg_props);
	...
	if (oplus_daily_build() || get_eng_version() == HIGH_TEMP_AGING || get_eng_version() == AGING)
