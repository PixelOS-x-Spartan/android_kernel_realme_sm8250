...
static void oplus_otg_init_status_func(struct work_struct *work)
{
	...
	while (count--) {
		if (bcdev->is_chargepd_ready && (g_oplus_chip->wireless_support && !oplus_wpc_check_chip_is_null()))
			break;
...
static void oplus_oem_lcm_en_check_work(struct work_struct *work)
{
	...
	vph_track_high =
		((chip->sw_full || chip->hw_full_by_sw || chip->charging_state == CHARGING_STATUS_FAIL) ? 1 : 0);
...
static void oplus_adsp_crash_recover_func(struct work_struct *work)
{
	...
	schedule_delayed_work(&bcdev->reset_turn_on_chg_work,
			      round_jiffies_relative(msecs_to_jiffies(RESET_TURN_ON_TIMES)));
	if (oplus_ccdetect_check_is_gpio(chip) == true) {
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	if (g_oplus_chip && usb_psy_desc.type != POWER_SUPPLY_TYPE_UNKNOWN) {
	...
	if (g_oplus_chip && usb_psy_desc.type == POWER_SUPPLY_TYPE_UNKNOWN &&
	    (oplus_chg_get_voocphy_support() == NO_VOOCPHY || oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY ||
	     oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY)) {
		...
		if (oplus_vooc_get_fastchg_to_warm() == true || oplus_vooc_get_fastchg_dummy_started() == true ||
	...
	if (pst->prop[USB_ADAP_TYPE] != POWER_SUPPLY_USB_TYPE_UNKNOWN) {
		oplus_chg_wake_update_work();
		if (g_oplus_chip && oplus_chg_get_charger_voltage() >= 0 &&
		    oplus_chg_get_charger_voltage() < RESET_CURRENT_LIMIT_VOLT &&
		    oplus_chg_get_voocphy_support() == ADSP_VOOCPHY && oplus_vooc_get_fastchg_dummy_started() == true) {
...
{
	if (g_oplus_chip && g_oplus_chip->chg_ops->check_chrdet_status()) {
...
void oplus_adsp_voocphy_set_full_para_qbg(int full_volt_curr)
{
	...
	chg_err("ap set  full_voltage[%d],cuurent[%d] to adsp qbg\n", ((full_volt_curr & 0xFFFF0000) >> 16),
		(full_volt_curr & 0x0000FFFF));
}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, check_charger_type_work);
	static int count = 1;
...
static int usb_psy_get_prop(struct power_supply *psy, enum power_supply_property prop, union power_supply_propval *pval)
{
	...
	if (prop_id == USB_ONLINE) {
		if (oplus_vchg_trig_is_support() == true && oplus_get_vchg_trig_status() == 1 && pval->intval == 1) {
	...
	if (prop_id == USB_ADAP_TYPE) {
		if (oplus_vchg_trig_is_support() == true && oplus_get_vchg_trig_status() == 1 && adap_type != 0) {
		...
		if (adap_type ^ pval->intval) {
			...
			if (oplus_chg_get_voocphy_support() == NO_VOOCPHY ||
				...
				printk(KERN_ERR
				       "!!! usb adap type[%d]intval[%d]to_warm[%d]dummy[%d]to_normal[%d]started[%d]\n",
				       adap_type, pval->intval, oplus_vooc_get_fastchg_to_warm(),
...
static void oplus_vchg_trig_work(struct work_struct *work)
{
	...
	if (oplus_voocphy_get_fastchg_to_warm() == false && oplus_voocphy_get_fastchg_to_normal() == false &&
	    oplus_voocphy_get_fastchg_dummy_start() == false) {
		oplus_chg_wake_update_work();
	}
}
...
void oplus_chg_parse_usbtemp_dt(struct oplus_chg_chip *chip)
{
	...
	if (rc < 0 || rc > 3)
		...
		length = rc;
		rc = of_property_read_u32_array(node, "temp_curr_monitor_table", (u32 *)temp_curr_monitor_table,
						length);
		if (rc)
			...
			for (i = 0; i < length / 3; i++) {
				pr_err("temp_curr_monitor_table[%d, %d, %d]\n", temp_curr_monitor_table[0].batt_curr,
				       temp_curr_monitor_table[0].temp_delta, temp_curr_monitor_table[0].reset_time);
			}
		}
	}
	return;
...
bool usbtemp_check_curr_temp(int delay)
{
	...
	if (batt_current >= temp_curr_monitor_table[0].batt_curr) {
		...
		time_count = 0;
	} else if (batt_current <= temp_curr_monitor_table[0].batt_curr && usbtemp_check_status) {
		time_count += delay;
	}

	if (usbtemp_check_status && time_count > temp_curr_monitor_table[0].reset_time) {
	...
	dischg:
		...
		if (usbtemp_recover_test || ((condition1 == true || condition2 == true) && chip->dischg_flag == true)) {
			...
			if (bcdev->pmic_is_pm7250b == false) {
				oplus_set_usbtemp_wakelock(true);
				cancel_delayed_work(&bcdev->usbtemp_recover_work);
				schedule_delayed_work(&bcdev->usbtemp_recover_work,
						      msecs_to_jiffies(usbtemp_recover_interval));
			}
...
aicl_boost_back:
	...
	chg_debug(
		"aicl_boost_back: usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_boost_back\n",
		chg_vol, i, usb_icl[i], aicl_point);
	goto aicl_return;
aicl_return:
	return rc;
...
get_type_done:
	if ((charger_type == POWER_SUPPLY_TYPE_USB_CDP || charger_type == POWER_SUPPLY_TYPE_USB) && force_dcp) {
		charger_type = POWER_SUPPLY_TYPE_USB_DCP;
		usb_psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
	}
...
bool oplus_chg_is_usb_present(void)
{
	...
	if ((oplus_chg_get_voocphy_support() == NO_VOOCPHY || oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY) &&
...
static void usb_enum_check(struct work_struct *work)
{
	...
	case CHECK_CHARGER_EXIST:
		...
		if (g_oplus_chip->charger_exist) {
			...
			schedule_delayed_work(&g_oplus_chip->pmic_spmi.bcdev_chip->usb_enum_check_work,
