...
#ifdef OPLUS_FEATURE_CHG_BASIC
			if (chg_curr > pinfo->data.step3_current_ma &&
			    pinfo->step_chg_current >= pinfo->data.step3_current_ma)
#else
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		if (chip->charger_volt > 7500 && (chip->calling_on || chip->soc >= 90 || chip->camera_on ||
...
static void oplus_chg_pps_get_source_cap(struct charger_manager *info)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		while (1) {
			...
			chg_err("pps_boundary[%d], %d mv ~ %d mv, %d ma pl:%d", cap_i, apdo_cap.min_mv, apdo_cap.max_mv,
		...
		for (i = 0; i < info->srccap.nr; i++) {
			chg_err("pps_cap[%d:%d], %d mv ~ %d mv, %d ma pl:%d pdp:%d", i, (int)info->srccap.nr,
		...
		if (cap_i == 0 || info->srccap.nr == 0)
			...
			oplus_pps_set_power(OPLUS_PPS_POWER_THIRD, info->srccap.ma[info->srccap.nr - 1],
...
static void oplus_chg_batcon_adc_switch_ctrl(struct oplus_chg_chip *chip)
{
	...
	chr_debug("chargeric_temp_cal=%d, battery_btb_temp_cal=%d, flashlight_temp_uv_cal=%d\n", chargeric_temp_cal,
		  battery_btb_temp_cal, flashlight_temp_uv_cal);
}
