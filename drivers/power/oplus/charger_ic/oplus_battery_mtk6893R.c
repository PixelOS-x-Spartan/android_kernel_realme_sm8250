...
int charger_manager_enable_high_voltage_charging(struct charger_consumer *consumer, bool en)
{
	...
	if (!en && consumer->hv_charging_disabled == false)
		...
		pr_info("[%s] already set: %d %d\n", __func__, consumer->hv_charging_disabled, en);
		return 0;
	}
	...
	pr_info("%s: user: %s, en = %d\n", __func__, dev_name(consumer->dev), info->enable_hv_charging);
...
int charger_manager_enable_power_path(struct charger_consumer *consumer, int idx, bool en)
{
	int ret = 0;
	bool is_en = true;
	struct charger_manager *info = consumer->cm;
...
static int _charger_manager_enable_charging(struct charger_consumer *consumer, int idx, bool en)
{
	struct charger_manager *info = consumer->cm;
...
int charger_manager_enable_charging(struct charger_consumer *consumer, int idx, bool en)
{
	struct charger_manager *info = consumer->cm;
	int ret = 0;
...
int charger_manager_set_input_current_limit(struct charger_consumer *consumer, int idx, int input_current)
{
	struct charger_manager *info = consumer->cm;
	...
	if (info != NULL) {
		...
		if (info->data.parallel_vbus) {
			if (idx == TOTAL_CHARGER) {
				info->chg1_data.thermal_input_current_limit = input_current;
				info->chg2_data.thermal_input_current_limit = input_current;
			} else
				return -ENOTSUPP;
		} else {
			if (idx == MAIN_CHARGER)
		...
		chr_err("%s: dev:%s idx:%d en:%d\n", __func__, dev_name(consumer->dev), idx, input_current);
...
int charger_manager_set_charging_current_limit(struct charger_consumer *consumer, int idx, int charging_current)
{
	struct charger_manager *info = consumer->cm;
	...
	if (info != NULL) {
		...
		chr_err("%s: dev:%s idx:%d en:%d\n", __func__, dev_name(consumer->dev), idx, charging_current);
		_mtk_charger_change_current_setting(info);
		_wake_up_charger(info);
		return 0;
	}
...
int charger_manager_force_charging_current(struct charger_consumer *consumer, int idx, int charging_current)
{
	struct charger_manager *info = consumer->cm;
...
int charger_manager_enable_chg_type_det(struct charger_consumer *consumer, bool en)
{
	...
	if (info != NULL) {
		...
		chr_err("%s: chg%d is doing bc12\n", __func__, info->data.bc12_charger + 1);
...
int unregister_charger_manager_notifier(struct charger_consumer *consumer, struct notifier_block *nb)
{
	int ret = 0;
	struct charger_manager *info = consumer->cm;
...
void charger_ftm_init(void)
{
	...
	class_dev = (struct class_device *)device_create(charger_class, NULL, charger_devno, NULL, CHARGER_DEVNAME);
	if (IS_ERR(class_dev)) {
...
bool is_typec_adapter(struct charger_manager *info)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK && rp != 500 && info->chr_type != STANDARD_HOST &&
	    info->chr_type != CHARGING_HOST && mtk_pe20_get_is_connect(info) == false &&
	    mtk_pe_get_is_connect(info) == false && info->enable_type_c == true)
		return true;
...
static ssize_t store_sw_jeita(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	signed int temp;
...
static ssize_t store_pe20(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	signed int temp;
...
static ssize_t store_pe40(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	signed int temp;
...
static ssize_t store_charger_log_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret = 0;
...
static ssize_t store_pdc_max_watt_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	int temp;
...
int charger_psy_event(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (strcmp(psy->desc->name, "battery") == 0) {
		...
		if (!ret) {
			tmp = val.intval / 10;
			if (info->battery_temp != tmp && mt_get_charger_type() != CHARGER_UNKNOWN) {
				_wake_up_charger(info);
				chr_err("%s: %ld %s tmp:%d %d chr:%d\n", __func__, event, psy->desc->name, tmp,
					info->battery_temp, mt_get_charger_type());
			}
		}
	}
		...
		if (mt_get_charger_type() != CHARGER_UNKNOWN) {
			...
			if (mtkhv_flashled_pinctrl.hv_flashled_support) {
				...
				if (mt6360_get_vbus_rising() != true) {
					...
					pr_err("[OPLUS_CHG] chgvin_gpio = %d",
...
static void check_dynamic_mivr(struct charger_manager *info)
{
	...
	if (info->enable_dynamic_mivr) {
		if (!mtk_pe40_get_is_connect(info) && !mtk_pe20_get_is_connect(info) && !mtk_pe_get_is_connect(info) &&
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d\n", temperature,
		info->enable_sw_jeita, info->sw_jeita.sm, info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout, info->vbusov_stat, info->sc.disable_charger,
...
static int charger_pm_event(struct notifier_block *notifier, unsigned long pm_event, void *unused)
{
	...
	case PM_POST_SUSPEND:
		...
		if (timespec_compare(&now, &pinfo->endtime) >= 0 && pinfo->endtime.tv_sec != 0 &&
...
{
	struct charger_manager *info = container_of(alarm, struct charger_manager, charger_timer);
	unsigned long flags;
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		chr_err("Vbat=%d,Ibat=%d,I=%d,VChr=%d,T=%d,Soc=%d:%d,CT:%d:%d hv:%d pd:%d:%d\n",
			battery_get_bat_voltage(), bat_current, chg_current, battery_get_vbus(),
			battery_get_bat_temperature(), battery_get_soc(), battery_get_uisoc(), mt_get_charger_type(),
			info->chr_type, info->enable_hv_charging, info->pd_type, info->pd_reset);
...
static int oplus_step_charging_parse_dt(struct charger_manager *info, struct device *dev)
{
	struct device_node *np = dev->of_node;
	u32 val;
	...
	chg_err("step1_time: %d, step1_current: %d, step2_time: %d, step2_current: %d, step3_current: %d\n",
		info->data.step1_time, info->data.step1_current_ma, info->data.step2_time, info->data.step2_current_ma,
		info->data.step3_current_ma);
...
static ssize_t show_Pump_Express(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_debug("[%s] chr_type:%d UISOC:%d startsoc:%d stopsoc:%d\n", __func__, mt_get_charger_type(),
		 battery_get_uisoc(), pinfo->data.ta_start_battery_soc, pinfo->data.ta_stop_battery_soc);
	...
	pr_debug("%s: detected = %d, pe20_connect = %d, pe_connect = %d\n", __func__, is_ta_detected,
		 mtk_pe20_get_is_connect(pinfo), mtk_pe_get_is_connect(pinfo));
...
static ssize_t store_input_current(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	int ret;
...
static ssize_t store_chg1_current(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	int ret;
...
static ssize_t store_chg2_current(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	int ret;
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	proc_create_data("current_cmd", 0640, battery_dir, &mtk_chg_current_cmd_fops, info);
	proc_create_data("en_power_path", 0640, battery_dir, &mtk_chg_en_power_path_fops, info);
	proc_create_data("en_safety_timer", 0640, battery_dir, &mtk_chg_en_safety_timer_fops, info);
...
static void oplus_mt6360_dump_registers(void)
{
	...
	if (musb_hdrc_release == false && g_oplus_chip->unwakelock_chg == 1 &&
		...
		if (musb_hdrc_release == true && g_oplus_chip->unwakelock_chg == 0 &&
		    mt_get_charger_type() == NONSTANDARD_CHARGER) {
...
static int oplus_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (g_oplus_chip->dual_charger_support && (g_oplus_chip->slave_charger_enable || em_mode)) {
...
void oplus_mt_power_off(void)
{
	...
	if (g_oplus_chip->ac_online != true) {
		...
		if (tcpc_dev) {
			if (!(tcpc_dev->pd_wait_hard_reset_complete) && !oplus_mt_get_vbus_status()) {
				kernel_power_off();
			}
		}
	} else {
...
static struct temp_param battery_temp_table[] = {
	...
	{ 44, 42670 },	  { 45, 40900 },    { 46, 39210 },    { 47, 37601 },	{ 48, 36063 },	  { 49, 34595 },
...
int oplus_force_get_battery_temp(void)
{
	...
	if (!is_param_init) {
		...
		if (of_property_read_bool(g_oplus_chip->dev->of_node, "qcom,sub_board_pull_up_r_support")) {
			if (of_property_read_u32(g_oplus_chip->dev->of_node, "qcom,sub_board_pull_up_r", &pull_up_r) >=
			    0) {
...
int oplus_mt6360_pd_setup_forvoocphy(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd9v svooc [%d %d %d %d %d]", __func__, pinfo->pd_type,
	       chip->limits.vbatt_pdqc_to_9v_thr, chip->limits.vbatt_pdqc_to_5v_thr, chip->batt_volt, chip->camera_on,
	       chip->charger_volt);
	...
	if (is_mtksvooc_project) {
		...
		if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
			...
			if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
				...
				for (i = 0; i < cap.nr; i++) {
					printk(KERN_ERR "PD cap %d: mV:%d,%d mA:%d type:%d\n", i, cap.max_mv[i],
					       cap.min_mv[i], cap.ma[i], cap.type[i]);
				}
...
int oplus_chg_set_qc_config_forvoocphy(void)
{
	...
	chip->charger_volt = battery_meter_get_charger_voltage();
	printk(KERN_ERR "%s: qc9v svooc [%d %d %d %d %d]", __func__, chip->limits.vbatt_pdqc_to_9v_thr,
	       chip->limits.vbatt_pdqc_to_5v_thr, chip->batt_volt, chip->camera_on, chip->charger_volt);
	chip->charger_volt = chip->chg_ops->get_charger_volt();
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
	    chip->temperature <= 420 && chip->cool_down_force_5v == false) {
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90 && chip->temperature <= 530 &&
		...
		if (is_vooc_support_single_batt_svooc() == true) {
			...
			mt6360_set_register(MT6360_PMU_DPDM_CTRL, 0x1F,
					    0x15); /*Before request 9V, need to force 5V first.*/
			msleep(300);
			oplus_chg_suspend_charger();
			oplus_chg_config_charger_vsys_threshold(0x02); /*set Vsys Skip threshold 104%*/
			oplus_chg_enable_burst_mode(false);
	...
	if (pinfo->data.dual_charger_support) {
		if (chip->tbatt_status == BATTERY_STATUS__NORMAL) {
			...
			if (pinfo->step_status != pinfo->step_status_pre) {
				pr_err("%s, step status: %d, step charging current: %d\n", __func__, pinfo->step_status,
				       pinfo->step_chg_current);
...
void oplus_chg_set_camera_on(bool val)
{
	if (!g_oplus_chip) {
		...
		if (g_oplus_chip->dual_charger_support || oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		...
		if (mtkhv_flashled_pinctrl.hv_flashled_support) {
			...
			if (mtkhv_flashled_pinctrl.bc1_2_done) {
				if (val) {
					...
					chg_err("chgvin_gpio : %d", gpio_get_value(mtkhv_flashled_pinctrl.chgvin_gpio));
					if (g_oplus_chip->charger_exist &&
					    POWER_SUPPLY_TYPE_USB_DCP == g_oplus_chip->charger_type) {
...
static int proc_dump_log_show(struct seq_file *m, void *v)
{
	...
	if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		...
		adapter_dev_get_cap(pinfo->pd_adapter, MTK_PD_APDO, &cap);
		for (i = 0; i < cap.nr; i++) {
			seq_printf(m, "%d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n", i, cap.max_mv[i],
				   cap.min_mv[i], cap.ma[i], cap.type[i], cap.pwr_limit[i], cap.pdp);
		}
	} else if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK ||
		   pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30) {
...
void charger_debug_init(void)
{
	...
	proc_create("dump_log", 0644, charger_dir, &charger_dump_log_proc_fops);
}
...
void scd_ctrl_cmd_from_user(void *nl_data, struct sc_nl_msg_t *ret_msg)
{
	...
	case SC_DAEMON_CMD_SET_DAEMON_PID: {
		memcpy(&pinfo->sc.g_scd_pid, &msg->sc_data[0], sizeof(pinfo->sc.g_scd_pid));
	...
	case SC_DAEMON_CMD_SETTING: {
		...
		chr_debug("rcv data:%d %d %d %d %d %d %d %d %d %d %d %d %d %d Ans:%d\n", data.data[0], data.data[1],
			  data.data[2], data.data[3], data.data[4], data.data[5], data.data[6], data.data[7],
			  data.data[8], data.data[9], data.data[10], data.data[11], data.data[12], data.data[13],
			  data.data[14]);
...
void sc_select_charging_current(struct charger_manager *info, struct charger_data *pdata)
{
	...
	if (pdata->thermal_charging_current_limit != -1) {
		...
		pinfo->sc.disable_in_this_plug = true;
		   ...
		   info->sc.sc_ibat < pdata->charging_current_limit && pinfo->sc.g_scd_pid != 0 &&
		   pinfo->sc.disable_in_this_plug == false) {
...
static ssize_t store_sc_en(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t store_sc_stime(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t store_sc_etime(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t store_sc_tuisoc(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t store_sc_ibat_limit(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
/* add for charger_wakelock */
	...
	/* sysfs node */
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_enable_sc);
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_sc_stime);
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_sc_etime);
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_sc_tuisoc);
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_sc_ibat_limit);
