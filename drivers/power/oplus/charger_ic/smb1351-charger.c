...
static int smb1351_masked_write(struct smb1351_charger *chip, int reg, u8 mask, u8 val)
{
	s32 rc;
	u8 temp;
...
static int smb1351_enable_volatile_writes(struct smb1351_charger *chip)
{
	...
	rc = smb1351_masked_write(chip, CMD_I2C_REG, CMD_BQ_CFG_ACCESS_BIT, CMD_BQ_CFG_ACCESS_BIT);
	if (rc)
...
static int smbchg_charging_suspend(struct smb1351_charger *chip, bool enable)
{
	...
	u8 reg = 0;
	pr_err("%s, suspend= %d\n", __func__, enable);
	rc = smb1351_masked_write(chip, CMD_INPUT_LIMIT_REG, CMD_SUSPEND_MODE_BIT, enable ? CMD_SUSPEND_MODE_BIT : 0);
	if (rc)
...
static int smbchg_charging_enable(struct smb1351_charger *chip, bool enable)
{
	...
	u8 reg = 0;
	pr_err("%s, enable= %d\n", __func__, enable);
	rc = smb1351_masked_write(chip, CMD_CHG_REG, CMD_CHG_EN_BIT, enable ? CMD_CHG_ENABLE : 0);
	if (rc)
...
static int smb1351_fastchg_current_set(int fastchg_current)
{
	...
	if ((fastchg_current < SMB1351_CHG_PRE_MIN_MA) || (fastchg_current > SMB1351_CHG_FAST_MAX_MA)) {
	...
	if (is_pre_chg) {
		...
		rc = smb1351_masked_write(smb1351_chip, CHG_OTH_CURRENT_CTRL_REG, PRECHG_CURRENT_MASK, i);
		if (rc)
			pr_err("Couldn't write CHG_OTH_CURRENT_CTRL_REG rc=%d\n", rc);

		return smb1351_masked_write(smb1351_chip, VARIOUS_FUNC_2_REG, PRECHG_TO_FASTCHG_BIT,
		...
		rc = smb1351_masked_write(smb1351_chip, VARIOUS_FUNC_2_REG, PRECHG_TO_FASTCHG_BIT, 0);
		if (rc)
			pr_err("Couldn't write VARIOUS_FUNC_2_REG rc=%d\n", rc);

		return smb1351_masked_write(smb1351_chip, CHG_CURRENT_CTRL_REG, FAST_CHG_CURRENT_MASK, i);
	}
}
...
static int smb1351_iterm_set(int iterm_ma)
{
	...
	rc = smb1351_masked_write(smb1351_chip, CHG_OTH_CURRENT_CTRL_REG, ITERM_MASK, reg);
	if (rc) {
		pr_err("Couldn't set iterm rc = %d\n", rc);
		return rc;
	}
	/* enable the iterm */
	rc = smb1351_masked_write(smb1351_chip, CHG_CTRL_REG, ITERM_EN_BIT, ITERM_ENABLE);
	if (rc) {
...
static int smb1351_hw_init(void)
{
	...
	if (smb1351_chip->pinctrl_state_name) {
		smb1351_chip->smb_pinctrl = pinctrl_get_select(smb1351_chip->dev, smb1351_chip->pinctrl_state_name);
	...
	rc = smb1351_masked_write(smb1351_chip, HVDCP_BATT_MISSING_CTRL_REG, mask, reg);
	if (rc) {
	...
	rc = smb1351_masked_write(smb1351_chip, OTG_MODE_POWER_OPTIONS_REG, mask, reg);
	if (rc) {
	...
	rc = smb1351_masked_write(smb1351_chip, FLEXCHARGER_REG, mask, reg);
	if (rc) {
	...
	rc = smb1351_masked_write(smb1351_chip, VARIOUS_FUNC_3_REG, mask, reg);
	if (rc) {
	...
	reg = EN_BY_I2C_0_DISABLE | USBCS_CTRL_BY_I2C | CHG_ERR_BIT | APSD_DONE_BIT | LED_BLINK_FUNC_BIT;
	mask = EN_PIN_CTRL_MASK | USBCS_CTRL_BIT | CHG_ERR_BIT | APSD_DONE_BIT | LED_BLINK_FUNC_BIT;
	...
	reg = HOT_COLD_HARD_LIMIT_BIT | HOT_COLD_SOFT_LIMIT_BIT | INPUT_OVLO_BIT | INPUT_UVLO_BIT | AICL_DONE_FAIL_BIT;
	rc = smb1351_write_reg(smb1351_chip, FAULT_INT_REG, reg);
	if (rc) {
		pr_err("Couldn't set FAULT_INT_REG rc=%d\n", rc);
		return rc;
	}
	reg = CHG_OR_PRECHG_TIMEOUT_BIT | BATT_OVP_BIT | FAST_TERM_TAPER_RECHG_INHIBIT_BIT | BATT_MISSING_BIT |
	      BATT_LOW_BIT;
	...
	if (!smb1351_chip->using_pmic_therm) {
		rc = smb1351_masked_write(smb1351_chip, THERM_A_CTRL_REG, THERM_MONITOR_BIT, THERM_MONITOR_EN);
		if (rc) {
	...
	reg = 0;
	mask = AFCV_MASK;
	rc = smb1351_masked_write(smb1351_chip, CHG_CTRL_REG, mask, reg);
	if (rc) {
		pr_err("Couldn't SET DCD  rc=%d\n", rc);
		return rc;
	}
	...
	if (smb1351_chip->recharge_mv != -EINVAL) {
		if (smb1351_chip->recharge_disabled) {
			...
			rc = smb1351_masked_write(smb1351_chip, CHG_CTRL_REG, AUTO_RECHG_BIT | AUTO_RECHG_TH_BIT, reg);
			if (rc) {
	...
	if (rc) {
		pr_err("Unable to %s battery charging. rc=%d\n",
		       smb1351_chip->usb_suspended_status ? "disable" : "enable", rc);
	}
...
static int smb1351_input_current_limit_write(struct smb1351_charger *chip, int value)
{
	...
	if (smb1351_registers_read_full()) {
		smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				     REG00_SMB1351_INPUT_CURRENT_LIMIT_500MA);
		smb1351_dump_regs();
	...
	j = 2; /* 500 */
	rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				  REG00_SMB1351_INPUT_CURRENT_LIMIT_500MA);
	msleep(90);
	...
	j = 3; /* 900 */
	rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				  REG00_SMB1351_INPUT_CURRENT_LIMIT_900MA);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = j - 1;
...
#endif
	rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				  REG00_SMB1351_INPUT_CURRENT_LIMIT_1500MA);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
...
#endif
	rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				  REG00_SMB1351_INPUT_CURRENT_LIMIT_2000MA);
	msleep(90);
#if defined(CONFIG_OPLUS_CHARGER_MTK6763) || defined(CONFIG_OPLUS_CHARGER_MTK6771)
	for (i = 0; i < AICL_COUNT; i++) {
		chg_vol = battery_meter_get_charger_voltage();
	...
	j = 7; /* 3000 */
	rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
				  REG00_SMB1351_INPUT_CURRENT_LIMIT_2200MA);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = j - 1;
...
aicl_pre_step:
	...
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, j,
		  smb1351_usbin_input_current_limit[j], aicl_point_temp);
	switch (j) {
	case 0:
		rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
					  REG00_SMB1351_INPUT_CURRENT_LIMIT_500MA);
	...
	case 4:
		rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
					  REG00_SMB1351_INPUT_CURRENT_LIMIT_1200MA);
		break;
	case 5:
		rc = smb1351_masked_write(chip, CHG_CURRENT_CTRL_REG, AC_INPUT_CURRENT_LIMIT_MASK,
					  REG00_SMB1351_INPUT_CURRENT_LIMIT_1500MA);
...
static int smb1351_set_usb_chg_current(int current_ma)
{
	...
	case STANDARD_HOST:
		...
		mask = CMD_INPUT_CURRENT_MODE_BIT | CMD_USB_2_3_SEL_BIT | CMD_USB_1_5_AC_CTRL_MASK;
...
static int rerun_apsd(struct smb1351_charger *chip)
{
	...
	rc = smb1351_masked_write(chip, CMD_HVDCP_REG, CMD_APSD_RE_RUN_BIT, CMD_APSD_RE_RUN_BIT);
	if (rc)
...
static int smb1351_apsd_complete_handler(struct smb1351_charger *chip, u8 status)
{
	int rc;
	u8 reg = 0;
	pr_err("%s, status:%d\n", __func__, status);
...
static int smb1351_safety_timeout_handler(struct smb1351_charger *chip, u8 status)
{
...
static int smb1351_battery_missing_handler(struct smb1351_charger *chip, u8 status)
{
	if (status)
...
static irqreturn_t smb1351_chg_stat_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < ARRAY_SIZE(handlers); i++) {
		rc = smb1351_read_reg(chip, handlers[i].stat_reg, &handlers[i].val);
		if (rc) {
	...
	for (i = 0; i < ARRAY_SIZE(handlers); i++)
		for (j = 0; j < 4; j++) {
			seq_printf(m, "%s=%d\t(high=%d low=%d)\n", handlers[i].irq_info[j].name,
				   handlers[i].irq_info[j].high + handlers[i].irq_info[j].low,
				   handlers[i].irq_info[j].high, handlers[i].irq_info[j].low);
			total += (handlers[i].irq_info[j].high + handlers[i].irq_info[j].low);
		}
...
void smb1351_dump_regs(void)
{
	...
	pr_err("[%s]:smb1351_cnfg_reg[0x00-0x16]:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
	       __func__, dump_reg[0], dump_reg[1], dump_reg[2], dump_reg[3], dump_reg[4], dump_reg[5], dump_reg[6],
	       dump_reg[7], dump_reg[8], dump_reg[9], dump_reg[10], dump_reg[11], dump_reg[12], dump_reg[13],
	       dump_reg[14], dump_reg[15], dump_reg[16], dump_reg[17], dump_reg[18], dump_reg[19], dump_reg[20],
	       dump_reg[21], dump_reg[22]);
	...
	pr_err("[%s]:smb1351_status_reg[0x36-0x3f]:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n", __func__,
	       dump_reg[0], dump_reg[1], dump_reg[2], dump_reg[3], dump_reg[4], dump_reg[5], dump_reg[6], dump_reg[7],
	       dump_reg[8], dump_reg[9]);
	...
	pr_err("[%s]:smb1351_cmd_reg[0x30-0x34]:0x%x,0x%x,0x%x,0x%x,0x%x\n", __func__, dump_reg[0], dump_reg[1],
	       dump_reg[2], dump_reg[3], dump_reg[4]);
	charger_ic_flag = 3;
	return;
}
#else
...
static int smb1351_parse_dt(struct smb1351_charger *chip)
{
	...
	chip->force_hvdcp_2p0 = of_property_read_bool(node, "qcom,force-hvdcp-2p0");

	rc = of_property_read_string(node, "qcom,bms-psy-name", &chip->bms_psy_name);
	if (rc)
		chip->bms_psy_name = NULL;

	rc = of_property_read_u32(node, "qcom,fastchg-current-max-ma", &chip->target_fastchg_current_max_ma);
	if (rc)
	...
	chip->recharge_disabled = of_property_read_bool(node, "qcom,recharge-disabled");

	/* thermal and jeita support */
	rc = of_property_read_u32(node, "qcom,batt-cold-decidegc", &chip->batt_cold_decidegc);
	if (rc < 0)
		chip->batt_cold_decidegc = -EINVAL;

	rc = of_property_read_u32(node, "qcom,batt-hot-decidegc", &chip->batt_hot_decidegc);
	if (rc < 0)
		chip->batt_hot_decidegc = -EINVAL;

	rc = of_property_read_u32(node, "qcom,batt-warm-decidegc", &chip->batt_warm_decidegc);

	rc |= of_property_read_u32(node, "qcom,batt-cool-decidegc", &chip->batt_cool_decidegc);

	if (!rc) {
		rc = of_property_read_u32(node, "qcom,batt-cool-mv", &chip->batt_cool_mv);

		rc |= of_property_read_u32(node, "qcom,batt-warm-mv", &chip->batt_warm_mv);

		rc |= of_property_read_u32(node, "qcom,batt-cool-ma", &chip->batt_cool_ma);

		rc |= of_property_read_u32(node, "qcom,batt-warm-ma", &chip->batt_warm_ma);
		if (rc)
	...
	rc = of_property_read_u32(node, "qcom,batt-missing-decidegc", &chip->batt_missing_decidegc);
...
static int create_debugfs_entries(struct smb1351_charger *chip)
{
	...
	if (!chip->debug_root) {
		...
		ent = debugfs_create_file("config_registers", S_IFREG | S_IRUGO, chip->debug_root, chip,
					  &cnfg_debugfs_ops);
		if (!ent)
			pr_err("Couldn't create cnfg debug file\n");

		ent = debugfs_create_file("status_registers", S_IFREG | S_IRUGO, chip->debug_root, chip,
					  &status_debugfs_ops);
		if (!ent)
		...
		ent = debugfs_create_x32("address", S_IFREG | S_IWUSR | S_IRUGO, chip->debug_root,
					 &(chip->peek_poke_address));
		if (!ent)
			pr_err("Couldn't create address debug file\n");

		ent = debugfs_create_file("data", S_IFREG | S_IWUSR | S_IRUGO, chip->debug_root, chip,
					  &poke_poke_debug_ops);
		if (!ent)
			pr_err("Couldn't create data debug file\n");

		ent = debugfs_create_file("force_irq", S_IFREG | S_IWUSR | S_IRUGO, chip->debug_root, chip,
					  &force_irq_ops);
		if (!ent)
			pr_err("Couldn't create data debug file\n");

		ent = debugfs_create_file("irq_count", S_IFREG | S_IRUGO, chip->debug_root, chip,
...
int smbchg_otg_enable(void)
{
	...
	rc = smb1351_masked_write(smb1351_chip, CMD_CHG_REG, CMD_OTG_EN_BIT, CMD_OTG_EN_BIT);
	if (rc)
		pr_err("Couldn't enable  OTG mode rc=%d\n", rc);
	pr_err("%s \n", __func__);
	return rc;
...
static int smb1351_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int rc;
	...
	if (client->irq) {
		rc = devm_request_threaded_irq(&client->dev, client->irq, NULL, smb1351_chg_stat_handler,
					       IRQF_TRIGGER_LOW | IRQF_ONESHOT, "smb1351_chg_stat_irq", chip);
		if (rc) {
