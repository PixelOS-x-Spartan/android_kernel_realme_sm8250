...
enum drp_toggle_type { TOGGLE_DFP_DRP_30 = 0, TOGGLE_DFP_DRP_40, TOGGLE_DFP_DRP_50, TOGGLE_DFP_DRP_60 };
...
enum cable_attach_type { CABLE_NOT_ATTACHED = 0, CABLE_ATTACHED };
...
enum cable_dir_type { ORIENT_CC2 = 0, ORIENT_CC1 };
...
enum en_gpio_type { EN_CC_MUX = 0, EN_VREG_5V, EN_GPIO_MAX = 3 };
...
static ssize_t mode_select_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
	int mode;
	u8 value;
...
static ssize_t current_advertise_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
	int mode;
	u8 value;
...
static ssize_t gpio_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
...
static ssize_t gpio_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
	int level;
	int ret;
...
static ssize_t show_chip_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
	u8 reg_val;
...
#define TYPE_C_ATTR(field, format_string)                                                                              \
	...
	{                                                                                                              \
		struct sgm7220_info *info = dev_get_drvdata(dev);                                                      \
		return snprintf(buf, PAGE_SIZE, format_string, info->type_c_param.field);                              \
	}                                                                                                              \
	static DEVICE_ATTR(field, S_IRUGO, field##_show, NULL);

static DEVICE_ATTR(gpio_debug, S_IRUGO | S_IWUSR, gpio_debug_show, gpio_debug_store);
...
static struct device_attribute *usb_typec_attributes[] = { &dev_attr_mode_select, &dev_attr_current_advertise,
							   &dev_attr_current_det_string, &dev_attr_chip_info,
							   &dev_attr_current_det, &dev_attr_accessory_attach,
							   &dev_attr_active_cable_attach, &dev_attr_attach_state,
							   &dev_attr_cable_dir, &dev_attr_gpio_debug,
							   /*end*/
							   NULL };
/******************************************************************************/
...
static void state_disorder_detect(struct sgm7220_info *info)
{
	...
	if ((info->type_c_param.attach_state == CABLE_STATE_NOT_ATTACHED) && info->monitor.err_detected) {
		/* enable id irq */
...
static int oplus_register_extcon(struct sgm7220_info *info)
{
	...
	rc = extcon_set_property_capability(info->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB, EXTCON_PROP_USB_SS);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS);
...
static void oplus_notify_extcon_props(struct sgm7220_info *info, int id)
{
	...
	extcon_set_property(info->extcon, id, EXTCON_PROP_USB_TYPEC_POLARITY, val);
	val.intval = true;
	extcon_set_property(info->extcon, id, EXTCON_PROP_USB_SS, val);
}
...
static void process_interrupt_register(struct sgm7220_info *info)
{
	...
	if (pre_state_type == CABLE_STATE_NOT_ATTACHED && info->type_c_param.attach_state != CABLE_STATE_NOT_ATTACHED) {
		cur_attach = true;
	} else if (pre_state_type != CABLE_STATE_NOT_ATTACHED &&
		   info->type_c_param.attach_state == CABLE_STATE_NOT_ATTACHED) {
		cur_attach = false;
	}
	...
	pr_debug("%s attach[%d] cabledir[%d] probe[%d]\n", __func__, info->type_c_param.attach_state, typec_dir,
		 info->probe);
	if (attach_changed) {
		if (cur_attach) {
			...
			if (info->typec_port_support) {
				...
				if (!info->partner) {
					memset(&info->partner_identity, 0, sizeof(info->partner_identity));
					info->partner_desc.usb_pd = false;
					info->partner_desc.accessory = TYPEC_ACCESSORY_NONE;
					info->partner = typec_register_partner(info->typec_port, &info->partner_desc);
...
static irqreturn_t sgm7220_irq_thread(int irq, void *handle)
{
	...
	if (atomic_read(&info->suspended) == 1) {
		...
		wait_event_interruptible_timeout(info->wait, atomic_read(&info->suspended) == 0,
						 msecs_to_jiffies(OPLUS_WAIT_RESUME_TIM));
	}
	...
	ret = sgm7220_update_reg(info->i2c, REG_INT, (0x1 << INT_INTERRUPT_STATUS_SHIFT), INT_INTERRUPT_STATUS);
	if (ret < 0) {
		pr_err("%s: update reg fail!\n", __func__);
	}
...
int oplus_sgm7220_set_mode(int mode)
{
	...
	pr_err("%s: mode[%d] readback REG_SET:0x%0x[0x%0x] done\n", __func__, mode, REG_SET,
	       i2c_smbus_read_byte_data(gchip->i2c, REG_SET));
	...
	pr_err("%s: mode[%d] readback REG_INT:0x%0x[0x%0x] done\n", __func__, mode, REG_INT,
	       i2c_smbus_read_byte_data(gchip->i2c, REG_INT));
	return 0;
}
...
static int sgm7220_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef OPLUS_ARCH_EXTENDS
	ret = of_property_read_u32(client->dev.of_node, "oplus,headphone-det-support", &headphone_det_support);
	if (ret || headphone_det_support == 0) {
		info->headphone_det_support = 0;
	} else {
		info->headphone_det_support = 1;
	}
	dev_err(info->dev, "%s: headphone_det_support = %d\n", __func__, info->headphone_det_support);
#endif /* OPLUS_ARCH_EXTENDS */
	...
	info->typec_inter_active = pinctrl_lookup_state(info->pinctrl, "typec_inter_active");
	if (IS_ERR_OR_NULL(info->typec_inter_active)) {
		pr_err(": %d Failed to get the state pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	info->typec_inter_sleep = pinctrl_lookup_state(info->pinctrl, "typec_inter_sleep");
	...
	ret = request_threaded_irq(client->irq, NULL, sgm7220_irq_thread, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
				   "sgm7220_irq", info);
	if (ret) {
