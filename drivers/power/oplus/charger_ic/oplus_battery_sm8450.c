...
static int oplus_chg_track_upload_icl_err_info(struct battery_chg_dev *bcdev, int err_type);
static int oplus_chg_track_upload_adsp_err_info(struct battery_chg_dev *bcdev, int err_type);
...
static const int usb_prop_map[USB_PROP_MAX] = {
	[USB_ONLINE] = POWER_SUPPLY_PROP_ONLINE,	[USB_VOLT_NOW] = POWER_SUPPLY_PROP_VOLTAGE_NOW,
	[USB_VOLT_MAX] = POWER_SUPPLY_PROP_VOLTAGE_MAX, [USB_CURR_NOW] = POWER_SUPPLY_PROP_CURRENT_NOW,
	[USB_CURR_MAX] = POWER_SUPPLY_PROP_CURRENT_MAX, [USB_INPUT_CURR_LIMIT] = POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
	[USB_ADAP_TYPE] = POWER_SUPPLY_PROP_USB_TYPE,	[USB_TEMP] = POWER_SUPPLY_PROP_TEMP,
};

static const int wls_prop_map[WLS_PROP_MAX] = {
	[WLS_ONLINE] = POWER_SUPPLY_PROP_ONLINE,	[WLS_VOLT_NOW] = POWER_SUPPLY_PROP_VOLTAGE_NOW,
	[WLS_VOLT_MAX] = POWER_SUPPLY_PROP_VOLTAGE_MAX, [WLS_CURR_NOW] = POWER_SUPPLY_PROP_CURRENT_NOW,
	[WLS_CURR_MAX] = POWER_SUPPLY_PROP_CURRENT_MAX,
};

/* Standard usb_type definitions similar to power_supply_sysfs.c */
static const char *const power_supply_usb_type_text[] = { "Unknown", "SDP", "DCP",    "CDP",	"ACA",
							  "C",	     "PD",  "PD_DRP", "PD_PPS", "BrickID" };

/* Custom usb_type definitions */
static const char *const qc_power_supply_usb_type_text[] = { "HVDCP", "HVDCP_3", "HVDCP_3P5" };
...
static int oem_battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->oem_read_ack, msecs_to_jiffies(OEM_READ_WAIT_TIME_MS));
		if (!rc) {
			pr_err("Error, timed out sending message\n");
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_OEM_GLINK_ABNORMAL);
...
static void handle_oem_read_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg, size_t len)
{
	u32 buf_len;
...
static int bcc_battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	rc = pmic_glink_write(bcdev->client, data, len);
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->bcc_read_ack, msecs_to_jiffies(OEM_READ_WAIT_TIME_MS));
		if (!rc) {
			pr_err("Error, timed out sending message\n");
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_BCC_GLINK_ABNORMAL);
...
static void handle_bcc_read_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg, size_t len)
{
	u32 buf_len;
	...
	if (len > sizeof(bcdev->bcc_read_buffer_dump)) {
		pr_err("Incorrect length received: %zu expected: %zu\n", len, sizeof(bcdev->bcc_read_buffer_dump));
	...
	bcdev->bcc_read_buffer_dump.data_buffer[8] =
		DIV_ROUND_CLOSEST((int)bcdev->bcc_read_buffer_dump.data_buffer[8], 1000);
	...
	printk(KERN_ERR
		...
		soc_ext_1[%d], soc_ext_2[%d], atl_last_geat_current[%d], charging_flag[%d], bcc_curr_done[%d], guage[%d], batt_type[%d]",
	       __func__, bcdev->bcc_read_buffer_dump.data_buffer[0], bcdev->bcc_read_buffer_dump.data_buffer[1],
	       bcdev->bcc_read_buffer_dump.data_buffer[2], bcdev->bcc_read_buffer_dump.data_buffer[3],
	       bcdev->bcc_read_buffer_dump.data_buffer[4], bcdev->bcc_read_buffer_dump.data_buffer[5],
	       bcdev->bcc_read_buffer_dump.data_buffer[6], bcdev->bcc_read_buffer_dump.data_buffer[7],
	       bcdev->bcc_read_buffer_dump.data_buffer[8], bcdev->bcc_read_buffer_dump.data_buffer[9],
	       bcdev->bcc_read_buffer_dump.data_buffer[10], bcdev->bcc_read_buffer_dump.data_buffer[11],
	       bcdev->bcc_read_buffer_dump.data_buffer[12], bcdev->bcc_read_buffer_dump.data_buffer[13],
	       bcdev->bcc_read_buffer_dump.data_buffer[14], bcdev->bcc_read_buffer_dump.data_buffer[15],
	       bcdev->bcc_read_buffer_dump.data_buffer[16], bcdev->bcc_read_buffer_dump.data_buffer[17],
	       bcdev->bcc_read_buffer_dump.data_buffer[18]);
	complete(&bcdev->bcc_read_ack);
}

static int adsp_track_battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	oplus_chg_track_handle_adsp_info(bcdev->adsp_track_read_buffer.data_buffer,
...
static void handle_adsp_track_read_buffer(struct battery_chg_dev *bcdev, struct adsp_track_read_resp_msg *resp_msg,
					  size_t len)
{
	u32 buf_len;

	if (len > sizeof(bcdev->adsp_track_read_buffer)) {
		pr_err("Incorrect length received: %zu expected: %lu\n", len, sizeof(bcdev->adsp_track_read_buffer));
		return;
	}
	...
	memcpy(bcdev->adsp_track_read_buffer.data_buffer, resp_msg->data_buffer, buf_len);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, adsp_track_notify_work.work);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int pps_battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	rc = pmic_glink_write(bcdev->client, data, len);
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->pps_read_ack, msecs_to_jiffies(OEM_READ_WAIT_TIME_MS));
		if (!rc) {
			chg_err("Error, timed out sending message\n");
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_PPS_GLINK_ABNORMAL);
...
static void handle_pps_read_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg, size_t len)
{
	u32 buf_len;

	if (len > sizeof(bcdev->pps_read_buffer_dump)) {
		chg_err("Incorrect length received: %zu expected: %zu\n", len, sizeof(bcdev->pps_read_buffer_dump));
		return;
	}
...
static int oplus_get_bcc_parameters_from_adsp(char *buf)
{
	...
	for (i = 0; i < BCC_PARMS_COUNT - 1; i++) {
		len = snprintf(tmpbuf, BCC_PAGE_SIZE - idx, "%d,", bcdev->bcc_read_buffer_dump.data_buffer[i]);
...
static int battery_chg_fw_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	...
	rc = pmic_glink_write(bcdev->client, data, len);
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->fw_buf_ack, msecs_to_jiffies(WLS_FW_WAIT_TIME_MS));
		if (!rc) {
			pr_err("Error, timed out sending message\n");
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_FW_GLINK_ABNORMAL);
			return -ETIMEDOUT;
		}
...
static int battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	...
	if (!rc) {
		...
		if (!rc) {
			...
			oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_GLINK_ABNORMAL);
	...
	if (bcdev->debug_pmic_glink_err) {
		oplus_chg_track_upload_adsp_err_info(bcdev, bcdev->debug_pmic_glink_err);
...
static int write_property_id(struct battery_chg_dev *bcdev, struct psy_state *pst, u32 prop_id, u32 val)
{
	...
	pr_debug("psy: %s prop_id: %u val: %u\n", pst->psy->desc->name, req_msg.property_id, val);
...
static int get_property_id(struct psy_state *pst, enum power_supply_property prop)
{
	u32 i;
...
irqreturn_t oplus_ccdetect_change_handler(int irq, void *data)
{
	...
	schedule_delayed_work(&bcdev->ccdetect_work, msecs_to_jiffies(CCDETECT_DELAY_MS));
	return IRQ_HANDLED;
}
...
int oplus_ccdetect_gpio_init(struct oplus_chg_chip *chip)
{
	...
	bcdev->oplus_custom_gpio.ccdetect_active =
		pinctrl_lookup_state(bcdev->oplus_custom_gpio.ccdetect_pinctrl, "ccdetect_active");
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.ccdetect_active)) {
		chg_err("get ccdetect_active fail\n");
		return -EINVAL;
	}

	bcdev->oplus_custom_gpio.ccdetect_sleep =
		pinctrl_lookup_state(bcdev->oplus_custom_gpio.ccdetect_pinctrl, "ccdetect_sleep");
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.ccdetect_pinctrl, bcdev->oplus_custom_gpio.ccdetect_active);
...
static void oplus_ccdetect_irq_register(struct oplus_chg_chip *chip)
{
	...
	bcdev = chip->pmic_spmi.bcdev_chip;
	ret = devm_request_threaded_irq(chip->dev, bcdev->ccdetect_irq, NULL, oplus_ccdetect_change_handler,
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, hvdcp_disable_work.work);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, pd_type_check_work.work);
...
{
	int count = 20;
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, otg_vbus_enable_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
...
void lcm_frequency_ctrl(void)
{
	...
	if ((oplus_chg_get_charger_voltage() > 2500) || (oplus_chg_is_wls_present())) {
		if (chip->sw_full || (oplus_pps_get_ffc_started() == true) || chip->hw_full_by_sw ||
		...
		mod_delayed_work(system_highpri_wq, &bcdev->ctrl_lcm_frequency, 5000);
	} else {
		if (lcm_en_flag != LCM_EN_ENABLE) {
			lcm_en_flag = LCM_EN_ENABLE;
			smbchg_lcm_en(true);
...
static void oplus_adsp_voocphy_enable_check_func(struct work_struct *work)
{
	...
	if (chip->mmi_chg == 0 || chip->charger_exist == false || chip->charger_type != POWER_SUPPLY_TYPE_USB_DCP ||
 ...
 */
int qti_battery_charger_get_prop(const char *name, enum battery_charger_prop prop_id, int *val)
{
	...
	if (strcmp(name, "battery") && strcmp(name, "usb") && strcmp(name, "wireless"))
		return -EINVAL;
...
static bool validate_message(struct battery_charger_resp_msg *resp_msg, size_t len)
{
	if (len != sizeof(*resp_msg)) {
		pr_err("Incorrect response length %zu for opcode %#x\n", len, resp_msg->hdr.opcode);
		return false;
	}

	if (resp_msg->ret_code) {
		pr_err("Error in response for opcode %#x prop_id %u, rc=%d\n", resp_msg->hdr.opcode,
		       resp_msg->property_id, (int)resp_msg->ret_code);
		return false;
	}
...
static bool current_message_check(struct battery_charger_resp_msg *resp_msg, size_t len)
{
	if (len != sizeof(*resp_msg)) {
		pr_err("Incorrect response length %zu for opcode %#x\n", len, resp_msg->hdr.opcode);
		return false;
	...
	if (resp_msg->ret_code == BATTMNGR_EFAILED && resp_msg->property_id == BATT_CURR_NOW) {
		return true;
	}
...
static void handle_message(struct battery_chg_dev *bcdev, void *data, size_t len)
{
	...
	case BC_BATTERY_STATUS_GET:
		...
		if (pst->model && len == sizeof(*model_resp_msg)) {
			...
			ack_set = true;
			bcdev->debug_battery_detected = !strcmp(pst->model, MODEL_DEBUG_BOARD);
			break;
		}
		...
		if (current_message_check(resp_msg, len) && resp_msg->property_id < pst->prop_count) {
			ack_set = true;
		}
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, usb_type_work);
#else
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, usb_type_work);
#endif
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
	...
	if (g_oplus_chip) {
		if (usb_psy_desc.type != POWER_SUPPLY_TYPE_UNKNOWN) {
			if (chg_type == POWER_SUPPLY_TYPE_USB_DCP &&
			    pst->prop[USB_ADAP_TYPE] != POWER_SUPPLY_USB_TYPE_DCP && bcdev->hvdcp_disable == false) {
				usb_psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
		...
		if (usb_psy_desc.type == POWER_SUPPLY_TYPE_UNKNOWN &&
			...
			chg_err("!!! usb_psy_desc.type: [%d]to_warm[%d]dummy[%d]to_normal[%d]started[%d]\n",
				usb_psy_desc.type, oplus_vooc_get_fastchg_to_warm(),
			...
			if (oplus_vooc_get_fastchg_to_warm() == true ||
			    ...
			    oplus_vooc_get_fastchg_to_normal() == true || oplus_vooc_get_fastchg_started() == true) {
				usb_psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
...
static void handle_notification(struct battery_chg_dev *bcdev, void *data, size_t len)
{
#ifdef OPLUS_FEATURE_CHG_BASIC
	int chg_type;
	int sub_chg_type;
	...
	case BC_VOOC_VBUS_ADC_ENABLE:
		...
		if (is_ext_chg_ops()) {
			...
			schedule_delayed_work(&bcdev->vbus_adc_enable_work,
					      0); /*excute in work to avoid glink dead loop*/
		}
		break;
	case BC_CID_DETECT:
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, status_keep_clean_work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (chip == NULL) {
		...
		schedule_delayed_work(&bcdev->status_keep_delay_unlock_work, msecs_to_jiffies(SK_DELAY_UNLOCK_TIME));
	}
}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, status_keep_delay_unlock_work);
...
static int wls_psy_get_prop(struct power_supply *psy, enum power_supply_property prop, union power_supply_propval *pval)
{
#ifndef OPLUS_FEATURE_CHG_BASIC
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (chip->wls_status_keep != WLS_SK_NULL) {
			...
			if (pre_wls_online && pval->intval == 0) {
				...
				pval->intval = 1;
				schedule_delayed_work(&bcdev->status_keep_clean_work,
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW,
						(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_MAX,
						(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_CURRENT_NOW,
						(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_CURRENT_MAX,
						(union oplus_chg_mod_propval *)pval);
		break;
...
static int wls_psy_set_prop(struct power_supply *psy, enum power_supply_property prop,
			    const union power_supply_propval *pval)
{
	return 0;
}

static int wls_psy_prop_is_writeable(struct power_supply *psy, enum power_supply_property prop)
{
	return 0;
}
...
static const char *get_usb_type_name(u32 usb_type)
{
	...
	if (usb_type >= QTI_POWER_SUPPLY_USB_TYPE_HVDCP && usb_type <= QTI_POWER_SUPPLY_USB_TYPE_HVDCP_3P5) {
...
#endif
static int usb_psy_get_prop(struct power_supply *psy, enum power_supply_property prop, union power_supply_propval *pval)
{
	struct battery_chg_dev *bcdev = power_supply_get_drvdata(psy);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int prop_id, rc;
	...
	if (USB_ONLINE == prop_id) {
		...
		if (pre_usb_in_status != bcdev->usb_in_status) {
			...
			if ((oplus_get_wired_chg_present() == false) && (g_oplus_chip->charger_volt < 3500)) {
				bcdev->pd_svooc = false; //remove svooc flag
			}
		}

		if (oplus_chg_get_voocphy_support() != ADSP_VOOCPHY) {
			if (pval->intval == 2 || (oplus_vooc_get_fastchg_started() == true) ||
	...
	if (prop == POWER_SUPPLY_PROP_VOLTAGE_NOW) {
		if (is_ext_mp2650_chg_ops() && g_oplus_chip->chg_ops->get_charger_volt) {
...
static int usb_psy_set_prop(struct power_supply *psy, enum power_supply_property prop,
			    const union power_supply_propval *pval)
{
	struct battery_chg_dev *bcdev = power_supply_get_drvdata(psy);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
...
static int usb_psy_prop_is_writeable(struct power_supply *psy, enum power_supply_property prop)
{
	switch (prop) {
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		return 1;
...
static enum power_supply_property usb_props[] = {
	POWER_SUPPLY_PROP_ONLINE,      POWER_SUPPLY_PROP_VOLTAGE_NOW, POWER_SUPPLY_PROP_VOLTAGE_MAX,
	POWER_SUPPLY_PROP_CURRENT_NOW, POWER_SUPPLY_PROP_CURRENT_MAX, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
...
static int __battery_psy_set_charge_current(struct battery_chg_dev *bcdev, u32 fcc_ua)
{
	int rc;
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_BATTERY], BATT_CHG_CTRL_LIM, fcc_ua);
	if (rc < 0)
...
static int battery_psy_get_prop(struct power_supply *psy, enum power_supply_property prop,
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (IS_ENABLED(CONFIG_QTI_PMIC_GLINK_CLIENT_DEBUG) && (bcdev->fake_soc >= 0 && bcdev->fake_soc <= 100))
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (oplus_switching_support_parallel_chg()) {
			if ((chip->tbatt_temp > PARALLEL_SWITCH_HIGH_TEMP) ||
				...
				pval->intval = chip->tbatt_temp > chip->sub_batt_temperature ?
						       chip->tbatt_temp :
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		...
		if (qpnp_is_power_off_charging() == true && (oplus_chg_get_curr_time_ms(&cur_chg_time) < 12000) &&
...
static int battery_psy_set_prop(struct power_supply *psy, enum power_supply_property prop,
				const union power_supply_propval *pval)
{
	struct battery_chg_dev *bcdev = power_supply_get_drvdata(psy);
...
static int battery_psy_prop_is_writeable(struct power_supply *psy, enum power_supply_property prop)
{
	switch (prop) {
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		return 1;
...
static int battery_chg_init_psy(struct battery_chg_dev *bcdev)
{
	...
	bcdev->psy_list[PSY_TYPE_BATTERY].psy = devm_power_supply_register(bcdev->dev, &batt_psy_desc, &psy_cfg);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	bcdev->psy_list[PSY_TYPE_WLS].psy = devm_power_supply_register(bcdev->dev, &wls_psy_desc, &psy_cfg);
	...
	bcdev->psy_list[PSY_TYPE_USB].psy = devm_power_supply_register(bcdev->dev, &usb_psy_desc, &psy_cfg);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, subsys_up_work);
	int rc;
...
#ifndef OPLUS_FEATURE_CHG_BASIC
	if (bcdev->usb_icl_ua) {
		rc = usb_psy_set_icl(bcdev, USB_INPUT_CURR_LIMIT, bcdev->usb_icl_ua);
		if (rc < 0)
			pr_err("Failed to set ICL(%u uA), rc=%d\n", bcdev->usb_icl_ua, rc);
	}
#endif
...
static int wireless_fw_send_firmware(struct battery_chg_dev *bcdev, const struct firmware *fw)
{
...
static int wireless_fw_check_for_update(struct battery_chg_dev *bcdev, u32 version, size_t size)
{
...
static int wireless_fw_update(struct battery_chg_dev *bcdev, bool force)
{
	...
	rc = wait_for_completion_timeout(&bcdev->fw_update_ack, msecs_to_jiffies(WLS_FW_UPDATE_TIME_MS));
	if (!rc) {
...
static ssize_t wireless_fw_crc_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t wireless_fw_version_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t wireless_fw_force_update_store(struct class *c, struct class_attribute *attr, const char *buf,
					      size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	bool val;
...
static ssize_t wireless_fw_update_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	bool val;
	int rc;
...
static ssize_t usb_typec_compliant_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
...
static ssize_t usb_real_type_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
...
static ssize_t restrict_cur_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	int rc;
	u32 fcc_ua, prev_fcc_ua;
...
static ssize_t restrict_cur_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t restrict_chg_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	int rc;
	bool val;
	...
	rc = __battery_psy_set_charge_current(bcdev,
					      bcdev->restrict_chg_en ? bcdev->restrict_fcc_ua : bcdev->thermal_fcc_ua);
	if (rc < 0)
		return rc;
...
static ssize_t restrict_chg_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fake_soc_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
	int val;
...
static ssize_t fake_soc_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t wireless_boost_en_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	int rc;
	bool val;
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_WLS], WLS_BOOST_EN, val);
	if (rc < 0)
		return rc;
...
static ssize_t wireless_boost_en_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_WLS];
	int rc;
...
static ssize_t moisture_detection_en_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	int rc;
	bool val;
	...
	rc = write_property_id(bcdev, &bcdev->psy_list[PSY_TYPE_USB], USB_MOISTURE_DET_EN, val);
	if (rc < 0)
		return rc;
...
static ssize_t moisture_detection_en_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
...
static ssize_t moisture_detection_status_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
...
static ssize_t resistance_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
	int rc;
...
static ssize_t soh_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_BATTERY];
	int rc;
...
static ssize_t ship_mode_en_store(struct class *c, struct class_attribute *attr, const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t ship_mode_en_show(struct class *c, struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev, battery_class);
...
static int oplus_otg_ovp_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl, bcdev->oplus_custom_gpio.otg_ovp_en_sleep);
...
static void oplus_set_otg_ovp_en_val(int value)
{
	...
	chg_err("<~OTG~>set value:%d, gpio_val:%d\n", value, gpio_get_value(bcdev->oplus_custom_gpio.otg_ovp_en_gpio));
}
...
static int oplus_tx_boost_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.tx_boost_en_pinctrl, bcdev->oplus_custom_gpio.tx_boost_en_sleep);
...
static void oplus_set_tx_boost_en_val(int value)
{
	...
	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(bcdev->oplus_custom_gpio.tx_boost_en_gpio));
}
...
static int oplus_tx_ovp_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl, bcdev->oplus_custom_gpio.tx_ovp_en_sleep);
...
static void oplus_set_tx_ovp_en_val(int value)
{
	...
	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(bcdev->oplus_custom_gpio.tx_ovp_en_gpio));
}
...
static int oplus_wrx_ovp_off_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl, bcdev->oplus_custom_gpio.tx_ovp_en_sleep);
...
static void oplus_set_wrx_ovp_off_val(int value)
{
	...
	chg_err("set value:%d, gpio_val:%d\n", value, gpio_get_value(bcdev->oplus_custom_gpio.wrx_ovp_off_gpio));
}
...
static int oplus_vchg_trig_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.vchg_trig_pinctrl, bcdev->oplus_custom_gpio.vchg_trig_default);
...
static void oplus_vchg_trig_irq_register(struct battery_chg_dev *bcdev)
{
	...
	ret = devm_request_threaded_irq(bcdev->dev, bcdev->vchg_trig_irq, NULL, oplus_vchg_trig_change_handler,
					IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "vchg_trig_change",
					bcdev);
	if (ret < 0) {
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	bcdev->oplus_custom_gpio.mcu_en_gpio = of_get_named_gpio(node, "qcom,mcu-en-gpio", 0);
	if (bcdev->oplus_custom_gpio.mcu_en_gpio <= 0) {
	...
	bcdev->oplus_custom_gpio.vchg_trig_gpio = of_get_named_gpio(node, "qcom,vchg_trig-gpio", 0);
	if (bcdev->oplus_custom_gpio.vchg_trig_gpio <= 0) {
	...
	bcdev->oplus_custom_gpio.otg_boost_en_gpio = of_get_named_gpio(node, "qcom,otg-booster-en-gpio", 0);
	if (bcdev->oplus_custom_gpio.otg_boost_en_gpio <= 0) {
	...
	bcdev->oplus_custom_gpio.otg_ovp_en_gpio = of_get_named_gpio(node, "qcom,otg-ovp-en-gpio", 0);
	if (bcdev->oplus_custom_gpio.otg_ovp_en_gpio <= 0) {
	...
	rc = of_property_read_u32(node, "qcom,otg_scheme", &bcdev->otg_scheme);
	if (rc) {
		bcdev->otg_scheme = OTG_SCHEME_UNDEFINE;
	}

	rc = of_property_read_u32(node, "qcom,otg_boost_src", &bcdev->otg_boost_src);
	if (rc) {
		bcdev->otg_boost_src = OTG_BOOST_SOURCE_EXTERNAL;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_thread_100w_support", &bcdev->usbtemp_thread_100w_support);
	if (rc) {
		bcdev->usbtemp_thread_100w_support = false;
	}
	chg_err("usbtemp_thread_100w_support:%d\n", bcdev->usbtemp_thread_100w_support);

	rc = of_property_read_u32(node, "qcom,otg_curr_limit_max", &bcdev->otg_curr_limit_max);
	if (rc) {
		bcdev->otg_curr_limit_max = USB_OTG_CURR_LIMIT_MAX;
	}

	rc = of_property_read_u32(node, "qcom,otg_curr_limit_high", &bcdev->otg_curr_limit_high);
	if (rc) {
		bcdev->otg_curr_limit_high = USB_OTG_CURR_LIMIT_HIGH;
	}

	rc = of_property_read_u32(node, "qcom,otg_real_soc_min", &bcdev->otg_real_soc_min);
	if (rc) {
...
static int battery_chg_parse_dt(struct battery_chg_dev *bcdev)
{
...
#endif
	of_property_read_string(node, "qcom,wireless-fw-name", &bcdev->wls_fw_name);
	...
	for (i = 0; i < len; i++) {
		rc = of_property_read_u32_index(node, "qcom,thermal-mitigation", i, &val);
		if (rc < 0)
			return rc;
	...
	bcdev->thermal_levels = devm_kcalloc(bcdev->dev, len + 1, sizeof(*bcdev->thermal_levels), GFP_KERNEL);
	...
	rc = of_property_read_u32_array(node, "qcom,thermal-mitigation", &bcdev->thermal_levels[1], len);
	if (rc < 0) {
...
static int battery_chg_ship_mode(struct notifier_block *nb, unsigned long code, void *unused)
{
	struct battery_charger_notify_msg msg_notify = { { 0 } };
	struct battery_charger_ship_mode_req_msg msg = { { 0 } };
	struct battery_chg_dev *bcdev = container_of(nb, struct battery_chg_dev, reboot_notifier);
	int rc;
...
static int oplus_usbtemp_iio_init(struct oplus_chg_chip *chip)
{
	...
	if (rc >= 0) {
		bcdev->iio.usbtemp_sup_v_chan = iio_channel_get(bcdev->dev, "usb_supplementary_temp_adc");
...
static int oplus_subboard_temp_iio_init(struct oplus_chg_chip *chip)
{
	...
	if (rc >= 0) {
		bcdev->iio.subboard_temp_v_chan = iio_channel_get(bcdev->dev, "subboard_temp_adc");
...
static void oplus_usbtemp_recover_tbatt_temp(struct oplus_chg_chip *chip)
{
	...
	if ((chg_type == ADAPTER_ID_20W_0X13 || chg_type == ADAPTER_ID_20W_0X34 || chg_type == ADAPTER_ID_20W_0X45) &&
	    chip->usbtemp_change_gap) { /*20W*/
		...
		chip->usbtemp_batttemp_recover_gap = USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
		    ...
		    chg_type == ADAPTER_ID_30W_0X46) &&
		   chip->usbtemp_change_gap) { /*30W*/
		chip->usbtemp_batttemp_gap = USBTEMP_BATTTEMP_GAP_HIGH;
		chip->usbtemp_batttemp_current_gap = USBTEMP_BATTTEMP_CURRENT_GAP_HIGH;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH;
		chip->usbtemp_batttemp_recover_gap = USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
		    ...
		    chg_type == ADAPTER_ID_33W_0X61) &&
		   chip->usbtemp_change_gap) { /*33W*/
		chip->usbtemp_batttemp_gap = USBTEMP_BATTTEMP_GAP_HIGH;
		chip->usbtemp_batttemp_current_gap = USBTEMP_BATTTEMP_CURRENT_GAP_HIGH;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH;
		...
		chip->usbtemp_batttemp_recover_gap = USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
		    ...
		    chg_type == ADAPTER_ID_65W_0X6E) &&
		   chip->usbtemp_change_gap) { /*65W*/
		chip->usbtemp_batttemp_gap = USBTEMP_BATTTEMP_GAP_HIGH;
		chip->usbtemp_batttemp_current_gap = USBTEMP_BATTTEMP_CURRENT_GAP_HIGH;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH;
		...
		chip->usbtemp_batttemp_recover_gap = USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
		    ...
		    chg_type == ADAPTER_ID_80W_0X65) &&
		   chip->usbtemp_change_gap) { /*80W*/
		chip->usbtemp_batttemp_gap = USBTEMP_BATTTEMP_GAP_HIGH;
		chip->usbtemp_batttemp_current_gap = USBTEMP_BATTTEMP_CURRENT_GAP_HIGH;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH;
...
static void oplus_update_usbtemp_current_status_100w(struct oplus_chg_chip *chip)
{
	...
	if (((chip->usb_temp_r - g_tbatt_temp / 10) >= chip->usbtemp_batttemp_current_gap) ||
	    (chip->usb_temp_r >= usbtemp_batttemp_current_thr)) {
		limit_cur_cnt_r++;
		if (limit_cur_cnt_r >= RETRY_COUNT) {
			limit_cur_cnt_r = RETRY_COUNT;
		}
		recover_cur_cnt_r = 0;
		   ...
		   (chip->usb_temp_r <= usbtemp_batttemp_recover_thr)) {
		recover_cur_cnt_r++;
		if (recover_cur_cnt_r >= RETRY_COUNT) {
			recover_cur_cnt_r = RETRY_COUNT;
		}
	...
	if (((chip->usb_temp_l - g_tbatt_temp / 10) >= chip->usbtemp_batttemp_current_gap) ||
	    (chip->usb_temp_l >= usbtemp_batttemp_current_thr)) {
		limit_cur_cnt_l++;
		if (limit_cur_cnt_l >= RETRY_COUNT) {
			limit_cur_cnt_l = RETRY_COUNT;
		}
		recover_cur_cnt_l = 0;
		   ...
		   (chip->usb_temp_l <= usbtemp_batttemp_recover_thr)) {
		recover_cur_cnt_l++;
		if (recover_cur_cnt_l >= RETRY_COUNT) {
			recover_cur_cnt_l = RETRY_COUNT;
		}
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (g_tbatt_temp / 10 > USB_50C &&
		    (((chip->usb_temp_l >= g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
		      (chip->usb_temp_l < USB_100C)) ||
		     ((chip->usb_temp_r >= g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
			...
			for (i = 1; i <= retry_cnt; i++) {
				...
				oplus_get_usbtemp_volt(chip);
				get_usb_temp(chip);
				if ((chip->usb_temp_r >= g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >= g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
				    chip->usb_temp_l < USB_100C)
					count_l++;
				pr_err("countl : %d", count_l);
			}
		...
		if ((((chip->usb_temp_l - g_tbatt_temp / 10) > chip->usbtemp_batttemp_gap) &&
		     (chip->usb_temp_l < USB_100C)) ||
		    (((chip->usb_temp_r - g_tbatt_temp / 10) > chip->usbtemp_batttemp_gap) &&
		     (chip->usb_temp_r < USB_100C))) {
			if (count == 0) {
				last_usb_temp_r = chip->usb_temp_r;
				last_usb_temp_l = chip->usb_temp_l;
...
bool oplus_usbtemp_l_trigger_current_status(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((g_oplus_chip->usb_temp_l >= g_oplus_chip->usbtemp_cool_down_ntc_low) ||
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) >= g_oplus_chip->usbtemp_cool_down_gap_low)
			return true;
		return false;
		...
		if ((g_oplus_chip->usb_temp_l >= g_oplus_chip->usbtemp_cool_down_ntc_high) ||
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) >= g_oplus_chip->usbtemp_cool_down_gap_high)
			return true;
		return false;
	} else {
		return false;
...
bool oplus_usbtemp_l_recovery_current_status(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((g_oplus_chip->usb_temp_l <= g_oplus_chip->usbtemp_cool_down_recover_ntc_low) &&
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) <= g_oplus_chip->usbtemp_cool_down_recover_gap_low)
			return true;
		return false;
	} else if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_HIGH_CURR) {
		if ((g_oplus_chip->usb_temp_l <= g_oplus_chip->usbtemp_cool_down_recover_ntc_high) &&
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) <= g_oplus_chip->usbtemp_cool_down_recover_gap_high)
			return true;
		return false;
	} else {
		return false;
...
bool oplus_usbtemp_r_trigger_current_status(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((g_oplus_chip->usb_temp_r >= g_oplus_chip->usbtemp_cool_down_ntc_low) ||
		    (g_oplus_chip->usb_temp_r - g_tbatt_temp / 10) >= g_oplus_chip->usbtemp_cool_down_gap_low)
			return true;
		return false;
	} else if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_HIGH_CURR) {
		if ((g_oplus_chip->usb_temp_r >= g_oplus_chip->usbtemp_cool_down_ntc_high) ||
		    (g_oplus_chip->usb_temp_r - g_tbatt_temp / 10) >= g_oplus_chip->usbtemp_cool_down_gap_high)
			return true;
		return false;
	} else {
		return false;
...
bool oplus_usbtemp_r_recovery_current_status(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		...
		if ((g_oplus_chip->usb_temp_r <= g_oplus_chip->usbtemp_cool_down_recover_ntc_high) &&
		    (g_oplus_chip->usb_temp_r - g_tbatt_temp / 10) <= g_oplus_chip->usbtemp_cool_down_recover_gap_high)
			return true;
		return false;
	} else {
		return false;
...
bool oplus_usbtemp_temp_rise_fast_with_batt_temp(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if (g_tbatt_temp / 10 > g_oplus_chip->usbtemp_batt_temp_low &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_tbatt_temp / 10 + g_oplus_chip->usbtemp_temp_gap_low_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		     ((g_oplus_chip->usb_temp_r >=
		       g_tbatt_temp / 10 + g_oplus_chip->usbtemp_temp_gap_low_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_r < USB_100C))))
			return true;
		return false;
		...
		if (g_tbatt_temp / 10 > g_oplus_chip->usbtemp_batt_temp_high &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_tbatt_temp / 10 + g_oplus_chip->usbtemp_temp_gap_high_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		     ((g_oplus_chip->usb_temp_r >=
		       g_tbatt_temp / 10 + g_oplus_chip->usbtemp_temp_gap_high_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_r < USB_100C))))
...
bool oplus_usbtemp_trigger_for_rise_fast_temp(struct oplus_chg_chip *chip, int retry_cnt, int count_r, int count_l)
{
	...
	if (oplus_usbtemp_temp_rise_fast_with_batt_temp()) {
		...
		for (i = 1; i <= retry_cnt; i++) {
			...
			if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
				if ((chip->usb_temp_r >=
				     g_tbatt_temp / 10 + chip->usbtemp_temp_gap_low_with_batt_temp) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >=
				     g_tbatt_temp / 10 + chip->usbtemp_temp_gap_low_with_batt_temp) &&
				...
				if ((chip->usb_temp_r >=
				     g_tbatt_temp / 10 + chip->usbtemp_temp_gap_high_with_batt_temp) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >=
				     g_tbatt_temp / 10 + chip->usbtemp_temp_gap_high_with_batt_temp) &&
...
static void oplus_usbtemp_thread_init(void)
{
	if (g_oplus_chip->support_usbtemp_protect_v2)
		oplus_usbtemp_kthread = kthread_run(oplus_usbtemp_monitor_main_new_method, 0, "usbtemp_kthread");
	else
...
static void dump_regs(void)
{
	...
	if (dump_count == DUMP_LOG_CNT_30S) {
		...
		if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
			printk(KERN_ERR
			       "sm8450_st_dump: [chg_en=%d, suspend=%d, pd_svooc=%d, subtype=0x%02x],"
			       ...
			       "[0x%4x=0x%02x, 0x%4x=0x%02x, 0x%4x=0x%02x, 0x%4x=0x%02x], "
			       "[0x%4x=0x%02x], \n",
			       smbchg_get_charge_enable(), bcdev->read_buffer_dump.data_buffer[9],
			       bcdev->read_buffer_dump.data_buffer[11], oplus_chg_get_charger_subtype(),
			       ...
			       bcdev->read_buffer_dump.data_buffer[extra_num - 1],
			       bcdev->read_buffer_dump.data_buffer[extra_num],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 1],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 2],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 3],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 4],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 5],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 6],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 7],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 8],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 9],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 10],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 11],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 12],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 13],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 14],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 15],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 16],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 17],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 18],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 19],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 20],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 21],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 22],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 23],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 24],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 25],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 26],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 27],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 28],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 29],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 30],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 31],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 32],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 33],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 34],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 35],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 36],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 37],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 38],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 39],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 40],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 41],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 42],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 43],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 44],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 45],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 46],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 47],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 48],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 49],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 50],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 51],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 52],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 53],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 54],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 55],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 56],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 57],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 58],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 59],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 60],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 61],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 62],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 63],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 64],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 65],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 66],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 67],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 68],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 69],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 70],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 71],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 72],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 73],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 74],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 75],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 76],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 77],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 78],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 79],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 80],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 81],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 82],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 83],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 84],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 85],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 86],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 87],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 88],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 89],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 90],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 91],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 92],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 93],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 94],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 95],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 96],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 97],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 98],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 99],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 100],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 101],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 102],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 103],
			       bcdev->read_buffer_dump.data_buffer[extra_num + 104]);
		} else {
			printk(KERN_ERR
			       "sm8350_st_dump: [chg_en=%d, suspend=%d, pd_svooc=%d, subtype=0x%02x, is_abnormal_adapter=%d]\n",
			       smbchg_get_charge_enable(), bcdev->read_buffer_dump.data_buffer[9],
			       bcdev->read_buffer_dump.data_buffer[11], oplus_chg_get_charger_subtype(),
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		snprintf(battlog_buck_ic_reg_info, BATTERY_LOG_REG_MAX_SIZE, ", %d, %d, %d, 0x%02x, %d, %d, 0x%02x, %d",
			 smbchg_get_charge_enable(), bcdev->read_buffer_dump.data_buffer[9],
			 bcdev->read_buffer_dump.data_buffer[11], oplus_chg_get_charger_subtype(),
			 bcdev->read_buffer_dump.data_buffer[10], bcdev->read_buffer_dump.data_buffer[12],
			 bcdev->cid_status, bcdev->usb_in_status);
	} else {
		snprintf(battlog_buck_ic_reg_info, BATTERY_LOG_REG_MAX_SIZE, ", %d, %d, %d, 0x%02x, %d",
			 smbchg_get_charge_enable(), bcdev->read_buffer_dump.data_buffer[9],
			 bcdev->read_buffer_dump.data_buffer[11], oplus_chg_get_charger_subtype(),
			 chip->is_abnormal_adapter);
	}
...
aicl_pre_step:
	...
	if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP && current_ma >= OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA &&
	     usb_icl[i] < OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA) ||
	    bcdev->debug_force_icl_err)
		oplus_chg_track_upload_icl_err_info(bcdev, TRACK_PMIC_ERR_ICL_VBUS_LOW_POINT);
...
aicl_boost_back:
	...
	if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP && current_ma >= OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA &&
	     usb_icl[i] < OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA) ||
	    bcdev->debug_force_icl_err)
		oplus_chg_track_upload_icl_err_info(bcdev, TRACK_PMIC_ERR_ICL_VBUS_COLLAPSE);
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY &&
	    (oplus_voocphy_get_fastchg_ing() == true || oplus_voocphy_get_fastchg_start() == true) &&
...
static void oplus_chg_status_send_adsp_work(struct work_struct *work)
{
	...
	if (!oplus_chg_get_adsp_notify_ap_suspend() && chg_enable && !chg_suspend) {
		chg_status = true;
	}
...
static void oplus_recheck_input_current_work(struct work_struct *work)
{
	...
	chg_vol = oplus_chg_get_charger_voltage();
	if (!oplus_voocphy_get_fastchg_start() && !oplus_voocphy_get_fastchg_ing()) {
	...
	if (chg_vol > VOLTAGE_2000MV) {
		...
		if (count > COUNT_TEN) {
			...
			chg_err("reset input current err_cnt: %d,chg_vol:%d,fastchg_start:%d,fastchg_ing:%d,ibus_curr:%d,fast_chg_type:%d\n",
				err_cnt, chg_vol, oplus_voocphy_get_fastchg_start(), oplus_voocphy_get_fastchg_ing(),
				ibus_curr, fast_chg_type);
...
static void oplus_plugin_irq_work(struct work_struct *work)
{
	...
	if (chip->support_check_usbin_status && usbin_status_test > 0) {
		bcdev->usb_in_status = 1;
		usb_plugin_status = 1;
...
int oplus_get_otg_online_status(void)
{
	...
	if (otg_scheme == OTG_SCHEME_CID && bcdev->cid_status == 0 && chip->otg_switch == false) {
		chip->otg_online = 0;
		return 0;
	}
	...
	if ((pre_cid_status ^ cid_status) || (pre_typec_otg ^ typec_otg) || (pre_level ^ level)) {
		...
		printk(KERN_ERR "[OPLUS_CHG][%s]: level[%d], cid_status[%d], typec_otg[%d], otg_online[%d]\n", __func__,
		       level, cid_status, typec_otg, online);
	}
...
static void oplus_check_abnormal_usbin_status_work(struct work_struct *work)
{
	...
	if (bcdev->usb_in_status && oplus_vooc_get_fastchg_started() == false && !chip->usbin_abnormal_status &&
	    chip->charger_volt <= VOLTAGE_800MV && chip->usb_present_vbus0_count <= MAX_VBUS_CHECK_COUNTS) {
		chip->usb_present_vbus0_count++;

		if (chip->usb_present_vbus0_count == MAX_VBUS_CHECK_COUNTS || !bcdev->cid_status) {
	...
	if ((!bcdev->usb_in_status) || (bcdev->usb_in_status && chip->charger_volt > VOLTAGE_800MV)) {
		chip->usbin_abnormal_status = false;
		chip->usb_present_vbus0_count = 0;
		chip->check_usbin_from_adsp_cnt = 0;
	}

	if (bcdev->usb_in_status && chip->usbin_abnormal_status && chip->charger_volt <= VOLTAGE_800MV &&
	...
	chg_err("[%d  %d  %d  %d  %d]\n", chip->usbin_abnormal_status, chip->usb_present_vbus0_count,
		chip->check_usbin_from_adsp_cnt, bcdev->usb_in_status, chip->charger_volt);
}
...
bool oplus_chg_is_usb_present(void)
{
	...
	if (oplus_vchg_trig_is_support() == true && oplus_get_vchg_trig_status() == 1 && vbus_rising == true) {
	...
	if ((oplus_chg_get_voocphy_support() != ADSP_VOOCPHY) && vbus_rising == false &&
	    oplus_vooc_get_fastchg_started() == true) {
...
static int oplus_chg_get_dyna_aicl_result(void)
{
	...
	if (usb_psy) {
		power_supply_get_property(usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
...
int oplus_chg_get_charger_subtype(void)
{
	...
	case POWER_SUPPLY_USB_TYPE_PD_PPS:
		charg_subtype = CHARGER_SUBTYPE_PD;
		if (oplus_pps_get_chg_status() != PPS_NOT_SUPPORT && oplus_get_pps_type() == true &&
...
static int fg_bq27541_get_battery_mvolts(void)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY && oplus_pps_get_chg_status() != PPS_CHARGERING) {
...
static int oplus_chg_usb_get_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop,
				  union oplus_chg_mod_propval *pval)
{
	return -EINVAL;
}

static int oplus_chg_usb_set_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop,
				  const union oplus_chg_mod_propval *pval)
{
	return -EINVAL;
}

static int oplus_chg_usb_prop_is_writeable(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop)
{
	return 0;
}
...
static int oplus_chg_usb_event_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
...
static int oplus_chg_usb_init_mod(struct battery_chg_dev *bcdev)
{
	...
	bcdev->usb_ocm = oplus_chg_mod_register(bcdev->dev, &oplus_chg_usb_mod_desc, &ocm_cfg);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, otg_status_check_work);
	int rc;
	...
	if (bcdev->otg_boost_src == OTG_BOOST_SOURCE_EXTERNAL) {
		...
		if (oplus_switching_support_parallel_chg()) {
			batt_current = oplus_gauge_get_batt_current() + oplus_gauge_get_sub_batt_current();
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int oplus_chg_ssr_notifier_cb(struct notifier_block *nb, unsigned long code, void *data)
{
	struct battery_chg_dev *bcdev = NULL;
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	case QCOM_SSR_BEFORE_SHUTDOWN:
		...
		oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_SSR_BEFORE_SHUTDOWN);
		break;
	case QCOM_SSR_AFTER_POWERUP:
		oplus_adsp_crash_recover_work();
		oplus_chg_track_upload_adsp_err_info(bcdev, TRACK_ADSP_ERR_SSR_AFTER_POWERUP);
		break;
	default:
		break;
	}
...
static void oplus_chg_track_dump_reg_info(char *dump_info, int len)
{
	...
	index += snprintf(&(dump_info[index]), len - index,
			  ...
			  bcdev->read_buffer_dump.data_buffer[10], bcdev->read_buffer_dump.data_buffer[12],
			  bcdev->cid_status, bcdev->usb_in_status);
	for (i = OPLUS_REG_INDEX_START; i < OPLUS_REG_INDEX_MAX && i < (MAX_OEM_PROPERTY_DATA_SIZE - 1); i += 2) {
		index += snprintf(&(dump_info[index]), len - index, "0x%4x=0x%02x,",
				  bcdev->read_buffer_dump.data_buffer[i], bcdev->read_buffer_dump.data_buffer[i + 1]);
...
static int oplus_chg_track_upload_adsp_err_info(struct battery_chg_dev *bcdev, int err_type)
{
	...
	bcdev->adsp_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	bcdev->adsp_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_PLAT_PMIC_ABNORMAL;
	bcdev->adsp_err_uploading = true;
	upload_count++;
	pre_upload_time = oplus_chg_track_get_local_time_s();
	mutex_unlock(&bcdev->track_adsp_err_lock);

	index += snprintf(&(bcdev->adsp_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "pm8350b");
	index += snprintf(&(bcdev->adsp_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_ADSP_ERR);
...
static int oplus_chg_track_upload_icl_err_info(struct battery_chg_dev *bcdev, int err_type)
{
	...
	bcdev->icl_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	bcdev->icl_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_PLAT_PMIC_ABNORMAL;
	bcdev->icl_err_uploading = true;
	upload_count++;
	pre_upload_time = oplus_chg_track_get_local_time_s();
	mutex_unlock(&bcdev->track_icl_err_lock);

	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "pm8350b");
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
	...
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  bcdev->chg_power_info);
...
static void oplus_chg_track_icl_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, icl_err_load_trigger_work);
...
static void oplus_chg_track_adsp_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork, struct battery_chg_dev, adsp_err_load_trigger_work);
...
static int oplus_chg_track_debugfs_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->debug_pmic_glink_err = 0;
	debugfs_create_u32("debug_force_icl_err", 0644, debugfs_pm8350b, &(bcdev->debug_force_icl_err));
	debugfs_create_u32("debug_pmic_glink_err", 0644, debugfs_pm8350b, &(bcdev->debug_pmic_glink_err));
...
static int oplus_chg_track_init(struct battery_chg_dev *bcdev)
{
	...
	INIT_DELAYED_WORK(&bcdev->icl_err_load_trigger_work, oplus_chg_track_icl_err_load_trigger_work);
	INIT_DELAYED_WORK(&bcdev->adsp_err_load_trigger_work, oplus_chg_track_adsp_err_load_trigger_work);
...
static int buck_ic_get_log_head(char *buffer, int size, void *dev_data)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		...
		snprintf(buffer, size, ", [buck_ic]:chg_en, suspend, pd_svooc, subtype, is_abnormal_adapter");
	}
	return 0;
}
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < PSY_TYPE_MAX; i++) {
		bcdev->psy_list[i].prop =
			devm_kcalloc(&pdev->dev, bcdev->psy_list[i].prop_count, sizeof(u32), GFP_KERNEL);
		if (!bcdev->psy_list[i].prop)
			return -ENOMEM;
	}

	bcdev->psy_list[PSY_TYPE_BATTERY].model = devm_kzalloc(&pdev->dev, MAX_STR_LEN, GFP_KERNEL);
	...
	INIT_DELAYED_WORK(&bcdev->otg_status_check_work,
			  oplus_otg_status_check_work); //OTG software OCP/OTP protect work
	INIT_DELAYED_WORK(&bcdev->adsp_track_notify_work, adsp_track_notification_handler);
	...
	bcdev->subsys_handle = qcom_register_ssr_notifier("lpass", &bcdev->ssr_nb);
	...
	schedule_delayed_work(&bcdev->adsp_voocphy_enable_check_work, round_jiffies_relative(msecs_to_jiffies(1500)));
...
static void battery_chg_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip && g_oplus_chip->pmic_spmi.bcdev_chip->wls_sim_detect_wr &&
	    oplus_chg_wls_is_present(g_oplus_chip)) {
		if (is_wls_ocm_available(g_oplus_chip)) {
			pval.intval = 0;
			oplus_chg_mod_set_property(g_oplus_chip->wls_ocm, OPLUS_CHG_PROP_CHG_ENABLE, &pval);
	...
	if (g_oplus_chip && g_oplus_chip->chg_ops->charger_suspend && g_oplus_chip->chg_ops->charger_unsuspend) {
