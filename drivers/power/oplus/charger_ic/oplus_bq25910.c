...
static ssize_t bq25910_reg_access_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "0x%02x\n", reg_access_allow);
}

static ssize_t bq25910_reg_access_store(struct device *dev, struct device_attribute *attr, const char *buf,
					size_t count)
{
	unsigned int val = 0;
	int rc = 0;
...
static ssize_t bq25910_reg_set_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t count = 0;
	int reg_val = 0;
...
static ssize_t bq25910_reg_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t bq25910_regs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int i = 0;
	int reg_val = 0;
...
static ssize_t bq25910_regs_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
...
static struct attribute *bq25910_attributes[] = { &dev_attr_reg_access.attr, &dev_attr_reg_set.attr,
						  &dev_attr_read_regs.attr, NULL };

static struct attribute_group bq25910_attribute_group = { .attrs = bq25910_attributes };
...
int bq25910_set_vindpm_vol(int vol)
{
	...
	rc = bq25910_config_interface(REG02_BQ25910_ADDRESS, tmp << REG02_BQ25910_VINDPM_THRESHOLD_SHIFT,
				      REG02_BQ25910_VINDPM_THRESHOLD_MASK);
...
int bq25910_input_current_limit_write(int current_ma)
{
	...
	rc = bq25910_config_interface(REG03_BQ25910_ADDRESS, (current_ma - REG03_BQ25910_CURRENT_LIMIT_OFFSET) / 100,
				      REG03_BQ25910_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
int bq25910_charging_current_write_fast(int chg_cur)
{
	...
	rc = bq25910_config_interface(REG01_BQ25910_ADDRESS, tmp << REG01_BQ25910_CHARGE_CURRENT_SETTING_SHIFT,
				      REG01_BQ25910_CHARGE_CURRENT_SETTING_MASK);
...
int bq25910_set_complete_charge_timeout(int val)
{
	...
	rc = bq25910_config_interface(REG05_BQ25910_ADDRESS, val,
				      REG05_BQ25910_FAST_CHARGE_TIMER_MASK | REG05_BQ25910_CHARGE_TIMER_EN_MASK);
	if (rc < 0) {
...
int bq25910_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq25910_config_interface(REG00_BQ25910_ADDRESS, tmp << REG00_BQ25910_CHARGE_FULL_VOL_SHIFT,
				      REG00_BQ25910_CHARGE_FULL_VOL_MASK);
...
int bq25910_set_chging_term_disable(void)
{
	...
	rc = bq25910_config_interface(REG05_BQ25910_ADDRESS, REG05_BQ25910_CHARGE_TERMINATION_EN_DISABLE,
				      REG05_BQ25910_CHARGE_TERMINATION_EN_MASK);
	if (rc) {
...
int bq25910_enable_charging(void)
{
	...
	dump_stack();
	rc = bq25910_config_interface(REG06_BQ25910_ADDRESS, REG06_BQ25910_CHG_EN_ENABLE, REG06_BQ25910_CHG_EN_MASK);
	if (rc < 0) {
...
int bq25910_disable_charging(void)
{
	...
	rc = bq25910_config_interface(REG06_BQ25910_ADDRESS, REG06_BQ25910_CHG_EN_DISABLE, REG06_BQ25910_CHG_EN_MASK);
	if (rc < 0) {
...
int bq25910_registers_read_full(void)
{
	...
	reg_full =
		((reg_full & REG09_BQ25910_CHRG_TERM_FLAG_MASK) == REG09_BQ25910_CHARGING_STATUS_CHARGE_TERMINATION) ?
			1 :
			0;
	if (reg_full) {
...
int bq25910_reset_charger_for_wired_charge(void)
{
	...
	rc = bq25910_config_interface(REG0D_BQ25910_ADDRESS, REG0D_BQ25910_REG_RST_RESET, REG0D_BQ25910_REG_RST_MASK);
	if (rc < 0) {
...
void bq25910_dump_registers(void)
{
	...
	if (dump_count == DUMP_REG_LOG_CNT_30S) {
		...
		printk(KERN_ERR
		       "bq25910[0-0D]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
		       val_buf[0], val_buf[1], val_buf[2], val_buf[3], val_buf[4], val_buf[5], val_buf[6], val_buf[7],
		       val_buf[8], val_buf[9], val_buf[10], val_buf[11], val_buf[12], val_buf[13]);
	}
	dump_count++;
}
...
int bq25910_input_current_limit_init(void)
{
	...
	rc = bq25910_config_interface(REG03_BQ25910_ADDRESS, REG03_BQ25910_CURRENT_LIMIT_500MA,
				      REG03_BQ25910_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
static int bq25910_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chg_ic = devm_kzalloc(&client->dev, sizeof(struct chip_bq25910), GFP_KERNEL);
	if (!chg_ic) {
