...
struct USB_PORT_TEMPERATURE Usb_Port_Temperature_Table[USB_NTC_TABLE_SIZE] = {
	...
	{ -40, 4397119 }, { -35, 3088599 }, { -30, 2197225 }, { -25, 1581881 }, { -20, 1151037 }, { -15, 846579 },
	{ -10, 628988 },  { -5, 471632 },   { 0, 357012 },    { 5, 272500 },	{ 10, 209710 },	  { 15, 162651 },
	{ 20, 127080 },	  { 25, 100000 }, /* 100K */
	{ 30, 79222 },	  { 35, 63167 },    { 40, 50677 },    { 41, 48528 },	{ 42, 46482 },	  { 43, 44533 },
	...
	{ 62, 20560 },	  { 63, 19782 },    { 64, 19036 },    { 65, 18322 },	{ 66, 17640 },	  { 67, 16986 },
	{ 68, 16360 },	  { 69, 15759 },    { 70, 15184 },    { 71, 14631 },	{ 72, 14100 },	  { 73, 13591 },
	{ 74, 13103 },	  { 75, 12635 },    { 76, 12187 },    { 77, 11756 },	{ 78, 11343 },	  { 79, 10946 },
	{ 80, 10565 },	  { 81, 10199 },    { 82, 9847 },     { 83, 9509 },	{ 84, 9184 },	  { 85, 8872 },
	{ 86, 8572 },	  { 87, 8283 },	    { 88, 8005 },     { 89, 7738 },	{ 90, 7481 },	  { 95, 6337 },
	{ 100, 5384 },	  { 105, 4594 },    { 110, 3934 },    { 115, 3380 },	{ 120, 2916 },	  { 125, 2522 }
};
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (init_flag == true) {
			init_flag = false;
			if (oplus_chg_get_vbus_status(chip) == false && oplus_chg_get_otg_online() == false) {
		...
		if ((chip->usb_temp_r >= USB_57C) || (chip->usb_temp_l >= USB_57C)) {
			...
			if (count_r >= RETRY_COUNT || count_l >= RETRY_COUNT) {
				if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
					...
					chg_err("dischg enable...usb_temp[%d,%d], usb_volt[%d,%d]\n", chip->usb_temp_r,
						chip->usb_temp_l, chip->usbtemp_volt_r, chip->usbtemp_volt_l);
#ifndef CONFIG_HIGH_TEMP_VERSION
			...
			if (count <= time_count) {
				...
				if (((chip->usb_temp_r - last_usb_temp_r) >= 3 && chip->usb_temp_r >= USB_30C &&
				     chip->usb_temp_r <= USB_100C) ||
				    ((chip->usb_temp_l - last_usb_temp_l) >= 3 && chip->usb_temp_l >= USB_30C &&
					...
					if (count_r >= RETRY_COUNT || count_l >= RETRY_COUNT) {
						if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
							...
							chg_err("dischg enable...current_usb_temp[%d,%d], last_usb_temp[%d,%d], count[%d]\n",
								chip->usb_temp_r, chip->usb_temp_l, last_usb_temp_r,
								last_usb_temp_l, count);
#ifndef CONFIG_HIGH_TEMP_VERSION
							oplus_set_usb_status(USB_TEMP_HIGH);
							if (oplus_chg_get_otg_online() == true) {
								oplus_set_otg_switch_status(false);
							...
							alarm_start_relative(&usbotp_recover_timer,
									     ktime_set(15300, 0));
#endif /*CONFIG_HIGH_TEMP_VERSION*/
						}
					}
		...
		check_again:
			...
			last_usb_temp_l = chip->usb_temp_l;
			msleep(delay);
			wait_event_interruptible(oplus_usbtemp_wq, (oplus_chg_get_vbus_status(chip) == true ||
								    oplus_chg_get_otg_online() == true) &&
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	proc_create_data("current_cmd", 0644, battery_dir, &mtk_chg_current_cmd_fops, info);
	proc_create_data("en_power_path", 0644, battery_dir, &mtk_chg_en_power_path_fops, info);
	proc_create_data("en_safety_timer", 0644, battery_dir, &mtk_chg_en_safety_timer_fops, info);
...
static int oplus_charger_probe(struct platform_device *pdev)
{
	...
	printk("oplus_charger_probe end %p, prev %p, next %p\n", &oplus_chip->batt_psy->dev.power.wakeup->entry,
	       oplus_chip->batt_psy->dev.power.wakeup->entry.prev, oplus_chip->batt_psy->dev.power.wakeup->entry.next);
