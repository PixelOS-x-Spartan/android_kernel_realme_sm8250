...
static const char *rt9471_ic_stat_name[RT9471_ICSTAT_MAX] = {
	"hz/sleep",	     "ready",	 "trickle-charge", "pre-charge", "fast-charge", "ieoc-charge",
	"background-charge", "done",	 "fault",	   "RESERVED",	 "RESERVED",	"RESERVED",
...
static const char *rt9471_port_name[RT9471_PORTSTAT_MAX] = {
	"NOINFO",    "RESERVED",    "RESERVED", "RESERVED",  "RESERVED", "RESERVED", "RESERVED", "RESERVED",
	"APPLE_10W", "SAMSUNG_10W", "APPLE_5W", "APPLE_12W", "NSDP",	 "SDP",	     "CDP",	 "DCP",
};
...
static int rt9471_write_device(void *client, u32 addr, int len, const void *src)
{
	int rc = 0;
	int retry = 3;
...
static inline int __rt9471_i2c_write_byte(struct rt9471_chip *chip, u8 cmd, u8 data)
{
	int ret = 0;
...
static inline int __rt9471_i2c_read_byte(struct rt9471_chip *chip, u8 cmd, u8 *data)
{
	int ret = 0;
	u8 regval = 0;
...
static inline int __rt9471_i2c_block_write(struct rt9471_chip *chip, u8 cmd, u32 len, const u8 *data)
{
	int ret = 0;
...
static int rt9471_i2c_block_write(struct rt9471_chip *chip, u8 cmd, u32 len, const u8 *data)
{
	int ret = 0;
...
static inline int __rt9471_i2c_block_read(struct rt9471_chip *chip, u8 cmd, u32 len, u8 *data)
{
	int ret = 0;
...
static int rt9471_i2c_block_read(struct rt9471_chip *chip, u8 cmd, u32 len, u8 *data)
{
	int ret = 0;
...
static int rt9471_i2c_test_bit(struct rt9471_chip *chip, u8 cmd, u8 shift, bool *is_one)
{
	int ret = 0;
	u8 regval = 0;
...
static int rt9471_i2c_update_bits(struct rt9471_chip *chip, u8 cmd, u8 data, u8 mask)
{
	int ret = 0;
	u8 regval = 0;
...
static inline u8 rt9471_closest_reg_via_tbl(const u32 *tbl, u32 tbl_size, u32 target)
{
	u32 i = 0;
...
static bool rt9471_is_vbusgd(struct rt9471_chip *chip)
{
	...
	ret = rt9471_i2c_test_bit(chip, RT9471_REG_STAT0, RT9471_ST_VBUSGD_SHIFT, &vbus_gd);
	if (ret < 0)
...
wait:
	...
	ret = (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_DPDMDET, RT9471_BC12_EN_MASK);
	if (ret < 0)
...
static int rt9471_enable_hidden_mode(struct rt9471_chip *chip, bool en)
{
	...
	if (en) {
		if (chip->hidden_mode_cnt == 0) {
			ret = rt9471_i2c_block_write(chip, 0xA0, ARRAY_SIZE(rt9471_val_en_hidden_mode),
...
static int __rt9471_get_ic_stat(struct rt9471_chip *chip, enum rt9471_ic_stat *stat)
{
	int ret = 0;
	u8 regval = 0;
...
static int __rt9471_get_mivr(struct rt9471_chip *chip, u32 *mivr)
{
	...
	*mivr = rt9471_closest_value(RT9471_MIVR_MIN, RT9471_MIVR_MAX, RT9471_MIVR_STEP, regval);
...
static int __rt9471_get_ichg(struct rt9471_chip *chip, u32 *ichg)
{
	...
	*ichg = rt9471_closest_value(RT9471_ICHG_MIN, RT9471_ICHG_MAX, RT9471_ICHG_STEP, regval);
...
static int __rt9471_get_aicr(struct rt9471_chip *chip, u32 *aicr)
{
	...
	*aicr = rt9471_closest_value(RT9471_AICR_MIN, RT9471_AICR_MAX, RT9471_AICR_STEP, regval);
...
static int __rt9471_get_cv(struct rt9471_chip *chip, u32 *cv)
{
	...
	*cv = rt9471_closest_value(RT9471_CV_MIN, RT9471_CV_MAX, RT9471_CV_STEP, regval);
...
static int __rt9471_get_ieoc(struct rt9471_chip *chip, u32 *ieoc)
{
	...
	*ieoc = rt9471_closest_value(RT9471_IEOC_MIN, RT9471_IEOC_MAX, RT9471_IEOC_STEP, regval);
...
{
	return rt9471_i2c_test_bit(chip, RT9471_REG_FUNCTION, RT9471_CHG_EN_SHIFT, en);
}
...
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_FUNCTION, RT9471_BATFETDIS_MASK);
}
...
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_CHGTIMER, RT9471_SAFETMR_EN_MASK);
}

static int __rt9471_enable_te(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_EOC, RT9471_TE_MASK);
}

static int __rt9471_enable_jeita(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_JEITA, RT9471_JEITA_EN_MASK);
}

static int __rt9471_disable_i2c_tout(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_TOP, RT9471_DISI2CTO_MASK);
}

static int __rt9471_enable_qon_rst(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_TOP, RT9471_QONRST_MASK);
}

static int __rt9471_enable_autoaicr(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_IBUS, RT9471_AUTOAICR_MASK);
}

static int __rt9471_enable_hz(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_FUNCTION, RT9471_HZ_MASK);
}

static int __rt9471_enable_otg(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_FUNCTION, RT9471_OTG_EN_MASK);
}

static int __rt9471_enable_chg(struct rt9471_chip *chip, bool en)
{
	dev_info(chip->dev, "%s en = %d\n", __func__, en);
	return (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_FUNCTION, RT9471_CHG_EN_MASK);
}
...
static int __rt9471_set_wdt(struct rt9471_chip *chip, u32 sec)
{
	...
	if (sec <= 40 && sec > 0)
		sec = 40;
	regval = rt9471_closest_reg_via_tbl(rt9471_wdt, ARRAY_SIZE(rt9471_wdt), sec);

	dev_info(chip->dev, "%s time = %d(0x%02X)\n", __func__, sec, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_TOP, regval << RT9471_WDT_SHIFT, RT9471_WDT_MASK);
}

static int __rt9471_set_otgcc(struct rt9471_chip *chip, u32 cc)
{
	dev_info(chip->dev, "%s cc = %d\n", __func__, cc);
	return (cc <= rt9471_otgcc[0] ? rt9471_clr_bit : rt9471_set_bit)(chip, RT9471_REG_OTGCFG, RT9471_OTGCC_MASK);
}
...
static int __rt9471_set_ichg(struct rt9471_chip *chip, u32 ichg)
{
	...
	regval = rt9471_closest_reg(RT9471_ICHG_MIN, RT9471_ICHG_MAX, RT9471_ICHG_STEP, ichg);

	dev_info(chip->dev, "%s ichg = %d(0x%02X)\n", __func__, ichg, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_ICHG, regval << RT9471_ICHG_SHIFT, RT9471_ICHG_MASK);
}
...
static int __rt9471_set_aicr(struct rt9471_chip *chip, u32 aicr)
{
	...
	regval = rt9471_closest_reg(RT9471_AICR_MIN, RT9471_AICR_MAX, RT9471_AICR_STEP, aicr);
	/* 0 & 1 are both 50mA */
	...
	return rt9471_i2c_update_bits(chip, RT9471_REG_IBUS, regval << RT9471_AICR_SHIFT, RT9471_AICR_MASK);
}
...
static int __rt9471_set_mivr(struct rt9471_chip *chip, u32 mivr)
{
	...
	regval = rt9471_closest_reg(RT9471_MIVR_MIN, RT9471_MIVR_MAX, RT9471_MIVR_STEP, mivr);

	dev_info(chip->dev, "%s mivr = %d(0x%02X)\n", __func__, mivr, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_VBUS, regval << RT9471_MIVR_SHIFT, RT9471_MIVR_MASK);
}

static int __rt9471_set_ovp(struct rt9471_chip *chip)
{
	return rt9471_i2c_update_bits(chip, RT9471_REG_VBUS, 0x2 << RT9471_OVP_SHIFT, RT9471_OVP_MASK);
}
...
static int __rt9471_set_cv(struct rt9471_chip *chip, u32 cv)
{
	...
	regval = rt9471_closest_reg(RT9471_CV_MIN, RT9471_CV_MAX, RT9471_CV_STEP, cv);

	dev_info(chip->dev, "%s cv = %d(0x%02X)\n", __func__, cv, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_VCHG, regval << RT9471_CV_SHIFT, RT9471_CV_MASK);
}
...
static int __rt9471_set_ieoc(struct rt9471_chip *chip, u32 ieoc)
{
	...
	regval = rt9471_closest_reg(RT9471_IEOC_MIN, RT9471_IEOC_MAX, RT9471_IEOC_STEP, ieoc);

	dev_info(chip->dev, "%s ieoc = %d(0x%02X)\n", __func__, ieoc, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_EOC, regval << RT9471_IEOC_SHIFT, RT9471_IEOC_MASK);
}
...
static int __rt9471_set_safe_tmr(struct rt9471_chip *chip, u32 hr)
{
	...
	regval = rt9471_closest_reg(RT9471_SAFETMR_MIN, RT9471_SAFETMR_MAX, RT9471_SAFETMR_STEP, hr);

	dev_info(chip->dev, "%s time = %d(0x%02X)\n", __func__, hr, regval);

	return rt9471_i2c_update_bits(chip, RT9471_REG_CHGTIMER, regval << RT9471_SAFETMR_SHIFT, RT9471_SAFETMR_MASK);
}
...
static int __rt9471_set_mivrtrack(struct rt9471_chip *chip, u32 mivr_track)
{
	...
	return rt9471_i2c_update_bits(chip, RT9471_REG_VBUS, mivr_track << RT9471_MIVRTRACK_SHIFT,
...
{
	int ret = 0, i = 0;
	struct rt9471_chip *chip = container_of(work, struct rt9471_chip, buck_dwork.work);
	bool chg_rdy = false, chg_done = false;
	u8 reg_addrs[] = { RT9471_REG_BUCK_HDEN4, RT9471_REG_BUCK_HDEN1, RT9471_REG_BUCK_HDEN2,
			   RT9471_REG_BUCK_HDEN4, RT9471_REG_BUCK_HDEN2, RT9471_REG_BUCK_HDEN1 };
	u8 reg_vals[] = { 0x77, 0x2F, 0xA2, 0x71, 0x22, 0x2D };

	dev_info(chip->dev, "%s chip_rev = %d\n", __func__, chip->chip_rev);
	if (chip->chip_rev > 4)
		return;
	ret = rt9471_i2c_test_bit(chip, RT9471_REG_STAT0, RT9471_ST_CHGRDY_SHIFT, &chg_rdy);
	if (ret < 0)
		return;
	dev_info(chip->dev, "%s chg_rdy = %d\n", __func__, chg_rdy);
	if (!chg_rdy)
		return;
	ret = rt9471_i2c_test_bit(chip, RT9471_REG_STAT0, RT9471_ST_CHGDONE_SHIFT, &chg_done);
	if (ret < 0)
		return;
	...
	for (i = 0; i < ARRAY_SIZE(reg_addrs); i++) {
		...
		if (ret < 0)
			dev_notice(chip->dev, "%s reg0x%02X = 0x%02X fail(%d)\n", __func__, reg_addrs[i], reg_vals[i],
				   ret);
		if (i == 1)
			udelay(1000);
	}
...
static void rt9471_bc12_done_handler(struct rt9471_chip *chip)
{
	...
	if (bc12_done) {
		...
		dev_info(chip->dev, "%s %d %s\n", __func__, chip->port, rt9471_port_name[chip->port]);
		mutex_unlock(&chip->bc12_lock);
	}
}
...
static int rt9471_chg_mivr_irq_handler(struct rt9471_chip *chip)
{
	...
	ret = rt9471_i2c_test_bit(chip, RT9471_REG_STAT1, RT9471_ST_MIVR_SHIFT, &mivr);
	if (ret < 0) {
		dev_notice(chip->dev, "%s check stat fail(%d)\n", __func__, ret);
		return ret;
	}
...
static int rt9471_vac_ov_irq_handler(struct rt9471_chip *chip)
{
	...
	ret = rt9471_i2c_test_bit(chip, RT9471_REG_STAT3, RT9471_ST_VACOV_SHIFT, &vacov);
	if (ret < 0) {
		dev_notice(chip->dev, "%s check stat fail(%d)\n", __func__, ret);
		return ret;
	}
...
static const struct irq_mapping_tbl rt9471_irq_mapping_tbl[] = {
	RT9471_IRQ_MAPPING(wdt, 29),	    RT9471_IRQ_MAPPING(vbus_gd, 7),	RT9471_IRQ_MAPPING(chg_rdy, 6),
	RT9471_IRQ_MAPPING(bc12_done, 0),   RT9471_IRQ_MAPPING(detach, 1),	RT9471_IRQ_MAPPING(rechg, 2),
	RT9471_IRQ_MAPPING(chg_done, 3),    RT9471_IRQ_MAPPING(bg_chg, 4),	RT9471_IRQ_MAPPING(ieoc, 5),
	RT9471_IRQ_MAPPING(chg_batov, 9),   RT9471_IRQ_MAPPING(chg_sysov, 10),	RT9471_IRQ_MAPPING(chg_tout, 11),
	RT9471_IRQ_MAPPING(chg_busuv, 12),  RT9471_IRQ_MAPPING(chg_threg, 13),	RT9471_IRQ_MAPPING(chg_aicr, 14),
	RT9471_IRQ_MAPPING(chg_mivr, 15),   RT9471_IRQ_MAPPING(sys_short, 16),	RT9471_IRQ_MAPPING(sys_min, 17),
	RT9471_IRQ_MAPPING(jeita_cold, 20), RT9471_IRQ_MAPPING(jeita_cool, 21), RT9471_IRQ_MAPPING(jeita_warm, 22),
	RT9471_IRQ_MAPPING(jeita_hot, 23),  RT9471_IRQ_MAPPING(otg_fault, 24),	RT9471_IRQ_MAPPING(otg_lbp, 25),
	RT9471_IRQ_MAPPING(otg_cc, 26),	    RT9471_IRQ_MAPPING(vac_ov, 30),	RT9471_IRQ_MAPPING(otp, 31),
};
...
static irqreturn_t rt9471_irq_handler(int irq, void *data)
{
	...
	ret = rt9471_i2c_block_read(chip, RT9471_REG_IRQ0, RT9471_IRQIDX_MAX, evt);
	if (ret < 0) {
		dev_notice(chip->dev, "%s read evt fail(%d)\n", __func__, ret);
		goto out;
	}

	ret = rt9471_i2c_block_read(chip, RT9471_REG_MASK0, RT9471_IRQIDX_MAX, mask);
	if (ret < 0) {
...
static int rt9471_register_irq(struct rt9471_chip *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, chip->irq, NULL, rt9471_irq_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT, name, chip);
	if (ret < 0) {
...
{
	dev_info(chip->dev, "%s\n", __func__);
	return rt9471_i2c_block_write(chip, RT9471_REG_MASK0, ARRAY_SIZE(chip->irq_mask), chip->irq_mask);
}

static inline int rt9471_get_irq_number(struct rt9471_chip *chip, const char *name)
{
	int i = 0;
...
{
	dev_dbg(chip->dev, "%s irq(%d, %s)\n", __func__, irqnum, rt9471_get_irq_name(irqnum));
...
static int rt9471_parse_dt(struct rt9471_chip *chip)
{
	...
	if (of_property_read_string(np, "chg_alias_name", &chip->chg_props.alias_name) < 0) {
		dev_notice(chip->dev, "%s no chg alias name\n", __func__);
		chip->chg_props.alias_name = "rt9471_chg";
	}
	dev_info(chip->dev, "%s name %s, alias name %s\n", __func__, desc->chg_name, chip->chg_props.alias_name);
...
#else
	ret = of_property_read_u32(parent_np, "rt,intr_gpio_num", &chip->intr_gpio);
	if (ret < 0)
		return ret;
	if (strcmp(desc->chg_name, "secondary_chg") == 0) {
		ret = of_property_read_u32(parent_np, "rt,ceb_gpio_num", &chip->ceb_gpio);
		if (ret < 0)
			return ret;
	}
#endif
	...
	if (strcmp(desc->chg_name, "secondary_chg") == 0) {
		...
		ret = devm_gpio_request_one(chip->dev, chip->ceb_gpio, GPIOF_DIR_OUT, ceb_name);
		if (ret < 0) {
			dev_notice(chip->dev, "%s gpio request fail(%d)\n", __func__, ret);
			return ret;
		}
...
static int rt9471_sw_workaround(struct rt9471_chip *chip)
{
	...
	chip->chip_rev = (regval & RT9471_CHIP_REV_MASK) >> RT9471_CHIP_REV_SHIFT;
	dev_info(chip->dev, "%s chip_rev = %d\n", __func__, chip->chip_rev);

	/* OTG load transient improvement */
	if (chip->chip_rev <= 3)
		ret = rt9471_i2c_update_bits(chip, RT9471_REG_OTG_HDEN2, 0x10, RT9471_REG_OTG_RES_COMP_MASK);
...
static int rt9471_init_setting(struct rt9471_chip *chip)
{
	...
	/* Mask all IRQs */
	ret = rt9471_i2c_block_write(chip, RT9471_REG_MASK0, ARRAY_SIZE(rt9471_irq_maskall), rt9471_irq_maskall);
	if (ret < 0)
	...
	if (strcmp(chip->desc->chg_name, "secondary_chg") == 0) {
		...
		if (ret < 0)
			dev_notice(chip->dev, "%s set disable chip fail(%d)\n", __func__, ret);
	}
...
static bool rt9471_check_devinfo(struct rt9471_chip *chip)
{
	...
	if (chip->dev_id != RT9470_DEVID && chip->dev_id != RT9470D_DEVID && chip->dev_id != RT9471_DEVID &&
	    chip->dev_id != RT9471D_DEVID) {
...
static int __rt9471_dump_registers(struct rt9471_chip *chip)
{
	...
	ret = rt9471_i2c_block_read(chip, RT9471_REG_STAT0, RT9471_STATIDX_MAX, stats);
	...
	dev_info(chip->dev,
		 ...
		 __func__, ichg / 1000, aicr / 1000, mivr / 1000, ieoc / 1000, cv / 1000, chg_en,
		 rt9471_ic_stat_name[ic_stat], stats[RT9471_STATIDX_STAT0], stats[RT9471_STATIDX_STAT1],
...
{
	struct rt9471_chip *chip = container_of(work, struct rt9471_chip, init_work);
...
{
	*uA = rt9471_closest_value(RT9471_ICHG_MIN, RT9471_ICHG_MAX, RT9471_ICHG_STEP, 0);
	return 0;
}
...
{
	*uA = rt9471_closest_value(RT9471_AICR_MIN, RT9471_AICR_MAX, RT9471_AICR_STEP, 0);
	return 0;
}
...
static int rt9471_get_mivr_state(struct charger_device *chg_dev, bool *in_loop)
{
	...
	return rt9471_i2c_test_bit(chip, RT9471_REG_STAT1, RT9471_ST_MIVR_SHIFT, in_loop);
}
...
static int rt9471_is_safety_timer_enabled(struct charger_device *chg_dev, bool *en)
{
	struct rt9471_chip *chip = dev_get_drvdata(&chg_dev->dev);

	return rt9471_i2c_test_bit(chip, RT9471_REG_CHGTIMER, RT9471_SAFETMR_EN_SHIFT, en);
}
...
static int rt9471_enable_discharge(struct charger_device *chg_dev, bool en)
{
	...
	ret = (en ? rt9471_set_bit : rt9471_clr_bit)(chip, RT9471_REG_TOP_HDEN, RT9471_FORCE_EN_VBUS_SINK_MASK);
	if (ret < 0)
		dev_notice(chip->dev, "%s en = %d fail(%d)\n", __func__, en, ret);
...
static int rt9471_set_boost_current_limit(struct charger_device *chg_dev, u32 uA)
{
	struct rt9471_chip *chip = dev_get_drvdata(&chg_dev->dev);
...
static int __rt9471_enable_chip(struct rt9471_chip *chip, bool en)
{
	...
	ret = rt9471_i2c_update_bits(chip, RT9471_HD_CHG_DIG2, is_enable << RT9471_FORCE_HZ_SHIFT,
...
static ssize_t shipping_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0, tmp = 0;
...
static ssize_t dump_more_reg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct rt9471_chip *chip = dev_get_drvdata(dev);
	int ret = 0;
...
static int rt9471_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = 0;
	...
	if (chip->dev_id == RT9470D_DEVID || chip->dev_id == RT9471D_DEVID) {
		snprintf(chip->bc12_en_name, sizeof(chip->bc12_en_name), "rt9471.%s", dev_name(chip->dev));
		...
		chip->bc12_en_kthread = kthread_run(rt9471_bc12_en_kthread, chip, chip->bc12_en_name);
...
//#ifdef CONFIG_RT_REGMAP
	...
	chip->chg_dev =
		charger_device_register(chip->desc->chg_name, chip->dev, chip, &rt9471_chg_ops, &chip->chg_props);
