...
static void oplus_mt_power_off(void)
{
	...
	if (g_oplus_chip->ac_online != true) {
		...
		if (tcpc_dev) {
			if (!(tcpc_dev->pd_wait_hard_reset_complete) && !oplus_mt6360_get_vbus_status()) {
				kernel_power_off();
			}
		}
	} else {
...
bool oplus_chg_check_qchv_condition(void)
{
	...
	if (chip->dual_charger_support && chip->charger_volt < QC_CHARGER_VOLTAGE_HIGH && chip->soc < QC_SOC_HIGH &&
