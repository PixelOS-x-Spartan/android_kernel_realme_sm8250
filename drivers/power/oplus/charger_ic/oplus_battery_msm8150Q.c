...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((chip->usbtemp_volt_l > USB_50C_VOLT) &&
		    (chip->usbtemp_volt_r > USB_50C_VOLT)) //get vbus when usbtemp < 50C
		...
		if ((chip->usbtemp_volt_l > USB_50C_VOLT) && (chip->usbtemp_volt_r > USB_50C_VOLT) &&
			...
			if (((chip->usb_temp_l >= USB_57C) && (chip->usbtemp_volt_l > USB_100C_VOLT)) ||
				...
				if (dischg_flag == false) {
					...
					if (((current_temp_l - last_usb_temp_l) >= 3) ||
						...
						if (((count_l >= retry_cnt) &&
						     (chip->usbtemp_volt_l > USB_100C_VOLT)) ||
						    ((count_r >= retry_cnt) &&
						     (chip->usbtemp_volt_r > USB_100C_VOLT))) {
							if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
								dischg_flag = true;
				...
				if (chip->vbatt_num == 2) {
					wait_event_interruptible(oplus_usbtemp_wq,
								 oplus_usb_or_otg_is_present() == true);
				} else {
					wait_event_interruptible(oplus_usbtemp_wq,
								 gpio_get_value(chg->ccdetect_gpio) == 0);
				}
			}
