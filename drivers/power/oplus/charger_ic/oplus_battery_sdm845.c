...
int smblib_get_prop_batt_voltage_now(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, val);
	return rc;
}

int smblib_get_prop_batt_current_now(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CURRENT_NOW, val);
	return rc;
}

int smblib_get_prop_batt_temp(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_TEMP, val);
	return rc;
}
...
int smblib_set_prop_system_temp_level(struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	vote(chg->pl_disable_votable, THERMAL_DAEMON_VOTER, chg->system_temp_level ? true : false, 0);
...
static int oplus_usbid_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.usbid_active = pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, "usbid_active");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.usbid_active)) {
		chg_err("get usbid_active fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.usbid_sleep = pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, "usbid_sleep");
...
void oplus_usbid_irq_register(struct oplus_chg_chip *chip)
{
	...
	retval = devm_request_threaded_irq(chip->dev, chip->normalchg_gpio.usbid_irq, NULL, usbid_change_handler,
					   IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "usbid-change",
					   chip);
	if (retval < 0) {
		chg_err("Unable to request "
			"usbid-change"
			" irq: %d\n",
			retval);
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	chip->normalchg_gpio.usbid_gpio = of_get_named_gpio(node, "qcom,usbid-gpio", 0);
	if (chip->normalchg_gpio.usbid_gpio <= 0) {
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.fv_votable, BATT_PROFILE_VOTER, true, vfloat_mv * 1000);
	if (rc < 0)
...
#ifdef OPLUS_FEATURE_CHG_BASIC /*Fanhong.Kong@ProDrv.CHG,add 2018/06/02 for SVOOC OTG*/
	if (chip->pmic_spmi.smb2_chip->chg.typec_mode == POWER_SUPPLY_TYPEC_SINK && chip->vbatt_num == 2) {
