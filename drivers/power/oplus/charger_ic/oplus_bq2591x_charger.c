...
static int bq2591x_update_bits(struct bq2591x *bq, u8 reg, u8 mask, u8 data)
{
	int ret;
	u8 tmp;
...
static int bq2591x_enable_charger(struct bq2591x *bq)
{
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_06, BQ2591X_EN_CHG_MASK, val);
	return ret;
}
...
static int bq2591x_enable_term(struct bq2591x *bq, bool enable)
{
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_05, BQ2591X_EN_TERM_MASK, val);
...
static int bq2591x_set_chargecurrent(struct charger_device *chg_dev, u32 curr)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_01, BQ2591X_ICHG_MASK, ichg);
}
...
static int bq2591x_set_chargevoltage(struct charger_device *chg_dev, u32 volt)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_00, BQ2591X_VREG_MASK, val);
}
...
static int bq2591x_set_input_volt_limit(struct charger_device *chg_dev, u32 volt)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_02, BQ2591X_VINDPM_MASK, val);
}
...
static int bq2591x_set_input_current_limit(struct charger_device *chg_dev, u32 curr)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_03, BQ2591X_IINLIM_MASK, val);
}
...
int bq2591x_set_watchdog_timer(struct bq2591x *bq, u8 timeout)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_05, BQ2591X_WDT_MASK, val);
}
...
static int bq2591x_reset_watchdog_timer(struct charger_device *chg_dev)
{
	...
	return bq2591x_update_bits(bq, BQ2591X_REG_05, BQ2591X_WDT_RESET_MASK, val);
}
...
int bq2591x_reset_chip(struct bq2591x *bq)
{
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_0D, BQ2591X_RESET_MASK, val);
	return ret;
}
...
int bq2591x_exit_hiz_mode(struct bq2591x *bq)
{
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_05, BQ2591X_WDT_RESET_MASK, val);
...
static int bq2591x_set_vbatlow_volt(struct bq2591x *bq, int volt)
{
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_06, BQ2591X_VBATLOWV_MASK, val);

	return ret;
}

static int bq2591x_enable_safety_timer(struct charger_device *chg_dev, bool enable)
{
	struct bq2591x *bq = dev_get_drvdata(&chg_dev->dev);
	u8 val;
	int ret;
	...
	ret = bq2591x_update_bits(bq, BQ2591X_REG_05, BQ2591X_EN_TIMER_MASK, val);
	return ret;
}

static int bq2591x_is_safety_timer_enable(struct charger_device *chg_dev, bool *en)
{
	struct bq2591x *bq = dev_get_drvdata(&chg_dev->dev);
	int ret;
	u8 val;
...
static ssize_t bq2591x_show_registers(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (addr = 0x0; addr <= 0x0D; addr++) {
		...
		if (ret == 0) {
			len = snprintf(tmpbuf, PAGE_SIZE - idx, "Reg[%02X] = 0x%02X\n", addr, val);
...
static ssize_t bq2591x_store_registers(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct bq2591x *bq = dev_get_drvdata(dev);
	int ret;
	unsigned int reg;
...
static DEVICE_ATTR(registers, S_IRUGO | S_IWUSR, bq2591x_show_registers, bq2591x_store_registers);
...
static int bq2591x_parse_dt(struct device *dev, struct bq2591x *bq)
{
	...
	ret = of_property_read_u32(np, "ti,bq2591x,charge-voltage", &bq->cfg.chg_mv);
	if (ret) {
		bq->cfg.chg_mv = 4400;
	...
	ret = of_property_read_u32(np, "ti,bq2591x,charge-current", &bq->cfg.chg_ma);
	if (ret) {
		bq->cfg.chg_ma = 500;
		pr_err("Failed to read node of ti,bq2591x,charge-voltage\n");
		return ret;
	}
	ret = of_property_read_u32(np, "ti,bq2591x,input-current-limit", &bq->cfg.icl_ma);
	if (ret) {
		bq->cfg.icl_ma = 500;
		pr_err("Failed to read node of ti,bq2591x,charge-voltage\n");
		return ret;
	...
	ret = of_property_read_u32(np, "ti,bq2591x,input-voltage-limit", &bq->cfg.ivl_mv);
	if (ret) {
		bq->cfg.icl_ma = 4400;
		pr_err("Failed to read node of ti,bq2591x,charge-voltage\n");
		return ret;
	...
	ret = of_property_read_u32(np, "ti,bq2591x,vbatlow-volt", &bq->cfg.batlow_mv);
	if (ret) {
	...
	ret = of_property_read_u32(np, "ti,bq2591x,term-current", &bq->cfg.iterm_ma);
	if (ret) {
		bq->cfg.iterm_ma = 250;
		pr_err("Failed to read node of ti,bq2591x,charge-voltage\n");
		return ret;
...
static void bq2591x_stat_handler(struct bq2591x *bq)
{
	...
	pr_info("%s\n", (bq->reg_stat & BQ2591X_PG_STAT_MASK) ? "Power Good" : "Power Poor");
	...
	bq->charge_state = (bq->reg_stat & BQ2591X_CHRG_STAT_MASK) >> BQ2591X_CHRG_STAT_SHIFT;
...
static void bq2591x_fault_handler(struct bq2591x *bq)
{
	...
	if (bq->reg_fault_flag & BQ2591X_VBUS_OVP_FLAG_MASK)
		pr_info("VBus OVP fault occured, current stat:%d", bq->reg_fault & BQ2591X_VBUS_OVP_STAT_MASK);

	if (bq->reg_fault_flag & BQ2591X_TSHUT_FLAG_MASK)
		pr_info("Thermal shutdown occured, current stat:%d", bq->reg_fault & BQ2591X_TSHUT_STAT_MASK);

	if (bq->reg_fault_flag & BQ2591X_BATOVP_FLAG_MASK)
		pr_info("Battery OVP fault occured, current stat:%d", bq->reg_fault & BQ2591X_BATOVP_STAT_MASK);

	if (bq->reg_fault_flag & BQ2591X_CFLY_FLAG_MASK)
		pr_info("CFLY fault occured, current stat:%d", bq->reg_fault & BQ2591X_CFLY_STAT_MASK);

	if (bq->reg_fault_flag & BQ2591X_TMR_FLAG_MASK)
		pr_info("Charge safety timer fault, current stat:%d", bq->reg_fault & BQ2591X_TMR_STAT_MASK);

	if (bq->reg_fault_flag & BQ2591X_CAP_COND_FLAG_MASK)
		pr_info("CAP conditon fault occured, current stat:%d", bq->reg_fault & BQ2591X_CAP_COND_STAT_MASK);
}
...
int oplus_bq2591x_kick_wdt(void)
{
	...
	return bq2591x_update_bits(g_bq, BQ2591X_REG_05, BQ2591X_WDT_RESET_MASK, val);
}
...
int oplus_bq2591x_set_ichg(int cur)
{
	...
	return bq2591x_update_bits(g_bq, BQ2591X_REG_01, BQ2591X_ICHG_MASK, ichg);
}
...
void oplus_bq2591x_set_mivr(int vbatt)
{
	...
	bq2591x_update_bits(g_bq, BQ2591X_REG_02, BQ2591X_VINDPM_MASK, val);
}
...
static int oplus_bq2591x_set_input_current_limit(int curr)
{
	...
	return bq2591x_update_bits(g_bq, BQ2591X_REG_03, BQ2591X_IINLIM_MASK, val);
}
...
int oplus_bq2591x_set_cv(int mv)
{
	...
	return bq2591x_update_bits(g_bq, BQ2591X_REG_00, BQ2591X_VREG_MASK, val);
}
...
static int bq2591x_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	bq->chg_dev = charger_device_register("bq25910", &client->dev, bq, &bq2591x_chg_ops, &bq->chg_props);
	...
	if (strcmp(g_bq->chg_dev_name, "secondary_chg") != 0) {
		if (client->irq) {
			ret = devm_request_threaded_irq(&client->dev, client->irq, NULL, bq2591x_charger_interrupt,
							IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "bq2591x charger irq", bq);
			if (ret < 0) {
