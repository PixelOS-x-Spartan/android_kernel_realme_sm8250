...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int mt_mastercharger_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
	default:
...
static int mastercharger_get_property(struct power_supply *psy, enum power_supply_property psp,
				      union power_supply_propval *val)
{
	int rc = 0;
...
static int masterchg_power_supply_init(struct master_chg_psy *master_chg)
{
	...
	mastercharger_psy->mastercharger_psy_desc.property_is_writeable = mt_mastercharger_prop_is_writeable;
	mastercharger_psy->mastercharger_psy_cfg.drv_data = mastercharger_psy;
	mastercharger_psy->mastercharger_psy =
		power_supply_register(mastercharger_psy->dev, &mastercharger_psy->mastercharger_psy_desc,
...
bool oplus_chg_check_qchv_condition(void)
{
	...
	if (chip->dual_charger_support && chip->soc < QC_SOC_HIGH && chip->charger_volt < QC_CHARGER_VOLTAGE_HIGH &&
	    chip->temperature <= QC_TEMP_HIGH && !chip->cool_down_force_5v)
		return true;
