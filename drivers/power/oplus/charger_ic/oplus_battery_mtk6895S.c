...
static int mtk_charger_plug_in(struct mtk_charger *info, int chr_type)
{
	...
	if (oplus_vooc_get_fastchg_started() == FALSE && oplus_vooc_get_allow_reading() == true) {
...
uint32_t pps_svooc_nodr_adapter[] = {
	0x10001, 0x40001, 0x50001, 0x10002, 0x20002, 0x30002, 0x10003, 0x20003,
};
...
static uint32_t pps_random[PPS_RANDOM_NUMBER] = {
	1111,
	2222,
	3333,
	4444,
...
int mt_power_supply_type_check(void)
{
	...
	if (!oplus_pd_without_usb()) {
		chr_type = POWER_SUPPLY_USB_TYPE_PD_SDP;
		   ...
		   ((chr_type == POWER_SUPPLY_TYPE_USB_CDP) || (chr_type == POWER_SUPPLY_TYPE_USB))) {
...
int oplus_chg_set_qc_config_forvoocphy(void)
{
	...
	chg_err(" qc9v svooc [%d %d %d %d]", chip->limits.vbatt_pdqc_to_9v_thr, chip->limits.vbatt_pdqc_to_5v_thr,
		chip->batt_volt, info->hvdcp_disable);
...
bool oplus_mt_get_vbus_status(void)
{
	...
	if (g_oplus_chip->vbatt_num == 2 && tcpm_inquire_typec_attach_state(pinfo->tcpc) == TYPEC_ATTACHED_SRC) {
		return false;
	}
...
int oplus_chg_set_pd_config_voocphy(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			...
			for (i = 0; i < cap.nr; i++) {
				printk(KERN_ERR "PD APDO cap %d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n", i,
				       cap.max_mv[i], cap.min_mv[i], cap.ma[i], cap.type[i], cap.pwr_limit[i], cap.pdp);
			}
		...
		if (chip->charger_volt > 7500 && (chip->calling_on || chip->camera_on || chip->soc >= 90 ||
