...
#include <mt-plat/mtk_rtc.h>
	...
	defined(CONFIG_OPLUS_CHARGER_MTK6833) || defined(CONFIG_OPLUS_CHARGER_MTK6781)
#include <linux/module.h>
...
int bq25601d_input_current_limit_write(int value)
{
	...
	j = 2; /* 500 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_500MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	...
	j = 3; /* 900 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_900MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	j = 4; /* 1200 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_1200MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	j = 5; /* 1500 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_1500MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	j = 6; /* 2000 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_2000MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
#if defined(CONFIG_OPLUS_CHARGER_MTK6763) || defined(CONFIG_OPLUS_CHARGER_MTK6771) ||                                  \
	defined(CONFIG_OPLUS_CHARGER_MTK6873)
	for (i = 0; i < AICL_COUNT; i++) {
		chg_vol = battery_meter_get_charger_voltage();
	...
	j = 7; /* 3000 */
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_3000MA,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
...
aicl_pre_step:
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, j,
		  bq25601d_usbin_input_current_limit[j], aicl_point_temp);
	goto aicl_rerun;
aicl_end:
...
aicl_rerun:
	...
	switch (j) {
	case 0:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_100MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 1:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_300MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 2:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_500MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 3:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_900MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 4:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_1200MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 5:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_1500MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 6:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_2000MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 7:
		bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_INPUT_CURRENT_LIMIT_3000MA,
					  REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
		break;
	default:
		break;
...
int bq25601d_input_current_limit_write(int value)
{
	...
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, (value - 100) / 100,
				       REG00_BQ25601D_INPUT_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
#endif
{
	...
	if (chg_cur > REG02_BQ25601D_MAX_FAST_CHG_CURRENT_MA || chg_cur < REG02_BQ25601D_MIN_FAST_CHG_CURRENT_MA)
	...
	rc = bq25601d_config_interface(chip, REG02_BQ25601D_ADDRESS, value, REG02_BQ25601D_FAST_CHG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
static int bq25601d_set_vindpm_vol(int vol)
{
	...
	rc = bq25601d_config_interface(chip, REG06_BQ25601D_ADDRESS, value, REG06_BQ25601D_VINDPM_MASK);
	return rc;
}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = bq25601d_config_interface(chip, REG06_BQ25601D_ADDRESS, REG06_BQ25601D_OVP_9V, REG06_BQ25601D_OVP_MASK);
	if (rc < 0) {
...
static int bq25601d_set_termchg_current(int term_curr)
{
	...
	rc = bq25601d_config_interface(chip, REG03_BQ25601D_ADDRESS, value, REG03_BQ25601D_TERM_CHG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
...
static int bq25601d_set_wdt_timer(int reg)
{
	...
	rc = bq25601d_config_interface(chip, REG05_BQ25601D_ADDRESS, reg, REG05_BQ25601D_WATCHDOG_TIMER_MASK);
...
#endif
{
	...
	rc = bq25601d_config_interface(chip, REG05_BQ25601D_ADDRESS, REG05_BQ25601D_TERMINATION_DISABLE,
				       REG05_BQ25601D_TERMINATION_MASK);
...
#endif
{
	...
	rc = bq25601d_config_interface(chip, REG01_BQ25601D_ADDRESS, REG01_BQ25601D_WDT_TIMER_RESET,
				       REG01_BQ25601D_WDT_TIMER_RESET_MASK);
	if (rc < 0) {
...
static int bq25601d_enable_charging(void)
{
	...
	rc = bq25601d_config_interface(chip, REG01_BQ25601D_ADDRESS, REG01_BQ25601D_CHARGING_ENABLE,
				       REG01_BQ25601D_CHARGING_MASK);
	if (rc < 0) {
...
int bq25601d_disable_charging(void)
{
	...
	rc = bq25601d_config_interface(chip, REG01_BQ25601D_ADDRESS, REG01_BQ25601D_CHARGING_DISABLE,
				       REG01_BQ25601D_CHARGING_MASK);
	if (rc < 0) {
...
#endif
{
	...
	rc = bq25601d_config_interface(chip, REG07_BQ25601D_ADDRESS, REG07_BQ25601D_BATFET_DIS_Q4_ON,
				       REG07_BQ25601D_BATFET_DIS_MASK);
	if (rc < 0) {
...
#if defined(CONFIG_OPLUS_CHARGER_MTK6781)
	rc = bq25601d_config_interface(chip, REG07_BQ25601D_ADDRESS, REG07_BQ25601D_BATFET_DIS_Q4_OFF,
				       REG07_BQ25601D_BATFET_DIS_MASK);
	if (rc < 0) {
		chg_err("Couldn't config batfet rc = %d\n", rc);
	}
...
static bool bq25601d_check_charger_suspend_enable(void)
{
	...
	charger_suspend_enable =
		((reg_val & REG00_BQ25601D_SUSPEND_MODE_MASK) == REG00_BQ25601D_SUSPEND_MODE_ENABLE) ? true : false;
...
int bq25601d_otg_enable(void)
{
	...
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_SUSPEND_MODE_DISABLE,
				       REG00_BQ25601D_SUSPEND_MODE_MASK);
	if (rc) {
	...
	rc = bq25601d_config_interface(chip, REG01_BQ25601D_ADDRESS, REG01_BQ25601D_OTG_ENABLE,
				       REG01_BQ25601D_OTG_MASK);
	if (rc) {
...
int bq25601d_otg_disable(void)
{
	...
	rc = bq25601d_config_interface(chip, REG01_BQ25601D_ADDRESS, REG01_BQ25601D_OTG_DISABLE,
				       REG01_BQ25601D_OTG_MASK);
	if (rc)
...
#endif
{
	...
	rc = bq25601d_config_interface(chip, REG00_BQ25601D_ADDRESS, REG00_BQ25601D_SUSPEND_MODE_ENABLE,
				       REG00_BQ25601D_SUSPEND_MODE_MASK);
	if (rc < 0) {
...
static int bq25601d_reset_charger(void)
{
	...
	rc = bq25601d_config_interface(chip, REG0B_BQ25601D_ADDRESS, REG0B_BQ25601D_REG_RST_RESET,
				       REG0B_BQ25601D_REG_RST_MASK);
	if (rc < 0) {
...
#endif
{
	...
	if (dump_count == DUMP_REG_LOG_CNT_30S) {
		dump_count = 0;
		chg_debug(
			"bq25601d_reg: [0x%02x,0x%02x,0x%02x,0x%02x], [0x%02x,0x%02x,0x%02x,0x%02x], [0x%02x,0x%02x,0x%02x,0x%02x]\n",
...
#endif
{
	...
	bq25601d_set_vindpm_vol(chip->hw_aicl_point);
#if defined(CONFIG_OPLUS_CHARGER_MTK6873) || defined(CONFIG_OPLUS_CHARGER_MTK6781)
	rc = bq25601d_config_interface(chip, REG06_BQ25601D_ADDRESS, 0x80, REG06_BQ25601D_OVP_MASK);
	if (rc < 0) {
		chg_err("Couldn't config ovp rc = %d\n", rc);
	}
#endif
...
enum charger_type mt_charger_type_detection_bq25601d(void)
{
	...
	bq25601d_config_interface(charger_ic, REG07_BQ25601D_ADDRESS, REG07_BQ25601D_IINDET_EN_FORCE_DET,
				  REG07_BQ25601D_IINDET_EN_MASE);
	...
	} else if (val_buf == REG08_BQ25601D_VBUS_STAT_DCP || val_buf == REG08_BQ25601D_VBUS_STAT_OCP ||
...
static int bq25601d_irq_registration(void)
{
	...
	ret = request_threaded_irq(gpio_to_irq(chip->irq_gpio), NULL, bq25601d_irq_handler_fn,
				   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "BQ25601D-eint", chip);
	if (ret < 0) {
