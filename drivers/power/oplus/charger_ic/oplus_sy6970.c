...
extern struct charger_consumer *charger_manager_get_by_name(struct device *dev, const char *name);
...
static const char g_sy6970_regdata_on_reset[] = { 0x48, 0x06, 0x1d, 0x1a, 0x20, 0x13, 0x5e, 0x9d, 0x03, 0x44, 0x73,
						  0x5e, 0x00, 0x12, 0x6d, 0x71, 0x47, 0x98, 0x00, 0x08, 0x0c };
...
static int sy6970_enable_otg(struct sy6970 *bq)
{
	...
	sy6970_update_bits(bq, SY6970_REG_03, SY6970_OTG_CONFIG_MASK, val);
...
static int sy6970_disable_otg(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_03, SY6970_OTG_CONFIG_MASK, val);
}
...
static int sy6970_enable_hvdcp(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_HVDCPEN_MASK, val);
	return ret;
}
...
static int sy6970_disable_maxc(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_MAXCEN_MASK, val);
	return ret;
}
...
static int sy6970_disable_batfet_rst(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_09, SY6970_BATFET_RST_EN_MASK, val);
	return ret;
}
...
static int sy6970_enable_charger(struct sy6970 *bq)
{
	...
	if (bq->is_sy6970 && bq->set_cv_value && (cv_value == bq->platform_data->fastchg_cv_for_ovp)) {
	...
	ret = sy6970_update_bits(bq, SY6970_REG_03, SY6970_CHG_CONFIG_MASK, val);

	if ((g_oplus_chip != NULL) && (g_oplus_chip->is_double_charger_support) && g_oplus_chip->slave_charger_enable) {
		if (!sy6970_is_usb(bq)) {
...
int sy6970_set_chargecurrent(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_04, SY6970_ICHG_MASK, ichg << SY6970_ICHG_SHIFT);
}
...
int sy6970_set_term_current(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_05, SY6970_ITERM_MASK, iterm << SY6970_ITERM_SHIFT);
}
...
int sy6970_set_prechg_current(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_05, SY6970_IPRECHG_MASK, iprechg << SY6970_IPRECHG_SHIFT);
}
...
int sy6970_set_chargevolt(struct sy6970 *sy, int volt)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_06, SY6970_VREG_MASK, val << SY6970_VREG_SHIFT);
}
...
int sy6970_set_input_volt_limit(struct sy6970 *sy, int volt)
{
	...
	sy6970_update_bits(sy, SY6970_REG_0D, SY6970_FORCE_VINDPM_MASK,
			   SY6970_FORCE_VINDPM_ENABLE << SY6970_FORCE_VINDPM_SHIFT);

	return sy6970_update_bits(sy, SY6970_REG_0D, SY6970_VINDPM_MASK, val << SY6970_VINDPM_SHIFT);
}
...
int sy6970_set_input_current_limit(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_00, SY6970_IINLIM_MASK, val << SY6970_IINLIM_SHIFT);
}
...
int sy6970_set_watchdog_timer(struct sy6970 *sy, u8 timeout)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_07, SY6970_WDT_MASK, val);
}
...
int sy6970_reset_watchdog_timer(struct sy6970 *sy)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_03, SY6970_WDT_RESET_MASK, val);
}
...
int sy6970_reset_chip(struct sy6970 *sy)
{
	...
	ret = sy6970_update_bits(sy, SY6970_REG_14, SY6970_RESET_MASK, val);
	return ret;
}
...
int sy6970_enter_hiz_mode(struct sy6970 *sy)
{
	...
	chg_info(" enter");
#ifdef CONFIG_OPLUS_CHARGER_MTK
	if ((boot_mode == META_BOOT) || (g_oplus_chip->mmi_chg == 0)) {
...
static int sy6970_enable_term(struct sy6970 *bq, bool enable)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_07, SY6970_EN_TERM_MASK, val);
	if ((g_oplus_chip != NULL) && (g_oplus_chip->is_double_charger_support)) {
...
int sy6970_set_boost_current(struct sy6970 *bq, int curr)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_0A, SY6970_BOOST_LIM_MASK, temp << SY6970_BOOST_LIM_SHIFT);
}
...
static int sy6970_vmin_limit(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_03, SY6970_SYS_MINV_MASK, val);
}
...
static int sy6970_enable_auto_dpdm(struct sy6970 *bq, bool enable)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_AUTO_DPDM_EN_MASK, val);
...
int sy6970_set_boost_voltage(struct sy6970 *bq, int volt)
{
	...
	if (volt > SY6970_BOOSTV_BASE + (SY6970_BOOSTV_MASK >> SY6970_BOOSTV_SHIFT) * SY6970_BOOSTV_LSB)
		volt = SY6970_BOOSTV_BASE + (SY6970_BOOSTV_MASK >> SY6970_BOOSTV_SHIFT) * SY6970_BOOSTV_LSB;

	val = ((volt - SY6970_BOOSTV_BASE) / SY6970_BOOSTV_LSB) << SY6970_BOOSTV_SHIFT;

	return sy6970_update_bits(bq, SY6970_REG_0A, SY6970_BOOSTV_MASK, val);
}
...
static int sy6970_read_idpm_limit(struct sy6970 *bq, int *icl)
{
	...
	if (ret < 0) {
		...
		*icl = SY6970_IDPM_LIM_BASE +
		       ((val & SY6970_IDPM_LIM_MASK) >> SY6970_IDPM_LIM_SHIFT) * SY6970_IDPM_LIM_LSB;
		return 0;
	}
}
...
static struct sy6970_platform_data *sy6970_parse_dt(struct device_node *np, struct sy6970 *bq)
{
	int ret = 0;
	struct sy6970_platform_data *pdata;

	pdata = devm_kzalloc(bq->dev, sizeof(struct sy6970_platform_data), GFP_KERNEL);
	if (!pdata)
		return NULL;
	...
	bq->chg_det_enable = of_property_read_bool(np, "ti,sy6970,charge-detect-enable");
	...
	ret = of_property_read_u32(np, "ti,sy6970,precharge-current", &pdata->iprechg);
	if (ret) {
		pdata->iprechg = 256;
		pr_err("Failed to read node of ti,sy6970,precharge-current\n");
	}

	ret = of_property_read_u32(np, "ti,sy6970,termination-current", &pdata->iterm);
	if (ret) {
		pdata->iterm = 250;
		pr_err("Failed to read node of ti,sy6970,termination-current\n");
	}

	ret = of_property_read_u32(np, "ti,sy6970,boost-voltage", &pdata->boostv);
	if (ret) {
		pdata->boostv = 5000;
		pr_err("Failed to read node of ti,sy6970,boost-voltage\n");
	}

	ret = of_property_read_u32(np, "ti,sy6970,boost-current", &pdata->boosti);
	if (ret) {
		pdata->boosti = 1200;
	...
	ret = of_property_read_u32(np, "ti,sy6970,fastchg-cv-for-ovp", &pdata->fastchg_cv_for_ovp);
	if (ret) {
...
static irqreturn_t sy6970_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&bq->is_suspended) == 1) {
		...
		wait_event_interruptible_timeout(bq->wait, atomic_read(&bq->is_suspended) == 0,
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB) || (cur_chg_type == POWER_SUPPLY_TYPE_USB_CDP)) {
		...
		if ((prev_chg_type == POWER_SUPPLY_TYPE_USB) || (prev_chg_type == POWER_SUPPLY_TYPE_USB_CDP))
		...
		if (!disable_QC) {
			if (!bq->hvdcp_checked && !sy6970_is_hvdcp(bq)) {
				...
				chg_err("oplus_chg_type = %d, hvdcp_checked = %d", bq->oplus_chg_type,
					bq->hvdcp_checked);
			}
		}
	} else {
...
#endif
	ret = devm_request_threaded_irq(bq->dev, bq->irq, NULL, sy6970_irq_handler, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
					bq->eint_name, bq);
	if (ret < 0) {
...
static int sy6970_detect_device(struct sy6970 *bq)
{
	...
	if (!ret) {
		...
		bq->revision = (data & SY6970_DEV_REV_MASK) >> SY6970_DEV_REV_SHIFT;
	}
...
static ssize_t sy6970_show_registers(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t sy6970_store_registers(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct sy6970 *bq = dev_get_drvdata(dev);
	int ret;
	unsigned int reg;
	...
	if (ret == 2 && reg < 0x14) {
		sy6970_write_byte(bq, (unsigned char)reg, (unsigned char)val);
	}

	return count;
}

static DEVICE_ATTR(registers, S_IRUGO | S_IWUSR, sy6970_show_registers, sy6970_store_registers);
...
static int sy6970_is_safety_timer_enabled(struct charger_device *chg_dev, bool *en)
{
	struct sy6970 *bq = dev_get_drvdata(&chg_dev->dev);
	int ret;
	u8 reg_val;
...
static int sy6970_enter_ship_mode(struct sy6970 *bq, bool en)
{
	...
	if (en) {
		...
		ret = sy6970_update_bits(bq, SY6970_REG_09, SY6970_BATFET_DLY_MASK, val);

		val = SY6970_BATFET_OFF;
		val <<= SY6970_BATFET_DIS_SHIFT;
		ret = sy6970_update_bits(bq, SY6970_REG_09, SY6970_BATFET_DIS_MASK, val);
	} else {
...
static bool oplus_usbtemp_condition(void)
{
	...
	if ((g_bq && g_bq->power_good) || (g_oplus_chip && g_oplus_chip->charger_exist)) {
		return true;
	} else {
		return false;
	}
...
aicl_pre_step:
	...
	chg_info(
		"aicl_pre_step: current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d, main %d mA, slave %d mA, slave_charger_enable:%d\n",
		chg_vol, i, usb_icl[i], aicl_point_temp, main_cur, slave_cur, chip->slave_charger_enable);
	return rc;
aicl_end:
	if ((g_oplus_chip->is_double_charger_support) && (chip->slave_charger_enable || chip->em_mode)) {
	...
	g_bq->is_force_aicl = false;
	chg_info(
		"aicl_end: current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d, main %d mA, slave %d mA, slave_charger_enable:%d\n",
		chg_vol, i, usb_icl[i], aicl_point_temp, main_cur, slave_cur, chip->slave_charger_enable);
	return rc;
}
...
int oplus_sy6970_charger_unsuspend(void)
{
	...
	if (oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY ||
		...
		if ((g_bq->before_unsuspend_icl == 0) || (g_bq->before_suspend_icl == 0) ||
		    (g_bq->before_unsuspend_icl != 100) || (g_bq->before_unsuspend_icl == g_bq->before_suspend_icl)) {
		...
		rc = sy6970_update_bits(g_bq, SY6970_REG_00, SY6970_ENHIZ_MASK,
					SY6970_HIZ_DISABLE << SY6970_ENHIZ_SHIFT);
		if (rc < 0) {
...
static int oplus_sy6970_get_charger_subtype(void)
{
	...
	if ((sy6970_is_hvdcp(g_bq) || g_bq->hvdcp_can_enabled) && (!disable_QC)) {
...
int oplus_sy6970_set_qc_config(void)
{
	...
	chg_info("temperature %d, tbatt_pdqc_to_5v_thr %d, charger_volt %d, batt_volt %d, ui_soc %d, \
		...
		cool_down_force_5v %d\n",
		 chip->temperature, chip->limits.tbatt_pdqc_to_5v_thr, chip->charger_volt, chip->batt_volt,
		 chip->ui_soc, chip->icharging, chip->limits.vbatt_pdqc_to_5v_thr, chip->cool_down_force_5v);
	if (chip->cool_down_force_5v ||
		...
		if (oplus_sy6970_get_vbus() > COOL_DOWN_5V_CHARGER_VOL) {
	...
	} else if (!chip->cool_down_force_5v && (chip->batt_volt <= chip->limits.vbatt_pdqc_to_9v_thr) &&
		   ...
		   (chip->ui_soc < COOL_DOWN_5V_UI_SOC_LEVEL) && (oplus_sy6970_get_vbus() < PULL_UP_9V_CHARGER_VOL)) {
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
	if ((boot_mode != META_BOOT) && (oplus_sy6970_get_vbus() > VBUS_VALID_MV) && !g_oplus_chip->ac_online) {
#else
...
static void sy6970_hvdcp_bc12_work_handler(struct work_struct *work)
{
	...
	if (!oplus_check_pdphy_ready() && (time < WAIT_PD_TIME_S)) {
		if (g_bq->power_good) {
			...
			oplus_set_usb_props_type(g_bq->oplus_chg_type);
			oplus_chg_wake_update_work();
			schedule_delayed_work(&g_bq->sy6970_hvdcp_bc12_work,
					      msecs_to_jiffies(SY6970_HVDCP_BC12_WORK_DELAY));
		}
		return;
	}
...
static void sy6970_monitor_cv_work_handler(struct work_struct *work)
{
	...
	if (oplus_vooc_get_fastchg_started() == false && (cv_value == g_bq->platform_data->fastchg_cv_for_ovp)) {
		cv_count++;
		if (cv_count >= CV_COUNT) {
			cv_count = 0;
			oplus_sy6970_set_cv(g_oplus_chip->limits.vfloat_sw_set);
			g_bq->set_cv_value = false;
			chg_info("cv error,pre-value %d, now %d\n", cv_value, g_oplus_chip->limits.vfloat_sw_set);
		}
	} else {
		cv_count = 0;
	}
	...
	if (cv_value != g_bq->platform_data->fastchg_cv_for_ovp) {
		...
		schedule_delayed_work(&g_bq->sy6970_monitor_cv_work, msecs_to_jiffies(MONITOR_CV_DELAY_MS));
	}
}
...
int oplus_sy6970_set_ichg(int cur)
{
	...
	chg_info(" current = %d, slave_pct=%d uA = %d, main_uA = %d, slave_uA = %d, slave_charger_enable = %d\n", cur,
		 chip->slave_pct, uA, main_uA, slave_uA, chip->slave_charger_enable);
	return 0;
}
...
static int sy6970_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct sy6970 *bq;
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
	bq->chg_dev = charger_device_register(bq->chg_dev_name, &client->dev, bq, &sy6970_chg_ops, &sy6970_chg_props);
