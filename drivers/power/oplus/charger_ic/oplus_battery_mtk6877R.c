	...
	if (g_oplus_chip->pdqc_9v_voltage_adaptive) {
		if ((g_oplus_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC ||
			...
			if (((hw_aicl_point == 4400) || (hw_aicl_point == pdqc_hw_aicl_point)) && vbatt > 4100) {
				hw_aicl_point = 4500;
			} else if (((hw_aicl_point == 4400) || (hw_aicl_point == pdqc_hw_aicl_point)) &&
				   vbatt <= 4100) {
				hw_aicl_point = 4400;
			}
		}
...
static int oplus_mt6360_input_current_limit_write(int value)
{
	...
	if (g_oplus_chip->dual_charger_support || g_oplus_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC ||
	    g_oplus_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		chg_vol = battery_meter_get_charger_voltage();
		...
		if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90 && chip->temperature <= 530 &&
			...
			if (chip->charger_volt > 7500 &&
			    (chip->calling_on || chip->soc >= 90 ||
			     chip->batt_volt >= chip->limits.vbatt_pdqc_to_5v_thr || chip->temperature > 530 ||
...
void oplus_chg_choose_gauge_curve(int index_curve)
{
	...
	printk(KERN_ERR "%s: index_curve() =%d  target_index_curve =%d last_curve_index =%d", __func__, index_curve,
	       target_index_curve, last_curve_index);
...
void charger_debug_init(void)
{
	...
	proc_create("dump_log", 0640, charger_dir, &charger_dump_log_proc_fops);
}
...
{
	chr_err("sck: en:%d pid:%d %d %d %d %d %d\n", info->sc.enable, info->sc.g_scd_pid, info->sc.pre_ibat,
		info->sc.sc_ibat, pdata->charging_current_limit, pdata->thermal_charging_current_limit,
		info->sc.solution);
