...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int oplus_step_charging_parse_dt(struct mtk_charger *info, struct device *dev)
{
	struct device_node *np = dev->of_node;
	u32 val;
...
static int oplus_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (g_oplus_chip->dual_charger_support && bq25910_is_detected() == true &&
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (pinfo->tcpc != NULL) {
		...
		i = register_tcp_dev_notifier(pinfo->tcpc, &pinfo->pd_nb, TCP_NOTIFY_TYPE_USB | TCP_NOTIFY_TYPE_MISC);
	} else {
