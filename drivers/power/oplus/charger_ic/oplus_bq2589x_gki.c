...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	if ((oplus_vooc_get_fastchg_started() == true)) {
		chg_err("fast_chg_type=%d, wait_ffc_flag=%d, prev_pg = %d, bq->power_good = %d, fastchg_started = %d\n",
			oplus_vooc_get_fast_chg_type(), chip->waiting_for_ffc, prev_pg, bq->power_good,
...
int oplus_bq2589x_set_aicr(int current_ma)
{
	...
	chg_info("usb input max current limit=%d, aicl_point_temp=%d, chip->stop_chg=%d, chip->mmi_chg=%d", current_ma,
		 aicl_point_temp, chip->stop_chg, chip->mmi_chg);
	if (chip->mmi_chg == 0) {
...
int oplus_bq2589x_get_charger_type(void)
{
	...
	if ((oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC) ||
		...
		chg_info("%s,fastchg_to_normal = %d;fast_chg_type = %d;fastchg_to_warm = %d\n", __func__,
			 oplus_vooc_get_fastchg_to_normal(), oplus_vooc_get_fast_chg_type(),
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct bq2589x *bq = (struct bq2589x *)container_of(nb, struct bq2589x, pd_nb);
...
static int bq2589x_charger_get_property(struct power_supply *psy, enum power_supply_property psp,
...
static char *bq2589x_charger_supplied_to[] = { "battery", "mtk-master-charger" };
