...
#endif
#if defined(CONFIG_MTK_PMIC_CHIP_MT6353) || defined(CONFIG_OPLUS_CHARGER_MTK6763) ||                                   \
...
*/
	...
	j = 2; /* 500 */
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_500MA,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = 2;
	...
	j = 3; /* 900 */
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_900MA,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	aicl_point_temp = charger_ic->sw_aicl_point;
#endif
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_1500MA,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	aicl_point_temp = charger_ic->sw_aicl_point;
#endif
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_2000MA,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
#if defined(CONFIG_OPLUS_CHARGER_MTK6763) || defined(CONFIG_OPLUS_CHARGER_MTK6771)
	for (i = 0; i < AICL_COUNT; i++) {
	...
	j = 7; /* 3000 */
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_3000MA,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
...
aicl_pre_step:
	...
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, j,
		  bq25890h_usbin_input_current_limit[j], aicl_point_temp);
	switch (j) {
	case 0:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_100MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 1:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_150MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 2:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_500MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 3:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_900MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 4:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_1200MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 5:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_1500MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 6:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_2000MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	case 7:
		rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_3000MA,
					       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
	default:
		break;
...
static int bq25890h_input_current_limit_write(int value)
{
	...
	for (j = 2; j <= i; j++) {
		bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
					  j << REG00_BQ25890H_INPUT_CURRENT_LIMIT_SHIFT,
					  REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
#ifdef CONFIG_OPLUS_CHARGER_MTK
		msleep(90);
		chg_vol = battery_meter_get_charger_voltage();
		...
		if (chg_vol < charger_ic->sw_aicl_point) {
			...
			chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d\n", chg_vol, j,
				  bq25890h_usbin_input_current_limit[j], charger_ic->sw_aicl_point);
			bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS,
						  j << REG00_BQ25890H_INPUT_CURRENT_LIMIT_SHIFT,
						  REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
			return 0;
		}
	...
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d\n", chg_vol, j,
		  bq25890h_usbin_input_current_limit[j]);
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
				       j << REG00_BQ25890H_INPUT_CURRENT_LIMIT_SHIFT,
				       REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	return rc;
...
static int bq25890h_set_vindpm_vol(struct chip_bq25890h *chip, int vol)
{
	...
	rc = bq25890h_config_interface(chip, REG0D_BQ25890H_ADDRESS, value, REG0D_BQ25890H_VINDPM_MASK);
	return rc;
}
...
static int bq25890h_set_complete_charge_timeout(struct chip_bq25890h *chip, int val)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, val,
				       REG07_BQ25890H_CHARGING_SAFETY_TIME_MASK |
					       REG07_BQ25890H_FAST_CHARGING_TIMEOUT_MASK);
	if (rc < 0) {
...
int bq25890h_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG06_BQ25890H_ADDRESS, value,
				       REG06_BQ25890H_CHARGING_VOL_LIMIT_MASK);
	return rc;
}
...
int bq25890h_set_prechg_current(int ipre_mA)
{
	...
	return bq25890h_config_interface(charger_ic, REG05_BQ25890H_ADDRESS, value,
					 REG05_BQ25890H_PRE_CHARGING_CURRENT_LIMIT_MASK);
}
...
static int bq25890h_set_termchg_current(int term_curr)
{
	...
	bq25890h_config_interface(charger_ic, REG05_BQ25890H_ADDRESS, value,
				  REG05_BQ25890H_TERM_CHARGING_CURRENT_LIMIT_MASK);
	return rc;
}
...
int bq25890h_set_rechg_voltage(int recharge_mv)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG06_BQ25890H_ADDRESS, reg,
				       REG06_BQ25890H_RECHARGING_THRESHOLD_VOL_MASK);
	if (rc) {
...
int bq25890h_set_wdt_timer(int reg)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, reg, REG07_BQ25890H_I2C_WATCHDOG_TIME_MASK);
...
static int bq25890h_set_chging_term_disable(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, REG07_BQ25890H_TERMINATION_DISABLE,
				       REG07_BQ25890H_TERMINATION_MASK);
...
int bq25890h_kick_wdt(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, REG03_BQ25890H_WDT_TIMER_RESET,
				       REG03_BQ25890H_WDT_TIMER_RESET_MASK);
...
int bq25890h_enable_charging(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, REG03_BQ25890H_CHARGING_ENABLE,
				       REG03_BQ25890H_CHARGING_MASK);
	if (rc < 0) {
...
int bq25890h_disable_charging(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, REG03_BQ25890H_CHARGING_DISABLE,
				       REG03_BQ25890H_CHARGING_MASK);
	if (rc < 0) {
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, REG07_BQ25890H_CHARGING_SAFETY_TIME_DISABLE,
				       REG07_BQ25890H_I2C_WATCHDOG_TIME_MASK);
	if (rc < 0) {
...
int bq25890h_disable_charging(void)
{
	...
	//}
	bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_400MA,
				  REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
...
static bool bq25890h_check_charger_suspend_enable(void)
{
	...
	charger_suspend_enable =
		((reg_val & REG00_BQ25890H_SUSPEND_MODE_MASK) == REG00_BQ25890H_SUSPEND_MODE_ENABLE) ? true : false;
...
int bq25890h_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG0B_BQ25890H_CHARGING_STATUS_CHARGING_MASK) ==
		    REG0B_BQ25890H_CHARGING_STATUS_TERM_CHARGING) ?
			   1 :
			   0;
	...
	reg_ovp =
		((reg_ovp & REG0C_BQ25890H_BATTERY_VOLATGE_MASK) == REG0C_BQ25890H_BATTERY_VOLATGE_HIGH_ERROR) ? 1 : 0;
...
#endif /* CONFIG_OPLUS_CHARGER_MTK */
	bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_SUSPEND_MODE_DISABLE,
				  REG00_BQ25890H_SUSPEND_MODE_MASK);

	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, REG03_BQ25890H_OTG_ENABLE,
				       REG03_BQ25890H_OTG_MASK);
	if (rc) {
...
#endif /* CONFIG_OPLUS_CHARGER_MTK */
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, REG03_BQ25890H_OTG_DISABLE,
				       REG03_BQ25890H_OTG_MASK);
	if (rc)
...
int bq25890h_suspend_charger(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_SUSPEND_MODE_ENABLE,
				       REG00_BQ25890H_SUSPEND_MODE_MASK);
	chg_debug(" rc = %d\n", rc);
	if (rc < 0) {
...
int bq25890h_unsuspend_charger(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_SUSPEND_MODE_DISABLE,
				       REG00_BQ25890H_SUSPEND_MODE_MASK);
	chg_debug(" rc = %d\n", rc);
	if (rc < 0) {
...
int bq25890h_reset_charger(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG01_BQ25890H_ADDRESS, REG14_BQ25890H_REGISTER_RESET,
				       REG14_BQ25890H_REGISTER_RESET_MASK);
	chg_debug(" rc = %d\n", rc);
	if (rc < 0) {
...
void bq25890h_dump_registers(void)
{
	...
	if (dump_count == DUMP_REG_LOG_CNT_30S) {
		...
		printk("bq25890h_reg[b-14]:0x%02x(b),0x%02x(c),0x%02x(d),0x%02x(e),0x%02x(f),0x%02x(10),0x%02x(11),0x%02x(12),0x%02x(13),0x%02x(14)\n",
		       val_buf[11], val_buf[12], val_buf[13], val_buf[14], val_buf[15], val_buf[16], val_buf[17],
		       val_buf[18], val_buf[19], val_buf[20]);
	}
	dump_count++;
	return;
}
...
static int bq25890h_irq_registration(void)
{
	...
	ret = request_threaded_irq(gpio_to_irq(chip->irq_gpio), NULL, bq25890h_irq_handler_fn,
				   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "BQ25890H-eint", chip);
	if (ret < 0) {
...
static int bq25890h_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct chip_bq25890h), GFP_KERNEL);
	if (!chip) {
