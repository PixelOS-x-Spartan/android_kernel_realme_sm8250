...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	chg_err("chargeric_temp:%d con_volt_19165[%d] = %d con_temp_19165[%d] = %d chargeric_temp_volt = %d step_vol = %d temp_offset = %d\n",
		pdata->chargeric_temp, i, con_volt_19165[i], i, con_temp_19165[i], pdata->chargeric_temp_volt, step_vol,
		temp_offset);
	return;
}
#endif
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if ((chip->usb_temp_r >= USB_57C && chip->usb_temp_r < USB_100C) ||
			...
			count_l = 0;
			count = 0;
		} else if (((chip->usb_temp_r - chip->temperature / 10) > 12 && chip->usb_temp_r >= USB_20C &&
			    chip->usb_temp_r < USB_100C) ||
			   ((chip->usb_temp_l - chip->temperature / 10) > 12 && chip->usb_temp_l >= USB_20C &&
		...
		check_again:
			...
			wait_event_interruptible(oplus_usbtemp_wq,
						 oplus_chg_get_vbus_status(chip) ==
							 true /*|| oplus_chg_get_otg_online() == true*/);
		}
	}
...
void oplus_chg_set_camera_on(bool val)
{
	if (!g_oplus_chip) {
		...
		if (mtkhv_flashled_pinctrl.hv_flashled_support) {
			chr_err("%s: bc1.2_done = %d camera_on %d \n", __func__, mtkhv_flashled_pinctrl.bc1_2_done,
				val);
