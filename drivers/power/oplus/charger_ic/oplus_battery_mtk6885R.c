...
static int mt_usb_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
	default:
...
static int mt_usb_set_property(struct power_supply *psy, enum power_supply_property psp,
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	if (get_project() == 0X2169E || get_project() == 0X2069F || get_project() == 21711 || get_project() == 21712 ||
	    get_project() == 0X2162D) {
...
int oplus_mt6360_pd_setup(void)
{
	...
	if (is_mtksvooc_project) {
		...
		if (get_project() == 0X2169E || get_project() == 0X2069F || get_project() == 21711 ||
		    get_project() == 21712 || get_project() == 0X2162D) {
			chip->charger_volt = chip->chg_ops->get_charger_volt();
			if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
			    chip->temperature <= 420 && chip->cool_down_force_5v == false) {
				...
				if (chip->charger_volt > 7500 &&
				    (chip->calling_on || chip->camera_on || chip->soc >= 90 ||
				     chip->batt_volt >= 4450 || chip->temperature > 420 ||
				     chip->cool_down_force_5v == true)) {
			...
			if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 &&
				...
				if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
					...
					for (i = 0; i < cap.nr; i++) {
						printk(KERN_ERR
						       "PD APDO cap %d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n",
						       i, cap.max_mv[i], cap.min_mv[i], cap.ma[i], cap.type[i],
						       cap.pwr_limit[i], cap.pdp);
					}
