...
module_param_named(pd_disabled, __pd_disabled, int, 0600);
...
static int smb5_chg_config_init(struct smb5 *chip)
{
	...
	case PMI632_SUBTYPE:
		...
		chg->wa_flags |= WEAK_ADAPTER_WA | USBIN_OV_WA | CHG_TERMINATION_WA;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, arb_monitor_work.work);
	int rc = 0;
...
irqreturn_t usbid_change_handler(int irq, void *data)
{
	...
	if (oplus_usbid_check_is_gpio(chip) == true) {
		if (g_smb_chip->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
			...
			schedule_delayed_work(&g_smb_chip->uusb_otg_work, msecs_to_jiffies(g_smb_chip->otg_delay_ms));
			return IRQ_HANDLED;
		}
	} else {
...
int smblib_set_prop_pr_swap_in_progress(struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	if (chg->pr_swap_in_progress) {
		...
		/* enable DRP */
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK, 0);
		if (rc < 0)
...
static void smblib_moisture_protection_work(struct work_struct *work)
{
	...
	if (stat == U_USB_STATUS_WATER_PRESENT || stat == U_USB_FMB1_BIT || stat == U_USB_FMB2_BIT ||
		...
		alarm_start_relative(&chg->moisture_protection_alarm, ms_to_ktime(MOISTURE_PROTECTION_CHECK_DELAY_MS));
	} else {
...
static void jeita_update_work(struct work_struct *work)
{
	...
	if (IS_ERR(pnode)) {
		...
		pnode = of_batterydata_get_best_profile(batt_node, NONSTD_BAT_ID_OHM / 1000, NULL);
		if (IS_ERR(pnode)) {
			rc = PTR_ERR(pnode);
	...
	if (bat_thermal_1k == true) {
		rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds_oplus", jeita_hard_thresholds, 2);
	} else {
		rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds_oplus", jeita_hard_thresholds, 2);
	}
#endif
...
#else
	if (bat_thermal_1k == true) {
		rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-thresholds_oplus", chg->jeita_soft_thlds, 2);
	} else {
		rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-thresholds_oplus", chg->jeita_soft_thlds, 2);
	}
#endif
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, check_camera_and_hotspot_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (vbus_rising || chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT ||
	    chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_MEDIUM || chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_HIGH) {
		schedule_delayed_work(&chg->check_camera_and_hotspot_work, msecs_to_jiffies(2000));
	}
}
#endif
...
static char *dr_mode_text[] = { "ufp", "dfp", "none" };
...
int smblib_force_dr_mode(struct smb_charger *chg, int mode)
{
	...
	case DUAL_ROLE_PROP_MODE_UFP:
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK | EN_TRY_SNK_BIT,
					 EN_SNK_ONLY_BIT);
		if (rc < 0) {
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, role_reversal_check.work);
	int rc = 0;
	...
	case DUAL_ROLE_PROP_MODE_UFP:
		if (chg->typec_mode < POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) {
			...
			rc = smblib_force_dr_mode(chg, DUAL_ROLE_PROP_MODE_NONE);
			if (rc < 0)
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->role_reversal_check, smblib_dual_role_check_work);
#ifdef OPLUS_FEATURE_CHG_BASIC
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (rc < 0)
		chg->otg_cl_ua = (chip->chg.smb_version == PMI632_SUBTYPE) ? MICRO_1PA : MICRO_3PA;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (g_oplus_chip) {
		g_oplus_chip->normalchg_gpio.usbid_gpio = of_get_named_gpio(node, "qcom,usbid-gpio", 0);
	...
	chg->uusb_moisture_protection_enabled = chg->uusb_moisture_protection_enabled &&
			     ...
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_THERM_ICL_LIMIT:
		...
		if ((icl + val->intval) > 0)
			rc = vote(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER, true, icl + val->intval);
		else
			rc = -EINVAL;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_LIMIT:
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
		if ((chg->smb_version == PMI632_SUBTYPE) && (chg->flash_active != val->intval)) {
			chg->flash_active = val->intval;
#ifdef OPLUS_FEATURE_CHG_BASIC
			/* Yichun.Chen  PSW.BSP.CHG  2018-06-28  avoid flash current ripple when flash work */
			smblib_set_opt_switcher_freq(chg, chg->flash_active ? chg->chg_freq.freq_removal :
...
static int smb5_dr_get_property(struct dual_role_phy_instance *dual_role, enum dual_role_property prop,
				unsigned int *val)
{
...
static int smb5_dr_set_property(struct dual_role_phy_instance *dual_role, enum dual_role_property prop,
	...
	case DUAL_ROLE_PROP_MODE:
		...
		case DUAL_ROLE_PROP_MODE_UFP:
			if (chg->typec_mode >= POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) {
				...
				rc = smblib_force_dr_mode(chg, DUAL_ROLE_PROP_MODE_UFP);
				if (rc < 0) {
					chg->pr_swap_in_progress = false;
		...
		case DUAL_ROLE_PROP_MODE_DFP:
			if (chg->typec_mode < POWER_SUPPLY_TYPEC_SOURCE_DEFAULT &&
				...
				chg->pr_swap_in_progress = true;
				rc = smblib_force_dr_mode(chg, DUAL_ROLE_PROP_MODE_DFP);
				if (rc < 0) {
					chg->pr_swap_in_progress = false;
...
static int smb5_dr_prop_writeable(struct dual_role_phy_instance *dual_role, enum dual_role_property prop)
{
	struct smb_charger *chg = dual_role_get_drvdata(dual_role);
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	if (rc < 0)
		dev_err(chg->dev, "Couldn't USBIN_OPTIONS_2_CFG_REG rc=%d\n", rc);
...
static void otg_enable_pmic_id_value(void)
{
	...
	/* set DRP mode */
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK, 0x0); //bit[2:0]=0
	if (rc < 0) {
...
static void otg_disable_pmic_id_value(void)
{
	...
	/* set sink mode only */
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK, EN_SNK_ONLY_BIT); //bit[2:0]=0x2
	if (rc < 0) {
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		if (oplus_usbid_check_is_gpio(g_oplus_chip) == true) {
			...
			printk(KERN_ERR "[OPLUS_CHG][%s]: usbid_gpio=%d\n", __func__,
			       gpio_get_value(g_oplus_chip->normalchg_gpio.usbid_gpio));
		}
	} else {
		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC)
...
static int smb5_init_hw(struct smb5 *chip)
{
	 ...
	 */
	val = (chg->hvdcp_disable) ? 0 : (HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT);
	if (chip->dt.hvdcp_autonomous)
		val |= HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT;

	rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
				 (HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT | HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT), val);
	if (rc < 0) {
...
static int smb5_request_interrupts(struct smb5 *chip)
{
	...
	if (chg->irq_info[WDOG_SNARL_IRQ].irq &&
	    (!(chg->wa_flags & SW_THERM_REGULATION_WA) && chip->dt.wd_snarl_time_cfg == -EINVAL)) {
...
static int smbchg_set_fastchg_current_raw(int current_ma)
{
	...
	rc = vote(g_smb_chip->fcc_votable, DEFAULT_VOTER, true, current_ma * 1000);
	if (rc < 0) {
...
static void smbchg_aicl_enable(bool enable)
{
	...
	rc = smblib_masked_write(g_smb_chip, USBIN_AICL_OPTIONS_CFG_REG, USBIN_AICL_EN_BIT,
...
static int smbchg_charging_disble(void)
{
	...
	rc = vote(g_smb_chip->chg_disable_votable, DEFAULT_VOTER, true, 0);
	if (rc < 0) {
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (!chip->pmic_spmi.usb_temp_v_l_chan || !chip->pmic_spmi.usb_temp_v_r_chan) {
		...
		if ((USB_57C <= chip->usb_temp_r && chip->usb_temp_r < USB_100C) ||
			...
			if (dischg_flag == false) {
				if (count <= total_count) {
					...
					if ((current_temp_r - last_usb_temp_r) >= 3 ||
					    (current_temp_l - last_usb_temp_l) >= 3) {
						for (i = 1; i < retry_cnt; i++) {
							mdelay(RETRY_CNT_DELAY);
							oplus_get_usbtemp_volt(chip);
							get_usb_temp(chip);
						...
						if (count_r >= retry_cnt || count_l >= retry_cnt) {
							if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
								...
								chg_err("dischg enable3...,current_temp=[%d,%d],last_usb_temp=[%d,%d],last_usb_tempcount =%d\n",
									current_temp_r, current_temp_l, last_usb_temp_r,
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(oplus_chip->dev->of_node, "qcom,pmi632chg-vadc", NULL)) {
		rc = smblib_get_iio_channel(chg, "usb_in_voltage", &chg->iio.chgid_v_chan);
		if (rc < 0) {
