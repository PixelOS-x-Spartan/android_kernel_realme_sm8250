...
static struct bq2589x_pep20_efficiency_table bq2589x_pe2_table[BQ2589X_PE2_TABLE_COUNT] = {
	{ 3400000, 8000000 }, { 3500000, 8000000 }, { 3600000, 8000000 }, { 3700000, 8500000 }, { 3800000, 8500000 },
	{ 3900000, 8500000 }, { 4000000, 9000000 }, { 4100000, 9000000 }, { 4200000, 9000000 }, { 4400000, 9000000 },
};
...
static int bq2589x_disable_batfet_rst(struct bq2589x *bq)
{
	...
	ret = bq2589x_update_bits(bq, BQ2589X_REG_09, BQ2589X_BATFET_RST_EN_MASK, val);
	return ret;
}
...
int bq2589x_set_input_volt_limit(struct bq2589x *bq, int volt)
{
	...
	bq2589x_update_bits(bq, BQ2589X_REG_0D, BQ2589X_FORCE_VINDPM_MASK,
			    BQ2589X_FORCE_VINDPM_ENABLE << BQ2589X_FORCE_VINDPM_SHIFT);
...
int bq2589x_enter_hiz_mode(struct bq2589x *bq)
{
	...
	chg_info(" enter");
	if ((boot_mode == META_BOOT) || (oplus_usb_temp_is_high(g_oplus_chip)) || (g_oplus_chip->mmi_chg == 0)) {
...
static int bq2589x_vmin_limit(struct bq2589x *bq)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_03, BQ2589X_SYS_MINV_MASK, val);
}
...
static int bq2589x_read_idpm_limit(struct bq2589x *bq, int *icl)
{
	...
	if (ret < 0) {
		...
		*icl = BQ2589X_IDPM_LIM_BASE +
		       ((val & BQ2589X_IDPM_LIM_MASK) >> BQ2589X_IDPM_LIM_SHIFT) * BQ2589X_IDPM_LIM_LSB;
		return 0;
	}
}
...
static int bq2589x_enable_safety_timer(struct bq2589x *bq)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_07, BQ2589X_EN_TIMER_MASK, val);
}
...
static struct bq2589x_platform_data *bq2589x_parse_dt(struct device_node *np, struct bq2589x *bq)
{
	int ret;
	...
	bq->chg_det_enable = of_property_read_bool(np, "ti,bq2589x,charge-detect-enable");
	...
	ret = of_property_read_u32(np, "ti,bq2589x,precharge-current", &pdata->iprechg);
	if (ret) {
		pdata->iprechg = BQ2598X_IPRECHG;
		chg_err("Failed to read node of ti,bq2589x,precharge-current\n");
	}

	ret = of_property_read_u32(np, "ti,bq2589x,termination-current", &pdata->iterm);
	if (ret) {
		pdata->iterm = BQ2598X_ITERM;
		chg_err("Failed to read node of ti,bq2589x,termination-current\n");
	}

	ret = of_property_read_u32(np, "ti,bq2589x,boost-voltage", &pdata->boostv);
	if (ret) {
		pdata->boostv = BQ2598X_BOOSTV;
		chg_err("Failed to read node of ti,bq2589x,boost-voltage\n");
	}

	ret = of_property_read_u32(np, "ti,bq2589x,boost-current", &pdata->boosti);
	if (ret) {
...
static int bq2589x_register_interrupt(struct device_node *np, struct bq2589x *bq)
{
	...
	ret = devm_request_threaded_irq(bq->dev, bq->irq, NULL, bq2589x_irq_handler,
...
static ssize_t bq2589x_store_registers(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct bq2589x *bq = dev_get_drvdata(dev);
	int ret;
	unsigned int reg;
...
static int bq2589x_set_pep20_current_pattern(struct charger_device *chg_dev, u32 chr_vol)
{
	struct bq2589x *bq = dev_get_drvdata(&chg_dev->dev);
	int ret;
	int value;
	...
	for (i = 4; i >= 0; i--) {
		...
		if (flag == 0) {
			...
			if (cptime[j][1] < 30 || cptime[j][1] > 65) {
				chg_info("charging_set_ta20_current_pattern fail1: idx:%d target:%d actual:%d\n", i,
			...
			if (cptime[j][1] < 90 || cptime[j][1] > 115) {
				chg_info("charging_set_ta20_current_pattern fail2: idx:%d target:%d actual:%d\n", i,
					 PEOFFTIME, cptime[j][1]);
				return -EIO;
			}
			j++;
			...
			cptime[j][1] = dtime(j);
			if (cptime[j][1] < 90 || cptime[j][1] > 115) {
				chg_info("charging_set_ta20_current_pattern fail3: idx:%d target:%d actual:%d\n", i,
					 PEOFFTIME, cptime[j][1]);
				return -EIO;
			}
			j++;
			...
			cptime[j][1] = dtime(j);
			if (cptime[j][1] < 30 || cptime[j][1] > 65) {
				chg_info("charging_set_ta20_current_pattern fail4: idx:%d target:%d actual:%d\n", i,
					 PEOFFTIME, cptime[j][1]);
				return -EIO;
			}
			j++;
	...
	if (cptime[j][1] < 150 || cptime[j][1] > 240) {
		chg_info("charging_set_ta20_current_pattern fail5: idx:%d target:%d actual:%d\n", i, PEOFFTIME,
			 cptime[j][1]);
		return -EIO;
	}
	j++;
...
{
	if ((bq2589x_is_hvdcp(g_bq) || g_bq->hvdcp_can_enabled) && (!disable_qc)) {
...
int oplus_bq2589x_pd_setup(void)
{
	...
	if ((chip->charger_volt > CHG_VOL_7V5) && ((chip->temperature >= 450) || (chip->temperature < 0))) {
		vbus = VBUS_VOL_5V;
		ibus = IBUS_VOL_3A;
...
static int bq2589x_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	bq->chg_dev = charger_device_register(bq->chg_dev_name, &client->dev, bq, &bq2589x_chg_ops, &bq2589x_chg_props);
