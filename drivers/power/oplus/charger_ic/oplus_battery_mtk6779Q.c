...
int charger_manager_enable_kpoc_shutdown(struct charger_consumer *consumer, bool en)
{
...
{
	if (info->pd_type == PD_CONNECT_TYPEC_ONLY_SNK && tcpm_inquire_typec_remote_rp_curr(info->tcpc) != 500 &&
	    info->chr_type != STANDARD_HOST && info->chr_type != CHARGING_HOST &&
	    mtk_pe20_get_is_connect(info) == false && mtk_pe_get_is_connect(info) == false &&
...
static int oplus_mt6360_input_current_limit_write(int value)
{
	...
	if (g_oplus_chip->chg_ops->oplus_chg_get_pd_type) {
		if (g_oplus_chip->chg_ops->oplus_chg_get_pd_type() == true) {
			...
			if (rc >= 0 && ibus_ma >= 500 && ibus_ma < g_oplus_chip->limits.pd_input_current_charger_ma &&
			    value > ibus_ma) {
				value = ibus_ma;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (pinfo->tcpc != NULL) {
		...
		ret = register_tcp_dev_notifier(pinfo->tcpc, &pinfo->pd_nb, TCP_NOTIFY_TYPE_USB | TCP_NOTIFY_TYPE_MISC);
	} else {
