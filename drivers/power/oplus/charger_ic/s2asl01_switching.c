...
static enum power_supply_property s2asl01_main_props[] = { POWER_SUPPLY_PROP_VOLTAGE_NOW,
							   POWER_SUPPLY_PROP_CURRENT_NOW };

static enum power_supply_property s2asl01_sub_props[] = { POWER_SUPPLY_PROP_VOLTAGE_NOW,
							  POWER_SUPPLY_PROP_CURRENT_NOW };
...
static int s2asl01_write_reg(struct i2c_client *client, int reg, u8 data)
{
	...
	while (ret < 0) {
		pr_info("%s [%s]: reg(0x%x), ret(%d), retry:%d\n", __func__,
			current_limiter_type_str[switching->pdata->bat_type], reg, ret, retry);
		msleep(50);
		mutex_lock(&switching->i2c_lock);
		ret = i2c_smbus_write_byte_data(client, reg, data);
		mutex_unlock(&switching->i2c_lock);
...
static void s2asl01_set_in_ok(struct s2asl01_switching_data *switching, bool onoff)
{
	...
	if (onoff)
		s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3, 1 << S2ASL01_CTRL3_INOK_SHIFT,
				   S2ASL01_CTRL3_INOK_MASK);
	else
		s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3, 0 << S2ASL01_CTRL3_INOK_SHIFT,
				   S2ASL01_CTRL3_INOK_MASK);
}
...
static void s2asl01_set_recharging_start(struct s2asl01_switching_data *switching)
{
	...
	s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL1, 1 << S2ASL01_CTRL1_RESTART_SHIFT,
			   S2ASL01_CTRL1_RESTART_MASK);
}
...
static void s2asl01_set_eoc_on(struct s2asl01_switching_data *switching)
{
	...
	s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL1, 1 << S2ASL01_CTRL1_EOC_SHIFT,
			   S2ASL01_CTRL1_EOC_MASK);
}
...
static void s2asl01_set_dischg_mode(struct s2asl01_switching_data *switching, int chg_mode)
{
	...
	ret = s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3, val << S2ASL01_CTRL3_DIS_MODE_SHIFT,
				 S2ASL01_CTRL3_DIS_MODE_MASK);
	if (ret < 0)
...
static void s2asl01_set_chg_mode(struct s2asl01_switching_data *switching, int chg_mode)
{
	...
	ret = s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3, val << S2ASL01_CTRL3_CHG_MODE_SHIFT,
				 S2ASL01_CTRL3_CHG_MODE_MASK);
	if (ret < 0)
		pr_err("%s, i2c read fail\n", __func__);
...
static void s2asl01_tsd_onoff(struct s2asl01_switching_data *switching, bool onoff)
{
	pr_info("%s(%d)\n", __func__, onoff);
	if (onoff) {
		s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_COMMON1, S2ASL01_COMMON1_CM_TSD_EN,
				   S2ASL01_COMMON1_CM_TSD_EN);
		switching->tsd = true;
	} else {
		s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_COMMON1, 0, S2ASL01_COMMON1_CM_TSD_EN);
		switching->tsd = false;
	}
}

static int s2asl01_get_property(struct power_supply *psy, enum power_supply_property psp,
...
static int s2asl01_set_property(struct power_supply *psy, enum power_supply_property psp,
...
static int s2asl01_switching_parse_dt(struct device *dev, struct s2asl01_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->pinctrl)) {
			...
			pdata->switch_active = pinctrl_lookup_state(pdata->pinctrl, "switch_active");
			if (IS_ERR_OR_NULL(pdata->switch_active)) {
				chg_err(": %d Failed to get the switch_active, pinctrl handle\n", __LINE__);
			} else {
...
static ssize_t powermeter_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp;
...
static ssize_t v_referesh_time_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t v_avg1_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t v_avg2_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t i_referesh_time_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t i_avg1_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t i_avg2_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	int ret;
	u8 temp = 0, val = 0;
...
static ssize_t fastchg_curr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int curr_ma;
...
static ssize_t dischg_curr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int curr_ma;
...
int switching_hw_enable(int enable)
{
	...
	if (s2asl01_boot_mode == ATE_FACTORY_BOOT || s2asl01_boot_mode == FACTORY_BOOT ||
	    g_switching->switching_chip->error_status != 0) {
#else
...
static int s2asl01_switching_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_switching->switching_chip = devm_kzalloc(&client->dev, sizeof(struct oplus_switch_chip), GFP_KERNEL);
...
static const struct i2c_device_id s2asl01_switching_id[] = { { "s2asl01-switching", 0 }, {} };
