...
void oplus_set_opluschg_pd_sdp(bool value)
{
	...
	if (value == true && g_oplus_chip && g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP &&
...
int smblib_get_prop_smb_health(struct smb_charger *chg)
{
	 ...
	 */
	if (is_cp_available(chg)) {
		rc = power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_SWITCHER_EN, &prop);
		if (!rc && !prop.intval)
			return POWER_SUPPLY_HEALTH_UNKNOWN;
	}
...
int smblib_set_prop_thermal_overheat(struct smb_charger *chg, int therm_overheat)
{
	...
	if (chg->cp_disable_votable) {
		vote(chg->cp_disable_votable, OVERHEAT_LIMIT_VOTER, therm_overheat, 0);
		vote(chg->usb_icl_votable, OVERHEAT_LIMIT_VOTER, therm_overheat, icl_ua);
	}
...
static void smblib_handle_hvdcp_3p0_auth_done(struct smb_charger *chg, bool rising)
{
	...
	if (apsd_result->bit & QC_3P0_BIT) {
		...
		if (!chg->apsd_ext_timeout && !timer_pending(&chg->apsd_timer)) {
			smblib_dbg(chg, PR_MISC, "APSD Extented timer started at %lld\n", jiffies_to_msecs(jiffies));

			mod_timer(&chg->apsd_timer, msecs_to_jiffies(APSD_EXTENDED_TIMEOUT_MS) + jiffies);
		}
	}
...
static void smblib_dual_role_check_work(struct work_struct *work)
{
	...
	case DUAL_ROLE_PROP_MODE_DFP:
		if (chg->typec_mode >= POWER_SUPPLY_TYPEC_SOURCE_DEFAULT ||
			...
			rc = smblib_role_switch_failure(chg, DUAL_ROLE_PROP_MODE_DFP);
			if (rc < 0)
...
void smbchg_set_chargerid_switch_val(int value)
{
	...
	if (value) {
		...
		pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.chargerid_switch_active);
	} else {
		//gpio_direction_output(chip->normalchg_gpio.chargerid_switch_gpio, 0);
...
static const struct vadc_map_temp origin_table[] = {
	{ 1831000, -40000 }, { 1814000, -35000 }, { 1791000, -30000 }, { 1761000, -25000 }, { 1723000, -20000 },
	{ 1675000, -15000 }, { 1616000, -10000 }, { 1545000, -5000 },  { 1463000, 0 },	    { 1370000, 5000 },
	{ 1268000, 10000 },  { 1160000, 15000 },  { 1049000, 20000 },  { 937000, 25000 },   { 828000, 30000 },
	{ 726000, 35000 },   { 630000, 40000 },	  { 544000, 45000 },   { 467000, 50000 },   { 399000, 55000 },
	{ 340000, 60000 },   { 290000, 65000 },	  { 247000, 70000 },   { 209000, 75000 },   { 179000, 80000 },
	{ 153000, 85000 },   { 130000, 90000 },	  { 112000, 95000 },   { 96000, 100000 },   { 82000, 105000 },
	{ 71000, 110000 },   { 62000, 115000 },	  { 53000, 120000 },   { 46000, 125000 },
};
int oplus_turn_usbtemp_to_volt(int temp, const struct vadc_map_temp *table, int length)
{
	...
	volt = (temp - table[i - 1].temp) * (table[i].volt - table[i - 1].volt) / (table[i].temp - table[i - 1].temp) +
	       table[i - 1].volt;
	return volt;
}
...
int oplus_chg_get_shutdown_soc(void)
{
	...
	if (g_smb_chip->batt_psy == NULL)
		chg_err("tongfeng test  aaaaaaa %s, g_smb_chip->batt_info[BATT_INFO_SOC][%d]\n", __func__,
			g_smb_chip->batt_info[BATT_INFO_SOC]);
