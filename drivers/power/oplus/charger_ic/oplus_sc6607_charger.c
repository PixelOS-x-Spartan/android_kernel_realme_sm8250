...
static struct sc6607_temp_param pst_temp_table[] = {
	{ 34, 4966 }, { 40, 4043 }, { 45, 3396 }, { 50, 2847 }, { 55, 2386 }, { 60, 2000 }, { 65, 1678 },
	{ 70, 1411 }, { 75, 1189 }, { 76, 1149 }, { 77, 1111 }, { 78, 1074 }, { 79, 1039 }, { 80, 1005 },
	{ 81, 971 },  { 82, 940 },  { 83, 909 },  { 84, 879 },	{ 85, 851 },  { 90, 723 },  { 95, 616 },
	{ 100, 527 }, { 110, 389 }, { 120, 291 }, { 125, 253 },
};
...
static const char *const state_str[] = {
	"bc1.2 detect Init", "non-standard adapter detection", "floating Detection", "bc1.2 Primary Detection",
	"hiz set",	     "bc1.2 Secondary Detection",      "hvdcp hanke",
};

static const char *const dpdm_str[] = {
	"0v to 0.325v", "0.325v to 1v", "1v to 1.35v", "1.35v to 2.2v", "2.2v to 3v", "higher than 3v",
};
...
static int sc6607_get_charger_type(struct sc6607 *chip, enum power_supply_type *type)
{
	int ret;
	u8 vbus_stat;
	u8 hw_bc12_done = 0;
...
static int oplus_sc6607_charger_suspend(void)
{
	...
	chg_err(" [%d, %d]\n", atomic_read(&g_chip->driver_suspended), atomic_read(&g_chip->charger_suspended));
...
static bool oplus_sc6607_check_chrdet_status(void)
{
	...
	if (oplus_vooc_get_fastchg_started() || oplus_vooc_get_fastchg_dummy_started() ||
	    oplus_vooc_get_fastchg_to_warm() || oplus_vooc_get_fastchg_to_normal() || chg_chip->camera_on)
		return true;
...
static int oplus_mtk_get_pd_type(void)
{
	if (g_chip) {
		pr_info("pd_type=%d\n", g_chip->pd_type);
		if (g_chip->pd_type == PD_CONNECT_PE_READY_SNK || g_chip->pd_type == PD_CONNECT_PE_READY_SNK_PD30)
			return PD_ACTIVE;
...
static int sc6607_dump_reg_info(char *dump_info, int len, struct sc6607_track_check_reg *p_check_reg, int check_reg_len)
{
	...
	struct sc6607 *chip = g_chip;
	u8 track_dump_reg[] = {
		SC6607_REG_HK_GEN_FLG,	SC6607_REG_VAC_VBUS_OVP,    SC6607_REG_HK_INT_STAT,
		SC6607_REG_HK_INT_FLG,	SC6607_REG_PROTECTION_DIS,  SC6607_REG_CHG_CTRL1,
		SC6607_REG_CHG_INT_FLG, SC6607_REG_CHG_INT_FLG + 1, SC6607_REG_CHG_INT_FLG3,
		SC6607_REG_CHG_FLT_FLG, SC6607_REG_CHG_FLT_FLG2,    SC6607_REG_CP_CTRL,
		SC6607_REG_CP_INT_STAT, SC6607_REG_DPDM_INT_FLAG,   SC6607_REG_VOOCPHY_FLAG,
	};
...
void oplus_chg_set_camera_on(bool val)
{
	...
	if (val) {
		...
		if (chg_chip->charger_exist && !chg_chip->pd_svooc &&
		    chg_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD)
...
static void sc6607_voocphy_update_data(struct oplus_voocphy_manager *chip)
{
	...
	pr_info(" [%d, %d, %d, %d, %d, %d]", chip->cp_ichg, chip->cp_vbus, chip->cp_vac, chip->cp_vbat, chip->cp_vsys,
		chip->int_flag);
}
...
static int sc6607_track_match_hk_err(struct sc6607 *chip, u8 data)
{
	...
	struct sc6607_track_check_reg track_check_reg[] = {
		{ SC6607_REG_HK_FLT_FLG, data },
		{ SC6607_REG_CP_FLT_FLG, 0 },
		{ SC6607_REG_CP_PMID2OUT_FLG, 0 },
	};
...
static int sc6607_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
	g_chip->chg_dev =
		charger_device_register(g_chip->chg_dev_name, &client->dev, g_chip, &sc6607_chg_ops, &sc6607_chg_props);
	...
	if (chip->tcpc != NULL) {
		...
		ret = register_tcp_dev_notifier(chip->tcpc, &chip->pd_nb, TCP_NOTIFY_TYPE_USB | TCP_NOTIFY_TYPE_MISC);
	} else {
