...
static const struct irq_mapping_tbl rt9467_irq_mapping_tbl[] = {
	RT9467_IRQ_MAPPING(chg_treg, 4),       RT9467_IRQ_MAPPING(chg_aicr, 5),
	RT9467_IRQ_MAPPING(chg_mivr, 6),       RT9467_IRQ_MAPPING(pwr_rdy, 7),
	RT9467_IRQ_MAPPING(chg_vsysuv, 12),    RT9467_IRQ_MAPPING(chg_vsysov, 13),
	RT9467_IRQ_MAPPING(chg_vbatov, 14),    RT9467_IRQ_MAPPING(chg_vbusov, 15),
	RT9467_IRQ_MAPPING(ts_batcold, 20),    RT9467_IRQ_MAPPING(ts_batcool, 21),
	RT9467_IRQ_MAPPING(ts_batwarm, 22),    RT9467_IRQ_MAPPING(ts_bathot, 23),
	RT9467_IRQ_MAPPING(ts_statci, 24),     RT9467_IRQ_MAPPING(chg_faulti, 25),
	RT9467_IRQ_MAPPING(chg_statci, 26),    RT9467_IRQ_MAPPING(chg_tmri, 27),
	RT9467_IRQ_MAPPING(chg_batabsi, 28),   RT9467_IRQ_MAPPING(chg_adpbadi, 29),
	RT9467_IRQ_MAPPING(chg_rvpi, 30),      RT9467_IRQ_MAPPING(otpi, 31),
	RT9467_IRQ_MAPPING(chg_aiclmeasi, 32), RT9467_IRQ_MAPPING(chg_ichgmeasi, 33),
	RT9467_IRQ_MAPPING(chgdet_donei, 34),  RT9467_IRQ_MAPPING(wdtmri, 35),
	RT9467_IRQ_MAPPING(ssfinishi, 36),     RT9467_IRQ_MAPPING(chg_rechgi, 37),
	RT9467_IRQ_MAPPING(chg_termi, 38),     RT9467_IRQ_MAPPING(chg_ieoci, 39),
	RT9467_IRQ_MAPPING(adc_donei, 40),     RT9467_IRQ_MAPPING(pumpx_donei, 41),
	RT9467_IRQ_MAPPING(bst_batuvi, 45),    RT9467_IRQ_MAPPING(bst_midovi, 46),
	RT9467_IRQ_MAPPING(bst_olpi, 47),      RT9467_IRQ_MAPPING(attachi, 48),
	RT9467_IRQ_MAPPING(detachi, 49),       RT9467_IRQ_MAPPING(hvdcpi, 52),
	RT9467_IRQ_MAPPING(chgdeti, 54),       RT9467_IRQ_MAPPING(dcdti, 55),
};
...
static int rt9467_device_write(void *client, u32 addr, int leng, const void *src)
{
	struct i2c_client *i2c = (struct i2c_client *)client;
	int rc = 0;
	int retry = 3;
...
static inline int __rt9467_i2c_write_byte(struct rt9467_info *info, u8 cmd, u8 data)
{
	int ret = 0, retry = 0;
...
static inline int __rt9467_i2c_block_write(struct rt9467_info *info, u8 cmd, u32 leng, const u8 *data)
{
	int ret = 0;
...
static int rt9467_i2c_block_write(struct rt9467_info *info, u8 cmd, u32 leng, const u8 *data)
{
	int ret = 0;
...
static inline int __rt9467_i2c_block_read(struct rt9467_info *info, u8 cmd, u32 leng, u8 *data)
{
	int ret = 0;
...
static int rt9467_i2c_block_read(struct rt9467_info *info, u8 cmd, u32 leng, u8 *data)
{
	int ret = 0;
...
static int rt9467_i2c_test_bit(struct rt9467_info *info, u8 cmd, u8 shift, bool *is_one)
{
	int ret = 0;
	u8 data = 0;
...
static int rt9467_i2c_update_bits(struct rt9467_info *info, u8 cmd, u8 data, u8 mask)
{
	int ret = 0;
	u8 reg_data = 0;
...
static inline void rt9467_irq_set_flag(struct rt9467_info *info, u8 *irq, u8 mask)
{
...
static inline void rt9467_irq_clr_flag(struct rt9467_info *info, u8 *irq, u8 mask)
{
	mutex_lock(&info->irq_access_lock);
	*irq &= ~mask;
	mutex_unlock(&info->irq_access_lock);
...
static inline const char *rt9467_get_irq_name(struct rt9467_info *info, int irqnum)
{
	int i = 0;
...
{
	dev_dbg(info->dev, "%s: irq = %d, %s\n", __func__, irqnum, rt9467_get_irq_name(info, irqnum));
...
static inline u8 rt9467_closest_reg_via_tbl(const u32 *tbl, u32 tbl_size, u32 target)
{
	u32 i = 0;
...
static int rt9467_get_adc(struct rt9467_info *info, enum rt9467_adc_sel adc_sel, int *adc_val)
{
	int ret = 0, i = 0;
	...
	ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_ADC, adc_sel << RT9467_SHIFT_ADC_IN_SEL,
	...
	for (i = 0; i < max_wait_times; i++) {
		msleep(35);
		ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_ADC, RT9467_SHIFT_ADC_START, &adc_start);
		if (ret >= 0 && !adc_start)
			break;
	}
	if (i == max_wait_times) {
	...
	dev_dbg(info->dev, "%s: adc_tune = (0x%02X, 0x%02X), adc_org = (0x%02X, 0x%02X)\n", __func__, adc_data[2],
		adc_data[3], adc_data[4], adc_data[5]);

	/* Calculate ADC value */
	*adc_val = ((adc_data[0] << 8) + adc_data[1]) * rt9467_adc_unit[adc_sel] + rt9467_adc_offset[adc_sel];

	dev_dbg(info->dev, "%s: adc_sel = %d, adc_h = 0x%02X, adc_l = 0x%02X, val = %d\n", __func__, adc_sel,
		adc_data[0], adc_data[1], *adc_val);
...
{
	int ret = 0;
	enum rt9467_usbsw_state usbsw = en ? RT9467_USBSW_CHG : RT9467_USBSW_USB;

	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	rt9467_set_usbsw_state(info, usbsw);
	ret = (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_DPDM1, RT9467_MASK_USBCHGEN);
	if (ret >= 0)
		info->bc12_en = en;
...
static int rt9467_enable_chgdet_flow(struct rt9467_info *info, bool en)
{
	...
	if (en) {
		...
		for (i = 0; i < max_wait_cnt; i++) {
			...
			ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_STATC, RT9467_SHIFT_PWR_RDY, &pwr_rdy);
			if (ret >= 0 && !pwr_rdy) {
...
static int rt9467_inform_psy_changed(struct rt9467_info *info)
{
	...
	ret = power_supply_set_property(info->psy, POWER_SUPPLY_PROP_ONLINE, &propval);
	if (ret < 0)
		dev_notice(info->dev, "%s: psy online fail(%d)\n", __func__, ret);

	propval.intval = info->chg_type;
	ret = power_supply_set_property(info->psy, POWER_SUPPLY_PROP_CHARGE_TYPE, &propval);
	if (ret < 0)
...
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL3, RT9467_MASK_ILIM_EN);
}
...
static inline int rt9467_toggle_chgdet_flow(struct rt9467_info *info)
{
	...
	dev_info(info->dev, "%s\n", __func__);
	/* read data */
	ret = i2c_smbus_read_i2c_block_data(client, RT9467_REG_CHG_DPDM1, 1, &data);
...
static int __rt9467_chgdet_handler(struct rt9467_info *info)
{
	...
	if (info->pwr_rdy == pwr_rdy && atomic_read(&info->bc12_wkard) == 0 && atomic_read(&info->hvdcp_en_cnt) >= 5) {
	...
	if (atomic_read(&info->bc12_sdp_cnt) < 2 && info->chg_type == STANDARD_HOST) {
...
static int rt9467_set_aicl_vth(struct rt9467_info *info, u32 aicl_vth)
{
	...
	reg_aicl_vth = rt9467_closest_reg(RT9467_AICL_VTH_MIN, RT9467_AICL_VTH_MAX, RT9467_AICL_VTH_STEP, aicl_vth);

	dev_info(info->dev, "%s: vth = %d(0x%02X)\n", __func__, aicl_vth, reg_aicl_vth);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL14, reg_aicl_vth << RT9467_SHIFT_AICL_VTH,
...
static int __rt9467_set_aicr(struct rt9467_info *info, u32 aicr)
{
	...
	reg_aicr = rt9467_closest_reg(RT9467_AICR_MIN, RT9467_AICR_MAX, RT9467_AICR_STEP, aicr);

	dev_info(info->dev, "%s: aicr = %d(0x%02X)\n", __func__, aicr, reg_aicr);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL3, reg_aicr << RT9467_SHIFT_AICR, RT9467_MASK_AICR);
}
...
static int __rt9467_run_aicl(struct rt9467_info *info)
{
	...
	ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_STATC, RT9467_SHIFT_CHG_MIVR, &mivr_act);
	if (ret < 0) {
	...
	rt9467_irq_clr_flag(info, &info->irq_flag[RT9467_IRQIDX_CHG_IRQ2], RT9467_MASK_CHG_AICLMEASI);

	mutex_lock(&info->pe_access_lock);
	mutex_lock(&info->aicr_access_lock);

	ret = rt9467_set_bit(info, RT9467_REG_CHG_CTRL14, RT9467_MASK_AICL_MEAS);
	if (ret < 0)
		goto unlock_out;
...
static void rt9467_chgdet_work_handler(struct work_struct *work)
{
	int ret = 0;
	bool pwr_rdy = false;
	struct rt9467_info *info = (struct rt9467_info *)container_of(work, struct rt9467_info, chgdet_work);
...
static int rt9467_chg_vbusov_irq_handler(struct rt9467_info *info)
{
	...
	dev_notice(info->dev, "%s\n", __func__);
	ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_FAULT, RT9467_SHIFT_VBUSOV, &vbusov);
	if (ret < 0)
		return ret;
...
{
	dev_notice(info->dev, "%s\n", __func__);
	rt9467_irq_set_flag(info, &info->irq_flag[RT9467_IRQIDX_CHG_IRQ2], RT9467_MASK_CHG_AICLMEASI);
...
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL13, RT9467_MASK_IRQ_REZ);
}
...
static int __rt9467_irq_handler(struct rt9467_info *info)
{
	...
	usb_status_old = (ret & RT9467_MASK_USB_STATUS) >> RT9467_SHIFT_USB_STATUS;
	...
	ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL1, RT9467_SHIFT_OPA_MODE, &is_otg_mode);
	if (ret < 0) {
	...
	if (is_otg_mode) {
		mutex_lock(&info->adc_access_lock);
		ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_ADC, RT9467_SHIFT_ADC_START, &is_adc_done);
		if (ret < 0) {
		...
		if (is_adc_done) {
			/*Read CHG_IRQ3*/
			ret = rt9467_i2c_block_read(info, RT9467_REG_CHG_IRQ3, 1, &evt[5]);
			if (ret < 0) {
				dev_notice(info->dev, "%s: read chg_irq3 fail(%d)\n", __func__, ret);
				mutex_unlock(&info->adc_access_lock);
				goto err_read_irq;
			}
		}
	...
	usb_status_new = (ret & RT9467_MASK_USB_STATUS) >> RT9467_SHIFT_USB_STATUS;

	/* Read mask */
	ret = rt9467_i2c_block_read(info, RT9467_REG_CHG_STATC_CTRL, ARRAY_SIZE(mask), mask);
	if (ret < 0) {
	...
	/* Detach */
	if (usb_status_old != RT9467_CHG_TYPE_NOVBUS && usb_status_new == RT9467_CHG_TYPE_NOVBUS)
		evt[RT9467_IRQIDX_DPDM_IRQ] |= 0x02;

	/* Attach */
	if (usb_status_new >= RT9467_CHG_TYPE_SDP && usb_status_new <= RT9467_CHG_TYPE_CDP &&
...
static int rt9467_irq_register(struct rt9467_info *info)
{
	...
	ret = devm_request_threaded_irq(info->dev, info->irq, NULL, rt9467_irq_handler,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT, name, info);
	if (ret < 0) {
...
{
	dev_info(info->dev, "%s\n", __func__);
	return rt9467_i2c_block_write(info, RT9467_REG_CHG_STATC_CTRL, ARRAY_SIZE(rt9467_irq_maskall),
				      rt9467_irq_maskall);
}
...
{
	dev_info(info->dev, "%s\n", __func__);
	return rt9467_i2c_block_write(info, RT9467_REG_CHG_STATC_CTRL, ARRAY_SIZE(info->irq_mask), info->irq_mask);
}
...
static int rt9467_set_safety_timer(struct rt9467_info *info, u32 hr)
{
	...
	reg_st = rt9467_closest_reg_via_tbl(rt9467_safety_timer, ARRAY_SIZE(rt9467_safety_timer), hr);

	dev_info(info->dev, "%s: time = %d(0x%02X)\n", __func__, hr, reg_st);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL12, reg_st << RT9467_SHIFT_WT_FC, RT9467_MASK_WT_FC);
}

static inline int rt9467_enable_wdt(struct rt9467_info *info, bool en)
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL13, RT9467_MASK_WDT_EN);
}

static inline int rt9467_select_input_current_limit(struct rt9467_info *info, enum rt9467_iin_limit_sel sel)
{
	dev_info(info->dev, "%s: sel = %d\n", __func__, sel);
	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL2, sel << RT9467_SHIFT_IINLMTSEL, RT9467_MASK_IINLMTSEL);
}
...
static int rt9467_enable_hidden_mode(struct rt9467_info *info, bool en)
{
	...
	if (en) {
		if (info->hidden_mode_cnt == 0) {
			ret = rt9467_i2c_block_write(info, 0x70, ARRAY_SIZE(rt9467_val_en_hidden_mode),
...
static int rt9467_set_iprec(struct rt9467_info *info, u32 iprec)
{
	...
	reg_iprec = rt9467_closest_reg(RT9467_IPREC_MIN, RT9467_IPREC_MAX, RT9467_IPREC_STEP, iprec);

	dev_info(info->dev, "%s: iprec = %d(0x%02X)\n", __func__, iprec, reg_iprec);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL8, reg_iprec << RT9467_SHIFT_IPREC, RT9467_MASK_IPREC);
}
...
static int rt9467_sw_workaround(struct rt9467_info *info)
{
	...
	if (info->chip_rev <= RT9467_CHIP_REV_E3) {
		...
		if (get_boot_mode() == NORMAL_BOOT) {
			...
			ret = rt9467_i2c_write_byte(info, RT9467_REG_CHG_HIDDEN_CTRL7, 0x1c);
			if (ret < 0)
				goto out;
		}
	}
...
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL1, RT9467_MASK_HZ_EN);
}
...
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL2, RT9467_MASK_TE_EN);
}

static inline int __rt9467_enable_safety_timer(struct rt9467_info *info, bool en)
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL12, RT9467_MASK_TMR_EN);
}
...
static int __rt9467_set_ieoc(struct rt9467_info *info, u32 ieoc)
{
	...
	reg_ieoc = rt9467_closest_reg(RT9467_IEOC_MIN, RT9467_IEOC_MAX, RT9467_IEOC_STEP, ieoc);

	dev_info(info->dev, "%s: ieoc = %d(0x%02X)\n", __func__, ieoc, reg_ieoc);

	ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL9, reg_ieoc << RT9467_SHIFT_IEOC, RT9467_MASK_IEOC);
...
static int __rt9467_get_mivr(struct rt9467_info *info, u32 *mivr)
{
	...
	*mivr = rt9467_closest_value(RT9467_MIVR_MIN, RT9467_MIVR_MAX, RT9467_MIVR_STEP, reg_mivr);
...
static int __rt9467_set_mivr(struct rt9467_info *info, u32 mivr)
{
	...
	reg_mivr = rt9467_closest_reg(RT9467_MIVR_MIN, RT9467_MIVR_MAX, RT9467_MIVR_STEP, mivr);

	dev_info(info->dev, "%s: mivr = %d(0x%02X)\n", __func__, mivr, reg_mivr);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL6, reg_mivr << RT9467_SHIFT_MIVR, RT9467_MASK_MIVR);
}

static inline int rt9467_enable_jeita(struct rt9467_info *info, bool en)
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL16, RT9467_MASK_JEITA_EN);
}

static int rt9467_get_charging_status(struct rt9467_info *info, enum rt9467_charging_status *chg_stat)
{
	int ret = 0;
...
static int rt9467_get_ieoc(struct rt9467_info *info, u32 *ieoc)
{
	...
	*ieoc = rt9467_closest_value(RT9467_IEOC_MIN, RT9467_IEOC_MAX, RT9467_IEOC_STEP, reg_ieoc);

	return ret;
}

static inline int __rt9467_is_charging_enable(struct rt9467_info *info, bool *en)
{
	return rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL2, RT9467_SHIFT_CHG_EN, en);
}
...
static int __rt9467_get_ichg(struct rt9467_info *info, u32 *ichg)
{
	...
	*ichg = rt9467_closest_value(RT9467_ICHG_MIN, RT9467_ICHG_MAX, RT9467_ICHG_STEP, reg_ichg);
...
static inline int rt9467_ichg_workaround(struct rt9467_info *info, u32 uA)
{
	...
	if (info->ichg >= 900000 && uA < 900000)
		ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_HIDDEN_CTRL7, 0x00, 0x60);
	else if (uA >= 900000 && info->ichg < 900000)
		ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_HIDDEN_CTRL7, 0x40, 0x60);
...
static int __rt9467_set_ichg(struct rt9467_info *info, u32 ichg)
{
	...
	reg_ichg = rt9467_closest_reg(RT9467_ICHG_MIN, RT9467_ICHG_MAX, RT9467_ICHG_STEP, ichg);

	dev_info(info->dev, "%s: ichg = %d(0x%02X)\n", __func__, ichg, reg_ichg);

	ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL7, reg_ichg << RT9467_SHIFT_ICHG, RT9467_MASK_ICHG);
	if (ret < 0)
		return ret;
...
static int __rt9467_set_cv(struct rt9467_info *info, u32 cv)
{
	...
	reg_cv = rt9467_closest_reg(RT9467_CV_MIN, RT9467_CV_MAX, RT9467_CV_STEP, cv);

	dev_info(info->dev, "%s: cv = %d(0x%02X)\n", __func__, cv, reg_cv);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL4, reg_cv << RT9467_SHIFT_CV, RT9467_MASK_CV);
}
...
static int rt9467_set_ircmp_resistor(struct rt9467_info *info, u32 uohm)
{
	...
	reg_resistor = rt9467_closest_reg(RT9467_IRCMP_RES_MIN, RT9467_IRCMP_RES_MAX, RT9467_IRCMP_RES_STEP, uohm);

	dev_info(info->dev, "%s: resistor = %d(0x%02X)\n", __func__, uohm, reg_resistor);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL18, reg_resistor << RT9467_SHIFT_IRCMP_RES,
...
static int rt9467_set_ircmp_vclamp(struct rt9467_info *info, u32 uV)
{
	...
	reg_vclamp = rt9467_closest_reg(RT9467_IRCMP_VCLAMP_MIN, RT9467_IRCMP_VCLAMP_MAX, RT9467_IRCMP_VCLAMP_STEP, uV);

	dev_info(info->dev, "%s: vclamp = %d(0x%02X)\n", __func__, uV, reg_vclamp);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL18, reg_vclamp << RT9467_SHIFT_IRCMP_VCLAMP,
...
static int rt9467_enable_pump_express(struct rt9467_info *info, bool en)
{
	...
	ret = (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL17, RT9467_MASK_PUMPX_EN);
	if (ret < 0)
		goto out;

	for (i = 0; i < max_wait_times; i++) {
		msleep(2500);
		ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL17, RT9467_SHIFT_PUMPX_EN, &pumpx_en);
		if (ret >= 0 && !pumpx_en)
			break;
	}
	if (i == max_wait_times) {
...
{
	dev_info(info->dev, "%s: en = %d\n", __func__, en);
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL1, RT9467_MASK_IRQ_PULSE);
}

static inline int rt9467_get_irq_number(struct rt9467_info *info, const char *name)
{
	int i = 0;
...
static int rt9467_parse_dt(struct rt9467_info *info, struct device *dev)
{
	...
	if (ret < 0)
		return ret;
	if (strcmp(desc->chg_dev_name, "secondary_chg") == 0) {
		ret = of_property_read_u32(np, "rt,ceb_gpio_num", &info->ceb_gpio);
		if (ret < 0)
			return ret;
	}
#endif

	dev_info(info->dev, "%s: intr/ceb gpio = %d, %d\n", __func__, info->intr_gpio, info->ceb_gpio);

	/* request ceb gpio for secondary charger */
	if (strcmp(desc->chg_dev_name, "secondary_chg") == 0) {
		ret = devm_gpio_request_one(info->dev, info->ceb_gpio, GPIOF_DIR_OUT, "rt9467_sec_ceb_gpio");
		if (ret < 0) {
	...
	if (of_property_read_u32(np, "regmap_represent_slave_addr", &desc->regmap_represent_slave_addr) < 0)
	...
	while (true) {
		ret = of_property_read_string_index(np, "interrupt-names", irq_cnt, &name);
		if (ret < 0)
			break;
		irq_cnt++;
...
static int __rt9467_enable_charging(struct rt9467_info *info, bool en)
{
	...
	return (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL2, RT9467_MASK_CHG_EN);
}
...
static int __rt9467_set_boost_current_limit(struct rt9467_info *info, u32 current_limit)
{
	u8 reg_ilimit = 0;

	reg_ilimit = rt9467_closest_reg_via_tbl(rt9467_boost_oc_threshold, ARRAY_SIZE(rt9467_boost_oc_threshold),
						current_limit);

	dev_info(info->dev, "%s: boost ilimit = %d(0x%02X)\n", __func__, current_limit, reg_ilimit);

	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL10, reg_ilimit << RT9467_SHIFT_BOOST_OC,
				      RT9467_MASK_BOOST_OC);
}

static int rt9467_set_boost_current_limit(struct charger_device *chg_dev, u32 current_limit)
{
	u8 reg_ilimit = 0;
	struct rt9467_info *info = dev_get_drvdata(&chg_dev->dev);
...
static int __rt9467_enable_otg(struct rt9467_info *info, bool en)
{
	...
	/*
		 ...
		 */
	ret = rt9467_i2c_write_byte(info, RT9467_REG_CHG_HIDDEN_CTRL4, lg_slew_rate);
	if (ret < 0) {
		dev_notice(info->dev, "%s: set Low side mos Gate drive speed fail(%d)\n", __func__, ret);
		goto out;
	}
	...
	/* Switch OPA mode */
	ret = (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL1, RT9467_MASK_OPA_MODE);

	msleep(20);

	if (en) {
		ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL1, RT9467_SHIFT_OPA_MODE, &en_otg);
		if (ret < 0 || !en_otg) {
	...
	/*
		 ...
		 */
	ret = rt9467_i2c_write_byte(info, RT9467_REG_CHG_HIDDEN_CTRL6, hidden_val);
	if (ret < 0)
...
err_en_otg:
	...
	if (ret < 0)
		dev_notice(info->dev, "%s: recover Low side mos Gate drive speed fail(%d)\n", __func__, ret);
	ret = -EIO;
out:
	rt9467_enable_hidden_mode(info, false);
	mutex_unlock(&info->otg_access_lock);
...
static int rt9467_enable_discharge(struct charger_device *chg_dev, bool en)
{
	...
	ret = (en ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_HIDDEN_CTRL2, 0x04);
	if (ret < 0) {
	...
	if (!en) {
		for (i = 0; i < check_dischg_max; i++) {
			ret = rt9467_i2c_test_bit(info, RT9467_REG_CHG_HIDDEN_CTRL2, 2, &is_dischg);
			if (ret >= 0 && !is_dischg)
				break;
			/* Disable discharging */
			ret = rt9467_clr_bit(info, RT9467_REG_CHG_HIDDEN_CTRL2, 0x04);
		}
...
static int rt9467_set_mivr(struct charger_device *chg_dev, u32 mivr)
{
	...
	if (!en) {
		dev_info(info->dev, "%s: power path is disabled, op is not allowed\n", __func__);
		return -EINVAL;
	}
...
static int rt9467_set_pep_current_pattern(struct charger_device *chg_dev, bool is_increase)
{
	int ret = 0;
	struct rt9467_info *info = dev_get_drvdata(&chg_dev->dev);
	...
	/* Set to PE1.0 */
	ret = rt9467_clr_bit(info, RT9467_REG_CHG_CTRL17, RT9467_MASK_PUMPX_20_10);

	/* Set Pump Up/Down */
	ret = (is_increase ? rt9467_set_bit : rt9467_clr_bit)(info, RT9467_REG_CHG_CTRL17, RT9467_MASK_PUMPX_UP_DN);
...
static int rt9467_set_pep20_current_pattern(struct charger_device *chg_dev, u32 uV)
{
	int ret = 0;
	u8 reg_volt = 0;
	struct rt9467_info *info = dev_get_drvdata(&chg_dev->dev);
	...
	reg_volt = rt9467_closest_reg(RT9467_PEP20_VOLT_MIN, RT9467_PEP20_VOLT_MAX, RT9467_PEP20_VOLT_STEP, uV);

	dev_info(info->dev, "%s: volt = %d(0x%02X)\n", __func__, uV, reg_volt);

	/* Set to PEP2.0 */
	ret = rt9467_set_bit(info, RT9467_REG_CHG_CTRL17, RT9467_MASK_PUMPX_20_10);
	if (ret < 0)
		goto out;

	/* Set Voltage */
	ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL17, reg_volt << RT9467_SHIFT_PUMPX_DEC,
...
static int rt9467_enable_cable_drop_comp(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	struct rt9467_info *info = dev_get_drvdata(&chg_dev->dev);
	...
	/* Set Voltage */
	ret = rt9467_i2c_update_bits(info, RT9467_REG_CHG_CTRL17, 0x1F << RT9467_SHIFT_PUMPX_DEC,
				     RT9467_MASK_PUMPX_DEC);
	if (ret < 0)
		goto out;
...
static int __rt9467_get_aicr(struct rt9467_info *info, u32 *aicr)
{
	...
	*aicr = rt9467_closest_value(RT9467_AICR_MIN, RT9467_AICR_MAX, RT9467_AICR_STEP, reg_aicr);
...
static int rt9467_get_cv(struct charger_device *chg_dev, u32 *cv)
{
	...
	*cv = rt9467_closest_value(RT9467_CV_MIN, RT9467_CV_MAX, RT9467_CV_STEP, reg_cv);

	return ret;
}

static int rt9467_get_tchg(struct charger_device *chg_dev, int *tchg_min, int *tchg_max)
{
	...
	while (adc_temp >= 120 && retry_cnt > 0) {
		dev_notice(info->dev, "%s: [WARNING] t = %d\n", __func__, adc_temp);
...
static int rt9467_get_mivr_state(struct charger_device *chg_dev, bool *in_loop)
{
	...
	return rt9467_i2c_test_bit(info, RT9467_REG_CHG_STATC, RT9467_SHIFT_CHG_MIVR, in_loop);
}
...
static int rt9467_is_safety_timer_enable(struct charger_device *chg_dev, bool *en)
{
	struct rt9467_info *info = dev_get_drvdata(&chg_dev->dev);

	return rt9467_i2c_test_bit(info, RT9467_REG_CHG_CTRL12, RT9467_SHIFT_TMR_EN, en);
}
...
static int __rt9467_enable_auto_sensing(struct rt9467_info *info, bool en)
{
	...
	/* enter hidden mode */
	ret = rt9467_device_write(info->client, 0x70, ARRAY_SIZE(rt9467_val_en_hidden_mode), rt9467_val_en_hidden_mode);
	if (ret < 0)
		return ret;

	ret = rt9467_device_read(info->client, RT9467_REG_CHG_HIDDEN_CTRL15, 1, &auto_sense);
	if (ret < 0) {
	...
	ret = rt9467_device_write(info->client, RT9467_REG_CHG_HIDDEN_CTRL15, 1, &auto_sense);
	if (ret < 0)
		dev_notice(info->dev, "%s: en = %d fail\n", __func__, en);
...
static int rt9467_sw_reset(struct rt9467_info *info)
{
	...
	/* Register 0x01 ~ 0x10 */
	u8 reg_data[] = {
		0x10, 0x03, 0x23, 0x3C, 0x67, 0x0B, 0x4C, 0xA1, 0x3C, 0x58, 0x2C, 0x02, 0x52, 0x05, 0x00, 0x10
	};
	...
	/* Mask all irq */
	mutex_lock(&info->i2c_access_lock);
	ret = rt9467_device_write(info->client, RT9467_REG_CHG_STATC_CTRL, ARRAY_SIZE(rt9467_irq_maskall),
				  rt9467_irq_maskall);
	if (ret < 0)
	...
	ret = rt9467_device_write(info->client, RT9467_REG_CHG_CTRL1, ARRAY_SIZE(reg_data), reg_data);
	if (ret < 0)
...
static int rt9467_init_setting(struct rt9467_info *info)
{
	...
	/* clear event */
	ret = rt9467_i2c_block_read(info, RT9467_REG_CHG_STATC, ARRAY_SIZE(evt), evt);
	if (ret < 0) {
...
static int rt9467_get_min_ichg(struct charger_device *chg_dev, u32 *uA)
{
	...
	*uA = rt9467_closest_value(RT9467_ICHG_MIN, RT9467_ICHG_MAX, RT9467_ICHG_STEP, 0);
...
static int __rt9467_dump_registers(struct rt9467_info *info)
{
	...
	dev_info(info->dev,
		 ...
		 __func__, ichg / 1000, aicr / 1000, mivr / 1000, ieoc / 1000, adc_vsys / 1000, adc_vbat / 1000,
		 adc_ibat / 1000, adc_ibus / 1000, chg_en, rt9467_chg_status_name[chg_status], chg_stat, chg_ctrl[0],
		 chg_ctrl[1]);
...
static int rt9467_dump_register(struct charger_device *chg_dev)
{
	...
	dev_info(info->dev, "%s: ICHG = %dmA, AICR = %dmA, MIVR = %dmV, IEOC = %dmA\n", __func__, ichg / 1000,
		 aicr / 1000, mivr / 1000, ieoc / 1000);

	dev_info(info->dev, "%s: VSYS = %dmV, VBAT = %dmV, IBAT = %dmA, IBUS = %dmA\n", __func__, adc_vsys / 1000,
		 adc_vbat / 1000, adc_ibat / 1000, adc_ibus / 1000);

	dev_info(info->dev, "%s: CHG_EN = %d, CHG_STATUS = %s, CHG_STAT = 0x%02X\n", __func__, chg_en,
		 rt9467_chg_status_name[chg_status], chg_stat);
...
static int rt9467_send_hvdcp_pattern_ex(struct charger_device *chg_dev, bool up)
{
	...
	return rt9467_i2c_update_bits(info, RT9467_REG_CHG_DPDM2, up ? 0x20 : 0x80, 0xE0);
}
#endif
...
static ssize_t shipping_mode_9467_store(struct device *dev, struct device_attribute *attr, const char *buf,
					size_t count)
{
	int ret = 0, tmp = 0;
...
static void rt9467_init_setting_work_handler(struct work_struct *work)
{
	...
	struct rt9467_info *info = (struct rt9467_info *)container_of(work, struct rt9467_info, init_work);
...
static int rt9467_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	int ret = 0;
	...
	info = devm_kzalloc(&client->dev, sizeof(struct rt9467_info), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	rt9467 = info;
#ifdef OPLUS_FEATURE_CHG_BASIC
	info->chg_consumer = charger_manager_get_by_name(&client->dev, "rt9467");
#endif
...
//#ifdef CONFIG_RT_REGMAP
	...
	info->chg_dev =
		charger_device_register(info->desc->chg_dev_name, info->dev, info, &rt9467_chg_ops, &info->chg_props);
...
static const struct i2c_device_id rt9467_i2c_id[] = { { "rt9467", 0 }, {} };
...
static struct i2c_board_info rt9467_i2c_board_info __initdata = { I2C_BOARD_INFO("rt9467", RT9467_SALVE_ADDR) };
#endif /* CONFIG_OF */
