...
static void oplus_3rd_pps_slave_cp_ibus_check(struct oplus_pps_chip *chip)
{
	...
	if (chip->data.ap_input_current > PPS_THIRD_IBUS_SLAVE_ENABLE_MAX && !slave_cp_enable &&
	    chip->data.slave_trouble_count <= 1) {
...
static int pps_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", chip->pps_support_type, chip->pps_status,
