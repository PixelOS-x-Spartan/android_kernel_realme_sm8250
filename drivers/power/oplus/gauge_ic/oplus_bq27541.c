...
static int bq28z610_get_true_fcc(struct chip_bq27541 *chip, int *true_fcc)
{
	...
	ret = bq27541_read_block(chip, BQ28Z610_REG_TRUE_FCC, buf, BQ28Z610_TRUE_FCC_NUM_SIZE, BQ28Z610_TRUE_FCC_OFFSET,
				 false);
	if (ret < 0)
		return ret;
...
static void bq27541_fcc_too_small_check_work(struct work_struct *work)
{
	int ret;
	int true_fcc = 0;
	struct chip_bq27541 *chip = container_of(work, struct chip_bq27541, fcc_too_small_check_work);
...
static int bq27541_track_upload_mode_info(struct chip_bq27541 *chip)
{
	...
	if (chip->capacity_pct == 0) {
		index += snprintf(&(p_track_mode->load_trigger->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$device_id@@%s", chip->device_name);
	} else {
		index +=
			snprintf(&(p_track_mode->load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				 "$$device_id@@%s_%d", chip->device_name, chip->gauge_num);
	}
	if (p_track_mode->mode_check_buf) {
		index += snprintf(&(p_track_mode->load_trigger->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s", p_track_mode->mode_check_buf);
...
static void bq27541_track_mode_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct chip_bq27541 *chip = container_of(dwork, struct chip_bq27541, track_mode.load_trigger_work);
...
int bq27541_track_update_mode_buf(struct chip_bq27541 *chip, char *buf)
{
	...
	if (!chip || !chip->track_mode.track_init_done || !buf || !chip->track_mode.mode_check_buf)
		return -EFAULT;
...
static int bq27541_track_init(struct chip_bq27541 *chip)
{
	...
	INIT_DELAYED_WORK(&chip->track_mode.load_trigger_work, bq27541_track_mode_load_trigger_work);
