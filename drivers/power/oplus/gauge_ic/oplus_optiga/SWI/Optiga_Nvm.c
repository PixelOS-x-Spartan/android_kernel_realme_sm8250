 ...
 ************************************************************************* */
BOOL Nvm_ProgramData(BOOL b_WaitForFinish, BOOL b_VerifyData, uint16_t uw_Address, uint8_t ub_BytesToProgram,
		     uint8_t *ubp_Data)
{
	BOOL bResult;
	uint8_t ubData;
...
uint16_t Nvm_ReadODC(uint8_t *ubODC, uint8_t *ubPUBKEY, NVM_OPS ReadMode)
{
	...
	for (j = 0; j < (NVM_ODC_PAGE_COUNT + NVM_PUK_PAGE_COUNT);
	     j++) { /*!< Read 9 pages from NVM, starting from page 23 */
		printk(" Nvm_ReadODC start  j =%d\n", j);
		spin_lock_irqsave(&optiga_chip_info->slock, flags);

		if (ret != INF_SWI_SUCCESS) {
			Swi_WriteRegisterSpaceNoIrq(NVM_ADDR_REG, 0x18,
						    0x3F); /*!< Set NVM data length read/write to 8 bytes to WIP */
		}

		/*!< load 8 bytes of NVM data to WIP2 starting for below page*/
		Swi_WriteRegisterSpaceNoIrq(
			CTRL2_NVM_REG, (uint8_t)(0x97 + j),
			REGISTER_OVERWRITE); /*!< Set Base Page to 16 + offset 7 = Page 23 for ODC */
...
uint16_t nvm_read_page(uint8_t page, uint8_t page_count, uint8_t *data)
{
	...
	for (p = 0; p < page_count; p++) {
		...
		Swi_WriteRegisterSpaceNoIrq(CTRL2_NVM_REG, (uint8_t)(CTRL2_NVM_REG__NVM_ACT + page + p),
					    REGISTER_OVERWRITE);
