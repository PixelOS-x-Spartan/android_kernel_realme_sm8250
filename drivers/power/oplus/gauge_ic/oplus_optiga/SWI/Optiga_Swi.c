...
BOOL Swi_WriteRegisterSpace(uint16_t uw_Address, uint8_t ub_Data, uint8_t ub_BitSelect, BOOL b_WaitForInterrupt,
			    BOOL b_ImmediateInterrupt, BOOL *bp_IrqDetected)
{
	BOOL bIntOccured;
...
void Swi_SendRawWord(uint8_t ub_Code, uint8_t ub_Data, BOOL b_WaitForInterrupt, BOOL b_ImmediateInterrupt,
		     BOOL *bp_IrqDetected)
{
	uint8_t inversion = 0;
...
BOOL Swi_ReceiveRawWord(uint8_t *up_Byte)
{
	...
	*up_Byte = (((ulTimes[8] > ulThreshold) ? 1u : 0u) << 7) | (((ulTimes[7] > ulThreshold) ? 1u : 0u) << 6) |
		   (((ulTimes[6] > ulThreshold) ? 1u : 0u) << 5) | (((ulTimes[5] > ulThreshold) ? 1u : 0u) << 4) |
		   (((ulTimes[4] > ulThreshold) ? 1u : 0u) << 3) | (((ulTimes[3] > ulThreshold) ? 1u : 0u) << 2) |
		   (((ulTimes[2] > ulThreshold) ? 1u : 0u) << 1) | (((ulTimes[1] > ulThreshold) ? 1u : 0u) << 0);
	...
	do {
		...
		for (; ubBitCnt > 0; ubBitCnt--) {
			...
			if (b_UidSearch_GetDipDoneBit(ubBitInfo[ubCurrentIdPtr]) == FALSE) // need to do DIP
			{
				...
				if ((bFound_0 == TRUE) &&
				    (bFound_1 == TRUE)) // both DIP0 and DIP1 has positive response
				{
