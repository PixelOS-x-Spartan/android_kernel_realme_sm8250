...
int oplus_optiga_parse_batt_info_dt(struct oplus_optiga_chip *chip)
{
	...
	if (length != 0 && (length * sizeof(int)) % sizeof(struct optiga_batt_vendor_info) == 0) {
		vendor_info_buffer =
			devm_kzalloc(chip->dev, chip->barcode_num * sizeof(struct optiga_batt_vendor_info), GFP_KERNEL);

		if (vendor_info_buffer) {
			rc = of_property_read_u32_array(node, "oplus,optiga_batt_info", (int *)vendor_info_buffer,
							length);
			if (rc < 0) {
...
static int oplus_optiga_chk_barcode(void)
{
	...
	if (chip->barcode_num <= 0 || chip->barcode_num > MAX_BARCODE_NUM) {
