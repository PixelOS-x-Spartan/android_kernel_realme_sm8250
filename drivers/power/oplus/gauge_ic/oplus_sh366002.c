...
static int sh366002_track_upload_fix_cadc_info(int old_cadcoffset, int new_cadcoffset, int packlot,
					       struct chip_bq27541 *bq_chip);
...
static int fg_read_block(struct chip_bq27541 *chip, u32 reg, u8 startIndex, u8 length, u8 *val)
{
	...
	if ((reg & CMDMASK_CNTL_R) == CMDMASK_CNTL_R) {
		...
		msleep(CMD_SBS_DELAY);
		ret |= __fg_read_buffer(chip, (u8)(CMD_BLOCK + DF_PAGE_HALF_LEN), (u8)(DF_PAGE_LEN - DF_PAGE_HALF_LEN),
					&readbuf[DF_PAGE_HALF_LEN]);
		if (ret < 0) {
...
static s32 fg_gauge_fix_cadcOffset_then_disable_sleep_mode(struct chip_bq27541 *chip) /* 20220625, Ethan */
{
	...
	if (temp16 == DFCONFIG_CALIED) {
		...
		if (ret == 0) {
			...
			sh366002_track_upload_fix_cadc_info((buf_cadcoffet[1] | (buf_cadcoffet[0] << 8)),
							    (buf0[1] | (buf0[0] << 8)), (buf1[1] | (buf1[0] << 8)),
							    chip);
		}
	}
...
s32 fg_gauge_calibrate_board(struct chip_bq27541 *chip) /* 20220625, Ethan */
{
	...
	pr_err("cali ok! cur_avg=%d, cadc_ratio=%d, cadc_offset_origin=%d, cadc_offset_new=%d, ret=%d\r\n", cur_avg,
	       cadc_ratio, cadc_offset_origin, cadc_offset, ret);
...
fg_gauge_restore_cell_model_end:
	...
	bq27541_track_update_mode_buf(chip, track_buf);
	len = strlen(str);
	snprintf(&(str[len]), sizeof(str) - len - 1, "$$cycle_%d@@%d$$ret_%d@@%d", track_count, cycle0, track_count,
		 ret);
	bq27541_track_update_mode_buf(chip, str);
	track_count += 1;
	fg_gauge_seal(chip);
...
s32 fg_gauge_check_por_soc(struct chip_bq27541 *chip)
{
	...
	snprintf(&(track_buf[0]), sizeof(track_buf), "$$scene_%d@@por_soc$$cc_info@@%d,%d", track_count, cycleCount,
		 cycleModel);
	bq27541_track_update_mode_buf(chip, track_buf);
					       ...
					       struct chip_bq27541 *bq_chip)
{
	...
	chip->load_trigger->type_reason = TRACK_NOTIFY_TYPE_GENERAL_RECORD;
	chip->load_trigger->flag_reason = TRACK_NOTIFY_FLAG_GAUGE_MODE;
	chip->uploading = true;
	mutex_unlock(&chip->track_lock);
	if (bq_chip->capacity_pct == 0) {
		index += snprintf(&(chip->load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				  "$$device_id@@%s", "zy0602");
	} else {
		index += snprintf(&(chip->load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				  "$$device_id@@%s_%d", "zy0602", bq_chip->gauge_num);
	}

	index += snprintf(&(chip->load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$scene@@%s",
			  "fix_cadc_offset");
...
static void sh366002_track_fix_cadc_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct sh366002_model_track *chip = container_of(dwork, struct sh366002_model_track, load_trigger_work);
...
static int sh366002_track_init(void)
{
	...
	INIT_DELAYED_WORK(&fix_cadc_offset.load_trigger_work, sh366002_track_fix_cadc_load_trigger_work);
