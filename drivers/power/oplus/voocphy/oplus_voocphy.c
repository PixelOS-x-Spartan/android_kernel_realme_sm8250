...
static int voocphy_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", chip->current_max, chip->current_ap,
		 chip->current_batt_temp, chip->current_bcc, chip->current_slow_chg, chip->fast_chg_type,
		 chip->fastchg_notify_status, chip->fastchg_to_warm, oplus_voocphy_get_fastchg_to_warm(),
		 chip->fastchg_dummy_start, chip->fastchg_err_commu, chip->fastchg_check_stop,
