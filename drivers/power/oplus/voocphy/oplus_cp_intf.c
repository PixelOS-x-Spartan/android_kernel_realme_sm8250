...
static bool oplus_cp_slave_get_enable(void)
{
	...
	if (cp_device[CP_SLAVE_A].dev_ops != NULL && cp_device[CP_SLAVE_A].dev_ops->oplus_get_cp_enable != NULL) {
...
static int oplus_cp_master_get_maxcur(void)
{
	...
	if (cp_device[CP_MASTER].dev_ops != NULL && cp_device[CP_MASTER].dev_ops->oplus_get_cp_maxcur != NULL)
