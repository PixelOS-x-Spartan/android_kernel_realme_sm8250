...
struct oplus_impedance_node *oplus_imp_node_register(struct device_node *of_node, struct device *dev, void *priv_data,
						     int (*get_imp_func)(void *))
{
	struct oplus_impedance_node *node;
	int rc;
	...
	node = devm_kzalloc(dev, sizeof(struct oplus_impedance_node) + rc * sizeof(s32), GFP_KERNEL);
	if (node == NULL) {
...
static int __oplus_imp_uint_register(struct device_node *of_node, struct device *dev)
{
	...
	imp_uint = devm_kzalloc(dev,
				sizeof(struct oplus_impedance_unit) + node_num * sizeof(struct oplus_impedance_node *),
				GFP_KERNEL);
	if (imp_uint == NULL) {
