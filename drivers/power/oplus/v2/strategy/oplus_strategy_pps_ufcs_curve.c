...
static int __read_signed_data_from_node(struct device_node *node, const char *prop_str, s32 *addr, int len_max)
{
	int rc = 0, length;
...
static int __read_unsigned_data_from_node(struct device_node *node, const char *prop_str, u32 *addr, int len_max)
{
	int rc = 0, length;
...
static int puc_strategy_get_vbat(struct puc_strategy *puc, int *vbat)
{
	...
	rc = oplus_mms_get_item_data(gauge_topic, GAUGE_ITEM_VOL_MAX, &data, false);
	if (rc < 0) {
...
static int puc_strategy_get_temp(struct puc_strategy *puc, int *temp)
{
	...
	case STRATEGY_USE_BATT_TEMP:
		...
		rc = oplus_mms_get_item_data(gauge_topic, GAUGE_ITEM_TEMP, &data, false);
		if (rc < 0) {
	...
	case STRATEGY_USE_SHELL_TEMP:
		...
		rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_SHELL_TEMP, &data, false);
		if (rc < 0) {
...
static enum puc_soc_range puc_get_soc_region(struct puc_strategy *puc)
{
	int soc;
...
static enum puc_temp_range puc_get_temp_region(struct puc_strategy *puc)
{
	int temp;
...
static struct oplus_chg_strategy *puc_strategy_alloc(unsigned char *buf, size_t size)
{
	return ERR_PTR(-ENOTSUPP);
}

static struct oplus_chg_strategy *puc_strategy_alloc_by_node(struct device_node *node)
{
	...
	rc = __read_unsigned_data_from_node(node, "oplus,soc_range", (u32 *)puc->soc_range_data,
					    PUC_BATT_CURVE_SOC_RANGE_MAX + 1);
	if (rc < 0) {
		chg_err("get oplus,soc_range property error, rc=%d\n", rc);
		goto base_info_err;
	}
	rc = __read_signed_data_from_node(node, "oplus,temp_range", (s32 *)puc->temp_range_data,
	...
	for (i = 0; i < PUC_BATT_CURVE_SOC_RANGE_MAX; i++) {
		...
		for (j = 0; j < PUC_BATT_CURVE_TEMP_RANGE_MAX; j++) {
			length = of_property_count_elems_of_size(soc_node, puc_strategy_temp[j], sizeof(u32));
			if (length < 0) {
			...
			rc = of_property_read_u32_array(soc_node, puc_strategy_temp[j],
							(u32 *)puc->soc_curves[i].temp_curves[j].data, length);
			if (rc < 0) {
...
out:
	...
	if (curve_level_update) {
		...
		if (data->target_time > 0)
			...
			puc->timeout = 0;
		puc->over_time = 0;
		chg_info("level[%d]: %d %d %d %d %d\n", puc->curr_level, data->target_vbus, data->target_vbat,
