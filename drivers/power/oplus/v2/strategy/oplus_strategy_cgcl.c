...
static struct oplus_chg_strategy *cgcl_strategy_alloc(unsigned char *buf, size_t size)
{
	...
	data_num = cgcl_strategy_get_data_table_num(buf + sizeof(u32), data_size);
	if (data_num == 0) {
	...
	if ((strategy->temp_type != STRATEGY_USE_BATT_TEMP) && (strategy->temp_type != STRATEGY_USE_SHELL_TEMP)) {
	...
	strategy->temp_max = strategy->data[strategy->data_num - 1].heat_temp;
...
static int cgcl_strategy_init(struct oplus_chg_strategy *strategy)
{
	...
	cgcl->temp_region.index = cgcl_strategy_get_index(cgcl, temp);
...
static int cgcl_strategy_get_data(struct oplus_chg_strategy *strategy, void *ret)
{
	...
	while (temp_diff != 0) {
		if (temp_diff > 0 && temp > cgcl->data[cgcl->temp_region.index].heat_temp) {
			cgcl->temp_region.index = cgcl->data[cgcl->temp_region.index].heat_next_index;
			if (temp > cgcl->data[cgcl->temp_region.index].heat_temp &&
			    cgcl->data[cgcl->temp_region.index].heat_temp < cgcl->temp_max)
				cgcl->temp_region.temp = cgcl->data[cgcl->temp_region.index].heat_temp;
			else
				cgcl->temp_region.temp = temp;
		} else if (temp_diff < 0 && temp <= cgcl->data[cgcl->temp_region.index].cool_temp) {
			cgcl->temp_region.index = cgcl->data[cgcl->temp_region.index].cool_next_index;
			if (temp <= cgcl->data[cgcl->temp_region.index].cool_temp &&
			    cgcl->data[cgcl->temp_region.index].cool_temp > cgcl->temp_min)
				cgcl->temp_region.temp = cgcl->data[cgcl->temp_region.index].cool_temp;
			else
				cgcl->temp_region.temp = temp;
		} else {
			cgcl->temp_region.temp = temp;
