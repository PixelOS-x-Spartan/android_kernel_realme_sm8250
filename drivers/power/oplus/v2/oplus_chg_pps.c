...
static const int pps_cool_down_oplus_curve[] = { 15000, 1000,  1000,  1200,  1500,  1700,  2000,  2200,	 2500,	2700,
...
__maybe_unused static bool is_wired_suspend_votable_available(struct oplus_pps *chip)
{
...
__maybe_unused static bool is_wired_icl_votable_available(struct oplus_pps *chip)
{
...
__maybe_unused static bool is_gauge_topic_available(struct oplus_pps *chip)
{
	if (!chip->gauge_topic)
...
__maybe_unused static int pps_find_current_to_level(int val, const struct current_level *const table, int len)
{
	int i;
...
__maybe_unused static int32_t oplus_pps_get_curve_vbus(struct oplus_pps *chip)
{
...
__maybe_unused static int32_t oplus_pps_get_curve_ibus(struct oplus_pps *chip)
{
	struct puc_strategy_ret_data data;
	int rc;
...
__maybe_unused static int oplus_pps_pdo_set(struct oplus_pps *chip, int vol_mv, int curr_ma)
{
	int rc;
...
__maybe_unused static int oplus_pps_hard_reset(struct oplus_pps *chip)
{
	int rc;
...
__maybe_unused static int oplus_pps_exit_pps_mode(struct oplus_pps *chip)
{
	int rc;
...
__maybe_unused static int oplus_pps_get_err_info(struct oplus_pps *chip, u64 *err_info)
{
	int rc;
...
__maybe_unused static int oplus_pps_get_pps_status_info(struct oplus_pps *chip, u32 *pps_status_info)
{
	int rc;
...
__maybe_unused static int oplus_pps_get_pdo_info(struct oplus_pps *chip, u32 *pdo, int num)
{
	int rc;
...
__maybe_unused static int oplus_pps_verify_adapter(struct oplus_pps *chip)
{
	int rc;
...
__maybe_unused static int oplus_pps_get_power_change_info(struct oplus_pps *chip, u32 *pwr_change_info, int num)
{
	int rc;
	...
	rc = oplus_chg_ic_func(chip->pps_ic, OPLUS_IC_FUNC_PPS_GET_POWER_CHANGE_INFO, pwr_change_info, num);

	return rc;
}

__maybe_unused static int oplus_pps_cp_enable(struct oplus_pps *chip, bool en)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_hw_init(struct oplus_pps *chip)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_set_work_mode(struct oplus_pps *chip, enum oplus_cp_work_mode mode)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_work_mode(struct oplus_pps *chip, enum oplus_cp_work_mode *mode)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_check_work_mode_support(struct oplus_pps *chip, enum oplus_cp_work_mode mode)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_set_iin(struct oplus_pps *chip, int iin)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_vin(struct oplus_pps *chip, int *vin)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_iin(struct oplus_pps *chip, int *iin)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_vout(struct oplus_pps *chip, int *vout)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_iout(struct oplus_pps *chip, int *iout)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_vac(struct oplus_pps *chip, int *vac)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_set_work_start(struct oplus_pps *chip, bool start)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_get_work_status(struct oplus_pps *chip, bool *start)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_adc_enable(struct oplus_pps *chip, bool en)
{
	int rc;
...
__maybe_unused static int oplus_pps_cp_reg_dump(struct oplus_pps *chip)
{
	int rc;
...
static int oplus_pps_switch_to_normal(struct oplus_pps *chip)
{
	...
	if (chip->wired_online)
		rc = oplus_chg_ic_func(chip->pps_ic, OPLUS_IC_FUNC_FIXED_PDO_SET, OPLUS_FIXED_PDO_DEF_VOL,
				       OPLUS_FIXED_PDO_CURR_MA);
...
static int oplus_pps_set_online(struct oplus_pps *chip, bool online)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, PPS_ITEM_ONLINE);
	if (msg == NULL) {
...
static int oplus_pps_set_charging(struct oplus_pps *chip, bool charging)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, PPS_ITEM_CHARGING);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_pps_set_adapter_id(struct oplus_pps *chip, u16 id)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, PPS_ITEM_ADAPTER_ID);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_pps_set_oplus_adapter(struct oplus_pps *chip, bool oplus_adapter)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, PPS_ITEM_OPLUS_ADAPTER);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static bool oplus_pps_charge_allow_check(struct oplus_pps *chip)
{
	...
	if (chg_temp < chip->limits.pps_batt_over_low_temp ||
		...
		if (chip->ui_soc > chip->limits.pps_warm_allow_soc || vbat_mv > chip->limits.pps_warm_allow_vol)
			vote(chip->pps_not_allow_votable, BATT_TEMP_VOTER, true, 1, false);
		else
	...
	if (chip->ui_soc < chip->limits.pps_strategy_soc_over_low || chip->ui_soc > chip->limits.pps_strategy_soc_high)
...
static void oplus_pps_switch_check_work(struct work_struct *work)
{
	...
	struct oplus_pps *chip = container_of(dwork, struct oplus_pps, switch_check_work);
	...
	for (i = 0; i < chip->pdo_num; i++) {
		...
		chg_err("pdo[%d]: voltage[%u-%umV,  max current[%umA]\n", i,
			PPS_PDO_VOL_MIN(chip->pdo[i].min_voltage100mv), PPS_PDO_VOL_MAX(chip->pdo[i].max_voltage100mv),
...
#define PPS_START_PDO_DELAY_MS 500
	...
	if ((cp_vin >= target_vbus && cp_vin <= (target_vbus + OPLUS_PPS_VOLT_UPDATE_200MV)) || chip->mos_on_check) {
...
static int oplus_pps_set_current_warm_range(struct oplus_pps *chip, int vbat_temp_cur)
{
...
static int oplus_pps_set_current_temp_normal_range(struct oplus_pps *chip, int vbat_temp_cur)
{
	int ret = chip->limits.pps_strategy_normal_current;
...
static int oplus_pps_set_current_temp_low_normal_range(struct oplus_pps *chip, int vbat_temp_cur)
{
	int ret = chip->limits.pps_strategy_normal_current;
...
static int oplus_pps_set_current_temp_little_cool_range(struct oplus_pps *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_pps_set_current_temp_cool_range(struct oplus_pps *chip, int vbat_temp_cur)
{
	int ret = chip->limits.pps_strategy_normal_current;
	;
...
static int oplus_pps_set_current_temp_little_cold_range(struct oplus_pps *chip, int vbat_temp_cur)
{
	int ret = chip->limits.pps_strategy_normal_current;
...
static void oplus_pps_check_low_curr_temp_status(struct oplus_pps *chip)
{
	...
	} else if (chip->pps_low_curr_full_temp_status != PPS_LOW_CURR_FULL_CURVE_TEMP_LITTLE_COOL) {
		t_cnts++;
		if (t_cnts >= PPS_TEMP_OVER_COUNTS) {
...
#define PPS_FULL_COUNTS_HW 3
	...
	if ((batt_temp >= chip->limits.pps_cool_temp) && (batt_temp < chip->limits.pps_batt_over_high_temp)) {
	...
	if ((chip->pps_fastchg_batt_temp_status == PPS_BAT_TEMP_WARM) && (vbat_mv > chip->limits.pps_full_warm_vbat)) {
		chip->count.cool_fw++;
		if (chip->count.cool_fw >= PPS_FULL_COUNTS_COOL) {
			chip->count.cool_fw = 0;
			vote(chip->pps_disable_votable, CHG_FULL_VOTER, true, 1, false);
...
#define PPS_TBATT_OV_CNT 1
	...
	if (!oplus_pps_btb_temp_check(chip)) {
		...
		if (chip->count.btb_high >= PPS_BTB_OV_CNT) {
			...
			if (is_wired_icl_votable_available(chip))
				vote(chip->wired_icl_votable, BTB_TEMP_OVER_VOTER, true, BTB_TEMP_OVER_MAX_INPUT_CUR,
				     true);
		}
	} else {
		chip->count.btb_high = 0;
...
#define IBAT_OVER_ITARGET_CNT (4)
	...
	chg_info("last_vol_set_mv=%d, vtarget_mv=%d, vbus_min=%d, Itarget=%d", chip->vol_set_mv, vtarget_mv,
		 allowed_vbus_min, chip->target_curr_ma);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_pps *chip = container_of(dwork, struct oplus_pps, monitor_work);
	struct puc_strategy_ret_data data;
	int rc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_pps *chip = container_of(dwork, struct oplus_pps, current_work);
	int target_vbus;
...
{
	struct oplus_pps *chip = container_of(work, struct oplus_pps, wired_online_work);
...
{
	struct oplus_pps *chip = container_of(work, struct oplus_pps, force_exit_work);
...
{
	struct oplus_pps *chip = container_of(work, struct oplus_pps, soft_exit_work);
...
static void oplus_pps_gauge_update_work(struct work_struct *work)
{
	...
	struct oplus_pps *chip = container_of(work, struct oplus_pps, gauge_update_work);
...
static void oplus_pps_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_pps_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(topic, chip, oplus_pps_comm_subs_callback, "pps");
...
static void oplus_pps_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_pps *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_pps_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_pps *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_pps_wired_subs_callback, "pps");
...
static void oplus_pps_cpa_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_pps *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_pps_subscribe_cpa_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_pps *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->cpa_topic = topic;
	chip->cpa_subs = oplus_mms_subscribe(chip->cpa_topic, chip, oplus_pps_cpa_subs_callback, "pps");
...
static void oplus_pps_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_pps *chip = subs->priv_data;
...
static void oplus_pps_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_pps *chip = prv_data;

	chip->gauge_topic = topic;
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_pps_gauge_subs_callback, "pps");
...
static int oplus_pps_update_online(struct oplus_mms *mms, union mms_msg_data *data)
{
...
static int oplus_pps_update_charging(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_pps *chip;
...
static int oplus_pps_update_adapter_id(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_pps *chip;
...
static int oplus_pps_update_oplus_adapter(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_pps *chip;
...
static int oplus_pps_topic_init(struct oplus_pps *chip)
{
	...
	chip->pps_topic = devm_oplus_mms_register(chip->dev, &oplus_pps_desc, &mms_cfg);
...
static int oplus_pps_curr_vote_callback(struct votable *votable, void *data, int curr_ma, const char *client, bool step)
{
...
static int oplus_pps_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client,
					   bool step)
{
	struct oplus_pps *chip = data;
...
static int oplus_pps_not_allow_vote_callback(struct votable *votable, void *data, int not_allow, const char *client,
					     bool step)
{
	struct oplus_pps *chip = data;
	union mms_msg_data mms_data = { 0 };
...
static int oplus_pps_boot_vote_callback(struct votable *votable, void *data, int boot, const char *client, bool step)
{
	struct oplus_pps *chip = data;
...
static int oplus_pps_parse_charge_strategy(struct oplus_pps *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,pps_current_change_timeout", &chip->pps_current_change_timeout);
	if (rc)
	...
	chg_info("pps_charge_strategy_soc num = %d, [%d, %d, %d, %d, %d, %d, "
		 "%d]\n",
		 chip->limits.pps_strategy_soc_num, soc_tmp[0], soc_tmp[1], soc_tmp[2], soc_tmp[3], soc_tmp[4],
		 soc_tmp[5], soc_tmp[6]);
...
static int oplus_pps_vote_init(struct oplus_pps *chip)
{
	...
	chip->pps_curr_votable = create_votable("PPS_CURR", VOTE_MIN, oplus_pps_curr_vote_callback, chip);
	...
	chip->pps_disable_votable = create_votable("PPS_DISABLE", VOTE_SET_ANY, oplus_pps_disable_vote_callback, chip);
	...
	chip->pps_boot_votable = create_votable("PPS_BOOT", VOTE_SET_ANY, oplus_pps_boot_vote_callback, chip);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_pps *chip = container_of(dwork, struct oplus_pps, imp_uint_init_work);
	static int retry_count = 0;
...
{
	struct oplus_pps *chip = container_of(filp->private_data, struct oplus_pps, misc_dev);
...
static ssize_t oplus_pps_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
...
static long oplus_pps_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct oplus_pps *chip = filp->private_data;
...
static ssize_t oplus_pps_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	return count;
}
