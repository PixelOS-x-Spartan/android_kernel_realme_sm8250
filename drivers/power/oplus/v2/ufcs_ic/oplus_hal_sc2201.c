...
__printf(3, 4) static int sc2201_publish_ic_err_msg(struct oplus_sc2201 *chip, int sub_type, const char *format, ...)
{
	...
	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, ERR_ITEM_IC, "[%s]-[%d]-[%d]:%s",
					    "ufcs-sc2201", OPLUS_IC_ERR_UFCS, sub_type, buf);
	kfree(buf);
	if (topic_msg == NULL) {
...
static void sc2201_push_i2c_err(struct oplus_sc2201 *chip, bool read, u16 addr, int err_code)
{
	...
	if (chip->err_topic != NULL)
		sc2201_publish_ic_err_msg(chip, read ? UFCS_ERR_READ : UFCS_ERR_WRITE, "%s 0x%04x error, err_code=%d",
...
static int sc2201_read_flags(struct oplus_sc2201 *chip)
{
	...
	if (chip->ufcs->handshake_state == UFCS_HS_WAIT) {
		if ((flag_buf[7] & SC2201_FLAG_HANDSHAKE_SUCCESS) && !(flag_buf[7] & SC2201_FLAG_HANDSHAKE_FAIL)) {
...
static int sc2201_dump_registers(struct oplus_sc2201 *chip)
{
	...
	chg_err(":[a~f][0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x]\n", val_buf[10], val_buf[11], val_buf[12], val_buf[13],
		val_buf[14], val_buf[15]);
...
static int sc2201_ufcs_enable(struct ufcs_dev *ufcs)
{
	...
	if (IS_ERR_OR_NULL(chip) || IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->ufcs_en_active)) {
...
{
	struct oplus_sc2201 *chip = container_of(work, struct oplus_sc2201, regdump_work);
	struct mms_msg *topic_msg;
	char *buf;
	int rc;
	int i;
	...
	for (i = 0; i < SC2201_FLAG_NUM; i++)
		index += snprintf(buf + index, ERR_MSG_BUF, "0x%04x=%02x,", (SC2201_ADDR_GENERAL_INT_FLAG + i),
				  chip->reg_dump[i]);
	if (index > 0)
		buf[index - 1] = 0;

	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, ERR_ITEM_IC, "[%s]-[%d]-[%d]:$$reg_info@@%s",
					    "ufcs-sc2201", OPLUS_IC_ERR_UFCS, UFCS_ERR_REG_DUMP, buf);
	kfree(buf);
	if (topic_msg == NULL) {
		chg_err("alloc topic msg error\n");
		return;
...
static void sc2201_err_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void sc2201_subscribe_error_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->err_subs = oplus_mms_subscribe(chip->err_topic, chip, sc2201_err_subs_callback, "sc2201");
