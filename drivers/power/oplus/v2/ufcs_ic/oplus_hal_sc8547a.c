...
__printf(3, 4) static int sc8547a_publish_ic_err_msg(struct sc8547a_device *chip, int sub_type, const char *format, ...)
{
	...
	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, ERR_ITEM_IC, "[%s]-[%d]-[%d]:%s",
					    "cp-sc8547a", OPLUS_IC_ERR_UFCS, sub_type, buf);
	kfree(buf);
	if (topic_msg == NULL) {
	...
	if (happen) {
		...
		if (voocphy->voocphy_iic_err_num >= I2C_ERR_NUM) {
			...
			if (chip->err_topic != NULL)
				sc8547a_publish_ic_err_msg(chip, read ? UFCS_ERR_READ : UFCS_ERR_WRITE, "%s error",
							   read ? "read" : "write");
...
static int sc8547a_read_data(struct sc8547a_device *chip, u8 addr, u8 *buf, int len)
{
	u8 addr_buf = addr & 0xff;
	int rc = 0;
...
static int sc8547a_write_data(struct sc8547a_device *chip, u8 addr, u16 length, u8 *data)
{
	u8 *buf;
	int rc = 0;
...
static int sc8547a_write_bit_mask(struct sc8547a_device *chip, u8 reg, u8 mask, u8 data)
{
	u8 temp = 0;
	int rc = 0;
...
static int sc8547_voocphy_get_voocphy_enable(struct oplus_voocphy_manager *chip, u8 *data)
{
	int ret = 0;
...
static int sc8547_voocphy_set_chg_enable(struct oplus_voocphy_manager *chip, bool enable)
{
	if (!chip) {
		chg_err("Failed\n");
		return -1;
...
static void sc8547_voocphy_set_pd_svooc_config(struct oplus_voocphy_manager *chip, bool enable)
{
	int ret = 0;
	u8 reg_data = 0;
	if (!chip) {
...
static int sc8547_voocphy_set_adc_enable(struct oplus_voocphy_manager *chip, bool enable)
{
	if (!chip) {
		chg_err("Failed\n");
		return -1;
...
static bool sc8547_voocphy_check_cp_int_happened(struct oplus_voocphy_manager *chip, bool *dump_reg, bool *send_info)
{
	int i = 0;
	...
	for (i = 0; i < IRQ_EVNET_NUM; i++) {
		if ((int_flag[i].mask & chip->interrupt_flag) && int_flag[i].mark_except) {
			...
			if (int_flag[i].mask != VOUT_OVP_FLAG_MASK && int_flag[i].mask != ADAPTER_INSERT_FLAG_MASK &&
...
static int sc8547a_retrieve_reg_flags(struct sc8547a_device *chip)
{
	...
	if (chip->ufcs->handshake_state == UFCS_HS_WAIT) {
		if ((flag_buf[1] & SC8547A_FLAG_HANDSHAKE_SUCCESS) && !(flag_buf[1] & SC8547A_FLAG_HANDSHAKE_FAIL)) {
...
static int sc8547a_ufcs_write_msg(struct ufcs_dev *ufcs, unsigned char *buf, int len)
{
	...
	rc = sc8547a_write_bit_mask(chip, SC8547A_ADDR_UFCS_CTRL0, SC8547A_MASK_SND_CMP, SC8547A_CMD_SND_CMP);
	if (rc < 0) {
...
retry:
	...
	rc = sc8547a_write_bit_mask(chip, SC8547A_ADDR_UFCS_CTRL0, SEND_SOURCE_HARDRESET, SEND_SOURCE_HARDRESET);
	if (rc < 0) {
...
static int sc8547a_ufcs_set_baud_rate(struct ufcs_dev *ufcs, enum ufcs_baud_rate baud)
{
	...
	rc = sc8547a_write_bit_mask(chip, SC8547A_ADDR_UFCS_CTRL0, FLAG_BAUD_RATE_VALUE, (baud << FLAG_BAUD_NUM_SHIFT));
	if (rc < 0)
...
static int sc8547a_ufcs_disable(struct ufcs_dev *ufcs)
{
	...
	chip->rested = false;
	rc = sc8547a_write_byte(chip, SC8547A_ADDR_UFCS_CTRL0, SC8547A_CMD_DIS_CHIP);
	if (rc < 0) {
		chg_err("write i2c failed\n");
		return rc;
	}
...
static int sc8547_irq_register(struct sc8547a_device *chip)
{
	...
	if (voocphy->irq) {
		ret = request_threaded_irq(voocphy->irq, NULL, sc8547_interrupt_handler,
					   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "voocphy_irq", chip);
		if (ret < 0) {
...
{
	struct sc8547a_device *chip = container_of(work, struct sc8547a_device, ufcs_regdump_work);
	struct mms_msg *topic_msg;
	char *buf;
	int rc;
	int i;
	...
	for (i = 0; i < SC8547A_FLAG_NUM; i++)
		index += snprintf(buf + index, ERR_MSG_BUF, "0x%04x=%02x,", (SC8547A_ADDR_GENERAL_INT_FLAG1 + i),
				  chip->ufcs_reg_dump[i]);
	if (index > 0)
		buf[index - 1] = 0;

	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, ERR_ITEM_IC, "[%s]-[%d]-[%d]:$$reg_info@@%s",
					    "ufcs-sc8547a", OPLUS_IC_ERR_UFCS, UFCS_ERR_REG_DUMP, buf);
	kfree(buf);
	if (topic_msg == NULL) {
		chg_err("alloc topic msg error\n");
		return;
...
static void sc8547a_err_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void sc8547a_subscribe_error_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->err_subs = oplus_mms_subscribe(chip->err_topic, chip, sc8547a_err_subs_callback, "sc8547a");
