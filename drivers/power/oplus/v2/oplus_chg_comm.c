...
struct oplus_comm_spec_config {
	...
	int32_t wired_ffc_fv_cutoff_mv[FFC_CHG_STEP_MAX][FFC_TEMP_REGION_MAX - 2];
	int32_t wired_ffc_fcc_ma[FFC_CHG_STEP_MAX][FFC_TEMP_REGION_MAX - 2];
	int32_t wired_ffc_fcc_cutoff_ma[FFC_CHG_STEP_MAX][FFC_TEMP_REGION_MAX - 2];
	int32_t wired_ffc_fcc_sub_cutoff_ma[FFC_CHG_STEP_MAX][FFC_TEMP_REGION_MAX - 2];
...
static const char *const oplus_comm_ffc_temp_region_text[] = {
	[FFC_TEMP_REGION_COOL] = "cool",     [FFC_TEMP_REGION_PRE_NORMAL] = "pre-normal",
	[FFC_TEMP_REGION_NORMAL] = "normal", [FFC_TEMP_REGION_WARM] = "warm",
...
const char *oplus_comm_get_ffc_temp_region_str(enum oplus_ffc_temp_region temp_region)
{
...
__maybe_unused static bool is_wired_charging_disable_votable_available(struct oplus_chg_comm *chip)
{
	if (!chip->wired_charging_disable_votable)
		chip->wired_charging_disable_votable = find_votable("WIRED_CHARGING_DISABLE");
	return !!chip->wired_charging_disable_votable;
}

__maybe_unused static bool is_wired_charge_suspend_votable_available(struct oplus_chg_comm *chip)
{
	if (!chip->wired_charge_suspend_votable)
		chip->wired_charge_suspend_votable = find_votable("WIRED_CHARGE_SUSPEND");
	return !!chip->wired_charge_suspend_votable;
}

__maybe_unused static bool is_wls_charging_disable_votable_available(struct oplus_chg_comm *chip)
{
	if (!chip->wls_charging_disable_votable)
		chip->wls_charging_disable_votable = find_votable("WLS_NOR_OUT_DISABLE");
	return !!chip->wls_charging_disable_votable;
}

__maybe_unused static bool is_wls_charge_suspend_votable_available(struct oplus_chg_comm *chip)
{
	if (!chip->wls_charge_suspend_votable)
		chip->wls_charge_suspend_votable = find_votable("WLS_CHARGE_SUSPEND");
...
static enum oplus_temp_region oplus_comm_get_temp_region(struct oplus_chg_comm *chip)
{
	int temp;
...
static void oplus_comm_temp_thr_update(struct oplus_chg_comm *chip, enum oplus_temp_region now_temp_region,
				       enum oplus_temp_region pre_temp_region)
{
	...
	if ((pre_temp_region > now_temp_region) && (pre_temp_region - now_temp_region == 1) &&
	    (now_temp_region >= TEMP_REGION_COLD) && (now_temp_region < TEMP_REGION_WARM)) {
		if (pre_temp_region < TEMP_REGION_MAX - 1) {
			...
			chg_info("now_temp_region=%d, pre_temp_region=%d, update the current batt_temp_thr[%d] to %d\n",
				 now_temp_region, pre_temp_region, now_temp_region,
				 chip->batt_temp_dynamic_thr[now_temp_region]);
		} else {
			chg_err("now_temp_region=%d is out of the batt_temp_dynamic_thr\n", now_temp_region);
		}
	} else if ((pre_temp_region < now_temp_region) && (now_temp_region - pre_temp_region == 1) &&
		   (now_temp_region > TEMP_REGION_LITTLE_COLD) && (now_temp_region < TEMP_REGION_MAX)) {
		if (pre_temp_region < TEMP_REGION_MAX - 1) {
			...
			chg_info("now_temp_region=%d, pre_temp_region = %d, update the lower batt_temp_thr[%d] to %d\n",
				 now_temp_region, pre_temp_region, pre_temp_region,
				 chip->batt_temp_dynamic_thr[pre_temp_region]);
		} else {
	...
	chg_debug("temp_region: %d %d, shell_temp: %d, temp_thr: %d %d %d %d %d %d %d\n", now_temp_region,
		  pre_temp_region, chip->shell_temp, chip->batt_temp_dynamic_thr[0], chip->batt_temp_dynamic_thr[1],
		  chip->batt_temp_dynamic_thr[2], chip->batt_temp_dynamic_thr[3], chip->batt_temp_dynamic_thr[4],
		  chip->batt_temp_dynamic_thr[5], chip->batt_temp_dynamic_thr[6]);
}

static enum oplus_ffc_temp_region oplus_comm_get_ffc_temp_region(struct oplus_chg_comm *chip)
{
	int temp;
...
static void oplus_comm_ffc_temp_thr_init(struct oplus_chg_comm *chip, enum oplus_ffc_temp_region ffc_temp_region)
{
	...
	for (i = 0; i < FFC_TEMP_REGION_MAX - 1; i++) {
		if (i == ffc_temp_region - 1 && ffc_temp_region > FFC_TEMP_REGION_NORMAL)
			chip->ffc_temp_dynamic_thr[i] = spec->ffc_temp_thr[i] - BATT_TEMP_HYST;
	...
	for (i = 1; i < FFC_TEMP_REGION_MAX - 1; i++) {
		if (chip->ffc_temp_dynamic_thr[i] < chip->ffc_temp_dynamic_thr[i - 1])
			chip->ffc_temp_dynamic_thr[i] = chip->ffc_temp_dynamic_thr[i - 1];
	}
}
...
static void oplus_comm_check_temp_region(struct oplus_chg_comm *chip)
{
	...
	if (temp_region != chip->temp_region) {
		if (chip->temp_region == TEMP_REGION_WARM && temp_region < TEMP_REGION_WARM) {
			if (chip->soc != NORMAL_FULL_SOC && chip->batt_full &&
				...
				if (chip->wls_online) {
					if (is_wls_charging_disable_votable_available(chip))
						vote(chip->wls_charging_disable_votable, CHG_FULL_VOTER, false, 0,
						     false);
					else
						chg_err("wls_charging_disable_votable not found, can't enable charging");
				} else {
					if (is_wired_charging_disable_votable_available(chip))
						vote(chip->wired_charging_disable_votable, CHG_FULL_VOTER, false, 0,
						     false);
					else
	...
	if (chip->wired_online || chip->wls_online) {
		vote(chip->fv_max_votable, SPEC_VOTER, true, spec->fv_mv[temp_region], false);
		if (chip->fv_over) {
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_TEMP_REGION);
	if (msg == NULL) {
...
static void oplus_comm_check_fcc_gear(struct oplus_chg_comm *chip)
{
	...
	if (chip->vbat_mv <
	    ((chip->fcc_gear == FCC_GEAR_LOW) ? chip->spec.fcc_gear_thr_mv : (chip->spec.fcc_gear_thr_mv - 100)))
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_FCC_GEAR);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		goto out;
	}
...
static void oplus_comm_set_rechging(struct oplus_chg_comm *chip, bool rechging)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_RECHGING);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_set_batt_full(struct oplus_chg_comm *chip, bool full)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_CHG_FULL);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_set_sub_batt_full(struct oplus_chg_comm *chip, bool full)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_CHG_SUB_BATT_FULL);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_check_sw_full(struct oplus_chg_comm *chip)
{
	...
	if (chip->vooc_by_normal_path) {
		if (chip->vooc_charging && sid_to_adapter_chg_type(chip->vooc_sid) != CHARGER_TYPE_VOOC)
			return;
	} else {
...
static void oplus_comm_check_hw_full(struct oplus_chg_comm *chip)
{
	...
	hw_fv_mv = spec->fv_mv[chip->temp_region] + spec->hw_fv_inc_mv[chip->temp_region];
	if (!chip->authenticate || !chip->hmac)
		hw_fv_mv = spec->non_standard_fv_mv + spec->non_standard_hw_fv_inc_mv;
	if (hw_fv_mv <= 0)
		goto clean;
...
static void oplus_comm_check_rechg(struct oplus_chg_comm *chip)
{
	...
	if (chip->rechg_count > RECHG_COUNT_MAX) {
		...
		if (spec->max_chg_time_sec > 0) {
			schedule_delayed_work(&chip->charge_timeout_work,
					      msecs_to_jiffies(spec->max_chg_time_sec * 1000));
		}
	}
}
...
static void oplus_comm_check_vbatt_is_good(struct oplus_chg_comm *chip)
{
	...
	if (chip->vbat_mv >= ov_thr && !chip->vbatt_over) {
		...
		if (vbat_counts >= VBAT_CNT) {
			...
			vote(chip->chg_disable_votable, UOVP_VOTER, true, 1, false);
		}
...
static void oplus_comm_charge_timeout_work(struct work_struct *work)
{
	...
	struct oplus_chg_comm *chip = container_of(dwork, struct oplus_chg_comm, charge_timeout_work);
...
static bool oplus_comm_is_not_charging(struct oplus_chg_comm *chip)
{
	...
	if (chip->wired_online && is_wired_charging_disable_votable_available(chip)) {
		if (get_client_vote(chip->wired_charging_disable_votable, BATT_TEMP_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wired_charging_disable_votable, DEF_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wired_charging_disable_votable, UOVP_VOTER) > 0)
			return true;
	} else if (chip->wls_online && is_wls_charging_disable_votable_available(chip)) {
		if (get_client_vote(chip->wls_charging_disable_votable, BATT_TEMP_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wls_charging_disable_votable, DEF_VOTER) > 0)
			return true;
	}
...
static bool oplus_comm_is_discharging(struct oplus_chg_comm *chip)
{
	if (get_effective_result(chip->chg_suspend_votable) != 0)
		return true;
	if (chip->wired_online && is_wired_charge_suspend_votable_available(chip)) {
		if (get_client_vote(chip->wired_charge_suspend_votable, UOVP_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wired_charge_suspend_votable, WLAN_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wired_charge_suspend_votable, DEF_VOTER) > 0)
			return true;
	} else if (chip->wls_online && is_wls_charge_suspend_votable_available(chip)) {
		if (get_client_vote(chip->wls_charge_suspend_votable, UOVP_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wls_charge_suspend_votable, WLAN_VOTER) > 0)
			return true;
		if (get_client_vote(chip->wls_charge_suspend_votable, DEF_VOTER) > 0)
			return true;
	}
...
static void oplus_comm_check_battery_status(struct oplus_chg_comm *chip)
{
	...
	if (chip->batt_full && mmi_chg) {
		if ((chip->temp_region > TEMP_REGION_COLD) && (chip->temp_region < TEMP_REGION_WARM) && chip->hmac) {
			if (chip->ui_soc == 100) {
...
check_done:
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_BATT_STATUS);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_check_battery_health(struct oplus_chg_comm *chip)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_BATT_HEALTH);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
check_done:
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_BATT_CHG_TYPE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static int oplus_comm_push_ui_soc_shutdown_msg(struct oplus_chg_comm *chip)
{
	...
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, ERR_ITEM_UI_SOC_SHUTDOWN, 0);
	if (msg == NULL) {
...
static int oplus_comm_set_ui_soc(struct oplus_chg_comm *chip, int soc)
{
	...
	chip->ui_soc = soc;
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_UI_SOC);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_set_smooth_soc(struct oplus_chg_comm *chip, int soc)
{
	...
	chip->smooth_soc = soc;
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_SMOOTH_SOC);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_set_vbat_uv_thr(struct oplus_chg_comm *chip, int uv_thr)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_VBAT_UV_THR);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_push_vbat_too_low_msg(struct oplus_chg_comm *chip)
{
	...
	msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_VBAT_TOO_LOW,
				      "$$soc@@%d$$smooth_soc@@%d$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d", chip->soc,
				      chip->smooth_soc, chip->ui_soc, chip->vbat_mv, chip->vbat_min_mv);
	if (msg == NULL) {
...
static void oplus_comm_ui_soc_update(struct oplus_chg_comm *chip)
{
	...
	if (ui_soc == 100) {
		soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(300 * HZ);
	} else if (ui_soc >= 95) {
		soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(150 * HZ);
	} else if (ui_soc >= 60) {
		soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(60 * HZ);
	} else if (charging && ui_soc == 1) {
		soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(90 * HZ);
	} else {
		soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(40 * HZ);
	}

	if (chip->vbat_min_mv < (charging ? spec->vbat_charging_uv_thr_mv : spec->vbat_uv_thr_mv) ||
	    (is_support_parallel_battery(chip->gauge_topic) && spec->sub_vbat_charging_uv_thr_mv > 0 &&
	     spec->sub_vbat_uv_thr_mv > 0 && chip->sub_vbat_mv > 2500 &&
	     chip->sub_vbat_mv < (charging ? spec->sub_vbat_charging_uv_thr_mv : spec->sub_vbat_uv_thr_mv))) {
		if (charging)
			force_dec_interval = CHARGE_FORCE_DEC_INTERVAL;
		else
			force_dec_interval = NON_CHARGE_FORCE_DEC_INTERVAL;

		chg_err("[%d %d %d %d %d %d]\n", spec->vbat_charging_uv_thr_mv, spec->vbat_uv_thr_mv, charging,
			chip->vbat_min_mv, chip->vbat_mv, force_dec_interval);

		vbat_uv_jiffies = chip->vbat_uv_jiffies + (unsigned long)(force_dec_interval * HZ);
		/* Force ui_soc to drop to 0 when the voltage is too low */
		if (time_is_before_jiffies(vbat_uv_jiffies)) {
			soc_down_jiffies = chip->soc_update_jiffies + (unsigned long)(15 * HZ);
			force_down = true;
		} else {
			force_down = false;
		}
	...
	if (chip->sleep_tm_sec > 0 && ui_soc > smooth_soc && !(chip->batt_full)) {
		ui_soc_update = true;
	...
	if (charging) {
		if (ui_soc < smooth_soc && time_is_before_jiffies(soc_up_jiffies)) {
			ui_soc = (ui_soc < 100) ? (ui_soc + 1) : 100;
			chip->sleep_tm_sec = 0;
			chip->save_sleep_tm_sec = 0;
		} else if (ui_soc < 100 && (chip->sw_full || chip->hw_full_by_sw) &&
			   (chip->temp_region < TEMP_REGION_WARM) && (chip->hmac && chip->authenticate) &&
			   time_is_before_jiffies(soc_up_jiffies)) {
			ui_soc = (ui_soc < 100) ? (ui_soc + 1) : 100;
			chip->sleep_tm_sec = 0;
			chip->save_sleep_tm_sec = 0;
		} else if (ui_soc > (smooth_soc + MIN_DELTA_SOC) && !(chip->sw_full || chip->hw_full_by_sw) &&
			   time_is_before_jiffies(soc_down_jiffies)) {
			ui_soc = (ui_soc > 1) ? (ui_soc - 1) : 1;
			chip->save_sleep_tm_sec = 0;
		}
	} else {
		if (ui_soc > smooth_soc && time_is_before_jiffies(soc_down_jiffies)) {
			ui_soc = (ui_soc > 1) ? (ui_soc - 1) : 1;
			chip->save_sleep_tm_sec = 0;
		}
	}
...
check_update_time:
	...
	if (chip->vooc_online && config->vooc_show_ui_soc_decimal && soc_decimal->decimal_control &&
	    ui_soc < chip->ui_soc)
		ui_soc = chip->ui_soc;
	...
	if (chip->ui_soc != ui_soc) {
		...
		if (chip->vooc_by_normal_path && chip->vooc_charging &&
		...
		if (!chip->batt_full && ui_soc == 100 && charging &&
		    ...
		    (!chip->vooc_charging || vooc_by_normalpath_chg) && mmi_chg && !chip->ufcs_charging &&
		    !is_wls_fastchg_started(chip)) {
			tmp = chip->batt_full_jiffies + (unsigned long)(60 * HZ);
		...
		} else if (ui_soc < 100 && chip->batt_full && !chip->sw_full && !chip->hw_full_by_sw) {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip = container_of(dwork, struct oplus_chg_comm, ui_soc_update_work);
	...
	if (chip->ui_soc == 100) {
		...
		if ((soc_decimal->ui_soc_integer + soc_decimal->ui_soc_decimal) > soc_decimal->last_decimal_ui_soc &&
			...
			soc_decimal->ui_soc_decimal = ((soc_decimal->last_decimal_ui_soc % 1000 - 50) > 0) ?
							      (soc_decimal->last_decimal_ui_soc % 1000 - 50) :
							      0;
		}
	}
...
static void oplus_comm_set_ffc_step(struct oplus_chg_comm *chip, int step)
{
	...
	chg_info("ffc_step=%d\n", step);
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_FFC_STEP);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_set_ffc_status(struct oplus_chg_comm *chip, enum oplus_chg_ffc_status ffc_status)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_FFC_STATUS);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip = container_of(dwork, struct oplus_chg_comm, ffc_start_work);
	struct oplus_comm_spec_config *spec = &chip->spec;
	enum oplus_ffc_temp_region ffc_temp_region;
	...
	if (chip->wired_online) {
		...
		vote(chip->fv_max_votable, FFC_VOTER, true, spec->wired_ffc_fv_mv[chip->ffc_step], false);
		...
		vote(chip->wls_icl_votable, FFC_VOTER, true, spec->wls_ffc_icl_ma[chip->ffc_step][ffc_temp_region - 1],
		     true);
		vote(chip->fv_max_votable, FFC_VOTER, true, spec->wls_ffc_fv_mv[chip->ffc_step], false);
		vote(chip->wls_fcc_votable, FFC_VOTER, true, spec->wls_ffc_fcc_ma[chip->ffc_step][ffc_temp_region - 1],
		     false);
	} else {
		chg_err("wired and wireless charge is offline\n");
		goto err;
...
static int oplus_comm_check_sub_vbat_ffc_end(struct oplus_chg_comm *chip, int step, int ffc_temp_region)
{
	struct oplus_comm_spec_config *spec = &chip->spec;
	int fv_max_mv;
...
static int oplus_comm_check_sub_ibat_ffc_end(struct oplus_chg_comm *chip, int step, int ffc_temp_region)
{
	struct oplus_comm_spec_config *spec = &chip->spec;
	int cutoff_ma;
...
static void oplus_comm_check_ffc(struct oplus_chg_comm *chip)
{
	...
	case FFC_FAST:
		...
		if (is_support_parallel_battery(chip->gauge_topic)) {
			sub_vbat_ffc_end_status =
				oplus_comm_check_sub_vbat_ffc_end(chip, chip->ffc_step, ffc_temp_region);
			sub_ibat_ffc_end_status =
				oplus_comm_check_sub_ibat_ffc_end(chip, chip->ffc_step, ffc_temp_region);
		}
...
static void oplus_comm_check_fgreset(struct oplus_chg_comm *chip)
{
	...
	if (oplus_wired_get_chg_type() == OPLUS_CHG_USB_TYPE_UNKNOWN ||
	    oplus_wired_get_chg_type() == OPLUS_CHG_USB_TYPE_SDP || chip->temp_region != TEMP_REGION_NORMAL ||
	    chip->vbat_min_mv < SOFT_REST_VOL_THRESHOLD || chip->fg_soft_reset_done)
		is_need_check = false;
...
static int oplus_comm_charging_disable(struct oplus_chg_comm *chip, bool en)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_CHARGING_DISABLE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_charge_suspend(struct oplus_chg_comm *chip, bool en)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_CHARGE_SUSPEND);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_set_cool_down_level(struct oplus_chg_comm *chip, int level)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_COOL_DOWN);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static void oplus_comm_battery_notify_tbat_check(struct oplus_chg_comm *chip, unsigned int *notify_code)
{
		...
		if (chip->batt_temp < chip->spec.removed_bat_decidegc) {
			...
			*notify_code |= BIT(NOTIFY_BAT_NOT_CONNECT);
			chg_err("bat_temp(%d) < %d mC, Battery does not exist\n", chip->batt_temp,
				chip->spec.removed_bat_decidegc);
		} else {
...
static void oplus_comm_battery_notify_vbat_check(struct oplus_chg_comm *chip, unsigned int *notify_code)
{
	static int count = 0;
	...
	if (chip->vbatt_over) {
		...
		count = 0;
		if (chip->batt_full) {
			if (chip->temp_region == TEMP_REGION_WARM && chip->ui_soc != 100) {
				*notify_code |= BIT(NOTIFY_BAT_FULL_PRE_HIGH_TEMP);
			} else if ((chip->temp_region == TEMP_REGION_COLD) && (chip->ui_soc != 100)) {
				*notify_code |= BIT(NOTIFY_BAT_FULL_PRE_LOW_TEMP);
	...
	*/
	if (pre_notify_code & BIT(NOTIFY_BAT_FULL_PRE_HIGH_TEMP) &&
	    !(*notify_code & BIT(NOTIFY_BAT_FULL_PRE_HIGH_TEMP))) {
		if (!pre_high_recov)
			pre_high_recov_jiffies = jiffies + (unsigned long)(PRE_HIGH_TEMP_RECOVERY_TIME * HZ);
		pre_high_recov = true;
...
static int oplus_comm_set_notify_code(struct oplus_chg_comm *chip, unsigned int notify_code)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_NOTIFY_CODE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_comm_set_notify_flag(struct oplus_chg_comm *chip, unsigned int notify_flag)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_NOTIFY_FLAG);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static void oplus_comm_battery_notify_check(struct oplus_chg_comm *chip)
{
	...
	if (notify_code & (BIT(NOTIFY_BAT_NOT_CONNECT) | BIT(NOTIFY_GAUGE_I2C_ERR)))
		notify_code &= ~BIT(NOTIFY_BAT_LOW_TEMP);
	...
	if (is_parallel_topic_available(chip)) {
		oplus_mms_get_item_data(chip->parallel_topic, SWITCH_ITEM_ERR_NOTIFY, &data, true);
		if (data.intval)
...
static bool oplus_comm_override_by_shell_temp(struct oplus_chg_comm *chip, int temp)
{
	struct oplus_comm_spec_config *spec = &chip->spec;
	int batt_temp_thr;
	...
	batt_temp_thr = min(spec->batt_temp_thr[ARRAY_SIZE(spec->batt_temp_thr) - 1], BATT_NTC_CTRL_THRESHOLD_HIGH);
	if (chip->wired_online && (temp > BATT_NTC_CTRL_THRESHOLD_LOW) && (temp < batt_temp_thr))
		return true;
...
static void oplus_comm_check_shell_temp(struct oplus_chg_comm *chip, bool update)
{
	...
	if (chip->shell_themal == NULL) {
		shell_themal = thermal_zone_get_zone_by_name("shell_back");
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_SHELL_TEMP);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, gauge_check_work);
	union mms_msg_data data = { 0 };
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MIN, &data, false);
	...
	if (is_support_parallel_battery(chip->gauge_topic)) {
		if (is_main_gauge_topic_available(chip)) {
			...
			chg_info("main_vbat_mv=%d, main_ibat_ma=%d, main_batt_temp=%d, main_soc=%d\n",
				 chip->main_vbat_mv, chip->main_ibat_ma, chip->main_batt_temp, chip->main_soc);
		}
		if (is_sub_gauge_topic_available(chip)) {
			...
			chg_info("sub_vbat_mv=%d, sub_ibat_ma=%d, sub_batt_temp=%d, sub_soc=%d\n", chip->sub_vbat_mv,
				 chip->sub_ibat_ma, chip->sub_batt_temp, chip->sub_soc);
		}
	...
	if (time_after(jiffies, count_time)) {
		...
		if (chip->config.gauge_stuck_jump_support &&
		    (get_eng_version() == AGING || get_eng_version() == HIGH_TEMP_AGING ||
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, gauge_remuse_work);
	union mms_msg_data data = { 0 };
...
static void oplus_comm_get_vol(struct oplus_chg_comm *chip)
{
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MIN, &data, true);
	chip->vbat_min_mv = data.intval;
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, noplug_batt_volt_work);
	int vbat_mv = 0;
	int vbat_min_mv = 0;
	int vbat_grap, i;
...
static void oplus_comm_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_comm_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(topic, chip, oplus_comm_gauge_subs_callback, "chg_comm");
	if (IS_ERR_OR_NULL(chip->gauge_subs)) {
		chg_err("subscribe gauge topic error, rc=%ld\n", PTR_ERR(chip->gauge_subs));
		return;
	}

	oplus_mms_get_item_data(topic, GAUGE_ITEM_VOL_MAX, &data, true);
	chip->vbat_mv = data.intval;
	if (chip->vbat_mv < chip->spec.fcc_gear_thr_mv)
		chip->fcc_gear = FCC_GEAR_LOW;
	else
		chip->fcc_gear = FCC_GEAR_HIGH;
	oplus_mms_get_item_data(topic, GAUGE_ITEM_VOL_MIN, &data, true);
	chip->vbat_min_mv = data.intval;
	oplus_mms_get_item_data(topic, GAUGE_ITEM_CURR, &data, true);
	chip->ibat_ma = data.intval;
	oplus_mms_get_item_data(topic, GAUGE_ITEM_TEMP, &data, true);
	chip->batt_temp = data.intval;
	oplus_mms_get_item_data(topic, GAUGE_ITEM_SOC, &data, true);
	chip->soc = data.intval;
	oplus_mms_get_item_data(topic, GAUGE_ITEM_CC, &data, true);
	chip->batt_cc = data.intval;
	rc = oplus_mms_get_item_data(topic, GAUGE_ITEM_HMAC, &data, true);
	if (rc < 0) {
		chg_err("can't get GAUGE_ITEM_HMAC data, rc=%d\n", rc);
		chip->hmac = false;
	} else {
	...
	rc = oplus_mms_get_item_data(topic, GAUGE_ITEM_AUTH, &data, true);
	if (rc < 0) {
		chg_err("can't get GAUGE_ITEM_AUTH data, rc=%d\n", rc);
	...
	rc = oplus_mms_get_item_data(topic, GAUGE_ITEM_BATT_EXIST, &data, true);
	if (rc < 0)
		chip->batt_exist = false;
	else
		chip->batt_exist = data.intval;
	rc = oplus_mms_get_item_data(topic, GAUGE_ITEM_ERR_CODE, &data, true);
	if (rc < 0)
		chip->gauge_err_code = 0;
	else
		chip->gauge_err_code = data.intval;

	rc = oplus_mms_get_item_data(topic, GAUGE_ITEM_VBAT_UV, &data, false);
	if (rc < 0) {
		chg_err("can't get GAUGE_ITEM_VBAT_UV data, rc=%d\n", rc);
	} else {
		if (data.intval > GAUGE_VBAT_UV_DELATA) {
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (get_eng_version() == HIGH_TEMP_AGING || get_eng_version() == AGING || get_eng_version() == FACTORY) {
		chg_info("HIGH_TEMP_AGING/AGING/FACTORY, disable chg timeout\n");
		chip->spec.max_chg_time_sec = -1;
	} else {
#endif
		chip->spec.max_chg_time_sec = MAH_TO_MAX_CHARGE_TIME(oplus_gauge_get_batt_capacity_mah(topic));
	...
	if ((chip->wired_online || chip->wls_online) && chip->spec.max_chg_time_sec > 0) {
		schedule_delayed_work(&chip->charge_timeout_work, msecs_to_jiffies(chip->spec.max_chg_time_sec * 1000));
	}
	...
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, COMM_ITEM_SHUTDOWN_SOC, chip->shutdown_soc);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_comm *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_comm_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_comm_wired_subs_callback, "chg_comm");
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_ERR_CODE, &data, false);
	chip->wired_err_code = data.intval;
	if (!chip->wired_online)
		schedule_work(&chip->plugin_work);
}

static void oplus_comm_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_comm *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	struct ui_soc_decimal *soc_decimal = &chip->soc_decimal;
	...
	case MSG_TYPE_ITEM:
		...
		case VOOC_ITEM_ONLINE:
			...
			if (!chip->vooc_online && soc_decimal->calculate_decimal_time != 0) {
...
static void oplus_comm_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_comm *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_comm_vooc_subs_callback, "chg_comm");
	if (IS_ERR_OR_NULL(chip->vooc_subs)) {
		chg_err("subscribe vooc topic error, rc=%ld\n", PTR_ERR(chip->vooc_subs));
		return;
	}

	oplus_mms_get_item_data(chip->vooc_topic, VOOC_ITEM_VOOC_CHARGING, &data, true);
	chip->vooc_charging = data.intval;
	oplus_mms_get_item_data(chip->vooc_topic, VOOC_ITEM_ONLINE, &data, true);
	chip->vooc_online = data.intval;
	oplus_mms_get_item_data(chip->vooc_topic, VOOC_ITEM_ONLINE_KEEP, &data, true);
...
static void oplus_comm_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_comm *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_comm_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_comm *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_comm_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->ufcs_subs = oplus_mms_subscribe(topic, chip, oplus_comm_ufcs_subs_callback, "chg_comm");
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, plugin_work);
	struct oplus_comm_spec_config *spec = &chip->spec;
	struct ui_soc_decimal *soc_decimal = &chip->soc_decimal;
	union mms_msg_data data = { 0 };
	...
	if (chip->wired_online || chip->wls_online) {
		...
		chip->fv_over = false;
		vote(chip->fv_min_votable, OVER_FV_VOTER, false, 0, false);
		vote(chip->fv_max_votable, SPEC_VOTER, true, chip->spec.fv_mv[chip->temp_region], false);
		...
		if (is_wired_fcc_votable_available(chip)) {
			vote(chip->wired_fcc_votable, FFC_VOTER, false, 0, false);
		}
		if (is_wls_charging_disable_votable_available(chip))
			vote(chip->wls_charging_disable_votable, CHG_FULL_VOTER, false, 0, false);
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, chg_type_change_work);
...
static void oplus_comm_set_unwakelock(struct oplus_chg_comm *chip, bool en)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_UNWAKELOCK);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_set_power_save(struct oplus_chg_comm *chip, bool en)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_POWER_SAVE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_comm_set_chg_cycle_status(struct oplus_chg_comm *chip, int status)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_CHG_CYCLE_STATUS);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static int oplus_comm_update_temp_region(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_fcc_gear(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_chg_full(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_chg_sub_batt_full(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_ffc_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_chg_disable(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_chg_suspend(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_cool_down(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_batt_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_batt_health(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_batt_chg_type(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_ui_soc(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_vbat_uv_thr(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
	struct oplus_comm_spec_config *spec;
...
static int oplus_comm_update_notify_code(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_notify_flag(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_shell_temp(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
	...
	if (chip->gauge_topic != NULL) {
		...
		oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_TEMP, &tmp, false);
...
static int oplus_comm_update_factory_test(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
	int factory_test = 0;
...
static int oplus_comm_update_led_on(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
	bool led_on = false;
...
static int oplus_comm_update_unwakelock(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_power_save(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_rechging(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_ffc_step(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_smooth_soc(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_update_chg_cycle_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_chg_comm *chip;
...
static int oplus_comm_topic_init(struct oplus_chg_comm *chip)
{
	...
	chip->comm_topic = devm_oplus_mms_register(chip->dev, &oplus_comm_desc, &mms_cfg);
...
int read_signed_data_from_node(struct device_node *node, const char *prop_str, s32 *addr, int len_max)
{
	int rc = 0, length;
...
int read_unsigned_data_from_node(struct device_node *node, const char *prop_str, u32 *addr, int len_max)
{
	int rc = 0, length;
...
static void oplus_comm_parse_aging_ffc_dt(struct oplus_chg_comm *comm_dev)
{
	...
	rc = of_property_read_u32(node, "oplus_spec,wired-aging-ffc-version", &spec->wired_aging_ffc_version);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-aging-ffc-cycle-thr",
					  (u32 *)spec->wired_aging_ffc_cycle_thr, AGAIN_FFC_CYCLY_THR_COUNT);
	if (rc < 0) {
...
static int oplus_comm_parse_dt(struct oplus_chg_comm *comm_dev)
{
	...
	rc = read_signed_data_from_node(node, "oplus_spec,batt-them-thr", (s32 *)spec->batt_temp_thr,
					TEMP_REGION_MAX - 1);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,fv-mv", (u32 *)spec->fv_mv, TEMP_REGION_MAX);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,sw-fv-mv", (u32 *)spec->sw_fv_mv, TEMP_REGION_MAX);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,hw-fv-inc-mv", (u32 *)spec->hw_fv_inc_mv, TEMP_REGION_MAX);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,sw-over-fv-mv", (u32 *)spec->sw_over_fv_mv,
					  TEMP_REGION_MAX);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus_spec,sw-over-fv-dec-mv", &spec->sw_over_fv_dec_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,sw-over-fv-dec-mv property error, rc=%d\n", rc);
		spec->sw_over_fv_dec_mv = default_spec.sw_over_fv_dec_mv;
	}

	rc = of_property_read_u32(node, "oplus_spec,non-standard-sw-fv-mv", &spec->non_standard_sw_fv_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,non-standard-sw-fv-mv property error, rc=%d\n", rc);
		spec->non_standard_sw_fv_mv = default_spec.non_standard_sw_fv_mv;
	}
	rc = of_property_read_u32(node, "oplus_spec,non-standard-fv-mv", &spec->non_standard_fv_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,non-standard-fv-mv property error, rc=%d\n", rc);
		spec->non_standard_fv_mv = default_spec.non_standard_fv_mv;
	}
	rc = of_property_read_u32(node, "oplus_spec,non-standard-hw-fv-inc-mv", &spec->non_standard_hw_fv_inc_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,non-standard-hw-fv-inc-mv property error, rc=%d\n", rc);
		spec->non_standard_hw_fv_inc_mv = default_spec.non_standard_hw_fv_inc_mv;
	}
	rc = of_property_read_u32(node, "oplus_spec,non-standard-sw-over-fv-mv", &spec->non_standard_sw_over_fv_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,non-standard-sw-over-fv-mv property error, rc=%d\n", rc);
		spec->non_standard_sw_over_fv_mv = default_spec.non_standard_sw_over_fv_mv;
	}
	rc = of_property_read_u32(node, "oplus_spec,non-standard-vbatdet-mv", &spec->non_standard_vbatdet_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,non-standard-vbatdet-mv property error, rc=%d\n", rc);
		spec->non_standard_vbatdet_mv = default_spec.non_standard_vbatdet_mv;
	}

	rc = read_signed_data_from_node(node, "oplus_spec,ffc-temp-thr", (s32 *)spec->ffc_temp_thr,
	...
	rc = of_property_read_u32(node, "oplus_spec,wired-ffc-step-max", &spec->wired_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-ffc-fv-mv", (u32 *)spec->wired_ffc_fv_mv,
					  spec->wired_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wired-ffc-fv-mv property error, rc=%d\n", rc);
		for (i = 0; i < spec->wired_ffc_step_max; i++)
			spec->wired_ffc_fv_mv[i] = default_spec.wired_ffc_fv_mv[i];
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-ffc-fv-cutoff-mv",
	...
	if (rc < 0) {
		...
		for (i = 0; i < spec->wired_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wired_ffc_fv_cutoff_mv[i][m] = default_spec.wired_ffc_fv_cutoff_mv[i][m];
		}
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-ffc-fcc-ma", (u32 *)spec->wired_ffc_fcc_ma,
					  (FFC_TEMP_REGION_MAX - 2) * spec->wired_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wired-ffc-fcc-ma property error, rc=%d\n", rc);
		for (i = 0; i < spec->wired_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wired_ffc_fcc_ma[i][m] = default_spec.wired_ffc_fcc_ma[i][m];
		}
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-ffc-fcc-cutoff-ma",
	...
	if (rc < 0) {
		...
		for (i = 0; i < spec->wired_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wired_ffc_fcc_cutoff_ma[i][m] = default_spec.wired_ffc_fcc_cutoff_ma[i][m];
		}
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-ffc-fcc-sub-cutoff-ma",
	...
	if (rc < 0) {
		...
		for (i = 0; i < spec->wired_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wired_ffc_fcc_sub_cutoff_ma[i][m] = default_spec.wired_ffc_fcc_cutoff_ma[i][m];
		}
	}

	rc = of_property_read_u32(node, "oplus_spec,wls-ffc-step-max", &spec->wls_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-fv-mv", (u32 *)spec->wls_ffc_fv_mv,
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-fv-cutoff-mv", (u32 *)spec->wls_ffc_fv_cutoff_mv,
					  spec->wls_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wls-ffc-fv-cutoff-mv property error, rc=%d\n", rc);
		for (i = 0; i < spec->wls_ffc_step_max; i++)
			spec->wls_ffc_fv_cutoff_mv[i] = default_spec.wls_ffc_fv_cutoff_mv[i];
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-icl-ma", (u32 *)spec->wls_ffc_icl_ma,
					  (FFC_TEMP_REGION_MAX - 2) * spec->wls_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wls-ffc-icl-ma property error, rc=%d\n", rc);
		for (i = 0; i < spec->wls_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wls_ffc_icl_ma[i][m] = default_spec.wls_ffc_icl_ma[i][m];
		}
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-fcc-ma", (u32 *)spec->wls_ffc_fcc_ma,
					  (FFC_TEMP_REGION_MAX - 2) * spec->wls_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wls-ffc-fcc-ma property error, rc=%d\n", rc);
		for (i = 0; i < spec->wls_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wls_ffc_fcc_ma[i][m] = default_spec.wls_ffc_fcc_ma[i][m];
		}
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-fcc-cutoff-ma", (u32 *)spec->wls_ffc_fcc_cutoff_ma,
					  (FFC_TEMP_REGION_MAX - 2) * spec->wls_ffc_step_max);
	if (rc < 0) {
		chg_err("get oplus_spec,wls-ffc-fcc-cutoff-ma property error, rc=%d\n", rc);
		for (i = 0; i < spec->wls_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wls_ffc_fcc_cutoff_ma[i][m] = default_spec.wls_ffc_fcc_cutoff_ma[i][m];
		}
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-ffc-fcc-sub-cutoff-ma",
	...
	if (rc < 0) {
		...
		for (i = 0; i < spec->wls_ffc_step_max; i++) {
			for (m = 0; m < (FFC_TEMP_REGION_MAX - 2); m++)
				spec->wls_ffc_fcc_sub_cutoff_ma[i][m] = default_spec.wls_ffc_fcc_cutoff_ma[i][m];
		}
	}

	oplus_comm_parse_aging_ffc_dt(comm_dev);

	rc = read_unsigned_data_from_node(node, "oplus_spec,wired-vbatdet-mv", (u32 *)spec->wired_vbatdet_mv,
					  TEMP_REGION_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,wired-vbatdet-mv property error, rc=%d\n", rc);
		for (i = 0; i < TEMP_REGION_MAX; i++)
			spec->wired_vbatdet_mv[i] = default_spec.wired_vbatdet_mv[i];
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,wls-vbatdet-mv", (u32 *)spec->wls_vbatdet_mv,
					  TEMP_REGION_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,wls-vbatdet-mv property error, rc=%d\n", rc);
		for (i = 0; i < TEMP_REGION_MAX; i++)
			spec->wls_vbatdet_mv[i] = default_spec.wls_vbatdet_mv[i];
	}

	rc = of_property_read_u32(node, "oplus_spec,fcc-gear-thr-mv", &spec->fcc_gear_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,fcc-gear-thr-mv property error, rc=%d\n", rc);
		spec->fcc_gear_thr_mv = default_spec.fcc_gear_thr_mv;
	}

	rc = of_property_read_u32(node, "oplus_spec,full-pre-ffc-mv", &spec->full_pre_ffc_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,full-pre-ffc-mv property error, rc=%d\n", rc);
		spec->full_pre_ffc_mv = 4455;
	}
	spec->full_pre_ffc_judge = of_property_read_bool(node, "oplus_spec,full_pre_ffc_judge");

	rc = of_property_read_u32(node, "oplus_spec,vbatt-ov-thr-mv", &spec->vbatt_ov_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,vbatt-ov-thr-mv property error, rc=%d\n", rc);
		spec->vbatt_ov_thr_mv = default_spec.vbatt_ov_thr_mv;
	}

	rc = of_property_read_u32(node, "oplus_spec,vbat_uv_thr_mv", &spec->vbat_uv_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,vbat_uv_thr_mv property error, rc=%d\n", rc);
		spec->vbat_uv_thr_mv = default_spec.vbat_uv_thr_mv;
	}
	rc = of_property_read_u32(node, "oplus_spec,vbat_charging_uv_thr_mv", &spec->vbat_charging_uv_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,vbat_charging_uv_thr_mv property error, rc=%d\n", rc);
		spec->vbat_charging_uv_thr_mv = default_spec.vbat_charging_uv_thr_mv;
	}

	rc = of_property_read_u32(node, "oplus_spec,tbatt_power_off_cali_temp", &spec->tbatt_power_off_cali_temp);
	if (rc < 0) {
		chg_err("get oplus_spec,tbatt_power_off_cali_temp error, rc=%d\n", rc);
		spec->tbatt_power_off_cali_temp = 0;
	}

	config->gauge_stuck_jump_support = of_property_read_bool(node, "oplus,gauge_stuck_jump_support");

	if (config->gauge_stuck_jump_support) {
		rc = of_property_read_u32(node, "oplus,gauge_stuck_threshold", &spec->gauge_stuck_threshold);
		if (rc < 0) {
			chg_err("get oplus_spec,gauge_stuck_threshold property error, rc=%d\n", rc);
			spec->gauge_stuck_threshold = default_spec.gauge_stuck_threshold;
		}

		rc = of_property_read_u32(node, "oplus,gauge_stuck_time", &spec->gauge_stuck_time);
		if (rc < 0) {
			chg_err("get oplus_spec,gauge_stuck_time property error, rc=%d\n", rc);
			spec->gauge_stuck_time = default_spec.gauge_stuck_time;
		}
	}

	rc = of_property_read_u32(node, "oplus_spec,sub_vbat_uv_thr_mv", &spec->sub_vbat_uv_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,sub_vbat_uv_thr_mv property error, rc=%d\n", rc);
		spec->sub_vbat_uv_thr_mv = 0;
	}
	rc = of_property_read_u32(node, "oplus_spec,sub_vbat_charging_uv_thr_mv", &spec->sub_vbat_charging_uv_thr_mv);
	if (rc < 0) {
		chg_err("get oplus_spec,sub_vbat_charging_uv_thr_mv property error, rc=%d\n", rc);
		spec->sub_vbat_charging_uv_thr_mv = 0;
	}

	rc = of_property_read_u32(node, "oplus_spec,removed_bat_decidegc", &spec->removed_bat_decidegc);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus,ui_soc_decimal_speedmin", &config->ui_soc_decimal_speedmin);
	if (rc < 0) {
		chg_err("get oplus,ui_soc_decimal_speedmin property error, rc=%d\n", rc);
		config->ui_soc_decimal_speedmin = 2;
	}

	config->vooc_show_ui_soc_decimal = of_property_read_bool(node, "oplus,vooc_show_ui_soc_decimal");
...
static int oplus_comm_fv_max_vote_callback(struct votable *votable, void *data, int fv_mv, const char *client,
					   bool step)
{
...
static int oplus_comm_fv_min_vote_callback(struct votable *votable, void *data, int fv_mv, const char *client,
					   bool step)
{
	int rc;
...
static int oplus_comm_chg_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client,
						bool step)
{
	struct oplus_chg_comm *chip = data;
	int rc;
...
static int oplus_comm_chg_suspend_vote_callback(struct votable *votable, void *data, int suspend, const char *client,
						bool step)
{
	struct oplus_chg_comm *chip = data;
	int rc;
...
static int oplus_comm_cool_down_vote_callback(struct votable *votable, void *data, int cool_down, const char *client,
					      bool step)
{
	struct oplus_chg_comm *chip = data;
	int rc;
...
static int oplus_comm_vote_init(struct oplus_chg_comm *chip)
{
	...
	chip->fv_max_votable = create_votable("FV_MAX", VOTE_MAX, oplus_comm_fv_max_vote_callback, chip);
	...
	chip->fv_min_votable = create_votable("FV_MIN", VOTE_MIN, oplus_comm_fv_min_vote_callback, chip);
	...
	chip->chg_disable_votable =
		create_votable("CHG_DISABLE", VOTE_SET_ANY, oplus_comm_chg_disable_vote_callback, chip);
	...
	chip->chg_suspend_votable =
		create_votable("CHG_SUSPEND", VOTE_SET_ANY, oplus_comm_chg_suspend_vote_callback, chip);
	...
	chip->cool_down_votable = create_votable("COOL_DOWN", VOTE_MIN, oplus_comm_cool_down_vote_callback, chip);
...
static ssize_t proc_ui_soc_decimal_write(struct file *file, const char __user *buf, size_t len, loff_t *data)
{
...
static ssize_t proc_ui_soc_decimal_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	struct oplus_chg_comm *chip = pde_data(file_inode(file));
	struct oplus_comm_config *config = &chip->config;
	struct ui_soc_decimal *soc_decimal = &chip->soc_decimal;
	...
	if (config->vooc_show_ui_soc_decimal) {
		svooc_is_control_by_vooc = (chg_type == OPLUS_CHG_USB_TYPE_VOOC || chg_type == OPLUS_CHG_USB_TYPE_DCP);

		if (!svooc_is_control_by_vooc && !soc_decimal->calculate_decimal_time && !chip->wls_online &&
		    chip->vooc_online && vooc_sid) {
			cancel_delayed_work_sync(&chip->ui_soc_decimal_work);
...
retry:
	chip->tbatt_pwroff_task = kthread_create(oplus_comm_tbatt_power_off_kthread, chip, "tbatt_pwroff");
...
static ssize_t proc_tbatt_pwroff_write(struct file *file, const char __user *buf, size_t len, loff_t *data)
{
	char buffer[2] = { 0 };
...
static ssize_t proc_tbatt_pwroff_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[3] = { 0 };
	int len = 0;
...
static ssize_t proc_charger_factorymode_test_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, COMM_ITEM_FACTORY_TEST);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static void oplus_comm_reset_chginfo(struct oplus_chg_comm *chip)
{
	...
	if ((chip->wired_online || chip->wls_online) && spec->max_chg_time_sec > 0) {
		schedule_delayed_work(&chip->charge_timeout_work, msecs_to_jiffies(spec->max_chg_time_sec * 1000));
	}
...
static ssize_t oplus_comm_chg_cycle_write(struct file *file, const char __user *buff, size_t count, loff_t *ppos)
{
	struct oplus_chg_comm *chip = pde_data(file_inode(file));
	...
	if ((strncmp(proc_chg_cycle_data, "en808", 5) == 0) || (strncmp(proc_chg_cycle_data, "user_enable", 11) == 0)) {
		...
		if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
			oplus_comm_set_chg_cycle_status(chip,
							chip->chg_cycle_status & (~(int)CHG_CYCLE_VOTER__ENGINEER));
...
static ssize_t oplus_comm_chg_cycle_read(struct file *file, char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (chip) {
		oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_NOTIFY_CODE, &data, true);
		notify_code = data.intval;
	} else {
...
static int oplus_comm_init_proc(struct oplus_chg_comm *chip)
{
	...
	pr_entry_tmp = proc_create_data("ui_soc_decimal", 0664, NULL, &ui_soc_decimal_ops, chip);
	if (pr_entry_tmp == NULL)
		chg_err("Couldn't create ui_soc_decimal proc entry\n");

	pr_entry_tmp = proc_create_data("charger_cycle", 0664, NULL, &chg_cycle_proc_fops, chip);
	if (pr_entry_tmp == NULL)
		chg_err("Couldn't create charger_cycle proc entry\n");

	pr_entry_tmp = proc_create_data("tbatt_pwroff", 0664, NULL, &tbatt_pwroff_proc_fops, chip);
	if (pr_entry_tmp == NULL)
		chg_err("Couldn't create tbatt_pwroff proc entry\n");

	pr_entry_tmp = proc_create_data("batt_param_noplug", 0664, NULL, &batt_param_noplug_proc_fops, chip);
	if (pr_entry_tmp == NULL)
...
#endif
		pr_entry_tmp = proc_create_data("charger_factorymode_test", 0666, pr_entry_da,
	...
	pr_entry_tmp = proc_create_data("integrate_gauge_fcc_flag", 0664, pr_entry_da,
					&proc_integrate_gauge_fcc_flag_ops, chip);
	if (pr_entry_tmp == NULL)
		chg_err("Couldn't create integrate_gauge_fcc_flag proc entry\n");

	pr_entry_tmp = proc_create_data("hmac", 0444, pr_entry_da, &hmac_proc_fops, chip);
	if (pr_entry_tmp == NULL)
		chg_err("Couldn't create hmac proc entry\n");
	pr_entry_tmp = proc_create_data("reserve_soc_debug", 0644, pr_entry_da, &proc_reserve_soc_debug_ops, chip);
	if (pr_entry_tmp == NULL)
					       ...
					       struct panel_event_notification *notification, void *client_data)
{
	...
	chg_debug("Notification type:%d, early_trigger:%d", notification->notif_type,
...
static int fb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct oplus_chg_comm *chip = container_of(nb, struct oplus_chg_comm, chg_fb_notify);
...
static int oplus_comm_register_lcd_notify(struct oplus_chg_comm *chip)
{
	...
	if (chip->active_panel) {
		cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
						       ...
						       &oplus_comm_panel_notifier_callback, chip);
		if (!cookie) {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip = container_of(dwork, struct oplus_chg_comm, lcd_notify_reg_work);
	static int retry_count;
	int rc;
	...
	if (rc < 0) {
		...
		schedule_delayed_work(&chip->lcd_notify_reg_work, msecs_to_jiffies(LCD_REG_RETRY_DELAY_MS));
		return;
	}
	retry_count = 0;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip = container_of(dwork, struct oplus_chg_comm, fg_soft_reset_work);

	if (chip->soc < SOFT_REST_SOC_THRESHOLD || oplus_gauge_check_reset_condition() || fg_reset_test) {
...
{
	struct oplus_chg_comm *chip = container_of(work, struct oplus_chg_comm, wired_chg_check_work);
...
static int oplus_comm_driver_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&comm_dev->chg_type_change_work, oplus_comm_chg_type_change_work);
...
static int oplus_comm_driver_remove(struct platform_device *pdev)
{
	...
	if (comm_dev->lcd_notify_reg) {
		...
		if (comm_dev->active_panel && comm_dev->notifier_cookie)
			panel_event_notifier_unregister(comm_dev->notifier_cookie);
#else /* CONFIG_DRM_PANEL_NOTIFY */
#if IS_ENABLED(CONFIG_DRM_MSM) || IS_ENABLED(CONFIG_DRM_OPLUS_NOTIFY)
...
int oplus_comm_get_wired_ffc_cutoff_fv(struct oplus_mms *topic, int step, enum oplus_ffc_temp_region temp_region)
{
	struct oplus_chg_comm *chip;
	struct oplus_comm_spec_config *spec;
