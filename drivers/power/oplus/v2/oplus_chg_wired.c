...
static struct oplus_wired_spec_config default_config = {
	.pd_iclmax_ma = 3000,
	.qc_iclmax_ma = 2000,
	.input_power_mw = { 2500, 2500, 7500, 10000, 18000, 18000, 18000 },
...
static const char *const oplus_wired_chg_mode_text[] = {
	[OPLUS_WIRED_CHG_MODE_UNKNOWN] = "unknown", [OPLUS_WIRED_CHG_MODE_SDP] = "sdp",
	[OPLUS_WIRED_CHG_MODE_CDP] = "cdp",	    [OPLUS_WIRED_CHG_MODE_DCP] = "dcp",
	[OPLUS_WIRED_CHG_MODE_VOOC] = "vooc",	    [OPLUS_WIRED_CHG_MODE_QC] = "qc",
	[OPLUS_WIRED_CHG_MODE_PD] = "pd",	    [OPLUS_WIRED_CHG_MODE_MAX] = "invalid",
};
...
__maybe_unused static bool is_pd_svooc_votable_available(struct oplus_chg_wired *chip)
{
...
__maybe_unused static bool is_vooc_chg_auto_mode_votable_available(struct oplus_chg_wired *chip)
{
...
static const char *oplus_wired_get_chg_mode_region_str(enum oplus_wired_charge_mode mode)
{
...
static int oplus_wired_set_err_code(struct oplus_chg_wired *chip, unsigned int err_code)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_ERR_CODE, err_code);
	if (msg == NULL) {
...
static int oplus_wired_current_set(struct oplus_chg_wired *chip, bool vbus_changed)
{
	...
	case OPLUS_WIRED_CHG_MODE_PD:
		...
		if (cool_down > 0)
			cool_down_curr = spec->cool_down_pdqc_curr_ma[cool_down - 1];
		else
			cool_down_curr = 0;
		break;
	case OPLUS_WIRED_CHG_MODE_VOOC:
		...
		if (cool_down > 0)
			cool_down_curr = spec->cool_down_vooc_curr_ma[cool_down - 1];
		else
			cool_down_curr = 0;
		break;
	default:
		...
		if (cool_down > 0)
			cool_down_curr = spec->cool_down_normal_curr_ma[cool_down - 1];
		else
			cool_down_curr = 0;
		break;
	}
	...
	chg_info("chg_type=%s, chg_mode=%s, spec_icl=%d, spec_fcc=%d, cool_down_icl=%d, sale_mode=%d, cool_down=%d\n",
		 oplus_wired_get_chg_type_str(chip->chg_type), oplus_wired_get_chg_mode_region_str(chip->chg_mode),
		 icl_ma, fcc_ma, cool_down_curr, chip->chg_ctrl_by_sale_mode, chip->cool_down);
	...
	if (!chip->authenticate || !chip->hmac) {
		vote(chip->fcc_votable, NON_STANDARD_VOTER, true, spec->non_standard_ibatmax_ma, false);
	...
	if (led_on) {
		if (chip->chg_type == OPLUS_CHG_USB_TYPE_VOOC)
			...
			vote(chip->fcc_votable, LED_ON_VOTER, true, spec->led_on_fcc_max_ma[chip->temp_region], false);
	} else {
		vote(chip->fcc_votable, LED_ON_VOTER, false, 0, false);
	}
	icl_changed = (icl_tmp_ma != get_effective_result(chip->icl_votable));
	chg_info("vbus_changed=%s, icl_changed=%s\n", true_or_false_str(vbus_changed), true_or_false_str(icl_changed));
...
static int oplus_wired_get_vbatt_pdqc_to_9v_thr(struct oplus_chg_wired *chip)
{
	...
	if (node != NULL) {
		rc = of_property_read_u32(node, "oplus_spec,vbatt_pdqc_to_9v_thr", &thr);
		if (rc < 0) {
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, qc_config_work.work);
	struct oplus_wired_spec_config *spec = &chip->spec;
	...
	if (chip->cool_down > 0) {
		cool_down = chip->cool_down > spec->cool_down_pdqc_level_max ? spec->cool_down_pdqc_level_max :
									       chip->cool_down;
	...
	case OPLUS_ACTION_BOOST:
		...
		if (spec->vbatt_pdqc_to_9v_thr > 0 && chip->vbat_mv < spec->vbatt_pdqc_to_9v_thr) {
			...
			if (rc == -EAGAIN) {
				...
				if (chip->qc_retry_count < QC_RETRY_COUNT_MAX) {
					...
					schedule_delayed_work(&chip->qc_config_work, QC_RETRY_DELAY);
					return;
				} else {
					chip->qc_retry_count = 0;
					chip->qc_action = OPLUS_ACTION_NULL;
	...
	rc = wait_for_completion_timeout(&chip->qc_action_ack, msecs_to_jiffies(PDQC_CONFIG_WAIT_TIME_MS));
	if (!rc) {
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, pd_config_work.work);
	struct oplus_wired_spec_config *spec = &chip->spec;
	int cool_down, cool_down_vol;
	int vbus_set_mv = OPLUS_CHG_VBUS_5V; /* vbus default setting voltage is 5V */
	bool vbus_changed = false;
...
#define OPLUS_PD_9V_PDO 0x32d12c
	...
	case OPLUS_ACTION_BOOST:
		if (is_pd_svooc_votable_available(chip) && !!get_effective_result(chip->pd_svooc_votable)) {
		...
		if (spec->vbatt_pdqc_to_9v_thr > 0 && chip->vbat_mv < spec->vbatt_pdqc_to_9v_thr) {
			...
			if (rc < 0 || vbus_get_mv < 7500) {
				if (chip->pd_retry_count < PD_RETRY_COUNT_MAX) {
					chip->pd_retry_count++;
					schedule_delayed_work(&chip->pd_config_work, PD_RETRY_DELAY);
					return;
				} else {
					chip->pd_retry_count = 0;
					chip->pd_action = OPLUS_ACTION_NULL;
					vote(chip->pd_boost_disable_votable, TIMEOUT_VOTER, true, 1, false);
	...
	rc = wait_for_completion_timeout(&chip->pd_action_ack, msecs_to_jiffies(PDQC_CONFIG_WAIT_TIME_MS));
	if (!rc) {
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, gauge_update_work);
	struct oplus_wired_spec_config *spec = &chip->spec;
	union mms_msg_data data = { 0 };
	int cool_down_vol = 0;
	int cool_down;
	...
	if (chip->vbus_mv > 7500 && ((spec->vbatt_pdqc_to_5v_thr > 0 && chip->vbat_mv >= spec->vbatt_pdqc_to_5v_thr) ||
...
static void oplus_wired_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_wired_gauge_subs_callback, "chg_wired");
...
static void oplus_wired_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_wired *chip = subs->priv_data;
...
static void oplus_wired_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_wired_wired_subs_callback, "chg_wired");
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, plugin_work);
	union mms_msg_data data = { 0 };
#ifdef CONFIG_OPLUS_CHARGER_MTK
	int boot_mode = 0;
	...
	if (chip->chg_online) {
		...
		vote(chip->pd_boost_disable_votable, TIMEOUT_VOTER, false, 0, false);
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, chg_type_change_work);
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, charger_current_changed_work);
	union mms_msg_data data = { 0 };
	int rc;

	rc = oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_CHARGER_CURR_MAX, &data, false);
	if (rc < 0) {
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, temp_region_update_work);

	if (chip->temp_region == TEMP_REGION_HOT || chip->temp_region == TEMP_REGION_COLD)
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, led_on_changed_work);
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, icl_changed_work);
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, fcc_changed_work);
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, qc_check_work);
	int rc;

	reinit_completion(&chip->qc_check_ack);
	oplus_cpa_switch_start(chip->cpa_topic, CHG_PROTOCOL_QC);
	oplus_wired_qc_detect_enable(true);
	rc = wait_for_completion_timeout(&chip->qc_check_ack, msecs_to_jiffies(QC_CHECK_WAIT_TIME_MS));
	if (!rc) {
...
{
	struct oplus_chg_wired *chip = container_of(work, struct oplus_chg_wired, pd_check_work);
	int rc;
	...
	case OPLUS_CHG_USB_TYPE_PD_PPS:
		...
		if (get_client_vote(chip->pd_boost_disable_votable, SVID_VOTER) > 0) {
			rc = wait_for_completion_timeout(&chip->pd_check_ack, msecs_to_jiffies(PD_CHECK_WAIT_TIME_MS));
			if (!rc) {
...
static void oplus_wired_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case COMM_ITEM_COOL_DOWN:
			...
			if (chip->cool_down == SALE_MODE_COOL_DOWN || chip->cool_down == SALE_MODE_COOL_DOWN_TWO) {
		...
		case COMM_ITEM_CHARGING_DISABLE:
			...
			if (rc < 0)
				...
				vote(chip->output_suspend_votable, USER_VOTER, !!data.intval, data.intval, false);
			break;
		case COMM_ITEM_CHARGE_SUSPEND:
			...
			if (rc < 0)
				...
				vote(chip->input_suspend_votable, USER_VOTER, !!data.intval, data.intval, false);
			break;
			...
			if (!!data.intval && spec->vbatt_pdqc_to_9v_thr > 0) {
				if (oplus_gauge_get_batt_num() == 2) {
					if (chip->adjust_pdqc_vol_thr_support)
						spec->vbatt_pdqc_to_9v_thr = FACTORY_MODE_PDQC_9V_THR;
					else
						spec->vbatt_pdqc_to_9v_thr = oplus_wired_get_vbatt_pdqc_to_9v_thr(chip);
					schedule_work(&chip->chg_type_change_work);
				}
			} else {
				spec->vbatt_pdqc_to_9v_thr = oplus_wired_get_vbatt_pdqc_to_9v_thr(chip);
...
static void oplus_wired_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_wired_comm_subs_callback, "chg_wired");
	...
	chip->temp_region = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_FCC_GEAR, &data, true);
	chip->fcc_gear = data.intval;
...
static void oplus_wired_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_wired_vooc_subs_callback, "chg_wired");
...
static void oplus_wired_subscribe_cpa_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->cpa_subs = oplus_mms_subscribe(chip->cpa_topic, chip, oplus_wired_cpa_subs_callback, "chg_wired");
...
static int oplus_wired_fcc_vote_callback(struct votable *votable, void *data, int fcc_ma, const char *client, bool step)
{
...
static int oplus_wired_icl_vote_callback(struct votable *votable, void *data, int icl_ma, const char *client, bool step)
{
	struct oplus_chg_wired *chip = data;
	int rc;
...
static int oplus_wired_input_suspend_vote_callback(struct votable *votable, void *data, int disable, const char *client,
						   bool step)
{
	struct oplus_chg_wired *chip = data;
	...
	if (chip->chg_online && !disable && !suspend_check_only) {
		if (is_vooc_chg_auto_mode_votable_available(chip))
			vote(chip->vooc_chg_auto_mode_votable, CHARGE_SUSPEND_VOTER, disable, disable, false);
		else
...
static int oplus_wired_output_suspend_vote_callback(struct votable *votable, void *data, int disable,
						    const char *client, bool step)
{
	...
	if (chip->chg_online && !disable && !suspend_check_only) {
		if (is_vooc_chg_auto_mode_votable_available(chip))
			vote(chip->vooc_chg_auto_mode_votable, CHAEGE_DISABLE_VOTER, disable, disable, false);
		else
			chg_err("vooc_chg_auto_mode_votable not found\n");
	}
...
static int oplus_wired_pd_boost_disable_vote_callback(struct votable *votable, void *data, int disable,
						      const char *client, bool step)
{
	struct oplus_chg_wired *chip = data;
...
static int oplus_wired_vote_init(struct oplus_chg_wired *chip)
{
	...
	chip->fcc_votable = create_votable("WIRED_FCC", VOTE_MIN, oplus_wired_fcc_vote_callback, chip);
	...
	chip->icl_votable = create_votable("WIRED_ICL", VOTE_MIN, oplus_wired_icl_vote_callback, chip);
	...
	chip->input_suspend_votable =
		create_votable("WIRED_CHARGE_SUSPEND", VOTE_SET_ANY, oplus_wired_input_suspend_vote_callback, chip);
	...
	chip->output_suspend_votable =
		create_votable("WIRED_CHARGING_DISABLE", VOTE_SET_ANY, oplus_wired_output_suspend_vote_callback, chip);
	...
	chip->pd_boost_disable_votable =
		create_votable("PD_BOOST_DISABLE", VOTE_SET_ANY, oplus_wired_pd_boost_disable_vote_callback, chip);
...
static int oplus_wired_parse_dt(struct oplus_chg_wired *chip)
{
	...
	chip->adjust_pdqc_vol_thr_support = of_property_read_bool(node, "oplus,adjust-pdqc-vol-thr-support");

	rc = of_property_read_u32(node, "oplus_spec,pd-iclmax-ma", &spec->pd_iclmax_ma);
	if (rc < 0) {
		chg_err("oplus_spec,pd-iclmax-ma reading failed, rc=%d\n", rc);
		spec->pd_iclmax_ma = default_config.pd_iclmax_ma;
	}
	rc = of_property_read_u32(node, "oplus_spec,qc-iclmax-ma", &spec->qc_iclmax_ma);
	if (rc < 0) {
		chg_err("oplus_spec,qc-iclmax-ma reading failed, rc=%d\n", rc);
		spec->qc_iclmax_ma = default_config.qc_iclmax_ma;
	}
	rc = of_property_read_u32(node, "oplus_spec,non-standard-ibatmax-ma", &spec->non_standard_ibatmax_ma);
	if (rc < 0) {
		chg_err("oplus_spec,non-standard-ibatmax-ma reading failed, rc=%d\n", rc);
		spec->non_standard_ibatmax_ma = default_config.non_standard_ibatmax_ma;
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,input-power-mw", (u32 *)(spec->input_power_mw),
					  OPLUS_WIRED_CHG_MODE_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,input-power-mw error, rc=%d\n", rc);
		for (i = 0; i < OPLUS_WIRED_CHG_MODE_MAX; i++)
			spec->input_power_mw[i] = default_config.input_power_mw[i];
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,led_on-fccmax-ma", (u32 *)(spec->led_on_fcc_max_ma),
					  TEMP_REGION_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,led_on-fccmax-ma error, rc=%d\n", rc);
		for (i = 0; i < TEMP_REGION_MAX; i++)
			spec->led_on_fcc_max_ma[i] = default_config.led_on_fcc_max_ma[i];
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,fccmax-ma-lv", (u32 *)(spec->fcc_ma[0]),
					  OPLUS_WIRED_CHG_MODE_MAX * TEMP_REGION_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,fccmax-ma-lv error, rc=%d\n", rc);
		for (i = 0; i < OPLUS_WIRED_CHG_MODE_MAX; i++) {
			for (m = 0; m < TEMP_REGION_MAX; m++)
				spec->fcc_ma[0][i][m] = default_config.fcc_ma[0][i][m];
		}
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,fccmax-ma-hv", (u32 *)(spec->fcc_ma[1]),
					  OPLUS_WIRED_CHG_MODE_MAX * TEMP_REGION_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,fccmax-ma-hv error, rc=%d\n", rc);
		for (i = 0; i < OPLUS_WIRED_CHG_MODE_MAX; i++) {
			for (m = 0; m < TEMP_REGION_MAX; m++)
				spec->fcc_ma[1][i][m] = default_config.fcc_ma[1][i][m];
		}
	}

	rc = of_property_read_u32(node, "oplus_spec,vbatt_pdqc_to_5v_thr", &spec->vbatt_pdqc_to_5v_thr);
	if (rc < 0) {
		chg_err("oplus_spec,vbatt_pdqc_to_5v_thr reading failed, rc=%d\n", rc);
		spec->vbatt_pdqc_to_5v_thr = default_config.vbatt_pdqc_to_5v_thr;
	}
	rc = of_property_read_u32(node, "oplus_spec,vbatt_pdqc_to_9v_thr", &spec->vbatt_pdqc_to_9v_thr);
	if (rc < 0) {
		chg_err("oplus_spec,vbatt_pdqc_to_9v_thr reading failed, rc=%d\n", rc);
		spec->vbatt_pdqc_to_9v_thr = default_config.vbatt_pdqc_to_9v_thr;
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,cool_down_pdqc_vol_mv",
					  (u32 *)(spec->cool_down_pdqc_vol_mv), WIRED_COOL_DOWN_LEVEL_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,cool_down_pdqc_vol_mv error, rc=%d\n", rc);
		for (i = 0; i < WIRED_COOL_DOWN_LEVEL_MAX; i++) {
			spec->cool_down_pdqc_vol_mv[i] = default_config.cool_down_pdqc_vol_mv[i];
			spec->cool_down_pdqc_level_max = default_config.cool_down_pdqc_level_max;
		}
	} else {
		spec->cool_down_pdqc_level_max = rc;
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,cool_down_pdqc_curr_ma",
					  (u32 *)(spec->cool_down_pdqc_curr_ma), WIRED_COOL_DOWN_LEVEL_MAX);
	if (rc < 0 || spec->cool_down_pdqc_level_max != rc) {
		chg_err("get oplus_spec,cool_down_pdqc_curr_ma error, rc=%d\n", rc);
		for (i = 0; i < WIRED_COOL_DOWN_LEVEL_MAX; i++) {
			spec->cool_down_pdqc_curr_ma[i] = default_config.cool_down_pdqc_curr_ma[i];
			spec->cool_down_pdqc_level_max = default_config.cool_down_pdqc_level_max;
		}
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,cool_down_vooc_curr_ma",
					  (u32 *)(spec->cool_down_vooc_curr_ma), WIRED_COOL_DOWN_LEVEL_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,cool_down_vooc_curr_ma error, rc=%d\n", rc);
		for (i = 0; i < WIRED_COOL_DOWN_LEVEL_MAX; i++) {
			spec->cool_down_vooc_curr_ma[i] = default_config.cool_down_vooc_curr_ma[i];
			spec->cool_down_vooc_level_max = default_config.cool_down_vooc_level_max;
		}
	} else {
		spec->cool_down_vooc_level_max = rc;
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,cool_down_normal_curr_ma",
					  (u32 *)(spec->cool_down_normal_curr_ma), WIRED_COOL_DOWN_LEVEL_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,cool_down_normal_curr_ma error, rc=%d\n", rc);
		for (i = 0; i < WIRED_COOL_DOWN_LEVEL_MAX; i++) {
			spec->cool_down_normal_curr_ma[i] = default_config.cool_down_normal_curr_ma[i];
			spec->cool_down_normal_level_max = default_config.cool_down_normal_level_max;
		}
	} else {
		spec->cool_down_normal_level_max = rc;
	}

	rc = read_unsigned_data_from_node(node, "oplus_spec,vbus_ov_thr_mv", (u32 *)(spec->vbus_ov_thr_mv),
					  OPLUS_VBUS_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,vbus_ov_thr_mv error, rc=%d\n", rc);
		for (i = 0; i < OPLUS_VBUS_MAX; i++)
			spec->vbus_ov_thr_mv[i] = default_config.vbus_ov_thr_mv[i];
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,vbus_uv_thr_mv", (u32 *)(spec->vbus_uv_thr_mv),
					  OPLUS_VBUS_MAX);
	if (rc < 0) {
		chg_err("get oplus_spec,vbus_uv_thr_mv error, rc=%d\n", rc);
		for (i = 0; i < OPLUS_VBUS_MAX; i++)
			spec->vbus_uv_thr_mv[i] = default_config.vbus_uv_thr_mv[i];
	}

	rc = of_property_read_string(node, "oplus,vooc_strategy_name", (const char **)&config->vooc_strategy_name);
	if (rc >= 0) {
		chg_info("vooc_strategy_name=%s\n", config->vooc_strategy_name);
		rc = oplus_chg_strategy_read_data(chip->dev, "oplus,vooc_strategy_data", &config->vooc_strategy_data);
		if (rc < 0) {
...
static int oplus_wired_strategy_init(struct oplus_chg_wired *chip)
{
	...
	chip->vooc_strategy = oplus_chg_strategy_alloc(config->vooc_strategy_name, config->vooc_strategy_data,
...
static void oplus_wired_shutdown(struct platform_device *pdev)
{
	...
	chip->chg_type = oplus_wired_get_chg_type();
	chg_info("wired_type=%s, chg_mode = %d\n", oplus_wired_get_chg_type_str(chip->chg_type), chip->chg_mode);
	switch (chip->chg_type) {
	case OPLUS_CHG_USB_TYPE_PD:
	case OPLUS_CHG_USB_TYPE_PD_DRP:
	case OPLUS_CHG_USB_TYPE_PD_PPS:
...
static int oplus_wired_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_chg_wired), GFP_KERNEL);
	if (chip == NULL) {
	...
	INIT_WORK(&chip->chg_type_change_work, oplus_wired_chg_type_change_work);
	INIT_WORK(&chip->temp_region_update_work, oplus_wired_temp_region_update_work);
	...
	INIT_WORK(&chip->charger_current_changed_work, oplus_wired_charger_current_changed_work);
