...
const int32_t default_wls_epp_strategy[] = { 0,	  375, 900, 1100, 1, 350, 430, 800, 1000, 1,
					     370, 450, 600, 800,  1, 400, 530, 400, 500,  1 };

const int32_t default_wls_epp_plus_strategy[] = { 0,   375, 1200, 1500, 1, 350, 430, 800, 1000, 1,
						  370, 450, 600,  800,	1, 400, 530, 400, 500,	1 };

static int oplus_chg_wls_track_upload_wls_err_info(struct oplus_chg_wls *wls_dev, enum wls_err_scene scene_type,
						   enum wls_err_reason reason_type);
...
static int oplus_chg_wls_rx_set_vout_step(struct oplus_wls_chg_rx *wls_rx, int target_vol_mv, int step_mv,
					  int wait_time_s)
{
	int next_step_mv;
...
__maybe_unused static int oplus_chg_wls_nor_fv_vote_callback(struct votable *votable, void *data, int fv_mv,
							     const char *client, bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static void oplus_chg_wls_get_third_part_verity_data_work(struct work_struct *work)
{
	...
	oplus_chg_wls_set_mutual_cmd(wls_dev, CMD_WLS_THIRD_PART_AUTH, sizeof(wls_status->vendor_id),
	...
	case WLS_RESPONE_INTO_FASTCHAGE:
		if (rx_msg->msg_type == WLS_CMD_INTO_FASTCHAGE) {
			...
			if (wls_dev->static_config.fastchg_fod_enable) {
				...
				(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx->rx_ic,
								    wls_dev->static_config.disable_fod_parm,
								    wls_dev->static_config.fod_parm_len);
			}
...
retry:
	if (!wls_status->aes_verity_data_ok) {
		chg_err("verity data not ready\n");
		ret = oplus_chg_wls_set_mutual_cmd(wls_dev, CMD_WLS_THIRD_PART_AUTH, sizeof(wls_status->vendor_id),
						   &(wls_status->vendor_id));
...
noinline
	...
	static void
	oplus_chg_wls_update_track_info(struct oplus_chg_wls *wls_dev, char *crux_info, bool clear)
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	int batt_temp = 0;
	u32 rx_version = 0;
	...
	if (crux_info) {
		snprintf(crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN,
			 ...
			 wls_status->fastchg_started, wls_status->vout_mv, wls_status->iout_ma, wls_status->break_count,
			 wls_status->trx_err, highest_temp, max_iout, min_cool_down, min_skewing_current,
			 !wls_status->verity_pass, wls_status->switch_quiet_mode);
		chg_info("%s\n", crux_info);
	}
}
						   ...
						   enum wls_err_reason reason_type)
{
	...
	oplus_chg_ic_creat_err_msg(wls_dev->wls_rx->rx_ic, OPLUS_IC_ERR_WLS_RX, 0,
				   ...
				   "%s",
				   wls_err_scene_text[scene_type], wls_err_reason_text[reason_type], rx_crux_info);
...
static int oplus_chg_wls_set_trx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (en) {
		...
		if (wls_status->trx_transfer_start_time &&
			...
			oplus_chg_wls_track_upload_tx_general_info(wls_dev, wls_status->trx_usb_present_once);
		}
		oplus_chg_wls_reset_variables(wls_dev);
...
static enum oplus_chg_mod_property oplus_chg_wls_props[] = {
	...
	OPLUS_CHG_PROP_ONLINE_KEEP,    OPLUS_CHG_PROP_TX_VOLTAGE_NOW,	 OPLUS_CHG_PROP_TX_CURRENT_NOW,
	OPLUS_CHG_PROP_CP_VOLTAGE_NOW, OPLUS_CHG_PROP_CP_CURRENT_NOW,	 OPLUS_CHG_PROP_WIRELESS_MODE,
	OPLUS_CHG_PROP_WIRELESS_TYPE,  OPLUS_CHG_PROP_CEP_INFO,		 OPLUS_CHG_PROP_REAL_TYPE,
	OPLUS_CHG_PROP_COOL_DOWN,      OPLUS_CHG_PROP_RX_VOUT_UVP,	 OPLUS_CHG_PROP_FW_UPGRADING,
...
static void oplus_chg_wls_fast_switch_next_step(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->fastchg_level >= fcc_chg->max_step) {
		...
		vote(wls_dev->fcc_votable, FCC_VOTER, true, fcc_chg->fcc_step[wls_status->fastchg_level].curr_ma,
		     false);
	}
...
static void oplus_chg_wls_fast_switch_prev_step(struct oplus_chg_wls *wls_dev)
{
	...
	vote(wls_dev->fcc_votable, FCC_VOTER, true, fcc_chg->fcc_step[wls_status->fastchg_level].curr_ma, false);
...
static int oplus_chg_wls_choose_epp_curve(struct oplus_chg_wls *wls_dev)
{
	...
	vote(wls_dev->nor_icl_votable, EPP_CURVE_VOTER, true, epp_cfg->fcc_step[wls_status->epp_chg_level].curr_ma,
	     true);

	if (last_epp_chg_level != wls_status->epp_chg_level)
		chg_info("[%s %d %d %d]\n", oplus_chg_wls_rx_state_text[wls_status->wls_type], batt_temp,
			 wls_status->epp_chg_level, epp_cfg->max_step);
...
static int oplus_chg_wls_rx_enter_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_START:
		...
		wls_status->current_rx_state = OPLUS_CHG_WLS_RX_STATE_FAST;
		if (is_nor_fv_votable_available(wls_dev))
			vote(wls_dev->nor_fv_votable, USER_VOTER, true, dynamic_cfg->batt_vol_max_mv + 100, false);
		delay_ms = 0;
		break;
	}
...
static int oplus_chg_wls_rx_exit_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx->rx_ic, wls_dev->static_config.disable_fod_parm,
					    wls_dev->static_config.fod_parm_len);
...
static int oplus_chg_wls_parse_dt(struct oplus_chg_wls *wls_dev)
{
	...
	rc = of_property_read_u8_array(node, "oplus,disable-fod-parm", (u8 *)&static_cfg->disable_fod_parm,
				       static_cfg->fod_parm_len);
	if (rc < 0) {
	...
	if (static_cfg->fastchg_fod_enable) {
		...
		rc = of_property_read_u8_array(node, "oplus,fastchg-fod-parm-12V",
					       ...
					       sizeof(oplus_chg_wls_disable_fod_parm));
	...
	if (rc < 0) {
		...
		length = rc;
		rc = of_property_read_u32_array(node, "oplus,wls_epp_strategy", (u32 *)&dynamic_cfg->epp_steps.fcc_step,
						length);
		if (rc < 0) {
	...
	for (k = 0; k < dynamic_cfg->epp_steps.max_step; k++) {
		chg_info("wls_epp_strategy[%d]%d %d %d %d %d\n", k, dynamic_cfg->epp_steps.fcc_step[k].low_threshold,
			 dynamic_cfg->epp_steps.fcc_step[k].high_threshold, dynamic_cfg->epp_steps.fcc_step[k].curr_ma,
			 dynamic_cfg->epp_steps.fcc_step[k].vol_max_mv, dynamic_cfg->epp_steps.fcc_step[k].need_wait);
	}
	...
	if (rc < 0) {
		...
		memcpy(&dynamic_cfg->epp_plus_steps.fcc_step, default_wls_epp_plus_strategy,
		...
		for (i = 0; i < WLS_FAST_SOC_MAX; i++) {
			...
			for (j = 0; j < WLS_FAST_TEMP_MAX; j++) {
				...
				rc = of_property_read_u32_array(
					soc_strategy_node, strategy_temp[j],
	...
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		for (j = 0; j < wls_dev->non_ffc_step[i].max_step; j++) {
			chg_info("%d %d %d %d %d\n", dynamic_cfg->non_ffc_step[i][j].low_threshold,
				 ...
				 dynamic_cfg->non_ffc_step[i][j].curr_ma, dynamic_cfg->non_ffc_step[i][j].vol_max_mv,
	...
	rc = read_unsigned_data_from_node(node, "oplus,strategy_soc", dynamic_cfg->wls_strategy_soc, WLS_SOC_NUM_MAX);
	if (rc < 0) {
							...
							loff_t *lo)
{
	...
	if (pmw_pulse == WLS_FASTCHG_MODE) {
		if (!wls_dev->wls_status.rx_present || wls_dev->wls_status.adapter_type == WLS_ADAPTER_TYPE_USB ||
...
static int oplus_chg_wls_init_charge_proc(struct oplus_chg_wls *wls_dev)
{
	...
	pr_entry_tmp = proc_create_data("enable_tx", 0664, pr_entry_da, &oplus_chg_wls_proc_tx_ops, wls_dev);
	...
	pr_entry_tmp = proc_create_data("deviated", 0664, pr_entry_da, &oplus_chg_wls_proc_deviated_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("user_sleep_mode", 0664, pr_entry_da, &oplus_chg_wls_proc_user_sleep_mode_ops,
					wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp =
		proc_create_data("idt_adc_test", 0664, pr_entry_da, &oplus_chg_wls_proc_idt_adc_test_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("enable_rx", 0664, pr_entry_da, &oplus_chg_wls_proc_rx_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("rx_power", 0664, pr_entry_da, &oplus_chg_wls_proc_rx_power_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("tx_power", 0664, pr_entry_da, &oplus_chg_wls_proc_tx_power_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("rx_version", 0664, pr_entry_da, &oplus_chg_wls_proc_rx_version_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("tx_version", 0664, pr_entry_da, &oplus_chg_wls_proc_tx_version_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
	...
	pr_entry_tmp = proc_create_data("ftm_mode", 0664, pr_entry_da, &oplus_chg_wls_proc_ftm_mode_ops, wls_dev);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
...
ssize_t oplus_chg_wls_send_mutual_cmd(struct oplus_mms *mms, char *buf)
{
	...
	chg_info("success to copy to user space cmd:%d, size:%d\n", ((struct oplus_chg_cmd *)buf)->cmd,
		 ((struct oplus_chg_cmd *)buf)->data_size);
...
static void oplus_chg_wls_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	wls_dev->gauge_subs =
		oplus_mms_subscribe(wls_dev->gauge_topic, wls_dev, oplus_chg_wls_gauge_subs_callback, "chg_wls");
...
static void oplus_chg_wls_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	wls_dev->wired_subs =
		oplus_mms_subscribe(wls_dev->wired_topic, wls_dev, oplus_chg_wls_wired_subs_callback, "chg_wls");
...
static void oplus_chg_wls_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	wls_dev->comm_subs =
		oplus_mms_subscribe(wls_dev->comm_topic, wls_dev, oplus_chg_wls_comm_subs_callback, "chg_wls");
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, temp_region_update_work);
	enum oplus_chg_temp_region temp_region;
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, cool_down_update_work);
	union mms_msg_data data = { 0 };
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, wls_err_handler_work);
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, wls_present_handler_work);
	struct mms_msg *msg;
	int rc;
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, wls_online_handler_work);
	struct mms_msg *msg;
	int rc;
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, wls_offline_handler_work);
	struct mms_msg *msg;
	int rc;
...
{
	struct oplus_chg_wls *wls_dev = container_of(work, struct oplus_chg_wls, wls_event_changed_handler_work);
	enum oplus_chg_wls_event_code event_code;
	enum oplus_chg_wls_rx_mode rx_mode;
	int rc;
...
static int oplus_chg_wls_virq_register(struct oplus_chg_wls *wls_dev)
{
	...
	rc = oplus_chg_ic_virq_register(wls_dev->wls_rx->rx_ic, OPLUS_IC_VIRQ_ERR, oplus_chg_wls_err_handler, wls_dev);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ERR error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(wls_dev->wls_rx->rx_ic, OPLUS_IC_VIRQ_PRESENT, oplus_chg_wls_present_handler,
					wls_dev);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_PRESENT error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(wls_dev->wls_rx->rx_ic, OPLUS_IC_VIRQ_ONLINE, oplus_chg_wls_online_handler,
					wls_dev);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ONLINE error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(wls_dev->wls_rx->rx_ic, OPLUS_IC_VIRQ_OFFLINE, oplus_chg_wls_offline_handler,
					wls_dev);
	if (rc < 0)
...
static int oplus_chg_wls_mms_update_bcc_exit_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	chg_info("get stop curr, enter soc is [%d], batt_temp_plugin[%d], stop curr is %d\n", batt_soc_plugin,
		 batt_temp_plugin, wls_stop_curr);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, wls_mms_init_work);
	struct device_node *node = wls_dev->dev->of_node;
...
static int oplus_chg_wls_driver_probe(struct platform_device *pdev)
{
	...
	rc = oplus_chg_wls_rx_register_msg_callback(wls_dev->wls_rx->rx_ic, wls_dev, oplus_chg_wls_rx_msg_callback);
	if (!rc)
		wls_dev->msg_callback_ok = true;
	...
	wls_dev->fcc_votable = create_votable("WLS_FCC", VOTE_MIN, oplus_chg_wls_fcc_vote_callback, wls_dev);
	...
	wls_dev->fastchg_disable_votable = create_votable("WLS_FASTCHG_DISABLE", VOTE_SET_ANY,
	...
	wls_dev->wrx_en_votable =
		create_votable("WLS_WRX_EN", VOTE_SET_ANY, oplus_chg_wls_wrx_en_vote_callback, wls_dev);
	...
	wls_dev->nor_icl_votable =
		create_votable("WLS_NOR_ICL", VOTE_MIN, oplus_chg_wls_nor_icl_vote_callback, wls_dev);
	...
	wls_dev->nor_fcc_votable =
		create_votable("WLS_NOR_FCC", VOTE_MIN, oplus_chg_wls_nor_fcc_vote_callback, wls_dev);
	...
	wls_dev->nor_out_disable_votable = create_votable("WLS_NOR_OUT_DISABLE", VOTE_SET_ANY,
	...
	wls_dev->nor_input_disable_votable = create_votable("WLS_NOR_INPUT_DISABLE", VOTE_SET_ANY,
							    oplus_chg_wls_nor_input_disable_vote_callback, wls_dev);
	...
	wls_dev->rx_disable_votable =
		create_votable("WLS_RX_DISABLE", VOTE_SET_ANY, oplus_chg_wls_rx_disable_vote_callback, wls_dev);
