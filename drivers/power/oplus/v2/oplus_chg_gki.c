...
__maybe_unused static bool is_wired_icl_votable_available(struct oplus_gki_device *chip)
{
...
__maybe_unused static bool is_wired_fcc_votable_available(struct oplus_gki_device *chip)
{
...
__maybe_unused static bool is_fv_votable_available(struct oplus_gki_device *chip)
{
	if (!chip->fv_votable)
...
__maybe_unused static bool is_vooc_curr_votable_available(struct oplus_gki_device *chip)
{
...
__maybe_unused static bool is_ufcs_curr_votable_available(struct oplus_gki_device *chip)
{
...
__maybe_unused static bool is_pps_curr_votable_available(struct oplus_gki_device *chip)
{
...
static enum power_supply_property wls_props[] = {
	...
	POWER_SUPPLY_PROP_TEMP,	       POWER_SUPPLY_PROP_PRESENT,
};
...
static int oplus_to_power_supply_type[] = {
	POWER_SUPPLY_TYPE_UNKNOWN, POWER_SUPPLY_TYPE_USB,	 POWER_SUPPLY_TYPE_USB_DCP,
	POWER_SUPPLY_TYPE_USB_CDP, POWER_SUPPLY_TYPE_USB_ACA,	 POWER_SUPPLY_TYPE_USB_TYPE_C,
	POWER_SUPPLY_TYPE_USB_PD,  POWER_SUPPLY_TYPE_USB_PD_DRP, POWER_SUPPLY_TYPE_USB_PD,
	POWER_SUPPLY_TYPE_USB,	   POWER_SUPPLY_TYPE_USB_DCP,	 POWER_SUPPLY_TYPE_USB_DCP,
	POWER_SUPPLY_TYPE_USB_DCP, POWER_SUPPLY_TYPE_USB_DCP,	 POWER_SUPPLY_TYPE_USB_DCP,
	POWER_SUPPLY_TYPE_USB_DCP,
};
...
static int usb_psy_get_prop(struct power_supply *psy, enum power_supply_property prop, union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		pval->intval = oplus_wired_get_voltage_max(chip->wired_topic) * 1000;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (is_wired_icl_votable_available(chip))
			pval->intval = get_client_vote(chip->wired_icl_votable, SPEC_VOTER) * 1000;
		else
			rc = -ENOTSUPP;
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		if (is_wired_icl_votable_available(chip))
			pval->intval = get_effective_result(chip->wired_icl_votable) * 1000;
		else
			rc = -ENOTSUPP;
		break;
	case POWER_SUPPLY_PROP_USB_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_TEMP:
		oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_USB_TEMP_L, &data, true);
		usb_temp_l = data.intval;
		oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_USB_TEMP_R, &data, true);
...
static int oplus_gki_get_ui_soc(struct oplus_gki_device *chip)
{
	...
	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_SOC, &data, true);
	if (rc < 0) {
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		if (is_fv_votable_available(chip))
			pval->intval = (get_client_vote(chip->fv_votable, SPEC_VOTER) + 50) * 1000;
		else
			rc = -ENOTSUPP;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (is_support_parallel_battery(chip->gauge_topic) && is_main_gauge_topic_available(chip))
			rc = oplus_mms_get_item_data(chip->main_gauge_topic, GAUGE_ITEM_CURR, &data,
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		if (chip->wired_online && chip->vooc_started == true && chip->wired_type == OPLUS_CHG_USB_TYPE_SVOOC)
			pval->intval = 10000;
		else if (chip->wls_online)
			pval->intval = 0; /* TODO */
		else
			pval->intval = oplus_wired_get_vbus();
		if (oplus_is_power_off_charging() && oplus_chg_get_curr_time_ms(&cur_chg_time) < FORCE_VBUS_5V_TIME)
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		if (chip->wired_online && is_wired_icl_votable_available(chip)) {
			if (chip->led_on) {
				...
				if (is_vooc_curr_votable_available(chip))
					vote(chip->vooc_curr_votable, HIDL_VOTER, (val == 0) ? false : true, val,
					     false);
				if (is_ufcs_curr_votable_available(chip))
					vote(chip->ufcs_curr_votable, HIDL_VOTER, (val == 0) ? false : true, val,
					     false);
				if (is_pps_curr_votable_available(chip))
...
{
	struct oplus_gki_device *chip = container_of(work, struct oplus_gki_device, gauge_update_work);
	union mms_msg_data data = { 0 };
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	int eng_version;
#endif
	...
	if (eng_version == AGING || eng_version == HIGH_TEMP_AGING || eng_version == FACTORY || chip->wired_online ||
	    chip->wls_online) {
#else
...
static void oplus_gki_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case GAUGE_ITEM_BATT_EXIST:
			rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_BATT_EXIST, &data, false);
			if (rc < 0)
...
static void oplus_gki_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_gki_gauge_subs_callback, "gki");
	...
	chip->temperature = data.intval;
	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_BATT_EXIST, &data, true);
	if (rc < 0)
		chip->batt_exist = false;
	else
		chip->batt_exist = data.intval;
...
static bool oplus_gki_bc12_is_completed(struct oplus_gki_device *chip)
{
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_BC12_COMPLETED, &data, true);
	return !!data.intval;
}

static void oplus_gki_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_gki_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_gki_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_gki_wired_subs_callback, "gki");
	...
	if (chip->wired_online) {
		usb_psy_desc.type = oplus_to_power_supply_type[chip->wired_type];
		if ((usb_psy_desc.type == POWER_SUPPLY_TYPE_UNKNOWN) && oplus_gki_bc12_is_completed(chip))
			usb_psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
	} else {
		usb_psy_desc.type = POWER_SUPPLY_TYPE_UNKNOWN;
	}
	...
	psy_cfg.drv_data = chip;
	chip->usb_psy = devm_power_supply_register(&topic->dev, &usb_psy_desc, &psy_cfg);
...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	...
	struct oplus_gki_device *chip = container_of(dwork, struct oplus_gki_device, status_keep_clean_work);
	union mms_msg_data data = { 0 };
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_gki_device *chip = container_of(dwork, struct oplus_gki_device, status_keep_delay_unlock_work);
...
static void oplus_gki_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_gki_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_gki_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_gki_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_gki_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_gki_device *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->comm_topic = topic;
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_gki_comm_subs_callback, "gki");
	if (IS_ERR_OR_NULL(chip->comm_subs)) {
		chg_err("subscribe common topic error, rc=%ld\n", PTR_ERR(chip->comm_subs));
		return;
	}

	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_BATT_STATUS, &data, true);
	chip->batt_status = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_BATT_HEALTH, &data, true);
	chip->batt_health = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_BATT_CHG_TYPE, &data, true);
	chip->batt_chg_type = data.intval;
...
static void oplus_gki_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_gki_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_gki_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_gki_device *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_gki_vooc_subs_callback, "gki");
...
static void oplus_gki_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_gki_device *chip = subs->priv_data;
...
static void oplus_gki_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_gki_device *chip = prv_data;
...
static __init int oplus_chg_gki_init(void)
{
	...
	if (node == NULL)
		...
		gki_dev->smart_charging_screenoff = of_property_read_bool(node, "oplus,smart_charging_screenoff");
