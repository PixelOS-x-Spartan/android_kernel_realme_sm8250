 ...
 *	True if the client's vote is enabled; false otherwise.
 */
bool is_client_vote_enabled_locked(struct votable *votable, const char *client_str)
{
	int client_id;
 ...
 */
int vote_override(struct votable *votable, const char *override_client, bool enabled, int val, bool step)
{
	int rc = 0;
...
static ssize_t votable_status_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < votable->num_clients; i++) {
		if (votable->client_strs[i]) {
			len += snprintf(status_buf + len, PAGE_SIZE - len, "%s: %s:\t\t\ten=%d v=%d\n", votable->name,
					votable->client_strs[i], votable->votes[i].enabled, votable->votes[i].value);
		}
	}
	...
	len += snprintf(status_buf + len, PAGE_SIZE - len, "%s: effective=%s type=%s v=%d\n", votable->name,
			effective_client_str ? effective_client_str : "none", type_str,
			get_effective_result_locked(votable));
	unlock_votable(votable);

	rc = simple_read_from_buffer(buff, count, off, status_buf, (len < PAGE_SIZE ? len : PAGE_SIZE));
...
static ssize_t votable_force_active_write(struct file *file, const char __user *buff, size_t len, loff_t *data)
{
	struct votable *votable = pde_data(file_inode(file));
	char buf[128] = { 0 };
	u32 val;
...
static ssize_t votable_force_active_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	struct votable *votable = pde_data(file_inode(file));
	char buf[128] = { 0 };
	size_t len;
	...
	rc = simple_read_from_buffer(buff, count, off, buf, (len < ARRAY_SIZE(buf) ? len : ARRAY_SIZE(buf)));
...
static ssize_t votable_force_val_write(struct file *file, const char __user *buff, size_t len, loff_t *data)
{
	struct votable *votable = pde_data(file_inode(file));
	char buf[128] = { 0 };
	u32 val;
...
static ssize_t votable_force_val_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	struct votable *votable = pde_data(file_inode(file));
	char buf[128] = { 0 };
	size_t len;
			       ...
			       void *data)
{
	...
	votable->status_ent = proc_create_data("status", S_IFREG | 0444, votable->root, &votable_status_ops, votable);
	...
	votable->force_val_ent =
		proc_create_data("force_val", S_IFREG | 0644, votable->root, &votable_force_val_ops, votable);
	...
	votable->force_active_ent =
		proc_create_data("force_active", S_IFREG | 0444, votable->root, &votable_force_active_ops, votable);
