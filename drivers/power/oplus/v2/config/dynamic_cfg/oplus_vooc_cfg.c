...
static void oplus_vooc_update_temp_region_config(struct oplus_param_head *param_head,
...
static void oplus_vooc_update_soc_region_config(struct oplus_param_head *param_head,
						struct oplus_vooc_spec_config *spec)
{
	struct oplus_cfg_data_head *data_head;
...
static void oplus_vooc_update_spec_misc_config(struct oplus_param_head *param_head, struct oplus_vooc_spec_config *spec)
{
	struct oplus_cfg_data_head *data_head;
	int32_t buf;
	int rc;
...
static void oplus_vooc_update_mcu_bad_volt_config(struct oplus_param_head *param_head,
						  struct oplus_vooc_spec_config *spec)
{
	struct oplus_cfg_data_head *data_head;
...
struct batt_bcc_curves *g_bcc_curves_metadata[] = { bcc_curves_soc0_2_50, bcc_curves_soc50_2_75, bcc_curves_soc75_2_85,
...
static int oplus_get_strategy_data(struct oplus_param_head *param_head, const char *name,
				   struct batt_bcc_curves *metadata[])
{
	struct oplus_cfg_data_head *data_head;
	ssize_t data_len;
	...
	for (i = 0; i < BCC_BATT_SOC_90_TO_100; i++) {
		for (j = 0; j < BATT_BCC_CURVE_MAX; j++) {
			...
			rc = oplus_cfg_get_data(
				data_head, tmp_buf + (BATT_BCC_CURVE_MAX * i + j) * BATT_BCC_ROW_MAX * range_data_size,
				data_len);
			if (rc < 0) {
		...
		for (j = 0; j < BATT_BCC_CURVE_MAX; j++) {
			...
			for (m = 0; m < curv_num[i][j]; m++)
				chg_info("[TEST]:%s:%s:%s[%d]: %u %u %u %d\n", name, strategy_soc[i], strategy_temp[j],
					 m, tmp[m].target_volt, tmp[m].max_ibus, tmp[m].min_ibus, tmp[m].exit);
		}
	}
...
static void oplus_vooc_update_bcc_curve_config(struct oplus_param_head *param_head, struct oplus_vooc_spec_config *spec)
{
	int rc;
...
static void oplus_vooc_update_misc_config(struct oplus_param_head *param_head, struct oplus_vooc_config *config)
{
...
static void oplus_vooc_update_strategy_config(struct oplus_param_head *param_head, struct oplus_vooc_config *config)
{
	struct oplus_chg_vooc *chip;
	struct oplus_cfg_data_head *data_head;
	...
	if (g_log_buf) {
		...
		for (i = 0; i < (data_len / sizeof(u32)); i++) {
			index += snprintf(g_log_buf + index, LOG_BUF_SIZE - index - 1, "%s%d", (i == 0) ? "" : ", ",
					  le32_to_cpu(*((u32 *)(data_buf + i * sizeof(u32)))));
			g_log_buf[index] = 0;
		}
