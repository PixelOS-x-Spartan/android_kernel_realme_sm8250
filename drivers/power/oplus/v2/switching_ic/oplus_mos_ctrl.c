...
static int oplus_mos_en_gpio_init(struct chip_mos_ctl *dev)
{
	...
	dev->mos_en_gpio_active = pinctrl_lookup_state(dev->mos_en_pinctrl, "mos_en_gpio_active");
	if (IS_ERR_OR_NULL(dev->mos_en_gpio_active)) {
		chg_err("get mos_en_gpio_active fail\n");
		return -EINVAL;
	}

	dev->mos_en_gpio_sleep = pinctrl_lookup_state(dev->mos_en_pinctrl, "mos_en_gpio_sleep");
...
static int oplus_mos_ctrl_set_enable(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	...
	if (!mos_ctl->mos_en_pinctrl || !mos_ctl->mos_en_gpio_active || !mos_ctl->mos_en_gpio_sleep) {
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_mos_ctl_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_mos_ctl_exit);
		break;
	case OPLUS_IC_FUNC_SET_HW_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_HW_ENABLE, oplus_mos_ctrl_set_enable);
		break;
	case OPLUS_IC_FUNC_GET_HW_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_HW_ENABLE, oplus_mos_ctrl_get_enable);
		break;
	default:
...
static int oplus_mos_ctrl_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(mos_ic->dev->of_node, "oplus,ic_type", &ic_type);
	if (rc < 0) {
		chg_err("can't get ic type, rc=%d\n", rc);
		goto error;
	}
	rc = of_property_read_u32(mos_ic->dev->of_node, "oplus,ic_index", &ic_index);
	if (rc < 0) {
