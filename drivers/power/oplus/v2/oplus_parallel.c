...
enum { MAIN_TEMP_INIT, SUB_TEMP_INIT, ALL_TEMP_INIT };
...
__maybe_unused static bool is_err_topic_available(struct oplus_parallel_chip *chip)
{
	if (!chip->err_topic)
...
__maybe_unused static bool is_wired_charging_disable_votable_available(struct oplus_parallel_chip *chip)
{
...
int oplus_switching_hw_enable(int en)
{
	...
	rc = oplus_chg_ic_func(g_parallel_chip->ic_dev, OPLUS_IC_FUNC_SET_HW_ENABLE, en);
	if (rc < 0) {
...
int oplus_switching_get_hw_enable(void)
{
	...
	rc = oplus_chg_ic_func(g_parallel_chip->ic_dev, OPLUS_IC_FUNC_GET_HW_ENABLE, &en);
	if (rc < 0) {
...
static ssize_t oplus_chg_read(struct file *fp, char __user *buff, size_t count, loff_t *off)
{
	...
	len = sprintf(page, "sub_current=%d\nsub_voltage=%d\nsub_soc=%d\nsub_temperature=%d\nmain_soc=%d\n",
		      g_parallel_chip->sub_ibat_ma, g_parallel_chip->sub_batt_volt, g_parallel_chip->sub_batt_soc,
		      g_parallel_chip->sub_batt_temp, g_parallel_chip->main_batt_soc);
...
static int oplus_switching_get_if_need_balance_bat(int vbat0_mv, int vbat1_mv)
{
	...
	if (!chip) {
		...
		if (oplus_switching_get_error_status() && oplus_chg_parallel_support() == PARALLEL_SWITCH_IC) {
		...
		if (oplus_vooc_get_fastchg_started() == true && atomic_read(&chip->mos_lock)) {
			if (oplus_switching_get_hw_enable() &&
			    ...
			    (abs(chip->main_ibat_ma - chip->sub_ibat_ma) >=
			     g_parallel_chip->parallel_mos_abnormal_gap_curr)) {
		...
		if (oplus_chg_parallel_support() == PARALLEL_MOS_CTRL && atomic_read(&chip->mos_lock) &&
		    fg_error_count == 0) {
			if (chip->tbatt_status != TEMP_REGION_WARM && chip->sub_batt_full && !chip->main_batt_full &&
			    chip->wired_online && diff_volt < g_parallel_chip->parallel_vbat_gap_full && mmi_chg)
			...
			if ((pre_error_reason & REASON_SUB_BATT_FULL) && !oplus_switching_get_hw_enable() &&
			    (diff_volt > g_parallel_chip->parallel_vbat_gap_full || !mmi_chg)) {
				error_reason &= ~REASON_SUB_BATT_FULL;
...
static int oplus_parallel_parse_dt(struct oplus_parallel_chip *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,parallel_mos_abnormal_litter_curr",
...
static int oplus_chg_parallel_support(void)
{
	...
	if (!g_parallel_chip) {
		...
		rc = oplus_chg_ic_func(g_parallel_chip->ic_dev, OPLUS_IC_FUNC_GET_DEVICE_TYPE, &type);
		if (rc < 0) {
...
static void parallel_battery_anti_shake(int change_type, int *bat_decidegc, int temp_status, int low_shake,
					int high_shake, int low_shake_0c, int high_shake_0c)
{
	struct oplus_parallel_chip *chip = g_parallel_chip;

	oplus_init_parallel_temp_threshold(change_type);
	if (temp_status == BATTERY_STATUS__HIGH_TEMP) {
		bat_decidegc[BATTERY_STATUS__HIGH_TEMP] = chip->anti_shake_bound[BATTERY_STATUS__HIGH_TEMP] + low_shake;
		...
		bat_decidegc[BATTERY_STATUS__COOL_TEMP] = chip->anti_shake_bound[BATTERY_STATUS__COOL_TEMP] + low_shake;
		...
		bat_decidegc[BATTERY_STATUS__NORMAL] = chip->anti_shake_bound[BATTERY_STATUS__NORMAL] + high_shake;
	} else if (temp_status == BATTERY_STATUS__NORMAL) {
		bat_decidegc[BATTERY_STATUS__NORMAL] = chip->anti_shake_bound[BATTERY_STATUS__NORMAL] + low_shake;
		...
		bat_decidegc[BATTERY_STATUS__WARM_TEMP] = chip->anti_shake_bound[BATTERY_STATUS__WARM_TEMP] + low_shake;
...
static void parallel_battery_anti_shake_handle(int status_change, int main_temp, int sub_temp)
{
	...
	if (status_change == MAIN_STATUS_CHG || status_change == ALL_STATUS_CHG) {
		...
		chg_err("MAIN_BAT [%d-%d-%d-%d-%d-%d-%d] t=[%d %d] s=%d\n",
			...
			chip->main_bat_decidegc[BATTERY_STATUS__HIGH_TEMP], chip->main_pre_shake, tbatt_cur_shake,
	...
	if (status_change == SUB_STATUS_CHG || status_change == ALL_STATUS_CHG) {
		...
		parallel_battery_anti_shake(SUB_STATUS_CHG, chip->sub_bat_decidegc, chip->sub_tbatt_status, low_shake,
					    high_shake, low_shake_0c, high_shake_0c);
		chg_err("SUB_BAT [%d-%d-%d-%d-%d-%d-%d] t=[%d %d] s=%d\n",
			...
			chip->sub_bat_decidegc[BATTERY_STATUS__HIGH_TEMP], chip->sub_pre_shake, tbatt_cur_shake,
...
static int oplus_vooc_fastchg_disable(const char *client_str, bool disable)
{
	...
	rc = vote(chip->vooc_disable_votable, client_str, disable, disable ? 1 : 0, false);
	if (rc < 0)
...
static int oplus_comm_chg_disable(const char *client_str, bool disable)
{
	...
	rc = vote(chip->chg_disable_votable, client_str, disable, disable ? 1 : 0, false);
	if (rc < 0)
...
static int oplus_comm_chg_vote_fcc(const char *client_str, int curr)
{
	...
	rc = vote(chip->wired_fcc_votable, client_str, true, curr, false);
	if (rc < 0)
...
static int oplus_comm_chg_disable_vote_fcc(const char *client_str)
{
	...
	rc = vote(chip->wired_fcc_votable, client_str, false, 0, false);
	if (rc < 0)
...
static void parallel_chg_set_balance_bat_status(int status)
{
	...
	if (chip->balancing_bat_status != status) {
		...
		msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, SWITCH_ITEM_STATUS);
		if (msg == NULL) {
		...
		msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, SWITCH_ITEM_HW_ENABLE_STATUS);
		if (msg == NULL) {
			chg_err("alloc msg error\n");
			return;
		}
...
static int parallel_chg_check_balance_bat_status(void)
{
	...
	if (chip->vooc_online_keep && !oplus_vooc_get_fastchg_started() &&
	    status != PARALLEL_BAT_BALANCE_ERROR_STATUS8 && status != PARALLEL_BAT_BALANCE_ERROR_STATUS9) {
...
static void oplus_chg_switch_err_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
...
static int oplus_chg_switch_virq_register(struct oplus_parallel_chip *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->ic_dev, OPLUS_IC_VIRQ_ERR, oplus_chg_switch_err_handler, chip);
	if (rc < 0)
...
static int oplus_chg_switch_update_curr_limit(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_parallel_chip *chip;
...
static int oplus_chg_switch_update_hw_enable_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_parallel_chip *chip;
	int status;
...
static int oplus_chg_switch_update_err_notify(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_parallel_chip *chip;
...
static void oplus_switch_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	switch (type) {
	case MSG_TYPE_ITEM:
		switch (id) {
...
static void oplus_switch_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;

	chip->comm_topic = topic;
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_switch_comm_subs_callback, "chg_switch");
...
static void oplus_switch_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_ONLINE:
			...
			if (chip->wired_online != data.intval) {
				...
				if (chip->wired_online && oplus_chg_parallel_support() == PARALLEL_MOS_CTRL) {
...
static void oplus_switch_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_switch_wired_subs_callback, "chg_switch");
...
static void oplus_switch_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	switch (type) {
	case MSG_TYPE_ITEM:
		switch (id) {
...
static void oplus_switch_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_switch_vooc_subs_callback, "chg_switch");
...
static void oplus_switch_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	switch (type) {
	case MSG_TYPE_TIMER:
		break;
...
static void oplus_switch_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;

	chip->gauge_topic = topic;
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_switch_gauge_subs_callback, "chg_switch");
...
static void oplus_switch_main_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	switch (type) {
	case MSG_TYPE_TIMER:
		break;
...
static void oplus_switch_subscribe_main_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;
	int error_status = 0;
	int rc;
	...
	chip->main_gauge_subs =
		oplus_mms_subscribe(chip->gauge_topic, chip, oplus_switch_main_gauge_subs_callback, "chg_switch");
	if (IS_ERR_OR_NULL(chip->main_gauge_subs)) {
		chg_err("subscribe gauge topic error, rc=%ld\n", PTR_ERR(chip->main_gauge_subs));
		return;
	}
	if (!IS_ERR_OR_NULL(chip->sub_gauge_subs) && oplus_chg_parallel_support() == PARALLEL_MOS_CTRL) {
...
static void oplus_switch_sub_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	switch (type) {
	case MSG_TYPE_TIMER:
		break;
...
static void oplus_switch_subscribe_sub_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_parallel_chip *chip = prv_data;
	int error_status = 0;
	int rc;
	...
	chip->sub_gauge_subs =
		oplus_mms_subscribe(chip->gauge_topic, chip, oplus_switch_sub_gauge_subs_callback, "chg_switch");
	if (IS_ERR_OR_NULL(chip->sub_gauge_subs)) {
		chg_err("subscribe gauge topic error, rc=%ld\n", PTR_ERR(chip->sub_gauge_subs));
		return;
	}
	if (!IS_ERR_OR_NULL(chip->main_gauge_subs) && oplus_chg_parallel_support() == PARALLEL_MOS_CTRL) {
		update_batt_info(chip, true);
		if (chip->sub_batt_temp == FG_I2C_ERROR || chip->main_batt_temp == FG_I2C_ERROR) {
			chg_err("gauge i2c error! disable mos\n");
			error_status = REASON_I2C_ERROR;
...
static int update_batt_info(struct oplus_parallel_chip *chip, bool update)
{
	...
	if (is_support_parallel_battery(chip->gauge_topic)) {
		oplus_mms_get_item_data(chip->main_gauge_topic, GAUGE_ITEM_VOL_MAX, &data, update);
		chip->batt_volt = data.intval;
		oplus_mms_get_item_data(chip->main_gauge_topic, GAUGE_ITEM_CURR, &data, update);
		chip->main_ibat_ma = data.intval;
		oplus_mms_get_item_data(chip->main_gauge_topic, GAUGE_ITEM_TEMP, &data, update);
		chip->main_batt_temp = data.intval;
		oplus_mms_get_item_data(chip->main_gauge_topic, GAUGE_ITEM_SOC, &data, update);
		chip->main_batt_soc = data.intval;
		oplus_mms_get_item_data(chip->sub_gauge_topic, GAUGE_ITEM_VOL_MAX, &data, update);
		chip->sub_batt_volt = data.intval;
		oplus_mms_get_item_data(chip->sub_gauge_topic, GAUGE_ITEM_CURR, &data, update);
		chip->sub_ibat_ma = data.intval;
		oplus_mms_get_item_data(chip->sub_gauge_topic, GAUGE_ITEM_TEMP, &data, update);
		chip->sub_batt_temp = data.intval;
		oplus_mms_get_item_data(chip->sub_gauge_topic, GAUGE_ITEM_SOC, &data, update);
...
{
	struct oplus_parallel_chip *chip = container_of(work, struct oplus_parallel_chip, err_handler_work);
...
static int oplus_parallel_get_track_info(char *temp_str, int index)
{
	...
	return snprintf(&(temp_str[index]), REASON_LENGTH_MAX - index,
			...
			"main_sub_temp %d %d,"
			"mos %s,",
			chip->main_batt_soc, chip->sub_batt_soc, chip->batt_volt, chip->sub_batt_volt,
			chip->main_ibat_ma, chip->sub_ibat_ma, chip->main_batt_temp, chip->sub_batt_temp,
...
static void oplus_parallel_track_check(void)
{
	...
	if (((chip->main_batt_full && !chip->soc_not_full_report &&
	      ...
	      (chip->tbatt_status != TEMP_REGION_COLD && chip->tbatt_status != TEMP_REGION_WARM &&
...
static void oplus_chg_parallel_check_work(struct work_struct *work)
{
	...
	struct oplus_parallel_chip *chip = container_of(dwork, struct oplus_parallel_chip, parallel_check_work);

	if (IS_ERR_OR_NULL(chip->main_gauge_subs) || IS_ERR_OR_NULL(chip->sub_gauge_subs)) {
...
subs_error:
	if (chip->wired_online)
		schedule_delayed_work(&chip->parallel_check_work, OPLUS_CHG_UPDATE_INTERVAL(CHECK_TIME_WITH_CHARGER));
	else
...
static void oplus_parallel_curr_limit_publish(struct oplus_parallel_chip *chip)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, SWITCH_ITEM_CURR_LIMIT);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_parallel_curr_spec_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_parallel_chip *chip =
		container_of(dwork, struct oplus_parallel_chip, parallel_curr_spec_check_work);
	int target_curr = 0;
	int spec_status;
	...
	update_batt_info(chip, true);
	spec_status = oplus_chg_is_parellel_ibat_over_spec(chip->main_batt_temp, chip->sub_batt_temp, &target_curr);
...
static void oplus_parallel_chg_mos_test_work(struct work_struct *work)
{
	...
	if (!oplus_switching_get_hw_enable() || chip->balancing_bat_status == PARALLEL_BAT_BALANCE_ERROR_STATUS8 ||
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_parallel_chip *chip = container_of(dwork, struct oplus_parallel_chip, parallel_init_work);
...
init_try_again:
	if (retry > 0) {
		retry--;
		schedule_delayed_work(&chip->parallel_init_work, msecs_to_jiffies(OPLUS_CHG_IC_INIT_RETRY_DELAY));
	} else {
...
static int oplus_chg_parallel_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_parallel_chip), GFP_KERNEL);
	if (chip == NULL) {
