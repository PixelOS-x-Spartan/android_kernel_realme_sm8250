...
static int oplus_svooc_curr_table[CURR_LIMIT_MAX - 1] = { 2500, 2000, 3000, 4000, 5000, 6500 };
static int oplus_vooc_curr_table[CURR_LIMIT_MAX - 1] = { 3600, 2500, 3000, 4000, 5000, 6000 };
static int oplus_vooc_7bit_curr_table[CURR_LIMIT_7BIT_MAX - 1] = {
	1500, 1500, 2000, 2500, 3000, 3500, 4000,  4500,  5000,	 5500,	6000,  6300,  6500,
...
static int oplus_cp_7bit_curr_table[CP_CURR_LIMIT_7BIT_MAX - 1] = {
	1000, 1000, 1200, 1500, 1700, 2000, 2200, 2500, 2700, 3000, 3200, 3500, 3700,  4000,
	4500, 5000, 5500, 6000, 6300, 6500, 7000, 7500, 8000, 8500, 9000, 9500, 10000,
};
...
struct current_level svooc_1_0_curr_table[] = { { 1, 2000 },   { 2, 2000 },   { 3, 2000 },   { 4, 3000 },  { 5, 4000 },
...
static int oplus_vooc_get_voocphy_bcc_fastchg_ing(struct oplus_mms *mms, union mms_msg_data *data);
...
static unsigned int oplus_get_adapter_sid(struct oplus_chg_vooc *chip, unsigned char id)
{
	...
	for (i = 0; i < ARRAY_SIZE(adapter_id_table); i++) {
		...
		if (id >= adapter_info->id_min && id <= adapter_info->id_max) {
			if (!chip->config.svooc_support && adapter_info->adapter_chg_type == CHARGER_TYPE_SVOOC) {
			...
			sid = adapter_info_to_sid(id, power, adapter_info->adapter_type, adapter_chg_type);
...
__maybe_unused static bool is_wired_charging_disable_votable_available(struct oplus_chg_vooc *chip)
{
...
__maybe_unused static bool is_wired_charge_suspend_votable_available(struct oplus_chg_vooc *chip)
{
...
static void oplus_vooc_set_vooc_started(struct oplus_chg_vooc *chip, bool started)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_VOOC_STARTED);
	if (msg == NULL) {
...
static void oplus_vooc_set_online(struct oplus_chg_vooc *chip, bool online)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_ONLINE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_vooc_set_online_keep(struct oplus_chg_vooc *chip, bool keep)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_ONLINE_KEEP);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_vooc_set_vooc_charging(struct oplus_chg_vooc *chip, bool charging)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_VOOC_CHARGING);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_set_fast_status(struct oplus_chg_vooc *chip, enum oplus_fast_chg_status status)
{
...
static void oplus_select_abnormal_max_cur(struct oplus_chg_vooc *chip)
{
	...
	if (chip->support_abnormal_over_80w_adapter && index > 0 &&
		...
		if (index < chip->abnormal_over_80w_adapter_cur_arraycnt) {
			chip->abnormal_allowed_current_max = config->abnormal_over_80w_adapter_cur_array[index];
		} else {
			chip->abnormal_allowed_current_max = config->abnormal_over_80w_adapter_cur_array[0];
		}
	} else if (chip->support_abnormal_adapter && index > 0 && config->abnormal_adapter_cur_array != NULL) {
		if (index < chip->abnormal_adapter_cur_arraycnt) {
			chip->abnormal_allowed_current_max = config->abnormal_adapter_cur_array[index];
		} else {
			chip->abnormal_allowed_current_max = config->abnormal_adapter_cur_array[0];
		}
	}

	chg_info("abnormal info [%d %d %d %d %d %d %d]\n", chip->pd_svooc, chip->support_abnormal_adapter,
		 chip->support_abnormal_over_80w_adapter, chip->abnormal_adapter_dis_cnt,
		 chip->abnormal_adapter_cur_arraycnt, chip->abnormal_over_80w_adapter_cur_arraycnt,
...
static void oplus_vooc_set_sid(struct oplus_chg_vooc *chip, unsigned int sid)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_SID);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_vooc_chg_bynormal_path(struct oplus_chg_vooc *chip)
{
	...
	if (config->support_vooc_by_normal_charger_path && sid_to_adapter_chg_type(chip->sid) == CHARGER_TYPE_VOOC)
		chip->vooc_chg_bynormal_path = true;
	else
		chip->vooc_chg_bynormal_path = false;

	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_VOOC_BY_NORMAL_PATH);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static bool oplus_vooc_batt_volt_is_good(struct oplus_chg_vooc *chip)
{
	...
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_TEMP_REGION, &data, false);
	if (rc < 0) {
	...
	if (is_wired_charge_suspend_votable_available(chip) && get_effective_result(chip->wired_charge_suspend_votable))
...
static void oplus_vooc_bad_volt_check(struct oplus_chg_vooc *chip)
{
	...
	if (config->vooc_bad_volt_check_support) {
		if (oplus_vooc_batt_volt_is_good(chip)) {
			vote(chip->vooc_not_allow_votable, BAD_VOLT_VOTER, false, 0, false);
		} else {
			vote(chip->vooc_not_allow_votable, BAD_VOLT_VOTER, true, 1, false);
...
static bool oplus_fastchg_is_allow_retry(struct oplus_chg_vooc *chip)
{
	...
	if (!chip->wired_online || !is_client_vote_enabled(chip->vooc_disable_votable, TIMEOUT_VOTER))
		return false;
...
static void oplus_fastchg_check_retry(struct oplus_chg_vooc *chip)
{
	...
	if (chip->bat_temp_region <= TEMP_REGION_LITTLE_COLD && temp_region >= TEMP_REGION_COOL &&
	...
	if (chip->bat_temp_region >= TEMP_REGION_WARM && temp_region <= TEMP_REGION_NORMAL &&
...
static void oplus_vooc_fastchg_allow_or_enable_check(struct oplus_chg_vooc *chip)
{
	struct oplus_vooc_spec_config *spec = &chip->spec;
	struct oplus_vooc_config *config = &chip->config;
	union mms_msg_data data = { 0 };
	...
	if (is_client_vote_enabled(chip->vooc_not_allow_votable, BATT_TEMP_VOTER)) {
		if (chip->temperature > spec->vooc_low_temp && chip->temperature < spec->vooc_high_temp) {
			chg_info("allow fastchg, temp=%d\n", chip->temperature);
			vote(chip->vooc_not_allow_votable, BATT_TEMP_VOTER, false, 0, false);
		}
	}
	if (is_client_vote_enabled(chip->vooc_not_allow_votable, BATT_SOC_VOTER)) {
		if (chip->ui_soc >= spec->vooc_low_soc && chip->ui_soc <= spec->vooc_high_soc) {
			chg_info("allow fastchg, soc=%d\n", chip->ui_soc);
			vote(chip->vooc_not_allow_votable, BATT_SOC_VOTER, false, 0, false);
		}
	}

	if (config->vooc_bad_volt_check_support &&
	    is_client_vote_enabled(chip->vooc_not_allow_votable, BAD_VOLT_VOTER)) {
		if (oplus_vooc_batt_volt_is_good(chip)) {
			vote(chip->vooc_not_allow_votable, BAD_VOLT_VOTER, false, 0, false);
		}
	}
	...
	if (is_client_vote_enabled(chip->vooc_not_allow_votable, WARM_SOC_VOTER)) {
		if (chip->ui_soc < spec->vooc_warm_soc_thr ||
		    chip->temperature < (chip->efficient_vooc_normal_high_temp)) {
			vote(chip->vooc_not_allow_votable, WARM_SOC_VOTER, false, 0, false);
		}
	}
	if (is_client_vote_enabled(chip->vooc_not_allow_votable, WARM_VOL_VOTER)) {
...
enable_check:
	if (!chip->fastchg_disable)
		return;
	if (is_client_vote_enabled(chip->vooc_disable_votable, WARM_FULL_VOTER)) {
		if (chip->temperature < (chip->efficient_vooc_normal_high_temp - BATT_TEMP_HYST)) {
			vote(chip->vooc_disable_votable, WARM_FULL_VOTER, false, 0, false);
		}
	}

	if (is_client_vote_enabled(chip->vooc_disable_votable, BATT_TEMP_VOTER)) {
		if (chip->temperature > spec->vooc_low_temp && chip->temperature < spec->vooc_high_temp)
			vote(chip->vooc_disable_votable, BATT_TEMP_VOTER, false, 0, false);
	}

	if (is_client_vote_enabled(chip->vooc_disable_votable, SWITCH_RANGE_VOTER) &&
	...
	if (is_client_vote_enabled(chip->vooc_disable_votable, CURR_LIMIT_VOTER)) {
		if (is_parallel_topic_available(chip)) {
			oplus_mms_get_item_data(chip->parallel_topic, SWITCH_ITEM_CURR_LIMIT, &data, true);
			curr_limit = data.intval;
			oplus_mms_get_item_data(chip->parallel_topic, SWITCH_ITEM_HW_ENABLE_STATUS, &data, true);
		...
		if (!chip->check_curr_delay && (curr_limit == 0 || curr_limit >= FASTCHG_MIN_CURR) &&
		    (mos_status ||
		     (!mos_status && main_soc >= spec->vooc_low_soc && main_soc <= spec->vooc_high_soc))) {
...
static bool oplus_vooc_is_allow_fast_chg(struct oplus_chg_vooc *chip)
{
	...
	if (chip->temperature < spec->vooc_low_temp || chip->temperature > spec->vooc_high_temp) {
		vote(chip->vooc_not_allow_votable, BATT_TEMP_VOTER, true, 1, false);
	} else {
		vote(chip->vooc_not_allow_votable, BATT_TEMP_VOTER, false, 0, false);
	}
	if (chip->ui_soc < spec->vooc_low_soc || chip->ui_soc > spec->vooc_high_soc) {
		vote(chip->vooc_not_allow_votable, BATT_SOC_VOTER, true, 1, false);
	} else {
		vote(chip->vooc_not_allow_votable, BATT_SOC_VOTER, false, 0, false);
	}
	...
	if (chip->temperature > chip->efficient_vooc_normal_high_temp) {
		if (chip->ui_soc >= spec->vooc_warm_soc_thr)
			vote(chip->vooc_not_allow_votable, WARM_SOC_VOTER, true, 1, false);
		else
			vote(chip->vooc_not_allow_votable, WARM_SOC_VOTER, false, 0, false);

		if (chip->batt_volt >= spec->vooc_warm_vol_thr)
			vote(chip->vooc_not_allow_votable, WARM_VOL_VOTER, true, 1, false);
		else
			vote(chip->vooc_not_allow_votable, WARM_VOL_VOTER, false, 0, false);
	} else {
		vote(chip->vooc_not_allow_votable, WARM_SOC_VOTER, false, 0, false);
	...
	fastchg_to_warm_full = is_client_vote_enabled(chip->vooc_disable_votable, WARM_FULL_VOTER);
	...
	if (fastchg_to_warm_full && chip->temperature > warmfull_fastchg_temp) {
		vote(chip->vooc_not_allow_votable, WARM_FULL_VOTER, true, 1, false);
		chg_info(" oplus_vooc_get_fastchg_to_warm_full is true\n");
	} else {
		vote(chip->vooc_not_allow_votable, WARM_FULL_VOTER, false, 0, false);
	}
	...
	if (btb_check_cnt == 0) {
		...
		if (chip->wired_icl_votable)
			vote(chip->wired_icl_votable, BTB_TEMP_OVER_VOTER, true, BTB_TEMP_OVER_MAX_INPUT_CUR, true);
	}
...
static void oplus_vooc_switch_fast_chg(struct oplus_chg_vooc *chip)
{
	...
	if (!is_client_vote_enabled(chip->vooc_disable_votable, UPGRADE_FW_VOTER) &&
...
static void oplus_qc_check_setup_timer(struct oplus_chg_vooc *chip, unsigned int ms)
{
...
static void oplus_vooc_setup_watchdog_timer(struct oplus_chg_vooc *chip, unsigned int ms)
{
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0))
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, vooc_watchdog_work);
...
static void oplus_vooc_switch_check_work(struct work_struct *work)
{
	...
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, vooc_switch_check_work);
	int chg_type;
	bool present = false;
	...
	if (!chip->cpa_support) {
		...
		if ((chg_type == OPLUS_CHG_USB_TYPE_PD_PPS || chg_type == OPLUS_CHG_USB_TYPE_PD) && !chip->pd_svooc) {
			chip->switch_retry_count = 0;
			chg_info("pd_svooc=false\n");
			oplus_vooc_cpa_switch_end(chip);
			return;
	...
	chg_info("switch_retry_count=%d, fast_chg_status=%d fastchg_check_timeout=%lu\n", chip->switch_retry_count,
		 chip->fast_chg_status, fastchg_check_timeout);
	if (chip->switch_retry_count == 0) {
		if ((chip->fast_chg_status == CHARGER_STATUS_SWITCH_TEMP_RANGE ||
		    ...
		    oplus_vooc_is_allow_fast_chg(chip) && is_wired_charge_suspend_votable_available(chip)) {
		...
		if (chip->wired_online && (oplus_chg_vooc_get_switch_mode(chip->vooc_ic) != VOOC_SWITCH_MODE_VOOC)) {
			switch_fast_chg(chip->vooc_ic);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, check_charger_out_work);
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_CC_DETECT, &topic_data, true);
	...
	if (!wired_online || chip->fastchg_force_exit) {
		...
		chip->icon_debounce = false;
		if (!is_client_vote_enabled(chip->vooc_disable_votable, FASTCHG_DUMMY_VOTER) || !wired_online) {
			oplus_vooc_set_online(chip, false);
			oplus_vooc_set_sid(chip, 0);
			oplus_vooc_chg_bynormal_path(chip);
		}
...
static void oplus_vooc_fastchg_exit(struct oplus_chg_vooc *chip, bool restore_nor_chg)
{
	...
	if (chip->wired_online && !get_effective_result(chip->vooc_disable_votable))
		oplus_vooc_switch_request(chip);
}

static int oplus_vooc_get_min_curr_level(struct oplus_chg_vooc *chip, int level_base, int level_new, unsigned int sid,
					 bool fw_7bit)
{
	...
	if (fw_7bit) {
		...
		if (level_base >= max_level || level_new >= max_level) {
...
static int oplus_vooc_get_temp_range(struct oplus_chg_vooc *chip, int vbat_temp_cur)
{
	...
	chg_info("vooc_temp_cur_range[%d], vbat_temp_cur[%d]", chip->vooc_temp_cur_range, vbat_temp_cur);
...
static void oplus_vooc_reset_temp_range(struct oplus_chg_vooc *chip)
{
	...
	chg_info("[%d %d %d %d %d]\n", chip->efficient_vooc_little_cold_temp, chip->efficient_vooc_cool_temp,
		 chip->efficient_vooc_little_cool_temp, chip->efficient_vooc_normal_low_temp,
...
static void oplus_vooc_rang_rise_update(struct oplus_chg_vooc *chip)
{
	...
	if (pre_vooc_temp_rang == FASTCHG_TEMP_RANGE_WARM ||
	    (chip->spec.vooc_normal_high_temp == -EINVAL && pre_vooc_temp_rang == FASTCHG_TEMP_RANGE_NORMAL_HIGH)) {
		...
		if (oplus_get_cur_ui_soc(chip) <= COOL_REANG_SWITCH_LIMMIT_SOC) {
			oplus_set_fast_status(chip, CHARGER_STATUS_SWITCH_TEMP_RANGE);
			chip->fastchg_batt_temp_status = BAT_TEMP_EXIT;
			chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_INIT;
		} else {
...
static void oplus_vooc_check_temp_range(struct oplus_chg_vooc *chip, int cur_temp)
{
	...
	if ((cur_temp < chip->spec.vooc_over_low_temp || cur_temp > chip->spec.vooc_over_high_temp) &&
...
static int oplus_vooc_fastchg_process(struct oplus_chg_vooc *chip)
{
	...
	if (chip->fastchg_allow) {
		...
		if (is_wired_charging_disable_votable_available(chip) &&
		    !is_client_vote_enabled(chip->wired_charging_disable_votable, FASTCHG_VOTER)) {
		...
		if (config->support_vooc_by_normal_charger_path) {
			if (is_wired_charge_suspend_votable_available(chip) &&
			    !is_client_vote_enabled(chip->wired_charge_suspend_votable, FASTCHG_VOTER)) {
				normal_chg_disable = false;
			}
			...
			if (!normal_chg_disable && (chip->sid != 0) &&
				...
				if (is_wired_charging_disable_votable_available(chip)) {
					vote(chip->wired_charging_disable_votable, FASTCHG_VOTER, true, 1, false);
				}
				if (is_wired_charge_suspend_votable_available(chip)) {
					vote(chip->wired_charge_suspend_votable, FASTCHG_VOTER, true, 1, false);
				}
			...
			if (!normal_chg_disable && chip->icharging < -2000 && !chip->enable_dual_chan) {
				vote(chip->wired_charging_disable_votable, FASTCHG_VOTER, true, 1, false);
			}
		}
	}
	...
	if (chip->cool_down > 0) {
		ret_info = oplus_vooc_get_min_curr_level(chip, ret_info, chip->cool_down, chip->sid,
							 config->data_width == 7);
	}
	...
	chg_info("volt=%d, temp=%d, soc=%d, uisoc=%d, curr=%d, cool_down=%d\n", chip->batt_volt, chip->temperature,
		 chip->soc, chip->ui_soc, chip->icharging, chip->cool_down);
	...
	if (pre_curve_idx == chip->bcc_curve_idx) {
		...
		if (idx_cnt >= 20) {
			...
			if (chip->bcc_curve_idx < chip->svooc_batt_curve[0].bcc_curv_num)
				tem_curr = chip->svooc_batt_curve[0].batt_bcc_curve[chip->bcc_curve_idx + 1].max_ibus;
			else
				tem_curr = chip->svooc_batt_curve[0].batt_bcc_curve[chip->bcc_curve_idx].min_ibus;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, boot_fastchg_allow_work);
	union mms_msg_data data = { 0 };
	int rc;
	...
	if ((rc < 0 || chip->temperature == GAUGE_INVALID_TEMP) && retry_cnt > 0) {
		schedule_delayed_work(&chip->boot_fastchg_allow_work,
				      msecs_to_jiffies(500)); /*500ms check,detection 15s*/
		retry_cnt--;
	} else {
...
static int oplus_vooc_push_break_code(struct oplus_chg_vooc *chip, int code)
{
	...
	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, VOOC_ITEM_BREAK_CODE, code);
	if (msg == NULL) {
...
static bool oplus_vooc_fastchg_range_switch(struct oplus_chg_vooc *chip)
{
	...
	if (chip->fastchg_batt_temp_status == BAT_TEMP_EXIT &&
		...
		if (chip->fast_chg_status == CHARGER_STATUS_FAST_TO_WARM) {
			vote(chip->vooc_disable_votable, BATT_TEMP_VOTER, true, 1, false);
			vote(chip->vooc_not_allow_votable, BATT_TEMP_VOTER, true, 1, false);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, adsp_recover_work);
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, fastchg_work);
	struct oplus_vooc_spec_config *spec = &chip->spec;
	struct oplus_vooc_config *config = &chip->config;
	enum oplus_wired_cc_detect_status cc_detect = CC_DETECT_NULL;
	union mms_msg_data topic_data = { 0 };
	...
	if (((data & 0xf0) != 0x50) && ((data & 0xf0) != 0x70) && (!fw_ver_info) && (!adapter_fw_ver_info) &&
	    (!chip->adapter_model_factory) && data != VOOC_NOTIFY_ABNORMAL_ADAPTER &&
	    ((is_support_parallel_battery(chip->gauge_topic) && data != VOOC_NOTIFY_CURR_LIMIT_SMALL) ||
	...
	case VOOC_NOTIFY_FAST_PRESENT:
		...
		if (oplus_vooc_is_allow_fast_chg(chip)) {
			...
			oplus_vooc_set_vooc_started(chip, false);
			vote(chip->vooc_disable_votable, FASTCHG_DUMMY_VOTER, true, 1, false);
	...
	case VOOC_NOTIFY_ALLOW_READING_IIC:
		...
		if (chip->general_strategy != NULL) {
			rc = oplus_chg_strategy_get_data(chip->general_strategy, &ret_tmp);
			if (rc < 0)
		...
		if ((chip->support_abnormal_adapter || chip->support_abnormal_over_80w_adapter) &&
		    chip->abnormal_adapter_dis_cnt > 0) {
			ret_info = oplus_vooc_get_min_curr_level(chip, ret_info, chip->abnormal_allowed_current_max,
	...
	case VOOC_NOTIFY_NORMAL_TEMP_FULL:
		charger_delay_check = true;
		if (spec->vooc_normal_high_temp != -EINVAL && chip->vooc_temp_cur_range == FASTCHG_TEMP_RANGE_WARM) {
			vote(chip->vooc_disable_votable, WARM_FULL_VOTER, true, 1, false);
			oplus_vooc_reset_temp_range(chip);
			oplus_set_fast_status(chip, CHARGER_STATUS_FAST_TO_WARM);
		} else {
			vote(chip->vooc_disable_votable, CHG_FULL_VOTER, true, 1, false);
	...
	case VOOC_NOTIFY_LOW_TEMP_FULL:
		...
		if (config->data_width == VOOC_FW_7BIT) {
			oplus_vooc_push_break_code(chip, TRACK_VOOCPHY_BREAK_DEFAULT);
	...
	case VOOC_NOTIFY_DATA_UNKNOWN:
		...
		if (chip->support_fake_vooc_check) {
			vote(chip->vooc_disable_votable, COPYCAT_ADAPTER, true, 1, false);
	...
	case VOOC_NOTIFY_BAD_CONNECTED:
		charger_delay_check = true;
		oplus_set_fast_status(chip, CHARGER_STATUS_FAST_TO_NORMAL);
		vote(chip->vooc_disable_votable, BAD_CONNECTED_VOTER, true, 1, false);
	...
	case VOOC_NOTIFY_CURR_LIMIT_SMALL:
		if (!chip->adapter_model_factory) {
			...
			vote(chip->vooc_disable_votable, CURR_LIMIT_VOTER, true, 1, false);
			oplus_vooc_fastchg_exit(chip, true);
		} else {
	...
	case VOOC_NOTIFY_CURR_LIMIT_MAX:
		charger_delay_check = true;
		oplus_set_fast_status(chip, CHARGER_STATUS_FAST_TO_NORMAL);
		vote(chip->vooc_disable_votable, SVOOC_CURR_OCP_VOTER, true, 1, false);
		oplus_vooc_push_break_code(chip, TRACK_MCU_VOOCPHY_CURR_OVER_ERROR);
		oplus_vooc_fastchg_exit(chip, true);
		ret_info = VOOC_DEF_REPLY_DATA;
		break;
	...
	case VOOC_NOTIFY_BTB_TEMP_OVER:
		...
		vote(chip->vooc_disable_votable, BTB_TEMP_OVER_VOTER, true, 1, false);
	...
	if (ignore_device_type)
		oplus_vooc_reply_data_no_type(chip->vooc_ic, ret_info, chip->config.data_width, vooc_curr);
	else
		oplus_vooc_reply_data(chip->vooc_ic, ret_info, oplus_gauge_get_device_type_for_vooc(),
				      chip->config.data_width, vooc_curr);
...
out:
	if (charger_delay_check) {
		...
		if (data == 0x74 && config->voocphy_support == ADSP_VOOCPHY && charger_delay_check_time != 0) {
	...
	if ((data == VOOC_NOTIFY_FAST_ABSENT && !charger_delay_check && !chip->icon_debounce) || data_err) {
		chip->fastchg_force_exit = true;
		schedule_delayed_work(&chip->check_charger_out_work, 0);
	}

	if ((chip->support_abnormal_adapter || chip->support_abnormal_over_80w_adapter) &&
	    data == VOOC_NOTIFY_FAST_ABSENT && chip->icon_debounce) {
		chip->fastchg_force_exit = false;
...
static void oplus_vooc_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_CC_DETECT:
			oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_CC_DETECT, &data, false);
		...
		case WIRED_TIME_TYPEC_STATE:
			oplus_mms_get_item_data(chip->wired_topic, WIRED_TIME_TYPEC_STATE, &data, false);
		...
		case WIRED_TIME_ABNORMAL_ADAPTER:
			oplus_mms_get_item_data(chip->wired_topic, WIRED_TIME_ABNORMAL_ADAPTER, &data, false);
			if (data.intval < 0) {
...
static void oplus_vooc_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_vooc_wired_subs_callback, "vooc");
	...
	chip->cc_detect = data.intval;
	oplus_mms_get_item_data(chip->wired_topic, WIRED_TIME_TYPEC_STATE, &data, true);
	chip->typec_state = data.intval;
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, plugin_work);
	union mms_msg_data data = { 0 };
	int ret;
	if (!chip->cpa_support)
	...
	if (chip->wired_online) {
		if (chip->comm_topic != NULL) {
			ret = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_TEMP_REGION, &data, true);
			if (ret < 0) {
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, chg_type_change_work);
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, abnormal_adapter_check_work);
	union mms_msg_data data = { 0 };
	int mmi_chg = 0;
	...
	if (chip->common_chg_suspend_votable)
		mmi_chg = !get_client_vote(chip->common_chg_suspend_votable, MMI_CHG_VOTER);
	...
	if (!chip->wired_present) {
		...
		chg_info("pre_is_abnormal_adapter = %d, is_abnormal_adapter = %d, cc_detect = %d.\n",
			 chip->pre_is_abnormal_adapter, chip->is_abnormal_adapter, cc_detect);
		chip->is_abnormal_adapter = 0;
		if (mmi_chg == 0 || chip->mcu_vote_detach || chip->fast_chg_status == CHARGER_STATUS_FAST_TO_WARM ||
		    chip->fast_chg_status == CHARGER_STATUS_CURR_LIMIT ||
		    chip->fast_chg_status == CHARGER_STATUS_FAST_TO_NORMAL ||
		    chip->fast_chg_status == CHARGER_STATUS_FAST_DUMMY ||
		    chip->fast_chg_status == CHARGER_STATUS_FAST_BTB_TEMP_OVER ||
		    chip->fast_chg_status == CHARGER_STATUS_SWITCH_TEMP_RANGE || !chip->fastchg_ing)
		...
		if (mmi_chg && chip->pre_is_abnormal_adapter && down_to_up_time <= ABNORMAL_ADAPTER_BREAK_CHECK_TIME &&
	...
	chg_info("%s [%d %d %d %d %d %d %d %d %d %d]\n", chip->wired_present == true ? "vbus up" : "vbus down", mmi_chg,
		 chip->fastchg_ing, chip->mcu_vote_detach, chip->pre_is_abnormal_adapter, chip->fast_chg_status,
		 chip->icon_debounce, chip->abnormal_adapter_dis_cnt, chip->fastchg_ing, cc_detect, down_to_up_time);
}

static void oplus_vooc_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_vooc *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		...
		case COMM_ITEM_UNWAKELOCK:
			oplus_mms_get_item_data(chip->comm_topic, id, &data, false);
			vote(chip->vooc_disable_votable, DEBUG_VOTER, data.intval, data.intval, false);
			break;
		default:
			break;
		}
...
static void oplus_vooc_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	bool disable;
	union mms_msg_data data = { 0 };
	...
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_vooc_comm_subs_callback, "vooc");
	...
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_TEMP_REGION, &data, true);
	chip->bat_temp_region = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_COOL_DOWN, &data, true);
	chip->cool_down = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_SHELL_TEMP, &data, true);
	chip->temperature = data.intval;
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_CHARGING_DISABLE, &data, true);
	if (rc < 0) {
	...
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_CHARGE_SUSPEND, &data, true);
	if (rc < 0) {
	...
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_UNWAKELOCK, &data, true);
	if (rc < 0) {
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, gauge_update_work);
	union mms_msg_data data = { 0 };
	int rc;
...
static void oplus_vooc_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_vooc *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	case MSG_TYPE_ITEM:
		...
		case GAUGE_ITEM_AUTH:
			...
			vote(chip->vooc_disable_votable, NON_STANDARD_VOTER, (!chip->batt_hmac || !chip->batt_auth), 0,
			     false);
			break;
		...
		case GAUGE_ITEM_SUBBOARD_TEMP_ERR:
			...
			if (rc < 0) {
				...
				if (!!data.intval) {
					...
					vote(chip->vooc_curr_votable, BAD_SUBBOARD_VOTER, false, 0, false);
				}
			}
			break;
		default:
...
static void oplus_vooc_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_vooc_gauge_subs_callback, "vooc");
	if (IS_ERR_OR_NULL(chip->gauge_subs)) {
		chg_err("subscribe gauge topic error, rc=%ld\n", PTR_ERR(chip->gauge_subs));
		return;
	}

	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_HMAC, &data, true);
	if (rc < 0) {
		chg_err("can't get GAUGE_ITEM_HMAC data, rc=%d\n", rc);
		chip->batt_hmac = false;
	} else {
	...
	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_AUTH, &data, true);
	if (rc < 0) {
		chg_err("can't get GAUGE_ITEM_AUTH data, rc=%d\n", rc);
		chip->batt_auth = false;
	} else {
	...
	if (!chip->batt_hmac || !chip->batt_auth) {
		vote(chip->vooc_disable_votable, NON_STANDARD_VOTER, true, 1, false);
	}
...
static void oplus_vooc_parallel_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_vooc *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		...
		case SWITCH_ITEM_STATUS:
			oplus_mms_get_item_data(chip->parallel_topic, id, &data, false);
...
static void oplus_vooc_subscribe_parallel_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->parallel_subs =
		oplus_mms_subscribe(chip->parallel_topic, chip, oplus_vooc_parallel_subs_callback, "vooc");
	if (IS_ERR_OR_NULL(chip->parallel_subs)) {
		chg_err("subscribe switch topic error, rc=%ld\n", PTR_ERR(chip->parallel_subs));
		return;
	}

	oplus_mms_get_item_data(chip->parallel_topic, SWITCH_ITEM_STATUS, &data, true);
	if (data.intval == PARALLEL_BAT_BALANCE_ERROR_STATUS8) {
		oplus_vooc_set_online(chip, false);
		oplus_vooc_set_online_keep(chip, false);
...
static void oplus_vooc_dual_chan_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_vooc *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		...
		case SWITCH_ITEM_DUAL_CHAN_STATUS:
			oplus_mms_get_item_data(chip->dual_chan_topic, id, &data, false);
...
static void oplus_vooc_subscribe_dual_chan_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->dual_chan_subs =
		oplus_mms_subscribe(chip->dual_chan_topic, chip, oplus_vooc_dual_chan_subs_callback, "vooc");
	if (IS_ERR_OR_NULL(chip->dual_chan_subs)) {
		chg_err("subscribe dual_chan topic error, rc=%ld\n", PTR_ERR(chip->dual_chan_subs));
		return;
	}

	oplus_mms_get_item_data(chip->dual_chan_topic, SWITCH_ITEM_DUAL_CHAN_STATUS, &data, true);
	chip->enable_dual_chan = !!data.intval;
}

static void oplus_vooc_cpa_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_chg_vooc *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	int ret = 0;
...
static void oplus_vooc_subscribe_cpa_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_chg_vooc *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->cpa_topic = topic;
	chip->cpa_subs = oplus_mms_subscribe(chip->cpa_topic, chip, oplus_vooc_cpa_subs_callback, "vooc");
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, err_handler_work);
	...
	list_for_each_entry_safe (msg, tmp, &msg_list, list) {
		if (is_err_topic_available(chip))
			oplus_mms_publish_ic_err_msg(chip->err_topic, ERR_ITEM_IC, msg);
...
{
	struct oplus_chg_vooc *chip = container_of(work, struct oplus_chg_vooc, comm_charge_disable_work);
	union mms_msg_data data = { 0 };
	bool disable;
	int rc;
	...
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_CHARGING_DISABLE, &data, false);
	if (rc < 0) {
		chg_err("can't get charging disable status, rc=%d", rc);
		disable = false;
	} else {
	...
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_CHARGE_SUSPEND, &data, false);
	if (rc < 0) {
		chg_err("can't get charge suspend status, rc=%d", rc);
		data.intval = false;
	} else {
...
static int oplus_chg_vooc_virq_register(struct oplus_chg_vooc *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->vooc_ic, OPLUS_IC_VIRQ_ERR, oplus_chg_vooc_err_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ERR error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->vooc_ic, OPLUS_IC_VIRQ_VOOC_DATA, oplus_chg_vooc_data_handler, chip);
	if (rc < 0)
...
static int oplus_vooc_update_vooc_started(struct oplus_mms *topic, union mms_msg_data *data)
{
...
static int oplus_vooc_update_vooc_charging(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
	bool charging = false;
...
static int oplus_vooc_update_online(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
	bool online = false;
...
static int oplus_vooc_update_sid(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
	unsigned int sid = 0;
...
static int oplus_vooc_update_online_keep(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
...
static int oplus_vooc_update_bynormal_path(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
...
static int oplus_vooc_update_temp_range(struct oplus_mms *topic, union mms_msg_data *data)
{
	struct oplus_chg_vooc *chip;
...
static int oplus_vooc_topic_init(struct oplus_chg_vooc *chip)
{
	...
	chip->vooc_topic = devm_oplus_mms_register(chip->dev, &oplus_vooc_desc, &mms_cfg);
...
static int oplus_chg_vooc_parse_dt(struct oplus_chg_vooc *chip, struct device_node *node)
{
	struct oplus_vooc_spec_config *spec = &chip->spec;
	struct oplus_vooc_config *config = &chip->config;
	int rc;

	rc = of_property_read_s32(node, "oplus_spec,vooc_low_temp", &spec->vooc_low_temp);
	if (rc < 0) {
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_little_cold_temp", &spec->vooc_little_cold_temp);
	if (rc < 0) {
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_cool_temp", &spec->vooc_cool_temp);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_cool_temp reading failed, rc=%d\n", rc);
		spec->vooc_cool_temp = default_spec_config.vooc_cool_temp;
	}

	rc = of_property_read_s32(node, "oplus_spec,vooc_little_cool_temp", &spec->vooc_little_cool_temp);
	if (rc < 0) {
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_normal_low_temp", &spec->vooc_normal_low_temp);
	if (rc < 0) {
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_normal_high_temp", &spec->vooc_normal_high_temp);
	if (rc < 0) {
	...
	rc = of_property_read_s32(node, "oplus_spec,vooc_high_temp", &spec->vooc_high_temp);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_high_temp reading failed, rc=%d\n", rc);
		spec->vooc_high_temp = default_spec_config.vooc_high_temp;
	}

	rc = of_property_read_s32(node, "oplus_spec,vooc_over_high_temp", &spec->vooc_over_high_temp);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_low_soc", &spec->vooc_low_soc);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_low_soc reading failed, rc=%d\n", rc);
		spec->vooc_low_soc = default_spec_config.vooc_low_soc;
	}
	rc = of_property_read_u32(node, "oplus_spec,vooc_high_soc", &spec->vooc_high_soc);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_high_soc reading failed, rc=%d\n", rc);
		spec->vooc_high_soc = default_spec_config.vooc_high_soc;
	}

	rc = of_property_read_u32(node, "oplus,vooc-version", &config->vooc_version);
	if (rc < 0) {
	...
	if (spec->vooc_normal_high_temp != -EINVAL) {
		rc = of_property_read_u32(node, "oplus_spec,vooc_warm_vol_thr", &spec->vooc_warm_vol_thr);
		if (rc < 0) {
			chg_err("oplus_spec,vooc_warm_vol_thr reading failed, rc=%d\n", rc);
			spec->vooc_warm_vol_thr = default_spec_config.vooc_warm_vol_thr;
		}
		rc = of_property_read_u32(node, "oplus_spec,vooc_warm_soc_thr", &spec->vooc_warm_soc_thr);
		if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,vooc_bad_volt", spec->vooc_bad_volt, VOOC_BAT_VOLT_REGION);
	if (rc < 0) {
		chg_err("oplus_spec,vooc_bad_volt reading failed, rc=%d\n", rc);
		config->vooc_bad_volt_check_support = false;
		goto skip_vooc_bad_volt_check;
	}
	rc = read_unsigned_data_from_node(node, "oplus_spec,vooc_bad_volt_suspend", spec->vooc_bad_volt_suspend,
	...
	if (config->data_width == 7) {
		if (type == VOOC_CP_CURR_TABLE)
			vote(chip->vooc_curr_votable, DEF_VOTER, true,
			     oplus_cp_7bit_curr_table[config->max_curr_level - 1], false);
		else
			vote(chip->vooc_curr_votable, DEF_VOTER, true,
			     oplus_vooc_7bit_curr_table[config->max_curr_level - 1], false);
	} else {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, fw_update_work);
	int rc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, fw_update_work_fix);
	int rc;
...
static int vooc_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, ",%d,%d,%d,%d,%d,%d", chip->vooc_online, chip->fastchg_started, chip->fastchg_ing,
		 chip->vooc_online_keep, chip->sid, chip->adapter_id);
...
static int vooc_get_log_head(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, ",vooc_online,vooc_started,vooc_charging,vooc_online_keep,vooc_sid,adapter_id");
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_vooc *chip = container_of(dwork, struct oplus_chg_vooc, vooc_init_work);
	...
	if (chip->vooc_ic == NULL) {
		if (retry > 0) {
			retry--;
			schedule_delayed_work(&chip->vooc_init_work, msecs_to_jiffies(OPLUS_CHG_IC_INIT_RETRY_DELAY));
			return;
		} else {
...
static int oplus_vooc_curr_vote_callback(struct votable *votable, void *data, int curr_ma, const char *client,
					 bool step)
{
	...
	if (chip->config.data_width == 7) {
		if (type == VOOC_CP_CURR_TABLE) {
			...
			curr_level = oplus_vooc_get_curr_level(curr_ma, oplus_cp_7bit_curr_table,
							       ARRAY_SIZE(oplus_cp_7bit_curr_table));
		} else {
			curr_level = oplus_vooc_get_curr_level(curr_ma, oplus_vooc_7bit_curr_table,
							       ARRAY_SIZE(oplus_vooc_7bit_curr_table));
		}
	} else {
		if (sid_to_adapter_chg_type(chip->sid) == CHARGER_TYPE_VOOC) {
			curr_level = oplus_vooc_get_curr_level(curr_ma, oplus_vooc_curr_table,
							       ARRAY_SIZE(oplus_vooc_curr_table));
		} else if (sid_to_adapter_chg_type(chip->sid) == CHARGER_TYPE_SVOOC) {
			curr_level = oplus_vooc_get_curr_level(curr_ma, oplus_svooc_curr_table,
...
static int oplus_vooc_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client,
					    bool step)
{
	struct oplus_chg_vooc *chip = data;
...
static int oplus_vooc_not_allow_vote_callback(struct votable *votable, void *data, int not_allow, const char *client,
					      bool step)
{
	struct oplus_chg_vooc *chip = data;
...
static int oplus_vooc_pd_svooc_vote_callback(struct votable *votable, void *data, int allow, const char *client,
					     bool step)
{
	struct oplus_chg_vooc *chip = data;
...
static int oplus_vooc_chg_auto_mode_vote_callback(struct votable *votable, void *data, int enable, const char *client,
						  bool step)
{
	struct oplus_chg_vooc *chip = data;
	int rc;
	...
	if (rc)
		chg_err("falied to %s %s chg auto mode, rc= %d\n", client, enable ? "enable" : "disable", rc);
	else
		chg_info("%s %s chg auto mode\n", client, enable ? "enable" : "disable");

	return rc;
}

static int oplus_vooc_boot_vote_callback(struct votable *votable, void *data, int boot, const char *client, bool step)
{
	struct oplus_chg_vooc *chip = data;
...
static int oplus_vooc_vote_init(struct oplus_chg_vooc *chip)
{
	...
	chip->vooc_curr_votable = create_votable("VOOC_CURR", VOTE_MIN, oplus_vooc_curr_vote_callback, chip);
	...
	chip->vooc_disable_votable =
		create_votable("VOOC_DISABLE", VOTE_SET_ANY, oplus_vooc_disable_vote_callback, chip);
	...
	chip->vooc_not_allow_votable =
		create_votable("VOOC_NOT_ALLOW", VOTE_SET_ANY, oplus_vooc_not_allow_vote_callback, chip);
	...
	chip->pd_svooc_votable = create_votable("PD_SVOOC", VOTE_SET_ANY, oplus_vooc_pd_svooc_vote_callback, chip);
	...
	chip->vooc_chg_auto_mode_votable =
		create_votable("VOOC_CHG_AUTO_MODE", VOTE_SET_ANY, oplus_vooc_chg_auto_mode_vote_callback, chip);
	...
	chip->vooc_boot_votable = create_votable("VOOC_BOOT", VOTE_SET_ANY, oplus_vooc_boot_vote_callback, chip);
...
static int oplus_abnormal_adapter_pase_dt(struct oplus_chg_vooc *chip)
{
	...
	do {
		...
		chip->abnormal_adapter_cur_arraycnt = of_property_count_elems_of_size(
			node, "oplus,abnormal_adapter_current", sizeof(*config->abnormal_adapter_cur_array));
		...
		config->abnormal_adapter_cur_array =
			devm_kcalloc(chip->dev, chip->abnormal_adapter_cur_arraycnt,
				     sizeof(*config->abnormal_adapter_cur_array), GFP_KERNEL);
		if (!config->abnormal_adapter_cur_array) {
			chg_info("devm_kcalloc abnormal_adapter_current fail\n");
			break;
		}
		rc = of_property_read_u32_array(node, "oplus,abnormal_adapter_current",
						config->abnormal_adapter_cur_array,
						chip->abnormal_adapter_cur_arraycnt);
		if (rc) {
	...
	do {
		...
		chip->abnormal_over_80w_adapter_cur_arraycnt =
			of_property_count_elems_of_size(node, "oplus,abnormal_over_80w_adapter_current",
		...
		config->abnormal_over_80w_adapter_cur_array =
			devm_kcalloc(chip->dev, chip->abnormal_over_80w_adapter_cur_arraycnt,
				     sizeof(*config->abnormal_over_80w_adapter_cur_array), GFP_KERNEL);
		if (!config->abnormal_over_80w_adapter_cur_array) {
			chg_info("devm_kcalloc abnormal_over_80w_adapter_current fail\n");
			break;
		}
		rc = of_property_read_u32_array(node, "oplus,abnormal_over_80w_adapter_current",
						config->abnormal_over_80w_adapter_cur_array,
						chip->abnormal_over_80w_adapter_cur_arraycnt);
		if (rc) {
		...
		for (loop = 0; loop < chip->abnormal_over_80w_adapter_cur_arraycnt; loop++) {
...
int oplus_mcu_bcc_stop_curr_dt(struct oplus_chg_vooc *chip, struct device_node *node)
{
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,bcc_stop_curr_0_to_50", spec->bcc_stop_curr_0_to_50,
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,bcc_stop_curr_51_to_75", spec->bcc_stop_curr_51_to_75,
					  VOOC_BCC_STOP_CURR_NUM);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,bcc_stop_curr_76_to_85", spec->bcc_stop_curr_76_to_85,
					  VOOC_BCC_STOP_CURR_NUM);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,bcc_stop_curr_86_to_90", spec->bcc_stop_curr_86_to_90,
					  VOOC_BCC_STOP_CURR_NUM);
	if (rc < 0) {
...
static bool get_fw_update_newmethod(struct device_node *node)
{
	...
	for (i = 0; i < asic_num; i++) {
		...
		if (asic_node) {
			newmethod = of_property_read_bool(asic_node, "oplus,vooc_fw_update_newmethod");
...
static int oplus_vooc_parse_dt(struct oplus_chg_vooc *chip)
{
	...
	chip->support_fake_vooc_check = of_property_read_bool(node, "oplus,support_fake_vooc_check");
	...
	rc = of_property_read_u32(node, "oplus,vooc_curr_table_type", &config->vooc_curr_table_type);
	if (rc < 0) {
	...
	config->support_vooc_by_normal_charger_path =
		of_property_read_bool(node, "oplus,support_vooc_by_normal_charger_path");

	chg_info("read support_vooc_by_normal_charger_path=%d\n", config->support_vooc_by_normal_charger_path);

	rc = of_property_read_string(node, "oplus,general_strategy_name", (const char **)&config->strategy_name);
	if (rc >= 0) {
		chg_info("strategy_name=%s\n", config->strategy_name);
		rc = oplus_chg_strategy_read_data(chip->dev, "oplus,general_strategy_data", &config->strategy_data);
		if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,vooc_soc_range", spec->vooc_soc_range, VOOC_SOC_RANGE_NUM);
...
#define VOOC_DEFAULT_SOC_RANGE_3 85
	...
	rc = read_unsigned_data_from_node(node, "oplus_spec,vooc_temp_range", spec->vooc_temp_range,
					  VOOC_TEMP_RANGE_NUM);
...
static int oplus_vooc_strategy_init(struct oplus_chg_vooc *chip)
{
	...
	chip->general_strategy =
		oplus_chg_strategy_alloc(config->strategy_name, config->strategy_data, config->strategy_data_size);
	if (chip->general_strategy == NULL)
...
static int oplus_vooc_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_chg_vooc), GFP_KERNEL);
	if (chip == NULL) {
	...
	INIT_DELAYED_WORK(&chip->fw_update_work_fix, oplus_vooc_fw_update_fix_work);
	rc = oplus_vooc_proc_init(chip);
	if (rc < 0)
		goto proc_init_err;

	INIT_DELAYED_WORK(&chip->vooc_init_work, oplus_vooc_init_work);
	INIT_DELAYED_WORK(&chip->vooc_switch_check_work, oplus_vooc_switch_check_work);
	INIT_DELAYED_WORK(&chip->check_charger_out_work, oplus_vooc_check_charger_out_work);
	...
	INIT_WORK(&chip->temp_region_update_work, oplus_vooc_temp_region_update_work);
...
static int oplus_vooc_get_bcc_max_curr(struct oplus_mms *topic, union mms_msg_data *data)
{
	...
	if (1 == oplus_vooc_read_voocphy_bcc_fastchg_ing(chip->vooc_ic) || chip->config.voocphy_support == ADSP_VOOCPHY)
...
static int oplus_vooc_get_svooc_type(struct oplus_mms *topic, union mms_msg_data *data)
{
	...
	if (chip->config.support_vooc_by_normal_charger_path && sid_to_adapter_chg_type(chip->sid) == CHARGER_TYPE_VOOC)
...
static int oplus_vooc_get_wired_type(void)
{
	...
	rc = oplus_mms_get_item_data(wired_topic, WIRED_ITEM_CHG_TYPE, &data, false);
	if (!rc)
...
int oplus_vooc_current_to_level(struct oplus_mms *topic, int curr)
{
	...
	if (table != NULL) {
		if (type == VOOC_CP_CURR_TABLE && sid_to_adapter_chg_type(chip->sid) == CHARGER_TYPE_SVOOC)
