...
static bool init_gauge_auth(struct chip_bq27541 *chip, oplus_gauge_auth_result *rst,
...
static __inline__ void bq27541_push_i2c_err(struct chip_bq27541 *chip, bool read)
{
	...
	if (atomic_read(&chip->i2c_err_count) > I2C_ERR_MAX) {
		...
		oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_I2C, 0, read ? "read error" : "write error");
...
static int oplus_zy0603_check_sfr(struct chip_bq27541 *chip, bool *need_reset)
{
	...
	ret = zy0603_read_altmac_block(chip, ZY0603_READ_QMAX_CMD, qmax_buf, ZY0603_QMAX_DATA_OFFSET, ZY0603_QMAX_LEN);
	if (ret < 0) {
...
static int oplus_detection_reset_condition(struct oplus_chg_ic_dev *ic_dev, bool *need_reset)
{
	...
	if (chip && chip->batt_zy0603 && chip->zy0603_soc_opt && zy0603_fg_condition_check(chip)) {
...
fg_read_altmac_block_end:
	...
	chg_info("[0x%02x][0x%02x][0x%02x][0x%02x] fg_version %d %s\n", version_data[0], version_data[1],
		 version_data[2], version_data[3], *fwversion, ret ? "i2c ok" : "i2c error");
	return ret;
}
...
static int zy0603_full_access_mode(struct chip_bq27541 *chip)
{
	...
	mutex_lock(&chip->bq28z610_alt_manufacturer_access);
	do {
		bq27541_write_i2c_block(chip, ZY0603_CTRL_REG, 2, full_access_cmd_1);
		pr_info("%s write {0xEF, 0xCD} --> 0x00\n", __func__);
		usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);

		bq27541_write_i2c_block(chip, ZY0603_CTRL_REG, 2, full_access_cmd_2);
		pr_info("%s write {0xAB, 0x90} --> 0x00\n", __func__);
		usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);

		bq27541_write_i2c_block(chip, ZY0603_CMD_ALTMAC, 2, full_access_op_st);
...
static int oplus_fg_reset(struct oplus_chg_ic_dev *ic_dev, bool *reset_done)
{
	struct chip_bq27541 *chip;
	...
	if (chip && chip->batt_zy0603 && chip->zy0603_soc_opt) {
		oplus_zy0603_reset_dynamic_para(chip, reset_done);
		pr_info("%s: reset_done = %s", __func__, *reset_done == true ? "true" : "false");
	}
	return 0;
}
...
error_occured:
	...
	if (oplus_gauge_get_afi_condition()) {
		if (!gauge_locked ||
		    (oplus_vooc_get_vooc_by_normal_path() && oplus_vooc_get_fast_chg_type() == BCC_TYPE_IS_VOOC)) {
...
static int zy0603_protect_check(struct oplus_chg_ic_dev *ic_dev)
{
	...
	printk(KERN_ERR "[%s] checksum_error[%d %d] qmaxfg_error[%d, %d]\n", __func__, checksum_error_flag,
	       checksum_error, qmaxfg_error_flag, qmax_fgstatus_error);
...
static int bq27541_get_battery_temperature(struct chip_bq27541 *chip)
{
	...
	if (chip->bq28z610_need_balancing) {
		...
		if (pre_batt_balancing_config == 0 && batt_balancing_config == 1 && temp_status == 0) {
			if (chip->temp_pre + ZERO_DEGREE_CELSIUS_IN_TENTH_KELVIN <= BATT_TEMP_16C) {
				temp_status = TEMP_LT_16C;
				delta_temp = 10;
			} else if (chip->temp_pre + ZERO_DEGREE_CELSIUS_IN_TENTH_KELVIN <= BATT_TEMP_39C) {
		...
		if (batt_balancing_config == 1) {
			...
			if (chip->bq28z610_device_chem == DEVICE_CHEMISTRY_C2A1 ||
			    chip->bq28z610_device_chem == DEVICE_CHEMISTRY_C2A2) {
...
static int bq28z610_get_dod0_parameters(struct chip_bq27541 *chip, int *batt_dod0_1, int *batt_dod0_2,
					int *batt_dod0_passed_q)
{
	...
	mutex_lock(&chip->bq28z610_alt_manufacturer_access);
	bq27541_i2c_txsubcmd(chip, BQ28Z610_MAC_CELL_DOD0_EN_ADDR, BQ28Z610_MAC_CELL_DOD0_CMD);
	usleep_range(1000, 1000);
	if (chip->batt_zy0603) {
		bq27541_read_i2c_block(chip, ZY0603_MAC_CELL_SOCCAL0, BQ28Z610_MAC_CELL_DOD0_SIZE, dod0_buffer);
	} else {
		bq27541_read_i2c_block(chip, BQ28Z610_MAC_CELL_DOD0_ADDR, BQ28Z610_MAC_CELL_DOD0_SIZE, dod0_buffer);
	}
	mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
...
static int bq28z610_get_qmax_parameters(struct chip_bq27541 *chip, int *batt_qmax_1, int *batt_qmax_2,
					int *batt_qmax_passed_q)
{
	...
	mutex_lock(&chip->bq28z610_alt_manufacturer_access);
	bq27541_i2c_txsubcmd(chip, BQ28Z610_MAC_CELL_QMAX_EN_ADDR, BQ28Z610_MAC_CELL_QMAX_CMD);
	usleep_range(1000, 1000);
	bq27541_read_i2c_block(chip, BQ28Z610_MAC_CELL_QMAX_ADDR_A, BQ28Z610_MAC_CELL_QMAX_SIZE_A, qmax_buffer_1);

	bq27541_i2c_txsubcmd(chip, BQ28Z610_MAC_CELL_QMAX_EN_ADDR, BQ28Z610_MAC_CELL_QMAX_CMD);
	usleep_range(1000, 1000);
	bq27541_read_i2c_block(chip, BQ28Z610_MAC_CELL_QMAX_ADDR_B, BQ28Z610_MAC_CELL_QMAX_SIZE_B, qmax_buffer_2);
	mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
...
__maybe_unused static int bq27541_get_prev_batt_remaining_capacity(struct chip_bq27541 *chip)
{
	if (!chip) {
		return 0;
	}
...
static int bq27541_sha1_hmac_authenticate(struct chip_bq27541 *chip,
					  struct bq27541_authenticate_data *authenticate_data);
...
static int bq27541_get_id_status(struct chip_bq27541 *chip)
{
	...
	if (!chip || !gpio_is_valid(chip->id_gpio) || IS_ERR_OR_NULL(chip->pinctrl) ||
	    IS_ERR_OR_NULL(chip->id_not_pull) || IS_ERR_OR_NULL(chip->id_pull_down) ||
	    IS_ERR_OR_NULL(chip->id_pull_up)) {
...
static void bq27541_parse_dt(struct chip_bq27541 *chip)
{
	...
	chip->zy0603_soc_opt = of_property_read_bool(node, "oplus,zy0603_soc_opt");
	/* workaround for I2C pull SDA can't trigger error issue 230504153935012779 */
...
static int bq27411_modify_soc_smooth_parameter(struct chip_bq27541 *chip, bool is_powerup)
{
	...
	if (chip->batt_zy0603 && is_powerup) {
		...
		if (chip->batt_bq28z610 && is_powerup && !chip->i2c_err && oplus_is_rf_ftm_mode() == false) {
...
static int bq28z610_get_2cell_voltage(struct chip_bq27541 *chip)
{
	...
	if (!chip->batt_zy0603) {
		mutex_lock(&chip->bq28z610_alt_manufacturer_access);
		bq27541_i2c_txsubcmd(chip, BQ28Z610_MAC_CELL_VOLTAGE_EN_ADDR, BQ28Z610_MAC_CELL_VOLTAGE_CMD);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(chip, BQ28Z610_MAC_CELL_VOLTAGE_ADDR, BQ28Z610_MAC_CELL_VOLTAGE_SIZE, cell_vol);
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
		...
		bq27541_i2c_txsubcmd(chip, BQ28Z610_MAC_CELL_VOLTAGE_EN_ADDR, BQ28Z610_MAC_CELL_VOLTAGE_CMD);
		usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
		bq27541_read_i2c_block(chip, BQ28Z610_MAC_CELL_VOLTAGE_EN_ADDR, BQ28Z610_MAC_CELL_VOLTAGE_SIZE + 2,
				       cell_vol_zy0603);
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
...
static int bq28z610_set_term_volt(struct chip_bq27541 *chip, int volt_mv)
{
	...
	u8 deep_write[BQ28Z610_DEEP_DISCHG_SIZE] = { 0x82,
						     0x40,
						     0x08,
						     0,
						     0,
						     0,
						     (volt_mv * 2) & BQ28Z610_DEEP_DISCHG_CHECK,
						     (volt_mv * 2) >> BQ28Z610_DEEP_DISCHG_SHIFT_MASK };
	...
	chg_info("[%d, %d][0x%x, 0x%x], [0x%x, 0x%x, 0x%x][0x%x, 0x%x, 0x%x]\n", value, volt_mv, deep_read[0],
		 deep_read[1], deep_read[2], deep_read[3], deep_read[4], deep_read[5], deep_read[6], deep_read[7]);

	if ((value == volt_mv * 2) && (deep_read[7] == (BQ28Z610_DEEP_DISCHG_CHECK - (deep_read[5] + deep_read[6]) &
							BQ28Z610_DEEP_DISCHG_CHECK))) {
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
		return 0;
	}
	...
	deep_check[0] = BQ28Z610_DEEP_DISCHG_CHECK -
				(deep_write[0] + deep_write[1] + deep_write[2] + deep_write[3] + deep_write[4] +
				 deep_write[5] + deep_write[6] + deep_write[7] + deep_write[8]) &
			BQ28Z610_DEEP_DISCHG_CHECK;
	...
	check_data[0] = BQ28Z610_DEEP_DISCHG_CHECK - (write_main[0] + write_main[1] + write_main[2] + write_main[3]) &
			BQ28Z610_DEEP_DISCHG_CHECK;
	...
	check_data[0] = BQ28Z610_DEEP_DISCHG_CHECK - (write_sub[0] + write_sub[1] + write_sub[2] + write_sub[3]) &
			BQ28Z610_DEEP_DISCHG_CHECK;
	rc = bq27541_write_i2c_block(chip, BQ28Z610_TERM_VOLT_CHECK_ADDR, 2, check_data);
...
static int bq28z610_set_deep_dischg_num(struct chip_bq27541 *chip, int dischg_num)
{
	int rc = -1;
	int value = 0;
	u8 write_data[BQ28Z610_DEEP_DISCHG_SIZE] = { 0x82, 0x40, 0x08, dischg_num & BQ28Z610_DEEP_DISCHG_CHECK,
						     dischg_num >> BQ28Z610_DEEP_DISCHG_SHIFT_MASK };
	u8 read_data[BQ28Z610_DEEP_DISCHG_SIZE] = { 0, 0, 0, 0, 0 };
	...
	if ((value == dischg_num) && (read_data[4] == (BQ28Z610_DEEP_DISCHG_CHECK - (read_data[2] + read_data[3]) &
						       BQ28Z610_DEEP_DISCHG_CHECK))) {
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
		return rc;
	}
					  ...
					  struct bq27541_authenticate_data *authenticate_data)
{
	...
	bq27541_write_i2c_block(chip, AUTHENDATA, authenticate_data->message_len, authenticate_data->message);
	msleep(5);
	...
	bq27541_read_i2c_block(chip, AUTHENDATA, authenticate_data->message_len, &recv_buf[0]);
...
static int bq28z610_get_info(struct chip_bq27541 *chip, u8 *info, int len)
{
	...
	struct gauge_track_info_reg standard[] = {
		{ chip->cmd_addr.reg_temp, 2 }, { chip->cmd_addr.reg_volt, 2 }, { chip->cmd_addr.reg_flags, 2 },
		{ chip->cmd_addr.reg_nac, 2 },	{ chip->cmd_addr.reg_rm, 2 },	{ chip->cmd_addr.reg_fcc, 2 },
		{ BQ28Z610_REG_AI, 2 },		{ chip->cmd_addr.reg_cc, 2 },	{ chip->cmd_addr.reg_soc, 2 },
		{ chip->cmd_addr.reg_soh, 2 },
	};

	struct gauge_track_info_reg extend[] = {
		{ BQ28Z610_SUBCMD_CHEMID, 2 },	    { BQ28Z610_SUBCMD_GAUGEING_STATUS, 3 },
		{ BQ28Z610_SUBCMD_DA_STATUS1, 12 }, { BQ28Z610_SUBCMD_IT_STATUS1, 16 },
		{ BQ28Z610_SUBCMD_IT_STATUS2, 24 }, { BQ28Z610_SUBCMD_IT_STATUS3, 16 },
...
static int bq28z610_get_calib_time(struct chip_bq27541 *chip, int *dod_calib_time, int *qmax_calib_time)
{
	...
	if (!init_flag || check_args[0] != chip->calib_check_args_pre[0] ||
	    check_args[3] != chip->calib_check_args_pre[3])
		chip->dod_time = 0;
	else
		chip->dod_time++;

	if (!init_flag || check_args[1] != chip->calib_check_args_pre[1] ||
	    check_args[2] != chip->calib_check_args_pre[2] || check_args[4] != chip->calib_check_args_pre[4] ||
...
static int oplus_bq27541_get_calib_time(struct oplus_chg_ic_dev *ic_dev, int *dod_calib_time, int *qmax_calib_time)
{
	struct chip_bq27541 *chip;
	int rc = -1;
...
static void bq27541_reset(struct i2c_client *client)
{
	...
	if (bq27541_get_battery_mvolts(chip) <= 3300 && bq27541_get_battery_mvolts(chip) > 2500 && ui_soc == 0 &&
	    bq27541_get_battery_temperature(chip) > 150) {
		if (!unseal(chip, BQ27541_UNSEAL_KEY)) {
			pr_err("bq27541 unseal fail !\n");
			return;
		}
		chg_info("bq27541 unseal OK vol = %d, ui_soc = %d, temp = %d!\n", bq27541_get_battery_mvolts(chip),
			 ui_soc, bq27541_get_battery_temperature(chip));
		if (chip->device_type == DEVICE_BQ27541 || chip->device_type == DEVICE_ZY0602) {
...
static int oplus_bq27541_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	return 0;
}
...
static int oplus_bq27541_get_batt_vol(struct oplus_chg_ic_dev *ic_dev, int index, int *volt)
{
	...
	switch (index) {
	case 0:
		if ((atomic_read(&chip->suspended) != 0) || (atomic_read(&chip->locked) != 0)) {
...
static int oplus_bq27541_get_batt_max(struct oplus_chg_ic_dev *ic_dev, int *vol_mv)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_min(struct oplus_chg_ic_dev *ic_dev, int *vol_mv)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_curr(struct oplus_chg_ic_dev *ic_dev, int *curr_ma)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_temp(struct oplus_chg_ic_dev *ic_dev, int *temp)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_auth(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_afi_update_done(struct oplus_chg_ic_dev *ic_dev, bool *status)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_hmac(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_set_batt_full(struct oplus_chg_ic_dev *ic_dev, bool full)
{
...
static int oplus_bq27541_update_soc_smooth_parameter(struct oplus_chg_ic_dev *ic_dev)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_cb_status(struct oplus_chg_ic_dev *ic_dev, int *status)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_passedchg(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_is_locked(struct oplus_chg_ic_dev *ic_dev, bool *locked)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_device_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_device_type_for_vooc(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	struct chip_bq27541 *chip;
...
static int bq28z610_get_battery_gauge_type_for_bcc(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	struct chip_bq27541 *chip;
	int gauge_type;
...
static int oplus_bq27541_is_suspend(struct oplus_chg_ic_dev *ic_dev, bool *suspend)
{
	struct chip_bq27541 *chip;
...
static int oplus_bq27541_get_batt_exist(struct oplus_chg_ic_dev *ic_dev, bool *exist)
{
	struct chip_bq27541 *chip;
...
{
	struct chip_bq27541 *chip = container_of(work, struct chip_bq27541, check_iic_recover.work);
...
RETRY:
	while (retry < BQ28Z610_BATT_SN_RETRY_MAX) {
		...
		if (chip->batt_zy0603 || chip->batt_bq28z610) {
			...
			usleep_range(1000, 1000);
			ret = bq27541_read_i2c_block(chip, BQ28Z610_BATT_SN_EN_ADDR, BQ28Z610_BATT_SN_READ_BUF_LEN,
						     buf);
			mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
			if (ret < 0)
				continue;

			if ((buf[0] == (BQ28Z610_BATT_SN_CMD & 0xFF)) && (buf[1] == (BQ28Z610_BATT_SN_CMD >> 8))) {
			...
			ret = bq27541_read_i2c_block(chip, ZY0602_BATT_SN_READ_ADDR, ZY0602_BATT_SN_READ_BUF_LEN,
						     buf_zy0602);
			mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
			if (ret < 0)
				continue;
	...
	if (retry < BQ28Z610_BATT_SN_RETRY_MAX) {
		if (batt_sn[OPLUS_BATT_SERIAL_NUM_SIZE - 1] != BQ28Z610_BATT_SN_NO_CHECKSUM) {
			...
			if (check_sum != batt_sn[OPLUS_BATT_SERIAL_NUM_SIZE - 1]) {
				chg_err("check_sum fail calc[%d] read[%d], retry.", check_sum,
					batt_sn[OPLUS_BATT_SERIAL_NUM_SIZE - 1]);
				goto RETRY;
			}
		}
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_bq27541_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_bq27541_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_bq27541_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_bq27541_smt_test);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL, oplus_bq27541_get_batt_vol);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX, oplus_bq27541_get_batt_max);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN, oplus_bq27541_get_batt_min);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR, oplus_bq27541_get_batt_curr);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP, oplus_bq27541_get_batt_temp);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC, oplus_bq27541_get_batt_soc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC, oplus_bq27541_get_batt_fcc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_CC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_CC, oplus_bq27541_get_batt_cc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_RM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_RM, oplus_bq27541_get_batt_rm);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH, oplus_bq27541_get_batt_soh);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH, oplus_bq27541_get_batt_auth);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC, oplus_bq27541_get_batt_hmac);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_BATT_FULL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_BATT_FULL, oplus_bq27541_set_batt_full);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_FC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_FC, oplus_bq27541_get_batt_fc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_QM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_QM, oplus_bq27541_get_batt_qm);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_PD:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_PD, oplus_bq27541_get_batt_pd);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_RCU:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_RCU, oplus_bq27541_get_batt_rcu);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_RCF:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_RCF, oplus_bq27541_get_batt_rcf);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_FCU:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_FCU, oplus_bq27541_get_batt_fcu);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_FCF:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_FCF, oplus_bq27541_get_batt_fcf);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SOU:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SOU, oplus_bq27541_get_batt_sou);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_DO0:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_DO0, oplus_bq27541_get_batt_do0);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_DOE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_DOE, oplus_bq27541_get_batt_doe);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_TRM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_TRM, oplus_bq27541_get_batt_trm);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_PC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_PC, oplus_bq27541_get_batt_pc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_QS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_QS, oplus_bq27541_get_batt_qs);
		break;
	case OPLUS_IC_FUNC_GAUGE_UPDATE_DOD0:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_UPDATE_DOD0, oplus_bq27541_update_dod0);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS, oplus_bq27541_get_cb_status);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_PASSEDCHG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_PASSEDCHG, oplus_bq27541_get_passedchg);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_LOCK:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_LOCK, oplus_bq27541_set_lock);
		break;
	case OPLUS_IC_FUNC_GAUGE_IS_LOCKED:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_IS_LOCKED, oplus_bq27541_is_locked);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_NUM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_NUM, oplus_bq27541_get_batt_num);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE, oplus_bq27541_get_device_type);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_IS_SUSPEND, oplus_bq27541_is_suspend);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BCC_PARMS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BCC_PARMS, bq28z610_get_battery_bcc_parameters);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_DOD0:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_DOD0, bq28z610_get_battery_dod0);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_QMAX:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_QMAX, bq28z610_get_battery_qmax);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_SET_BCC_PARMS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_BCC_PARMS, bq28z610_set_bcc_debug_parameters);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_PROTECT_CHECK:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_PROTECT_CHECK, zy0603_protect_check);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_EXIST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_EXIST, oplus_bq27541_get_batt_exist);
		break;
	case OPLUS_IC_FUNC_GAUGE_CHECK_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_CHECK_RESET, oplus_detection_reset_condition);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_RESET, oplus_fg_reset);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_SET_DEEP_DISCHG_COUNT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_DEEP_DISCHG_COUNT, oplus_sett_deep_dischg_count);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_DEEP_TERM_VOLT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_DEEP_TERM_VOLT, oplus_set_deep_term_volt);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATTID_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATTID_INFO, oplus_get_batt_id_info);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_REG_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_REG_INFO, oplus_bq27541_get_reg_info);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_CALIB_TIME:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_CALIB_TIME, oplus_bq27541_get_calib_time);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SN:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SN, oplus_bq27541_get_battinfo_sn);
		break;
	default:
...
*/
	...
	fg_ic->authenticate_data = devm_kzalloc(&client->dev, sizeof(struct bq27541_authenticate_data), GFP_KERNEL);
