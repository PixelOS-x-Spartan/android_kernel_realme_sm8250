...
#define gpio_reg_set(reg, value)                                                                                       \
	({                                                                                                             \
		__iowmb();                                                                                             \
		writel_relaxed_no_log(value, reg);                                                                     \
	})
...
int optiga_authenticate(void)
{
	...
	if (first_read_uid == 0) {
#ifdef BURST_READ
		if (Swi_ReadUID_burst((uint8_t *)&stDetectedPuids[0], HIGH_BYTE_FIRST, BURST_BYTES_READ) !=
		    INF_SWI_SUCCESS)
#else
...
int oplus_optiga_get_test_result(int *count_total, int *count_now, int *fail_count)
{
	...
	*fail_count = g_oplus_optiga_chip->test_result.test_fail_count;
	chg_err("count_total:%d,count_now:%d,fail_count:%d,real_count:%d,real_fail:%d\n", *count_total, *count_now,
		*fail_count, g_oplus_optiga_chip->test_result.real_test_count_now,
		g_oplus_optiga_chip->test_result.real_test_fail_count);
	return 0;
}

int oplus_optiga_get_hmac_status(int *status, int *fail_count, int *total_count, int *real_fail_count,
...
static const char *oplus_optiga_get_cmdline(void)
{
	...
	if (of_chosen) {
		optiga_auth = (char *)of_get_property(of_chosen, "optiga_auth", NULL);
		if (!optiga_auth)
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_optiga_guage_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_optiga_guage_exit);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH, oplus_optiga_guage_get_batt_auth);
		break;
	default:
