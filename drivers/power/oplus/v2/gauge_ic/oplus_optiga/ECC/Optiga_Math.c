...
const uint32_t irred_polynomial_131[ARRAY_LEN(GF2_131)] = { 0x10d, 0x0, 0x0, 0x0, 0x8 };
static const uint32_t irred_polynomial_163[ARRAY_LEN(GF2_163)] = { 0xc9, 0x0, 0x0, 0x0, 0x0, 0x8 };
...
static const uint16_t square_tab[256] = {
	0x0,	0x1,	0x4,	0x5,	0x10,	0x11,	0x14,	0x15,	0x40,	0x41,	0x44,	0x45,	0x50,	0x51,
	0x54,	0x55,	0x100,	0x101,	0x104,	0x105,	0x110,	0x111,	0x114,	0x115,	0x140,	0x141,	0x144,	0x145,
	0x150,	0x151,	0x154,	0x155,	0x400,	0x401,	0x404,	0x405,	0x410,	0x411,	0x414,	0x415,	0x440,	0x441,
	0x444,	0x445,	0x450,	0x451,	0x454,	0x455,	0x500,	0x501,	0x504,	0x505,	0x510,	0x511,	0x514,	0x515,
	0x540,	0x541,	0x544,	0x545,	0x550,	0x551,	0x554,	0x555,	0x1000, 0x1001, 0x1004, 0x1005, 0x1010, 0x1011,
 ...
 * return FALSE if everything is ok, TRUE otherwise
 ********************************************************************/
BOOL verify_response(dwordvec_t gf2n_XResponse, dwordvec_t gf2n_ZResponse, dwordvec_t gf2n_CheckValue,
 ...
 */
BOOL verify_mac64(const mac_t mac_value, const dwordvec_t Z, const dwordvec_t xC, const mac_t data,
		  const curve_parameter_t *curve, mac_t host_mac_value)
{
...
void remove_x_power(dwordvec_t a, dwordvec_t b)
{
	while (!(a[0] & 1UL)) {
		r_shift(a);
		if (b[0] & 1UL)
			gf2n_sum(b, actual_irred_polynomial);
		r_shift(b);
	}
}
...
void remove_2_power(dwordvec_t a, dwordvec_t b)
{
	while (!(a[0] & 1UL)) {
		r_shift(a);
		if (b[0] & 1)
			dwordvec_add(b, b, actual_base_point_order);
		r_shift(b);
	}
}
...
void mont_ecc_mul(dwordvec_t A, dwordvec_t B, dwordvec_t C, dwordvec_t D, const dwordvec_t point_x,
		  const dwordvec_t scalar)
{
	dwordvec_t E, F;
	int i;
 ...
 ********************************************************************/
BOOL ecc_add_point_x(dwordvec_t point_x, const dwordvec_t X1, const dwordvec_t Y1, const dwordvec_t Z1,
...
void sha256_compress(sha256_context_t *context)
{
	...
	for (i = 0; i < 64; i++) {
		if (i > 15) {
			W[i & 15] = LIT_SIG_1(W[(i - 2) & 15]) + W[(i - 7) & 15] + LIT_SIG_0(W[(i - 15) & 15]) +
				    W[(i - 16) & 15];
		}
