...
#define chg_err(fmt, ...)                                                                                              \
	({                                                                                                             \
		if (oplus_log_level >= LOG_LEVEL_ERR)                                                                  \
			printk(KERN_ERR "[ERROR]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                              \
	})
#define chg_info(fmt, ...)                                                                                             \
	({                                                                                                             \
		if (oplus_log_level >= LOG_LEVEL_ALL_ERR)                                                              \
			printk(KERN_ERR "[INFO]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                               \
		else if (oplus_log_level >= LOG_LEVEL_INFO)                                                            \
			printk(KERN_INFO "[INFO]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                              \
	})
#define chg_debug(fmt, ...)                                                                                            \
	({                                                                                                             \
		if (oplus_log_level >= LOG_LEVEL_ALL_ERR)                                                              \
			printk(KERN_ERR "[DEBUG]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                              \
		else if (oplus_log_level >= LOG_LEVEL_ALL_INFO)                                                        \
			printk(KERN_INFO "[DEBUG]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                             \
		else if (oplus_log_level >= LOG_LEVEL_DEBUG)                                                           \
			printk(KERN_NOTICE "[DEBUG]: OPLUS_CHG" pr_fmt(fmt), ##__VA_ARGS__);                           \
	})
...
enum oplus_chg_qc_version { OPLUS_CHG_QC_2_0, OPLUS_CHG_QC_3_0 };
