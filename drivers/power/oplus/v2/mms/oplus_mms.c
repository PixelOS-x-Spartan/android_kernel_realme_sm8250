...
int oplus_mms_get_item_data(struct oplus_mms *mms, u32 item_id, union mms_msg_data *data, bool update)
{
	struct mms_item *item;

	if (mms == NULL || mms->desc == NULL || !mms->desc->item_table) {
...
bool oplus_mms_item_update(struct oplus_mms *mms, u32 item_id, bool check_update)
{
	...
	if (item->desc.str_data) {
		...
		if (item->desc.dead_thr_enable) {
			if (abs(item->data.intval - item->pre_data.intval) >= item->desc.dead_zone_thr) {
				item->pre_data.intval = item->data.intval;
				item->updated = true;
				goto out;
			} else {
...
static int oplus_mms_item_update_by_msg(struct oplus_mms *mms, u32 item_id, struct mms_msg *msg)
{
	struct mms_item *item;
...
struct mms_msg *oplus_mms_alloc_msg(enum mms_msg_type type, enum mms_msg_prio prio, u32 item_id)
{
	struct mms_msg *msg;
...
struct mms_msg *oplus_mms_alloc_int_msg(enum mms_msg_type type, enum mms_msg_prio prio, u32 item_id, int data)
{
	struct mms_msg *msg;
...
__printf(4, 5) struct mms_msg *oplus_mms_alloc_str_msg(enum mms_msg_type type, enum mms_msg_prio prio, u32 item_id,
						       const char *format, ...)
{
	struct mms_msg *msg;
	va_list args;
...
int oplus_mms_publish_ic_err_msg(struct oplus_mms *topic, u32 item_id, struct oplus_chg_ic_err_msg *err_msg)
{
	struct mms_msg *topic_msg;
	int rc;

	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, item_id, "[%s]-[%d]-[%d]:%s",
					    err_msg->ic->name, err_msg->type, err_msg->sub_type, err_msg->msg);
	if (topic_msg == NULL) {
...
int oplus_mms_analysis_ic_err_msg(char *buf, size_t buf_size, int *name_index, int *type, int *sub_type, int *msg_index)
{
	char *str;
	int index = 0;
...
__printf(4,
	 5) struct mms_subscribe *oplus_mms_subscribe(struct oplus_mms *mms, void *priv_data,
...
int oplus_mms_restore_publish(struct oplus_mms *mms)
{
	...
	queue_delayed_work(mms_wq, &mms->update_work, msecs_to_jiffies(mms->normal_update_interval));

	return 0;
}

#ifdef CONFIG_OPLUS_CHG_MMS_DEBUG
static ssize_t item_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t item_id_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_mms *mms = dev_get_drvdata(dev);
	int val;
...
static ssize_t subscribe_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_mms *mms = dev_get_drvdata(dev);
...
static struct device_attribute *oplus_mms_attributes[] = { &dev_attr_item_id, &dev_attr_data, &dev_attr_subscribe,
							   NULL };
...
static void debug_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	...
	item = oplus_mms_get_item(mms, id);
	if (item == NULL) {
		snprintf(data_env_buf + data_env_len, PAGE_SIZE - data_env_len, "NA");
	} else {
		read_lock(&item->lock);
		if (item->desc.str_data)
			snprintf(data_env_buf + data_env_len, PAGE_SIZE - data_env_len, "\"%s\"", item->data.strval);
		else
			snprintf(data_env_buf + data_env_len, PAGE_SIZE - data_env_len, "%d", item->data.intval);
		read_unlock(&item->lock);
	}
...
{
	struct oplus_mms *mms = container_of(work, struct oplus_mms, update_work.work);
...
{
	struct oplus_mms *mms = container_of(work, struct oplus_mms, msg_work.work);
...
struct oplus_mms *oplus_mms_get_by_name(const char *name)
{
	...
	struct device *dev = class_find_device(oplus_mms_class, NULL, name, oplus_mms_match_device_by_name);
...
static struct oplus_mms *__must_check __oplus_mms_register(struct device *parent, const struct oplus_mms_desc *desc,
							   const struct oplus_mms_config *cfg, bool ws)
{
	...
	if (cfg) {
		...
		mms->of_node = cfg->fwnode ? to_of_node(cfg->fwnode) : cfg->of_node;
...
#ifdef CONFIG_OPLUS_CHG_MMS_DEBUG
	mms->debug_subs = oplus_mms_subscribe(mms, mms, debug_subs_callback, "debug");
	if (IS_ERR_OR_NULL(mms->debug_subs)) {
		chg_err("debug subscribe %s topic error, rc=%ld\n", mms->desc->name, PTR_ERR(mms->debug_subs));
	}
#endif /* CONFIG_OPLUS_CHG_MMS_DEBUG */
...
struct oplus_mms *__must_check oplus_mms_register(struct device *parent, const struct oplus_mms_desc *desc,
						  const struct oplus_mms_config *cfg)
{
	return __oplus_mms_register(parent, desc, cfg, true);
}

struct oplus_mms *__must_check oplus_mms_register_no_ws(struct device *parent, const struct oplus_mms_desc *desc,
							const struct oplus_mms_config *cfg)
{
...
struct oplus_mms *__must_check devm_oplus_mms_register(struct device *parent, const struct oplus_mms_desc *desc,
						       const struct oplus_mms_config *cfg)
{
...
struct oplus_mms *__must_check devm_oplus_mms_register_no_ws(struct device *parent, const struct oplus_mms_desc *desc,
							     const struct oplus_mms_config *cfg)
{
	struct oplus_mms **ptr, *mms;
