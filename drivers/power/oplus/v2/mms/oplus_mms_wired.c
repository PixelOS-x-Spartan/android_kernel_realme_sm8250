...
__maybe_unused static bool is_chg_icl_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_chg_suspend_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_chg_disable_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_cool_down_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_vooc_curr_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_ufcs_curr_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_pd_svooc_votable_available(struct oplus_mms_wired *chip)
{
...
__maybe_unused static bool is_err_topic_available(struct oplus_mms_wired *chip)
{
	if (!chip->err_topic)
...
__maybe_unused static bool is_vooc_chg_auto_mode_votable_available(struct oplus_mms_wired *chip)
{
	if (!chip->vooc_chg_auto_mode_votable)
		chip->vooc_chg_auto_mode_votable = find_votable("VOOC_CHG_AUTO_MODE");
	return !!chip->vooc_chg_auto_mode_votable;
}

__maybe_unused static bool is_fv_max_votable_available(struct oplus_mms_wired *chip)
{
	if (!chip->fv_max_votable)
		chip->fv_max_votable = find_votable("FV_MAX");
	return !!chip->fv_max_votable;
}

__maybe_unused static bool is_fv_min_votable_available(struct oplus_mms_wired *chip)
{
	if (!chip->fv_min_votable)
		chip->fv_min_votable = find_votable("FV_MIN");
...
int oplus_wired_get_charger_cycle(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_CHARGER_CYCLE, &cycle);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
int oplus_wired_get_vbus(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL, &vbus_mv);
	if (rc < 0) {
...
bool oplus_wired_get_vbus_collapse_status(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS, &collapse);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
#endif
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ICL, false, step, icl_ma);
	if (rc < 0)
		chg_err("set icl to %d error, rc=%d\n", icl_ma, rc);
	else
		chg_info("set icl to %d, step=%s\n", icl_ma, step ? "true" : "false");
...
int oplus_wired_set_icl_by_vooc(struct oplus_mms *topic, int icl_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ICL, true, false, icl_ma);
	if (rc)
...
int oplus_wired_set_fcc(int fcc_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_FCC, fcc_ma);
	if (rc < 0)
...
int oplus_wired_set_aicl_point(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_AICL_POINT, vbatt);
	if (rc < 0)
...
bool oplus_wired_is_present(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_INPUT_PRESENT, &present);
	if (rc < 0) {
...
int oplus_wired_input_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND, !enable);
	if (rc < 0)
...
bool oplus_wired_input_is_enable(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND, &enable);
	if (rc < 0) {
...
int oplus_wired_output_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND, !enable);
	if (rc < 0)
...
bool oplus_wired_output_is_enable(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND, &enable);
	if (rc < 0) {
...
int oplus_wired_get_icl(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_ICL, &icl_ma);
	if (rc < 0) {
...
int oplus_wired_set_fv(int fv_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_FV, fv_mv);
	if (rc < 0)
...
int oplus_wired_get_fv(int *fv_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_FV, fv_mv);
	if (rc < 0) {
...
int oplus_wired_set_iterm(int iterm_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ITERM, iterm_ma);
	if (rc < 0)
...
int oplus_wired_set_rechg_vol(int vol_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL, vol_mv);
	if (rc < 0)
...
int oplus_wired_get_ibus(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR, &ibus_ma);
	if (rc < 0) {
...
static int oplus_wired_get_real_chg_type(struct oplus_mms_wired *chip)
{
	...
	if (chip->wired_present) {
		rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE, &type);
		if (rc < 0) {
...
int oplus_wired_otg_boost_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_OTG_BOOST_ENABLE, enable);
	if (rc < 0) {
	...
	if (is_vooc_chg_auto_mode_votable_available(chip))
		vote(chip->vooc_chg_auto_mode_votable, OTG_ONLINE_VOTER, enable, enable, false);
	else
...
int oplus_wired_set_otg_boost_vol(int vol_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_OTG_BOOST_VOL, vol_mv);
	if (rc < 0)
...
int oplus_wired_set_otg_boost_curr_limit(int curr_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT, curr_ma);
	if (rc < 0)
...
int oplus_wired_aicl_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_AICL_ENABLE, enable);
	if (rc < 0)
...
int oplus_wired_get_cc_orientation(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION, &cc_orientation);
	if (rc < 0) {
...
int oplus_wired_qc_detect_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE, enable);
	if (rc < 0)
...
{
	if (!chip->voocphy_ic)
		chip->voocphy_ic = of_get_oplus_chg_ic(chip->dev->of_node, "oplus,voocphy_ic", 0);
...
int oplus_wired_set_qc_config(enum oplus_chg_qc_version version, int vol_mv)
{
	...
	if (is_voocphy_ic_available(chip)) {
		if (vol_mv > 5000) {
			rc = oplus_chg_ic_func(chip->voocphy_ic, OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG);
			if (rc < 0) {
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG, version, vol_mv);
	if (rc < 0)
...
int oplus_wired_set_pd_config(u32 pdo)
{
	...
	if (is_voocphy_ic_available(chip)) {
		if ((PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_FIXED) && ((PD_SRC_PDO_FIXED_VOLTAGE(pdo) * 50) > 5000)) {
			rc = oplus_chg_ic_func(chip->voocphy_ic, OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG);
			if (rc < 0) {
				chg_err("can't set pdqc config,rc=%d\n", rc);
				return rc;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG, pdo);
	if (rc < 0)
...
int oplus_wired_get_usb_temp_volt(int *vol_l, int *vol_r)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_USB_TEMP_VOLT, vol_l, vol_r);
	if (rc < 0)
...
int oplus_wired_get_usb_temp(int *temp_l, int *temp_r)
{
	...
	for (i = chip->adc_vol_temp_info->con_volt_table_size - 1; i >= 0; i--) {
		if (chip->adc_vol_temp_info->con_volt_table[i] >= chip->usbtemp_volt_l)
			break;
		else if (i == 0)
			break;
	}
...
next:
	...
	for (i = chip->adc_vol_temp_info->con_volt_table_size - 1; i >= 0; i--) {
		if (chip->adc_vol_temp_info->con_volt_table[i] >= chip->usbtemp_volt_r)
			break;
		else if (i == 0)
			break;
	}
...
bool oplus_wired_usb_temp_check_is_support(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_USB_TEMP_CHECK_IS_SUPPORT, &support);
	if (rc < 0) {
...
int oplus_wired_get_usb_btb_temp(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP, &usb_btb_temp);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
int oplus_wired_get_batt_btb_temp(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP, &batt_btb_temp);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
enum oplus_chg_typec_port_role_type oplus_wired_get_typec_mode(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_TYPEC_MODE, &mode);
	if (rc < 0) {
...
int oplus_wired_set_typec_mode(enum oplus_chg_typec_port_role_type mode)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_TYPEC_MODE, mode);
	if (rc < 0)
...
int oplus_wired_set_otg_switch_status(bool en)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS, en);
	if (rc < 0)
...
bool oplus_wired_get_otg_switch_status(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS, &en);
	if (rc < 0) {
...
int oplus_wired_wdt_enable(struct oplus_mms *topic, bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_WDT_ENABLE, enable);
	if (rc < 0) {
...
int oplus_wired_get_shutdown_soc(struct oplus_mms *topic)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_SHUTDOWN_SOC, &soc);
	if (rc < 0) {
...
int oplus_wired_get_otg_online_status(struct oplus_mms *topic)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_OTG_ONLINE_STATUS, &online);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
int oplus_wired_set_usb_status(struct oplus_mms_wired *chip, unsigned int status)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_USB_STATUS);
	if (msg == NULL) {
...
static void oplus_init_usbtemp_wakelock(struct oplus_mms_wired *chip)
{
	...
	if (!is_awake_init) {
		...
		chip->usbtemp_wakelock = wakeup_source_register(NULL, "usbtemp suspend wakelock");
		is_awake_init = true;
	}
	return;
}
...
static void oplus_usbtemp_recover_func(struct oplus_mms_wired *chip)
{
	...
	if (chip->usb_status == USB_TEMP_HIGH) {
		...
		if (count_time == 30) {
			...
			(void)oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, false);
			rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_TYPEC_MODE, TYPEC_PORT_ROLE_ENABLE);
			if (rc < 0)
...
static void oplus_usbtemp_recover_work(struct work_struct *work)
{
	...
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, usbtemp_recover_work);
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, usbtemp_restart_work);
	bool dischg;
	int rc;
...
{
	struct oplus_mms_wired *chip = container_of(alarm, struct oplus_mms_wired, usbtemp_alarm_timer);
...
static int oplus_usbtemp_dischg_action(struct oplus_mms_wired *chip)
{
	...
	if (get_eng_version() != HIGH_TEMP_AGING) {
		...
		if (is_chg_suspend_votable_available(chip))
			rc = vote(chip->chg_suspend_votable, USB_VOTER, true, 1, false);
		else
			rc = -ENOTSUPP;
		if (rc < 0)
			chg_err("can't set charge suspend, rc=%d\n", rc);
		usleep_range(5000, 5000);
		rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_TYPEC_MODE, TYPEC_PORT_ROLE_DISABLE);
		if (rc < 0)
			chg_err("can't open cc, rc=%d\n", rc);
		usleep_range(5000, 5000);
		chg_err("set vbus down");
		oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, true);
		if (chip->vooc_charging)
			oplus_api_vooc_turn_off_fastchg(vooc_topic);
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	} else {
		chg_err("CONFIG_HIGH_TEMP_VERSION enable here,do not set vbus down \n");
		oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, false);
	}
#endif
	return 0;
}
...
static int oplus_usbtemp_push_err_msg(struct oplus_mms_wired *chip, int condition, int last_usb_temp_l,
				      int last_usb_temp_r, int batt_curr)
{
	...
	if (condition == USBTEMP_TRIGGER_CONDITION_1) {
		msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_USBTEMP,
					      "$$reason@@%s$$batt_temp@@%d"
					      "$$usb_temp_l@@%d$$usb_temp_r@@%d",
					      "first_condition", batt_temp, chip->usb_temp_l, chip->usb_temp_r);
	} else if (condition == USBTEMP_TRIGGER_CONDITION_2) {
		msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_USBTEMP,
					      "$$reason@@%s$$batt_temp@@%d"
					      "$$usb_temp_l@@%d$$last_usb_temp_l@@%d"
					      "$$usb_temp_r@@%d$$last_usb_temp_r@@%d",
					      "second_condition", batt_temp, chip->usb_temp_l, last_usb_temp_l,
		...
		msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_USBTEMP,
					      ...
					      "$$usb_temp_l@@%d$$usb_temp_r@@%d"
					      "$$batt_current@@%d",
					      "cool_down_condition", batt_temp, chip->usb_temp_l, chip->usb_temp_r,
					      batt_curr);
		...
		msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_USBTEMP,
					      ...
					      "$$usb_temp_l@@%d$$usb_temp_r@@%d"
					      "$$batt_current@@%d",
					      "cool_down_recovery_condition", batt_temp, chip->usb_temp_l,
					      chip->usb_temp_r, batt_curr);
	} else {
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (batt_temp / 10 <= USB_50C && (((chip->usb_temp_l >= USB_57C) && (chip->usb_temp_l < USB_100C)) ||
	...
	dischg:
		...
		if (usbtemp_recover_test || ((condition1 || condition2) && chip->dischg_flag)) {
			oplus_usbtemp_push_err_msg(
				chip, condition1 ? USBTEMP_TRIGGER_CONDITION_1 : USBTEMP_TRIGGER_CONDITION_2,
...
static bool oplus_usbtemp_judge_temp_gap(struct oplus_mms_wired *chip, int current_temp, int last_temp)
{
	struct oplus_usbtemp_spec_config *spec;
...
static bool oplus_usbtemp_change_curr_range(struct oplus_mms_wired *chip, int retry_cnt,
					    int usbtemp_first_time_in_curr_range, bool curr_range_change)
{
	...
	if (((current_curr_change_temp_l - last_curr_change_usb_temp_l) >= OPLUS_USBTEMP_CURR_CHANGE_TEMP) ||
		...
		if ((count_curr_l >= retry_cnt && chip->usb_temp_l > USB_30C && chip->usb_temp_l < USB_100C) ||
			...
			chg_info(
				"change curr range...,current_temp_l=%d,last_usb_temp_l=%d,current_temp_r=%d,last_usb_temp_r =%d, batt_temp = %d\n",
...
static bool oplus_usbtemp_trigger_for_high_temp(struct oplus_mms_wired *chip, int retry_cnt, int count_r, int count_l)
{
	struct oplus_usbtemp_spec_config *spec;
	int i = 0;
...
static bool oplus_usbtemp_trigger_for_rise_fast_temp(struct oplus_mms_wired *chip, int retry_cnt, int count_r,
						     int count_l)
{
	struct oplus_usbtemp_spec_config *spec;
	int batt_temp;
	...
	if (oplus_usbtemp_temp_rise_fast_with_batt_temp(chip)) {
		...
		for (i = 1; i <= retry_cnt; i++) {
			...
			oplus_wired_get_usb_temp(&chip->usb_temp_l, &chip->usb_temp_r);
			if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
				if ((chip->usb_temp_r >= batt_temp / 10 + spec->usbtemp_temp_gap_low_with_batt_temp) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >= batt_temp / 10 + spec->usbtemp_temp_gap_low_with_batt_temp) &&
				    chip->usb_temp_l < USB_100C)
					count_l++;
			} else if (chip->usbtemp_curr_status == OPLUS_USBTEMP_HIGH_CURR) {
				if ((chip->usb_temp_r >= batt_temp / 10 + spec->usbtemp_temp_gap_high_with_batt_temp) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >= batt_temp / 10 + spec->usbtemp_temp_gap_high_with_batt_temp) &&
				    chip->usb_temp_l < USB_100C)
					count_l++;
			}
			chg_info("countl: %d countr: %d", count_l, count_r);
...
static bool oplus_usbtemp_trigger_for_rise_fast_without_temp(struct oplus_mms_wired *chip, int retry_cnt, int count_r,
...
int oplus_usbtemp_monitor_common_new_method(void *data)
{
	...
	chg_info("run first, usbtemp_max_temp_thr[%d], usbtemp_temp_up_time_thr[%d]!", spec->usbtemp_max_temp_thr,
		 spec->usbtemp_temp_up_time_thr);
	...
	while (!kthread_should_stop()) {
		...
		if (!curr_range_change && batt_current < 5000 && chip->usbtemp_pre_batt_current >= 5000) {
			curr_range_change = true;
			curr_range_change_first_time = oplus_current_kernel_time();
		} else if (curr_range_change && batt_current >= 5000 && chip->usbtemp_pre_batt_current < 5000) {
...
static void oplus_usbtemp_thread_init(struct oplus_mms_wired *chip)
{
	...
	if (chip->support_usbtemp_protect_v2)
		chip->oplus_usbtemp_kthread =
			kthread_run(oplus_usbtemp_monitor_common_new_method, chip, "usbtemp_kthread");
	else
		chip->oplus_usbtemp_kthread = kthread_run(oplus_usbtemp_monitor_main, chip, "usbtemp_kthread");
...
static void oplus_wired_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_wired_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_wired_gauge_subs_callback, "mms_wired");
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_REAL_TEMP, &data, false);
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, gauge_update_work);
	union mms_msg_data data = { 0 };
	...
	(void)oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_VOOCPHY_SET_MATCH_TEMP, chip->shell_temp);
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_CURR, &data, false);
	chip->batt_curr = data.intval;
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, otg_enable_handler_work);
	struct mms_msg *msg;
	bool enable;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_OTG_ENABLE, &enable);
	if (rc < 0)
		enable = false;

	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_DATA_ROLE, (int *)&role);
	if (rc < 0)
	...
	if (enable) {
		rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_DISABLE_VBUS, true, false);
		if (rc < 0 && rc != -ENOTSUPP)
	...
	if (!enable && role == TYPEC_DEVICE) {
		rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_DISABLE_VBUS, false, true);
		if (rc < 0 && rc != -ENOTSUPP)
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_OTG_ENABLE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, data_role_changed_handler_work);
	...
	chg_info("otg=%s, data_role=%s\n", enable ? "enable" : "disable", (role == TYPEC_HOST) ? "host" : "device");
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, voltage_change_work);
	struct mms_msg *msg;
	int rc;
	 ...
	 */
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_CHARGER_VOL_MAX);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, current_change_work);
	struct mms_msg *msg;
	int rc;

	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_CHARGER_CURR_MAX);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, bc12_completed_work);
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_BC12_COMPLETED);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, back_ui_soc_work);
	union mms_msg_data data = { 0 };
...
static void oplus_wired_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_wired *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_wired_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_wired *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_wired_vooc_subs_callback, "mms_wired");
...
static void oplus_wired_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_wired *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_wired_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_wired_comm_subs_callback, "mms_wired");
...
static void oplus_wired_cpa_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_wired *chip = subs->priv_data;
...
static void oplus_wired_subscribe_cpa_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_wired *chip = prv_data;

	chip->cpa_topic = topic;
	chip->cpa_subs = oplus_mms_subscribe(chip->cpa_topic, chip, oplus_wired_cpa_subs_callback, "mms_wired");
...
static void oplus_wired_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_wired *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_wired_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_wired *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->ufcs_topic = topic;
	chip->ufcs_subs = oplus_mms_subscribe(chip->ufcs_topic, chip, oplus_wired_ufcs_subs_callback, "mms_wired");
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, mms_wired_init_work);
...
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, err_handler_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, wam.online_status_err_work);
	...
	if (report) {
		...
		msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_ONLINE_STATUS_ERR, 1);
		if (msg == NULL) {
			chg_err("alloc msg error\n");
		} else {
			rc = oplus_mms_publish_msg(chip->wired_topic, msg);
	...
	schedule_delayed_work(&wam->online_status_err_work, msecs_to_jiffies(ONLINE_STATUS_ERR_CHECK_DELAY_MS));
}

static void oplus_mms_wired_plugin_handler_work(struct work_struct *work)
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, plugin_handler_work);
	struct mms_msg *msg;
	union mms_msg_data data = { 0 };
	bool online, present;
	...
	if (chip->vooc_topic) {
		oplus_mms_get_item_data(chip->vooc_topic, VOOC_ITEM_ONLINE_KEEP, &data, false);
		chip->vooc_online_keep = data.intval;
	} else {
	...
	if (chip->wired_present != present) {
		...
		if (!present)
			oplus_wired_set_aicl_point();
		msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_PRESENT);
		if (msg == NULL) {
			chg_err("alloc msg error\n");
			goto skip_present;
		}
...
skip_present:
	...
	chg_info("present=%d, vooc_online_keep=%d, pre_online=%d", present, chip->vooc_online_keep, chip->wired_online);
	if (online != present) {
	...
	if (chip->wired_online == online) {
		 ...
		 */
		if (online && present_changed && present && is_chg_icl_votable_available(chip))
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_ONLINE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		goto check_data_role;
	}
...
static void oplus_mms_wired_chg_type_change_handler_work(struct work_struct *work)
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, chg_type_change_handler_work);
	struct mms_msg *msg;
	int rc;
	int real_chg_type;
	union mms_msg_data data = { 0 };
...
static void oplus_mms_wired_cpa_chg_type_change_handler_work(struct work_struct *work)
{
	struct oplus_mms_wired *chip = container_of(work, struct oplus_mms_wired, cpa_chg_type_change_handler_work);
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_CHG_TYPE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, ccdetect_work);
	struct mms_msg *msg;
	int hw_detect;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_ITEM_CC_DETECT);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, typec_state_notify_work);
	struct mms_msg *msg;
	int typec_state;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_TIME_TYPEC_STATE);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, typec_state_change_work);
	int hw_detect;
	int rc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(dwork, struct oplus_mms_wired, svid_handler_work);
	struct votable *pd_boost_disable_votable;
	bool oplus_svid;
	int rc;
	struct mms_msg *msg;

	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_IS_OPLUS_SVID, &oplus_svid);
	if (rc < 0) {
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, WIRED_TIME_ABNORMAL_ADAPTER);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
	}
	rc = oplus_mms_publish_msg(chip->wired_topic, msg);
...
static void oplus_wired_err_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
	schedule_work(&chip->err_handler_work);
}

static void oplus_wired_plugin_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_type_change_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
	schedule_work(&chip->chg_type_change_handler_work);
}

static void oplus_wired_svid_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
	schedule_delayed_work(&chip->svid_handler_work, 0);
}

static void oplus_wired_otg_enable_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_cc_detect_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_typec_state_notify_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_cc_changed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_voltage_changed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_current_changed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_bc12_completed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static void oplus_wired_data_role_changed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_wired *chip = virq_data;
...
static int oplus_mms_wired_virq_register(struct oplus_mms_wired *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_ERR, oplus_wired_err_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ERR error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_PLUGIN, oplus_wired_plugin_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_PLUGIN error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_CHG_TYPE_CHANGE, oplus_wired_type_change_handler,
					chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_CHG_TYPE_CHANGE error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_SVID, oplus_wired_svid_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_SVID error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_OTG_ENABLE, oplus_wired_otg_enable_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_OTG_ENABLE error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_CC_DETECT, oplus_wired_cc_detect_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_CC_DETECT error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_CC_CHANGED, oplus_wired_cc_changed_handler, chip);
	if (rc < 0)
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_BC12_COMPLETED, oplus_wired_bc12_completed_handler,
					chip);
	if (rc < 0)
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic, OPLUS_IC_VIRQ_DATA_ROLE_CHANGED,
					oplus_wired_data_role_changed_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_DATA_ROLE_CHANGED error, rc=%d", rc);

	return 0;
}

static int oplus_mms_wired_update_present(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	bool present = false;
...
static int oplus_mms_wired_update_online(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	bool present = false;
...
static int oplus_mms_wired_update_chg_type(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
...
static int oplus_mms_wired_update_bc12_completed(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
...
static int oplus_mms_wired_update_cc_mode(struct oplus_mms *mms, union mms_msg_data *data)
{
	/* TODO */
	return 0;
}

static int oplus_mms_wired_update_cc_detect(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_HW_DETECT, &cc_detect);
	if (rc < 0) {
...
static int oplus_mms_wired_update_usb_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int usb_status = 0;
...
static int oplus_mms_wired_update_usb_temp_volt_l(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
...
static int oplus_mms_wired_update_usb_temp_volt_r(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int volt = USBTEMP_DEFAULT_VOLT_VALUE_MV;
...
static int oplus_mms_wired_update_usb_temp_l(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int temp = 25;
...
static int oplus_mms_wired_update_usb_temp_r(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int temp = 25;
...
static int oplus_mms_wired_update_otg_enable_status(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	bool enable = false;
...
static int oplus_mms_wired_update_chg_curr_max(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int curr_max;
	int rc;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_CHARGER_CURR_MAX, &curr_max);
	if (rc < 0) {
...
static int oplus_mms_wired_update_chg_vol_max(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int vol_max;
	int rc;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_CHARGER_VOL_MAX, &vol_max);
	if (rc < 0) {
...
static int oplus_mms_wired_update_chg_vol_min(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int vol_min;
	int rc;
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_CHARGER_VOL_MIN, &vol_min);
	if (rc < 0) {
...
static int oplus_mms_abnoramal_adapter(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
...
static int oplus_mms_get_typec_state(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int typec_state;
	int rc;
...
static int oplus_mms_wired_update_real_chg_type(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_wired *chip;
	int type = OPLUS_CHG_USB_TYPE_UNKNOWN;
...
static int oplus_mms_wired_vbus(struct oplus_mms *mms, union mms_msg_data *data)
{
	int vbus;
...
int oplus_wired_get_byb_id_info(struct oplus_mms *topic)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_BYBID_INFO, &bybid_info);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
static int oplus_mms_wired_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_mms_wired), GFP_KERNEL);
	if (chip == NULL) {
	...
	INIT_DELAYED_WORK(&chip->mms_wired_init_work, oplus_mms_wired_init_work);
	...
	INIT_WORK(&chip->plugin_handler_work, oplus_mms_wired_plugin_handler_work);
	INIT_WORK(&chip->chg_type_change_handler_work, oplus_mms_wired_chg_type_change_handler_work);
	INIT_WORK(&chip->cpa_chg_type_change_handler_work, oplus_mms_wired_cpa_chg_type_change_handler_work);
	...
	INIT_WORK(&chip->data_role_changed_handler_work, oplus_wired_data_role_changed_handler_work);
...
static void oplus_mms_wired_shutdown(struct platform_device *pdev)
{
	...
	if (chip->ship_mode) {
		rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE, true);
		if (rc < 0)
