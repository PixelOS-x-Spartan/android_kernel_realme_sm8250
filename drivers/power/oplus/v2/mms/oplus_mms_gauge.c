...
__maybe_unused static bool is_err_topic_available(struct oplus_mms_gauge *chip)
{
	if (!chip->err_topic)
...
static void oplus_gauge_get_deep_limits(struct oplus_mms_gauge *chip)
{
	...
	for (index = chip->deep_spec.count_curves.nums - 1; index >= 0; index--) {
		counts = chip->deep_spec.count_curves.limits[index].count < chip->deep_spec.config.count_cali ?
				 0 :
				 (chip->deep_spec.count_curves.limits[index].count - chip->deep_spec.config.count_cali);
...
static void oplus_gauge_init_sili_status(struct oplus_mms_gauge *chip)
{
	...
	snprintf(deep_id_info, DUMP_INFO_LEN,
		 "$$deep_support@@%d$$byb_id@@%d$$batt_id@@%d$$sili_err@@%d$$counts@@%d$$uv_thr@@%d",
		 chip->deep_spec.support, bybid, batt_id, chip->deep_spec.sili_err, chip->deep_spec.counts,
...
static int oplus_gauge_get_batt_temperature(struct oplus_mms_gauge *chip)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP,
			       &main_temp);
	if (rc < 0) {
...
bool oplus_gauge_is_exist(struct oplus_mms *topic)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_EXIST, &exist);
	if (rc < 0) {
...
int oplus_gauge_get_batt_capacity_mah(struct oplus_mms *topic)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		rc = oplus_chg_ic_func(chip->gauge_ic_parallel[i], OPLUS_IC_FUNC_GAUGE_GET_BATT_CAP, &cap_temp);
		if (rc < 0) {
...
int oplus_gauge_get_battinfo_sn(struct oplus_mms *topic, char *sn_buff, int size_buffer)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_SN, sn_buff,
			       size_buffer);
	len = rc;
	if (rc < 0) {
		chg_err("get main battery serial number error, rc=%d\n", rc);
	} else {
		if (chip->sub_gauge && ((size_buffer - len) > OPLUS_BATT_SERIAL_NUM_SIZE)) {
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, set_gauge_batt_full_work);
	...
	if ((chip->batt_temp_region == TEMP_REGION_COLD || chip->batt_temp_region == TEMP_REGION_LITTLE_COLD ||
	     chip->batt_temp_region == TEMP_REGION_COOL || chip->batt_temp_region == TEMP_REGION_LITTLE_COOL ||
	     chip->batt_temp_region == TEMP_REGION_PRE_NORMAL || chip->batt_temp_region == TEMP_REGION_NORMAL) &&
	    chip->batt_full) {
...
int oplus_gauge_get_dod0(struct oplus_mms *mms, int index, int *val)
{
	...
	if (is_support_parallel(chip)) {
		switch (index) {
		case 0:
			rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_DOD0,
					       0, val);
			if (rc < 0) {
...
int oplus_gauge_get_qmax(struct oplus_mms *mms, int index, int *val)
{
	...
	if (is_support_parallel(chip)) {
		switch (index) {
		case 0:
			rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_QMAX,
					       0, val);
			if (rc < 0) {
...
static int oplus_mms_gauge_set_err_code(struct oplus_mms_gauge *chip, unsigned int err_code)
{
	struct mms_msg *msg;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, GAUGE_ITEM_ERR_CODE);
	if (msg == NULL) {
...
static int oplus_gauge_parse_deep_spec(struct oplus_mms_gauge *chip)
{
	...
	rc = of_property_read_u32(node, "deep_spec,uv_thr", &chip->deep_spec.config.uv_thr);
	if (rc < 0)
		chip->deep_spec.config.uv_thr = 3000;

	rc = of_property_read_u32(node, "deep_spec,count_cali", &chip->deep_spec.config.count_cali);
	if (rc < 0)
		chip->deep_spec.config.count_cali = 0;

	rc = of_property_read_u32(node, "deep_spec,count_thr", &chip->deep_spec.config.count_thr);
	if (rc < 0)
		chip->deep_spec.config.count_thr = 1;

	rc = of_property_read_u32(node, "deep_spec,vbat_soc", &chip->deep_spec.config.soc);
	if (rc < 0)
	...
	length = rc;
	rc = of_property_read_u32_array(node, "deep_spec,batt_curve", (u32 *)chip->deep_spec.batt_curves.limits,
					length);
	...
	length = rc;
	rc = of_property_read_u32_array(node, "deep_spec,count_curve", (u32 *)chip->deep_spec.count_curves.limits,
					length);
...
static void oplus_mms_gauge_init_work(struct work_struct *work)
{
	...
	struct oplus_mms_gauge *chip = container_of(dwork, struct oplus_mms_gauge, hal_gauge_init_work);
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		if (chip->child_num >= GAUGE_PARALLEL_IC_NUM_MIN) {
			rc = of_property_read_u32_index(node, "oplus,gauge_ic_capacity_ratio", i,
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE, &chip->device_type);
	if (rc < 0) {
		chg_err("can't get device type, rc=%d\n", rc);
		chip->device_type = 0;
	}
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE_FOR_VOOC,
...
init_try_again:
	if (retry > 0) {
		retry--;
		schedule_delayed_work(&chip->hal_gauge_init_work, msecs_to_jiffies(OPLUS_CHG_IC_INIT_RETRY_DELAY));
	} else {
...
static void oplus_mms_gauge_err_analyze(struct oplus_mms_gauge *chip, struct oplus_chg_ic_err_msg *msg)
{
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, err_handler_work);
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, online_handler_work);
	struct mms_msg *msg;
	unsigned int err_code;
	int rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, GAUGE_ITEM_BATT_EXIST);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, offline_handler_work);
	struct mms_msg *msg;
	int rc;
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, resume_handler_work);
	struct mms_msg *msg;
	int rc;

	msg = oplus_mms_alloc_int_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, GAUGE_ITEM_RESUME, 1);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return;
	}
...
static void oplus_mms_gauge_err_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_gauge *chip = virq_data;
	schedule_work(&chip->err_handler_work);
}

static void oplus_mms_gauge_online_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_gauge *chip = virq_data;
	schedule_work(&chip->online_handler_work);
}

static void oplus_mms_gauge_offline_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_gauge *chip = virq_data;
	schedule_work(&chip->offline_handler_work);
}

static void oplus_mms_gauge_resume_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_mms_gauge *chip = virq_data;
...
static int oplus_mms_gauge_update_soc(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC, &main_soc);
	if (rc < 0) {
		chg_err("get battery soc error, rc=%d\n", rc);
		soc = DEFAULT_SOC;
		goto end;
	...
	if (chip->sub_gauge) {
		...
		soc_remainder = (main_soc * chip->child_list[chip->main_gauge].capacity_ratio +
				 sub_soc * chip->child_list[__ffs(chip->sub_gauge)].capacity_ratio) %
				100;
		soc = (main_soc * chip->child_list[chip->main_gauge].capacity_ratio +
		       sub_soc * chip->child_list[__ffs(chip->sub_gauge)].capacity_ratio) /
		      100;
		chg_info(" main_soc:%d, sub_soc:%d, main_ratio:%d, sub_ratio:%d, soc:%d, remainder:%d", main_soc,
			 sub_soc, chip->child_list[chip->main_gauge].capacity_ratio,
...
static int oplus_mms_sub_gauge_update_soc(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[gauge_index], OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC, &soc);
	if (rc < 0) {
		chg_err("get battery soc error, rc=%d\n", rc);
		soc = DEFAULT_SOC;
	}
...
static int oplus_mms_gauge_choice_fit_vol(struct oplus_mms_gauge *chip, int gauge_vol)
{
	...
	ret = oplus_chg_ic_func(chip->voocphy_ic, OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT, &cp_vbat);
	if (ret < 0) {
		chg_err("can't get cp voltage, rc=%d\n", ret);
		return gauge_vol;
	}

	if (cp_vbat >= vbatt_ov_thr_mv || cp_vbat <= 0 || abs(cp_vbat - gauge_vol) > MAX_CP_GAUGE_VBATT_DIFF) {
...
static int oplus_mms_sub_gauge_update_vol_max(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[gauge_index], OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX, &vol);
	if (rc < 0) {
		chg_err("get battery voltage max error, rc=%d\n", rc);
		vol = 0;
	}
...
static int oplus_mms_gauge_update_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR,
			       &main_curr);
	if (rc < 0)
		main_curr = 0;
	if (chip->sub_gauge) {
...
static int oplus_mms_sub_gauge_update_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[gauge_index], OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR, &curr);
	if (rc < 0) {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_gauge *chip = container_of(dwork, struct oplus_mms_gauge, subboard_ntc_err_work);
	...
	do {
		...
		if (((subboard_temp <= SUBBOARD_LOW_ABNORMAL_TEMP) && (gauge_temp >= GAUGE_LOW_ABNORMAL_TEMP)) ||
	...
	if (err_count >= ERR_COUNT_MAX) {
		...
		oplus_chg_ic_creat_err_msg(chip->child_list[chip->main_gauge].ic_dev, OPLUS_IC_ERR_GAUGE, 0,
					   "bad_subboard[%d]", subboard_temp);
...
static int oplus_mms_gauge_track_check(char *deep_msg)
{
	...
	msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_DEEP_DISCHG_INFO, deep_msg);
	if (msg == NULL) {
...
static void oplus_gauge_update_deep_dischg(struct oplus_mms_gauge *chip)
{
	...
	ui_soc = chip->ui_soc;
	rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_UI_SOC, &data, true);
	if (rc < 0) {
	...
	if (track_check || mms_gauge_debug_track) {
		...
		batt_id = oplus_gauge_get_batt_id_info(chip);
		index = snprintf(
			reg_info, REG_INFO_LEN,
			"$$dischg_counts@@%d$$count_thr@@%d$$count_cali@@%d$$vbat_uv@@%d$$vbat_min@@%d$$ui_soc@@%d$$ibat_ma@@%d$$bybid@@%d$$batt_id@@%d$$sili_err@@%d",
			chip->deep_spec.counts, chip->deep_spec.config.count_thr, chip->deep_spec.config.count_cali,
			chip->deep_spec.config.uv_thr, vbat_min_mv, chip->ui_soc, ibat_ma, bybid, batt_id,
			chip->deep_spec.sili_err);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_gauge *chip = container_of(dwork, struct oplus_mms_gauge, deep_dischg_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_gauge *chip = container_of(dwork, struct oplus_mms_gauge, deep_id_work);
...
static int oplus_mms_gauge_update_temp(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	if (chip->support_subboard_ntc && !chip->check_subboard_ntc_err) {
		...
		if (gauge_temp <= GAUGE_INVALID_TEMP) {
			temp = gauge_temp;
		} else if (((temp <= SUBBOARD_LOW_ABNORMAL_TEMP) && (gauge_temp >= GAUGE_LOW_ABNORMAL_TEMP)) ||
...
static int oplus_mms_sub_gauge_update_temp(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[gauge_index], OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP, &temp);
	if (rc < 0) {
		chg_err("get battery current error, rc=%d\n", rc);
		temp = GAUGE_INVALID_TEMP;
	}
...
static int oplus_mms_gauge_get_reg_info(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_REG_INFO, chip->gauge_reg_info[0],
			       GAUGE_REG_INFO_SIZE);
	if (rc == -ENOTSUPP)
		rc = 0;
...
static int oplus_mms_gauge_get_calib_time(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	chip = oplus_mms_get_drvdata(mms);
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_CALIB_TIME, &dod_calib_time, &qmax_calib_time);
	if (rc == -ENOTSUPP)
		rc = 0;
...
static int oplus_mms_gauge_update_fcc(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC, &main_fcc);
	if (rc < 0) {
		chg_err("get battery fcc error, rc=%d\n", rc);
		main_fcc = 0;
	}
...
static int oplus_mms_gauge_update_rm(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_RM, &main_rm);
	if (rc < 0) {
		chg_err("get battery remaining capacity error, rc=%d\n", rc);
		main_rm = 0;
	}
	if (chip->sub_gauge) {
		rc = oplus_chg_ic_func(chip->gauge_ic_parallel[__ffs(chip->sub_gauge)], OPLUS_IC_FUNC_GAUGE_GET_BATT_RM,
				       &sub_rm);
		if (rc < 0) {
...
static int oplus_mms_gauge_update_cc(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	chip = oplus_mms_get_drvdata(mms);
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_CC, &main_cc);
	if ((rc < 0) || (main_cc <= 0)) {
		chg_err("get battery cc error, rc=%d\n", rc);
		main_cc = 0;
	}
	if (chip->sub_gauge) {
		rc = oplus_chg_ic_func(chip->gauge_ic_parallel[__ffs(chip->sub_gauge)], OPLUS_IC_FUNC_GAUGE_GET_BATT_CC,
				       &sub_cc);
		if ((rc < 0) || (sub_cc <= 0)) {
			...
			cc = (main_cc * chip->child_list[chip->main_gauge].capacity_ratio +
			      sub_cc * chip->child_list[__ffs(chip->sub_gauge)].capacity_ratio) /
			     100;
		} else {
			cc = sub_cc;
		}
		chg_info("main_cc:%d, sub_cc:%d, main_ratio:%d, sub_ratio:%d, cc:%d\n", main_cc, sub_cc,
			 chip->child_list[chip->main_gauge].capacity_ratio,
...
static int oplus_mms_gauge_get_zy_gauge_soh(struct oplus_mms *mms, int gauge_type)
{
	...
	if (DEVICE_ZY0603 == gauge_type) {
		...
		chg_info("series qmax_1:%d, qmax_2:%d, capacity_mah:%d, soh:%d, gauge_type:%d\n", batt_qmax_1,
			 batt_qmax_2, batt_capacity_mah, batt_soh, gauge_type);
		...
		if (is_support_parallel(chip)) {
			...
			if (batt_qmax_1 > 0 && batt_qmax_2 >= 0) {
				...
				batt_capacity_mah =
					batt_capacity_mah * chip->child_list[chip->main_gauge].capacity_ratio / 100;
			...
			chg_info("singal qmax:%d, capacity_mah:%d, soh:%d, gauge_type:%d\n", batt_qmax,
				 batt_capacity_mah, batt_soh, gauge_type);
		}
	}
...
static int oplus_mms_gauge_update_soh(struct oplus_mms *mms, union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic_parallel[chip->main_gauge], OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH, &main_soh);
	if (rc < 0 || main_soh <= 0) {
	...
	if (chip->sub_gauge) {
		...
		if (rc < 0 || sub_soh <= 0) {
			...
			soh = (main_soh * chip->child_list[chip->main_gauge].capacity_ratio +
			       sub_soh * chip->child_list[__ffs(chip->sub_gauge)].capacity_ratio) /
			      100;
		} else {
			soh = sub_soh;
		}
...
static int oplus_mms_gauge_update_exist(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	bool exist;
	int rc;
...
static int oplus_mms_gauge_update_err_code(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	unsigned int err_code;
...
static int oplus_mms_gauge_update_hmac(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
...
static int oplus_mms_gauge_update_auth(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	bool auth;
	int rc;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH, &auth);
	if (rc < 0) {
		chg_err("get battery authenticate status error, rc=%d\n", rc);
		auth = false;
	}
...
static const u32 oplus_mms_gauge_update_item[] = {
	GAUGE_ITEM_SOC, GAUGE_ITEM_VOL_MAX, GAUGE_ITEM_VOL_MIN, GAUGE_ITEM_CURR, GAUGE_ITEM_TEMP,
	GAUGE_ITEM_FCC, GAUGE_ITEM_CC,	    GAUGE_ITEM_SOH,	GAUGE_ITEM_RM,	 GAUGE_ITEM_REAL_TEMP,
};
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, update_change_work);
...
static void oplus_mms_gauge_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_mms_gauge_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_gauge *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->comm_topic = topic;
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_mms_gauge_comm_subs_callback, "mms_gauge");
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, gauge_set_curve_work);
	int type;
	unsigned int adapter_id;

	type = oplus_wired_get_chg_type();
	if ((type != OPLUS_CHG_USB_TYPE_VOOC) && (type != OPLUS_CHG_USB_TYPE_SVOOC)) {
	...
	(void)oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_SET_BATTERY_CURVE, type, adapter_id,
				chip->pd_svooc);
}

static void oplus_mms_gauge_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_gauge *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_mms_gauge_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_gauge *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->wired_subs =
		oplus_mms_subscribe(chip->wired_topic, chip, oplus_mms_gauge_wired_subs_callback, "mms_gauge");
...
static int oplus_mms_gauge_push_hmac(struct oplus_mms_gauge *chip)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, GAUGE_ITEM_HMAC);
	if (msg == NULL) {
...
static int oplus_mms_gauge_push_auth(struct oplus_mms_gauge *chip)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH, GAUGE_ITEM_AUTH);
	if (msg == NULL) {
	...
	if (abs(vbat_max - vbat_min) > ALLOW_DIFF_VALUE_MV) {
		...
		if (chip->check_batt_vol_count > BATT_VOL_ERR_COUNT_MAX) {
			...
			if (chg_disable_votable)
				vote(chg_disable_votable, VOL_DIFF_VOTER, true, 1, false);
			if (!chip->bat_volt_different)
				oplus_mms_gauge_push_auth(chip);
		...
		if (chip->bat_volt_different) {
			...
			if (chip->check_batt_vol_count > BATT_VOL_ERR_COUNT_MAX) {
				...
				chg_disable_votable = find_votable("CHG_DISABLE");
				if (chg_disable_votable)
					vote(chg_disable_votable, VOL_DIFF_VOTER, false, 0, false);
				chg_info("Recovery BATTERY_SOFT_DIFF_VOLTAGE\n");
			}
		} else {
			chip->check_batt_vol_count = 0;
		}
...
{
	struct oplus_mms_gauge *chip = container_of(work, struct oplus_mms_gauge, gauge_update_work);

	oplus_mms_gauge_check_batt_vol_diff(chip);
}

static void oplus_mms_gauge_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_gauge *chip = subs->priv_data;
...
{
	chip->gauge_subs =
		oplus_mms_subscribe(chip->gauge_topic, chip, oplus_mms_gauge_gauge_subs_callback, "mms_gauge");
...
static void oplus_mms_gauge_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_gauge *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		...
		case VOOC_ITEM_SID:
			...
			if (chip->vooc_sid && chip->wired_online && is_voocphy_ic_available(chip))
				schedule_work(&chip->gauge_set_curve_work);
			break;
		default:
			break;
...
static void oplus_mms_gauge_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_gauge *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_mms_gauge_vooc_subs_callback, "mms_gauge");
...
static void oplus_mms_gauge_parallel_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_gauge *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_mms_gauge_subscribe_parallel_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_mms_gauge *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->parallel_subs =
		oplus_mms_subscribe(chip->parallel_topic, chip, oplus_mms_gauge_parallel_subs_callback, "mms_gauge");
...
static void oplus_mms_gauge_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_mms_gauge *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static int oplus_mms_gauge_topic_init(struct oplus_mms_gauge *chip)
{
	...
	for (i = 0; i < chip->child_num; i++)
		chip->gauge_reg_info[i] =
			devm_kzalloc(chip->dev, sizeof(unsigned char) * GAUGE_REG_INFO_SIZE, GFP_KERNEL);

	if (is_support_parallel(chip)) {
		mms_cfg.update_interval = 0;
		mms_desc = devm_kzalloc(chip->dev, sizeof(struct oplus_mms_desc) * chip->child_num, GFP_KERNEL);
		for (i = 0; i < chip->child_num; i++) {
			name = devm_kzalloc(chip->dev, sizeof(char) * GAUGE_NAME_LENGTH, GFP_KERNEL);
			if (i != 0) {
				item = devm_kzalloc(chip->dev,
						    sizeof(struct mms_item) * ARRAY_SIZE(oplus_mms_sub_gauge_item),
						    GFP_KERNEL);
...
static int oplus_gauge_update_vote_callback(struct votable *votable, void *data, int time_ms, const char *client,
					    bool step)
{
...
static int oplus_mms_gauge_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_mms_gauge), GFP_KERNEL);
	if (chip == NULL) {
	...
	chip->gauge_update_votable = create_votable("GAUGE_UPDATE", VOTE_MIN, oplus_gauge_update_vote_callback, chip);
