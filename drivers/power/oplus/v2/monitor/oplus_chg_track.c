...
#endif
static int oplus_chg_track_get_charger_type(struct oplus_monitor *monitor, struct oplus_chg_track_status *track_status,
					    int type);
...
static struct oplus_chg_track_speed_ref wls_series_double_cell_bpp[] = {
	{ TRACK_REF_SOC_50, TRACK_POWER_2000MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_10000MV },
	{ TRACK_REF_SOC_75, TRACK_POWER_2000MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_10000MV },
	{ TRACK_REF_SOC_90, TRACK_POWER_2500MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_10000MV },
};

static struct oplus_chg_track_speed_ref wls_equ_single_cell_bpp[] = {
	{ TRACK_REF_SOC_50, TRACK_POWER_2000MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_5000MV },
	{ TRACK_REF_SOC_75, TRACK_POWER_2000MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_5000MV },
	{ TRACK_REF_SOC_90, TRACK_POWER_2000MW, TRACK_POWER_2000MW * 1000 / TRACK_REF_VOL_5000MV },
};
...
static void oplus_track_upload_parallelchg_foldmode_info(struct work_struct *work)
{
	...
	index += snprintf(
		&(parallel_foldmode_p->load_trigger_info->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
		...
		"COMMON[%d %d %d 0x%x %d], "
		"SLOW[%d %d %d %d %d %d %d]",
		monitor->batt_temp, monitor->shell_temp, monitor->vbat_mv, monitor->vbat_min_mv, monitor->ibat_ma,
		monitor->batt_soc, monitor->smooth_soc, monitor->ui_soc, monitor->batt_rm, monitor->batt_fcc,
		monitor->batt_exist, monitor->batt_err_code, monitor->fv_mv, monitor->fcc_ma, monitor->chg_disable,
		monitor->chg_user_disable, monitor->wired_online, monitor->wired_ibus_ma, monitor->wired_vbus_mv,
		monitor->wired_icl_ma, monitor->wired_charge_type, monitor->wired_err_code, monitor->wired_suspend,
		monitor->wired_user_suspend, monitor->cc_mode, monitor->cc_detect, monitor->otg_enable,
		monitor->wls_online, monitor->wls_iout_ma, monitor->wls_vout_mv, monitor->wls_icl_ma,
		monitor->wls_charge_type, monitor->wls_err_code, monitor->wls_suspend, monitor->wls_user_suspend,
		monitor->vooc_online, monitor->vooc_started, monitor->vooc_charging, monitor->vooc_online_keep,
		monitor->vooc_sid, monitor->temp_region, monitor->ffc_status, monitor->cool_down, monitor->notify_code,
		monitor->led_on, track_status->has_judge_speed, track_status->soc_low_sect_incr_rm,
...
static int oplus_chg_track_get_wired_type_info(int charge_type, struct oplus_chg_track_status *track_status)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(wired_type_table); i++) {
		if (wired_type_table[i].type == charge_type) {
			strncpy(track_status->power_info.wired_info.adapter_type, wired_type_table[i].name,
...
__maybe_unused static int oplus_chg_track_get_wls_adapter_type_info(int charge_type,
								    struct oplus_chg_track_status *track_status)
{
	int i;
	int charge_index = -EINVAL;
...
__maybe_unused static int oplus_chg_track_get_wls_dock_type_info(int charge_type,
								 struct oplus_chg_track_status *track_status)
{
	int i;
	int charge_index = -EINVAL;
...
static int oplus_chg_track_record_general_info(struct oplus_monitor *monitor,
					       struct oplus_chg_track_status *track_status,
					       oplus_chg_track_trigger *p_trigger_data, int index)
{
	...
	index +=
		snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			 ...
			 "COMMON[%d %d %d 0x%x %d], "
			 "SLOW[%d %d %d %d %d %d %d]",
			 monitor->batt_temp, monitor->shell_temp, monitor->vbat_mv, monitor->vbat_min_mv,
			 monitor->ibat_ma, monitor->batt_soc, monitor->smooth_soc, monitor->ui_soc, monitor->batt_rm,
			 monitor->batt_fcc, monitor->batt_exist, monitor->batt_err_code, monitor->fv_mv,
			 monitor->fcc_ma, monitor->chg_disable, monitor->chg_user_disable, monitor->wired_online,
			 monitor->wired_ibus_ma, monitor->wired_vbus_mv, monitor->wired_icl_ma,
			 monitor->wired_charge_type, monitor->wired_err_code, monitor->wired_suspend,
			 monitor->wired_user_suspend, monitor->cc_mode, monitor->cc_detect, monitor->otg_enable,
			 monitor->wls_online, monitor->wls_iout_ma, monitor->wls_vout_mv, monitor->wls_icl_ma,
			 monitor->wls_charge_type, monitor->wls_err_code, monitor->wls_suspend,
			 monitor->wls_user_suspend, monitor->vooc_online, monitor->vooc_started, monitor->vooc_charging,
			 monitor->vooc_online_keep, monitor->vooc_sid, monitor->temp_region, monitor->ffc_status,
			 monitor->cool_down, monitor->notify_code, monitor->led_on, track_status->has_judge_speed,
	...
	if (track_status->power_info.power_type == TRACK_CHG_TYPE_WIRELESS) {
		if (strlen(track_status->wls_break_crux_info))
			index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
					  "%s ", track_status->wls_break_crux_info);
	}
	chg_info("index:%d\n", index);
	return 0;
}
...
static void oplus_chg_track_record_charger_info(struct oplus_monitor *monitor, oplus_chg_track_trigger *p_trigger_data,
						struct oplus_chg_track_status *track_status)
{
	int index = 0;
	...
	if (track_status->aging_ffc_trig && index < OPLUS_CHG_TRACK_CURX_INFO_LEN) {
		...
		for (i = 0; i < FFC_CHG_STEP_MAX; i++) {
			if (track_status->aging_ffc_judge_vol[i] <= 0 || index >= OPLUS_CHG_TRACK_CURX_INFO_LEN)
				break;
			index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
	...
	if (track_status->dual_chan_time)
		index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				  "$$dual_chan_status@@%d,%d", track_status->dual_chan_open_count,
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, usbtemp_load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, vbatt_too_low_load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, vbatt_diff_over_load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, uisoc_keep_1_t_load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, ic_err_msg_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, chg_into_liquid_trigger_work);
...
static void oplus_chg_track_chg_into_liquid_trigger_timeout_work(struct work_struct *work)
{
	...
	struct oplus_chg_track *chip =
		container_of(dwork, struct oplus_chg_track, chg_into_liquid_trigger_work_timeout);

	chip->oplus_liquid_intake_enable = true;
}

static void oplus_chg_track_dual_chan_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, dual_chan_err_load_trigger_work);
...
static void oplus_chg_track_cal_chg_five_mins_capacity_work(struct work_struct *work)
{
	...
	chip->track_status.chg_five_mins_cap = monitor->batt_soc - chip->track_status.chg_start_soc;
	chg_info("chg_five_mins_soc:%d, start_chg_soc:%d, chg_five_mins_cap:%d\n", monitor->batt_soc,
		 chip->track_status.chg_start_soc, chip->track_status.chg_five_mins_cap);
}
...
static void oplus_chg_track_cal_chg_ten_mins_capacity_work(struct work_struct *work)
{
	...
	chip->track_status.chg_ten_mins_cap = monitor->batt_soc - chip->track_status.chg_start_soc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, plugout_state_work);
	struct oplus_monitor *monitor = chip->monitor;
	struct oplus_chg_track_status *track_status = &chip->track_status;

	if (((monitor->wired_online == false) && (monitor->wired_vbus_mv < 2500)) &&
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, wired_online_err_trigger_work);
	int index = 0;
...
static int oplus_chg_track_init(struct oplus_chg_track *track_dev)
{
	...
	chip->ic_err_msg_load_trigger.type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_DEFAULT;
	chip->chg_into_liquid_load_trigger.type_reason = TRACK_NOTIFY_TYPE_CHARGING_BREAK;
	chip->chg_into_liquid_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_CHG_FEED_LIQUOR;
	chip->dual_chan_err_load_trigger.type_reason = TRACK_NOTIFY_TYPE_SOFTWARE_ABNORMAL;
	chip->dual_chan_err_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_DUAL_CHAN_ABNORMAL;
	...
	memset(&(chip->track_status.wired_break_crux_info), 0, sizeof(chip->track_status.wired_break_crux_info));
	memset(&(chip->track_status.wls_break_crux_info), 0, sizeof(chip->track_status.wls_break_crux_info));
	...
	INIT_DELAYED_WORK(&chip->ic_err_msg_trigger_work, oplus_chg_track_ic_err_msg_trigger_work);
	INIT_DELAYED_WORK(&chip->chg_into_liquid_trigger_work, oplus_chg_track_chg_into_liquid_trigger_work);
	...
	INIT_DELAYED_WORK(&chip->dual_chan_err_load_trigger_work, oplus_chg_track_dual_chan_err_load_trigger_work);
...
int oplus_chg_track_handle_wired_type_info(struct oplus_monitor *monitor, int type)
{
	...
	if (type == TRACK_CHG_GET_THTS_TIME_TYPE) {
		...
		oplus_chg_track_get_wired_type_info(track_status->pre_wired_type, track_status);
	}
...
static int oplus_chg_track_handle_batt_full_reason(struct oplus_monitor *monitor,
						   struct oplus_chg_track_status *track_status)
{
	...
	if (monitor->notify_flag == NOTIFY_BAT_FULL || monitor->notify_flag == NOTIFY_BAT_FULL_PRE_LOW_TEMP ||
	    monitor->notify_flag == NOTIFY_BAT_FULL_PRE_HIGH_TEMP || monitor->notify_flag == NOTIFY_BAT_NOT_CONNECT ||
...
static int oplus_chg_track_check_chg_abnormal(struct oplus_monitor *monitor,
					      struct oplus_chg_track_status *track_status)
{
	int notify_code;
	...
	chg_debug("track_notify_code:0x%x, chager_notify_code:0x%x, abnormal_reason[%s]\n", notify_code,
		  monitor->notify_code, track_status->chg_abnormal_reason);

	return 0;
}

static int oplus_chg_track_cal_chg_common_mesg(struct oplus_monitor *monitor,
					       struct oplus_chg_track_status *track_status)
{
	bool mmi_chg = false;
...
static int oplus_chg_track_cal_cool_down_stats(struct oplus_monitor *monitor,
					       struct oplus_chg_track_status *track_status)
{
	int cool_down_max;
	int curr_time;
	...
	if (monitor->batt_status == POWER_SUPPLY_STATUS_CHARGING && monitor->cool_down)
	...
	if (track_status->cool_down_status != monitor->cool_down) {
		cool_down_stats_table[monitor->cool_down].time += curr_time - track_status->cool_down_status_change_t;
	} else {
...
static int oplus_chg_track_cal_rechg_counts(struct oplus_monitor *monitor, struct oplus_chg_track_status *track_status)
...
static int oplus_chg_track_cal_no_charging_stats(struct oplus_monitor *monitor,
						 struct oplus_chg_track_status *track_status)
{
...
static int oplus_chg_track_cal_app_stats(struct oplus_monitor *monitor, struct oplus_chg_track_status *track_status)
{
	int curr_time;
	bool chg_start;
	bool chg_end;
...
static int oplus_chg_track_cal_led_on_stats(struct oplus_monitor *monitor, struct oplus_chg_track_status *track_status)
{
	int curr_time;
	bool chg_start;
	bool chg_end;
	...
	if (chg_end || track_status->chg_fast_full_time || monitor->batt_soc >= TRACK_LED_MONITOR_SOC_POINT)
		need_update = true;

	if (track_status->led_on && (!monitor->led_on || need_update)) {
		if (curr_time - track_status->led_change_t > TRACK_CYCLE_RECORDIING_TIME_2MIN) {
			track_status->ledon_rm += monitor->batt_rm - track_status->led_change_rm;
		...
		if (curr_time - track_status->led_change_t > TRACK_CYCLE_RECORDIING_TIME_2MIN) {
			track_status->ledoff_rm += monitor->batt_rm - track_status->led_change_rm;
	...
	if (!charger_exist || track_status->chg_report_full_time || track_status->chg_fast_full_time ||
	    monitor->batt_soc >= TRACK_LED_MONITOR_SOC_POINT)
		oplus_chg_track_cal_ledon_ledoff_average_speed(track_status);

	chg_info("ch_t:%d, ch_rm:%d, ledoff_rm:%d, ledoff_t:%d, ledon_rm:%d, ledon_t:%d\n", track_status->led_change_t,
		 track_status->led_change_rm, track_status->ledoff_rm, track_status->ledoff_time,
...
static void oplus_chg_track_record_break_charging_info(struct oplus_chg_track *track_chip,
						       struct oplus_monitor *monitor,
						       struct oplus_chg_track_power power_info,
						       const char *sub_crux_info)
{
	...
	if (power_info.power_type == TRACK_CHG_TYPE_WIRE) {
		...
		oplus_chg_track_record_general_info(monitor, track_status, &(track_chip->charging_break_trigger),
						    index);
		...
		oplus_chg_track_record_general_info(monitor, track_status, &(track_chip->wls_charging_break_trigger),
						    index);
	...
	if (wls_connect) {
		...
		if (delta_time_ms < track_chip->track_cfg.wls_chg_break_t_thd && track_status->wls_status_keep) {
			if (!break_recording) {
				...
				oplus_chg_track_record_break_charging_info(track_chip, monitor, power_info,
		...
		chg_info("pre_wls_connect[%d], wls_connect[%d], break_recording[%d], status_keep[%d], "
			 ...
			 pre_wls_connect, wls_connect, break_recording, track_status->wls_status_keep, delta_time_ms,
...
static bool oplus_chg_track_wired_fastchg_exit_code(struct oplus_chg_track *track_chip)
{
	bool ret = true;
	int code;
	struct oplus_chg_track_status *track_status;
...
int oplus_chg_track_check_wired_charging_break(int vbus_rising)
{
	...
	if (vbus_rising && (pre_vbus_rising != vbus_rising)) {
		...
		chg_info("detal_t:%llu, chg_attach_time = %llu, fastchg_break_code=0x%x\n",
...
int oplus_chg_water_inlet_detect(int reason, int plugin_count)
{
	...
	case TRACK_CMD_LIQUID_INTAKE:
		...
		if (plugin_timer_sum <= 5000) {
			chg_info(
				"plugin_timer_sum:%llu, timer0:%llu, timer1:%llu, timer2:%llu, timer3:%llu, timer4:%llu, \n",
				plugin_timer_sum, timer[0][0], timer[1][0], timer[2][0], timer[3][0], timer[4][0]);
			snprintf(
				chip->chg_into_liquid_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN,
				"plugin_timer_sum:%llu, timer0:%llu,%llu timer1:%llu,%llu timer2:%llu,%llu timer3:%llu,%llu timer4:%llu,%llu",
				plugin_timer_sum, timer[0][0], timer[0][1], timer[1][0], timer[1][1], timer[2][0],
				timer[2][1], timer[3][0], timer[3][1], timer[4][0], timer[4][1]);
...
int oplus_chg_track_cal_tbatt_status(struct oplus_monitor *monitor)
{
	...
	if (!track_status->tbatt_warm_once &&
	    ((monitor->temp_region == TEMP_REGION_WARM) || (monitor->temp_region == TEMP_REGION_HOT)))
		track_status->tbatt_warm_once = true;

	if (!track_status->tbatt_cold_once && (monitor->temp_region == TEMP_REGION_COLD))
...
int oplus_chg_track_record_ffc_soc(struct oplus_monitor *monitor, int main_soc, int sub_soc, bool start)
{
	struct oplus_chg_track *track_chip;
	struct oplus_chg_track_status *track_status;
...
int oplus_chg_track_record_dual_chan_end(struct oplus_monitor *monitor)
{
	...
	if (track_status->dual_chan_start_time) {
		track_status->dual_chan_time += dual_chan_end_time - track_status->dual_chan_start_time;
...
static int oplus_chg_track_cal_section_soc_inc_rm(struct oplus_monitor *monitor,
						  struct oplus_chg_track_status *track_status)
{
	...
	case TRACK_SOC_SECTION_LOW:
		...
		track_status->soc_low_sect_incr_rm = (monitor->batt_rm - rm_go_next_status) + pre_soc_low_sect_incr_rm;
		if (monitor->batt_soc > TRACK_REF_SOC_50) {
			pre_soc_low_sect_cont_time = track_status->soc_low_sect_cont_time;
			pre_soc_low_sect_incr_rm = track_status->soc_low_sect_incr_rm;
	...
	case TRACK_SOC_SECTION_MEDIUM:
		...
		track_status->soc_medium_sect_incr_rm =
			(monitor->batt_rm - rm_go_next_status) + pre_soc_medium_sect_incr_rm;
		if (monitor->batt_soc <= TRACK_REF_SOC_50) {
			pre_soc_medium_sect_cont_time = track_status->soc_medium_sect_cont_time;
			pre_soc_medium_sect_incr_rm = track_status->soc_medium_sect_incr_rm;
			time_go_next_status = curr_time;
	...
	case TRACK_SOC_SECTION_HIGH:
		...
		track_status->soc_high_sect_incr_rm =
			(monitor->batt_rm - rm_go_next_status) + pre_soc_high_sect_incr_rm;
	...
	chg_info("soc_sect_status:%d, time_go_next_status:%d, rm_go_next_status:%d\n", track_status->soc_sect_status,
		 time_go_next_status, rm_go_next_status);
...
static bool oplus_chg_track_burst_soc_sect_speed(struct oplus_chg_track_status *track_status,
...
static int oplus_chg_track_get_speed_slow_reason(struct oplus_chg_track_status *track_status)
{
	...
	if (track_status->tbatt_warm_once)
		...
		chip->slow_charging_trigger.flag_reason = TRACK_NOTIFY_FLAG_CHG_SLOW_NON_STANDARD_PA;
		 ...
		 (chip->track_cfg.wls_max_power - TRACK_POWER_10000MW > track_status->power_info.wls_info.power) &&
...
static bool oplus_chg_track_judge_speed_slow(struct oplus_monitor *monitor, struct oplus_chg_track *track_chip)
{
	struct oplus_chg_track_status *track_status;
	...
	if (track_status->power_info.power_type == TRACK_CHG_TYPE_UNKNOW) {
		...
		if (!strcmp(track_status->power_info.wls_info.adapter_type, "unknow")) {
			...
			track_status->chg_speed_is_slow =
				oplus_chg_track_burst_soc_sect_speed(track_status, track_status->wls_epp_speed_ref);
		} else if (!strcmp(track_status->power_info.wls_info.adapter_type, "bpp")) {
			if (!track_status->wls_bpp_speed_ref)
				return false;
			track_status->chg_speed_is_slow =
				oplus_chg_track_burst_soc_sect_speed(track_status, track_status->wls_bpp_speed_ref);
		} else {
			if (!track_status->wls_airvooc_speed_ref)
				return false;
			track_status->chg_speed_is_slow =
				oplus_chg_track_burst_soc_sect_speed(track_status, track_status->wls_airvooc_speed_ref);
		}
	} else {
		if (!track_status->wired_speed_ref)
			return false;
		track_status->chg_speed_is_slow =
			oplus_chg_track_burst_soc_sect_speed(track_status, track_status->wired_speed_ref);
	}
...
static int oplus_chg_track_cal_batt_full_time(struct oplus_monitor *monitor,
					      struct oplus_chg_track_status *track_status)
{
	int curr_time;
	...
	if (!track_status->chg_fast_full_time &&
	    ((monitor->ffc_status != FFC_DEFAULT) || track_status->wls_prop_status == TRACK_WLS_FASTCHG_FULL ||
		...
		chg_info("curr_time:%d, start_time:%d, fast_full_time:%d"
			 ...
			 curr_time, track_status->chg_start_time, track_status->chg_fast_full_time, monitor->batt_rm,
	...
	if (!track_status->chg_normal_full_time &&
		...
		if (track_status->ffc_end_time) {
			track_status->ffc_time = track_status->ffc_end_time - track_status->aging_ffc_start_time;
		...
		chg_info("curr_time:%d, start_time:%d, normal_full_time:%d"
			 "curr_rm:%d, chg_start_rm:%d, chg_average_speed:%d\n",
			 curr_time, track_status->chg_start_time, track_status->chg_normal_full_time, monitor->batt_rm,
			 track_status->chg_start_rm, track_status->chg_average_speed);
	}
...
static int oplus_chg_track_obtain_power_info(char *power_info, int len)
{
	...
	oplus_chg_track_get_charger_type(monitor, track_status, TRACK_CHG_GET_THTS_TIME_TYPE);
	...
	index += snprintf(&(power_info[index]), len - index, "$$soc@@%d", monitor->batt_soc);
	index += snprintf(&(power_info[index]), len - index, "$$smooth_soc@@%d", monitor->smooth_soc);
	index += snprintf(&(power_info[index]), len - index, "$$uisoc@@%d", monitor->ui_soc);
	index += snprintf(&(power_info[index]), len - index, "$$batt_temp@@%d", monitor->batt_temp);
	index += snprintf(&(power_info[index]), len - index, "$$shell_temp@@%d", monitor->shell_temp);
	index += snprintf(&(power_info[index]), len - index, "$$batt_vol@@%d", monitor->vbat_mv);
	index += snprintf(&(power_info[index]), len - index, "$$batt_curr@@%d", monitor->ibat_ma);
...
static int oplus_chg_track_get_err_comm_info(struct oplus_chg_track *track, char *buf, int buf_size)
{
	...
	oplus_chg_track_get_charger_type(monitor, track_status, TRACK_CHG_GET_THTS_TIME_TYPE);
	index = snprintf(buf, buf_size, "$$power_mode@@%s", track_status->power_info.power_mode);

	if (track_status->power_info.power_type == TRACK_CHG_TYPE_WIRE) {
		index += snprintf(buf + index, buf_size - index, "$$adapter_t@@%s",
				  track_status->power_info.wired_info.adapter_type);
		if (track_status->power_info.wired_info.adapter_id)
			index += snprintf(buf + index, buf_size - index, "$$adapter_id@@0x%x",
					  track_status->power_info.wired_info.adapter_id);
		index += snprintf(buf + index, buf_size - index, "$$power@@%d",
				  track_status->power_info.wired_info.power);
		index += snprintf(buf + index, buf_size - index, "$$vbus@@%d", monitor->wired_vbus_mv);
		if (track_status->wired_max_power <= 0)
		...
		index += snprintf(buf + index, buf_size - index, "$$adapter_t@@%s",
				  track_status->power_info.wls_info.adapter_type);
		if (strlen(track_status->power_info.wls_info.dock_type))
			index += snprintf(buf + index, buf_size - index, "$$dock_type@@%s",
					  track_status->power_info.wls_info.dock_type);
		index +=
			snprintf(buf + index, buf_size - index, "$$power@@%d", track_status->power_info.wls_info.power);
		index += snprintf(buf + index, buf_size - index, "$$vbus@@%d", monitor->wls_vout_mv);
		if (track_status->wls_max_power <= 0)
			index += snprintf(buf + index, buf_size - index, "$$match_power@@%d", -1);
		else
			index += snprintf(buf + index, buf_size - index, "$$match_power@@%d",
...
static int oplus_chg_track_upload_ic_err_info(struct oplus_chg_track *track)
{
	...
	rc = oplus_mms_get_item_data(track->monitor->err_topic, ERR_ITEM_IC, &data, false);
	if (rc < 0) {
	...
	rc = oplus_mms_analysis_ic_err_msg(msg_buf, TOPIC_MSG_STR_BUF, &name_index, &err_type, &sub_err_type,
					   &msg_index);
	if (rc < 0) {
	...
	index = snprintf(track_buf, OPLUS_CHG_TRACK_CURX_INFO_LEN, "$$device_id@@%s$$err_type@@%d",
			 msg_buf + name_index, sub_err_type);

	switch (err_type) {
	case OPLUS_IC_ERR_GPIO:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_GPIO_ABNORMAL;
		break;
	case OPLUS_IC_ERR_BATT_ID:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_BATT_ID_INFO;
		break;
	case OPLUS_IC_ERR_PLAT_PMIC:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_PLAT_PMIC_ABNORMAL;
		index += oplus_chg_track_get_err_comm_info(track, track_buf + index, OPLUS_CHG_TRIGGER_MSG_LEN - index);
		break;
	case OPLUS_IC_ERR_BUCK_BOOST:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_BOOST_BUCK_ERR;
		break;
	case OPLUS_IC_ERR_GAUGE:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_GAGUE_ABNORMAL;
		break;
	case OPLUS_IC_ERR_WLS_RX:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_WLS_ABNORMAL;
		index += oplus_chg_track_get_err_comm_info(track, track_buf + index, OPLUS_CHG_TRIGGER_MSG_LEN - index);
		break;
	case OPLUS_IC_ERR_CP:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_CP_ABNORMAL;
		break;
	case OPLUS_IC_ERR_CC_LOGIC:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_EXTERN_PMIC_ABNORMAL;
		break;
	case OPLUS_IC_ERR_PARALLEL_UNBALANCE:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_PARALLEL_UNBALANCE_ABNORMAL;
		break;
	case OPLUS_IC_ERR_MOS_ERROR:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_MOS_ERROR_ABNORMAL;
		break;
	case OPLUS_IC_ERR_UFCS:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_UFCS_IC_ABNORMAL;
		index += oplus_chg_track_obtain_power_info(track_buf + index, OPLUS_CHG_TRIGGER_MSG_LEN - index);
		break;
	case OPLUS_IC_ERR_GAN_MOS_ERROR:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_DCHG_ABNORMAL;
		break;
	case OPLUS_IC_ERR_NTC:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_NTC_ABNORMAL;
		break;
	case OPLUS_IC_ERR_I2C:
		track->ic_err_msg_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_I2C_ABNORMAL;
		break;
	...
	index += snprintf(track_buf + index, OPLUS_CHG_TRIGGER_MSG_LEN - index, "$$ic_msg@@%s", msg_buf + msg_index);
...
static int oplus_chg_track_upload_usbtemp_info(struct oplus_chg_track *track)
{
	...
	rc = oplus_mms_get_item_data(track->monitor->err_topic, ERR_ITEM_USBTEMP, &data, false);
	if (rc < 0) {
		chg_err("get msg data error, rc=%d\n", rc);
		return rc;
	}
	index = snprintf(track->usbtemp_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN, "%s", data.strval);
	oplus_chg_track_obtain_power_info(&track->usbtemp_load_trigger.crux_info[index],
					  OPLUS_CHG_TRACK_CURX_INFO_LEN - index);
...
static int oplus_chg_track_upload_vbatt_too_low_info(struct oplus_chg_track *track)
{
	union mms_msg_data data = { 0 };
	int rc;

	rc = oplus_mms_get_item_data(track->monitor->err_topic, ERR_ITEM_VBAT_TOO_LOW, &data, false);
	if (rc < 0) {
		chg_err("get msg data error, rc=%d\n", rc);
		return rc;
	}
	snprintf(track->vbatt_too_low_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN, "%s", data.strval);
...
static int oplus_chg_track_upload_vbatt_diff_over_info(struct oplus_chg_track *track)
{
	struct oplus_monitor *monitor = track->monitor;

	(void)snprintf(track->vbatt_diff_over_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN,
		       ...
		       monitor->batt_soc, monitor->smooth_soc, monitor->ui_soc, monitor->vbat_mv, monitor->vbat_min_mv,
		       monitor->vbat_mv - monitor->vbat_min_mv, monitor->batt_rm, monitor->batt_fcc, monitor->batt_cc,
		       (monitor->wired_online || monitor->wls_online),
		       (monitor->batt_status == POWER_SUPPLY_STATUS_FULL) ? "full" : "shutdown");
...
static int oplus_chg_track_upload_uisoc_keep_1_t_info(struct oplus_chg_track *chip)
{
	struct oplus_monitor *monitor = chip->monitor;
	int index = 0;
	...
	index += snprintf(&(chip->uisoc_keep_1_t_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  ...
			  chip->uisoc_1_start_vbatt_max, chip->uisoc_1_start_vbatt_min, monitor->vbat_mv,
			  monitor->vbat_min_mv, monitor->batt_soc, monitor->smooth_soc, monitor->ui_soc,
			  chip->uisoc_1_start_batt_rm, monitor->batt_rm, monitor->ibat_ma);
...
static int oplus_chg_track_upload_dual_chan_err_info(struct oplus_chg_track *track)
{
	union mms_msg_data data = { 0 };
	int rc;

	rc = oplus_mms_get_item_data(track->monitor->err_topic, ERR_ITEM_DUAL_CHAN, &data, false);
	if (rc < 0) {
		chg_err("get msg data error, rc=%d\n", rc);
		return rc;
	}
	snprintf(track->dual_chan_err_load_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN, "%s", data.strval);
...
static int oplus_chg_track_upload_wls_info(struct oplus_chg_track *chip)
{
	...
	index += snprintf(&(chip->wls_info_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  data.strval);
...
static int oplus_chg_track_upload_ufcs_info(struct oplus_chg_track *chip)
{
	...
	index += snprintf(&(chip->ufcs_info_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  data.strval);
...
static int oplus_chg_track_upload_deep_dischg_info(struct oplus_chg_track *chip)
{
	...
	index += snprintf(&(chip->deep_dischg_info_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "%s", data.strval);
							...
							struct oplus_monitor *monitor)
{
	...
	if (track_status->chg_average_speed == TRACK_PERIOD_CHG_AVERAGE_SPEED_INIT) {
		...
		chg_info("curr_rm:%d, chg_start_rm:%d, curr_time:%d, chg_start_time:%d,"
			 "chg_average_speed:%d\n",
			 monitor->batt_rm, track_status->chg_start_rm, curr_time, track_status->chg_start_time,
...
static int oplus_chg_track_cal_chg_end_soc(struct oplus_monitor *monitor, struct oplus_chg_track_status *track_status)
{
	if (!track_status || !monitor)
		return -EFAULT;
...
static int oplus_chg_track_status_reset_when_plugin(struct oplus_monitor *monitor,
						    struct oplus_chg_track_status *track_status)
{
	track_status->soc_sect_status = TRACK_SOC_SECTION_DEFAULT;
	track_status->chg_speed_is_slow = false;
...
static int oplus_chg_track_speed_check(struct oplus_monitor *monitor)
{
	...
	wls_break_work_delay_t = g_track_chip->track_cfg.wls_chg_break_t_thd + TRACK_TIME_1000MS_JIFF_THD;
	wired_break_work_delay_t = g_track_chip->track_cfg.fast_chg_break_t_thd + TRACK_TIME_500MS_JIFF_THD;
	...
	if (!monitor->wired_online && !monitor->wls_online) {
		if (track_record_charger_info) {
			...
			track_status->chg_plugout_utc_t =
				oplus_chg_track_get_current_time_s(&track_status->chg_plugout_rtc_t);
			...
			if (oplus_chg_track_is_no_charging(track_status)) {
				oplus_chg_track_record_charger_info(monitor, &g_track_chip->no_charging_trigger,
								    track_status);
				if (track_status->power_info.power_type == TRACK_CHG_TYPE_WIRELESS)
			...
			} else if (oplus_chg_track_judge_speed_slow(monitor, g_track_chip)) {
				oplus_chg_track_record_charger_info(monitor, &g_track_chip->slow_charging_trigger,
								    track_status);
				if (track_status->power_info.power_type == TRACK_CHG_TYPE_WIRELESS)
				...
				oplus_chg_track_record_charger_info(monitor, &g_track_chip->charger_info_trigger,
								    track_status);
				if (track_status->power_info.power_type == TRACK_CHG_TYPE_WIRELESS)
	...
	track_status->prop_status = monitor->batt_status;
	if (!track_record_charger_info && monitor->batt_status == POWER_SUPPLY_STATUS_FULL &&
...
static int oplus_chg_track_uisoc_soc_jump_check(struct oplus_monitor *monitor)
{
	...
	if (track_status->curr_soc == -EINVAL) {
		...
		if (monitor->soc_load >= 0 &&
		    abs(judge_curr_soc - monitor->soc_load) > OPLUS_CHG_TRACK_UI_SOC_LOAD_JUMP_THD) {
	...
	if (!track_status->uisoc_to_soc_jumped && !track_status->uisoc_load_jumped &&
		...
		ret = snprintf(g_track_chip->uisoc_to_soc_trigger.crux_info, OPLUS_CHG_TRACK_CURX_INFO_LEN,
			       ...
			       track_status->pre_time_utc, curr_time_utc, (curr_local_time - pre_local_time),
			       (monitor->wired_online || monitor->wls_online), track_status->curr_smooth_soc,
			       avg_current, monitor->ibat_ma, track_status->pre_fcc, track_status->pre_rm, curr_fcc,
			       curr_rm);
					     ...
					     int delta_time)
{
	...
	if (err_type == TRACK_GAGUE_GENERAL_INFO)
		index += snprintf(&(p_gauge_info->load_trigger->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$calib_t@@%s", data.strval);
	index += snprintf(&(p_gauge_info->load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, gauge_info.load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_track *chip = container_of(dwork, struct oplus_chg_track, sub_gauge_info.load_trigger_work);
...
static int oplus_chg_track_gauge_fifo_push(struct kfifo *kfifo, struct oplus_mms *gauge_topic)
{
	...
	if (ret == 0 && data.strval && strlen(data.strval)) {
		ret += snprintf(&gauge_data[ret], GAUGE_INFO_TRACK_FIFO_ONE_SIZE - ret,
				"time[%04d-%02d-%02d %02d:%02d:%02d]-%s", tm.tm_year, tm.tm_mon, tm.tm_mday, tm.tm_hour,
				tm.tm_min, tm.tm_sec, data.strval);
	} else {
		kfree(gauge_data);
		return -EINVAL;
	}
...
static void oplus_chg_track_err_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
{
	track->err_subs =
		oplus_mms_subscribe(track->monitor->err_topic, track, oplus_chg_track_err_subs_callback, "track");
...
static int oplus_chg_track_debugfs_init(struct oplus_chg_track *track_dev)
{
	...
	debugfs_create_bool("debug_close_3hours", 0644, debugfs_general, &(track_dev->track_status.debug_close_3hours));
	...
	debugfs_create_u32("debug_gauge_err_type", 0644, debugfs_gauge, &(track_dev->gauge_info.debug_err_type));
...
int oplus_chg_track_driver_init(struct oplus_monitor *monitor)
{
	...
	track_dev = devm_kzalloc(monitor->dev, sizeof(struct oplus_chg_track), GFP_KERNEL);
	if (track_dev == NULL) {
	...
	temp_track_status = (struct oplus_chg_track_status *)kmalloc(sizeof(struct oplus_chg_track_status), GFP_KERNEL);
