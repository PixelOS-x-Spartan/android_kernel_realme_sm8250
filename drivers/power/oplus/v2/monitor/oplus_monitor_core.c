...
__maybe_unused static bool is_wired_icl_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_wired_fcc_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_wired_charge_suspend_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_wired_charging_disable_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_wls_charge_suspend_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_wls_charging_disable_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_chg_disable_votable_available(struct oplus_monitor *chip)
{
...
__maybe_unused static bool is_vooc_curr_votable_available(struct oplus_monitor *chip)
{
...
{
	struct oplus_monitor *chip = container_of(work, struct oplus_monitor, charge_info_update_work);
	union mms_msg_data data = { 0 };
	...
	printk(KERN_INFO "OPLUS_CHG[oplus_charge_info]: "
	       ...
	       chip->batt_temp, chip->shell_temp, chip->vbat_mv, chip->vbat_min_mv, chip->ibat_ma, chip->batt_soc,
	       chip->ui_soc, chip->smooth_soc, chip->batt_rm, chip->batt_fcc, chip->batt_exist, chip->batt_err_code,
	       chip->fv_mv, chip->fcc_ma, chip->chg_disable, chip->chg_user_disable, chip->wired_online,
	       chip->wired_ibus_ma, chip->wired_vbus_mv, chip->wired_icl_ma, chip->wired_charge_type,
	       chip->wired_err_code, chip->wired_suspend, chip->wired_user_suspend, chip->cc_mode, chip->cc_detect,
	       chip->otg_enable, chip->wls_online, chip->wls_iout_ma, chip->wls_vout_mv, chip->wls_icl_ma,
	       chip->wls_charge_type, chip->wls_err_code, chip->wls_suspend, chip->wls_user_suspend, chip->vooc_online,
	       chip->vooc_started, chip->vooc_charging, chip->vooc_online_keep, chip->vooc_sid, chip->ufcs_online,
	       chip->ufcs_charging, chip->ufcs_oplus_adapter, chip->ufcs_adapter_id, chip->temp_region,
	       chip->ffc_status, chip->cool_down, chip->notify_code, chip->led_on, chip->deep_support);
...
static int comm_info_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size,
		 ...
		 "%d,%d,%d,%d,%d,%d,%d,%d,%d",
		 chip->batt_temp, chip->shell_temp, chip->vbat_mv, chip->vbat_min_mv, chip->ibat_ma, chip->batt_soc,
		 chip->ui_soc, chip->wired_online, chip->wired_charge_type, chip->notify_code, chip->wired_ibus_ma,
		 chip->wired_vbus_mv, chip->smooth_soc, chip->led_on, chip->fv_mv, chip->fcc_ma, chip->wired_icl_ma,
		 chip->otg_switch_status, chip->cool_down, chip->bcc_current, chip->normal_cool_down,
		 chip->chg_cycle_status, chip->mmi_chg, chip->usb_status, chip->cc_detect, chip->batt_full,
		 chip->rechging, chip->pd_svooc, chip->batt_status);
...
static void oplus_monitor_dual_chan_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_monitor_subscribe_dual_chan_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->dual_chan_subs =
		oplus_mms_subscribe(chip->dual_chan_topic, chip, oplus_monitor_dual_chan_subs_callback, "monitor");
...
static void oplus_monitor_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_TIMER:
		...
		oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_DEEP_SUPPORT, &data, false);
...
static void oplus_monitor_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_monitor *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->gauge_topic = topic;
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_monitor_gauge_subs_callback, "monitor");
	...
	chip->vbat_mv = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_TEMP, &data, true);
	chip->batt_temp = data.intval;
	...
	chip->ibat_ma = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_FCC, &data, true);
	chip->batt_fcc = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_RM, &data, true);
	chip->batt_rm = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_CC, &data, true);
	chip->batt_cc = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_SOH, &data, true);
	chip->batt_soh = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_BATT_EXIST, &data, true);
	chip->batt_exist = !!data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_ERR_CODE, &data, true);
	chip->batt_err_code = (unsigned int)data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_DEEP_SUPPORT, &data, true);
	chip->deep_support = data.intval;
}

static void oplus_monitor_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	int rc;
...
static void oplus_monitor_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_monitor *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->ufcs_subs = oplus_mms_subscribe(chip->ufcs_topic, chip, oplus_monitor_ufcs_subs_callback, "monitor");
...
{
	struct oplus_monitor *chip = container_of(work, struct oplus_monitor, wired_plugin_work);
...
static void oplus_mms_water_inlet_detect_work(struct work_struct *work)
{
	...
	struct oplus_monitor *chip = container_of(dwork, struct oplus_monitor, water_inlet_detect_work);
...
static void oplus_monitor_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_monitor_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_monitor *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_monitor_wired_subs_callback, "monitor");
	...
	chip->wired_online = !!data.intval;
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_ERR_CODE, &data, true);
	chip->wired_err_code = (unsigned int)data.intval;
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_CHG_TYPE, &data, true);
	chip->wired_charge_type = data.intval;
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_CC_MODE, &data, true);
	chip->cc_mode = data.intval;
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_TIME_ABNORMAL_ADAPTER, &data, true);
	chip->pd_svooc = !!data.intval;
}

static void oplus_monitor_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
{
	struct oplus_monitor *chip = container_of(work, struct oplus_monitor, ffc_step_change_work);
	union mms_msg_data data = { 0 };
	int rc;
...
{
	struct oplus_monitor *chip = container_of(work, struct oplus_monitor, ffc_end_work);

	oplus_chg_track_record_ffc_end_time(chip);
	oplus_monitor_record_ffc_soc(chip, false);
}

static void oplus_monitor_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		...
		case COMM_ITEM_BATT_STATUS:
			...
			if (chip->batt_status != pre_batt_status && chip->batt_status == POWER_SUPPLY_STATUS_FULL)
...
static void oplus_monitor_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_monitor *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->comm_topic = topic;
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_monitor_comm_subs_callback, "monitor");
	...
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_SMOOTH_SOC, &data, true);
	chip->smooth_soc = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_NOTIFY_CODE, &data, true);
	chip->notify_code = (unsigned int)data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_NOTIFY_FLAG, &data, true);
	chip->notify_flag = (unsigned int)data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_SHELL_TEMP, &data, true);
	chip->shell_temp = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_LED_ON, &data, true);
	chip->led_on = !!data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_RECHGING, &data, true);
	chip->rechging = !!data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_CHG_CYCLE_STATUS, &data, true);
	chip->chg_cycle_status = data.intval;
...
static void oplus_monitor_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_monitor *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_monitor_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_monitor *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_monitor_vooc_subs_callback, "monitor");
...
static int oplus_monitor_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_monitor), GFP_KERNEL);
	if (chip == NULL) {
	...
	chg_info("liquid_inlet_detection_switch = %d", chip->liquid_inlet_detection_switch);
	...
	chip->water_inlet_plugin_count = 0;
	INIT_WORK(&chip->charge_info_update_work, oplus_monitor_charge_info_update_work);
