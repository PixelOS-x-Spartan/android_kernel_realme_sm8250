...
int ufcs_check_refuse_msg(struct ufcs_class *class, struct ufcs_msg *msg, enum ufcs_msg_type type, u8 cmd)
{
...
re_recv:
	...
	case UFCS_EVENT_TIMEOUT:
		if (retry_count < HANDSHAKE_RETRY_MAX && !class->config.handshake_hard_retry) {
			retry_count++;
			ufcs_free_event(class, &event);
...
static int ufcs_send_dev_info(struct ufcs_class *class)
{
	...
	dev_info.info =
		UFCS_DEVICE_INFO_DATA(UFCS_VND_DEV_ID, class->config.ic_vendor_id, UFCS_HW_VERSION, UFCS_SW_VERSION);
...
static void ufcs_check_verify_response_msg(struct ufcs_class *class, struct ufcs_msg *msg)
{
	...
	rc = ufcs_get_encrypt_data(msg->data_msg.verify_response.random_data, class->verify_info.random_data,
				   class->verify_info.auth_data, encrypt_data);
	if (rc == 0) {
		rc = memcmp(msg->data_msg.verify_response.encrypted_data, encrypt_data,
...
static void ufcs_state_verify_request_handle(struct ufcs_class *class, struct ufcs_event *event)
{
	...
	memcpy(class->verify_info.random_data, verify_request.random_data, UFCS_VERIFY_RANDOM_DATA_SIZE);
	ufcs_free_event(class, &event);
...
static int ufcs_sm_task(void *arg)
{
	...
	retry:
		...
		if ((class->state.curr == PE_STATE_IDEL) && mutex_is_locked(&class->pe_lock))
			mutex_unlock(&class->pe_lock);
