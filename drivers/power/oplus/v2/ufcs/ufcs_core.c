...
static ssize_t pdo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t err_flag_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ufcs_dev *ufcs = dev_get_drvdata(dev);
	int rc;
...
static struct device_attribute *ufcs_ic_attributes[] = { &dev_attr_pdo, &dev_attr_err_flag, NULL };
