...
static struct ufcs_event *ufcs_alloc_no_data_event(struct ufcs_class *class, struct ufcs_msg *msg,
...
static bool ufcs_check_handshake(struct ufcs_class *class)
{
	...
	if (class->state.curr != PE_STATE_HANDSHAKE ||
	    (class->ufcs->handshake_state != UFCS_HS_SUCCESS && class->ufcs->handshake_state != UFCS_HS_FAIL))
		return false;
...
static int ufcs_check_error_info(struct ufcs_class *class, unsigned int dev_err_flag)
{
	...
	if (sender->status == MSG_WAIT_ACK) {
		...
		complete(&sender->ack);
		if ((dev_err_flag & BIT(UFCS_RECV_ERR_SENT_CMP)) && (dev_err_flag & BIT(UFCS_RECV_ERR_DATA_READY)))
			ufcs_err("ack interrupt read delayed! need to read msg!\n");
		else
			return 1;
	}
...
static void ufcs_msg_send_work(struct kthread_work *work)
{
	...
	if (sender->msg->head.type == UFCS_CTRL_MSG &&
	    (sender->msg->ctrl_msg.command == CTRL_MSG_ACK || sender->msg->ctrl_msg.command == CTRL_MSG_NCK))
		ack_nck_msg = true;
	else
		ack_nck_msg = false;
