...
void ufcs_pe_test_handle_work(struct work_struct *work)
{
	...
	ufcs_info("test_request: test:%u, vol_acc:%u, addr:%s, msg_type:%s, cmd_type:%s\n",
		  UFCS_TEST_REQUEST_ENABLE_TEST_MODE(request), UFCS_TEST_REQUEST_ENABLE_VOL_ACC_TEST_MODE(request),
		  ufcs_get_msg_addr_name(UFCS_TEST_REQUEST_DEV_ADDR(request)),
		  ufcs_get_msg_type_name(UFCS_TEST_REQUEST_MSG_TYPE(request)), cmd_name);
