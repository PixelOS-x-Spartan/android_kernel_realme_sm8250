...
void start_ack_receive_timer(struct ufcs_class *class)
{
	...
	if (class->config.msg_resend)
		hrtimer_start(&class->timer[ACK_RECEIVE_TIMER], ms_to_ktime(T_ACK_RECEIVE_MS), HRTIMER_MODE_REL);
	else
		hrtimer_start(&class->timer[ACK_RECEIVE_TIMER], ms_to_ktime(T_ACK_RECEIVE_NO_RESEND_MS),
			      HRTIMER_MODE_REL);
}
...
void start_sender_response_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[SENDER_RESPONSE_TIMER], ms_to_ktime(T_SENDER_RESPONSE_MS), HRTIMER_MODE_REL);
}
...
void start_power_supply_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[POWER_SUPPLY_TIMER], ms_to_ktime(T_POWER_SUPPLY_MS), HRTIMER_MODE_REL);
}
...
void start_cable_info_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[CABLE_INFO_TIMER], ms_to_ktime(T_CABLE_INFO_RESPONSE_MS), HRTIMER_MODE_REL);
}
...
void start_restart_trans_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[RESTART_TRANS_TIMER], ms_to_ktime(T_RESTART_TRANS_MS), HRTIMER_MODE_REL);
}
...
void start_cable_trans_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[CABLE_TRANS_TIMER], ms_to_ktime(T_CABLE_TRANS_MS), HRTIMER_MODE_REL);
}
...
void start_msg_trans_delay_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[MSG_TRANS_DELAY_TIMER], ms_to_ktime(T_MSG_TRANS_DELAY_MS), HRTIMER_MODE_REL);
}
...
void start_handshake_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[HANDSHAKE_TIMER], ms_to_ktime(T_HANDSHAKE_TIMEOUT_MS), HRTIMER_MODE_REL);
}
...
void start_wait_msg_timer(struct ufcs_class *class)
{
	...
	hrtimer_start(&class->timer[WAIT_MSG_TIMER], ms_to_ktime(T_WAIT_MSG_TIMEOUT_MS), HRTIMER_MODE_REL);
}
