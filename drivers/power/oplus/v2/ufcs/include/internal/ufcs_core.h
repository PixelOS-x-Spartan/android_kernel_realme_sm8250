...
#define ufcs_err(fmt, ...)                                                                                             \
	({                                                                                                             \
		if (ufcs_log_level >= LOG_LEVEL_ERR)                                                                   \
			printk(KERN_ERR "[ERROR]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                   \
	})
#define ufcs_info(fmt, ...)                                                                                            \
	({                                                                                                             \
		if (ufcs_log_level >= LOG_LEVEL_ALL_ERR)                                                               \
			printk(KERN_ERR "[INFO]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                    \
		else if (ufcs_log_level >= LOG_LEVEL_INFO)                                                             \
			printk(KERN_INFO "[INFO]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                   \
	})
#define ufcs_debug(fmt, ...)                                                                                           \
	({                                                                                                             \
		if (ufcs_log_level >= LOG_LEVEL_ALL_ERR)                                                               \
			printk(KERN_ERR "[DEBUG]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                   \
		else if (ufcs_log_level >= LOG_LEVEL_ALL_INFO)                                                         \
			printk(KERN_INFO "[DEBUG]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                  \
		else if (ufcs_log_level >= LOG_LEVEL_DEBUG)                                                            \
			printk(KERN_NOTICE "[DEBUG]: UFCS" pr_fmt(fmt), ##__VA_ARGS__);                                \
	})
