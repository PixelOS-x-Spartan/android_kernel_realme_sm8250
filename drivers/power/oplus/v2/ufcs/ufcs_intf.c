...
static int __ufcs_pdo_set(struct ufcs_class *class, int index, int vol_mv, int curr_ma)
{
	...
	event = devm_kzalloc(&class->ufcs->dev, sizeof(struct ufcs_data_msg_request) + sizeof(struct ufcs_event),
			     GFP_KERNEL);
	if (event == NULL) {
...
int ufcs_config_watchdog(struct ufcs_class *class, u16 time_ms)
{
	...
	event = devm_kzalloc(&class->ufcs->dev,
			     sizeof(struct ufcs_event) + sizeof(struct ufcs_data_msg_config_watchdog), GFP_KERNEL);
	if (event == NULL) {
		ufcs_err("alloc event buf error\n");
		return -ENOMEM;
	}
...
int ufcs_verify_adapter(struct ufcs_class *class, u8 key_index, u8 *auth_data, u8 data_len)
{
	...
	event = devm_kzalloc(&class->ufcs->dev, sizeof(struct ufcs_data_msg_verify_request) + sizeof(struct ufcs_event),
			     GFP_KERNEL);
	if (event == NULL) {
		ufcs_err("alloc event buf error\n");
		return -ENOMEM;
