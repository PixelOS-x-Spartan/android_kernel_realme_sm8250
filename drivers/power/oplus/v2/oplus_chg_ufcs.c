...
static const struct current_level g_ufcs_current_table[] = {
	{ 1, 1500 },   { 2, 1500 },   { 3, 2000 },   { 4, 2500 },   { 5, 3000 },   { 6, 3500 },
	{ 7, 4000 },   { 8, 4500 },   { 9, 5000 },   { 10, 5500 },  { 11, 6000 },  { 12, 6300 },
	{ 13, 6500 },  { 14, 7000 },  { 15, 7300 },  { 16, 8000 },  { 17, 8500 },  { 18, 9000 },
	{ 19, 9500 },  { 20, 10000 }, { 21, 10500 }, { 22, 11000 }, { 23, 11500 }, { 24, 12000 },
	{ 25, 12500 }, { 26, 13000 }, { 27, 13500 }, { 28, 14000 }, { 29, 14500 }, { 30, 15000 },
};

static const int ufcs_cool_down_oplus_curve[] = { 15000, 1500,	1500,  2000,  2500,  3000,  3500,  4000,
						  4500,	 5000,	5500,  6000,  6300,  6500,  7000,  7300,
...
static const int ufcs_cp_cool_down_oplus_curve[] = { 20000, 2000,  2000,  2400,	 3000,	3400,  4000,
						     4400,  5000,  5400,  6000,	 6400,	7000,  7400,
						     8000,  9000,  10000, 11000, 12000, 12600, 13000,
...
__maybe_unused static bool is_wired_suspend_votable_available(struct oplus_ufcs *chip)
{
...
__maybe_unused static bool is_gauge_topic_available(struct oplus_ufcs *chip)
{
	if (!chip->gauge_topic)
...
__maybe_unused static bool is_err_topic_available(struct oplus_ufcs *chip)
{
	if (!chip->err_topic)
...
__maybe_unused static int ufcs_find_current_to_level(int val, const struct current_level *const table, int len)
{
	int i;
...
__maybe_unused static int32_t oplus_ufcs_get_curve_vbus(struct oplus_ufcs *chip)
{
...
__maybe_unused static int32_t oplus_ufcs_get_curve_ibus(struct oplus_ufcs *chip)
{
	struct puc_strategy_ret_data data;
	int rc;
...
static int oplus_ufcs_switch_to_normal(struct oplus_ufcs *chip)
{
	...
	rc = oplus_chg_ic_func(chip->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, DPDM_SWITCH_TO_AP);

	return rc;
}

__maybe_unused static int oplus_ufcs_switch_to_ufcs(struct oplus_ufcs *chip)
{
	int rc;
	...
	rc = oplus_chg_ic_func(chip->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, DPDM_SWITCH_TO_UFCS);
...
static int oplus_ufcs_set_online(struct oplus_ufcs *chip, bool online)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, UFCS_ITEM_ONLINE);
	if (msg == NULL) {
...
static int oplus_ufcs_set_charging(struct oplus_ufcs *chip, bool charging)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, UFCS_ITEM_CHARGING);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_ufcs_set_adapter_id(struct oplus_ufcs *chip, u16 id)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, UFCS_ITEM_ADAPTER_ID);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static int oplus_ufcs_set_oplus_adapter(struct oplus_ufcs *chip, bool oplus_adapter)
{
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, UFCS_ITEM_OPLUS_ADAPTER);
	if (msg == NULL) {
		chg_err("alloc msg error\n");
		return -ENOMEM;
	}
...
static void oplus_ufcs_push_err_info(struct oplus_ufcs *chip, enum ufcs_user_err_type type, int value)
{
	...
	if (!is_err_topic_available(chip))
		return;
	if ((type == UFCS_ERR_ANTHEN_ERR) && (chip->config.curr_max_ma <= UFCS_VERIFY_CURR_THR_MA))
		return;
	...
	index = snprintf(buf, PAGE_SIZE,
			 "$$err_reason@@%s$$value@@%d$$dev_info@@0x%llx"
...
__maybe_unused static int oplus_ufcs_handshake(struct oplus_ufcs *chip)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_pdo_set(struct oplus_ufcs *chip, int vol_mv, int curr_ma)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_hard_reset(struct oplus_ufcs *chip)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_exit_ufcs_mode(struct oplus_ufcs *chip)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_config_wd(struct oplus_ufcs *chip, u16 time_ms)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_get_dev_info(struct oplus_ufcs *chip, u64 *dev_info)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_get_err_info(struct oplus_ufcs *chip, u64 *err_info)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_get_src_info(struct oplus_ufcs *chip, u64 *src_info)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_get_cable_info(struct oplus_ufcs *chip, u64 *cable_info)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_get_pdo_info(struct oplus_ufcs *chip, u64 *pdo, int num)
{
	int rc;
...
static int oplus_ufcs_verify_adapter(struct oplus_ufcs *chip, u8 index, u8 *auth_data, u8 data_len)
{
	int rc;
	...
	rc = oplus_chg_ic_func(chip->ufcs_ic, OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER, index, auth_data, data_len);

	return rc;
}

__maybe_unused static int oplus_ufcs_get_power_change_info(struct oplus_ufcs *chip, u32 *pwr_change_info, int num)
{
	int rc;
	...
	rc = oplus_chg_ic_func(chip->ufcs_ic, OPLUS_IC_FUNC_UFCS_GET_POWER_CHANGE_INFO, pwr_change_info, num);

	return rc;
}

__maybe_unused static int oplus_chg_ufcs_get_emark_info(struct oplus_ufcs *chip, u64 *info)
{
	int rc;
...
__maybe_unused static int oplus_chg_ufcs_get_power_info_ext(struct oplus_ufcs *chip, u64 *info, int num)
{
	int rc;
...
__maybe_unused static bool oplus_chg_ufcs_is_test_mode(struct oplus_ufcs *chip)
{
	bool en;
	int rc;
...
__maybe_unused static bool oplus_chg_ufcs_is_vol_acc_test_mode(struct oplus_ufcs *chip)
{
	bool en;
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_enable(struct oplus_ufcs *chip, bool en)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_hw_init(struct oplus_ufcs *chip)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_set_work_mode(struct oplus_ufcs *chip, enum oplus_cp_work_mode mode)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_work_mode(struct oplus_ufcs *chip, enum oplus_cp_work_mode *mode)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_check_work_mode_support(struct oplus_ufcs *chip, enum oplus_cp_work_mode mode)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_set_iin(struct oplus_ufcs *chip, int iin)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_vin(struct oplus_ufcs *chip, int *vin)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_iin(struct oplus_ufcs *chip, int *iin)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_vout(struct oplus_ufcs *chip, int *vout)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_iout(struct oplus_ufcs *chip, int *iout)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_vac(struct oplus_ufcs *chip, int *vac)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_set_work_start(struct oplus_ufcs *chip, bool start)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_get_work_status(struct oplus_ufcs *chip, bool *start)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_adc_enable(struct oplus_ufcs *chip, bool en)
{
	int rc;
...
__maybe_unused static int oplus_ufcs_cp_reg_dump(struct oplus_ufcs *chip)
{
	int rc;
...
{
	if (chip->shell_temp < chip->limits.ufcs_low_temp || chip->shell_temp > chip->limits.ufcs_high_temp)
	...
	if (chip->ui_soc < chip->limits.ufcs_low_soc || chip->ui_soc > chip->limits.ufcs_high_soc)
...
static void oplus_ufcs_wait_auth_data_work(struct work_struct *work)
{
	...
	struct oplus_ufcs *chip = container_of(dwork, struct oplus_ufcs, wait_auth_data_work);
...
static int oplus_ufcs_deal_emark_info(struct oplus_ufcs *chip)
{
	...
	chg_info("emark: cable_type=%s, hw_ver=%llu, curr_max=%llu mA, vol_max=%llumV, \n",
		 ...
		 UFCS_OPLUS_VND_EMARK_HW_VER(chip->emark_info), UFCS_OPLUS_VND_EMARK_CURR_MAX(chip->emark_info),
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_ufcs *chip = container_of(dwork, struct oplus_ufcs, switch_check_work);
	...
	for (i = 0; i < chip->pdo_num; i++) {
		...
		if (i > 0) {
			if (UFCS_OUTPUT_MODE_VOL_MIN(chip->pdo[i]) != UFCS_OUTPUT_MODE_VOL_MAX(chip->pdo[i - 1])) {
...
*/
	...
	pdo_vol = UFCS_OUTPUT_MODE_VOL_MIN(chip->pdo[0]) > UFCS_START_DEF_VOL_MV ?
			  UFCS_OUTPUT_MODE_VOL_MIN(chip->pdo[0]) :
			  UFCS_START_DEF_VOL_MV;
...
static int oplus_ufcs_set_current_warm_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
...
static int oplus_ufcs_set_current_temp_normal_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
	int ret = chip->limits.ufcs_strategy_normal_current;
...
static int oplus_ufcs_set_current_temp_low_normal_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
	int ret = chip->limits.ufcs_strategy_normal_current;
...
static int oplus_ufcs_set_current_temp_little_cool_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_ufcs_set_current_temp_cool_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_ufcs_set_current_temp_little_cold_range(struct oplus_ufcs *chip, int vbat_temp_cur)
{
	int ret = chip->limits.ufcs_strategy_normal_current;
...
static void oplus_ufcs_check_low_curr_temp_status(struct oplus_ufcs *chip)
{
	...
	} else if (chip->ufcs_low_curr_full_temp_status != UFCS_LOW_CURR_FULL_CURVE_TEMP_LITTLE_COOL) {
		t_cnts++;
		if (t_cnts >= UFCS_TEMP_OVER_COUNTS) {
...
#define UFCS_FULL_COUNTS_HW 3
	...
	if ((batt_temp >= chip->limits.ufcs_cool_temp) && (batt_temp < chip->limits.ufcs_batt_over_high_temp)) {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_ufcs *chip = container_of(dwork, struct oplus_ufcs, monitor_work);
	struct puc_strategy_ret_data data;
	int rc;
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, wired_online_work);
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, force_exit_work);
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, soft_exit_work);
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, gauge_update_work);
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, err_flag_push_work);
	struct mms_msg *msg;
	...
	ignored_flag = BIT(UFCS_HW_ERR_HARD_RESET) | BIT(UFCS_RECV_ERR_SENT_CMP) | BIT(UFCS_RECV_ERR_DATA_READY) |
...
{
	struct oplus_ufcs *chip = container_of(work, struct oplus_ufcs, fifo_overflow_push_work);
	struct mms_msg *msg;
	int rc;
	char *buf;
	int i;
	...
	index = snprintf(buf, PAGE_SIZE,
			 "$$reason@@fifo_overflow$$err_flag@@0x%x"
...
static void oplus_ufcs_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_ufcs_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(topic, chip, oplus_ufcs_comm_subs_callback, "ufcs");
...
static void oplus_ufcs_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_ufcs *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_ufcs_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_ufcs *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_ufcs_wired_subs_callback, "ufcs");
...
static void oplus_ufcs_cpa_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_ufcs *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_ufcs_subscribe_cpa_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_ufcs *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->cpa_topic = topic;
	chip->cpa_subs = oplus_mms_subscribe(chip->cpa_topic, chip, oplus_ufcs_cpa_subs_callback, "ufcs");
...
static void oplus_ufcs_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_ufcs *chip = subs->priv_data;
...
static void oplus_ufcs_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_ufcs *chip = prv_data;

	chip->gauge_topic = topic;
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_ufcs_gauge_subs_callback, "ufcs");
...
static int oplus_ufcs_update_online(struct oplus_mms *mms, union mms_msg_data *data)
{
...
static int oplus_ufcs_update_charging(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_adapter_id(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_oplus_adapter(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_bcc_max_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_bcc_min_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_bcc_exit_curr(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_update_bcc_temp_range(struct oplus_mms *mms, union mms_msg_data *data)
{
	struct oplus_ufcs *chip;
...
static int oplus_ufcs_topic_init(struct oplus_ufcs *chip)
{
	...
	chip->ufcs_topic = devm_oplus_mms_register(chip->dev, &oplus_ufcs_desc, &mms_cfg);
...
static int oplus_ufcs_curr_vote_callback(struct votable *votable, void *data, int curr_ma, const char *client,
					 bool step)
{
...
static int oplus_ufcs_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client,
					    bool step)
{
	struct oplus_ufcs *chip = data;
...
static int oplus_ufcs_not_allow_vote_callback(struct votable *votable, void *data, int not_allow, const char *client,
					      bool step)
{
	struct oplus_ufcs *chip = data;
	union mms_msg_data mms_data = { 0 };
...
static int oplus_ufcs_boot_vote_callback(struct votable *votable, void *data, int boot, const char *client, bool step)
{
	struct oplus_ufcs *chip = data;
...
static int oplus_ufcs_parse_charge_strategy(struct oplus_ufcs *chip)
{
	...
	chg_info("ufcs_charge_strategy_temp num = %d, [%d, %d, %d, %d, %d, "
		 "%d, %d]\n",
		 chip->limits.ufcs_strategy_temp_num, rang_temp_tmp[0], rang_temp_tmp[1], rang_temp_tmp[2],
	...
	chg_info("ufcs_charge_strategy_soc num = %d, [%d, %d, %d, %d, %d, %d, "
		 "%d]\n",
		 chip->limits.ufcs_strategy_soc_num, soc_tmp[0], soc_tmp[1], soc_tmp[2], soc_tmp[3], soc_tmp[4],
		 soc_tmp[5], soc_tmp[6]);
...
static int oplus_ufcs_parse_dt(struct oplus_ufcs *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,target_vbus_mv", &config->target_vbus_mv);
	if (rc < 0) {
		chg_err("get oplus,target_vbus_mv property error, rc=%d\n", rc);
		config->target_vbus_mv = 11000;
	}

	rc = of_property_read_u32(node, "oplus,curr_max_ma", &config->curr_max_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus,curr_table_type", &chip->curr_table_type);
	if (rc < 0) {
...
static int oplus_ufcs_vote_init(struct oplus_ufcs *chip)
{
	...
	chip->ufcs_boot_votable = create_votable("UFCS_BOOT", VOTE_SET_ANY, oplus_ufcs_boot_vote_callback, chip);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_ufcs *chip = container_of(dwork, struct oplus_ufcs, imp_uint_init_work);
...
{
	struct oplus_ufcs *chip = container_of(filp->private_data, struct oplus_ufcs, misc_dev);
...
static ssize_t oplus_ufcs_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
...
static long oplus_ufcs_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct oplus_ufcs *chip = filp->private_data;
...
static ssize_t oplus_ufcs_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	return count;
}
