...
static int oplus_chg_va_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < va->asic_num; i++) {
		if (va->asic_list[i].initialized)
			continue;
		va->asic_list[i].ic_dev = of_get_oplus_chg_ic(node, "oplus,asic_ic", i);
...
static int oplus_chg_va_user_fw_upgrade(struct oplus_chg_ic_dev *ic_dev, const u8 *fw_buf, u32 fw_size)
{
	struct oplus_virtual_asic_ic *va;
	int rc = 0;
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE, fw_buf, fw_size);
	if (rc < 0)
...
static int oplus_chg_va_fw_check_then_recover(struct oplus_chg_ic_dev *ic_dev)
{
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER);
	if (rc != FW_CHECK_MODE)
...
static int oplus_chg_va_fw_check_then_recover_fix(struct oplus_chg_ic_dev *ic_dev)
{
	struct oplus_virtual_asic_ic *va;
	int rc = 0;
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER_FIX);
	if (rc < 0)
...
static int oplus_chg_va_get_fw_version(struct oplus_chg_ic_dev *ic_dev, u32 *version)
{
	struct oplus_virtual_asic_ic *va;
	int rc = 0;
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_GET_FW_VERSION, version);
	if (rc < 0)
...
static bool oplus_chg_va_is_boot_by_gpio(struct oplus_virtual_asic_ic *va)
{
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_BOOT_BY_GPIO, &boot_by_gpio);
	if (rc < 0) {
...
static bool oplus_chg_va_is_upgrading(struct oplus_virtual_asic_ic *va)
{
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_UPGRADING, &upgrading);
	if (rc < 0) {
...
{
	struct oplus_virtual_asic_ic *va = container_of(work, struct oplus_virtual_asic_ic, data_handler_work);
...
static int oplus_chg_va_eint_register(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (node) {
		...
		rc = request_irq(va->gpio.data_irq, (irq_handler_t)irq_rx_handler, IRQF_TRIGGER_RISING,
...
#else
	oplus_chg_va_set_data_active(va->ic_dev);
	rc = request_irq(va->gpio.data_irq, irq_rx_handler, IRQF_TRIGGER_RISING, "asic_data", va);
	if (rc < 0) {
...
static int oplus_chg_va_get_reset_gpio_val(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct oplus_virtual_asic_ic *va;
...
static int oplus_chg_va_get_clock_gpio_val(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct oplus_virtual_asic_ic *va;
...
static int oplus_chg_va_get_data_gpio_val(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct oplus_virtual_asic_ic *va;
...
static int oplus_chg_va_reply_data(struct oplus_chg_ic_dev *ic_dev, int data, int data_width, int curr_ma)
{
	struct oplus_virtual_asic_ic *va;
	int i;
	int rc = 0;
	...
	for (i = 0; i < data_width; i++) {
		gpio_set_value(va->gpio.data_gpio, (data >> (data_width - i - 1)) & 0x01);
...
static int oplus_chg_va_switch_to_normal(struct oplus_virtual_asic_ic *va)
{
	...
	if (va->use_dpdm_switch_ic) {
		rc = oplus_chg_ic_func(va->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, DPDM_SWITCH_TO_AP);
		if (rc < 0) {
...
static int oplus_chg_va_switch_to_vooc(struct oplus_virtual_asic_ic *va)
{
	...
	if (va->use_dpdm_switch_ic) {
		rc = oplus_chg_ic_func(va->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, DPDM_SWITCH_TO_VOOC);
		if (rc < 0) {
			chg_err("set dpdm switch mode to vooc error, rc=%d\n", rc);
			return rc;
		}
...
static int oplus_chg_va_set_switch_mode(struct oplus_chg_ic_dev *ic_dev, int mode)
{
	struct oplus_virtual_asic_ic *va;
...
static int oplus_chg_va_get_switch_mode(struct oplus_chg_ic_dev *ic_dev, int *mode)
{
	struct oplus_virtual_asic_ic *va;
	...
	if (va->use_dpdm_switch_ic) {
		rc = oplus_chg_ic_func(va->dpdm_switch, OPLUS_IC_FUNC_GET_DPDM_SWITCH_MODE, &dpdm_mode);
		if (rc < 0) {
...
static int oplus_chg_va_get_fw_status(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
	struct oplus_virtual_asic_ic *va;
	int rc = 0;
...
static int oplus_chg_va_get_asic(struct oplus_chg_ic_dev *ic_dev, struct oplus_chg_ic_dev **asic)
{
	struct oplus_virtual_asic_ic *va;
...
static void *oplus_chg_va_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_va_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_va_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_chg_va_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_chg_va_smt_test);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_UPGRADE, oplus_chg_va_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE, oplus_chg_va_user_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER,
					       oplus_chg_va_fw_check_then_recover);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER_FIX:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER_FIX,
					       oplus_chg_va_fw_check_then_recover_fix);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_FW_VERSION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_FW_VERSION, oplus_chg_va_get_fw_version);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_DATA_ACTIVE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_DATA_ACTIVE, oplus_chg_va_set_data_active);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_DATA_SLEEP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_DATA_SLEEP, oplus_chg_va_set_data_sleep);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_CLOCK_ACTIVE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_CLOCK_ACTIVE, oplus_chg_va_set_clock_active);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_CLOCK_SLEEP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_CLOCK_SLEEP, oplus_chg_va_set_clock_sleep);
		break;
	case OPLUS_IC_FUNC_VOOC_RESET_SLEEP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_RESET_SLEEP, oplus_chg_va_reset_sleep);
		break;
	case OPLUS_IC_FUNC_VOOC_RESET_ACTIVE_FORCE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_RESET_ACTIVE_FORCE, oplus_chg_va_reset_active_force);
		break;
	case OPLUS_IC_FUNC_VOOC_RESET_ACTIVE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_RESET_ACTIVE, oplus_chg_va_reset_active);
		break;
	case OPLUS_IC_FUNC_VOOC_EINT_REGISTER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_EINT_REGISTER, oplus_chg_va_eint_register);
		break;
	case OPLUS_IC_FUNC_VOOC_EINT_UNREGISTER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_EINT_UNREGISTER, oplus_chg_va_eint_unregister);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_RESET_GPIO_VAL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_RESET_GPIO_VAL, oplus_chg_va_get_reset_gpio_val);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_CLOCK_GPIO_VAL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_CLOCK_GPIO_VAL, oplus_chg_va_get_clock_gpio_val);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_DATA_GPIO_VAL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_DATA_GPIO_VAL, oplus_chg_va_get_data_gpio_val);
		break;
	case OPLUS_IC_FUNC_VOOC_READ_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_READ_DATA, oplus_chg_va_get_data);
		break;
	case OPLUS_IC_FUNC_VOOC_REPLY_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_REPLY_DATA, oplus_chg_va_reply_data);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE, oplus_chg_va_set_switch_mode);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_SWITCH_MODE, oplus_chg_va_get_switch_mode);
		break;
	case OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS, oplus_chg_va_get_fw_status);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_IC_DEV:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_IC_DEV, oplus_chg_va_get_asic);
		break;

	case OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR, oplus_chg_va_get_curve_current);
		break;
...
static int oplus_va_child_init(struct oplus_virtual_asic_ic *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "oplus,asic_ic", sizeof(u32));
	if (rc < 0) {
		chg_err("can't get asic number, rc=%d\n", rc);
		return rc;
	}
	chip->asic_num = rc;
	chip->asic_list = devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_asic_child) * chip->asic_num, GFP_KERNEL);
...
static int oplus_va_gpio_init(struct oplus_virtual_asic_ic *chip)
{
	...
	if (!chip->use_audio_switch) {
		asic_gpio->switch_ctrl_gpio = of_get_named_gpio(node, "oplus,switch_ctrl-gpio", 0);
		...
		asic_gpio->gpio_switch_ctrl_ap = pinctrl_lookup_state(asic_gpio->pinctrl, "switch_ctrl_ap");
		...
		asic_gpio->gpio_switch_ctrl_asic = pinctrl_lookup_state(asic_gpio->pinctrl, "switch_ctrl_asic");
		if (IS_ERR_OR_NULL(asic_gpio->gpio_switch_ctrl_asic)) {
			chg_err("get switch_ctrl_asic fail\n");
			rc = -EINVAL;
			goto free_switch_ctrl_gpio;
		}
		rc = pinctrl_select_state(asic_gpio->pinctrl, asic_gpio->gpio_switch_ctrl_ap);
		if (rc < 0) {
	...
	asic_gpio->asic_switch1_gpio = of_get_named_gpio(node, "oplus,asic_switch1-gpio", 0);
...
skip_ctrl_gpio_init:
	asic_gpio->reset_gpio = of_get_named_gpio(node, "oplus,asic_reset-gpio", 0);
	...
	asic_gpio->asic_reset_active = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_reset_active");
	if (IS_ERR_OR_NULL(asic_gpio->asic_reset_active)) {
		chg_err("get asic_reset_active fail\n");
		rc = -EINVAL;
		goto free_reset_gpio;
	}
	asic_gpio->asic_reset_sleep = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_reset_sleep");
	if (IS_ERR_OR_NULL(asic_gpio->asic_reset_sleep)) {
		chg_err("get asic_reset_sleep fail\n");
		rc = -EINVAL;
		goto free_reset_gpio;
	}
	rc = pinctrl_select_state(asic_gpio->pinctrl, asic_gpio->asic_reset_sleep);
	if (rc < 0) {
		chg_err("set gpio asic_reset_sleep error, rc=%d\n", rc);
		goto free_reset_gpio;
	}

	asic_gpio->clock_gpio = of_get_named_gpio(node, "oplus,asic_clock-gpio", 0);
	...
	asic_gpio->asic_clock_active = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_clock_active");
	if (IS_ERR_OR_NULL(asic_gpio->asic_clock_active)) {
		chg_err("get asic_clock_active fail\n");
		rc = -EINVAL;
		goto free_clock_gpio;
	}
	asic_gpio->asic_clock_sleep = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_clock_sleep");
	if (IS_ERR_OR_NULL(asic_gpio->asic_clock_sleep)) {
		chg_err("get asic_clock_sleep fail\n");
		rc = -EINVAL;
		goto free_clock_gpio;
	}
	rc = pinctrl_select_state(asic_gpio->pinctrl, asic_gpio->asic_clock_sleep);
	if (rc < 0) {
		chg_err("set gpio asic_clock_sleep error, rc=%d\n", rc);
		goto free_clock_gpio;
	}

	asic_gpio->data_gpio = of_get_named_gpio(node, "oplus,asic_data-gpio", 0);
	...
	asic_gpio->asic_data_active = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_data_active");
	if (IS_ERR_OR_NULL(asic_gpio->asic_data_active)) {
		chg_err("get asic_data_active fail\n");
		rc = -EINVAL;
		goto free_data_gpio;
	}
	asic_gpio->asic_data_sleep = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_data_sleep");
	if (IS_ERR_OR_NULL(asic_gpio->asic_data_sleep)) {
		chg_err("get asic_data_sleep fail\n");
		rc = -EINVAL;
		goto free_data_gpio;
	}
	rc = pinctrl_select_state(asic_gpio->pinctrl, asic_gpio->asic_data_sleep);
	if (rc < 0) {
...
static int oplus_va_asic_id_check(struct oplus_virtual_asic_ic *chip)
{
	...
	asic_gpio->asic_id_gpio = of_get_named_gpio(node, "oplus,asic_id-gpio", 0);
	...
	asic_gpio->asic_id_active = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_id_active");
	if (IS_ERR_OR_NULL(asic_gpio->asic_id_active)) {
		chg_err("get asic_id_active fail\n");
		rc = -EINVAL;
		goto free_gpio;
	}
	asic_gpio->asic_id_sleep = pinctrl_lookup_state(asic_gpio->pinctrl, "asic_id_sleep");
	...
	rc = pinctrl_select_state(asic_gpio->pinctrl, asic_gpio->asic_id_active);
	if (rc < 0) {
...
static int oplus_va_virq_register(struct oplus_virtual_asic_ic *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->asic, OPLUS_IC_VIRQ_ERR, oplus_va_err_handler, chip);
	if (rc < 0)
...
static int oplus_virtual_asic_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_asic_ic), GFP_KERNEL);
	if (chip == NULL) {
