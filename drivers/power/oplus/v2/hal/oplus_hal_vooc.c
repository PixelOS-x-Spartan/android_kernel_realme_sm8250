...
static void oplus_vooc_set_switch_mode(struct oplus_chg_ic_dev *vooc_ic, enum oplus_chg_vooc_switch_mode switch_mode)
{
	int rc;
...
int oplus_vooc_read_voocphy_bcc_max_curr(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR, &bcc_data);
	if (rc < 0)
		return rc;
...
int oplus_vooc_read_voocphy_bcc_min_curr(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR, &bcc_data);
	if (rc < 0)
		return rc;
...
int oplus_vooc_read_voocphy_bcc_exit_curr(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_EXIT_CURR, &bcc_data);
	if (rc < 0)
		return rc;
...
int oplus_vooc_read_voocphy_bcc_fastchg_ing(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING, &bcc_data);
	if (rc < 0)
		return rc;
...
int oplus_vooc_read_voocphy_bcc_temp_range(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_TEMP_RANGE, &bcc_data);
	if (rc < 0)
		return rc;
...
void oplus_vooc_reply_data(struct oplus_chg_ic_dev *vooc_ic, int ret_info, int device_type, int data_width, int curr_ma)
{
	int data;
	int rc;
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOC_REPLY_DATA, data, data_width, curr_ma);
	if (rc < 0)
		chg_err("reply data error, rc=%d\n", rc);
}

void oplus_vooc_reply_data_no_type(struct oplus_chg_ic_dev *vooc_ic, int ret_info, int data_width, int curr_ma)
{
	int rc;
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOC_REPLY_DATA, ret_info, data_width, curr_ma);
	if (rc < 0)
		chg_err("reply data error, rc=%d\n", rc);
}
...
enum oplus_chg_vooc_switch_mode oplus_chg_vooc_get_switch_mode(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOC_GET_SWITCH_MODE, &mode);
	if (rc < 0) {
...
__maybe_unused static void set_vooc_chargerid_switch_val(struct oplus_chg_ic_dev *vooc_ic, int value)
{
	int level = 0;
...
__maybe_unused static int oplus_vooc_get_reset_gpio_val(struct oplus_chg_ic_dev *vooc_ic)
{
	int val;
	int rc;
...
__maybe_unused static int oplus_vooc_get_ap_clk_gpio_val(struct oplus_chg_ic_dev *vooc_ic)
{
	int val;
	int rc;
...
__maybe_unused static int oplus_vooc_get_switch_gpio_val(struct oplus_chg_ic_dev *vooc_ic)
{
	int val;
...
__maybe_unused static int oplus_vooc_get_fw_verion_from_ic(struct oplus_chg_ic_dev *vooc_ic)
{
	u32 version;
	int rc;
...
int set_chg_auto_mode(struct oplus_chg_ic_dev *vooc_ic, bool enable)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOCPHY_SET_CHG_AUTO_MODE, enable);
	if (rc == -ENOTSUPP)
		return 0;
	else if (rc < 0)
