...
static int oplus_vc_base_virq_register(struct oplus_virtual_cp_ic *chip, int index)
{
	...
	rc = oplus_chg_ic_virq_register(chip->child_list[index].ic_dev, OPLUS_IC_VIRQ_ONLINE, oplus_vc_online_handler,
					&chip->child_list[index]);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ONLINE error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->child_list[index].ic_dev, OPLUS_IC_VIRQ_OFFLINE, oplus_vc_offline_handler,
					&chip->child_list[index]);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ONLINE error, rc=%d", rc);
	rc = oplus_chg_ic_virq_register(chip->child_list[index].ic_dev, OPLUS_IC_VIRQ_ERR, oplus_vc_err_handler, chip);
	if (rc < 0 && rc != -ENOTSUPP)
...
{
	struct oplus_virtual_cp_child *child = container_of(work, struct oplus_virtual_cp_child, online_work);
...
{
	struct oplus_virtual_cp_child *child = container_of(work, struct oplus_virtual_cp_child, offline_work);
	struct oplus_virtual_cp_ic *chip;
...
static int oplus_vc_child_init(struct oplus_virtual_cp_ic *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,cp_ic_connect", &chip->connect_type);
	if (rc < 0) {
		chg_err("can't get cp ic connect type, rc=%d\n", rc);
		return rc;
	}
	chip->main_cp = 0;
	rc = of_property_count_elems_of_size(node, "oplus,cp_ic", sizeof(u32));
	if (rc < 0) {
		chg_err("can't get cp ic number, rc=%d\n", rc);
		return rc;
	}
	chip->child_num = rc;
	chip->child_list = devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_cp_child) * chip->child_num, GFP_KERNEL);
	...
	for (i = 0; i < chip->child_num; i++) {
		rc = of_property_read_u32_index(node, "oplus,input_curr_max_ma", i, &chip->child_list[i].max_curr_ma);
		if (rc < 0) {
...
static int oplus_chg_vc_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	...
	for (i = 0; i < vc->child_num; i++) {
		if (index >= len)
			return len;
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_SMT_TEST, buf + index, len - index);
		if (rc < 0) {
			if (rc != -ENOTSUPP) {
				chg_err("child ic[%d] smt test error, rc=%d\n", i, rc);
				rc = snprintf(buf + index, len - index, "[%s]-[%s]:%d\n",
					      vc->child_list[i].ic_dev->manu_name, "FUNC_ERR", rc);
			} else {
				rc = 0;
			}
		} else {
...
static int oplus_chg_vc_check_work_mode_support(struct oplus_chg_ic_dev *ic_dev, enum oplus_cp_work_mode mode)
{
	...
	for (i = 0; i < vc->child_num; i++) {
		if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_CHECK_WORK_MODE_SUPPORT,
					       mode);
			if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_set_iin(struct oplus_chg_ic_dev *ic_dev, int iin)
{
	...
	for (i = 0; i < vc->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_WORK_STATUS, &work_start);
		if (rc < 0) {
	...
	for (i = 0; i < vc->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_SET_WORK_START, true);
		if (rc < 0) {
...
static int oplus_chg_vc_get_vin(struct oplus_chg_ic_dev *ic_dev, int *vin)
{
	...
	for (i = 0; i < vc->child_num; i++) {
		if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_VIN, vin);
			if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_get_iin(struct oplus_chg_ic_dev *ic_dev, int *iin)
{
	...
	*iin = 0;
	for (i = 0; i < vc->child_num; i++) {
		if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_IIN, &curr);
			if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_get_vout(struct oplus_chg_ic_dev *ic_dev, int *vout)
{
	...
	for (i = 0; i < vc->child_num; i++) {
		if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_VOUT, vout);
			if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_get_iout(struct oplus_chg_ic_dev *ic_dev, int *iout)
{
	...
	*iout = 0;
	for (i = 0; i < vc->child_num; i++) {
		if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_IOUT, &curr);
			if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_get_vac(struct oplus_chg_ic_dev *ic_dev, int *vac)
{
	...
	vc = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vc->child_num; i++) {
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_VAC, &vol);
		if (rc < 0 && rc != -ENOTSUPP) {
			chg_err("child ic[%d] get iout error, rc=%d\n", i, rc);
			err = rc;
		} else if (rc >= 0) {
	...
	if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
		if (start) {
			...
			if (vc->main_cp < 0 || vc->main_cp >= vc->child_num)
				return -EINVAL;
			rc = oplus_chg_ic_func(vc->child_list[vc->main_cp].ic_dev, OPLUS_IC_FUNC_CP_SET_WORK_START,
					       true);
			if (rc < 0 && rc != -ENOTSUPP)
			...
			for (i = vc->child_num - 1; i >= 0; i--) {
				rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_SET_WORK_START,
						       false);
				if (rc < 0 && rc != -ENOTSUPP) {
...
static int oplus_chg_vc_get_work_status(struct oplus_chg_ic_dev *ic_dev, bool *start)
{
	...
	if (vc->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
		...
		if (vc->main_cp < 0 || vc->main_cp >= vc->child_num)
			return -EINVAL;
		rc = oplus_chg_ic_func(vc->child_list[vc->main_cp].ic_dev, OPLUS_IC_FUNC_CP_GET_WORK_STATUS, start);
		if (rc < 0 && rc != -ENOTSUPP)
...
static int oplus_chg_vc_adc_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	vc = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vc->child_num; i++) {
		rc = oplus_chg_ic_func(vc->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_SET_ADC_ENABLE, en);
		if (rc < 0 && rc != -ENOTSUPP) {
			chg_err("child ic[%d] %s error, rc=%d\n", i, en ? "enable" : "disable", rc);
			err = rc;
		} else if (rc >= 0) {
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
static int oplus_chg_vc_set_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, const void *buf,
				      size_t buf_len)
{
	int rc = 0;
...
static ssize_t oplus_chg_vc_get_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, void *buf)
{
	bool temp;
	int *item_data;
	ssize_t rc = 0;
...
static ssize_t oplus_vc_iin_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_IIN, &curr);
		if (rc == -ENOTSUPP) {
...
static ssize_t oplus_vc_vin_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (i > 0)
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, ",");
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_VIN, &vol);
		if (rc == -ENOTSUPP) {
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, "NULL");
			continue;
		} else if (rc < 0) {
...
static ssize_t oplus_vc_iout_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (i > 0)
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, ",");
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_IOUT, &curr);
		if (rc == -ENOTSUPP) {
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, "NULL");
			continue;
		} else if (rc < 0) {
...
static ssize_t oplus_vc_vout_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (i > 0)
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, ",");
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_VOUT, &vol);
		if (rc == -ENOTSUPP) {
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, "NULL");
			continue;
		} else if (rc < 0) {
...
static ssize_t oplus_vc_work_mode_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (i > 0)
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, ",");
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_WORK_MODE, &mode);
		if (rc == -ENOTSUPP) {
...
static ssize_t oplus_vc_work_status_proc_read(struct file *file, char __user *buff, size_t count, loff_t *off)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (i > 0)
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, ",");
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_CP_GET_WORK_STATUS, &work_start);
		if (rc == -ENOTSUPP) {
			len += snprintf(buf + len, ARRAY_SIZE(buf) - len, "Unknown");
			continue;
		} else if (rc < 0) {
...
#define PROBE_RETRY_MAX 300
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_cp_ic), GFP_KERNEL);
	if (chip == NULL) {
