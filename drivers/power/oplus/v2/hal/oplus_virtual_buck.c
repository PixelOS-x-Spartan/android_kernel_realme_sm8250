...
bool test_kit_typec_port_check(void *info, char *buf, size_t len, size_t *use_size)
{
	...
	*use_size = 0;
	...
	rc = oplus_chg_ic_func(vb->ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_PRESENT, &present);
	if (rc < 0) {
	...
	rc = oplus_chg_ic_func(vb->ic_dev, OPLUS_IC_FUNC_GET_OTG_ONLINE_STATUS, &otg_status);
	if (rc < 0) {
		chg_err("can't get otg status, rc=%d\n", rc);
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      "[%s][typec check case: %d] can't get otg status, rc=%d\n", typec_port_info->name,
				      typec_port_info->case_num, rc);
		return false;
	}
	...
	rc = oplus_chg_ic_func(vb->ic_dev, OPLUS_IC_FUNC_GET_TYPEC_ROLE, &typec_mode);
	if (rc < 0) {
	...
	if (typec_mode != typec_port_info->status) {
		*use_size +=
			snprintf(buf + *use_size, len - *use_size,
				 "[%s][typec check case: %d],typec mode error, expected: %d, actually: %u\n",
				 typec_port_info->name, typec_port_info->case_num, typec_port_info->status, typec_mode);
		pass = false;
	}
...
static int oplus_chg_vb_set_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type mode);
static int oplus_vb_virq_register(struct oplus_virtual_buck_ic *chip);

static inline bool func_is_support(struct oplus_virtual_buck_child *ic, enum oplus_chg_ic_func func_id)
{
	switch (func_id) {
...
static inline bool virq_is_support(struct oplus_virtual_buck_child *ic, enum oplus_chg_ic_virq_id virq_id)
{
	switch (virq_id) {
...
static int oplus_vc_ccdetect_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	chip->misc_gpio.ccdetect_active = pinctrl_lookup_state(chip->misc_gpio.pinctrl, "ccdetect_active");
	if (IS_ERR_OR_NULL(chip->misc_gpio.ccdetect_active)) {
		chg_err("get ccdetect_active fail\n");
		return -EINVAL;
	}

	chip->misc_gpio.ccdetect_sleep = pinctrl_lookup_state(chip->misc_gpio.pinctrl, "ccdetect_sleep");
...
static int oplus_ccdetect_enable(struct oplus_virtual_buck_ic *chip, bool en)
{
	...
	rc = oplus_chg_vb_set_typec_mode(chip->ic_dev, en ? TYPEC_PORT_ROLE_TRY_SNK : TYPEC_PORT_ROLE_SNK);
	if (rc < 0)
...
static int oplus_vc_ccdetect_irq_register(struct oplus_virtual_buck_ic *chip)
{
	...
	rc = devm_request_threaded_irq(chip->dev, chip->ccdetect_irq, NULL, oplus_vc_ccdetect_change_handler,
...
static int oplus_vc_child_init(struct oplus_virtual_buck_ic *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,buck_ic_connect", &chip->connect_type);
	if (rc < 0) {
	...
	rc = of_property_count_elems_of_size(node, "oplus,buck_ic", sizeof(u32));
	if (rc < 0) {
	...
	chip->child_num = rc;
	chip->child_list =
		devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_buck_child) * chip->child_num, GFP_KERNEL);
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		rc = of_property_read_u32_index(node, "oplus,buck_ic_current_ratio", i,
...
static int oplus_chg_vb_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		if (!func_is_support(&chip->child_list[i], OPLUS_IC_FUNC_BUCK_SET_ICL) ||
			...
			if (func_is_support(&chip->child_list[i], OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND))
		...
		if (!func_is_support(&chip->child_list[i], OPLUS_IC_FUNC_BUCK_SET_FCC) ||
		    chip->child_list[i].current_ratio == 0) {
			if (func_is_support(&chip->child_list[i], OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND))
...
static int oplus_chg_vb_exit(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CC_DETECT)) {
			oplus_chg_ic_virq_release(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CC_DETECT, chip);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_PLUGIN)) {
			oplus_chg_ic_virq_release(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_PLUGIN, chip);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CC_CHANGED)) {
			oplus_chg_ic_virq_release(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CC_CHANGED, chip);
		}
	}
	for (i = 0; i < chip->child_num; i++) {
		if (chip->child_list[i].virqs != NULL)
...
static int oplus_chg_vb_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (index >= len)
			return len;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SMT_TEST, buf + index, len - index);
		if (rc < 0) {
			if (rc != -ENOTSUPP) {
				chg_err("child ic[%d] smt test error, rc=%d\n", i, rc);
				rc = snprintf(buf + index, len - index, "[%s]-[%s]:%d\n",
					      vb->child_list[i].ic_dev->manu_name, "FUNC_ERR", rc);
			} else {
				rc = 0;
			}
		} else {
...
static int oplus_chg_vb_input_suspend(struct oplus_chg_ic_dev *ic_dev, bool suspend)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND,
					       suspend_temp);
			if (rc < 0) {
				chg_err("child ic[%d] input %s error, rc=%d\n", i,
					suspend_temp ? "suspend" : "unsuspend", rc);
				return rc;
			}
		} else if (vb->connect_type == OPLUS_CHG_IC_CONNECT_SERIAL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND)) {
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND,
					       &suspend_temp);
			if (rc < 0) {
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			if (vb->child_list[i].current_ratio == 0)
				...
				suspend_temp = suspend;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND,
					       suspend_temp);
			if (rc < 0) {
...
static int oplus_chg_vb_set_icl(struct oplus_chg_ic_dev *ic_dev, bool vooc_mode, bool step, int icl_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_SET_ICL))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_ICL, vooc_mode, step,
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_ICL))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_ICL, &temp_icl_ma);
			if (rc < 0) {
			...
			*icl_ma += temp_icl_ma;
			...
			if (i == 0) {
				rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_ICL, icl_ma);
				if (rc < 0) {
					chg_err("child ic[%d] get icl error, rc=%d\n", i, rc);
					return rc;
				}
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_SET_FCC))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_FCC,
...
static int oplus_chg_vb_set_fv(struct oplus_chg_ic_dev *ic_dev, int fv_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_SET_FV))
			return -ENOTSUPP;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_FV, fv_mv);
		if (rc < 0) {
			chg_err("child ic[%d] set fcc error, rc=%d\n", i, rc);
			return rc;
		}
...
static int oplus_chg_vb_set_iterm(struct oplus_chg_ic_dev *ic_dev, int iterm_ma)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			 ...
			 * the main charger.
			 */
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_ITERM, iterm_ma);
		} else if (vb->connect_type == OPLUS_CHG_IC_CONNECT_SERIAL) {
			return -EINVAL; /* TODO */
		} else {
			chg_err("Unknown connect type\n");
...
static int oplus_chg_vb_set_rechg_vol(struct oplus_chg_ic_dev *ic_dev, int vol_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			 ...
			 */
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL, vol_mv);
		} else if (vb->connect_type == OPLUS_CHG_IC_CONNECT_SERIAL) {
			return -EINVAL; /* TODO */
		} else {
			chg_err("Unknown connect type\n");
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR, &curr_temp);
			if (rc < 0) {
			...
			*curr_ma += curr_temp;
			...
			if (i == 0) {
				rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR,
						       curr_ma);
				if (rc < 0) {
					chg_err("child ic[%d] get intput current error, rc=%d\n", i, rc);
					return rc;
	...
	*vol_mv = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL, vol_mv);
			if (rc < 0)
...
static int oplus_chg_vb_otg_boost_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_OTG_BOOST_ENABLE, en);
			if (rc < 0)
...
static int oplus_chg_vb_set_otg_boost_vol(struct oplus_chg_ic_dev *ic_dev, int vol_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_OTG_BOOST_VOL, vol_mv);
			if (rc < 0)
...
static int oplus_chg_vb_set_otg_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev, int curr_ma)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT,
					       curr_ma);
			if (rc < 0)
...
static int oplus_chg_vb_get_cc_orientation(struct oplus_chg_ic_dev *ic_dev, int *orientation)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION, orientation);
		if (rc < 0)
...
static int oplus_chg_vb_get_hw_detect(struct oplus_chg_ic_dev *ic_dev, int *detected)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_HW_DETECT, detected);
		if (rc < 0)
...
static int oplus_chg_vb_get_charger_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE, &chg_type);
		if (rc < 0) {
			...
			if (chg_type == OPLUS_CHG_USB_TYPE_PD || chg_type == OPLUS_CHG_USB_TYPE_PD_DRP ||
			    chg_type == OPLUS_CHG_USB_TYPE_PD_PPS || chg_type == OPLUS_CHG_USB_TYPE_PD_SDP) {
				*type = chg_type;
				return 0;
			}
			if (!init)
...
static int oplus_chg_vb_rerun_bc12(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_RERUN_BC12);
		if (rc < 0)
...
static int oplus_chg_vb_qc_detect_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_shipmod_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_set_qc_config(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_qc_version version, int vol_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG, version, vol_mv);
		if (rc < 0)
...
static int oplus_chg_vb_set_pd_config(struct oplus_chg_ic_dev *ic_dev, u32 pdo)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG, pdo);
		if (rc < 0)
...
static int oplus_chg_vb_wls_boost_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_WLS_BOOST_ENABLE, en);
			if (rc < 0)
...
static int oplus_chg_vb_set_wls_boost_vol(struct oplus_chg_ic_dev *ic_dev, int vol_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_WLS_BOOST_VOL, vol_mv);
			if (rc < 0)
...
static int oplus_chg_vb_set_wls_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev, int curr_ma)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT,
					       curr_ma);
			if (rc < 0)
...
static int oplus_chg_vb_gauge_update(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GAUGE_UPDATE);
		if (rc < 0)
...
static int oplus_chg_vb_voocphy_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_VOOCPHY_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_voocphy_reset_again(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_VOOCPHY_RESET_AGAIN);
		if (rc < 0)
...
static int oplus_chg_vb_get_charger_cycle(struct oplus_chg_ic_dev *ic_dev, int *cycle)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_CHARGER_CYCLE, cycle);
		if (rc < 0) {
			if (rc != -ENOTSUPP)
...
static int oplus_common_get_shutdown_soc(struct oplus_virtual_buck_ic *chip, int *get_soc)
{
	...
	if (chip->soc_backup_nvmem) {
		...
		if (((soc & DATA_IS_VALID) == 0) || (soc & DATA_MASK) > SOC_MAX) {
...
static int oplus_chg_vb_get_shutdown_soc(struct oplus_chg_ic_dev *ic_dev, int *soc)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_SHUTDOWN_SOC, soc);
		if (rc < 0)
...
static int oplus_chg_vb_backup_soc(struct oplus_chg_ic_dev *ic_dev, int soc)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BACKUP_SOC, soc);
		if (rc < 0)
...
static int oplus_chg_vb_get_vbus_collapse_status(struct oplus_chg_ic_dev *ic_dev, bool *collapse)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS,
					       collapse);
			if (rc < 0)
...
static int oplus_chg_vb_get_typec_role(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_TYPEC_ROLE, mode);
		if (rc < 0 && rc != -ENOTSUPP)
...
static int oplus_chg_vb_get_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_TYPEC_MODE, mode);
		if (rc < 0 && rc != -ENOTSUPP)
			chg_err("child ic[%d] get typec mode error, rc=%d\n", i, rc);
		return rc;
	}
...
static int oplus_chg_vb_set_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type mode)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_TYPEC_MODE, mode);
		if (rc < 0)
...
static int oplus_chg_vb_set_usb_dischg_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, en);
		if (rc < 0)
			chg_err("child ic[%d] usbtemp disable en(=%d) error, rc=%d\n", i, en, rc);
		return rc;
	}

	if (IS_ERR_OR_NULL(vb->misc_gpio.pinctrl) || IS_ERR_OR_NULL(vb->misc_gpio.dischg_disable) ||
	...
	rc = pinctrl_select_state(vb->misc_gpio.pinctrl,
				  en ? vb->misc_gpio.dischg_enable : vb->misc_gpio.dischg_disable);
	if (rc < 0)
...
static int oplus_chg_vb_get_usb_dischg_status(struct oplus_chg_ic_dev *ic_dev, bool *en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_USB_DISCHG_STATUS, en);
		if (rc < 0)
...
static int oplus_chg_vb_set_otg_switch_status(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS, en);
		if (rc < 0)
...
static int oplus_chg_vb_get_otg_switch_status(struct oplus_chg_ic_dev *ic_dev, bool *en)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS, en);
		if (rc < 0)
...
static int oplus_chg_vb_cc_detect_happened(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_CC_DETECT_HAPPENED);
		if (rc < 0)
...
static int oplus_chg_vb_curr_drop(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			if (vb->child_list[i].current_ratio == 0)
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_CURR_DROP);
		} else if (vb->connect_type == OPLUS_CHG_IC_CONNECT_SERIAL) {
			if (i != 0)
...
static int oplus_chg_vb_wdt_enable(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_WDT_ENABLE))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_WDT_ENABLE, enable);
		if (rc < 0) {
...
static int oplus_chg_vb_kick_wdt(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_KICK_WDT))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_KICK_WDT);
		if (rc < 0) {
...
static int oplus_chg_vb_set_aicl_point(struct oplus_chg_ic_dev *ic_dev, int vbatt_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_SET_AICL_POINT))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_AICL_POINT, vbatt_mv);
		if (rc < 0) {
...
static int oplus_chg_vb_set_vindpm(struct oplus_chg_ic_dev *ic_dev, int vol_mv)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_SET_VINDPM))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_SET_VINDPM, vol_mv);
		if (rc < 0) {
...
static int oplus_chg_vb_hardware_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_HARDWARE_INIT))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_HARDWARE_INIT);
		if (rc < 0)
...
static int oplus_chg_vb_set_curr_level(struct oplus_chg_ic_dev *ic_dev, int cool_down)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_VOOCPHY_SET_CURR_LEVEL))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_VOOCPHY_SET_CURR_LEVEL, cool_down);
		if (rc < 0) {
...
static int oplus_chg_vb_set_match_temp(struct oplus_chg_ic_dev *ic_dev, int match_temp)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_VOOCPHY_SET_MATCH_TEMP))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_VOOCPHY_SET_MATCH_TEMP, match_temp);
		if (rc < 0) {
...
static int oplus_chg_vb_get_otg_enable(struct oplus_chg_ic_dev *ic_dev, bool *enable)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_OTG_ENABLE, enable);
		if (rc < 0) {
...
static int oplus_chg_vb_get_charger_vol_max(struct oplus_chg_ic_dev *ic_dev, int *vol)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_CHARGER_VOL_MAX, &vol_tmp);
		if (rc < 0) {
...
static int oplus_chg_vb_get_charger_vol_min(struct oplus_chg_ic_dev *ic_dev, int *vol)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_CHARGER_VOL_MIN, &vol_tmp);
		if (rc < 0) {
...
static int oplus_chg_vb_get_charger_curr_max(struct oplus_chg_ic_dev *ic_dev, int *curr)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_CHARGER_CURR_MAX, &curr_tmp);
		if (rc < 0) {
...
static int oplus_chg_vb_bc12_completed(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_BC12_COMPLETED))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_BC12_COMPLETED);
		if (rc < 0)
...
static int oplus_chg_vb_disable_vbus(struct oplus_chg_ic_dev *ic_dev, bool en, bool delay)
{
	...
	vb = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_DISABLE_VBUS))
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_DISABLE_VBUS, en, delay);
		if (rc < 0)
			chg_err("child ic[%d] %s vbus error, rc=%d\n", i, en ? "disable" : "enable", rc);
	}

	return rc;
}

static int oplus_chg_vb_is_oplus_svid(struct oplus_chg_ic_dev *ic_dev, bool *oplus_svid)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_IS_OPLUS_SVID);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_IS_OPLUS_SVID)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_IS_OPLUS_SVID, oplus_svid);
		if (rc < 0)
...
static int oplus_chg_vb_get_data_role(struct oplus_chg_ic_dev *ic_dev, int *role)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_GET_DATA_ROLE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_GET_DATA_ROLE)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_DATA_ROLE, role);
		if (rc < 0)
...
static int oplus_chg_vb_get_typec_state(struct oplus_chg_ic_dev *ic_dev, int *state)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_BUCK_GET_TYPEC_STATE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_TYPEC_STATE)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_TYPEC_STATE, state);
		if (rc < 0)
...
static int oplus_chg_vb_get_usb_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *usb_btb_temp)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP, usb_btb_temp);
		if (rc < 0)
...
static int oplus_chg_vb_get_batt_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *batt_btb_temp)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP, batt_btb_temp);
		if (rc < 0)
...
static int oplus_chg_vb_get_fv(struct oplus_chg_ic_dev *ic_dev, int *fv_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	int rc = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(ic_dev, OPLUS_IC_FUNC_BUCK_GET_FV);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_FV)) {
			rc = -ENOTSUPP;
			continue;
		}
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_FV, fv_ma);
		if (rc < 0)
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			if (vb->child_list[i].current_ratio == 0)
				...
				suspend_temp = suspend;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_WLS_INPUT_SUSPEND,
					       suspend_temp);
			if (rc < 0) {
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			*icl_ma += temp_icl_ma;
			...
			if (i == 0) {
				rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_WLS_ICL,
						       icl_ma);
				if (rc < 0) {
					chg_err("child ic[%d] get wls icl error, rc=%d\n", i, rc);
					return rc;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			if (!func_is_support(&vb->child_list[i], OPLUS_IC_FUNC_BUCK_GET_WLS_INPUT_CURR))
				continue;
			rc = oplus_chg_ic_func(vb->child_list[i].ic_dev, OPLUS_IC_FUNC_BUCK_GET_WLS_INPUT_CURR,
					       &curr_temp);
			if (rc < 0) {
...
static void *oplus_chg_vb_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT,
					       oplus_chg_vb_set_otg_boost_curr_limit);
		break;
	...
	case OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT,
					       oplus_chg_vb_set_wls_boost_curr_limit);
		break;
	...
	case OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS,
					       oplus_chg_vb_get_vbus_collapse_status);
		break;
	...
	case OPLUS_IC_FUNC_USB_TEMP_CHECK_IS_SUPPORT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_USB_TEMP_CHECK_IS_SUPPORT,
					       oplus_chg_vb_usb_temp_check_is_support);
		break;
	...
	case OPLUS_IC_FUNC_IS_OPLUS_SVID:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_IS_OPLUS_SVID, oplus_chg_vb_is_oplus_svid);
		break;
	...
	case OPLUS_IC_FUNC_GET_DATA_ROLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_DATA_ROLE, oplus_chg_vb_get_data_role);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP, oplus_chg_vb_get_usb_btb_temp);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP, oplus_chg_vb_get_batt_btb_temp);
		break;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
static int oplus_chg_vb_set_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, const void *buf,
				      size_t buf_len)
{
	int rc = 0;
	...
	case OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_qc_config(ic_dev, oplus_chg_ic_get_item_data(buf, 0),
	...
	case OPLUS_IC_FUNC_DISABLE_VBUS:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_disable_vbus(ic_dev, oplus_chg_ic_get_item_data(buf, 0),
					       oplus_chg_ic_get_item_data(buf, 1));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_AICL_POINT:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_aicl_point(ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_VINDPM:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_vindpm(ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	default:
...
static ssize_t oplus_chg_vb_get_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, void *buf)
{
	bool temp;
	int *item_data;
	ssize_t rc = 0;
	...
	case OPLUS_IC_FUNC_GET_USB_TEMP_VOLT:
		...
		rc = oplus_chg_vb_get_usb_temp_volt(ic_dev, oplus_chg_ic_get_item_data_addr(buf, 0),
...
static void oplus_vb_data_role_changed_handler(struct oplus_chg_ic_dev *ic_dev, void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;

	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_DATA_ROLE_CHANGED);
}
	...
	for (i = 0; i < chip->child_num; i++) {
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_ERR)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_ERR,
							oplus_vb_err_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_ERR error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CC_DETECT)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CC_DETECT,
							oplus_vb_cc_detect_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CC_DETECT error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_PLUGIN)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_PLUGIN,
							oplus_vb_plugin_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_PLUGIN error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CC_CHANGED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CC_CHANGED,
							oplus_vb_cc_changed_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CC_CHANGED error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_SUSPEND_CHECK)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_SUSPEND_CHECK,
							oplus_vb_suspend_check_handler, chip);
			if (rc < 0)
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CHG_TYPE_CHANGE)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CHG_TYPE_CHANGE,
							oplus_vb_chg_type_change_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CHG_TYPE_CHANGE error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_SVID)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_SVID,
							oplus_vb_svid_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_SVID error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_OTG_ENABLE)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_OTG_ENABLE,
							oplus_vb_otg_enable_handler, chip);
			if (rc < 0)
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_VOLTAGE_CHANGED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_VOLTAGE_CHANGED,
							oplus_vb_voltage_change_handler, chip);
			if (rc < 0)
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_CURRENT_CHANGED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_CURRENT_CHANGED,
							oplus_vb_current_change_handler, chip);
			if (rc < 0)
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_BC12_COMPLETED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_BC12_COMPLETED,
							oplus_vb_bc12_completed_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_BC12_COMPLETED error, rc=%d", rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_DATA_ROLE_CHANGED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_DATA_ROLE_CHANGED,
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_TYPEC_STATE)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_TYPEC_STATE,
							oplus_vb_typec_state_notify_handler, chip);
			if (rc < 0)
...
static int oplus_virtual_buck_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_buck_ic), GFP_KERNEL);
	if (chip == NULL) {
	...
	if (of_find_property(node, "nvmem-cells", NULL)) {
		chip->soc_backup_nvmem = devm_nvmem_cell_get(chip->dev, "oplus_soc_backup");
