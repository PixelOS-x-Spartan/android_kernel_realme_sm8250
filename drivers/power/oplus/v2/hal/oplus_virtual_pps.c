...
static int oplus_vpps_base_virq_register(struct oplus_virtual_pps_ic *chip, int index)
{
	...
	rc = oplus_chg_ic_virq_register(chip->child_list[index].ic_dev, OPLUS_IC_VIRQ_ERR, oplus_vpps_err_handler,
					chip);
	if (rc < 0 && rc != -ENOTSUPP)
...
static int oplus_vpps_child_init(struct oplus_virtual_pps_ic *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "oplus,pps_ic", sizeof(u32));
	if (rc < 0) {
	...
	chip->child_num = rc;
	chip->child_list =
		devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_pps_child) * chip->child_num, GFP_KERNEL);
...
static int oplus_chg_pps_get_power_change_info(struct oplus_chg_ic_dev *ic_dev, u32 *pwr_change_info, int num)
{
	return 0;
}
...
static void *oplus_chg_pps_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_pps_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_pps_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_chg_pps_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_chg_pps_smt_test);
		break;
	case OPLUS_IC_FUNC_PPS_PDO_SET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_PDO_SET, oplus_chg_pps_pdo_set);
		break;
	case OPLUS_IC_FUNC_FIXED_PDO_SET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_FIXED_PDO_SET, oplus_chg_fixed_pdo_set);
		break;
	case OPLUS_IC_FUNC_PPS_HARD_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_HARD_RESET, oplus_chg_pps_hard_reset);
		break;
	case OPLUS_IC_FUNC_PPS_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_EXIT, oplus_chg_pps_exit_pps_mode);
		break;
	case OPLUS_IC_FUNC_PPS_CONFIG_WD:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_CONFIG_WD, oplus_chg_pps_config_wd);
		break;
	case OPLUS_IC_FUNC_PPS_GET_DEV_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_GET_DEV_INFO, oplus_chg_pps_get_dev_info);
		break;
	case OPLUS_IC_FUNC_PPS_GET_ERR_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_GET_ERR_INFO, oplus_chg_pps_get_err_info);
		break;
	...
	case OPLUS_IC_FUNC_PPS_GET_CABLE_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_GET_CABLE_INFO, oplus_chg_pps_get_cable_info);
		break;
	case OPLUS_IC_FUNC_PPS_GET_PDO_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_GET_PDO_INFO, oplus_chg_pps_get_pdo_info);
		break;
	case OPLUS_IC_FUNC_PPS_VERIFY_ADAPTER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_PPS_VERIFY_ADAPTER, oplus_chg_pps_verify_adapter);
		break;
...
static int oplus_chg_pps_set_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, const void *buf,
				       size_t buf_len)
{
	int rc = 0;
	...
	case OPLUS_IC_FUNC_PPS_PDO_SET:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_pps_pdo_set(ic_dev, oplus_chg_ic_get_item_data(buf, 0),
					   oplus_chg_ic_get_item_data(buf, 1));
		break;
	case OPLUS_IC_FUNC_FIXED_PDO_SET:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_fixed_pdo_set(ic_dev, oplus_chg_ic_get_item_data(buf, 0),
					     oplus_chg_ic_get_item_data(buf, 1));
		break;
	case OPLUS_IC_FUNC_PPS_HARD_RESET:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
...
static ssize_t oplus_chg_pps_get_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id, void *buf)
{
	ssize_t rc = 0;
	int len;
	char *tmp_buf;
...
static int oplus_virtual_pps_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_pps_ic), GFP_KERNEL);
	if (chip == NULL) {
