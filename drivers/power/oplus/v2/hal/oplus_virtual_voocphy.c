...
static int oplus_vphy_virq_register(struct oplus_virtual_vphy_ic *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->vphy, OPLUS_IC_VIRQ_ERR, oplus_vphy_err_handler, chip);
	if (rc < 0)
		chg_err("register OPLUS_IC_VIRQ_ERR error, rc=%d", rc);

	for (i = 0; i < chip->vphy_num; i++) {
		rc = oplus_chg_ic_virq_register(chip->vphy_list[i].ic_dev, OPLUS_IC_VIRQ_VOOC_DATA,
...
static int oplus_chg_vphy_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < va->vphy_num; i++) {
		if (va->vphy_list[i].initialized)
			continue;
		va->vphy_list[i].ic_dev = of_get_oplus_chg_ic(node, "oplus,vphy_ic", i);
...
static int oplus_chg_vphy_exit(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < va->vphy_num; i++) {
		oplus_chg_ic_virq_release(va->vphy_list[i].ic_dev, OPLUS_IC_VIRQ_VOOC_DATA, va);
	};
...
static int oplus_chg_vvphy_reply_data(struct oplus_chg_ic_dev *ic_dev, int data, int data_width, int curr_ma)
{
	...
	rc = oplus_chg_ic_func(vvphy->vphy, OPLUS_IC_FUNC_VOOC_REPLY_DATA, data, data_width, curr_ma);
	if (rc < 0)
...
static int oplus_chg_vphy_set_switch_mode(struct oplus_chg_ic_dev *ic_dev, int mode)
{
	struct oplus_virtual_vphy_ic *va;
	int retry = 10;
	int rc = 0;
	...
	case VOOC_SWITCH_MODE_VOOC:
		do {
			if (va->use_dpdm_switch_ic) {
				rc = oplus_chg_ic_func(va->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE,
						       DPDM_SWITCH_TO_VOOC);
				if (rc < 0) {
				...
				rc = pinctrl_select_state(va->gpio.pinctrl, va->gpio.vphy_switch_vooc);
				if (rc < 0) {
					chg_err("set vphy_switch_vooc error, rc=%d\n", rc);
					goto err;
				}
				rc = pinctrl_select_state(va->gpio.pinctrl, va->gpio.gpio_switch_ctrl_vphy);
				if (rc < 0) {
	...
	default:
		if (va->use_dpdm_switch_ic) {
			...
			mutex_lock(&va->gpio.pinctrl_mutex);
			rc = pinctrl_select_state(va->gpio.pinctrl, va->gpio.vphy_switch_normal);
			if (rc < 0) {
...
err:
	if (!va->use_dpdm_switch_ic) {
		...
		oplus_chg_ic_func(va->dpdm_switch, OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, DPDM_SWITCH_TO_AP);
	}
	return rc;
}

static int oplus_chg_vphy_get_switch_mode(struct oplus_chg_ic_dev *ic_dev, int *mode)
{
	struct oplus_virtual_vphy_ic *va;
...
static int oplus_chg_vphy_get_cp_vbat(struct oplus_chg_ic_dev *ic_dev, int *cp_vbat)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT, cp_vbat);
	if (rc < 0)
...
static int oplus_chg_vphy_set_bcc_curr(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_SET_BCC_CURR, cp_data);
	if (rc < 0)
		chg_err("get cp vbat error, rc=%d\n", rc);
...
static int oplus_chg_vphy_set_ucp_time(struct oplus_chg_ic_dev *ic_dev, int value)
{
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME, value);
	if (rc < 0)
...
static int oplus_chg_vphy_get_bcc_max_curr(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR, cp_data);
	if (rc < 0)
...
static int oplus_chg_vphy_get_bcc_min_curr(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR, cp_data);
	if (rc < 0)
...
static int oplus_chg_vphy_get_bcc_exit_curr(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_EXIT_CURR, cp_data);
	if (rc < 0)
...
static int oplus_chg_vphy_get_fastchg_ing(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING, cp_data);
	if (rc < 0)
...
static int oplus_chg_vphy_get_bcc_temp_range(struct oplus_chg_ic_dev *ic_dev, int *cp_data)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_GET_BCC_TEMP_RANGE, cp_data);
	if (rc < 0)
...
static int oplus_chg_vphy_set_chg_auto_mode(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	int rc;
	struct oplus_virtual_vphy_ic *va;
	...
	rc = oplus_chg_ic_func(va->vphy, OPLUS_IC_FUNC_VOOCPHY_SET_CHG_AUTO_MODE, enable);
	if (rc < 0)
...
static int oplus_chg_vphy_get_vphy(struct oplus_chg_ic_dev *ic_dev, struct oplus_chg_ic_dev **vphy)
{
	struct oplus_virtual_vphy_ic *va;
...
static void *oplus_chg_vphy_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_vphy_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_vphy_exit);
		break;
	case OPLUS_IC_FUNC_VOOC_READ_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_READ_DATA, oplus_chg_vvphy_get_data);
		break;
	case OPLUS_IC_FUNC_VOOC_REPLY_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_REPLY_DATA, oplus_chg_vvphy_reply_data);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE, oplus_chg_vphy_set_switch_mode);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_SWITCH_MODE, oplus_chg_vphy_get_switch_mode);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG, oplus_chg_vphy_set_pdqc_config);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT, oplus_chg_vphy_get_cp_vbat);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_BCC_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_BCC_CURR, oplus_chg_vphy_set_bcc_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR, oplus_chg_vphy_get_bcc_max_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR, oplus_chg_vphy_get_bcc_min_curr);
		break;
	...
	case OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING, oplus_chg_vphy_get_fastchg_ing);
		break;
	...
	case OPLUS_IC_FUNC_VOOC_GET_IC_DEV:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_IC_DEV, oplus_chg_vphy_get_vphy);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR, oplus_chg_vphy_get_curve_current);
		break;
	case OPLUS_IC_FUNC_VOOC_RESET_SLEEP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_RESET_SLEEP, oplus_chg_vphy_reset_sleep);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME, oplus_chg_vphy_set_ucp_time);
		break;
	...
	case OPLUS_IC_FUNC_VOOCPHY_GET_RETRY_FLAG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_RETRY_FLAG, oplus_chg_vphy_get_retry_flag);
		break;
...
{
	struct oplus_virtual_vphy_ic *va = container_of(work, struct oplus_virtual_vphy_ic, data_handler_work);
...
static int oplus_vphy_child_init(struct oplus_virtual_vphy_ic *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "oplus,vphy_ic", sizeof(u32));
	if (rc < 0) {
		chg_err("can't get vphy number, rc=%d\n", rc);
		return rc;
	}
	chip->vphy_num = rc;
	chip->vphy_list = devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_vphy_child) * chip->vphy_num, GFP_KERNEL);
...
static int oplus_vphy_gpio_init(struct oplus_virtual_vphy_ic *chip)
{
	...
	vphy_gpio->switch_ctrl_gpio = of_get_named_gpio(node, "oplus,switch_ctrl-gpio", 0);
	...
	vphy_gpio->gpio_switch_ctrl_ap = pinctrl_lookup_state(vphy_gpio->pinctrl, "switch_ctrl_ap");
	if (IS_ERR_OR_NULL(vphy_gpio->gpio_switch_ctrl_ap)) {
		chg_err("get switch_ctrl_ap fail\n");
		rc = -EINVAL;
		goto free_switch_ctrl_gpio;
	}
	vphy_gpio->gpio_switch_ctrl_vphy = pinctrl_lookup_state(vphy_gpio->pinctrl, "switch_ctrl_vphy");
	if (IS_ERR_OR_NULL(vphy_gpio->gpio_switch_ctrl_vphy)) {
		chg_err("get switch_ctrl_vphy fail\n");
		rc = -EINVAL;
		goto free_switch_ctrl_gpio;
	}
	rc = pinctrl_select_state(vphy_gpio->pinctrl, vphy_gpio->gpio_switch_ctrl_ap);
	if (rc < 0) {
		chg_err("set gpio switch_ctrl_ap error, rc=%d\n", rc);
		goto free_switch_ctrl_gpio;
	}

	vphy_gpio->vphy_switch1_gpio = of_get_named_gpio(node, "oplus,vphy_switch1-gpio", 0);
...
static int oplus_virtual_vphy_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_vphy_ic), GFP_KERNEL);
	if (chip == NULL) {
