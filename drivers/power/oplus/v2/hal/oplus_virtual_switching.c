...
static int oplus_chg_switching_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		if (chip->child_list[i].initialized)
			continue;
		chip->child_list[i].ic_dev = of_get_oplus_chg_ic(node, "oplus,switch_ic", i);
...
skip_overwrite:
	...
	chip = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < chip->child_num; i++) {
		oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_SET_HW_ENABLE, enable);
	}
...
skip_overwrite:
	...
	chip = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < chip->child_num; i++) {
		oplus_chg_ic_func(chip->child_list[i].ic_dev, OPLUS_IC_FUNC_GET_HW_ENABLE, enable);
	}
...
static void *oplus_chg_switching_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_switching_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_switching_exit);
		break;
	case OPLUS_IC_FUNC_SET_HW_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_HW_ENABLE, oplus_chg_switching_set_enable);
		break;
	case OPLUS_IC_FUNC_GET_HW_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_HW_ENABLE, oplus_chg_switching_get_enable);
		break;
	case OPLUS_IC_FUNC_GET_DEVICE_TYPE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_DEVICE_TYPE, oplus_chg_switching_get_type);
		break;
	default:
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
static ssize_t oplus_chg_switching_get_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id,
						 void *buf)
{
	bool temp;
	int *item_data;
...
static int oplus_chg_switching_set_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id,
...
static int oplus_switching_child_init(struct oplus_virtual_switching_ic *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "oplus,switch_ic", sizeof(u32));
	if (rc < 0) {
	...
	chip->child_num = rc;
	chip->child_list =
		devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_switching_child) * chip->child_num, GFP_KERNEL);
...
static int oplus_virtual_switching_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_switching_ic), GFP_KERNEL);
	if (chip == NULL) {
