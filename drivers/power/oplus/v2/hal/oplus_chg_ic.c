...
{
	return ida_simple_get(&oplus_chg_ic_ida, 0, OPLUS_CHG_IC_MAX - 1, GFP_KERNEL);
}
...
bool oplus_chg_ic_is_overwrited(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
...
struct oplus_chg_ic_overwrite_data *oplus_chg_ic_get_overwrite_data(struct oplus_chg_ic_dev *ic_dev,
								    enum oplus_chg_ic_func func_id)
{
	struct oplus_chg_ic_overwrite_data *data;
...
{
	return sizeof(struct oplus_chg_ic_func_date) + (sizeof(struct oplus_chg_ic_func_date_item) + 4) * argc;
}
...
static ssize_t func_id_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	int val;
...
static ssize_t data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	rc = ic_dev->debug.get_func_data(ic_dev, ic_dev->debug.func_id, (void *)buf);
...
static ssize_t data_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rc = ic_dev->debug.set_func_data(ic_dev, ic_dev->debug.func_id, (void *)buf, count);

	return rc;
}
static DEVICE_ATTR_RW(data);

static ssize_t overwrite_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	int argc;
	...
	if (!oplus_chg_ic_func_check_support_by_table(ic_dev->debug.overwrite_funcs, ic_dev->debug.func_num,
						      ic_dev->debug.func_id)) {
	...
	if (argc < 0)
		return argc;
	for (i = 0; i < argc; i++)
		chg_err("overwrite argv[%d] = %d\n", i, oplus_chg_ic_get_item_data((const void *)buf, i));

	overwrite_data = devm_kzalloc(dev, sizeof(struct oplus_chg_ic_overwrite_data) + count, GFP_KERNEL);
...
static ssize_t clean_overwrite_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	struct oplus_chg_ic_overwrite_data *data;
...
static ssize_t virq_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
...
static ssize_t manu_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
...
static ssize_t fw_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
...
static ssize_t supported_funcs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
...
static ssize_t supported_virqs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	struct oplus_chg_ic_support_info *support = &ic_dev->support;
	int i;
...
static struct device_attribute *oplus_chg_ic_attributes[] = { &dev_attr_func_id,
							      &dev_attr_data,
							      &dev_attr_overwrite,
...
static ssize_t version_show(struct class *C, struct class_attribute *attr, char *buf)
{
...
int oplus_chg_ic_virq_register(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_virq_id virq_id,
			       virq_handler_t handler, void *virq_data)
{
	struct oplus_chg_ic_virq *virq;
	int i;
...
int oplus_chg_ic_virq_release(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_virq_id virq_id, void *virq_data)
{
	struct oplus_chg_ic_virq *virq;
	int i;
...
int oplus_chg_ic_virq_trigger(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_virq_id virq_id)
{
	struct oplus_chg_ic_virq *virq;
	int i;
	...
	for (i = 0; i < ic_dev->virq_num; i++) {
		virq = &ic_dev->virq_data[i];
		if (virq->virq_id == virq_id) {
			if (virq->virq_handler == NULL || virq->virq_data == NULL) {
...
__printf(4, 5) int oplus_chg_ic_creat_err_msg(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_err err_type,
					      int sub_err_type, const char *format, ...)
{
	va_list args;
...
int oplus_chg_ic_move_err_msg(struct oplus_chg_ic_dev *dest, struct oplus_chg_ic_dev *src)
{
	if (dest == NULL) {
	...
	if (!list_empty(&src->err_list))
		list_bulk_move_tail(&dest->err_list, src->err_list.next, src->err_list.prev);
...
int oplus_chg_ic_clean_err_msg(struct oplus_chg_ic_dev *ic_dev, struct oplus_chg_ic_err_msg *err_msg)
{
	if (ic_dev == NULL) {
		chg_err("ic_dev is null\n");
		return -ENODEV;
...
static struct oplus_chg_ic_dev *__oplus_chg_ic_register(struct device *dev, struct oplus_chg_ic_cfg *cfg)
{
	struct oplus_chg_ic_dev *dev_temp;
	struct oplus_chg_ic_dev *ic_dev;
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	...
	snprintf(ic_dev->cdev_name, sizeof(ic_dev->cdev_name), "vic-%d", ic_dev->minor);
	ic_dev->debug_dev =
		device_create(&oplus_chg_ic_class, ic_dev->dev, MKDEV(MAJOR(oplus_chg_ic_devno), ic_dev->minor), ic_dev,
			      "vic-%d", ic_dev->minor);
	...
	rc = cdev_add(&ic_dev->cdev, MKDEV(MAJOR(oplus_chg_ic_devno), ic_dev->minor), 1);
	if (rc < 0) {
...
cdev_add_err:
	device_destroy(&oplus_chg_ic_class, MKDEV(MAJOR(oplus_chg_ic_devno), ic_dev->minor));
device_create_err:
...
struct oplus_chg_ic_dev *oplus_chg_ic_register(struct device *dev, struct oplus_chg_ic_cfg *cfg)
{
...
static void devm_oplus_chg_ic_release(struct device *dev, void *res)
{
	...
	device_destroy(&oplus_chg_ic_class, MKDEV(MAJOR(oplus_chg_ic_devno), this->ic_dev->minor));
...
struct oplus_chg_ic_dev *devm_oplus_chg_ic_register(struct device *dev, struct oplus_chg_ic_cfg *cfg)
{
	struct ic_devres *dr;
	struct oplus_chg_ic_dev *ic_dev;
...
static __init int oplus_chg_ic_class_init(void)
{
	...
	rc = alloc_chrdev_region(&oplus_chg_ic_devno, 0, OPLUS_CHG_IC_MAX, "virtual_ic");
	if (rc < 0) {
