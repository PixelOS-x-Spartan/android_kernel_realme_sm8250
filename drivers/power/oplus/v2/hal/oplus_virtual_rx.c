...
static inline bool func_is_support(struct oplus_virtual_rx_child *ic, enum oplus_chg_ic_func func_id)
{
	switch (func_id) {
...
static inline bool virq_is_support(struct oplus_virtual_rx_child *ic, enum oplus_chg_ic_virq_id virq_id)
{
	switch (virq_id) {
...
static int oplus_chg_vr_child_funcs_init(struct oplus_virtual_rx_ic *chip, int child_num)
{
...
static int oplus_chg_vr_child_virqs_init(struct oplus_virtual_rx_ic *chip, int child_num)
{
	struct device_node *node = chip->dev->of_node;
...
static int oplus_chg_vr_child_init(struct oplus_virtual_rx_ic *chip)
{
	...
	chip->child_num = rc;
	chip->child_list = devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_rx_child) * chip->child_num, GFP_KERNEL);
...
static int oplus_chg_vr_exit(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vr->child_num; i++) {
		if (virq_is_support(&vr->child_list[i], OPLUS_IC_VIRQ_ERR)) {
			oplus_chg_ic_virq_release(vr->child_list[i].ic_dev, OPLUS_IC_VIRQ_ERR, vr);
		}
		if (virq_is_support(&vr->child_list[i], OPLUS_IC_VIRQ_PRESENT)) {
			oplus_chg_ic_virq_release(vr->child_list[i].ic_dev, OPLUS_IC_VIRQ_PRESENT, vr);
		}
		if (virq_is_support(&vr->child_list[i], OPLUS_IC_VIRQ_ONLINE)) {
			oplus_chg_ic_virq_release(vr->child_list[i].ic_dev, OPLUS_IC_VIRQ_ONLINE, vr);
		}
		if (virq_is_support(&vr->child_list[i], OPLUS_IC_VIRQ_OFFLINE)) {
			oplus_chg_ic_virq_release(vr->child_list[i].ic_dev, OPLUS_IC_VIRQ_OFFLINE, vr);
		}
		if (virq_is_support(&vr->child_list[i], OPLUS_IC_VIRQ_EVENT_CHANGED)) {
			oplus_chg_ic_virq_release(vr->child_list[i].ic_dev, OPLUS_IC_VIRQ_EVENT_CHANGED, vr);
		}
	}
	for (i = 0; i < vr->child_num; i++) {
...
static int oplus_chg_vr_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	...
	for (i = 0; i < vr->child_num; i++) {
		if (index >= len)
			return len;
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_SMT_TEST, buf + index, len - index);
		if (rc < 0) {
			if (rc != -ENOTSUPP) {
				chg_err("child ic[%d] smt test error, rc=%d\n", i, rc);
				rc = snprintf(buf + index, len - index, "[%s]-[%s]:%d\n",
					      vr->child_list[i].ic_dev->manu_name, "FUNC_ERR", rc);
			} else {
				rc = 0;
			}
		} else {
...
static int oplus_chg_vr_set_rx_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_ENABLE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vr_rx_is_enable(struct oplus_chg_ic_dev *ic_dev, bool *enable)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_IS_ENABLE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_IS_ENABLE, enable);
		if (rc < 0)
...
static int oplus_chg_vr_rx_is_connected(struct oplus_chg_ic_dev *ic_dev, bool *connected)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_IS_CONNECTED)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}

		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_IS_CONNECTED, connected);
		if (rc < 0)
...
static int oplus_chg_vr_get_vout(struct oplus_chg_ic_dev *ic_dev, int *vout)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_VOUT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_VOUT, vout);
		if (rc < 0)
...
static int oplus_chg_vr_set_vout(struct oplus_chg_ic_dev *ic_dev, int vout)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_VOUT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_VOUT, vout);
		if (rc < 0)
...
static int oplus_chg_vr_get_vrect(struct oplus_chg_ic_dev *ic_dev, int *vrect)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_VRECT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_VRECT, vrect);
		if (rc < 0)
...
static int oplus_chg_vr_get_iout(struct oplus_chg_ic_dev *ic_dev, int *iout)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_IOUT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_IOUT, iout);
		if (rc < 0)
...
static int oplus_chg_vr_get_tx_vout(struct oplus_chg_ic_dev *ic_dev, int *vout)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_TRX_VOL)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_TRX_VOL, vout);
		if (rc < 0)
...
static int oplus_chg_vr_get_tx_iout(struct oplus_chg_ic_dev *ic_dev, int *iout)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_TRX_CURR)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_TRX_CURR, iout);
		if (rc < 0)
...
static int oplus_chg_vr_get_cep_count(struct oplus_chg_ic_dev *ic_dev, int *count)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_CEP_COUNT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_CEP_COUNT, count);
		if (rc < 0)
...
static int oplus_chg_vr_get_cep_val(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_CEP_VAL)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_CEP_VAL, val);
		if (rc < 0)
...
static int oplus_chg_vr_get_work_freq(struct oplus_chg_ic_dev *ic_dev, int *freq)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_WORK_FREQ)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_WORK_FREQ, freq);
		if (rc < 0)
...
static int oplus_chg_vr_get_rx_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_wls_rx_mode *mode)
{
	struct oplus_virtual_rx_ic *vr;
	int i;
	int rc = 0;
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_RX_MODE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_RX_MODE, mode);
		if (rc < 0)
...
static int oplus_chg_vr_set_rx_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_wls_rx_mode mode)
{
	struct oplus_virtual_rx_ic *vr;
	int i;
	int rc = 0;
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_RX_MODE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_RX_MODE, mode);
		if (rc < 0)
...
static int oplus_chg_vr_set_dcdc_enable(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_DCDC_ENABLE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_DCDC_ENABLE, enable);
		if (rc < 0)
...
static int oplus_chg_vr_set_tx_enable(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_TRX_ENABLE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_TRX_ENABLE, enable);
		if (rc < 0)
...
static int oplus_chg_vr_set_tx_start(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_TRX_START)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_TRX_START);
		if (rc < 0)
...
static int oplus_chg_vr_get_tx_status(struct oplus_chg_ic_dev *ic_dev, u8 *status)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_TRX_STATUS)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_TRX_STATUS, status);
		if (rc < 0)
...
static int oplus_chg_vr_get_tx_err(struct oplus_chg_ic_dev *ic_dev, u8 *err)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_TRX_ERR)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_TRX_ERR, err);
		if (rc < 0)
...
static int oplus_chg_vr_get_headroom(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_HEADROOM)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_HEADROOM, val);
		if (rc < 0)
...
static int oplus_chg_vr_set_headroom(struct oplus_chg_ic_dev *ic_dev, int val)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_HEADROOM)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_HEADROOM, val);
		if (rc < 0)
...
static int oplus_chg_vr_send_match_q(struct oplus_chg_ic_dev *ic_dev, u8 data)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SEND_MATCH_Q)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SEND_MATCH_Q, data);
		if (rc < 0)
...
static int oplus_chg_vr_set_fod_parm(struct oplus_chg_ic_dev *ic_dev, u8 data[], int len)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SET_FOD_PARM)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SET_FOD_PARM, data, len);
		if (rc < 0)
...
static int oplus_chg_vr_send_msg(struct oplus_chg_ic_dev *ic_dev, unsigned char msg[], int len)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_SEND_MSG)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_SEND_MSG, msg, len);
		if (rc < 0)
...
static int oplus_chg_vr_register_msg_callback(struct oplus_chg_ic_dev *ic_dev, void *data,
					      void (*call_back)(void *, u8[]))
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_REG_MSG_CALLBACK)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_REG_MSG_CALLBACK, data, call_back);
		if (rc < 0)
...
static int oplus_chg_vr_get_rx_version(struct oplus_chg_ic_dev *ic_dev, u32 *version)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_RX_VERSION)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_RX_VERSION, version);
		if (rc < 0)
...
static int oplus_chg_vr_get_tx_version(struct oplus_chg_ic_dev *ic_dev, u32 *version)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_TRX_VERSION)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_TRX_VERSION, version);
		if (rc < 0)
...
static int oplus_chg_vr_upgrade_firmware_by_buf(struct oplus_chg_ic_dev *ic_dev, unsigned char *fw_buf, int fw_size)
{
	struct oplus_virtual_rx_ic *vr;
	int i;
	int rc = 0;
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_UPGRADE_FW_BY_BUF)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_UPGRADE_FW_BY_BUF, fw_buf, fw_size);
		if (rc < 0)
...
static int oplus_chg_vr_upgrade_firmware_by_img(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_UPGRADE_FW_BY_IMG)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
...
static int oplus_chg_vr_connect_check(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_CHECK_CONNECT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_CHECK_CONNECT);
		if (rc < 0)
...
static int oplus_chg_vr_get_event_code(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_wls_event_code *code)
{
	...
	vr = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vr->child_num; i++) {
		if (!func_is_support(&vr->child_list[i], OPLUS_IC_FUNC_RX_GET_EVENT_CODE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vr->child_list[i].ic_dev, OPLUS_IC_FUNC_RX_GET_EVENT_CODE, code);
		if (rc < 0)
...
static void *oplus_chg_vr_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	switch (func_id) {
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_vr_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_vr_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_chg_vr_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_chg_vr_smt_test);
		break;
	case OPLUS_IC_FUNC_RX_SET_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_ENABLE, oplus_chg_vr_set_rx_enable);
		break;
	case OPLUS_IC_FUNC_RX_IS_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_IS_ENABLE, oplus_chg_vr_rx_is_enable);
		break;
	case OPLUS_IC_FUNC_RX_IS_CONNECTED:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_IS_CONNECTED, oplus_chg_vr_rx_is_connected);
		break;
	case OPLUS_IC_FUNC_RX_GET_VOUT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_VOUT, oplus_chg_vr_get_vout);
		break;
	case OPLUS_IC_FUNC_RX_SET_VOUT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_VOUT, oplus_chg_vr_set_vout);
		break;
	case OPLUS_IC_FUNC_RX_GET_VRECT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_VRECT, oplus_chg_vr_get_vrect);
		break;
	case OPLUS_IC_FUNC_RX_GET_IOUT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_IOUT, oplus_chg_vr_get_iout);
		break;
	case OPLUS_IC_FUNC_RX_GET_TRX_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_TRX_VOL, oplus_chg_vr_get_tx_vout);
		break;
	case OPLUS_IC_FUNC_RX_GET_TRX_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_TRX_CURR, oplus_chg_vr_get_tx_iout);
		break;
	case OPLUS_IC_FUNC_RX_GET_CEP_COUNT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_CEP_COUNT, oplus_chg_vr_get_cep_count);
		break;
	case OPLUS_IC_FUNC_RX_GET_CEP_VAL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_CEP_VAL, oplus_chg_vr_get_cep_val);
		break;
	case OPLUS_IC_FUNC_RX_GET_WORK_FREQ:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_WORK_FREQ, oplus_chg_vr_get_work_freq);
		break;
	case OPLUS_IC_FUNC_RX_GET_RX_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_RX_MODE, oplus_chg_vr_get_rx_mode);
		break;
	case OPLUS_IC_FUNC_RX_SET_RX_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_RX_MODE, oplus_chg_vr_set_rx_mode);
		break;
	case OPLUS_IC_FUNC_RX_SET_DCDC_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_DCDC_ENABLE, oplus_chg_vr_set_dcdc_enable);
		break;
	case OPLUS_IC_FUNC_RX_SET_TRX_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_TRX_ENABLE, oplus_chg_vr_set_tx_enable);
		break;
	case OPLUS_IC_FUNC_RX_SET_TRX_START:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_TRX_START, oplus_chg_vr_set_tx_start);
		break;
	case OPLUS_IC_FUNC_RX_GET_TRX_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_TRX_STATUS, oplus_chg_vr_get_tx_status);
		break;
	case OPLUS_IC_FUNC_RX_GET_TRX_ERR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_TRX_ERR, oplus_chg_vr_get_tx_err);
		break;
	case OPLUS_IC_FUNC_RX_GET_HEADROOM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_HEADROOM, oplus_chg_vr_get_headroom);
		break;
	case OPLUS_IC_FUNC_RX_SET_HEADROOM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_HEADROOM, oplus_chg_vr_set_headroom);
		break;
	case OPLUS_IC_FUNC_RX_SEND_MATCH_Q:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SEND_MATCH_Q, oplus_chg_vr_send_match_q);
		break;
	case OPLUS_IC_FUNC_RX_SET_FOD_PARM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SET_FOD_PARM, oplus_chg_vr_set_fod_parm);
		break;
	case OPLUS_IC_FUNC_RX_SEND_MSG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_SEND_MSG, oplus_chg_vr_send_msg);
		break;
	case OPLUS_IC_FUNC_RX_REG_MSG_CALLBACK:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_REG_MSG_CALLBACK, oplus_chg_vr_register_msg_callback);
		break;
	case OPLUS_IC_FUNC_RX_GET_RX_VERSION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_RX_VERSION, oplus_chg_vr_get_rx_version);
		break;
	case OPLUS_IC_FUNC_RX_GET_TRX_VERSION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_TRX_VERSION, oplus_chg_vr_get_tx_version);
		break;
	...
	case OPLUS_IC_FUNC_RX_CHECK_CONNECT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_CHECK_CONNECT, oplus_chg_vr_connect_check);
		break;
	case OPLUS_IC_FUNC_RX_GET_EVENT_CODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_RX_GET_EVENT_CODE, oplus_chg_vr_get_event_code);
		break;
	default:
...
static struct oplus_chg_ic_virq oplus_chg_vr_virq_table[] = {
	{ .virq_id = OPLUS_IC_VIRQ_ERR },	    { .virq_id = OPLUS_IC_VIRQ_PRESENT },
	{ .virq_id = OPLUS_IC_VIRQ_ONLINE },	    { .virq_id = OPLUS_IC_VIRQ_OFFLINE },
...
static int oplus_chg_vr_virq_register(struct oplus_virtual_rx_ic *chip)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_PRESENT)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_PRESENT,
		...
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_EVENT_CHANGED)) {
			rc = oplus_chg_ic_virq_register(chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_EVENT_CHANGED,
