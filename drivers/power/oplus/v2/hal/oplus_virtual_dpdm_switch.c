...
static int oplus_dpdm_switch_set_switch_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_dpdm_switch_mode mode)
{
...
static int oplus_dpdm_switch_get_switch_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_dpdm_switch_mode *mode)
{
	struct oplus_dpdm_switch_ic *chip;
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	struct oplus_chg_ic_overwrite_data *data;
...
static void *oplus_dpdm_switch_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_DPDM_SWITCH_MODE, oplus_dpdm_switch_set_switch_mode);
		break;
	case OPLUS_IC_FUNC_GET_DPDM_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_DPDM_SWITCH_MODE, oplus_dpdm_switch_get_switch_mode);
		break;
	default:
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
static int oplus_dpdm_switch_set_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id,
...
static ssize_t oplus_dpdm_switch_get_func_data(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id,
					       void *buf)
{
	int *item_data;
	ssize_t rc = 0;
...
static int oplus_dpdm_switch_hw_init(struct oplus_dpdm_switch_ic *chip)
{
	...
	if (!chip->use_audio_switch) {
		...
		chip->switch_ctrl_gpio = of_get_named_gpio(node, "oplus,switch_ctrl-gpio", 0);
		...
		chip->gpio_switch_ctrl_ap = pinctrl_lookup_state(chip->pinctrl, "switch_ctrl_ap");
		if (IS_ERR_OR_NULL(chip->gpio_switch_ctrl_ap)) {
			chg_err("get switch_ctrl_ap fail\n");
			rc = -EINVAL;
			goto free_switch_ctrl_gpio;
		}
		chip->gpio_switch_ctrl_vooc = pinctrl_lookup_state(chip->pinctrl, "switch_ctrl_vooc");
		if (IS_ERR_OR_NULL(chip->gpio_switch_ctrl_vooc)) {
			chg_err("get switch_ctrl_vooc fail\n");
			rc = -EINVAL;
			goto free_switch_ctrl_gpio;
		}
		chip->gpio_switch_ctrl_ufcs = pinctrl_lookup_state(chip->pinctrl, "switch_ctrl_ufcs");
	...
	chip->dpdm_switch1_gpio = of_get_named_gpio(node, "oplus,dpdm_switch1-gpio", 0);
	...
	chip->dpdm_switch2_gpio = of_get_named_gpio(node, "oplus,dpdm_switch2-gpio", 0);
	if (!gpio_is_valid(chip->dpdm_switch2_gpio)) {
		chg_info("Couldn't read oplus,dpdm_switch2-gpio, %d\n", chip->dpdm_switch2_gpio);
	} else {
		rc = gpio_request(chip->dpdm_switch2_gpio, "dpdm_switch2-gpio");
		if (rc != 0) {
	...
	chip->gpio_dpdm_switch_ap = pinctrl_lookup_state(chip->pinctrl, "dpdm_switch_ap");
	if (IS_ERR_OR_NULL(chip->gpio_dpdm_switch_ap)) {
		chg_err("get dpdm_switch_ap fail\n");
		rc = -EINVAL;
		goto free_dpdm_switch2_gpio;
	}
	chip->gpio_dpdm_switch_vooc = pinctrl_lookup_state(chip->pinctrl, "dpdm_switch_vooc");
	if (IS_ERR_OR_NULL(chip->gpio_dpdm_switch_vooc)) {
		chg_err("get dpdm_switch_vooc fail\n");
		rc = -EINVAL;
		goto free_dpdm_switch2_gpio;
	}
	chip->gpio_dpdm_switch_ufcs = pinctrl_lookup_state(chip->pinctrl, "dpdm_switch_ufcs");
...
static int oplus_virtual_dpdm_switch_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_dpdm_switch_ic), GFP_KERNEL);
	if (chip == NULL) {
	...
	oplus_chg_ic_func_table_sort(oplus_dpdm_switch_overwrite_funcs, ARRAY_SIZE(oplus_dpdm_switch_overwrite_funcs));
