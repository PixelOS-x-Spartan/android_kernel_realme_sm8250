...
static inline bool func_is_support(struct oplus_virtual_ufcs_child *ic, enum oplus_chg_ic_func func_id)
{
	switch (func_id) {
...
static inline bool virq_is_support(struct oplus_virtual_ufcs_child *ic, enum oplus_chg_ic_virq_id virq_id)
{
	switch (virq_id) {
...
{
	struct oplus_virtual_ufcs_child *child = container_of(work, struct oplus_virtual_ufcs_child, online_work);
...
{
	struct oplus_virtual_ufcs_child *child = container_of(work, struct oplus_virtual_ufcs_child, offline_work);
	struct oplus_virtual_ufcs_ic *chip;
...
static int oplus_chg_vpu_child_funcs_init(struct oplus_virtual_ufcs_ic *chip, int child_num)
{
...
static int oplus_chg_vpu_child_virqs_init(struct oplus_virtual_ufcs_ic *chip, int child_num)
{
	struct device_node *node = chip->dev->of_node;
...
static int oplus_chg_vpu_child_init(struct oplus_virtual_ufcs_ic *chip)
{
	...
	chip->child_num = rc;
	chip->child_list =
		devm_kzalloc(chip->dev, sizeof(struct oplus_virtual_ufcs_child) * chip->child_num, GFP_KERNEL);
...
static int oplus_chg_vpu_exit(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < vpu->child_num; i++) {
		if (virq_is_support(&vpu->child_list[i], OPLUS_IC_VIRQ_ERR)) {
			oplus_chg_ic_virq_release(vpu->child_list[i].ic_dev, OPLUS_IC_VIRQ_ERR, vpu);
		}
		if (virq_is_support(&vpu->child_list[i], OPLUS_IC_VIRQ_ONLINE)) {
			oplus_chg_ic_virq_release(vpu->child_list[i].ic_dev, OPLUS_IC_VIRQ_ONLINE, vpu);
		}
		if (virq_is_support(&vpu->child_list[i], OPLUS_IC_VIRQ_OFFLINE)) {
			oplus_chg_ic_virq_release(vpu->child_list[i].ic_dev, OPLUS_IC_VIRQ_OFFLINE, vpu);
		}
	}
	for (i = 0; i < vpu->child_num; i++) {
...
static int oplus_chg_vpu_handshake(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_HANDSHAKE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_HANDSHAKE);
		if (rc < 0)
...
static int oplus_chg_vpu_pdo_set(struct oplus_chg_ic_dev *ic_dev, int vol_mv, int curr_ma)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_PDO_SET)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_PDO_SET, vol_mv, curr_ma);
		if (rc < 0)
...
static int oplus_chg_vpu_hard_reset(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_HARD_RESET)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_HARD_RESET);
		if (rc < 0)
...
static int oplus_chg_vpu_exit_ufcs_mode(struct oplus_chg_ic_dev *ic_dev)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_EXIT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_EXIT);
		if (rc < 0)
...
static int oplus_chg_vpu_config_wd(struct oplus_chg_ic_dev *ic_dev, u16 time_ms)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_CONFIG_WD)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_CONFIG_WD, time_ms);
		if (rc < 0)
...
static int oplus_chg_vpu_get_dev_info(struct oplus_chg_ic_dev *ic_dev, u64 *dev_info)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_DEV_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_DEV_INFO, dev_info);
		if (rc < 0)
...
static int oplus_chg_vpu_get_err_info(struct oplus_chg_ic_dev *ic_dev, u64 *err_info)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_ERR_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_ERR_INFO, err_info);
		if (rc < 0)
...
static int oplus_chg_vpu_get_src_info(struct oplus_chg_ic_dev *ic_dev, u64 *src_info)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_SRC_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_SRC_INFO, src_info);
		if (rc < 0)
...
static int oplus_chg_vpu_get_cable_info(struct oplus_chg_ic_dev *ic_dev, u64 *cable_info)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_CABLE_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_CABLE_INFO, cable_info);
		if (rc < 0)
...
static int oplus_chg_vpu_get_pdo_info(struct oplus_chg_ic_dev *ic_dev, u64 *pdo, int num)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_PDO_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_PDO_INFO, pdo, num);
		if (rc < 0)
...
static int oplus_chg_vpu_verify_adapter(struct oplus_chg_ic_dev *ic_dev, u8 key_index, u8 *auth_data, u8 data_len)
{
	struct oplus_virtual_ufcs_ic *vpu;
	int rc = 0;
	int i;
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER, key_index,
				       auth_data, data_len);
		if (rc < 0)
...
static int oplus_chg_vpu_get_power_change_info(struct oplus_chg_ic_dev *ic_dev, u32 *pwr_change_info, int num)
{
	struct oplus_virtual_ufcs_ic *vpu;
	int rc = 0;
	int i;
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_POWER_CHANGE_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_POWER_CHANGE_INFO,
				       pwr_change_info, num);
		if (rc < 0)
...
static int oplus_chg_vpu_get_emark_info(struct oplus_chg_ic_dev *ic_dev, u64 *info)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO, info);
		if (rc < 0)
...
static int oplus_chg_vpu_get_power_info_ext(struct oplus_chg_ic_dev *ic_dev, u64 *info, int num)
{
	struct oplus_virtual_ufcs_ic *vpu;
	int rc = 0;
	int i;
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_GET_POWER_INFO_EXT)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_GET_POWER_INFO_EXT, info, num);
		if (rc < 0)
...
static int oplus_chg_vpu_is_test_mode(struct oplus_chg_ic_dev *ic_dev, bool *en)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_IS_TEST_MODE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_IS_TEST_MODE, en);
		if (rc < 0)
...
static int oplus_chg_vpu_is_vol_acc_test_mode(struct oplus_chg_ic_dev *ic_dev, bool *en)
{
	...
	vpu = oplus_chg_ic_get_drvdata(ic_dev);
	for (i = 0; i < vpu->child_num; i++) {
		if (!func_is_support(&vpu->child_list[i], OPLUS_IC_FUNC_UFCS_IS_VOL_ACC_TEST_MODE)) {
			rc = (rc == 0) ? -ENOTSUPP : rc;
			continue;
		}
		rc = oplus_chg_ic_func(vpu->child_list[i].ic_dev, OPLUS_IC_FUNC_UFCS_IS_VOL_ACC_TEST_MODE, en);
		if (rc < 0)
...
static void *oplus_chg_vpu_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	switch (func_id) {
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_vpu_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_vpu_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_chg_vpu_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_chg_vpu_smt_test);
		break;
	case OPLUS_IC_FUNC_UFCS_HANDSHAKE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_HANDSHAKE, oplus_chg_vpu_handshake);
		break;
	case OPLUS_IC_FUNC_UFCS_PDO_SET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_PDO_SET, oplus_chg_vpu_pdo_set);
		break;
	case OPLUS_IC_FUNC_UFCS_HARD_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_HARD_RESET, oplus_chg_vpu_hard_reset);
		break;
	case OPLUS_IC_FUNC_UFCS_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_EXIT, oplus_chg_vpu_exit_ufcs_mode);
		break;
	case OPLUS_IC_FUNC_UFCS_CONFIG_WD:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_CONFIG_WD, oplus_chg_vpu_config_wd);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_DEV_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_DEV_INFO, oplus_chg_vpu_get_dev_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_ERR_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_ERR_INFO, oplus_chg_vpu_get_err_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_SRC_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_SRC_INFO, oplus_chg_vpu_get_src_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_CABLE_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_CABLE_INFO, oplus_chg_vpu_get_cable_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_PDO_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_PDO_INFO, oplus_chg_vpu_get_pdo_info);
		break;
	case OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER, oplus_chg_vpu_verify_adapter);
		break;
	...
	case OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO, oplus_chg_vpu_get_emark_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_POWER_INFO_EXT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_POWER_INFO_EXT, oplus_chg_vpu_get_power_info_ext);
		break;
	case OPLUS_IC_FUNC_UFCS_IS_TEST_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_IS_TEST_MODE, oplus_chg_vpu_is_test_mode);
		break;
...
static int oplus_virtual_platufcs_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_virtual_ufcs_ic), GFP_KERNEL);
	if (chip == NULL) {
