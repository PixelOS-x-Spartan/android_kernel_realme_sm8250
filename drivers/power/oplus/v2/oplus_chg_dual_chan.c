...
static int oplus_dual_chan_get_ibat_check_gear_from_dt(struct oplus_dual_chan_chip *chip, struct device_node *node)
{
	int length = 0;
	int rc;
	int i;
	...
	rc = of_property_read_u32_array(node, "oplus,dual_chan_buck_ichg_check", (u32 *)&chip->buck_ibus_check[0],
					length);
	if (rc < 0) {
...
static int dual_chan_get_ibus_and_ibat_from_dt(struct oplus_dual_chan_chip *chip, struct device_node *node)
{
	...
	rc = of_property_read_u32_array(node, "oplus,dual_chan_buck_ibus", (u32 *)&chip->buck_ibus_data[0], length);
	if (rc < 0) {
	...
	rc = of_property_read_u32_array(node, "oplus,dual_chan_buck_ibat", (u32 *)&chip->buck_ibat_data[0], length);
	if (rc < 0) {
...
static int oplus_dual_chan_parse_dt(struct oplus_dual_chan_chip *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,dual_chan_buck_ucp_ibat_thre", &chip->buck_ucp_thre);
	if (rc) {
		chip->buck_ucp_thre = 0;
	}
	chg_info("oplus dual chan buck_ucp_thre is %d\n", chip->buck_ucp_thre);

	rc = of_property_read_u32(node, "oplus,dual_chan_open_buck_thre", &chip->open_buck_thre);
	if (rc) {
		chip->open_buck_thre = 0;
	}
	chg_info("oplus dual chan open_buck_thre is %d\n", chip->open_buck_thre);

	rc = of_property_read_u32(node, "oplus,dual_chan_close_buck_thre", &chip->close_buck_thre);
	if (rc) {
	...
	for (i = 0; i < chip->buck_ibus_check_count; i++) {
		if (curr > chip->buck_ibus_check[i]) {
			chg_info("get i = %d buck ibus = %d buck ibus count=%d curr level =%d\n", i,
				 chip->buck_ibus_check[i], chip->buck_curr_change_count[i], chip->curr_level);
			if (chip->buck_curr_change_count[i] == OPLUS_DUAL_CHAN_BUCK_COUNT &&
			    chip->curr_level != i + 1) {
				chip->curr_level = i + 1;
				memset(chip->buck_curr_change_count, 0, OPLUS_DUAL_CHAN_CHECK_MAX);
			} else {
...
static bool oplus_get_vooc_normal_temp_status(struct oplus_dual_chan_chip *chip)
{
	...
	if (data.intval == FASTCHG_TEMP_RANGE_NORMAL_HIGH || data.intval == FASTCHG_TEMP_RANGE_NORMAL_LOW)
		return true;
	else
		return false;
}
...
static int oplus_dual_chan_set_buck_status(struct oplus_dual_chan_chip *chip, bool enable)
{
	...
	if (enable) {
		if (is_voocphy_ic_available(chip)) {
			rc = oplus_chg_ic_func(chip->voocphy_ic, OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME, UCP_UP);
			if (rc < 0) {
		...
		if (is_voocphy_ic_available(chip)) {
			rc = oplus_chg_ic_func(chip->voocphy_ic, OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME, UCP_DOWN);
			if (rc < 0) {
				chg_err("can't set ucp, rc=%d\n", rc);
				mutex_unlock(&chip->dual_chan_chg_mutex);
				return rc;
	...
	msg = oplus_mms_alloc_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, SWITCH_ITEM_DUAL_CHAN_STATUS);
	if (msg == NULL) {
...
static struct mms_item oplus_chg_dual_chan_item[] = { { .desc = {
...
static int dual_chan_track_debugfs_init(struct oplus_dual_chan_chip *chip)
{
	...
	debugfs_create_u32("debug_dual_chan_err", 0644, debugfs_dual_chan, &(chip->debug_dual_chan_err));
...
static void oplus_chg_dual_chan_init_work(struct work_struct *work)
{
	...
	struct oplus_dual_chan_chip *chip = container_of(dwork, struct oplus_dual_chan_chip, dual_chan_init_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_dual_chan_chip *chip = container_of(dwork, struct oplus_dual_chan_chip, dual_chan_check_work);
	union mms_msg_data data = { 0 };
	int vbat_temp_cur;
	...
	if (chip->enable_normal_chg && !oplus_wired_input_is_enable() && is_chg_suspend_votable_available(chip) &&
	...
	chg_info(
		"led_status = %d, vbat_status = %d!, curr = %d, open_buck_thre = %d, temp_status = %d, dual_chan_status = %d\n",
		oplus_get_led_status_only_for_dual_chan(chip), oplus_dual_chan_get_vbat_status(chip), vbat_temp_cur,
		chip->open_buck_thre, oplus_get_vooc_normal_temp_status(chip), oplus_get_dual_chan_status(chip));
	if (vbat_temp_cur > chip->open_buck_thre && !oplus_get_led_status_only_for_dual_chan(chip) &&
	    oplus_dual_chan_get_vbat_status(chip) && oplus_get_vooc_normal_temp_status(chip) &&
	...
	} else if ((vbat_temp_cur < chip->close_buck_thre || oplus_get_led_status_only_for_dual_chan(chip) ||
		    !oplus_dual_chan_get_vbat_status(chip) || !oplus_get_vooc_normal_temp_status(chip)) &&
		   oplus_get_dual_chan_status(chip)) {
	...
	chg_info("open count = %d close count = %d! enable_normal_chg = %d\n", chip->open_buck_count,
		 chip->close_buck_count, chip->enable_normal_chg);
	...
	if (chip->last_enable_normal_chg && !chip->enable_normal_chg && oplus_wired_input_is_enable() &&
	    is_chg_suspend_votable_available(chip) && get_effective_result(chip->chg_suspend_votable) &&
	    !chip->err_report_enter) {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_dual_chan_chip *chip = container_of(dwork, struct oplus_dual_chan_chip, dual_chan_reset_work);
...
static int oplus_dual_chan_get_track_info(struct oplus_dual_chan_chip *chip, char *temp_str, int index)
{
	union mms_msg_data data = { 0 };
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_dual_chan_chip *chip = container_of(dwork, struct oplus_dual_chan_chip, dual_chan_track_work);
	...
	if (chip->track_err_type && upload_count < TRACK_UPLOAD_COUNT_MAX) {
		...
		case DUAL_CHAN_EXIT_ERR:
			index += snprintf(&(temp_str[index]), REASON_LENGTH_MAX - index, "$$reason@@%s$$status@@",
					  "dual_chan_exit_err");
			break;
		...
		index += snprintf(&(temp_str[index]), REASON_LENGTH_MAX - index,
				  ...
				  bat_curr, oplus_get_led_status_only_for_dual_chan(chip),
				  oplus_dual_chan_get_vbat_status(chip), oplus_get_vooc_normal_temp_status(chip),
				  oplus_wired_input_is_enable());
		index += oplus_dual_chan_get_track_info(chip, temp_str, index);
		msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_MEDIUM, ERR_ITEM_DUAL_CHAN, temp_str);
		if (msg == NULL) {
...
static void oplus_dual_chan_common_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_dual_chan_subscribe_common_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->common_subs =
		oplus_mms_subscribe(chip->common_topic, chip, oplus_dual_chan_common_subs_callback, "dual_chan");
...
static void oplus_dual_chan_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_dual_chan_chip *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_dual_chan_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_dual_chan_chip *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_dual_chan_vooc_subs_callback, "dual_chan");
...
static int oplus_dual_chan_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_dual_chan_chip), GFP_KERNEL);
	if (chip == NULL) {
