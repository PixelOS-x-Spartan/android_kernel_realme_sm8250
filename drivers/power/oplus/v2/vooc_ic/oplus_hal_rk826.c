...
static bool rk826_fw_update_check(struct rk826_chip *chip, const u8 *fw_buf, u32 size)
{
	int ret = 0;
	u8 data_buf[4] = { 0 };
...
static int rk826_fw_write_00_code(struct rk826_chip *chip, const u8 *fw_buf, u8 fw_size)
{
	int ret = 0;
	int try_count = 3;
	struct_req req = { 0 };
...
static int rk826_fw_write_ff_code(struct rk826_chip *chip, const u8 *fw_buf, u8 fw_size)
{
	int ret = 0;
	int try_count = 3;
	struct_req req = { 0 };
...
static int rk826_fw_update(struct rk826_chip *chip, const u8 *fw_buf, u32 fw_size)
{
	int ret = 0;
	int try_count = 3;
	struct_req req = { 0 };
...
static int rk826_get_fw_verion_from_ic(struct rk826_chip *chip)
{
	...
	if (oplus_is_power_off_charging() || oplus_is_charger_reboot()) {
		chip->upgrading = true;
		update_result = rk826_fw_update(chip, chip->firmware_data, chip->fw_data_count);
...
static int __rk826_fw_check_then_recover(struct rk826_chip *chip)
{
	...
	if (oplus_is_power_off_charging() == true || oplus_is_charger_reboot() == true) {
		chip->upgrading = true;
	...
	update_asic_fw:
		...
		if (rk826_fw_update_check(chip, chip->firmware_data, chip->fw_data_count) == FW_CHECK_FAIL ||
		    fw_check_err) {
			chg_info("firmware update start\n");
			do {
				update_result = rk826_fw_write_00_code(chip, chip->firmware_data, chip->fw_data_count);
				update_result = rk826_fw_write_ff_code(chip, chip->firmware_data, chip->fw_data_count);
				update_result = rk826_fw_write_00_code(chip, chip->firmware_data, chip->fw_data_count);
				update_result = rk826_fw_update(chip, chip->firmware_data, chip->fw_data_count);
				update_result = rk826_fw_update(chip, chip->firmware_data, chip->fw_data_count);
				if (!update_result)
		...
		if (!update_result) {
			...
			if (rc < 0) {
				chg_info("rk826 read register 0x52f8 fail, rc = %d\n", rc);
				chg_info("rk826 fw upgrade check ok.");
			} else {
				chg_err("read 0x52f8 success 0x%x,0x%x", value_buf[0], value_buf[1]);
				fw_check_err++;
				if (fw_check_err > 3)
					goto update_fw_err;
				msleep(1000);
	...
	update_asic_fw:
		...
		if (!update_result) {
			...
			if ((value_buf[0] == 0x45) && (value_buf[1] == 0x4c) && (value_buf[2] == 0x44) &&
			    (value_buf[3] == 0x49)) {
				chg_info("read 0x52cc success 0x%x,0x%x,0x%x,0x%x", value_buf[0], value_buf[1],
...
static int __rk826_user_fw_upgrade(struct rk826_chip *chip, const u8 *fw_buf, u32 fw_size)
{
	int update_result = 0;
	int try_count = 5;
...
int rk826_asic_fw_status(struct rk826_chip *chip)
{
	...
	if ((value_buf[0] == 0x45) && (value_buf[1] == 0x4c) && (value_buf[2] == 0x44) && (value_buf[3] == 0x49)) {
		chg_info("read 0x52cc success 0x%x,0x%x,0x%x,0x%x", value_buf[0], value_buf[1], value_buf[2],
			 value_buf[3]);
		return 0;
	} else {
		chg_err("rk826 read register 0x52cc fail, rc = %d\n", rc);
...
static int rk826_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	gpDMABuf_va = (u8 *)dma_alloc_coherent(chip->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, &gpDMABuf_pa, GFP_KERNEL);
	if (!gpDMABuf_va) {
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
#if GTP_SUPPORT_I2C_DMA
	dma_free_coherent(chip->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, gpDMABuf_va, gpDMABuf_pa);
#endif
#endif
...
static int rk826_user_fw_upgrade(struct oplus_chg_ic_dev *ic_dev, const u8 *fw_buf, u32 fw_size)
{
	struct rk826_chip *chip;
	int rc = 0;
...
static int rk826_get_upgrade_status(struct oplus_chg_ic_dev *ic_dev, bool *upgrading)
{
	struct rk826_chip *chip;
...
static void *rk826_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, rk826_smt_test);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_UPGRADE, rk826_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE, rk826_user_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER, rk826_fw_check_then_recover);
		break;
	...
	case OPLUS_IC_FUNC_VOOC_GET_FW_VERSION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_FW_VERSION, rk826_get_fw_version);
		break;
	case OPLUS_IC_FUNC_VOOC_UPGRADING:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_UPGRADING, rk826_get_upgrade_status);
		break;
	case OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS, rk826_check_fw_status);
		break;
	case OPLUS_IC_FUNC_VOOC_BOOT_BY_GPIO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_BOOT_BY_GPIO, rk826_boot_by_gpio);
		break;
	default:
...
static int rk826_parse_fw_data_by_dts(struct rk826_chip *chip)
{
	...
	chg_info("version: 0x%x, count: %d\n", chip->fw_data_version, chip->fw_data_count);
...
static int rk826_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct rk826_chip), GFP_KERNEL);
	if (!chip) {
	...
	chip->vooc_fw_update_newmethod = of_property_read_bool(node, "oplus,vooc_fw_update_newmethod");
	if (!chip->vooc_fw_update_newmethod) {
