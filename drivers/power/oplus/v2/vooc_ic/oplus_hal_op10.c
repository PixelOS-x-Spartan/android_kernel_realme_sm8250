...
static bool op10_fw_update_check(struct op10_chip *chip, const u8 *fw_buf, u32 size)
{
	int i = 0;
	int ret = 0;
		...
		*((u32 *)rx_buf) = fw_offset;
		...
		while (write_len_temp) {
			...
			oplus_vooc_i2c_write(chip->client, CMD_XFER_W_DAT, chunk_len,
					     fw_buf + fw_offset + chunk_index * I2C_RW_LEN_MAX);
			msleep(1);
...
static int op10_get_fw_verion_from_ic(struct op10_chip *chip)
{
	...
	if (oplus_is_power_off_charging() || oplus_is_charger_reboot()) {
		chip->upgrading = true;
		update_result = op10_fw_update(chip, chip->firmware_data, chip->fw_data_count);
...
static int __op10_fw_check_then_recover(struct op10_chip *chip)
{
	...
	if (oplus_is_power_off_charging() || oplus_is_charger_reboot()) {
		...
		if (!op10_fw_update_check(chip, chip->firmware_data, chip->fw_data_count)) {
...
static int op10_user_fw_upgrade(struct oplus_chg_ic_dev *ic_dev, const u8 *fw_buf, u32 fw_size)
{
	return -ENOTSUPP;
}
...
static int op10_get_upgrade_status(struct oplus_chg_ic_dev *ic_dev, bool *upgrading)
{
	struct op10_chip *chip;
...
static void *op10_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, op10_smt_test);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_UPGRADE, op10_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_USER_FW_UPGRADE, op10_user_fw_upgrade);
		break;
	case OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_FW_CHECK_THEN_RECOVER, op10_fw_check_then_recover);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_FW_VERSION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_FW_VERSION, op10_get_fw_version);
		break;
	case OPLUS_IC_FUNC_VOOC_UPGRADING:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_UPGRADING, op10_get_upgrade_status);
		break;
	case OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS, op10_check_fw_status);
		break;
	case OPLUS_IC_FUNC_VOOC_BOOT_BY_GPIO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_BOOT_BY_GPIO, op10_boot_by_gpio);
		break;
	default:
