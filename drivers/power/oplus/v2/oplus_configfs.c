...
__maybe_unused static bool is_chg_disable_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_chg_suspend_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_cool_down_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_vooc_curr_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_ufcs_curr_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_pps_curr_votable_available(struct oplus_configfs_device *chip)
{
...
__maybe_unused static bool is_wls_curr_votable_available(struct oplus_configfs_device *chip)
{
	...
	if (chip->vooc_sid == 0) {
		...
		case OPLUS_CHG_USB_TYPE_PD_DRP:
			if (chip->pd_boost_disable_votable &&
				...
				pd_use_default = true;
			if (chip->pd_svooc_votable && !!get_effective_result(chip->pd_svooc_votable))
				pd_use_default = true;
...
static ssize_t usbtemp_volt_l_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_USB_TEMP_VOLT_L, &data, true);
...
static ssize_t usbtemp_volt_r_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	oplus_mms_get_item_data(chip->wired_topic, WIRED_ITEM_USB_TEMP_VOLT_R, &data, true);
...
static struct device_attribute *oplus_usb_attributes[] = { &dev_attr_otg_online,     &dev_attr_otg_switch,
							   &dev_attr_usb_status,     &dev_attr_typec_cc_orientation,
							   &dev_attr_fast_chg_type,  &dev_attr_usbtemp_volt_l,
							   &dev_attr_usbtemp_volt_r, NULL };
...
static ssize_t gauge_vbat_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_configfs_device *chip = dev->driver_data;
...
static ssize_t cool_down_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (chip->comm_topic)
		rc = oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_COOL_DOWN, &data, false);
	else
		rc = -ENOTSUPP;
	if (rc < 0) {
...
static ssize_t cool_down_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_cool_down_votable_available(chip))
		rc = vote(chip->cool_down_votable, USER_VOTER, val > 0 ? true : false, val, false);
	else
		rc = -ENOTSUPP;
	if (rc < 0) {
					 ...
					 size_t count)
{
	...
	if (is_chg_disable_votable_available(chip))
		rc = vote(chip->chg_disable_votable, MMI_CHG_VOTER, !val ? true : false, !val, false);
	else
		rc = -ENOTSUPP;
	if (rc < 0)
...
static ssize_t bcc_parms_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (chip->vooc_topic) {
		rc = oplus_mms_get_item_data(chip->vooc_topic, VOOC_ITEM_VOOC_STARTED, &data, true);
...
static ssize_t aging_ffc_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	index += sprintf(buf, "%d,%d,%d,%d,%d", version, oplus_gauge_get_batt_num(), 0, chip->debug_batt_cc,
			 chip->batt_cc);
...
static ssize_t parallel_chg_mos_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!mos_status || balancing_bat_status == PARALLEL_BAT_BALANCE_ERROR_STATUS8 ||
...
static ssize_t dual_chan_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int val = 0;
	ssize_t len = 0;
...
#ifdef WLS_QI_DEBUG
ssize_t __attribute__((weak)) oplus_chg_wls_upgrade_fw_show(struct oplus_mms *mms, char *buf)
{
	return 0;
}

ssize_t __attribute__((weak)) oplus_chg_wls_upgrade_fw_store(struct oplus_mms *mms, const char *buf, size_t count)
{
	return 0;
}
...
static struct device_attribute *oplus_wireless_attributes[] = { &dev_attr_tx_voltage_now,
...
static ssize_t deep_dischg_counts_store(struct device *dev, struct device_attribute *attr, const char *buf,
					size_t count)
{
	struct oplus_configfs_device *chip = dev->driver_data;
	int val = 0;
...
static ssize_t deep_dischg_count_cali_store(struct device *dev, struct device_attribute *attr, const char *buf,
					    size_t count)
{
	struct oplus_configfs_device *chip = dev->driver_data;
	int val = 0;
...
static int oplus_usb_dir_create(struct oplus_configfs_device *chip)
{
	...
	chip->oplus_usb_dir = device_create(chip->oplus_chg_class, NULL, devt, NULL, "%s", "usb");
	...
	for (attr = *attrs; attr != NULL; attr = *(++attrs)) {
		...
		if (err) {
			...
			device_destroy(chip->oplus_usb_dir->class, chip->oplus_usb_dir->devt);
			return err;
		}
	}
...
static int oplus_battery_dir_create(struct oplus_configfs_device *chip)
{
	...
	chip->oplus_battery_dir = device_create(chip->oplus_chg_class, NULL, devt, NULL, "%s", "battery");
	...
	for (attr = *attrs; attr != NULL; attr = *(++attrs)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(chip->oplus_battery_dir->class, chip->oplus_battery_dir->devt);
			return err;
		}
	}
...
static int oplus_wireless_dir_create(struct oplus_configfs_device *chip)
{
	...
	chip->oplus_wireless_dir = device_create(chip->oplus_chg_class, NULL, devt, NULL, "%s", "wireless");
	...
	for (attr = *attrs; attr != NULL; attr = *(++attrs)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(chip->oplus_wireless_dir->class, chip->oplus_wireless_dir->devt);
			return err;
		}
	}
...
static int oplus_common_dir_create(struct oplus_configfs_device *chip)
{
	...
	chip->oplus_common_dir = device_create(chip->oplus_chg_class, NULL, devt, NULL, "%s", "common");
	...
	for (attr = *attrs; attr != NULL; attr = *(++attrs)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(chip->oplus_common_dir->class, chip->oplus_common_dir->devt);
			return err;
		}
	}
...
{
	struct oplus_configfs_device *chip = container_of(work, struct oplus_configfs_device, gauge_update_work);
	union mms_msg_data data = { 0 };
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_GAUGE_VBAT, &data, false);
	chip->gauge_vbat = data.intval;
}

static void oplus_configfs_gauge_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_configfs_subscribe_gauge_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(chip->gauge_topic, chip, oplus_configfs_gauge_subs_callback, "configfs");
	...
	chip->batt_soh = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_GAUGE_VBAT, &data, true);
	chip->gauge_vbat = data.intval;
...
static void oplus_configfs_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_configfs_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	/* union mms_msg_data data = { 0 }; */
	int rc;
	...
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, oplus_configfs_wired_subs_callback, "configfs");
...
static void oplus_configfs_wls_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_configfs_subscribe_wls_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->wls_subs = oplus_mms_subscribe(chip->wls_topic, chip, oplus_configfs_wls_subs_callback, "configfs");
...
static void oplus_configfs_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_configfs_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->vooc_topic = topic;
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip, oplus_configfs_vooc_subs_callback, "configfs");
...
static void oplus_configfs_comm_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_configfs_subscribe_comm_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip, oplus_configfs_comm_subs_callback, "configfs");
	...
	chip->notify_code = data.intval;
	oplus_mms_get_item_data(chip->comm_topic, COMM_ITEM_VBAT_UV_THR, &data, true);
	chip->vbat_uv_thr = data.intval;
...
static void oplus_configfs_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	int rc;
...
static void oplus_configfs_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->ufcs_subs = oplus_mms_subscribe(chip->ufcs_topic, chip, oplus_configfs_ufcs_subs_callback, "configfs");
...
static void oplus_configfs_pps_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_configfs_device *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
	int rc;
...
static void oplus_configfs_subscribe_pps_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_configfs_device *chip = prv_data;
	union mms_msg_data data = { 0 };
	int rc;
	...
	chip->pps_subs = oplus_mms_subscribe(chip->pps_topic, chip, oplus_configfs_pps_subs_callback, "configfs");
...
static __init int oplus_configfs_init(void)
{
	...
	oplus_mms_wait_topic("gauge", oplus_configfs_subscribe_gauge_topic, chip);
	oplus_mms_wait_topic("wired", oplus_configfs_subscribe_wired_topic, chip);
	oplus_mms_wait_topic("wireless", oplus_configfs_subscribe_wls_topic, chip);
	oplus_mms_wait_topic("vooc", oplus_configfs_subscribe_vooc_topic, chip);
	oplus_mms_wait_topic("common", oplus_configfs_subscribe_comm_topic, chip);
