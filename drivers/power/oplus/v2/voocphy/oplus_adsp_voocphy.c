...
static void oplus_voocphy_check_charger_out_work_func(struct work_struct *work)
{
	struct oplus_voocphy_manager *chip =
		container_of(work, struct oplus_voocphy_manager, voocphy_check_charger_out_work.work);
	int chg_vol = 0;
...
static void oplus_voocphy_check_charger_out(struct oplus_voocphy_manager *chip, unsigned int delay_ms)
{
...
static void oplus_adsp_voocphy_handle_track_status(struct oplus_voocphy_manager *chip, int event)
{
	int track_status;
	...
	case ADSP_VPHY_FAST_NOTIFY_BAD_CONNECTED:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_BAD_CONNECTED);
		break;
	case ADSP_VPHY_FAST_NOTIFY_ERR_COMMU:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_FRAME_H_ERR);
		break;
	case ADSP_VPHY_FAST_NOTIFY_COMMU_CLK_ERR:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_CLK_ERR);
		break;
	case ADSP_VPHY_FAST_NOTIFY_HW_VBATT_HIGH:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_HW_VBATT_HIGH);
		break;
	case ADSP_VPHY_FAST_NOTIFY_HW_TBATT_HIGH:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_HW_TBATT_HIGH);
		break;
	case ADSP_VPHY_FAST_NOTIFY_COMMU_TIME_OUT:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_COMMU_TIME_OUT);
		break;
	case ADSP_VPHY_FAST_NOTIFY_ADAPTER_COPYCAT:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_ADAPTER_COPYCAT);
		break;
	case ADSP_VPHY_FAST_NOTIFY_BTB_TEMP_OVER:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_BTB_TEMP_OVER);
		break;
	case ADSP_VPHY_FAST_NOTIFY_PRESENT:
	case ADSP_VPHY_FAST_NOTIFY_DUMMY_START:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_BREAK_DEFAULT);
		break;
	...
	case ADSP_VPHY_FAST_NOTIFY_FULL:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_FULL);
		break;
	case ADSP_VPHY_FAST_NOTIFY_SWITCH_TEMP_RANGE:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_SWITCH_TEMP_RANGE);
		break;
	case ADSP_VPHY_FAST_NOTIFY_BATT_TEMP_OVER:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_BATT_TEMP_OVER);
		break;
	default:
		oplus_chglib_push_break_code(chip->dev, TRACK_ADSP_VOOCPHY_OTHER);
		break;
	}
}

static void oplus_adsp_voocphy_handle_present_event(struct oplus_voocphy_manager *chip, int event)
{
...
static void oplus_adsp_voocphy_handle_dummy_event(struct oplus_voocphy_manager *chip, int event)
{
	chip->fastchg_start = false;
	chip->fastchg_to_warm = false;
...
static void oplus_adsp_voocphy_handle_copycat_event(struct oplus_voocphy_manager *chip, int event)
{
	chip->fastchg_start = false;
	chip->fastchg_to_warm = false;
	chip->fastchg_dummy_start = false;
...
static void oplus_adsp_voocphy_common_handle(struct oplus_voocphy_manager *chip, int event)
{
...
static void oplus_adsp_voocphy_handle_err_commu_event(struct oplus_voocphy_manager *chip)
{
	...
	chg_info("1+ abnormal adpater icon hold[%d %d %d]\n", oplus_chglib_is_pd_svooc_adapter(chip->dev),
		 oplus_chglib_is_wired_present(chip->dev), oplus_chglib_get_vooc_is_started(chip->dev));
...
void oplus_adsp_voocphy_fastchg_event_handle(int event)
{
	...
	if (((event & 0xFF) != ADSP_VPHY_FAST_NOTIFY_PRESENT) && ((event & 0xFF) != ADSP_VPHY_FAST_NOTIFY_DUMMY_START))
		oplus_adsp_voocphy_handle_track_status(chip, event);
...
static int adsp_voocphy_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_voocphy_manager), GFP_KERNEL);
	if (!chip) {
	...
	INIT_DELAYED_WORK(&chip->voocphy_send_ongoing_notify, oplus_adsp_voocphy_send_ongoing_notify);
	g_voocphy_chip = chip;
