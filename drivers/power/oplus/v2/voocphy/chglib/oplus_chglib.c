...
int oplus_chglib_disable_charger(bool disable)
{
	...
	rc = vote(disable_votable, FASTCHG_VOTER, disable, disable ? 1 : 0, false);
	if (rc < 0)
...
int oplus_chglib_suspend_charger(bool suspend)
{
	...
	rc = vote(suspend_votable, FASTCHG_VOTER, suspend, suspend ? 1 : 0, false);
	if (rc < 0)
...
int oplus_chglib_vooc_fastchg_disable(const char *client_str, bool disable)
{
	...
	rc = vote(vooc_disable_votable, client_str, disable, disable ? 1 : 0, false);
	if (rc < 0)
...
int oplus_chglib_get_soc(struct device *dev)
{
	...
	if (chip->common_topic)
		oplus_mms_get_item_data(chip->common_topic, COMM_ITEM_UI_SOC, &data, true);
	else
...
int oplus_chglib_get_shell_temp(struct device *dev)
{
	...
	if (chip->common_topic)
		oplus_mms_get_item_data(chip->common_topic, COMM_ITEM_SHELL_TEMP, &data, false);
	else
		chg_err("common topic not found\n");
...
bool oplus_chglib_is_switch_temp_range(void)
{
	...
	vooc_disable_votable = find_votable("VOOC_DISABLE");
	if (vooc_disable_votable)
		enabled = is_client_vote_enabled(vooc_disable_votable, SWITCH_RANGE_VOTER);
	else
...
{
	struct vphy_chip *chip = container_of(work, struct vphy_chip, check_charger_out_work);
	union mms_msg_data data = { 0 };
...
{
	struct vphy_chip *chip = container_of(work, struct vphy_chip, err_report_work);
...
static void oplus_chglib_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_chglib_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs =
		oplus_mms_subscribe(chip->wired_topic, chip, oplus_chglib_wired_subs_callback, "voocphy_chglib");
...
{
	struct vphy_chip *chip = container_of(work, struct vphy_chip, send_absent_notify_work);
...
{
	struct vphy_chip *chip = container_of(work, struct vphy_chip, send_adapter_id_work);

	oplus_chglib_send_adapter_id_notify(chip);
}

static void oplus_chglib_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct vphy_chip *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_chglib_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct vphy_chip *chip = prv_data;

	chip->vooc_topic = topic;
	chip->vooc_subs =
		oplus_mms_subscribe(chip->vooc_topic, chip, oplus_chglib_vooc_subs_callback, "voocphy_chglib");
...
{
	struct vphy_chip *chip = container_of(work, struct vphy_chip, set_spec_current_work);

	if (!chip || !chip->dev || !oplus_chglib_get_vooc_is_started(chip->dev))
		return;

	if (chip && chip->vinf && chip->vinf->vphy_set_switch_curr_limit)
		chip->vinf->vphy_set_switch_curr_limit(chip->dev, chip->switching_curr_limit);
}

static void oplus_chglib_common_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct vphy_chip *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_chglib_subscribe_common_topic(struct oplus_mms *topic, void *prv_data)
{
	struct vphy_chip *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->common_subs =
		oplus_mms_subscribe(chip->common_topic, chip, oplus_chglib_common_subs_callback, "voocphy_chglib");
	if (IS_ERR_OR_NULL(chip->common_subs))
		chg_err("subscribe common topic error, rc=%ld\n", PTR_ERR(chip->common_subs));

	oplus_mms_get_item_data(chip->common_topic, COMM_ITEM_LED_ON, &data, true);
	chip->led_on = !!data.intval;
}

static void oplus_chglib_parallel_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct vphy_chip *chip = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_chglib_subscribe_parallel_topic(struct oplus_mms *topic, void *prv_data)
{
	struct vphy_chip *chip = prv_data;
	union mms_msg_data data = { 0 };
	...
	chip->parallel_subs =
		oplus_mms_subscribe(chip->parallel_topic, chip, oplus_chglib_parallel_subs_callback, "voocphy_chglib");
	if (IS_ERR_OR_NULL(chip->vooc_subs)) {
...
static int vphy_reply_data(struct oplus_chg_ic_dev *ic_dev, int data, int data_width, int curr_ma)
{
	struct vphy_chip *chip;
...
static void *vphy_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_VOOC_REPLY_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_REPLY_DATA, vphy_reply_data);
		break;
	case OPLUS_IC_FUNC_VOOC_READ_DATA:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_READ_DATA, vphy_get_data);
		break;
	case OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_SET_SWITCH_MODE, vphy_set_switch_mode);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_PDQC_CONFIG, vphy_set_pdqc_config);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_CP_VBAT, vphy_get_cp_vbat);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_BCC_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_BCC_CURR, vphy_set_bcc_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MAX_CURR, vphy_get_bcc_max_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_MIN_CURR, vphy_get_bcc_min_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_EXIT_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_EXIT_CURR, vphy_get_bcc_exit_curr);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_FASTCHG_ING, vphy_get_get_fastchg_ing);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_BCC_TEMP_RANGE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_BCC_TEMP_RANGE, vphy_get_bcc_temp_range);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_CHG_AUTO_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_CHG_AUTO_MODE, vphy_set_chg_auto_mode);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_CURVE_CURR, vphy_get_curve_current);
		break;
	case OPLUS_IC_FUNC_VOOC_RESET_SLEEP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_RESET_SLEEP, vphy_reset_sleep);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_UCP_TIME, vphy_set_ucp_time);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_AP_FASTCHG_ALLOW:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_SET_AP_FASTCHG_ALLOW, vphy_set_ap_fastchg_allow);
		break;
	case OPLUS_IC_FUNC_VOOC_GET_REAL_CURVE_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOC_GET_REAL_CURVE_CURR, vphy_get_real_curve_current);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_GET_RETRY_FLAG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_GET_RETRY_FLAG, vphy_get_retry_flag);
		break;
	default:
...
static int cp_track_debugfs_init(struct vphy_chip *chip)
{
	...
	chip->debug_cp_err = 0;
	debugfs_create_u32("debug_dual_chan_err", 0644, debugfs_cp, &(chip->debug_cp_err));
...
struct vphy_chip *oplus_chglib_register_vphy(struct device *dev, struct hw_vphy_info *vinf)
{
	...
	chip = devm_kzalloc(dev, sizeof(struct vphy_chip), GFP_KERNEL);
	if (!chip) {
