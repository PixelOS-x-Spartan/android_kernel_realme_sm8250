...
static int charger_boost_min_cpu_freq_set(struct oplus_voocphy_manager *chip, int *freq, unsigned int clear_time)
{
	int i;
...
{
	struct oplus_voocphy_manager *chip = container_of(work, struct oplus_voocphy_manager, clear_boost_work.work);
...
__maybe_unused static void oplus_voocphy_wake_modify_cpufeq_work(struct oplus_voocphy_manager *chip, int flag)
{
...
{
	struct oplus_voocphy_manager *chip = container_of(work, struct oplus_voocphy_manager, modify_cpufeq_work.work);

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
	if (atomic_read(&chip->voocphy_freq_state) == 1) {
		ppm_sys_boost_min_cpu_freq_set(chip->voocphy_freq_mincore, chip->voocphy_freq_midcore,
					       chip->voocphy_freq_maxcore, chip->voocphy_current_change_timeout);
	} else {
	...
	if (atomic_read(&chip->voocphy_freq_state) == 1) {
		charger_boost_min_cpu_freq_set(chip, freq, chip->voocphy_current_change_timeout);
	} else {
		charger_boost_min_cpu_freq_clear(chip);
	}
#endif
...
static void voocphy_cpufreq_init(struct oplus_voocphy_manager *chip)
{
	...
	ppm_sys_boost_min_cpu_freq_set(chip->voocphy_freq_mincore, chip->voocphy_freq_midcore,
				       chip->voocphy_freq_maxcore, 15000);
#else
...
static int oplus_voocphy_monitor_timer_start(struct oplus_voocphy_manager *chip, int timer_id, int duration_in_ms)
{
...
static int oplus_voocphy_monitor_timer_stop(struct oplus_voocphy_manager *chip, int timer_id, int duration_in_ms)
{
	if ((timer_id >= VOOC_THREAD_TIMER_INVALID) || (duration_in_ms < 0))
		return VOOCPHY_EFATAL;
...
static int oplus_voocphy_read_mesg_mask(unsigned char mask, unsigned char data, unsigned char *read_data)
{
...
static int oplus_voocphy_write_mesg_mask(unsigned char mask, unsigned char *pdata, unsigned char write_data)
{
	int status = VOOCPHY_SUCCESS;
...
static int oplus_voocphy_slave_hw_setting(struct oplus_voocphy_manager *chip, int reason)
{
	int rc = 0;
	if (!chip) {
...
static bool oplus_voocphy_set_fastchg_state(struct oplus_voocphy_manager *chip, unsigned char fastchg_state)
{
	if (chip) {
...
chg_exception:
	...
	voocphy_err("voocphydbg data[%d %d %d %d %d], status[%d, %d], init[%d, %d], set[%d, %d]"
		    ...
		    chip->vooc_vbus_status, chip->vbus_vbatt, chip->batt_soc_plugin, chip->batt_temp_plugin,
		    chip->current_expect, chip->current_max, chip->vooc_move_head, chip->voocphy_rx_buff,
		    chip->vooc_head, chip->adapter_type, chip->adapter_mesg, chip->fastchg_adapter_ask_cmd,
		    chip->voocphy_tx_buff[0], chip->voocphy_tx_buff[1], chip->vooc_flag, chip->interrupt_flag,
		    chip->irq_total_num, chip->irq_rcvok_num, chip->irq_rcverr_num, chip->irq_tx_timeout_num,
		    chip->irq_tx_timeout, chip->irq_hw_timeout_num, base_cpufreq_for_chg);
...
static void oplus_voocphy_update_data(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->parallel_charge_support && (chip->parallel_charge_project == PARALLEL_SWITCH_IC) &&
...
static int oplus_voocphy_handle_ask_bat_model_process_cmd(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->copycat_vooc_support && chip->start_vaild_frame &&
	    (chip->adapter_type == ADAPTER_SVOOC || chip->adapter_type == ADAPTER_VOOC30) && chip->adapter_check_ok &&
	    chip->adapter_model_ver != 0 && chip->ask_batt_sys > 0 && chip->ask_batt_sys <= BATT_SYS_ARRAY &&
	    chip->irq_tx_fail_num == 0) {
...
static int oplus_voocphy_set_chg_auto_mode(struct oplus_voocphy_manager *chip, bool enable)
{
	int rc = 0;
...
static int oplus_voocphy_handle_is_vbus_ok_cmd(struct oplus_voocphy_manager *chip)
{
	...
	* Considering the margin and the actual pressure regulation time, the number of times is limited to 80 times.*/
	if (chip->copycat_vooc_support && chip->adapter_type == ADAPTER_SVOOC &&
...
static int oplus_voocphy_handle_ask_current_level_cmd(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->copycat_vooc_support && chip->adapter_type == ADAPTER_SVOOC && chip->start_vaild_frame &&
...
static int oplus_voocphy_handle_get_batt_vol_cmd(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->copycat_vooc_support && chip->ask_batvol_first && chip->adapter_type == ADAPTER_SVOOC &&
...
static int oplus_voocphy_svooc_commu_with_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	if (oplus_voocphy_ap_allow_fastchg(chip)) {
		...
		if (chip->adapter_type == ADAPTER_SVOOC) {
			...
			if (oplus_chglib_is_pd_svooc_adapter(chip->dev) || chip->chip_id == CHIP_ID_HL7138) {
...
irqreturn_t oplus_voocphy_interrupt_handler(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->vooc_flag == 0xF) {
		...
		if (oplus_voocphy_get_bidirect_cp_support(chip) == false) {
			...
			if (chip->fastchg_adapter_ask_cmd == VOOC_CMD_IS_VUBS_OK && chip->copycat_vooc_support &&
			    chip->adapter_is_vbus_ok_count == 2 && chip->adapter_type == ADAPTER_SVOOC &&
			    (chip->cp_vbus < SVOOC_INIT_VBUS_VOL_LOW || chip->cp_vbus > SVOOC_INIT_VBUS_VOL_HIGH ||
			...
			if (chip->fastchg_adapter_ask_cmd == VOOC_CMD_ASK_CURRENT_LEVEL && chip->copycat_vooc_support &&
			    chip->adapter_type == ADAPTER_SVOOC && chip->adapter_is_vbus_ok_count == 1) {
	...
	if (oplus_get_dual_chan_support() && oplus_chglib_get_vooc_is_started(chip->dev) &&
	...
	voocphy_info("pre_adapter_ask: 0x%0x, adapter_ask_cmd: 0x%0x, "
		     ...
		     chip->pre_adapter_ask_cmd, chip->fastchg_adapter_ask_cmd, chip->interrupt_flag, chip->vooc_flag);
...
static bool oplus_voocphy_set_user_exit_fastchg(struct oplus_voocphy_manager *chip, unsigned char exit)
{
	if (chip) {
		voocphy_info("exit %d\n", exit);
...
static void oplus_voocphy_parse_dt(struct oplus_voocphy_manager *chip)
{
	...
	rc = of_property_read_u32(node, "oplus_spec,voocphy_vbus_low", &chip->voocphy_vbus_low);
	if (rc) {
		chip->voocphy_vbus_low = DEFUALT_VBUS_LOW;
	}
	voocphy_info("voocphy_vbus_high is %d\n", chip->voocphy_vbus_low);

	rc = of_property_read_u32(node, "oplus_spec,voocphy_vbus_high", &chip->voocphy_vbus_high);
	if (rc) {
		chip->voocphy_vbus_high = DEFUALT_VBUS_HIGH;
	}
	voocphy_info("voocphy_vbus_high is %d\n", chip->voocphy_vbus_high);

	rc = of_property_read_u32(node, "oplus_spec,parallel_charge_project", &chip->parallel_charge_project);
	if (rc) {
		chip->parallel_charge_project = 0;
	}
	voocphy_info("oplus_spec,parallel_charge_project is %d\n", chip->parallel_charge_project);

	rc = of_property_read_u32(node, "oplus,dual_chan_buck_ucp_ichg_thre", &chip->buck_ucp_thre);
	if (rc) {
...
{
	struct oplus_voocphy_manager *chip = container_of(work, struct oplus_voocphy_manager, check_chg_out_work.work);

	oplus_voocphy_check_chg_out_work_func(chip);
}

static void oplus_voocphy_wake_check_chg_out_work(struct oplus_voocphy_manager *chip, unsigned int delay_ms)
{
	voocphy_info("check chg out after %d ms\n", delay_ms);
	schedule_delayed_work(&chip->check_chg_out_work, round_jiffies_relative(msecs_to_jiffies(delay_ms)));
}

static void oplus_ap_voocphy_send_ongoing_notify(struct work_struct *work)
{
	struct oplus_voocphy_manager *chip =
		container_of(work, struct oplus_voocphy_manager, voocphy_send_ongoing_notify.work);
	int chg_vol = 0;
...
{
	voocphy_info("%s\n", chip->fastchg_notify_status == FAST_NOTIFY_FULL ? "FAST_NOTIFY_FULL" :
	...
	if (chip->fastchg_notify_status == FAST_NOTIFY_FULL) {
		...
		oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_BAD_CONNECTED);
	}
...
static void oplus_voocphy_handle_batt_temp_over_notify(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->fastchg_notify_status == FAST_NOTIFY_BATT_TEMP_OVER)
		oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_BATT_TEMP_OVER);
	else
		oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_CURR_LIMIT_SMALL);
}
...
static void oplus_voocphy_handle_user_exit_notify(struct oplus_voocphy_manager *chip)
{
	...
	oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_USER_EXIT_FASTCHG);
}
...
static void oplus_voocphy_handle_switch_temp_range_notify(struct oplus_voocphy_manager *chip)
{
	...
	oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_SWITCH_TEMP_RANGE);
}
...
static void oplus_voocphy_handle_adapter_abnormal_notify(struct oplus_voocphy_manager *chip)
{
	...
	oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_ADAPTER_ABNORMAL);
...
static void oplus_voocphy_notify_fastchg_work(struct work_struct *work)
{
	...
	case FAST_NOTIFY_BTB_TEMP_OVER:
		...
		oplus_chglib_notify_ap(chip->dev, intval);
		oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_BTB_TEMP_OVER);
		break;
	...
	case FAST_NOTIFY_CHECK_FASTCHG_REAL_ALLOW:
		...
		break;
	default:
		oplus_chglib_push_break_code(chip->dev, TRACK_CP_VOOCPHY_FAST_ABSENT);
...
{
	voocphy_info("FAST_NOTIFY_DUMMY_START or FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL:%d", chip->fastchg_notify_status);
	/* reset vooc_phy */
	oplus_voocphy_reset_voocphy(chip);
...
static void oplus_voocphy_common_handle(struct oplus_voocphy_manager *chip)
{
	...
	if ((chip->fastchg_notify_status == FAST_NOTIFY_FULL && chip->vooc_temp_cur_range != FASTCHG_TEMP_RANGE_WARM) ||
...
static void oplus_voocphy_handle_adapter_copycat_status(struct oplus_voocphy_manager *chip, int fastchg_notify_status)
{
	chip->fastchg_start = false;
	oplus_voocphy_reset_fastchg_after_usbout(chip);
	/* reset voocphy */
...
unsigned char oplus_voocphy_set_fastchg_current(struct oplus_voocphy_manager *chip)
{
	...
	voocphy_info("max_curr[%d] ap_curr[%d] temp_curr[%d]", chip->current_max, chip->current_ap,
...
static int oplus_voocphy_vol_event_handle(struct device *dev, unsigned long data)
{
	...
	if (chip->parallel_charge_support) {
		chip->main_vbatt = oplus_chglib_gauge_main_vbatt(chip->dev);
		chip->sub_vbatt = oplus_chglib_gauge_sub_vbatt(chip->dev);
		if (chip->parallel_charge_project == PARALLEL_MOS_CTRL && !oplus_chglib_get_switch_hw_status(chip->dev))
			chip->pre_gauge_vbatt = chip->main_vbatt;
		else
...
{
	struct oplus_voocphy_manager *chip = container_of(work, struct oplus_voocphy_manager, monitor_start_work.work);
...
{
	int i = 0;
	struct oplus_voocphy_manager *chip = container_of(timer, struct oplus_voocphy_manager, monitor_btimer);
...
static int oplus_voocphy_parse_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	chip->voocphy_dual_cp_support = of_property_read_bool(node, "oplus_spec,voocphy_dual_cp_support");
	...
	chip->impedance_calculation_newmethod =
		of_property_read_bool(node, "oplus_spec,impedance_calculation_newmethod");
	...
	chip->copycat_vooc_support = of_property_read_bool(node, "oplus_spec,copycat_vooc_support");
	...
	rc = of_property_read_u32(node, "oplus_spec,slave_cp_enable_thr", &chip->slave_cp_enable_thr);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,slave_cp_disable_thr_high", &chip->slave_cp_disable_thr_high);
	if (rc) {
		chip->slave_cp_disable_thr_high = 1600;
	}
	voocphy_info("slave_cp_disable_thr_high is %d\n", chip->slave_cp_disable_thr_high);

	rc = of_property_read_u32(node, "oplus_spec,slave_cp_disable_thr_low", &chip->slave_cp_disable_thr_low);
	if (rc) {
		chip->slave_cp_disable_thr_low = 1400;
	}
	voocphy_info("slave_cp_disable_thr_low is %d\n", chip->slave_cp_disable_thr_low);

	rc = of_property_read_u32(node, "oplus_spec,voocphy_current_default", &chip->current_default);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_little_cool_temp", &chip->vooc_little_cool_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_cool_temp", &chip->vooc_cool_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_little_cold_temp", &chip->vooc_little_cold_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_normal_low_temp", &chip->vooc_normal_low_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_normal_high_temp", &chip->vooc_normal_high_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy_normal_current", &chip->vooc_strategy_normal_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_batt_up_temp1", &chip->vooc_strategy2_batt_up_temp1);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_batt_up_temp3", &chip->vooc_strategy2_batt_up_temp3);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_batt_up_temp5", &chip->vooc_strategy2_batt_up_temp5);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_batt_up_temp6", &chip->vooc_strategy2_batt_up_temp6);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_high0_current", &chip->vooc_strategy2_high0_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_high1_current", &chip->vooc_strategy2_high1_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_high2_current", &chip->vooc_strategy2_high2_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_strategy2_high3_current", &chip->vooc_strategy2_high3_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_low_temp", &chip->vooc_low_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_over_high_temp", &chip->vooc_batt_over_high_temp);
	if (rc) {
	...
	chip->external_gauge_support = of_property_read_bool(node, "oplus_spec,external_gauge_support");
	...
	rc = of_property_read_u32_array(node, "oplus_spec,svooc_cool_down_current_limit",
					chip->svooc_cool_down_current_limit, length);
	if (rc < 0) {
	...
	rc = of_property_read_u32_array(node, "oplus_spec,vooc_cool_down_current_limit",
					chip->vooc_cool_down_current_limit, length);
	if (rc < 0) {
		voocphy_info("parse vooc_cool_down_current_limit failed, rc=%d\n", rc);
		return rc;
	}

	rc = of_property_read_u32(node, "oplus_spec,fastchg_timeout_time_init", &chip->fastchg_timeout_time_init);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus_spec,vooc_cool_full_voltage", &chip->vooc_cool_full_voltage);
	if (rc < 0) {
		voocphy_info("parse vooc_cool_full_voltage failed, rc=%d\n", rc);
		chip->vooc_cool_full_voltage = TEMP45_2_TEMP115_FULL_VOLTAGE;
	}

	rc = of_property_read_u32(node, "oplus_spec,vooc_warm_full_voltage", &chip->vooc_warm_full_voltage);
	if (rc < 0) {
		voocphy_info("parse vooc_warm_full_voltage failed, rc=%d\n", rc);
		chip->vooc_warm_full_voltage = -EINVAL;
	}

	rc = of_property_read_u32(node, "oplus_spec,vooc_1time_full_voltage", &chip->vooc_1time_full_voltage);
	if (rc < 0) {
		voocphy_info("parse vooc_1time_full_voltage failed, rc=%d\n", rc);
		chip->vooc_1time_full_voltage = BAT_FULL_1TIME_THD;
	}

	rc = of_property_read_u32(node, "oplus_spec,vooc_ntime_full_voltage", &chip->vooc_ntime_full_voltage);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus_spec,low_curr_full_t1", &chip->low_curr_full_t1);
	if (rc < 0) {
		voocphy_info("parse low_curr_full_t1 failed, rc=%d\n", rc);
		chip->low_curr_full_t1 = 120;
	}

	rc = of_property_read_u32(node, "oplus_spec,low_curr_full_t2", &chip->low_curr_full_t2);
	if (rc < 0) {
		voocphy_info("parse low_curr_full_t2 failed, rc=%d\n", rc);
		chip->low_curr_full_t2 = 350;
	}

	rc = of_property_read_u32(node, "oplus_spec,low_curr_full_t3", &chip->low_curr_full_t3);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus_spec,voocphy_freq_mincore", &chip->voocphy_freq_mincore);
	if (rc) {
		chip->voocphy_freq_mincore = 925000;
	}
	voocphy_info("voocphy_freq_mincore is %d\n", chip->voocphy_freq_mincore);

	rc = of_property_read_u32(node, "oplus_spec,voocphy_freq_midcore", &chip->voocphy_freq_midcore);
	if (rc) {
		chip->voocphy_freq_midcore = 925000;
	}
	voocphy_info("voocphy_freq_midcore is %d\n", chip->voocphy_freq_midcore);

	rc = of_property_read_u32(node, "oplus_spec,voocphy_freq_maxcore", &chip->voocphy_freq_maxcore);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,voocphy_ibat_over_current", &chip->voocphy_ibat_over_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,voocphy_cp_max_ibus", &chip->voocphy_cp_max_ibus);
	if (rc) {
		chip->voocphy_cp_max_ibus = 3200;
	}
	voocphy_info("voocphy_cp_max_ibus is %d\n", chip->voocphy_cp_max_ibus);
	rc = of_property_read_u32(node, "oplus_spec,voocphy_svooc_cp_max_ibus", &chip->voocphy_svooc_cp_max_ibus);
	if (rc) {
	...
	rc = of_property_read_u32(node, "oplus_spec,voocphy_vooc_cp_max_ibus", &chip->voocphy_vooc_cp_max_ibus);
	if (rc) {
...
static void oplus_apvphy_switch_chg_mode(struct device *dev, int mode)
{
	...
	if (mode == 1) {
		if (chip->fastchg_to_normal ||
		    (!oplus_voocphy_get_real_fastchg_allow(chip) && chip->fastchg_dummy_start) ||
		    chip->user_exit_fastchg) {
		...
		if (oplus_voocphy_get_real_fastchg_allow(chip) == false && chip->fastchg_to_warm == true) {
		...
		rc = wait_for_completion_interruptible_timeout(&vooc_init_check_ack,
							       msecs_to_jiffies(VOOC_INIT_WAIT_TIME_MS));
		if (!rc)
		...
		if (oplus_chglib_is_switch_temp_range()) {
			...
			oplus_voocphy_set_status_and_notify_ap(chip, FAST_NOTIFY_SWITCH_TEMP_RANGE);
		}
