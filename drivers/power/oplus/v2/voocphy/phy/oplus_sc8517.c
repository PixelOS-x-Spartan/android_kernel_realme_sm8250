...
static void sc8517_update_data(struct oplus_voocphy_manager *chip)
{
	...
	chg_info("cp_ichg = %d cp_vbus = %d, cp_vsys = %d cp_vbat = %d interrupt_flag = %d", chip->cp_ichg,
		 chip->cp_vbus, chip->cp_vsys, chip->cp_vbat, chip->interrupt_flag);
}
...
static bool sc8517_check_cp_int_happened(struct oplus_voocphy_manager *chip, bool *dump_reg, bool *send_info)
{
	int i = 0;
...
static int sc8517_irq_register(struct sc8517_device *chip)
{
	...
	if (voocphy->irq) {
		ret = request_threaded_irq(voocphy->irq, NULL, sc8517_interrupt_handler,
