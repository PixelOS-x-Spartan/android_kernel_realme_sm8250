...
static int sc8547_update_bits(struct i2c_client *client, unsigned char reg, unsigned char mask, unsigned char data)
{
	int ret;
	unsigned char tmp;
...
static void sc8547_update_data(struct oplus_voocphy_manager *chip)
{
	...
	pr_info("cp_ichg = %d cp_vbus = %d, cp_vsys = %d cp_vbat = %d cp_vac = %d int_flag = %d", chip->cp_ichg,
		chip->cp_vbus, chip->cp_vsys, chip->cp_vbat, chip->cp_vac, chip->interrupt_flag);
}
...
static bool sc8547_check_cp_int_happened(struct oplus_voocphy_manager *chip, bool *dump_reg, bool *send_info)
{
	int i = 0;
...
static int sc8547_irq_register(struct oplus_voocphy_manager *voocphy)
{
	...
	if (voocphy->irq) {
		ret = request_threaded_irq(voocphy->irq, NULL, sc8547_interrupt_handler,
					   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "voocphy_irq", voocphy);
		if (ret < 0) {
