...
static void hl7138_update_data(struct oplus_voocphy_manager *chip)
{
	...
	chg_debug("cp_ichg = %d cp_vbus = %d, cp_vsys = %d, cp_vbat = %d, int_flag = %d", chip->cp_ichg, chip->cp_vbus,
		  chip->cp_vsys, chip->cp_vbat, chip->interrupt_flag_hl7138);
}
...
static bool hl7138_check_cp_int_happened(struct oplus_voocphy_manager *chip, bool *dump_reg, bool *send_info)
{
	int i = 0;
...
static int hl7138_irq_register(struct oplus_voocphy_manager *voocphy)
{
	...
	if (voocphy->irq) {
		ret = request_threaded_irq(voocphy->irq, NULL, hl7138_interrupt_handler,
