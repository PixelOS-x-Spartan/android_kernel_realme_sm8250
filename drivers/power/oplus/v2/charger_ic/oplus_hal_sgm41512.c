...
__maybe_unused static int sgm41512_read_data(struct sgm41512_chip *chip, u8 addr, u8 *buf, int len)
{
	int rc;
	bool is_err = false;
	int retry = 3;
...
__maybe_unused static int sgm41512_write_data(struct sgm41512_chip *chip, u8 addr, u8 *buf, int len)
{
	u8 *buf_temp;
	int i;
	int rc;
...
__maybe_unused static int sgm41512_read_byte_mask(struct sgm41512_chip *chip, u8 addr, u8 mask, u8 *data)
{
	u8 temp;
	int rc;
...
__maybe_unused static int sgm41512_write_byte_mask(struct sgm41512_chip *chip, u8 addr, u8 mask, u8 data)
{
	u8 temp;
	int rc;
...
static int sgm41512_enable_hiz_mode(struct sgm41512_chip *chip, bool en)
{
	...
	rc = sgm41512_write_byte_mask(chip, HIZ_MODE_REG, HIZ_MODE_BIT, en ? HIZ_MODE_BIT : 0x00);
	if (rc < 0) {
...
{
	struct sgm41512_chip *chip = container_of(work, struct sgm41512_chip, event_work);
	u8 data;
	int rc;
	...
	if (READ_ONCE(chip->auto_bc12)) {
		...
		rc = sgm41512_write_byte_mask(chip, BC12_REG, BC12_RERUN_BIT, BC12_RERUN_BIT);
		if (rc < 0) {
	...
	rc = sgm41512_read_byte_mask(chip, BC12_RESULT_REG, BC12_RESULT_BIT, &data);
	if (rc < 0) {
...
static void sgm41512_bc12_boot_check(struct sgm41512_chip *chip)
{
	...
	rc = sgm41512_write_byte_mask(chip, VINDPM_OVP_CONFIG_REG, VINDPM_CONFIG_BIT, 0x01);
	if (rc < 0)
		chg_err("set vindpm thr error, rc=%d\n", rc);
	rc = sgm41512_write_byte_mask(chip, IRQ_MASK_REG, VINDPM_IRQ_MASK_BIT | IINDPM_IRQ_MASK_BIT,
				      VINDPM_IRQ_MASK_BIT | IINDPM_IRQ_MASK_BIT);
	if (rc < 0)
		chg_err("disable vindpm & iindpm interrupt error, rc=%d\n", rc);

	rc = sgm41512_write_byte_mask(chip, WATCHGDOG_CONFIG_REG, WATCHGDOG_CONFIG_BIT, 0x00);
	if (rc < 0)
...
static bool sgm41512_is_first_boot(struct sgm41512_chip *chip)
{
	...
	rc = sgm41512_read_byte_mask(chip, WATCHGDOG_CONFIG_REG, WATCHGDOG_CONFIG_BIT, &data);
	if (rc < 0) {
...
{
	struct sgm41512_chip *chip = container_of(work, struct sgm41512_chip, plugin_work);
...
static void sgm41512_bc12_timeout_work(struct work_struct *work)
{
	...
	struct sgm41512_chip *chip = container_of(dwork, struct sgm41512_chip, bc12_timeout_work);
	int rc;

	chg_err("BC1.2 check timeout\n");
	rc = sgm41512_write_byte_mask(chip, HIZ_MODE_REG, HIZ_MODE_BIT, HIZ_MODE_BIT);
	if (rc < 0)
		chg_err("can't enable hiz mode, rc=%d\n", rc);
}
...
static int sgm41512_reg_dump(struct oplus_chg_ic_dev *ic_dev)
{
	...
	print_hex_dump(KERN_ERR, "OPLUS_CHG[SGM41512]: ", DUMP_PREFIX_OFFSET, 32, 1, buf, ARRAY_SIZE(buf), false);
	return 0;
}

static int sgm41512_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	return 0;
}

static int sgm41512_input_present(struct oplus_chg_ic_dev *ic_dev, bool *present)
{
	struct sgm41512_chip *chip;
...
static int sgm41512_disable_vbus(struct oplus_chg_ic_dev *ic_dev, bool en, bool delay)
{
	struct sgm41512_chip *chip;
	int rc;
...
static void sgm41512_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void sgm41512_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	struct sgm41512_chip *chip = prv_data;
	union mms_msg_data data = { 0 };

	chip->wired_topic = topic;
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, sgm41512_wired_subs_callback, "sgm41512");
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, sgm41512_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, sgm41512_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, sgm41512_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, sgm41512_smt_test);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_PRESENT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_PRESENT, sgm41512_input_present);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE, sgm41512_get_charger_type);
		break;
	case OPLUS_IC_FUNC_BUCK_RERUN_BC12:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_RERUN_BC12, sgm41512_rerun_bc12);
		break;
	case OPLUS_IC_FUNC_DISABLE_VBUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_DISABLE_VBUS, sgm41512_disable_vbus);
		break;
	default:
...
static int sgm41512_gpio_init(struct sgm41512_chip *chip)
{
	...
	rc = devm_request_irq(chip->dev, chip->event_irq, sgm41512_event_handler, IRQF_TRIGGER_FALLING,
...
static int sgm41512_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct sgm41512_chip), GFP_KERNEL);
	if (!chip) {
