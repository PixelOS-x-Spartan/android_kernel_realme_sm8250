...
static void mtk_charger_parse_dt(struct mtk_charger *info, struct device *dev)
{
	...
	chr_debug("%s: support_ntc_01c_precision: %d, support_subboard_ntc = %d\n", __func__,
		  info->support_ntc_01c_precision, info->support_subboard_ntc);
}
...
static void mtk_charger_start_timer(struct mtk_charger *info)
{
	...
	chr_err("alarm timer start:%d, %lld %ld\n", ret, info->endtime.tv_sec, info->endtime.tv_nsec);
...
static ssize_t charger_ic_ntc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int temp = 0;
...
static ssize_t charger_subboard_ntc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int temp = 0;
...
static enum alarmtimer_restart mtk_charger_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	...
	if (ktime_us_delta(time_p[7], time_p[0]) > 5000)
		chr_err("delta_t: %lld %lld %lld %lld %lld %lld %lld (%lld)\n", ktime_us_delta(time_p[1], time_p[0]),
			ktime_us_delta(time_p[2], time_p[1]), ktime_us_delta(time_p[3], time_p[2]),
			ktime_us_delta(time_p[4], time_p[3]), ktime_us_delta(time_p[5], time_p[4]),
			ktime_us_delta(time_p[6], time_p[5]), ktime_us_delta(time_p[7], time_p[6]),
...
static void hvdcp_detect_dn_check(struct mtk_charger *info)
{
	...
	chg_info("%s event online:%d type:%d vbus:%d\n", __func__, prop.intval, prop2.intval, get_vbus(info));
...
static int oplus_mt6375_input_current_limit_write_without_aicl(struct mtk_charger *info, int value)
{
	struct charger_device *chg = NULL;
	int rc = 0;
...
static int oplus_mt6375_input_current_limit_write(struct mtk_charger *info, int value)
{
	...
	if (is_gauge_topic_available(info)) {
		oplus_mms_get_item_data(info->gauge_topic, GAUGE_ITEM_VOL_MAX, &data, false);
...
static int mtk_chg_set_icl(struct oplus_chg_ic_dev *ic_dev, bool vooc_mode, bool step, int icl_ma)
{
	struct mtk_charger *info = oplus_chg_ic_get_drvdata(ic_dev);
...
static int mtk_chg_get_charger_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	...
	rc = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_ONLINE, &prop);
	if (rc < 0) {
	...
	rc = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_USB_TYPE, &prop);
	if (rc < 0) {
	...
	if (!online || (charger_type == POWER_SUPPLY_TYPE_USB && usb_type == POWER_SUPPLY_USB_TYPE_UNKNOWN))
		charger_type = POWER_SUPPLY_TYPE_UNKNOWN;

	chr_debug("online:%d type:%d usb_type:%d\n", online, charger_type, usb_type);
	...
	if (chip->pd_type == MTK_PD_CONNECT_PE_READY_SNK || chip->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 ||
	...
	if (charger_type == POWER_SUPPLY_TYPE_USB_DCP && chip->tcpc && tcpm_inquire_pd_connected(chip->tcpc))
		*type = OPLUS_CHG_USB_TYPE_PD;
...
static void oplus_chg_get_soc_and_temp(int *soc, int *temp)
{
	...
	if (common_topic) {
		oplus_mms_get_item_data(common_topic, COMM_ITEM_UI_SOC, &data, false);
		*soc = data.intval;
		oplus_mms_get_item_data(common_topic, COMM_ITEM_SHELL_TEMP, &data, false);
		*temp = data.intval;
	} else {
...
{
	int chg_volt = 0;
	struct mtk_charger *chip = container_of(work, struct mtk_charger, hvdcp_detect_work.work);
...
{
	struct mtk_charger *chip = container_of(work, struct mtk_charger, detach_clean_work.work);
...
static int mtk_chg_get_typec_role(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct mtk_charger *chip;
	uint8_t typec_role;
...
static int mtk_chg_get_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct mtk_charger *chip;
	uint8_t typec_state;
...
static int mtk_chg_set_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type mode)
{
	struct mtk_charger *chip;
	uint8_t typec_role;
	int rc = 0;
...
static void *oplus_chg_mt6895_gauge_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_GAUGE_GET_SUBBOARD_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_SUBBOARD_TEMP, oplus_chg_get_subboard_temp);
		break;
	default:
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	switch (func_id) {
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, mtk_chg_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, mtk_chg_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, mtk_chg_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, mtk_chg_smt_test);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_PRESENT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_PRESENT, mtk_chg_input_present);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND, mtk_chg_input_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND, mtk_chg_input_is_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND, mtk_chg_output_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND, mtk_chg_output_is_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ICL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_ICL, mtk_chg_set_icl);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_ICL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_ICL, mtk_chg_get_icl);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FCC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_FCC, mtk_chg_set_fcc);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FV:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_FV, mtk_chg_set_fv);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_FV:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_FV, mtk_chg_get_fv);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ITERM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_ITERM, mtk_chg_set_iterm);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL, mtk_chg_set_rechg_vol);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR, mtk_chg_get_input_curr);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL, mtk_chg_get_input_vol);
		break;
	case OPLUS_IC_FUNC_OTG_BOOST_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_OTG_BOOST_ENABLE, mtk_chg_otg_boost_enable);
		break;
	case OPLUS_IC_FUNC_SET_OTG_BOOST_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_BOOST_VOL, mtk_chg_set_otg_boost_vol);
		break;
	...
	case OPLUS_IC_FUNC_BUCK_AICL_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_AICL_ENABLE, mtk_chg_aicl_enable);
		break;
	case OPLUS_IC_FUNC_BUCK_AICL_RERUN:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_AICL_RERUN, mtk_chg_aicl_rerun);
		break;
	case OPLUS_IC_FUNC_BUCK_AICL_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_AICL_RESET, mtk_chg_aicl_reset);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION, mtk_chg_get_cc_orientation);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_HW_DETECT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_HW_DETECT, mtk_chg_get_hw_detect);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_TYPEC_STATE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_TYPEC_STATE, mtk_chg_get_typec_state);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE, mtk_chg_get_charger_type);
		break;
	case OPLUS_IC_FUNC_BUCK_RERUN_BC12:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_RERUN_BC12, mtk_chg_rerun_bc12);
		break;
	case OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE, mtk_chg_qc_detect_enable);
		break;
	case OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE, mtk_chg_shipmode_enable);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG, mtk_chg_set_qc_config);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG, mtk_chg_set_pd_config);
		break;
	case OPLUS_IC_FUNC_GET_SHUTDOWN_SOC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_SHUTDOWN_SOC, mtk_chg_get_shutdown_soc);
		break;
	case OPLUS_IC_FUNC_BACKUP_SOC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BACKUP_SOC, mtk_chg_backup_soc);
		break;
	...
	case OPLUS_IC_FUNC_GET_TYPEC_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_TYPEC_MODE, mtk_chg_get_typec_mode);
		break;
	case OPLUS_IC_FUNC_SET_TYPEC_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_TYPEC_MODE, mtk_chg_set_typec_mode);
		break;
	case OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS, mtk_chg_set_otg_switch_status);
		break;
	case OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS, mtk_chg_get_otg_switch_status);
		break;
	case OPLUS_IC_FUNC_GET_OTG_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_OTG_ENABLE, mtk_chg_get_otg_enbale);
		break;
	case OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, mtk_chg_set_usbtemp_dischg_enable);
		break;
	case OPLUS_IC_FUNC_GET_USB_DISCHG_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_USB_DISCHG_STATUS, mtk_chg_get_usbtemp_dischg_status);
		break;
	case OPLUS_IC_FUNC_IS_OPLUS_SVID:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_IS_OPLUS_SVID, mtk_chg_is_oplus_svid);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP, mtk_chg_get_usb_btb_temp);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP, mtk_chg_get_batt_btb_temp);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_AICL_POINT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_AICL_POINT, oplus_mt6375_set_aicl_point);
		break;
	case OPLUS_IC_FUNC_GET_TYPEC_ROLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_TYPEC_ROLE, mtk_chg_get_typec_role);
		break;
	default:
		chg_err("this func(=%d) is not supported\n", func_id);
		func = NULL;
...
static int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	if (!is_param_init) {
		...
		chg_err("ntc_type:%d,critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
			...
			ntc_param.i_rap_pull_up_voltage, ntc_param.i_tap_min, ntc_param.i_tap_max,
			ntc_param.i_table_size);
	}
...
static int oplus_get_subboard_temp(void)
{
	...
	if (!is_param_init) {
		...
		chg_info(
			"ntc_type:%d,critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
			ntc_param.e_ntc_type, ntc_param.i_tap_over_critical_low, ntc_param.i_rap_pull_up_r,
			ntc_param.i_rap_pull_up_voltage, ntc_param.i_tap_min, ntc_param.i_tap_max,
			ntc_param.i_table_size);
	}
