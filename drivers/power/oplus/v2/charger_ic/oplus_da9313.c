...
static ssize_t da9313_regs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int addr = 0;
	int reg_val = 0;
...
static ssize_t da9313_regs_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(read_regs, S_IWUSR | S_IRUGO, da9313_regs_show, da9313_regs_store);
static struct attribute *da9313_attributes[] = { &dev_attr_read_regs.attr, NULL };

static struct attribute_group da9313_attribute_group = { .attrs = da9313_attributes };
