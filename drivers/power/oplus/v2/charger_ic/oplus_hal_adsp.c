...
QCOM_PM_TYPEC_PORT_ROLE_TYPE qcom_typec_port_role[] = { QCOM_TYPEC_PORT_ROLE_DRP,     QCOM_TYPEC_PORT_ROLE_SNK,
							QCOM_TYPEC_PORT_ROLE_SRC,     QCOM_TYPEC_PORT_ROLE_DRP,
							QCOM_TYPEC_PORT_ROLE_INVALID, QCOM_TYPEC_PORT_ROLE_DISABLE,
							QCOM_TYPEC_PORT_ROLE_INVALID, QCOM_TYPEC_PORT_ROLE_INVALID };
...
static const int wls_prop_map[WLS_PROP_MAX] = {
	...
	[WLS_CURR_MAX] = POWER_SUPPLY_PROP_CURRENT_MAX, [WLS_INPUT_CURR_LIMIT] = POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
...
static int ap_set_message_id(struct battery_chg_dev *bcdev, u32 message_id)
{
	...
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->ap_read_ack[AP_MESSAGE_ACK],
...
static void handle_ap_read_buffer(struct battery_chg_dev *bcdev, struct oplus_ap_read_buffer_resp_msg *resp_msg,
				  size_t len)
{
	u32 buf_len;
...
static void handle_bcc_read_buffer(struct battery_chg_dev *bcdev, struct oem_read_buffer_resp_msg *resp_msg, size_t len)
{
	...
	if (oplus_vooc_get_fastchg_ing(bcdev) && oplus_vooc_get_fast_chg_type(bcdev) != CHARGER_SUBTYPE_FASTCHG_VOOC) {
	...
	chg_info(
		"----dod0_1[%d], dod0_2[%d], dod0_passed_q[%d], qmax_1[%d], qmax_2[%d], qmax_passed_q[%d], "
		...
		bcdev->bcc_read_buffer_dump.data_buffer[0], bcdev->bcc_read_buffer_dump.data_buffer[1],
...
static int ufcs_battery_chg_write(struct battery_chg_dev *bcdev, void *data, int len)
{
	int rc;
	...
	rc = pmic_glink_write(bcdev->client, data, len);
	if (!rc) {
		rc = wait_for_completion_timeout(&bcdev->ufcs_read_ack, msecs_to_jiffies(AP_UFCS_WAIT_TIME_MS));
		if (!rc) {
			chg_err("Error, timed out sending message\n");
...
static void handle_ufcs_read_buffer(struct battery_chg_dev *bcdev, struct oplus_ap_read_ufcs_resp_msg *resp_msg,
				    size_t len)
{
	u32 buf_len;

	if (len > sizeof(bcdev->ufcs_read_buffer_dump)) {
		chg_err("Incorrect length received: %zu expected: %zd\n", len, sizeof(bcdev->ufcs_read_buffer_dump));
...
void oplus_chg_set_curr_level_to_voocphy(struct battery_chg_dev *bcdev)
{
	...
	if (is_common_topic_available(bcdev)) {
		oplus_mms_get_item_data(bcdev->common_topic, COMM_ITEM_COOL_DOWN, &data, false);
	} else {
...
static bool oplus_vooc_get_fastchg_ing(struct battery_chg_dev *bcdev)
{
	...
	oplus_mms_get_item_data(bcdev->vooc_topic, VOOC_ITEM_VOOC_CHARGING, &data, true);
...
static int oplus_vooc_get_fast_chg_type(struct battery_chg_dev *bcdev)
{
	...
	oplus_mms_get_item_data(bcdev->vooc_topic, VOOC_ITEM_GET_BCC_SVOOC_TYPE, &data, true);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, recheck_input_current_work.work);
	...
	if (chg_vol > VOLTAGE_2000MV) {
		...
		if (count > COUNT_TEN) {
			chg_err("reset input current err_cnt: %d,chg_vol:%d,"
				...
				err_cnt, chg_vol, fastchg_ing, ibus_curr, fast_chg_type);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, adsp_voocphy_status_work.work);
	struct psy_state *pst = NULL;
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, otg_init_work.work);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, cid_status_change_work.work);
	struct psy_state *pst = NULL;
	int cid_status = 0;
	int rc = 0;
...
static int oplus_wrx_otg_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	pinctrl_select_state(bcdev->oplus_custom_gpio.wrx_otg_en_pinctrl, bcdev->oplus_custom_gpio.wrx_otg_en_sleep);
...
static void oplus_set_wrx_otg_en_val(struct battery_chg_dev *bcdev, int value)
{
	...
	chg_info("set value:%d, gpio_val:%d\n", value, gpio_get_value(bcdev->oplus_custom_gpio.wrx_otg_en_gpio));
}
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, adsp_crash_recover_work.work);
	struct psy_state *pst = NULL;
...
static void oplus_voocphy_enable_check_func(struct work_struct *work)
{
	...
	int prop_id = 0;
	struct psy_state *pst;
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, voocphy_enable_check_work.work);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, pd_only_check_work.work);
...
static int oplus_chg_get_match_temp(struct battery_chg_dev *bcdev)
{
	...
	if (is_common_topic_available(bcdev)) {
		oplus_mms_get_item_data(bcdev->common_topic, COMM_ITEM_SHELL_TEMP, &data, false);
		temp = data.intval;
	} else {
		chg_err("common topic not found\n");
		temp = 320;
...
static int oplus_get_bat_info_for_otg_status_check(struct battery_chg_dev *bcdev, int *soc, int *ichaging)
{
	struct psy_state *pst = NULL;
	int rc = 0;
	int prop_id = 0;
...
__maybe_unused static bool oplus_get_wired_otg_online(void)
{
	struct battery_chg_dev *bcdev = g_bcdev;
...
{
	oplus_chg_ic_creat_err_msg(ic_dev, OPLUS_IC_ERR_GAN_MOS_ERROR, 0, "$$err_reason@@Gan_mos_10V");
...
static int wireless_fw_update(struct battery_chg_dev *bcdev, bool force)
{
	...
	rc = wait_for_completion_timeout(&bcdev->fw_update_ack, msecs_to_jiffies(WLS_FW_WAIT_TIME_MS));
	if (!rc) {
...
static void oplus_vchg_trig_work(struct work_struct *work)
{
	...
	if (level == 0) {
		...
		if (chip->wireless_support && chip->voocphy.fastchg_to_warm == false &&
		    chip->voocphy.fastchg_to_normal == false)
			oplus_switch_from_wired_charge(bcdev);
	}

	if (chip->voocphy.fastchg_to_warm == false && chip->voocphy.fastchg_to_normal == false) {
...
static int oplus_subboard_temp_iio_init(struct battery_chg_dev *bcdev)
{
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names", "batt_con_therm_adc");
	if (rc >= 0) {
		bcdev->iio.batt_con_btb_chan = iio_channel_get(bcdev->dev, "batt_con_therm_adc");
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names", "usb_con_therm_adc");
	if (rc >= 0) {
		bcdev->iio.usb_con_btb_chan = iio_channel_get(bcdev->dev, "usb_con_therm_adc");
...
static int oplus_chg_parse_custom_dt(struct battery_chg_dev *bcdev)
{
	...
	rc = of_property_read_u32(node, "oplus,otg_scheme", &bcdev->otg_scheme);
	if (rc) {
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, voocphy_err_work.work);
	int mmi_chg = 1;
...
static int oplus_get_batt_full_status(struct battery_chg_dev *bcdev)
{
	...
	if (is_common_topic_available(bcdev)) {
		oplus_mms_get_item_data(bcdev->common_topic, COMM_ITEM_CHG_FULL, &data, false);
	} else {
		chg_err("common topic not found\n");
	}
...
void lcm_frequency_ctrl(struct battery_chg_dev *bcdev)
{
	...
	if ((oplus_wired_get_vbus() > LCM_CHARGER_VOL_THR_MV)) {
		...
		mod_delayed_work(system_highpri_wq, &bcdev->ctrl_lcm_frequency, LCM_FREQUENCY_INTERVAL);
	} else {
		if (lcm_en_flag != LCM_EN_ENABLE) {
			lcm_en_flag = LCM_EN_ENABLE;
			smbchg_lcm_en(bcdev, true);
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, ctrl_lcm_frequency.work);
...
static void oplus_plugin_irq_work(struct work_struct *work)
{
	...
	if (usb_plugin_status && usb_plugin_status == usb_pre_plugin_status && type == pre_type &&
	    sub_type == pre_sub_type) {
		chg_info("usb_plugin_status:%d, type:%d, sub_type:%d, no change, return\n", usb_plugin_status, type,
			 sub_type);
		return;
	}
	pre_type = type;
...
static bool oplus_get_pps_charging(struct battery_chg_dev *bcdev)
{
	...
	if (bcdev->pps_topic) {
		oplus_mms_get_item_data(bcdev->pps_topic, PPS_ITEM_CHARGING, &data, false);
...
__maybe_unused static int fg_sm8350_get_battery_mvolts(void)
{
	...
	if (oplus_chg_get_voocphy_support(bcdev) == ADSP_VOOCPHY && !oplus_get_pps_charging(bcdev)) {
	...
	if (qpnp_get_prop_vbus_collapse_status(bcdev) == true) {
		if (bcdev->rerun_max > 0) {
			...
			schedule_delayed_work(
				&bcdev->vbus_collapse_rerun_icl_work,
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, vbus_collapse_rerun_icl_work.work);

	oplus_chg_set_input_current(bcdev, bcdev->g_icl_ma);
}

static int oplus_chg_8350_set_icl(struct oplus_chg_ic_dev *ic_dev, bool vooc_mode, bool step, int icl_ma)
{
	struct battery_chg_dev *bcdev;
	int rc = 0;
...
static int oplus_chg_8350_get_charger_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	...
	if (*type != POWER_SUPPLY_USB_TYPE_PD && *type != POWER_SUPPLY_USB_TYPE_PD_DRP &&
...
static int oplus_chg_8350_get_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	int rc = 0;
...
static int oplus_chg_8350_set_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type mode)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	int rc = 0;
...
static int oplus_chg_8350_set_otg_switch_status(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	int rc = 0;
...
static int oplus_chg_8350_get_otg_switch_status(struct oplus_chg_ic_dev *ic_dev, bool *en)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	int rc = 0;
...
static int oplus_chg_adsp_get_usb_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *usb_btb_temp)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_chg_adsp_get_batt_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *batt_btb_temp)
{
	struct battery_chg_dev *bcdev;
...
static void *oplus_chg_8350_buck_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT,
					       oplus_chg_8350_set_otg_boost_curr_limit);
		break;
	...
	case OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION,
					       oplus_chg_8350_get_cc_orientation);
		break;
	...
	case OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS,
					       oplus_chg_8350_get_vbus_collapse_status);
		break;
	...
	case OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT,
					       oplus_chg_set_wls_boost_curr_limit);
		break;
...
struct oplus_chg_ic_virq oplus_chg_8350_buck_virq_table[] = {
	{ .virq_id = OPLUS_IC_VIRQ_ERR },	    { .virq_id = OPLUS_IC_VIRQ_CC_DETECT },
	{ .virq_id = OPLUS_IC_VIRQ_PLUGIN },	    { .virq_id = OPLUS_IC_VIRQ_CC_CHANGED },
	{ .virq_id = OPLUS_IC_VIRQ_SUSPEND_CHECK }, { .virq_id = OPLUS_IC_VIRQ_CHG_TYPE_CHANGE },
	{ .virq_id = OPLUS_IC_VIRQ_OTG_ENABLE },    { .virq_id = OPLUS_IC_VIRQ_RESUME },
...
static int oplus_sm8350_get_batt_max(struct oplus_chg_ic_dev *ic_dev, int *vol_mv)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_get_batt_min(struct oplus_chg_ic_dev *ic_dev, int *vol_mv)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_get_batt_curr(struct oplus_chg_ic_dev *ic_dev, int *curr_ma)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_get_batt_temp(struct oplus_chg_ic_dev *ic_dev, int *temp)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_get_batt_auth(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
...
static int oplus_sm8350_get_batt_hmac(struct oplus_chg_ic_dev *ic_dev, bool *pass)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_set_batt_full(struct oplus_chg_ic_dev *ic_dev, bool full)
{
...
static int oplus_sm8350_update_soc_smooth_parameter(struct oplus_chg_ic_dev *ic_dev)
{
	struct battery_chg_dev *chip;
...
static int oplus_sm8350_get_cb_status(struct oplus_chg_ic_dev *ic_dev, int *status)
{
...
static int oplus_sm8350_get_device_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	*type = 0;
...
static int oplus_sm8350_get_device_type_for_vooc(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	*type = 0;
...
static int oplus_sm8350_get_battery_dod0(struct oplus_chg_ic_dev *ic_dev, int index, int *val)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_get_battery_dod0_passed_q(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_get_battery_qmax(struct oplus_chg_ic_dev *ic_dev, int index, int *val)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_get_battery_qmax_passed_q(struct oplus_chg_ic_dev *ic_dev, int *val)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_get_batt_vol(struct oplus_chg_ic_dev *ic_dev, int index, int *val)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_get_battery_gauge_type_for_bcc(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	struct battery_chg_dev *bcdev;
...
static int batt_info_read_buffer(struct battery_chg_dev *bcdev)
{
	...
	memcpy(&bcdev->battinfo, bcdev->ap_read_buffer_dump->data_buffer, sizeof(struct battery_manufacture_info));
	memset(bcdev->ap_read_buffer_dump, 0, sizeof(*bcdev->ap_read_buffer_dump));
	mutex_unlock(&bcdev->ap_read_buffer_lock);
	return index;
err:
...
static void *oplus_chg_8350_gauge_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	switch (func_id) {
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_sm8350_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_sm8350_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_sm8350_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_sm8350_smt_test);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL, oplus_sm8350_get_batt_vol);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX, oplus_sm8350_get_batt_max);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN, oplus_sm8350_get_batt_min);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR, oplus_sm8350_get_batt_curr);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP, oplus_sm8350_get_batt_temp);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC, oplus_sm8350_get_batt_soc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC, oplus_sm8350_get_batt_fcc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_CC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_CC, oplus_sm8350_get_batt_cc);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_RM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_RM, oplus_sm8350_get_batt_rm);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH, oplus_sm8350_get_batt_soh);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH, oplus_sm8350_get_batt_auth);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC, oplus_sm8350_get_batt_hmac);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_BATT_FULL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_BATT_FULL, oplus_sm8350_set_batt_full);
		break;
	case OPLUS_IC_FUNC_GAUGE_UPDATE_DOD0:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_UPDATE_DOD0, oplus_sm8350_update_dod0);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS, oplus_sm8350_get_cb_status);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_LOCK:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_LOCK, oplus_sm8350_set_lock);
		break;
	case OPLUS_IC_FUNC_GAUGE_IS_LOCKED:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_IS_LOCKED, oplus_sm8350_is_locked);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_NUM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_NUM, oplus_sm8350_get_batt_num);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE, oplus_sm8350_get_device_type);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_BCC_PARMS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BCC_PARMS, oplus_get_bcc_parameters_from_adsp);
		break;
	case OPLUS_IC_FUNC_GAUGE_SET_BCC_PARMS:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_BCC_PARMS, oplus_set_bcc_debug_parameters);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_DOD0:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_DOD0, oplus_sm8350_get_battery_dod0);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_GET_QMAX:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_QMAX, oplus_sm8350_get_battery_qmax);
		break;
	...
	case OPLUS_IC_FUNC_GAUGE_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_IS_SUSPEND, oplus_chg_8350_gauge_is_suspend);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_SUBBOARD_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_SUBBOARD_TEMP, oplus_get_subboard_temp);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_REG_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_REG_INFO, oplus_sm8350_get_reg_info);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_CALIB_TIME:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_CALIB_TIME, oplus_sm8350_get_calib_time);
		break;
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_SN:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_SN, oplus_get_battinfo_sn);
		break;
	default:
		chg_err("this func(=%d) is not supported\n", func_id);
		func = NULL;
...
#define REG_DUMP_SIZE (REG_DUMP_END - REG_DUMP_START + 1)
	...
	print_hex_dump(KERN_ERR, "OPLUS_CHG[PM8550B]: ", DUMP_PREFIX_OFFSET, 32, 1, data_buf, REG_DUMP_SIZE, false);
...
static int oplus_chg_adsp_dpdm_switch_set_switch_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_dpdm_switch_mode mode)
{
	struct battery_chg_dev *chip;
	int rc;
...
static int oplus_chg_adsp_ufcs_handshake(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (bcdev->charger_type == OPLUS_CHG_USB_TYPE_PD || bcdev->charger_type == OPLUS_CHG_USB_TYPE_PD_DRP ||
...
static void *oplus_chg_adsp_ufcs_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	switch (func_id) {
	case OPLUS_IC_FUNC_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_INIT, oplus_chg_adsp_ufcs_init);
		break;
	case OPLUS_IC_FUNC_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_EXIT, oplus_chg_adsp_ufcs_exit);
		break;
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, oplus_chg_adsp_ufcs_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, oplus_chg_adsp_ufcs_smt_test);
		break;
	case OPLUS_IC_FUNC_UFCS_HANDSHAKE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_HANDSHAKE, oplus_chg_adsp_ufcs_handshake);
		break;
	case OPLUS_IC_FUNC_UFCS_PDO_SET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_PDO_SET, oplus_chg_adsp_ufcs_pdo_set);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_DEV_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_DEV_INFO, oplus_chg_adsp_ufcs_get_dev_info);
		break;
	...
	case OPLUS_IC_FUNC_UFCS_IS_TEST_MODE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_IS_TEST_MODE, oplus_chg_adsp_ufcs_is_test_mode);
		break;
	...
	case OPLUS_IC_FUNC_UFCS_CONFIG_WD:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_CONFIG_WD, oplus_chg_adsp_ufcs_config_wd);
		break;
	case OPLUS_IC_FUNC_UFCS_EXIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_EXIT, oplus_chg_adsp_ufcs_exit_ufcs_mode);
		break;
	case OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_VERIFY_ADAPTER, oplus_chg_adsp_ufcs_verify_adapter);
		break;
	...
	case OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_EMARK_INFO, oplus_chg_adsp_ufcs_get_emark_info);
		break;
	case OPLUS_IC_FUNC_UFCS_GET_SRC_INFO:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_UFCS_GET_SRC_INFO, oplus_chg_adsp_ufcs_get_src_info);
		break;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *chip = container_of(dwork, struct battery_chg_dev, get_regmap_work);
	struct device *dev;
