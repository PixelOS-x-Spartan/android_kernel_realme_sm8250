...
static int mp2650_input_current_limit_write(struct chip_mp2650 *chip, int current_ma)
{
	int i = 0, rc = 0;
	int chg_vol = 0;
	int chg_type = 0;
	...
	if (chg_vol > AICL_POINT_SWITCH_THRE && (0 == oplus_vooc_get_fastchg_started()) &&
...
int mp2650_set_aicl_point(struct oplus_chg_ic_dev *ic_dev, int vbatt)
{
	...
	} else if (chip->hw_aicl_point == HW_AICL_POINT_LOW && vbatt > SWITCH_AICL_POINT_VBAT_HIGH) {
...
static int mp2650_push_buck_err(struct oplus_chg_ic_dev *ic_dev, int reg_value, bool i2c_err, int retry)
{
	oplus_chg_ic_creat_err_msg(ic_dev, OPLUS_IC_ERR_BUCK_BOOST, 0, "REG3F[%x],i2c_err[%d],retry[%d]", reg_value,
				   i2c_err, retry);
...
int mp2650_set_mps_otg_current(void)
{
	...
	rc = mp2650_config_interface(REG07_MP2650_ADDRESS, REG07_MP2650_OTG_CURRENT_LIMIT_1250MA,
...
static void mp2650_plugin_work(struct work_struct *work)
{
	...
	struct chip_mp2650 *chip = container_of(dwork, struct chip_mp2650, plugin_work);
	int tmp1, tmp2;
...
static int mp2650_gpio_init(struct chip_mp2650 *chip)
{
	...
	chip->mps_otg_en_gpio = of_get_named_gpio(node, "oplus,mps_otg_en-gpio", 0);
	...
	if (gpio_is_valid(chip->acok_gpio)) {
		...
		chip->acok_default = pinctrl_lookup_state(chip->pinctrl, "mps_acok_default");
		...
		rc = devm_request_irq(chip->dev, chip->acok_irq, mp2650_acok_handler,
				      IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "mps_acok-irq", chip);
		if (rc < 0) {
...
static int mp2650_reg_dump_internal(void)
{
	...
	printk(KERN_INFO "mp2650_dump_reg: [%02x, %02x, %02x, %02x], [%02x, %02x, %02x, %02x], "
	       ...
	       val_buf[16], val_buf[17], val_buf[18], val_buf[19], val_buf[20], val_buf[21]);

	memset(buck_ic_reg_info, 0, BATTERY_LOG_REG_MAX_SIZE);
	snprintf(buck_ic_reg_info, BATTERY_LOG_REG_MAX_SIZE,
		 ",0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,"
		 "0x%02x,0x%02x,0x%02x,0x%02x",
		 val_buf[0], val_buf[1], val_buf[2], val_buf[4], val_buf[8], val_buf[9], val_buf[19], val_buf[20],
		 val_buf[21]);
...
static int mp2650_input_suspend(struct oplus_chg_ic_dev *ic_dev, bool suspend)
{
	...
	rc = mp2650_config_interface(REG08_MP2650_ADDRESS,
				     suspend ? REG08_MP2650_LEARN_EN_ENABLE : REG08_MP2650_LEARN_EN_DISABLE,
...
static int mp2650_input_is_suspend(struct oplus_chg_ic_dev *ic_dev, bool *suspend)
{
	struct chip_mp2650 *chg_ic;
	int data = 0;
	int rc = 0;
...
static int mp2650_output_is_suspend(struct oplus_chg_ic_dev *ic_dev, bool *suspend)
{
...
static int mp2650_set_icl(struct oplus_chg_ic_dev *ic_dev, bool vooc_mode, bool step, int icl_ma)
{
	struct chip_mp2650 *chip;
	int rc = 0;
...
static int mp2650_curr_drop(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (pre_icl_index < 2) {
		...
		for (i = pre_icl_index - 1; i > 0; i--) {
			switch (i) {
			case 1:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_900MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_900MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
				break;
			case 2:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1100MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1100MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
			...
			case 3:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1400MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1400MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
			...
			case 4:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1600MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1600MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
			...
			case 5:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1900MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS,
							     REG00_MP2650_1ST_CURRENT_LIMIT_1900MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_REG_DUMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_REG_DUMP, mp2650_reg_dump);
		break;
	case OPLUS_IC_FUNC_SMT_TEST:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SMT_TEST, mp2650_smt_test);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_PRESENT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_PRESENT, mp2650_input_present);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND, mp2650_input_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND, mp2650_input_is_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND, mp2650_output_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND, mp2650_output_is_suspend);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ICL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_ICL, mp2650_set_icl);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_ICL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_ICL, mp2650_get_icl);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FCC:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_FCC, mp2650_set_fcc);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FV:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_FV, mp2650_set_fv);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ITERM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_ITERM, mp2650_set_iterm);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL, mp2650_set_rechg_vol);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR, mp2650_get_input_curr);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL, mp2650_get_input_vol);
		break;
	case OPLUS_IC_FUNC_OTG_BOOST_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_OTG_BOOST_ENABLE, mp2650_otg_boost_enable);
		break;
	case OPLUS_IC_FUNC_BUCK_CURR_DROP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_CURR_DROP, mp2650_curr_drop);
		break;
	case OPLUS_IC_FUNC_BUCK_WDT_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_WDT_ENABLE, mp2650_wdt_enable);
		break;
	case OPLUS_IC_FUNC_BUCK_KICK_WDT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_KICK_WDT, mp2650_kick_wdt);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_AICL_POINT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_AICL_POINT, mp2650_set_aicl_point);
		break;
	case OPLUS_IC_FUNC_BUCK_SET_VINDPM:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_SET_VINDPM, mp2650_mms_set_vindpm_vol);
		break;
	case OPLUS_IC_FUNC_BUCK_HARDWARE_INIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_HARDWARE_INIT, mp2650_set_hardware_init);
		break;
	default:
