...
static int tps6128xd_hardware_init(struct chip_tps6128xd *chip)
{
	...
	if (rc >= 0 && (buf[1] == vout_mv_to_reg(chip->vout_mv)) && (buf[2] == vout_mv_to_reg(chip->vout_mv)) &&
...
static int tps6128xd_push_err(struct oplus_chg_ic_dev *ic_dev, bool i2c_error, int err_code, char *reg, bool fault)
{
	...
	if (i2c_error)
		oplus_chg_ic_creat_err_msg(ic_dev, OPLUS_IC_ERR_I2C, 0, "$$err_scene@@i2c_err$$err_reason@@%d",
					   err_code);
	else
...
static ssize_t byb_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (gpio_status != chip->id_match_status) {
		...
		size += snprintf(buf + size, PAGE_SIZE - size, "id not match %d %d,\n", gpio_status,
				 chip->id_match_status);
	}

	rc = regmap_read(chip->regmap, STATUS_REG, &reg_val);
	if (rc < 0) {
		chg_err("can't read 0x%02x register, rc=%d", STATUS_REG, rc);
		size += snprintf(buf + size, PAGE_SIZE - size, "0x%02x read fail:%d|0x%02x read fail:%d\n", STATUS_REG,
				 rc, STATUS_REG, rc);
		return size;
	}
	...
	size += snprintf(buf + size, PAGE_SIZE - size, "%s|0x%02x:0x%02x", byb_status_name[status], STATUS_REG,
			 reg_val);
...
static ssize_t byb_vout_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	struct chip_tps6128xd *chip = oplus_chg_ic_get_drvdata(ic_dev);
	int reg_val = 0;
...
static ssize_t byb_vout_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_ic_dev *ic_dev = dev_get_drvdata(dev);
	struct chip_tps6128xd *chip = oplus_chg_ic_get_drvdata(ic_dev);
	int val = 0;
...
static struct device_attribute dev_attr_byb_vout = {
	.attr = { .name = __stringify(byb_vout), .mode = 0666 },
	.show = byb_vout_show,
	.store = byb_vout_store,
};

static struct device_attribute *tps6128xd_attributes[] = { &dev_attr_byb_status, &dev_attr_byb_vout, NULL };
#endif
