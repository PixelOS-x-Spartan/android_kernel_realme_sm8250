...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, qc_type_check_work.work);
	int type;
	int rc;
	...
	schedule_delayed_work(&bcdev->qc_type_check_work, msecs_to_jiffies(QC_TYPE_CHECK_INTERVAL));
}
...
static void oplus_adsp_voocphy_status_func(struct work_struct *work)
{
	...
	if ((intval & 0xFF) == ADSP_VPHY_FAST_NOTIFY_PRESENT || (intval & 0xFF) == ADSP_VPHY_FAST_NOTIFY_ONGOING) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!((last_usb_adap_type == POWER_SUPPLY_USB_TYPE_PD_PPS || last_usb_adap_type == POWER_SUPPLY_USB_TYPE_PD ||
...
{
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, mcu_en_init_work.work);
...
static int oplus_get_subboard_temp(struct oplus_chg_ic_dev *ic_dev, int *get_temp)
{
	...
	rc = iio_read_channel_processed(bcdev->iio.subboard_temp_chan, &subboard_temp_volt);
	if (rc < 0) {
...
static int oplus_chg_parse_custom_dt(struct battery_chg_dev *bcdev)
{
	...
	rc = of_property_read_u32(node, "oplus,otg_boost_src", &bcdev->otg_boost_src);
	if (rc) {
		bcdev->otg_boost_src = OTG_BOOST_SOURCE_EXTERNAL;
	}

	rc = of_property_read_u32(node, "oplus,otg_curr_limit_max", &bcdev->otg_curr_limit_max);
	if (rc) {
		chg_err("failed to get oplus,otg_curr_limit_max, rc = %d\n", rc);
		bcdev->otg_curr_limit_max = USB_OTG_CURR_LIMIT_MAX;
	}

	rc = of_property_read_u32(node, "oplus,otg_curr_limit_high", &bcdev->otg_curr_limit_high);
	if (rc) {
		chg_err("failed to get oplus,otg_curr_limit_high, rc = %d\n", rc);
		bcdev->otg_curr_limit_high = USB_OTG_CURR_LIMIT_HIGH;
	}

	rc = of_property_read_u32(node, "oplus,otg_real_soc_min", &bcdev->otg_real_soc_min);
	if (rc) {
...
static void oplus_ibus_collapse_rerun_aicl_work(struct work_struct *work)
{
	...
	struct battery_chg_dev *bcdev = container_of(work, struct battery_chg_dev, ibus_collapse_rerun_aicl_work.work);
...
static int oplus_chg_8350_get_usb_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *usb_btb_temp)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_chg_8350_get_batt_btb_temp(struct oplus_chg_ic_dev *ic_dev, int *batt_btb_temp)
{
	struct battery_chg_dev *bcdev;
...
static void *oplus_chg_8350_buck_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT,
					       oplus_chg_8350_set_wls_boost_curr_limit);
		break;
	...
	case OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_USB_BTB_TEMP, oplus_chg_8350_get_usb_btb_temp);
		break;
	case OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_BUCK_GET_BATT_BTB_TEMP, oplus_chg_8350_get_batt_btb_temp);
		break;
...
static int oplus_chg_8350_get_afi_update_done(struct oplus_chg_ic_dev *ic_dev, bool *status)
{
	struct battery_chg_dev *chip;
...
static int oplus_chg_8350_soft_reset(struct oplus_chg_ic_dev *ic_dev, bool *reset_done)
{
	struct battery_chg_dev *chip;
	int ret;
...
static int oplus_chg_8350_detection_reset_condition(struct oplus_chg_ic_dev *ic_dev, bool *need_reset)
{
	struct battery_chg_dev *chip;
...
static void *oplus_chg_8350_gauge_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_GAUGE_SET_RESET:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_SET_RESET, oplus_chg_8350_soft_reset);
		break;
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	bcdev->voocphy_bidirect_cp_support = of_property_read_bool(dev->of_node, "oplus,voocphy_bidirect_cp_support");
