...
extern int tcpc_mt6375_reg_debug_msg_handler(struct tcpc_device *tcpc, void (*msg_handler)(void *, int),
					     void *priv_data);
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_rvp_work);
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VCONN_RVP,
				   "vbus_err[%d],vcoon_en[%d],cc1[%d],cc2[%d]", vbus_err, vcoon_en, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}

static void mt6375_vcoon_ocp_work(struct work_struct *work)
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_ocp_work);
	int vbus_err, vcoon_en, cc1, cc2;
	int rc;
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VCONN_OCP,
				   "vbus_err[%d],vcoon_en[%d],cc1[%d],cc2[%d]", vbus_err, vcoon_en, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_ovp_work);
	int vbus_err, vcoon_en, cc1, cc2;
	int rc;
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VCONN_OVP,
				   "vbus_err[%d],vcoon_en[%d],cc1[%d],cc2[%d]", vbus_err, vcoon_en, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_uvp_work);
	int vbus_err, vcoon_en, cc1, cc2;
	int rc;
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VCONN_UVP,
				   "vbus_err[%d],vcoon_en[%d],cc1[%d],cc2[%d]", vbus_err, vcoon_en, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_open_work);
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, vcoon_close_work);
	int time = -EINVAL;
	int cc1, cc2;
	int rc;
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VCONN_CLOSE,
				   "open_time[%d],cc1[%d],cc2[%d]", time, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}
...
{
	struct mt6375_device *chip = container_of(work, struct mt6375_device, power_status_change_work);
	...
	oplus_chg_ic_creat_err_msg(chip->ic_dev, OPLUS_IC_ERR_PLAT_PMIC, PLAT_PMIC_ERR_VBUS_ABNORMAL,
				   "vbus_err[%d],vcoon_en[%d],cc1[%d],cc2[%d]", vbus_err, vcoon_en, cc1, cc2);
	oplus_chg_ic_virq_trigger(chip->ic_dev, OPLUS_IC_VIRQ_ERR);
}
...
static int mt6375_probe(struct platform_device *pdev)
{
	...
	chip = devm_kzalloc(&pdev->dev, sizeof(struct mt6375_device), GFP_KERNEL);
	if (chip == NULL) {
	...
	INIT_WORK(&chip->power_status_change_work, mt6375_power_status_change_work);
	chip->vconn_en_time = -EINVAL;

	rc = tcpc_mt6375_reg_debug_msg_handler(chip->tcpc, mt6375_debug_msg_handler, chip);
	if (rc < 0) {
