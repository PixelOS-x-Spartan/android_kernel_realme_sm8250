...
static bool oplus_pd_without_usb(struct pd_manager_chip *chip)
{
	if (!tcpm_inquire_pd_connected(chip->tcpc))
		return true;
	return (tcpm_inquire_dpm_flags(chip->tcpc) & DPM_FLAGS_PARTNER_USB_COMM) ? false : true;
}
...
static void tcpc_set_data_role(struct pd_manager_chip *chip, enum typec_data_role role)
{
...
static void tcpc_get_adapter_svid(struct pd_manager_chip *chip)
{
	...
	if (chip->pd_svooc)
		schedule_delayed_work(&chip->vconn_wait_work, msecs_to_jiffies(VCONN_TIMEOUT_MS));
	else
...
static int extcon_init(struct pd_manager_chip *chip)
{
	...
	extcon_set_property_capability(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_SS);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
	extcon_set_property_capability(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT);
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) */
	extcon_set_property_capability(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS);
out:
	return ret;
}
...
static inline void start_usb_peripheral(struct pd_manager_chip *chip)
{
	...
	if (!chip->bc12_completed) {
		...
		schedule_delayed_work(&chip->bc12_wait_work, msecs_to_jiffies(BC12_TIMEOUT_MS));
		return;
	}
	...
	val.intval = tcpm_inquire_cc_polarity(chip->tcpc);
	extcon_set_property(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY, val);
	...
	extcon_set_property(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT, val);
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) */
...
static void tcpc_bc12_wait_work(struct work_struct *work)
{
	...
	struct pd_manager_chip *chip = container_of(dwork, struct pd_manager_chip, bc12_wait_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pd_manager_chip *chip = container_of(dwork, struct pd_manager_chip, vconn_wait_work);
...
static void usb_dwork_handler(struct work_struct *work)
{
	int ret = 0;
	struct delayed_work *dwork = to_delayed_work(work);
	struct pd_manager_chip *chip = container_of(dwork, struct pd_manager_chip, usb_dwork);
	...
	case DR_DEVICE:
		...
		chg_info("polling_cnt=%d, ret=%d type=%d, bc12_ready=%d\n", ++chip->usb_type_polling_cnt, ret,
			 chip->chg_type, chip->bc12_ready);
		if (!chip->bc12_ready || chip->chg_type == OPLUS_CHG_USB_TYPE_UNKNOWN) {
			if (chip->usb_type_polling_cnt < USB_TYPE_POLLING_CNT_MAX)
				schedule_delayed_work(&chip->usb_dwork, msecs_to_jiffies(USB_TYPE_POLLING_INTERVAL));
			break;
		} else if (chip->chg_type == OPLUS_CHG_USB_TYPE_SDP || chip->chg_type == OPLUS_CHG_USB_TYPE_CDP ||
...
static void pd_sink_set_vol_and_cur(struct pd_manager_chip *chip, int mv, int ma, uint8_t type)
{
	unsigned long sel = 0;
	int max, min;
...
static int tcpc_pd_state_change(struct pd_manager_chip *chip, struct tcp_notify *noti)
{
	...
	case PD_CONNECT_PE_READY_SRC_PD30:
		...
		pd_type = noti->pd_state.connected == PD_CONNECT_PE_READY_SNK_APDO ? OPLUS_CHG_USB_TYPE_PD_PPS :
										     OPLUS_CHG_USB_TYPE_PD;
		tcpc_set_pd_type(chip, pd_type);
		pd_sink_set_vol_and_cur(chip, chip->sink_mv_old, chip->sink_ma_old, TCP_VBUS_CTRL_PD_STANDBY);
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	struct pd_manager_chip *chip = container_of(nb, struct pd_manager_chip, pd_nb);
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if ((chip->sink_mv_new != chip->sink_mv_old) || (chip->sink_ma_new != chip->sink_ma_old)) {
		...
		if (noti->vbus_state.type & TCP_VBUS_CTRL_PD_DETECT)
			pd_sink_set_vol_and_cur(chip, chip->sink_mv_new, chip->sink_ma_new, noti->vbus_state.type);
		break;
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
		    (new_state == TYPEC_ATTACHED_SNK || new_state == TYPEC_ATTACHED_NORP_SRC ||
		     new_state == TYPEC_ATTACHED_CUSTOM_SRC || new_state == TYPEC_ATTACHED_DBGACC_SNK)) {
			...
			opmode = noti->typec_state.rp_level - TYPEC_CC_VOLT_SNK_DFT;
		...
		} else if ((old_state == TYPEC_ATTACHED_SNK || old_state == TYPEC_ATTACHED_NORP_SRC ||
			    old_state == TYPEC_ATTACHED_CUSTOM_SRC || old_state == TYPEC_ATTACHED_DBGACC_SNK) &&
			...
			schedule_delayed_work(&chip->usb_dwork, 0);
			   ...
			   (new_state == TYPEC_ATTACHED_SRC || new_state == TYPEC_ATTACHED_DEBUG)) {
		...
		} else if ((old_state == TYPEC_ATTACHED_SRC || old_state == TYPEC_ATTACHED_DEBUG) &&
			   new_state == TYPEC_UNATTACHED) {
			...
			chip->usb_dr = DR_IDLE;
			schedule_delayed_work(&chip->usb_dwork, 0);
		} else if (old_state == TYPEC_UNATTACHED && new_state == TYPEC_ATTACHED_AUDIO) {
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			case TYPEC_ATTACHED_CUSTOM_SRC:
				chip->partner_desc.accessory = TYPEC_ACCESSORY_DEBUG;
				break;
			default:
		...
		} else if (new_state == TYPEC_ATTACHED_CUSTOM_SRC || new_state == TYPEC_ATTACHED_DBGACC_SNK) {
			switch (noti->typec_state.rp_level) {
			/* DAM_3000 */
			case TYPEC_CC_VOLT_SNK_3_0:
				break;
...
{
	struct pd_manager_chip *chip = container_of(cap, struct pd_manager_chip, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
...
static int tcpc_typec_dr_set(struct typec_port *port, enum typec_data_role role)
{
	struct pd_manager_chip *chip = typec_get_drvdata(port);
#else
static int tcpc_typec_dr_set(const struct typec_capability *cap, enum typec_data_role role)
{
	struct pd_manager_chip *chip = container_of(cap, struct pd_manager_chip, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
...
static int tcpc_typec_pr_set(struct typec_port *port, enum typec_role role)
{
	struct pd_manager_chip *chip = typec_get_drvdata(port);
#else
static int tcpc_typec_pr_set(const struct typec_capability *cap, enum typec_role role)
{
	struct pd_manager_chip *chip = container_of(cap, struct pd_manager_chip, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
...
static int tcpc_typec_vconn_set(struct typec_port *port, enum typec_role role)
{
	struct pd_manager_chip *chip = typec_get_drvdata(port);
#else
static int tcpc_typec_vconn_set(const struct typec_capability *cap, enum typec_role role)
{
	struct pd_manager_chip *chip = container_of(cap, struct pd_manager_chip, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
	int ret = 0;
...
#else
static int tcpc_typec_port_type_set(const struct typec_capability *cap, enum typec_port_type type)
{
	struct pd_manager_chip *chip = container_of(cap, struct pd_manager_chip, typec_caps);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) */
...
static int oplus_pdo_select(struct pd_manager_chip *chip, int vbus_mv, int ibus_ma)
{
	...
	if (chip->pd_type == OPLUS_CHG_USB_TYPE_PD_PPS) {
		while (1) {
			ret = tcpm_inquire_pd_source_apdo(chip->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS, &cap_i, &apdo_cap);
		...
		if (pd_cap.nr != 0) {
			for (i = 0; i < pd_cap.nr; i++) {
				...
				chg_info("%d mv:[%d,%d] type:%d %d\n", i, pd_cap.min_mv[i], pd_cap.max_mv[i],
...
static int pd_manager_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[], int len)
{
	return 0;
}

static int pd_manager_get_charger_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
...
static int pd_manager_get_cc_orientation(struct oplus_chg_ic_dev *ic_dev, int *orientation)
{
	struct pd_manager_chip *chip;
	int val;
...
static int pd_manager_get_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type *mode)
{
	struct pd_manager_chip *chip;
...
static int pd_manager_set_typec_mode(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_typec_port_role_type mode)
{
	struct pd_manager_chip *chip;
	uint8_t typec_role;
	int rc;
...
static int pd_manager_get_charger_vol_max(struct oplus_chg_ic_dev *ic_dev, int *vol)
{
	struct pd_manager_chip *chip;
...
static int pd_manager_get_charger_vol_min(struct oplus_chg_ic_dev *ic_dev, int *vol)
{
	struct pd_manager_chip *chip;
...
static int pd_manager_get_charger_curr_max(struct oplus_chg_ic_dev *ic_dev, int *curr)
{
	struct pd_manager_chip *chip;
...
static int pd_manager_bc12_completed(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (first_boot) {
		...
		if (chip->chg_type == OPLUS_CHG_USB_TYPE_SDP || chip->chg_type == OPLUS_CHG_USB_TYPE_CDP ||
		    chip->chg_type == OPLUS_CHG_USB_TYPE_PD_SDP)
			chip->start_peripheral = true;
	}
...
static void *oplus_chg_get_func(struct oplus_chg_ic_dev *ic_dev, enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_GET_TYPEC_ROLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_TYPEC_ROLE, pd_manager_get_typec_mode);
		break;
	default:
...
static void tcpc_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void tcpc_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs = oplus_mms_subscribe(chip->wired_topic, chip, tcpc_wired_subs_callback, "tcpc");
...
reg_ic_err:
	unregister_tcp_dev_notifier(chip->tcpc, &chip->pd_nb, TCP_NOTIFY_TYPE_ALL);
...
static int oplus_pd_manager_remove(struct platform_device *pdev)
{
	...
	ret = unregister_tcp_dev_notifier(chip->tcpc, &chip->pd_nb, TCP_NOTIFY_TYPE_ALL);
	if (ret < 0)
...
static const struct of_device_id oplus_pd_manager_of_match[] = { { .compatible = "oplus,hal-pd-manager" }, {} };
