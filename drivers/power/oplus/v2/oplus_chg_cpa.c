...
const char *const protocol_name_str[] = {
	[CHG_PROTOCOL_BC12] = "BC1.2", [CHG_PROTOCOL_PD] = "PD",     [CHG_PROTOCOL_PPS] = "PPS",
	[CHG_PROTOCOL_VOOC] = "VOOC",  [CHG_PROTOCOL_UFCS] = "UFCS", [CHG_PROTOCOL_QC] = "QC",
};
...
static int oplus_cpa_get_final_chg_type(struct oplus_cpa *cpa, bool update)
{
	...
	chg_debug("wired_type=%s, protocol_type=%s, current_protocol_type=%s\n",
		  oplus_wired_get_chg_type_str(wired_type), get_protocol_name_str(protocol_type),
...
static void oplus_cpa_protocol_switch_work(struct work_struct *work)
{
	...
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, protocol_switch_work);
	uint32_t protocol;
	int rc;
...
{
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, switch_end_work);
	struct mms_msg *msg;
	enum oplus_chg_protocol_type type;
	int rc;
...
{
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, chg_type_change_work);
	union mms_msg_data data = { 0 };
	struct mms_msg *msg;
	int wired_type;
	int rc;
...
{
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, fast_chg_type_change_work);
	struct mms_msg *msg;
	int rc;
...
static void oplus_cpa_protocol_switch_timeout_work(struct work_struct *work)
{
	...
	struct oplus_cpa *cpa = container_of(dwork, struct oplus_cpa, protocol_switch_timeout_work);
	struct mms_msg *msg;
	int rc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_cpa *cpa = container_of(dwork, struct oplus_cpa, protocol_ready_timeout_work);

	chg_err("wait protocol ready timeout, ready_protocol_type=0x%x, request_pending=%d\n", cpa->ready_protocol_type,
		cpa->request_pending);
...
{
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, wired_offline_work);
	int i;
	...
	for (i = 0; i < CHG_PROTOCOL_MAX; i++) {
		if (cpa->protocol_prio_table[i].type != CHG_PROTOCOL_INVALID)
			oplus_cpa_protocol_clear_power(cpa->cpa_topic, cpa->protocol_prio_table[i].type);
	}
}

static void oplus_cpa_wired_online_work(struct work_struct *work)
{
	struct oplus_cpa *cpa = container_of(work, struct oplus_cpa, wired_online_work);
	union mms_msg_data data = { 0 };
	int rc;
...
static void oplus_cpa_wired_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
...
static void oplus_cpa_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	cpa->wired_subs = oplus_mms_subscribe(cpa->wired_topic, cpa, oplus_cpa_wired_subs_callback, "cpa");
...
static void oplus_cpa_vooc_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_cpa *cpa = subs->priv_data;
	union mms_msg_data data = { 0 };
	...
	case MSG_TYPE_ITEM:
		switch (id) {
		case VOOC_ITEM_SID:
			oplus_mms_get_item_data(cpa->vooc_topic, id, &data, false);
...
static void oplus_cpa_subscribe_vooc_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_cpa *cpa = prv_data;
	union mms_msg_data data = { 0 };

	cpa->vooc_topic = topic;
	cpa->vooc_subs = oplus_mms_subscribe(cpa->vooc_topic, cpa, oplus_cpa_vooc_subs_callback, "cpa");
...
static void oplus_cpa_ufcs_subs_callback(struct mms_subscribe *subs, enum mms_msg_type type, u32 id)
{
	struct oplus_cpa *cpa = subs->priv_data;
	union mms_msg_data data = { 0 };
...
static void oplus_cpa_subscribe_ufcs_topic(struct oplus_mms *topic, void *prv_data)
{
	struct oplus_cpa *cpa = prv_data;
	union mms_msg_data data = { 0 };

	cpa->ufcs_topic = topic;
	cpa->ufcs_subs = oplus_mms_subscribe(topic, cpa, oplus_cpa_ufcs_subs_callback, "cpa");
...
static int oplus_cpa_probe(struct platform_device *pdev)
{
	...
	schedule_delayed_work(&cpa->protocol_ready_timeout_work, msecs_to_jiffies(PROTOCAL_READY_TIMEOUT_MS));
...
enum oplus_chg_protocol_type oplus_cpa_curr_high_prio_protocol_type(struct oplus_mms *topic)
{
	...
	if ((cpa->current_protocol_type > CHG_PROTOCOL_INVALID) && (cpa->current_protocol_type < CHG_PROTOCOL_MAX))
