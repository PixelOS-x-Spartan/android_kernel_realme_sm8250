...
#define MSM_ACCESSOR(name)                                                                                             \
	static u32 msm_readl_##name(struct msm_pinctrl *pctrl, const struct msm_pingroup *g)                           \
	{                                                                                                              \
...
/* common test func */
bool test_kit_typec_port_test(struct test_feature *feature, char *buf, size_t len)
{
	...
	if (g_test_kit == NULL) {
		...
		index += snprintf(buf + index, len - index, "g_test_kit is NULL");
		return false;
	}
	if (g_test_kit->typec_port_check == NULL) {
		pr_err("typec_port_check is NULL\n");
		index += snprintf(buf + index, len - index, "typec port check func is NULL");
		return false;
	}

	typec_port_info = feature->cfg->test_info;
	while (typec_port_info->name) {
		if (!g_test_kit->typec_port_check((void *)typec_port_info, buf + index, len - index, &use_size))
			pass = false;
		index += use_size;
		if (index >= len) {
...
bool test_kit_qcom_soc_gpio_test(struct test_feature *feature, char *buf, size_t len)
{
	...
	if (g_test_kit->qcom_soc_gpio_check == NULL) {
		...
		index += snprintf(buf + index, len - index, "gpio check func is NULL");
		return false;
	}

	gpio_info = feature->cfg->test_info;
	while (gpio_info->name) {
		if (!g_test_kit->qcom_soc_gpio_check((void *)gpio_info, buf + index, len - index, &use_size))
			pass = false;
		index += use_size;
		if (index >= len) {
			pr_err("str buf overflow\n");
...
bool test_kit_mtk_soc_gpio_test(struct test_feature *feature, char *buf, size_t len)
{
	struct test_kit_soc_gpio_info *gpio_info;
	size_t index = 0;
	size_t use_size;
	...
	gpio_info = feature->cfg->test_info;
	while (gpio_info->name) {
		if (!g_test_kit->mtk_soc_gpio_check((void *)gpio_info, buf + index, len - index, &use_size))
			pass = false;
		index += use_size;
		if (index >= len) {
			pr_err("str buf overflow\n");
...
struct test_feature *__must_check test_feature_register(const struct test_feature_cfg *cfg, void *private_data)
{
	struct test_feature *feature;
...
bool test_kit_mtk_gpio_check(void *info, char *buf, size_t len, size_t *use_size)
{
	...
	*use_size = 0;
	...
	do {
		...
		if (gdesc && !strncmp(pinctrl_name, gdesc->gdev->chip->label, strlen(pinctrl_name))) {
		...
		if (pin == 0 || !gdesc) {
			...
			*use_size += snprintf(buf + *use_size, len - *use_size, "[%s][gpio%u]:gpio_chip is NULL\n",
	...
	if (ret || !!val != gpio_info->is_high) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      ...
				      gpio_info->is_high ? "high" : "low", ret ? "null" : (val ? "high" : "low"));
		pass = false;
	}
	...
	if (ret || val < gpio_info->drive) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      ...
				      offset, gpio_info->drive, ret ? -1 : val);
		pass = false;
	}
	...
	if (ret) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      ...
				      offset, gpio_info->is_out ? "out" : "in", "null");
		return false;
	}
	...
	if (val != gpio_info->is_out) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      "[%s][gpio%u][direction error]:expected:%s, actually:%s\n", gpio_info->name,
				      offset, gpio_info->is_out ? "out" : "in", val ? "out" : "in");
		return false;
	}
	...
	if (!gpio_info->is_out) {
		...
		if (ret) {
			*use_size += snprintf(buf + *use_size, len - *use_size,
					      ...
					      offset, pulls_no_keeper[gpio_info->pull], "null");
			pass = false;
		} else if (!gpio_info->pull) {
			if (val_pullen) {
				*use_size += snprintf(buf + *use_size, len - *use_size,
						      "[%s][gpio%u][pull error]:expected:%s, actually:%s\n",
						      gpio_info->name, offset, pulls_no_keeper[gpio_info->pull],
			...
			*use_size += snprintf(buf + *use_size, len - *use_size,
					      "[%s][gpio%u][pull error]:expected:%s, actually:%s\n", gpio_info->name,
					      offset, pulls_no_keeper[gpio_info->pull], pulls_no_keeper[0]);
			pass = false;
		} else if (gpio_info->pull != val_pullsel + 1) {
			*use_size +=
				snprintf(buf + *use_size, len - *use_size,
					 "[%s][gpio%u][pull error]:expected:%s, actually:%s\n", gpio_info->name, offset,
					 pulls_no_keeper[gpio_info->pull], pulls_no_keeper[val_pullsel + 1]);
			pass = false;
		}
	}
...
bool test_kit_qcom_gpio_check(void *info, char *buf, size_t len, size_t *use_size)
{
	...
	static const char *const pulls_keeper[] = { "no pull", "pull down", "keeper", "pull up" };
	...
	*use_size = 0;
	...
	if (!gpiochip_line_is_valid(chip, offset)) {
		*use_size += snprintf(buf + *use_size, len - *use_size, "[%s][gpio%u]:is invalid\n", gpio_info->name,
				      offset);
		return false;
	}
	...
	if (is_out != gpio_info->is_out) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      "[%s][gpio%u][direction error]:expected:%s, actually:%s\n", gpio_info->name,
				      offset, gpio_info->is_out ? "out" : "in", is_out ? "out" : "in");
		pass = false;
	}
	...
	if (!!val != gpio_info->is_high) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      "[%s][gpio%u][level error]:expected:%s, actually:%s\n", gpio_info->name, offset,
				      gpio_info->is_high ? "high" : "low", val ? "high" : "low");
		pass = false;
	}
	...
	if (msm_regval_to_drive(drive) != gpio_info->drive) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      "[%s][gpio%u][drive error]:expected:%dmA, actually:%dmA\n", gpio_info->name,
				      offset, gpio_info->drive, msm_regval_to_drive(drive));
		pass = false;
	}
	...
	if (pull != gpio_info->pull) {
		if (pctrl->soc->pull_no_keeper)
			*use_size += snprintf(buf + *use_size, len - *use_size,
					      "[%s][gpio%u][pull error]:expected:%s, actually:%s\n", gpio_info->name,
					      offset, pulls_no_keeper[gpio_info->pull], pulls_no_keeper[pull]);
		else
			*use_size += snprintf(buf + *use_size, len - *use_size,
					      "[%s][gpio%u][pull error]:expected:%s, actually:%s\n", gpio_info->name,
					      offset, pulls_keeper[gpio_info->pull], pulls_keeper[pull]);
		pass = false;
	}
...
static ssize_t test_kit_get_feature_list(struct test_kit *test_kit, char *buf, size_t len)
{
	struct test_feature *feature;
	ssize_t index = 0;
	int i = 0;
...
{
	struct test_kit *test_kit = container_of(filp->private_data, struct test_kit, test_dev);

	filp->private_data = test_kit;
	return 0;
}

static ssize_t test_kit_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
...
static long test_kit_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct test_kit *test_kit = filp->private_data;
	...
	case TEST_KIT_GET_INDEX:
		rc = copy_to_user(argp, &test_kit->feature_index, sizeof(unsigned int));
		if (rc) {
	...
	case TEST_KIT_GET_FEATURE_LIST:
		...
		rc = test_kit_get_feature_list(test_kit, str_buf, STRING_BUF_SIZE);
		if (rc < 0) {
	...
	case TEST_KIT_GET_FEATURE_NUM:
		rc = copy_to_user(argp, &test_kit->feature_num, sizeof(unsigned int));
		if (rc) {
	...
	case TEST_KIT_RUN_TEST:
		...
		pass = test_kit->feature->cfg->test_func(test_kit->feature, str_buf, STRING_BUF_SIZE);
		rc = copy_to_user(argp, str_buf, STRING_BUF_SIZE);
		if (rc) {
...
static ssize_t test_kit_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	return count;
}
