...
static int oplus_chg_strategy_get_index(struct oplus_chg_strategy *strategy, int temp)
{
	int i;
...
int oplus_chg_get_chg_strategy_data_len(struct oplus_chg_strategy_data data[], int max_len)
{
	...
	for (i = 0; i < max_len; i++) {
		if (data[i].cool_temp == 0 && data[i].heat_temp == 0 && data[i].curr_data == 0 &&
		    data[i].heat_next_index == 0 && data[i].cool_next_index == 0)
			return i;
	}
			     ...
			     int temp)
{
	...
	strategy->temp_max = strategy->table_data[strategy->table_size - 1].heat_temp;
	strategy->temp_region.temp = temp;
	strategy->temp_region.index = oplus_chg_strategy_get_index(strategy, temp);
				...
				int temp)
{
	...
	while (temp_diff != 0) {
		if (temp_diff > 0 && temp > strategy->table_data[strategy->temp_region.index].heat_temp) {
			strategy->temp_region.index = strategy->table_data[strategy->temp_region.index].heat_next_index;
			if (temp > strategy->table_data[strategy->temp_region.index].heat_temp &&
				...
				strategy->temp_region.temp = temp;
		} else if (temp_diff < 0 && temp <= strategy->table_data[strategy->temp_region.index].cool_temp) {
			strategy->temp_region.index = strategy->table_data[strategy->temp_region.index].cool_next_index;
