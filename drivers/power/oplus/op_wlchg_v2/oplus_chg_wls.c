...
static u8 oplus_trx_id_table[] = { 0x02, 0x03, 0x04, 0x05, 0x06 };

/*static int oplus_chg_wls_pwr_table[] = {0, 12000, 12000, 35000, 50000};*/
static struct wls_pwr_table oplus_chg_wls_pwr_table[] = {
...
static const char *const oplus_chg_wls_rx_state_text[] = {
	[OPLUS_CHG_WLS_RX_STATE_DEFAULT] = "default", [OPLUS_CHG_WLS_RX_STATE_BPP] = "bpp",
	[OPLUS_CHG_WLS_RX_STATE_EPP] = "epp",	      [OPLUS_CHG_WLS_RX_STATE_EPP_PLUS] = "epp-plus",
	[OPLUS_CHG_WLS_RX_STATE_FAST] = "fast",	      [OPLUS_CHG_WLS_RX_STATE_FFC] = "ffc",
	[OPLUS_CHG_WLS_RX_STATE_DONE] = "done",	      [OPLUS_CHG_WLS_RX_STATE_QUIET] = "quiet",
	[OPLUS_CHG_WLS_RX_STATE_STOP] = "stop",	      [OPLUS_CHG_WLS_RX_STATE_DEBUG] = "debug",
	[OPLUS_CHG_WLS_RX_STATE_FTM] = "ftm",	      [OPLUS_CHG_WLS_RX_STATE_ERROR] = "error",
};
...
static bool oplus_chg_wls_track_set_prop(struct oplus_chg_wls *wls_dev, enum oplus_chg_mod_property prop, int val)
{
	...
	pval.intval = val;
	rc = oplus_chg_mod_set_property(wls_dev->track_ocm, prop, &pval);
	if (rc < 0)
		return false;
...
static int oplus_chg_wls_set_wrx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (IS_ERR_OR_NULL(wls_dev->pinctrl) || IS_ERR_OR_NULL(wls_dev->wrx_en_active) ||
	    IS_ERR_OR_NULL(wls_dev->wrx_en_sleep)) {
		pr_err("wrx_en pinctrl error\n");
		return -ENODEV;
	}

	rc = pinctrl_select_state(wls_dev->pinctrl, en ? wls_dev->wrx_en_active : wls_dev->wrx_en_sleep);
	if (rc < 0)
...
static int oplus_chg_wls_fcc_vote_callback(struct votable *votable, void *data, int fcc_ma, const char *client,
					   bool step)
{
...
static int oplus_chg_wls_fastchg_disable_vote_callback(struct votable *votable, void *data, int disable,
...
static int oplus_chg_wls_wrx_en_vote_callback(struct votable *votable, void *data, int en, const char *client,
					      bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_nor_icl_vote_callback(struct votable *votable, void *data, int icl_ma, const char *client,
					       bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_nor_fcc_vote_callback(struct votable *votable, void *data, int fcc_ma, const char *client,
					       bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_nor_fv_vote_callback(struct votable *votable, void *data, int fv_mv, const char *client,
					      bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_nor_out_disable_vote_callback(struct votable *votable, void *data, int disable,
						       const char *client, bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_nor_input_disable_vote_callback(struct votable *votable, void *data, int disable,
							 const char *client, bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_rx_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client,
						  bool step)
{
	struct oplus_chg_wls *wls_dev = data;
	int rc;
...
static int oplus_chg_wls_set_ext_pwr_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (IS_ERR_OR_NULL(wls_dev->pinctrl) || IS_ERR_OR_NULL(wls_dev->ext_pwr_en_active) ||
	    IS_ERR_OR_NULL(wls_dev->ext_pwr_en_sleep)) {
		pr_err("ext_pwr_en pinctrl error\n");
		return -ENODEV;
	}

	rc = pinctrl_select_state(wls_dev->pinctrl, en ? wls_dev->ext_pwr_en_active : wls_dev->ext_pwr_en_sleep);
	if (rc < 0)
...
static void oplus_chg_wls_get_third_part_verity_data_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, wls_get_third_part_verity_data_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;

	oplus_chg_common_set_mutual_cmd(wls_dev->comm_ocm, CMD_WLS_THIRD_PART_AUTH, sizeof(wls_status->vendor_id),
					&(wls_status->vendor_id));
}

static void oplus_chg_wls_standard_msg_handler(struct oplus_chg_wls *wls_dev, u8 mask, u8 data)
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	...
	case WLS_RESPONE_ADAPTER_TYPE:
		if (rx_msg->msg_type == WLS_CMD_INDENTIFY_ADAPTER) {
			...
			oplus_vote(wls_dev->fcc_votable, BASE_MAX_VOTER, true, (pwr_mw * 1000 / WLS_RX_VOL_MAX_MV),
				   false);
			if (wls_dev->wls_ocm)
				oplus_chg_mod_changed(wls_dev->wls_ocm);
		...
		if (rx_msg->msg_type == WLS_CMD_INTO_FASTCHAGE) {
			...
			if (wls_status->adapter_id != WLS_ADAPTER_THIRD_PARTY)
				...
				wls_status->pwr_max_mw =
					oplus_chg_wls_get_tripartite_r_power(wls_status->adapter_power);
			if (wls_status->pwr_max_mw > 0) {
				oplus_vote(wls_dev->fcc_votable, RX_MAX_VOTER, true,
					   (wls_status->pwr_max_mw * 1000 / WLS_RX_VOL_MAX_MV), false);
			}
			...
			if (wls_dev->static_config.fastchg_fod_enable) {
				if (wls_status->fod_parm_for_fastchg)
					(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx,
									    wls_dev->static_config.fastchg_fod_parm,
									    WLS_FOD_PARM_LENGTH);
				...
				(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx, oplus_chg_wls_disable_fod_parm,
								    WLS_FOD_PARM_LENGTH);
			}
			if (!wls_status->verity_started)
				oplus_vote(wls_dev->fcc_votable, VERITY_VOTER, true, dynamic_cfg->verity_curr_max_ma,
					   false);
		}
		break;
...
static void oplus_chg_wls_data_msg_handler(struct oplus_chg_wls *wls_dev, u8 mask, u8 data[3])
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	struct oplus_chg_rx_msg *rx_msg = &wls_dev->rx_msg;
	bool msg_ok = false;
...
static void oplus_chg_wls_extended_msg_handler(struct oplus_chg_wls *wls_dev, u8 data[])
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	struct oplus_chg_rx_msg *rx_msg = &wls_dev->rx_msg;
	bool msg_ok = false;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, wls_send_msg_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	struct oplus_chg_rx_msg *rx_msg = &wls_dev->rx_msg;
	int delay_ms = 1000;
	int cep;
	...
	if (rx_msg->long_data)
		rc = oplus_chg_wls_rx_send_data(wls_dev->wls_rx, rx_msg->msg_type, rx_msg->buf,
...
static int oplus_chg_wls_send_msg(struct oplus_chg_wls *wls_dev, u8 msg, u8 data, int wait_time_s)
{
	...
	if (!wls_dev->msg_callback_ok) {
		rc = oplus_chg_wls_rx_register_msg_callback(wls_dev->wls_rx, wls_dev, oplus_chg_wls_rx_msg_callback);
		if (rc < 0) {
...
retry:
	...
	pr_info("encrypt_data: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x",
		wls_status->encrypt_data[0], wls_status->encrypt_data[1], wls_status->encrypt_data[2],
		wls_status->encrypt_data[3], wls_status->encrypt_data[4], wls_status->encrypt_data[5],
		wls_status->encrypt_data[6], wls_status->encrypt_data[7]);

	if (memcmp(&wls_status->encrypt_data, &wls_status->verfity_data.encode_num, WLS_AUTH_ENCODE_LEN)) {
		pr_err("verity faile\n");
		wls_status->verity_pass = false;
	} else {
		pr_err("verity pass\n");
	...
	wls_status->verity_count = 0;
	if (!wls_status->verity_pass && wls_status->rx_online && time_before(jiffies, wls_status->verity_wait_timeout))
		goto retry;
...
retry:
	if (!wls_status->aes_verity_data_ok) {
		pr_err("verity data not ready\n");
		if (is_comm_ocm_available(wls_dev))
			ret = oplus_chg_common_set_mutual_cmd(wls_dev->comm_ocm, CMD_WLS_THIRD_PART_AUTH,
	...
	pr_info("aes encrypt_data: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x \
		...
		wls_status->aes_encrypt_data[0], wls_status->aes_encrypt_data[1], wls_status->aes_encrypt_data[2],
		wls_status->aes_encrypt_data[3], wls_status->aes_encrypt_data[4], wls_status->aes_encrypt_data[5],
		wls_status->aes_encrypt_data[6], wls_status->aes_encrypt_data[7], wls_status->aes_encrypt_data[8],
		wls_status->aes_encrypt_data[9], wls_status->aes_encrypt_data[10], wls_status->aes_encrypt_data[11],
		wls_status->aes_encrypt_data[12], wls_status->aes_encrypt_data[13], wls_status->aes_encrypt_data[14],
		wls_status->aes_encrypt_data[15]);

	if (memcmp(&wls_status->aes_encrypt_data, &wls_status->aes_verfity_data.aes_encode_num,
		   WLS_AUTH_AES_ENCODE_LEN)) {
		pr_err("verity faile\n");
		wls_status->verity_pass = false;
	} else {
		pr_err("verity pass\n");
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, wls_verity_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
...
static void oplus_chg_wls_exchange_batt_mesg(struct oplus_chg_wls *wls_dev)
{
	...
	wls_status = &wls_dev->wls_status;
	if (!wls_status->rx_online || !wls_status->aes_verity_done || wls_status->adapter_id != WLS_ADAPTER_THIRD_PARTY)
		return;
...
#endif
	static int
	oplus_chg_wls_get_skewing_current(struct oplus_chg_wls *wls_dev)
{
	int cep_curr_ma = 0;
...
static void oplus_chg_wls_update_track_info(struct oplus_chg_wls *wls_dev, char *crux_info, bool clear)
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	int batt_temp = 0;
	u32 rx_version = 0;
	...
	if (crux_info) {
		index += snprintf(&crux_info[index], OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				  ...
				  trx_version, rx_version, wls_status->adapter_type, wls_status->adapter_id,
				  wls_status->fastchg_started, wls_status->vout_mv, wls_status->iout_ma,
				  wls_status->break_count, wls_status->trx_err, highest_temp, max_iout, min_cool_down,
...
static int oplus_chg_wls_track_upload_trx_general_info(struct oplus_chg_wls *wls_dev, char *trx_crux_info,
						       bool usb_present_once)
{
	int index = 0;
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;

	memset(wls_dev->trx_info_load_trigger.crux_info, 0, sizeof(wls_dev->trx_info_load_trigger.crux_info));

	index += snprintf(&(wls_dev->trx_info_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "%s", "$$power_mode@@wireless");
	index += snprintf(&(wls_dev->trx_info_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$total_time@@%d",
	...
	if (trx_crux_info && strlen(trx_crux_info))
		index += snprintf(&(wls_dev->trx_info_load_trigger.crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s", trx_crux_info);
...
static int oplus_chg_wls_track_upload_rx_err_info(struct oplus_chg_wls *wls_dev, int err_type)
{
	...
	index += snprintf(&(wls_dev->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "wls");
	index += snprintf(&(wls_dev->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_WLS_RX_ERR);
...
static int oplus_chg_wls_set_trx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (en && wls_dev->usb_present && !wls_dev->support_tx_boost && !wls_dev->support_wls_and_tx_boost) {
	...
	if (en) {
		...
		pr_info("trx_online=%d, usb_present_once=%d,start_time=%d, end_time=%d\n", wls_status->trx_online,
			wls_status->trx_usb_present_once, wls_status->trx_transfer_start_time,
			wls_status->trx_transfer_end_time);
		if (wls_status->trx_online && wls_status->trx_transfer_start_time &&
		    (wls_status->trx_transfer_end_time - wls_status->trx_transfer_start_time >
		     WLS_TRX_INFO_UPLOAD_THD_2MINS)) {
			oplus_chg_wls_update_track_info(wls_dev, trx_crux_info, false);
			oplus_chg_wls_track_upload_trx_general_info(wls_dev, trx_crux_info,
...
static int oplus_chg_wls_path_ctrl(struct oplus_chg_wls *wls_dev, enum oplus_chg_wls_path_ctrl_type type)
{
...
static bool oplus_wls_check_bcc_support(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_dev->support_wls_chg_bcc && (wls_status->bcc_temp_range == WLS_BCC_TEMP_160_TO_400 ||
...
static void oplus_chg_wls_set_bcc_current_iout(struct oplus_chg_wls *wls_dev)
{
	...
	if (bcc_icl > 0 &&
	    (wls_status->wls_type == OPLUS_CHG_WLS_SVOOC || wls_status->wls_type == OPLUS_CHG_WLS_PD_65W)) {
...
static int oplus_chg_wls_choose_fastchg_curve(struct oplus_chg_wls *wls_dev)
{
	...
	if ((batt_temp_plugin != WLS_FAST_TEMP_MAX) && (batt_soc_plugin != WLS_FAST_SOC_MAX)) {
		...
		for (i = 0; i < wls_dev->wls_fcc_step.max_step; i++) {
			pr_err("%s: %d %d %d %d %d\n", __func__, wls_dev->wls_fcc_step.fcc_step[i].low_threshold,
			       wls_dev->wls_fcc_step.fcc_step[i].high_threshold,
			       wls_dev->wls_fcc_step.fcc_step[i].curr_ma, wls_dev->wls_fcc_step.fcc_step[i].vol_max_mv,
...
static int oplus_wls_bcc_choose_curve(struct oplus_chg_wls *wls_dev)
{
	...
	if ((batt_temp_plugin != WLS_BCC_TEMP_MAX) && (batt_soc_plugin != WLS_BCC_SOC_MAX)) {
		...
		for (i = 0; i < wls_dev->wls_bcc_step.max_step; i++) {
			pr_err("%s: %d %d %d %d\n", __func__, wls_dev->wls_bcc_step.bcc_step[i].max_batt_volt,
			       wls_dev->wls_bcc_step.bcc_step[i].max_curr, wls_dev->wls_bcc_step.bcc_step[i].min_curr,
...
static void oplus_wls_bcc_get_curve(struct oplus_chg_wls *wls_dev)
{
	...
	for (i = 0; i < wls_dev->wls_bcc_step.max_step; i++) {
		pr_err("bcc_batt is %d target_volt is %d\n", bcc_batt_volt_now, bcc_chg->bcc_step[i].max_batt_volt);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, wls_bcc_curr_update_work);
	struct oplus_chg_wls_bcc_step *bcc_chg = &wls_dev->wls_bcc_step;
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	int bcc_batt_volt_now;
	int i, rc;
	...
	if (pre_curve_idx == wls_status->bcc_curve_idx) {
		...
		if (idx_cnt >= CURVE_CHANGE_COUNT) {
			...
			pr_err("choose max curr is %d, min curr is %d true idx is %d\n", wls_status->wls_bcc_max_curr,
			       wls_status->wls_bcc_min_curr, wls_status->bcc_true_idx);
		}
	} else {
		idx_cnt = 0;
	}
...
ssize_t oplus_chg_wls_upgrade_fw_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t oplus_chg_wls_path_curr_store(struct device *dev, struct device_attribute *attr, const char *buf,
					     size_t count)
{
	struct oplus_chg_mod *ocm = dev_get_drvdata(dev);
	struct oplus_chg_wls *wls_dev = oplus_chg_mod_get_drvdata(ocm);
...
static ssize_t oplus_chg_wls_ftm_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_mod *ocm = dev_get_drvdata(dev);
	struct oplus_chg_wls *wls_dev = oplus_chg_mod_get_drvdata(ocm);
	int rc;
	...
	if (oplus_chg_wls_is_usb_present(wls_dev) && !wls_dev->support_tx_boost && !wls_dev->support_wls_and_tx_boost) {
...
static enum oplus_chg_mod_property oplus_chg_wls_props[] = {
	...
	OPLUS_CHG_PROP_DOCK_TYPE,      OPLUS_CHG_PROP_WLS_SKEW_CURR,	 OPLUS_CHG_PROP_VERITY,
	...
	OPLUS_CHG_PROP_RX_MAX_POWER,   OPLUS_CHG_PROP_FOD_CAL,		 OPLUS_CHG_PROP_BATT_CHG_ENABLE,
	...
	OPLUS_CHG_PROP_RX_VOUT_UVP,    OPLUS_CHG_PROP_FW_UPGRADING,	 OPLUS_CHG_PROP_BCC_SUPPORT,
	OPLUS_CHG_PROP_BCC_CURRENT,    OPLUS_CHG_PROP_WLS_BCC_MAX,	 OPLUS_CHG_PROP_WLS_BCC_MIN,
	OPLUS_CHG_PROP_WLS_BCC_STOP,
};

static enum oplus_chg_mod_property oplus_chg_wls_uevent_props[] = {
	OPLUS_CHG_PROP_ONLINE,	    OPLUS_CHG_PROP_PRESENT,	    OPLUS_CHG_PROP_VOLTAGE_NOW,
	OPLUS_CHG_PROP_CURRENT_NOW, OPLUS_CHG_PROP_WLS_TYPE,	    OPLUS_CHG_PROP_FASTCHG_STATUS,
	OPLUS_CHG_PROP_ADAPTER_SID, OPLUS_CHG_PROP_TRX_VOLTAGE_NOW, OPLUS_CHG_PROP_TRX_CURRENT_NOW,
	OPLUS_CHG_PROP_TRX_STATUS,  OPLUS_CHG_PROP_TRX_ONLINE,	    OPLUS_CHG_PROP_QUIET_MODE,
	OPLUS_CHG_PROP_BCC_SUPPORT, OPLUS_CHG_PROP_BCC_CURRENT,	    OPLUS_CHG_PROP_WLS_BCC_MAX,
	OPLUS_CHG_PROP_WLS_BCC_MIN, OPLUS_CHG_PROP_WLS_BCC_STOP,
};
...
static int oplus_chg_wls_get_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop,
				  union oplus_chg_mod_propval *pval)
{
	struct oplus_chg_wls *wls_dev = oplus_chg_mod_get_drvdata(ocm);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	...
	case OPLUS_CHG_PROP_INPUT_CURRENT_NOW:
		rc = oplus_chg_wls_nor_get_input_curr(wls_dev->wls_nor, &pval->intval);
		break;
...
static int oplus_chg_wls_set_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop,
				  const union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_VOLTAGE_MAX:
		if (wls_dev->debug_mode) {
			wls_status->fastchg_started = false;
			rc = oplus_chg_wls_rx_set_vout(wls_dev->wls_rx, pval->intval / 1000, 0);
			oplus_vote(wls_dev->fcc_votable, DEBUG_VOTER, false, 0, false);
		} else {
			rc = -EINVAL;
	...
	case OPLUS_CHG_PROP_STATUS_DELAY:
		break;
	case OPLUS_CHG_PROP_QUIET_MODE:
		if (!wls_status->rx_present || wls_status->adapter_type == WLS_ADAPTER_TYPE_USB ||
		...
		if (wls_status->switch_quiet_mode != wls_status->quiet_mode || wls_status->quiet_mode_init == false) {
...
static int oplus_chg_wls_prop_is_writeable(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop)
{
	switch (prop) {
	case OPLUS_CHG_PROP_VOLTAGE_MAX:
	case OPLUS_CHG_PROP_VOLTAGE_MIN:
...
static int oplus_chg_wls_event_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
...
static int oplus_chg_wls_mod_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	case OPLUS_CHG_EVENT_CAMERA_ON:
		pr_info("camera on\n");
		oplus_vote(wls_dev->fcc_votable, CAMERA_VOTER, true, dynamic_cfg->wls_fast_chg_camera_on_curr_ma,
			   false);
		break;
...
static int oplus_chg_wls_changed_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
...
static int oplus_chg_wls_aes_mutual_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
	int i;
	...
	if (aes_auth_result) {
		...
		for (i = 0; i < WLS_AUTH_AES_ENCODE_LEN; i++)
			pr_info("aes_encode_num[%d]:0x%02x ", i, (wls_status->aes_verfity_data.aes_encode_num)[i]);
	}
...
static int oplus_chg_wls_init_mod(struct oplus_chg_wls *wls_dev)
{
	...
	wls_dev->wls_ocm = oplus_chg_mod_register(wls_dev->dev, &oplus_chg_wls_mod_desc, &ocm_cfg);
...
static void oplus_chg_wls_start_work(struct work_struct *work)
{
	...
	if (wls_status->rx_online) {
		...
		if (wpc_online_cnt >= OPLUS_CHG_WLS_START_DETECT_CNT) {
			...
			schedule_delayed_work(&wls_dev->wls_start_work, round_jiffies_relative(msecs_to_jiffies(
										OPLUS_CHG_WLS_START_DETECT_DELAY)));
		}
	} else {
...
static void oplus_chg_wls_connect_work(struct work_struct *work)
{
	...
	if (wls_status->rx_online) {
		...
		oplus_chg_strategy_init(
			&wls_dev->wls_fast_chg_led_off_strategy, dynamic_cfg->wls_fast_chg_led_off_strategy_data,
			oplus_chg_get_chg_strategy_data_len(dynamic_cfg->wls_fast_chg_led_off_strategy_data,
							    CHG_STRATEGY_DATA_TABLE_MAX),
			skin_temp);
		oplus_chg_strategy_init(
			&wls_dev->wls_fast_chg_led_on_strategy, dynamic_cfg->wls_fast_chg_led_on_strategy_data,
			oplus_chg_get_chg_strategy_data_len(dynamic_cfg->wls_fast_chg_led_on_strategy_data,
							    CHG_STRATEGY_DATA_TABLE_MAX),
			skin_temp);
	} else {
...
static int oplus_chg_wls_nor_skin_check(struct oplus_chg_wls *wls_dev)
{
	...
	if ((skin_temp < skin_step->skin_step[*skin_step_count].low_threshold) && (*skin_step_count > 0)) {
...
static void oplus_chg_wls_fast_switch_next_step(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->fastchg_level >= fcc_chg->max_step) {
		if (batt_vol_mv >= batt_vol_max) {
			...
			oplus_chg_wls_track_set_prop(wls_dev, OPLUS_CHG_PROP_STATUS, TRACK_WLS_FASTCHG_FULL);
		}
	} else {
		wls_status->wait_cep_stable = true;
		oplus_vote(wls_dev->fcc_votable, FCC_VOTER, true, fcc_chg->fcc_step[wls_status->fastchg_level].curr_ma,
			   false);
	}
...
static void oplus_chg_wls_fast_switch_prev_step(struct oplus_chg_wls *wls_dev)
{
	...
	oplus_vote(wls_dev->fcc_votable, FCC_VOTER, true, fcc_chg->fcc_step[wls_status->fastchg_level].curr_ma, false);
...
static int oplus_chg_wls_fast_temp_check(struct oplus_chg_wls *wls_dev)
{
	...
	pr_err("battery temp = %d, vol = %d, level = %d, temp_diff = %d, fcc_curr_ma=%d\n", batt_temp, batt_vol_mv,
	       wls_status->fastchg_level, temp_diff, fcc_curr_ma);
	...
	if (wls_status->fastchg_level == 0) {
		...
		pr_info("jiffies=%u, timeout=%u, high_threshold=%d, batt_vol_max=%d\n", jiffies,
			fcc_chg->fcc_wait_timeout, fcc_chg->fcc_step[wls_status->fastchg_level].high_threshold,
			batt_vol_max);
...
static void oplus_chg_wls_fast_cep_check(struct oplus_chg_wls *wls_dev, int cep)
{
	...
	if (!wls_status->wait_cep_stable) {
		...
		if (cep < 3) {
			cep_ok_count++;
			cep_err_count = 0;
			if ((cep_ok_count >= CEP_OK_MAX) && time_after(jiffies, wls_status->cep_ok_wait_timeout) &&
			...
			if (cep_err_count >= CEP_ERR_MAX) {
				...
				if ((cep_curr_ma > 0) && (cep_curr_ma <= WLS_FASTCHG_CURR_MIN_MA)) {
					...
					curr_ma =
						curr_ma - (curr_ma % WLS_FASTCHG_CURR_ERR_MA) - WLS_FASTCHG_CURR_ERR_MA;
	...
	if (wls_status->led_on) {
		...
		if (wls_dev->wls_fast_chg_led_on_strategy.initialized) {
			curr_ma = oplus_chg_strategy_get_data(&wls_dev->wls_fast_chg_led_on_strategy,
							      &wls_dev->wls_fast_chg_led_on_strategy.temp_region,
							      skin_temp);
		...
		if (wls_dev->wls_fast_chg_led_off_strategy.initialized) {
			curr_ma = oplus_chg_strategy_get_data(&wls_dev->wls_fast_chg_led_off_strategy,
							      &wls_dev->wls_fast_chg_led_off_strategy.temp_region,
							      skin_temp);
...
static void oplus_chg_wls_set_quiet_mode(struct oplus_chg_wls *wls_dev, bool quiet_mode)
{
	...
	if (wls_status->adapter_id == WLS_ADAPTER_MODEL_0)
		(void)oplus_chg_wls_send_msg(wls_dev, quiet_mode ? WLS_CMD_SET_QUIET_MODE : WLS_CMD_SET_NORMAL_MODE,
					     0xff, 2);
	else
		(void)oplus_chg_wls_send_msg(wls_dev, WLS_CMD_SET_FAN_SPEED,
					     quiet_mode ? fan_pwm_pulse_silent : fan_pwm_pulse_fastchg, 2);

	(void)oplus_chg_wls_send_msg(wls_dev, WLS_CMD_SET_LED_BRIGHTNESS, quiet_mode ? QUIET_MODE_LED_BRIGHTNESS : 100,
				     2);
}
...
static int oplus_chg_wls_fastchg_restart_check(struct oplus_chg_wls *wls_dev)
{
	...
	if (!wls_status->fastchg_disable || wls_status->switch_quiet_mode || !wls_dev->batt_charge_enable)
		return -EPERM;
	...
	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable, RX_FAST_ERR_VOTER) &&
	    (wls_status->fastchg_err_count < 2) && time_is_before_jiffies(wls_status->fastchg_err_timer))
		oplus_vote(wls_dev->fastchg_disable_votable, RX_FAST_ERR_VOTER, false, 0, false);
	...
	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable, STARTUP_CEP_VOTER) &&
	    (wls_status->fastchg_retry_count < 10) && time_is_before_jiffies(wls_status->fastchg_retry_timer))
		oplus_vote(wls_dev->fastchg_disable_votable, STARTUP_CEP_VOTER, false, 0, false);
		...
		if ((wls_status->non_fcc_level == non_ffc_max_step - 1) && (wls_status->non_fcc_level > 0)) {
			...
			if (fallback_count >= FALLBACK_COUNT_MAX) {
				...
				pr_err("fallback: non_ffc_icl=%d, j=%d, level=%d\n",
				       dynamic_cfg->non_ffc_step[i][wls_status->non_fcc_level].curr_ma, j,
				       wls_status->non_fcc_level);
			} else {
				oplus_vote(wls_dev->nor_icl_votable, NON_FFC_VOTER, true,
					   dynamic_cfg->non_ffc_step[i][wls_status->non_fcc_level].curr_ma, true);
	...
	if (rc < 0 && wls_status->adapter_type == WLS_ADAPTER_TYPE_UNKNOWN) {
		...
		case WLS_ADAPTER_TYPE_PD_65W:
			...
			if (wls_dev->support_fastchg) {
		 		...
		 		*/
				if (wls_status->adapter_id == WLS_ADAPTER_MODEL_0 && !wls_status->quiet_mode_init &&
...
static int oplus_chg_wls_rx_handle_state_epp(struct oplus_chg_wls *wls_dev)
{
	...
	if ((wls_status->adapter_type != WLS_ADAPTER_TYPE_UNKNOWN) && !wls_status->verity_started) {
...
static int oplus_chg_wls_rx_handle_state_epp_plus(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->adapter_type == WLS_ADAPTER_TYPE_VOOC || wls_status->adapter_type == WLS_ADAPTER_TYPE_SVOOC ||
	    wls_status->adapter_type == WLS_ADAPTER_TYPE_PD_65W) {
		if (wls_status->pwr_max_mw > 0) {
			oplus_vote(wls_dev->nor_icl_votable, RX_MAX_VOTER, true,
				   wls_status->pwr_max_mw * 1000 / dynamic_cfg->vooc_vol_mv, false);
...
static int oplus_chg_wls_rx_enter_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->switch_quiet_mode && wls_status->state_sub_step > OPLUS_CHG_WLS_FAST_SUB_STATE_WAIT_FAST) {
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_CHECK_IOUT:
		...
		if (vout_mv < vbat_mv * 4 + CP_OPEN_OFFSET) {
			...
			(void)oplus_chg_wls_rx_set_vout_step(wls_dev->wls_rx, 4 * vbat_mv + CP_OPEN_OFFSET + 300, 1000,
							     10);
			delay_ms = 0;
			break;
		}
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_START:
		...
		wls_status->current_rx_state = OPLUS_CHG_WLS_RX_STATE_FAST;
		oplus_vote(wls_dev->nor_fv_votable, USER_VOTER, true, dynamic_cfg->batt_vol_max_mv + 100, false);
		delay_ms = 0;
		break;
	}
...
static int oplus_chg_wls_rx_handle_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	if (!wls_status->verity_started &&
	    (wls_status->fastchg_target_curr_ma - wls_status->iout_ma < WLS_CURR_ERR_MIN_MA)) {
		schedule_delayed_work(&wls_dev->wls_verity_work, msecs_to_jiffies(500));
	}
...
static int oplus_chg_wls_rx_exit_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx, oplus_chg_wls_disable_fod_parm, WLS_FOD_PARM_LENGTH);
	...
	case OPLUS_CHG_WLS_RX_STATE_DONE:
		...
		msleep(100);
		(void)oplus_chg_wls_fast_set_enable(wls_dev->wls_fast, false);
		(void)oplus_chg_wls_rx_set_vout_step(wls_dev->wls_rx, WLS_VOUT_FASTCHG_INIT_MV, 1000, -1);
		oplus_vote(wls_dev->fcc_votable, EXIT_VOTER, false, 0, false);
		break;
	case OPLUS_CHG_WLS_RX_STATE_QUIET:
		wls_status->next_rx_state = OPLUS_CHG_WLS_RX_STATE_QUIET;
	...
	case 0:
		...
		if (wls_dev->static_config.fastchg_12V_fod_enable)
			(void)oplus_chg_wls_rx_set_fod_parm(
				wls_dev->wls_rx, wls_dev->static_config.fastchg_fod_parm_12V, WLS_FOD_PARM_LENGTH);
		else
			(void)oplus_chg_wls_rx_set_fod_parm(wls_dev->wls_rx, oplus_chg_wls_disable_fod_parm,
							    WLS_FOD_PARM_LENGTH);
	...
	if (wls_status->current_rx_state != wls_status->target_rx_state) {
		if (wls_status->current_rx_state != wls_status->next_rx_state) {
			if (oplus_chg_wls_rx_state_handler[wls_status->next_rx_state].enter_state != NULL) {
				delay_ms =
					oplus_chg_wls_rx_state_handler[wls_status->next_rx_state].enter_state(wls_dev);
			} else {
				delay_ms = 0;
			}
		} else {
			if (oplus_chg_wls_rx_state_handler[wls_status->current_rx_state].exit_state != NULL) {
				delay_ms = oplus_chg_wls_rx_state_handler[wls_status->current_rx_state].exit_state(
					wls_dev);
			} else {
				delay_ms = 0;
			}
...
static void oplus_chg_wls_trx_sm(struct work_struct *work)
{
	...
	if (trx_status & WLS_TRX_STATUS_READY) {
		...
		delay_ms = 200;
	} else if ((trx_status & WLS_TRX_STATUS_DIGITALPING) || (trx_status & WLS_TRX_STATUS_ANALOGPING)) {
		wls_status->trx_present = true;
		wls_status->trx_online = false;
		if (is_batt_psy_available(wls_dev))
			power_supply_changed(wls_dev->batt_psy);
	...
	if (pre_trx_online && !wls_status->trx_online) {
		...
		pr_info("trx_online:%d, trx_start_time:%d, trx_end_time:%d,"
			...
			wls_status->trx_online, wls_status->trx_transfer_start_time, wls_status->trx_transfer_end_time,
			wls_status->trx_usb_present_once);
		...
		wls_status->trx_usb_present_once = false;
		wls_status->trx_transfer_start_time = oplus_chg_wls_get_local_time_s();
		pr_info("trx_online=%d, trx_start_time=%d, trx_end_time=%d\n", wls_status->trx_online,
			wls_status->trx_transfer_start_time, wls_status->trx_transfer_end_time);
	}
...
static void oplus_chg_wls_data_update_work(struct work_struct *work)
{
	...
	pr_info("Iout: target=%d, out=%d, ibat_max=%d, ibat=%d; vout=%d; vrect=%d; cep=%d\n",
		wls_status->fastchg_target_curr_ma, wls_status->iout_ma, wls_status->fastchg_ibat_max_ma, ibat_ma,
		wls_status->vout_mv, wls_status->vrect_mv, cep);
...
{
	struct oplus_chg_wls *wls_dev = container_of(filp->private_data, struct oplus_chg_wls, misc_dev);
...
static ssize_t oplus_chg_wls_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
...
static long oplus_chg_wls_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct oplus_chg_wls *wls_dev = filp->private_data;
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
...
static ssize_t oplus_chg_wls_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	return count;
}
...
static int read_skin_range_data_from_node(struct device_node *node, const char *prop_str,
					  struct oplus_chg_wls_skin_range_data *ranges, int max_threshold,
					  u32 max_value)
{
	int rc = 0, i, length, per_tuple_length, tuples;
...
static const char *const bcc_temp[] = {
	[WLS_BCC_TEMP_0_TO_50] = "bcc_temp_0_to_50",	   [WLS_BCC_TEMP_50_TO_120] = "bcc_temp_50_to_120",
	[WLS_BCC_TEMP_120_TO_160] = "bcc_temp_120_to_160", [WLS_BCC_TEMP_160_TO_400] = "bcc_temp_160_to_400",
...
static const char *const strategy_non_ffc[] = {
	[OPLUS_WLS_CHG_MODE_BPP] = "non-ffc-bpp",	    [OPLUS_WLS_CHG_MODE_EPP] = "non-ffc-epp",
	[OPLUS_WLS_CHG_MODE_EPP_PLUS] = "non-ffc-epp-plus", [OPLUS_WLS_CHG_MODE_AIRVOOC] = "non-ffc-airvooc",
	[OPLUS_WLS_CHG_MODE_AIRSVOOC] = "non-ffc-airsvooc",
};

static const char *const strategy_cv[] = {
	[OPLUS_WLS_CHG_MODE_BPP] = "cv-bpp",	       [OPLUS_WLS_CHG_MODE_EPP] = "cv-epp",
	[OPLUS_WLS_CHG_MODE_EPP_PLUS] = "cv-epp-plus", [OPLUS_WLS_CHG_MODE_AIRVOOC] = "cv-airvooc",
	[OPLUS_WLS_CHG_MODE_AIRSVOOC] = "cv-airsvooc",
};

static const char *const strategy_cool_down[] = {
	[OPLUS_WLS_CHG_MODE_BPP] = "cool-down-bpp",	      [OPLUS_WLS_CHG_MODE_EPP] = "cool-down-epp",
	[OPLUS_WLS_CHG_MODE_EPP_PLUS] = "cool-down-epp-plus", [OPLUS_WLS_CHG_MODE_AIRVOOC] = "cool-down-airvooc",
...
static int oplus_chg_wls_parse_dt(struct oplus_chg_wls *wls_dev)
{
	...
	if (rc < 0) {
		...
		length = rc;
		rc = of_property_read_u8_array(node, "oplus,fastchg-match-q", (u8 *)&static_cfg->fastchg_match_q,
					       length);
		if (rc < 0)
	...
	if (static_cfg->fastchg_fod_enable) {
		rc = of_property_read_u8_array(node, "oplus,fastchg-fod-parm", (u8 *)&static_cfg->fastchg_fod_parm,
					       WLS_FOD_PARM_LENGTH);
		if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "bcc_stop_curr_0_to_30", dynamic_cfg->bcc_stop_curr_0_to_30,
					  WLS_BCC_STOP_CURR_NUM);
	if (rc < 0) {
		...
		for (i = 0; i < WLS_BCC_STOP_CURR_NUM; i++) {
			pr_err("%s: bcc_stop_0_30: %d", __func__, dynamic_cfg->bcc_stop_curr_0_to_30[i]);
		}
	}

	rc = read_unsigned_data_from_node(node, "bcc_stop_curr_30_to_70", dynamic_cfg->bcc_stop_curr_30_to_70,
					  WLS_BCC_STOP_CURR_NUM);
	if (rc < 0) {
		...
		for (i = 0; i < WLS_BCC_STOP_CURR_NUM; i++) {
			pr_err("%s: bcc_stop_30_70: %d", __func__, dynamic_cfg->bcc_stop_curr_30_to_70[i]);
		}
	}

	rc = read_unsigned_data_from_node(node, "bcc_stop_curr_70_to_90", dynamic_cfg->bcc_stop_curr_70_to_90,
					  WLS_BCC_STOP_CURR_NUM);
	if (rc < 0) {
		...
		for (i = 0; i < WLS_BCC_STOP_CURR_NUM; i++) {
			pr_err("%s: bcc_stop_70_90: %d", __func__, dynamic_cfg->bcc_stop_curr_70_to_90[i]);
		}
	}
...
non_ffc:
	...
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		...
		length = rc;
		rc = of_property_read_u32_array(node, strategy_non_ffc[i], (u32 *)dynamic_cfg->non_ffc_step[i], length);
		wls_dev->non_ffc_step[i].max_step = length / 5;
	}
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		for (j = 0; j < wls_dev->non_ffc_step[i].max_step; j++) {
			pr_err("%s: %d %d %d %d %d\n", __func__, dynamic_cfg->non_ffc_step[i][j].low_threshold,
			       dynamic_cfg->non_ffc_step[i][j].high_threshold, dynamic_cfg->non_ffc_step[i][j].curr_ma,
			       dynamic_cfg->non_ffc_step[i][j].vol_max_mv, dynamic_cfg->non_ffc_step[i][j].need_wait);
		}
	}
	...
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		...
		length = rc;
		rc = of_property_read_u32_array(node, strategy_cv[i], (u32 *)dynamic_cfg->cv_step[i], length);
		wls_dev->cv_step[i].max_step = length / 5;
	}
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		for (j = 0; j < wls_dev->cv_step[i].max_step; j++) {
			pr_err("%s: %d %d %d %d %d\n", __func__, dynamic_cfg->cv_step[i][j].low_threshold,
			       dynamic_cfg->cv_step[i][j].high_threshold, dynamic_cfg->cv_step[i][j].curr_ma,
			       dynamic_cfg->cv_step[i][j].vol_max_mv, dynamic_cfg->cv_step[i][j].need_wait);
		}
	}
	...
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		...
		length = rc;
		rc = read_unsigned_data_from_node(node, strategy_cool_down[i], (u32 *)dynamic_cfg->cool_down_step[i],
						  length);
	...
	for (i = 0; i < OPLUS_WLS_SKEWING_MAX; i++) {
		...
		length = rc;
		rc = read_unsigned_data_from_node(node, strategy_skewing[i], (u32 *)dynamic_cfg->skewing_step[i],
						  length);
		wls_dev->skewing_step[i].max_step = length / 3;
	}
	for (i = 0; i < OPLUS_WLS_SKEWING_MAX; i++) {
		for (j = 0; j < wls_dev->skewing_step[i].max_step; j++) {
			pr_err("%s: %d %d %d\n", __func__, dynamic_cfg->skewing_step[i][j].curr_ma,
	...
	rc = read_skin_range_data_from_node(node, "oplus,epp_plus-skin-step", skin_step->skin_step, WLS_SKIN_TEMP_MAX,
	...
	if (rc < 0) {
		for (i = 0; i < WLS_MAX_STEP_CHG_ENTRIES; i++) {
			...
			if ((skin_step->skin_step[i].low_threshold == 0) &&
			    (skin_step->skin_step[i].high_threshold == 0) && (skin_step->skin_step[i].curr_ma == 0)) {
	...
	rc = read_skin_range_data_from_node(node, "oplus,epp-skin-step", skin_step->skin_step, WLS_SKIN_TEMP_MAX,
	...
	rc = read_skin_range_data_from_node(node, "oplus,bpp-skin-step", skin_step->skin_step, WLS_SKIN_TEMP_MAX,
					    wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_BPP]);
	if (rc < 0) {
		for (i = 0; i < WLS_MAX_STEP_CHG_ENTRIES; i++) {
			skin_step->skin_step[i].low_threshold = default_config.bpp_skin_step[i].low_threshold;
			skin_step->skin_step[i].high_threshold = default_config.bpp_skin_step[i].high_threshold;
			skin_step->skin_step[i].curr_ma = default_config.bpp_skin_step[i].curr_ma;
			if ((skin_step->skin_step[i].low_threshold == 0) &&
			    (skin_step->skin_step[i].high_threshold == 0) && (skin_step->skin_step[i].curr_ma == 0)) {
				skin_step->max_step = i;
				break;
	...
	rc = read_skin_range_data_from_node(node, "oplus,epp-led-on-skin-step", skin_step->skin_step, WLS_SKIN_TEMP_MAX,
					    wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_EPP]);
	if (rc < 0) {
		for (i = 0; i < WLS_MAX_STEP_CHG_ENTRIES; i++) {
	...
	pr_err("icl_max_ma BPP EPP EPP_PLUS AIRVOOC AIRSVOOC: %d %d %d %d %d\n",
	       wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_BPP], wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_EPP],
	       wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_EPP_PLUS], wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_AIRVOOC],
	       wls_dev->icl_max_ma[OPLUS_WLS_CHG_MODE_AIRSVOOC]);

	rc = read_chg_strategy_data_from_node(node, "oplus,wls-fast-chg-led-off-strategy-data",
	...
	rc = read_chg_strategy_data_from_node(node, "oplus,wls-fast-chg-led-on-strategy-data",
...
static int oplus_chg_wls_gpio_init(struct oplus_chg_wls *wls_dev)
{
	...
	rc = devm_request_irq(wls_dev->dev, wls_dev->usb_int_irq, oplus_chg_wls_usb_int_handler,
			      IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "usb_int_irq", wls_dev);
	if (rc < 0) {
...
#ifndef CONFIG_OPLUS_CHG_OOS
static ssize_t oplus_chg_wls_proc_deviated_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[7];
	size_t len = 7;
...
static ssize_t oplus_chg_wls_proc_tx_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[10] = { 0 };
	...
	rc = oplus_chg_mod_get_property(wls_dev->wls_ocm, OPLUS_CHG_PROP_TRX_STATUS, &val);
	if (rc < 0) {
...
static ssize_t oplus_chg_wls_proc_tx_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	char buffer[5] = { 0 };
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
	int val;
...
static ssize_t oplus_chg_wls_proc_rx_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[3];
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
...
static ssize_t oplus_chg_wls_proc_rx_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	char buffer[5] = { 0 };
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
	int val;
...
static ssize_t oplus_chg_wls_proc_user_sleep_mode_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[10];
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
...
static ssize_t oplus_chg_wls_proc_user_sleep_mode_write(struct file *file, const char __user *buf, size_t len,
							loff_t *lo)
{
	char buffer[5] = { 0 };
	int pmw_pulse = 0;
	...
	if (pmw_pulse == WLS_FASTCHG_MODE) {
		pval.intval = 0;
		rc = oplus_chg_mod_set_property(wls_dev->wls_ocm, OPLUS_CHG_PROP_QUIET_MODE, &pval);
		if (rc == 0)
...
static ssize_t oplus_chg_wls_proc_idt_adc_test_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[10];
	int rx_adc_result = 0;
...
static ssize_t oplus_chg_wls_proc_idt_adc_test_write(struct file *file, const char __user *buf, size_t len, loff_t *lo)
{
	char buffer[5] = { 0 };
	int rx_adc_cmd = 0;
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
...
static ssize_t oplus_chg_wls_proc_rx_power_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
	int rx_power = 0;
...
static ssize_t oplus_chg_wls_proc_rx_power_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	return count;
}
...
static ssize_t oplus_chg_wls_proc_tx_power_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
	int tx_power = 0;
...
static ssize_t oplus_chg_wls_proc_tx_power_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	return count;
}
...
static ssize_t oplus_chg_wls_proc_rx_version_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
	u32 rx_version = 0;
...
static ssize_t oplus_chg_wls_proc_rx_version_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	return count;
}
...
static ssize_t oplus_chg_wls_proc_tx_version_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
	u32 tx_version = 0;
...
static ssize_t oplus_chg_wls_proc_tx_version_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
	return count;
}
...
static ssize_t oplus_chg_wls_proc_ftm_mode_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[256];
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
...
static ssize_t oplus_chg_wls_proc_ftm_mode_write(struct file *file, const char __user *buf, size_t len, loff_t *lo)
{
	char buffer[4] = { 0 };
	int ftm_mode = 0;
	struct oplus_chg_wls *wls_dev = PDE_DATA(file_inode(file));
...
static int oplus_chg_wls_init_charge_proc(struct oplus_chg_wls *wls_dev)
{
	...
	prEntry_tmp = proc_create_data("enable_tx", 0664, prEntry_da, &oplus_chg_wls_proc_tx_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("deviated", 0664, prEntry_da, &oplus_chg_wls_proc_deviated_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp =
		proc_create_data("user_sleep_mode", 0664, prEntry_da, &oplus_chg_wls_proc_user_sleep_mode_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("idt_adc_test", 0664, prEntry_da, &oplus_chg_wls_proc_idt_adc_test_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("enable_rx", 0664, prEntry_da, &oplus_chg_wls_proc_rx_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("rx_power", 0664, prEntry_da, &oplus_chg_wls_proc_rx_power_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("tx_power", 0664, prEntry_da, &oplus_chg_wls_proc_tx_power_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("rx_version", 0664, prEntry_da, &oplus_chg_wls_proc_rx_version_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("tx_version", 0664, prEntry_da, &oplus_chg_wls_proc_tx_version_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("ftm_mode", 0664, prEntry_da, &oplus_chg_wls_proc_ftm_mode_ops, wls_dev);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
...
static void oplus_chg_wls_track_trx_info_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, trx_info_load_trigger_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(dwork, struct oplus_chg_wls, rx_err_load_trigger_work);
...
{
	wls_dev->trx_info_load_trigger.type_reason = TRACK_NOTIFY_TYPE_GENERAL_RECORD;
	wls_dev->trx_info_load_trigger.flag_reason = TRACK_NOTIFY_FLAG_WLS_TRX_INFO;
	...
	INIT_DELAYED_WORK(&wls_dev->trx_info_load_trigger_work, oplus_chg_wls_track_trx_info_load_trigger_work);
	INIT_DELAYED_WORK(&wls_dev->rx_err_load_trigger_work, oplus_chg_wls_track_rx_err_load_trigger_work);
...
static int oplus_chg_wls_driver_probe(struct platform_device *pdev)
{
	...
	wls_dev->fcc_votable = oplus_create_votable("WLS_FCC", VOTE_MIN, oplus_chg_wls_fcc_vote_callback, wls_dev);
	...
	wls_dev->fastchg_disable_votable = oplus_create_votable("WLS_FASTCHG_DISABLE", VOTE_SET_ANY,
								oplus_chg_wls_fastchg_disable_vote_callback, wls_dev);
	...
	wls_dev->wrx_en_votable =
		oplus_create_votable("WLS_WRX_EN", VOTE_SET_ANY, oplus_chg_wls_wrx_en_vote_callback, wls_dev);
	...
	wls_dev->nor_icl_votable =
		oplus_create_votable("WLS_NOR_ICL", VOTE_MIN, oplus_chg_wls_nor_icl_vote_callback, wls_dev);
	...
	wls_dev->nor_fcc_votable =
		oplus_create_votable("WLS_NOR_FCC", VOTE_MIN, oplus_chg_wls_nor_fcc_vote_callback, wls_dev);
	...
	wls_dev->nor_fv_votable =
		oplus_create_votable("WLS_NOR_FV", VOTE_MIN, oplus_chg_wls_nor_fv_vote_callback, wls_dev);
	...
	wls_dev->nor_out_disable_votable = oplus_create_votable("WLS_NOR_OUT_DISABLE", VOTE_SET_ANY,
								oplus_chg_wls_nor_out_disable_vote_callback, wls_dev);
	...
	wls_dev->nor_input_disable_votable = oplus_create_votable(
		"WLS_NOR_INPUT_DISABLE", VOTE_SET_ANY, oplus_chg_wls_nor_input_disable_vote_callback, wls_dev);
	...
	wls_dev->rx_disable_votable =
		oplus_create_votable("WLS_RX_DISABLE", VOTE_SET_ANY, oplus_chg_wls_rx_disable_vote_callback, wls_dev);
	...
	INIT_DELAYED_WORK(&wls_dev->wls_get_third_part_verity_data_work, oplus_chg_wls_get_third_part_verity_data_work);
#ifndef CONFIG_OPLUS_CHG_OOS
