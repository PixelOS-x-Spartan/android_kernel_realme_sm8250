...
static int p9415_track_upload_i2c_err_info(struct oplus_p9415 *chip, int err_type, u16 reg);
...
static struct attribute *p9415_attributes[] = { &dev_attr_write_reg.attr, &dev_attr_read_reg.attr, NULL };

static struct attribute_group p9415_attribute_group = { .attrs = p9415_attributes };
#endif /*WLS_QI_DEBUG*/
...
static void p9415_track_i2c_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, i2c_err_load_trigger_work);
...
static int p9415_track_upload_wls_rx_err_info(struct oplus_p9415 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
...
static void p9415_track_rx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, rx_err_load_trigger_work);
...
static int p9415_track_upload_wls_tx_err_info(struct oplus_p9415 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
...
static void p9415_track_tx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, tx_err_load_trigger_work);
...
static int p9415_track_upload_wls_update_err_info(struct oplus_p9415 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
...
static void p9415_track_update_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, update_err_load_trigger_work);
...
static int p9415_track_debugfs_init(struct oplus_p9415 *chip)
{
	...
	debugfs_create_u32("debug_force_i2c_err", 0644, debugfs_p9415, &(chip->debug_force_i2c_err));
	debugfs_create_u32("debug_force_rx_err", 0644, debugfs_p9415, &(chip->debug_force_rx_err));
	debugfs_create_u32("debug_force_tx_err", 0644, debugfs_p9415, &(chip->debug_force_tx_err));
	debugfs_create_u32("debug_force_update_err", 0644, debugfs_p9415, &(chip->debug_force_update_err));
	debugfs_create_u32("debug_force_upload_period", 0644, debugfs_p9415, &(chip->debug_force_upload_period));
...
static int p9415_track_init(struct oplus_p9415 *chip)
{
	...
	INIT_DELAYED_WORK(&chip->i2c_err_load_trigger_work, p9415_track_i2c_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->rx_err_load_trigger_work, p9415_track_rx_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->tx_err_load_trigger_work, p9415_track_tx_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->update_err_load_trigger_work, p9415_track_update_err_load_trigger_work);
...
static int p9415_get_rx_mode(struct oplus_chg_ic_dev *dev, enum oplus_chg_wls_rx_mode *rx_mode)
{
	...
	if (chip->adapter_type == P9415_RX_MODE_EPP) {
		...
		if (chip->rx_pwr_cap == P9415_RX_PWR_15W || chip->rx_pwr_cap == P9415_RX_PWR_11W) {
...
static int p9415_set_dcdc_enable(struct oplus_chg_ic_dev *dev, bool en)
{
	...
	rc = p9415_write_byte_mask(chip, P9415_REG_PWR_CTRL, P9415_DCDC_EN, P9415_DCDC_EN);
	if (rc < 0)
...
static int p9415_set_trx_enable(struct oplus_chg_ic_dev *dev, bool en)
{
	...
	rc = p9415_write_byte(chip, P9415_REG_TRX_CTRL, en ? P9415_TRX_EN : 0x00);
	if (rc < 0)
...
static int p9415_load_fw(struct oplus_p9415 *chip, unsigned char *fw_data, int CodeLength)
{
	...
	rc = p9415_write_data(chip, 0x400, fw_data, ((CodeLength + 8 + 15) / 16) * 16);
	if (rc != 0) {
...
static int p9415_mtp(struct oplus_p9415 *chip, unsigned char *fw_buf, int fw_size)
{
	...
	if (rc != 0) {
		...
		p9415_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_I2C);
		return rc;
	}
	...
	if (rc < 0) { // not OK
		...
		p9415_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_OTHER);
		goto MTP_ERROR;
	}
	...
	if (write_ack != 2) { // not OK
		...
		p9415_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_CRC);
		goto MTP_ERROR;
	}
...
static void p9415_event_process(struct oplus_p9415 *chip)
{
	...
	if (temp[0] & P9415_EVENT) {
		...
		if (rc) {
			...
			pr_err("Received TX data: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", val_buf[0], val_buf[1],
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, check_event_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, check_ldo_on_work);
	int vout = 0;
	char buf[1] = { 0 };
	int rc;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, event_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9415 *chip = container_of(dwork, struct oplus_p9415, connect_work);
...
static int p9415_gpio_init(struct oplus_p9415 *chip)
{
	...
	rc = devm_request_irq(chip->dev, chip->rx_event_irq, p9415_event_handler, IRQF_TRIGGER_FALLING, "rx_event_irq",
			      chip);
	if (rc < 0) {
...
static void p9415_shutdown(struct i2c_client *client)
{
	...
	if (is_connected &&
	    (p9415_get_wls_type(chip) == OPLUS_CHG_WLS_VOOC || p9415_get_wls_type(chip) == OPLUS_CHG_WLS_SVOOC ||
...
static int p9415_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct oplus_p9415 *chip;
