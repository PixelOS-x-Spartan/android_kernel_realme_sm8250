...
static int nu1619_track_upload_i2c_err_info(struct oplus_nu1619 *chip, int err_type, u16 reg);
...
{
	if (nu1619_rx_is_connected(chip->ic_dev) && (atomic_inc_return(&i2c_err_count) > I2C_ERR_MAX)) {
...
static int nu1619_track_upload_i2c_err_info(struct oplus_nu1619 *chip, int err_type, u16 reg)
{
	...
	if (chip->debug_force_upload_period > 0 && curr_time - pre_upload_time > chip->debug_force_upload_period)
		upload_count = 0;
	...
	chip->i2c_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_WLS_TRX_ABNORMAL;
...
static void nu1619_track_i2c_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, i2c_err_load_trigger_work);
...
static int nu1619_track_upload_wls_rx_err_info(struct oplus_nu1619 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
	...
	chip->rx_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->rx_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_WLS_TRX_ABNORMAL;
	chip->rx_err_uploading = true;
	upload_count++;
	pre_upload_time = nu1619_track_get_local_time_s();
	mutex_unlock(&chip->track_rx_err_lock);

	index += snprintf(&(chip->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "nu1619");
	index += snprintf(&(chip->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
	...
	oplus_chg_track_obtain_power_info(chip->chg_power_info, sizeof(chip->chg_power_info));
	index += snprintf(&(chip->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  chip->chg_power_info);
	oplus_chg_track_obtain_wls_general_crux_info(chip->wls_crux_info, sizeof(chip->wls_crux_info));
	index += snprintf(&(chip->rx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  chip->wls_crux_info);
...
static void nu1619_track_rx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, rx_err_load_trigger_work);
...
static int nu1619_track_upload_wls_tx_err_info(struct oplus_nu1619 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
	...
	chip->tx_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->tx_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_WLS_TRX_ABNORMAL;
	chip->tx_err_uploading = true;
	upload_count++;
	pre_upload_time = nu1619_track_get_local_time_s();
	mutex_unlock(&chip->track_tx_err_lock);

	index += snprintf(&(chip->tx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "nu1619");
	index += snprintf(&(chip->tx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_WLS_TX_ERR);
	...
	oplus_chg_track_obtain_power_info(chip->chg_power_info, sizeof(chip->chg_power_info));
	index += snprintf(&(chip->tx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  chip->chg_power_info);
	oplus_chg_track_obtain_wls_general_crux_info(chip->wls_crux_info, sizeof(chip->wls_crux_info));
	index += snprintf(&(chip->tx_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  chip->wls_crux_info);
...
static void nu1619_track_tx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, tx_err_load_trigger_work);
...
static int nu1619_track_upload_wls_update_err_info(struct oplus_nu1619 *chip, int err_type)
{
	int index = 0;
	int curr_time;
	static int upload_count = 0;
	...
	chip->update_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->update_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_WLS_TRX_ABNORMAL;
	chip->update_err_uploading = true;
	upload_count++;
	pre_upload_time = nu1619_track_get_local_time_s();
	mutex_unlock(&chip->track_update_err_lock);

	index += snprintf(&(chip->update_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "nu1619");
	index += snprintf(&(chip->update_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_WLS_UPDATE_ERR);
...
static void nu1619_track_update_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, update_err_load_trigger_work);
...
static int nu1619_track_debugfs_init(struct oplus_nu1619 *chip)
{
	...
	debugfs_create_u32("debug_force_i2c_err", 0644, debugfs_nu1619, &(chip->debug_force_i2c_err));
	debugfs_create_u32("debug_force_rx_err", 0644, debugfs_nu1619, &(chip->debug_force_rx_err));
	debugfs_create_u32("debug_force_tx_err", 0644, debugfs_nu1619, &(chip->debug_force_tx_err));
	debugfs_create_u32("debug_force_update_err", 0644, debugfs_nu1619, &(chip->debug_force_update_err));
	debugfs_create_u32("debug_force_upload_period", 0644, debugfs_nu1619, &(chip->debug_force_upload_period));
...
static int nu1619_track_init(struct oplus_nu1619 *chip)
{
	...
	INIT_DELAYED_WORK(&chip->i2c_err_load_trigger_work, nu1619_track_i2c_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->rx_err_load_trigger_work, nu1619_track_rx_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->tx_err_load_trigger_work, nu1619_track_tx_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->update_err_load_trigger_work, nu1619_track_update_err_load_trigger_work);
...
static int nu1619_set_rx_enable(struct oplus_chg_ic_dev *dev, bool en)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->rx_en_active) || IS_ERR_OR_NULL(chip->rx_en_sleep)) {
		pr_err("rx_en pinctrl error\n");
		return -ENODEV;
	}

	rc = pinctrl_select_state(chip->pinctrl, en ? chip->rx_en_active : chip->rx_en_sleep);
	if (rc < 0)
		pr_err("can't %s rx\n", en ? "enable" : "disable");
	else
		pr_info("set rx %s\n", en ? "enable" : "disable");
...
static int nu1619_get_rx_mode(struct oplus_chg_ic_dev *dev, enum oplus_chg_wls_rx_mode *rx_mode)
{
	...
	chip->rx_pwr_cap = nu1619_get_power_cap(chip);
	if (chip->adapter_type == NU1619_RX_MODE_EPP) {
		if (chip->rx_pwr_cap == NU1619_RX_PWR_15W || chip->rx_pwr_cap == NU1619_RX_PWR_11W)
...
static int nu1619_set_rx_mode(struct oplus_chg_ic_dev *dev, enum oplus_chg_wls_rx_mode rx_mode)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->mode_sw_active) ||
...
static int nu1619_set_dcdc_enable(struct oplus_chg_ic_dev *dev, bool en)
{
	...
	rc = pinctrl_select_state(chip->pinctrl, en ? chip->dcdc_en_active : chip->dcdc_en_sleep);
	if (rc < 0)
...
static bool nu1619_req_checksum_and_fw_version(struct oplus_nu1619 *chip, char *boot_check_sum, char *rx_check_sum,
					       char *tx_check_sum, char *boot_ver, char *rx_ver, char *tx_ver)
{
	...
	pr_err("<FW UPDATE> 0x0020,21,22,23,24 = 0x%x,0x%x,0x%x,0x%x,0x%x\n", read_buffer2[0], read_buffer2[1],
	       read_buffer2[2], read_buffer2[3], read_buffer2[4]);
...
static bool nu1619_check_firmware_version(struct oplus_nu1619 *chip)
{
	...
	nu1619_req_checksum_and_fw_version(chip, &boot_checksum, &rx_checksum, &tx_checksum, &boot_version, &rx_version,
					   &tx_version);
	pr_err("<FW UPDATE> check_sum: 0x%x,0x%x,0x%x  version:0x%x,0x%x,0x%x\n", boot_checksum, rx_checksum,
	       tx_checksum, boot_version, rx_version, tx_version);
...
static bool nu1619_onekey_download_firmware(struct oplus_nu1619 *chip)
{
	...
	if (!nu1619_check_i2c_is_ok(chip)) {
		...
		nu1619_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_I2C);
		return false;
	} else {
	...
	if (!status) {
		...
		nu1619_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_OTHER);
		return false;
	}
	...
	if (!status) {
		...
		nu1619_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_CRC);
		return false;
	}
...
static bool nu1619_upgrade_firmware(struct oplus_nu1619 *chip, u8 *fw_buf, int fw_size)
{
	...
	if (!nu1619_check_i2c_is_ok(chip) || chip->debug_force_update_err == TRACK_WLS_UPDATE_ERR_I2C) {
		pr_err("<FW UPDATE> i2c error!\n");
		nu1619_track_upload_wls_update_err_info(chip, TRACK_WLS_UPDATE_ERR_I2C);
		return false;
	} else {
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, check_ldo_on_work);
	int iout = 0;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, event_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619, connect_work);
...
static int nu1619_gpio_init(struct oplus_nu1619 *chip)
{
	...
	rc = devm_request_irq(chip->dev, chip->rx_event_irq, nu1619_event_handler, IRQF_TRIGGER_FALLING, "rx_event_irq",
			      chip);
	if (rc < 0) {
	...
	rc = devm_request_irq(chip->dev, chip->rx_con_irq, nu1619_connect_handler,
			      IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "rx_con_irq", chip);
	if (rc < 0) {
...
static void nu1619_shutdown(struct i2c_client *client)
{
	...
	if (is_connected &&
	    (nu1619_get_wls_type(chip) == OPLUS_CHG_WLS_VOOC || nu1619_get_wls_type(chip) == OPLUS_CHG_WLS_SVOOC ||
