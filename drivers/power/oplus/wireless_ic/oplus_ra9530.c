...
void __attribute__((weak)) notify_pen_state(int state)
{
	return;
}
...
static void check_int_enable(struct oplus_ra9530_ic *chip)
{
	...
	if (rc) {
		...
		if ((reg_int[0] != 0xDF) || (reg_int[1] != 0x81) || (reg_int[2] != 0x02)) {
			reg_int[0] = 0xDF;
			reg_int[1] = 0x81;
			reg_int[2] = 0x02;
...
static void ra9530_set_protect_parameter(struct oplus_ra9530_ic *chip)
{
	...
	chg_err("config ocp_threshold = 0x%02x, ovp_threshold = 0x%02x \
		...
		chip->ocp_threshold, chip->ovp_threshold, chip->lvp_threshold, chip->fod_threshold,
...
void ra9530_reg_print(void)
{
	...
	chg_err("0xbe-0xc3 REG: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n", debug_data[0], debug_data[1],
		debug_data[2], debug_data[3], debug_data[4], debug_data[5]);
}
...
static int RA9530_load_bootloader(struct oplus_ra9530_ic *chip)
{
	...
	for (index = 0; index < package_count - 1; index++) {
		rc = ra9530_write_reg_multi_byte(chip, RA9530_FW_CODE_LENGTH * index,
						 MTPBootloader9530 + RA9530_FW_CODE_LENGTH * index,
						 RA9530_FW_CODE_LENGTH); /* load provided by IDT array */
		if (rc != NUM_0) {
	...
	if (package_count) { /* write the last bootloader part */
		rc = ra9530_write_reg_multi_byte(chip, RA9530_FW_CODE_LENGTH * (package_count - 1),
						 ...
						 sizeof(MTPBootloader9530) -
							 RA9530_FW_CODE_LENGTH *
								 (package_count - 1)); /* load provided by IDT array */
		if (rc != NUM_0) {
...
static int RA9530_load_fw(struct oplus_ra9530_ic *chip, unsigned char *fw_data, int codelength)
{
	...
	rc = ra9530_write_reg_multi_byte(chip, 0x1008, fw_data, ((codelength + 8 + 15) / 16) * 16);
	if (rc != 0) {
...
static int RA9530_Crc32_fw(struct oplus_ra9530_ic *chip, int start_addr, int fw_size, unsigned int crc)
{
	...
	chg_err("RA9530_Crc32_fw crc data buff[0]=%x buff[1]=%x] buff[2]=%x buff[3]=%x crc=%x!\n", crc_buff[0],
		crc_buff[1], crc_buff[2], crc_buff[3], crc);
...
static void ra9530_set_match_firmware(struct oplus_ra9530_ic *chip)
{
	...
	if ((temp[0] == fw_buf[fw_ver_start_addr + 0x05]) && (temp[1] == fw_buf[fw_ver_start_addr + 0x06]) &&
	    (temp[2] == fw_buf[fw_ver_start_addr + 0x07]) && (temp[3] == fw_buf[fw_ver_start_addr + 0x08]) &&
...
static int ra9530_check_idt_fw_update(struct oplus_ra9530_ic *chip, bool force_update)
{
	...
	if (rc) {
		...
		if (force_update || (((temp[NUM_0] != fw_buf[fw_ver_start_addr + HEX_5]) ||
				     ...
				     (flag[NUM_0] != S_MAGIC_WORDS) && (flag[NUM_1] != S_MAGIC_WORDS))) {
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_ra9530_ic *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,ocp_threshold", &chip->ocp_threshold);
	if (rc) {
		chip->ocp_threshold = RA9530_OCP_THRESHOLD;
	}
	chg_err("ocp_threshold[%d]\n", chip->ocp_threshold);

	rc = of_property_read_u32(node, "qcom,ovp_threshold", &chip->ovp_threshold);
	if (rc) {
		chip->ovp_threshold = RA9530_OVP_THRESHOLD;
	}
	chg_err("ovp_threshold[%d]\n", chip->ovp_threshold);

	rc = of_property_read_u32(node, "qcom,lvp_threshold", &chip->lvp_threshold);
	if (rc) {
		chip->lvp_threshold = RA9530_LVP_THRESHOLD;
	}
	chg_err("lvp_threshold[%d]\n", chip->lvp_threshold);

	rc = of_property_read_u32(node, "qcom,pcop_threshold1", &chip->pcop_threshold1);
	if (rc) {
		chip->pcop_threshold1 = RA9530_PCOP_THRESHOLD1;
	}
	chg_err("pcop_threshold1[%d]\n", chip->pcop_threshold1);

	rc = of_property_read_u32(node, "qcom,pcop_threshold2", &chip->pcop_threshold2);
	if (rc) {
		chip->pcop_threshold2 = RA9530_PCOP_THRESHOLD2;
	}
	chg_err("pcop_threshold2[%d]\n", chip->pcop_threshold2);

	rc = of_property_read_u32(node, "qcom,fod_threshold", &chip->fod_threshold);
	if (rc) {
		chip->fod_threshold = RA9530_FOD_THRESHOLD2;
	}
	chg_err("fod_threshold[%d]\n", chip->fod_threshold);

	rc = of_property_read_u32(node, "rx,soc_threshold", &chip->soc_threshould);
	if (rc) {
		chip->soc_threshould = RA9530_SOC_THRESHOLD2;
	}

	rc = of_property_read_u32(node, "tx,enable_expired_time", &chip->power_expired_time);
	if (rc) {
...
void ra9530_ept_type_detect_func(struct oplus_ra9530_ic *chip)
{
	...
	if (rc) {
		...
		if ((recv_data & RA9530_RTX_ERR_TX_POCP) || (recv_data & RA9530_RTX_ERR_TX_OTP) ||
		    (recv_data & RA9530_RTX_ERR_TX_FOD) || (recv_data & RA9530_RTX_ERR_TX_LVP) ||
		    (recv_data & RA9530_RTX_ERR_TX_OVP) || (recv_data & RA9530_RTX_ERR_TX_OCP)) {
...
static void ra9530_commu_data_process(struct oplus_ra9530_ic *chip)
{
	...
	chg_err("int ATO 0x2C: %02x %02x %02x %02x\n", chip->int_flag_data[NUM_0], chip->int_flag_data[NUM_1],
		chip->int_flag_data[NUM_2], chip->int_flag_data[NUM_3]);
	...
	chg_err("int: %02x %02x %02x %02x\n", chip->int_flag_data[0], chip->int_flag_data[1], chip->int_flag_data[2],
		chip->int_flag_data[3]);
	...
	if (chip->int_flag_data[0] & RA9530_INT_FLAG_SS) {
		...
		notify_pen_state(1);
		check_int_enable(chip);
		schedule_delayed_work(&chip->check_point_dwork,
				      round_jiffies_relative(msecs_to_jiffies(RA9530_CHECK_SS_INT_TIME)));
...
static ssize_t wireless_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d:%d\n%d\n%d:%d\n%d:%d\n%d %s\n", chip->rx_soc, chip->soc_threshould, chip->is_power_on,
		       chip->power_enable_reason, chip->power_enable_times, chip->charger_done_time,
		       chip->power_expired_time, chip->power_disable_reason, tmp_buf);
}
...
static struct device_attribute *pencil_attributes[] = { &dev_attr_ble_mac_addr,
							&dev_attr_present,
							&dev_attr_fw_version,
							&dev_attr_tx_current,
							&dev_attr_tx_voltage,
...
static int ra9530_wireless_get_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int ra9530_wireless_set_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int ra9530_wireless_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc = 0;
...
static int ra9530_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_ra9530_ic), GFP_KERNEL);
	if (!chip) {
