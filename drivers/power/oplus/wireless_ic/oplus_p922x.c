...
static int p922x_decrease_vout_to_target(struct oplus_p922x_ic *chip, int vout_target, int vout_min)
{
	...
	chg_err("<~WPC~> vout_setting[%d], vout_sample[%d],target_voltage[%d]\n", chip->p922x_chg_status.charge_voltage,
		chip->p922x_chg_status.vout, vout_target);

	if (chip->p922x_chg_status.vout > vout_target) {
		if (chip->p922x_chg_status.charge_voltage > vout_target) {
			temp_value = p922x_decrease_vc_by_step(chip, chip->p922x_chg_status.charge_voltage, vout_target,
							       WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = p922x_decrease_vc_by_step(chip, chip->p922x_chg_status.charge_voltage, vout_min,
...
static int p922x_increase_vout_to_target(struct oplus_p922x_ic *chip, int vout_target, int vout_max)
{
	...
	if (chip->p922x_chg_status.vout < vout_target) {
		if (chip->p922x_chg_status.charge_voltage < vout_target) {
			temp_value = p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_voltage, vout_target,
							       WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_voltage, vout_max,
							       WPC_CHARGE_VOLTAGE_CHANGE_STEP_100MV);
		}
...
5V_en[%d], EXT2_otg_en[%d], EXT1_otg_en[%d]\n",
		p922x_get_vbat_en_val(), p922x_get_idt_con_val(), p922x_get_idt_int_val(), oplus_get_idt_en_val(),
		p922x_get_booster_en_val(), chargepump_get_chargepump_en_val(), oplus_get_wrx_en_val(),
		oplus_get_wrx_otg_en_val(), mp2650_wireless_get_mps_otg_en_val(), p922x_get_cp_ldo_5v_val(),
		p922x_get_ext2_wireless_otg_en_val(), p922x_get_ext1_wired_otg_en_val());
...
[Chg Vol: %d] [Chg Curr: %d] [Curr limit: %d] [ffc: %d] [silent: %d]\n",
		p922x_chip->p922x_chg_status.charge_status, p922x_chip->p922x_chg_status.adapter_type,
		p922x_chip->p922x_chg_status.charge_type, p922x_chip->p922x_chg_status.dock_version,
		p922x_chip->p922x_chg_status.charge_voltage, p922x_chip->p922x_chg_status.charge_current,
		p922x_chip->p922x_chg_status.fastchg_current_limit, p922x_chip->p922x_chg_status.wpc_ffc_charge,
...
[Batt vol max: %d] [Batt vol min: %d] [Term Vol: %d] [Term Curr: %d]\n",
		...
		g_oplus_chip->batt_volt_max, g_oplus_chip->batt_volt_min, mp2650_get_termchg_voltage(),
...
static int p922x_get_vrect_iout(struct oplus_p922x_ic *chip)
{
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		if ((p922x_get_CEP_flag(chip) == 0) &&
		    (abs(chip->p922x_chg_status.charge_voltage - vout_value) > 2000)) {
...
static void p922x_fastcharge_current_adjust_40w(struct oplus_p922x_ic *chip, bool is_init)
{
	...
	if (chip->p922x_chg_status.wpc_reach_stable_charge) {
		if (!chip->p922x_chg_status.wpc_reach_4370mv && (g_oplus_chip->batt_volt >= 4370)) {
			...
			if (fasctchg_current[chip->p922x_chg_status.fastcharge_level] >
...
{
	if ((chip->p922x_chg_status.wpc_reach_4370mv == false) && (g_oplus_chip->batt_volt <= 4370) &&
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_p922x_ic *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vooc_input_ma", &chip->p922x_chg_status.wpc_chg_param.vooc_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_iout_ma", &chip->p922x_chg_status.wpc_chg_param.vooc_iout_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_input_ma", &chip->p922x_chg_status.wpc_chg_param.svooc_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,wireless_power", &chip->p922x_chg_status.wireless_power);
	if (rc) {
		chip->p922x_chg_status.wireless_power = 0;
	}
	chg_err("wireless_power[%d]\n", chip->p922x_chg_status.wireless_power);

	rc = of_property_read_u32(node, "qcom,wireless_not_support_cool_down", &chip->wireless_not_support_cool_down);
	if (rc) {
...
int p922x_charge_set_max_current_by_tbatt(struct oplus_p922x_ic *chip)
{
	...
	case BATTERY_STATUS__NORMAL:
		...
		chip->p922x_chg_status.BPP_fastchg_current_ma =
			chip->p922x_chg_status.wpc_chg_param.bpp_temp_normal_fastchg_ma;
	...
	if (pre_tbatt != now_tbatt) {
		if (pre_tbatt == BATTERY_STATUS__REMOVED || pre_tbatt == BATTERY_STATUS__LOW_TEMP ||
		    pre_tbatt == BATTERY_STATUS__HIGH_TEMP || pre_tbatt == BATTERY_STATUS__COLD_TEMP ||
...
static int oplus_wpc_set_input_current(struct oplus_p922x_ic *chip)
{
		...
		if (chip->p922x_chg_status.BPP_current_step_cnt > 0) {
			...
			if (chip->p922x_chg_status.BPP_current_step_cnt == 0) {
				if (chip->p922x_chg_status.BPP_current_limit < 500) {
					...
					chip->p922x_chg_status.BPP_current_limit =
						chip->p922x_chg_status.wpc_chg_param.bpp_input_ma;
				}
			}
		}
	...
	if (current_limit != chip->p922x_chg_status.wpc_chg_param.pre_input_ma) {
		chg_err("<~WPC~> current_limit %d, pre_input_ma[%d], adapter_type[%d]\n", current_limit,
			chip->p922x_chg_status.wpc_chg_param.pre_input_ma, chip->p922x_chg_status.adapter_type);
...
static int p922x_charge_status_process(struct oplus_p922x_ic *chip)
{
		...
		if (chip->p922x_chg_status.charge_type == WPC_CHARGE_TYPE_FAST) {
			if (p922x_get_CEP_flag(chip) != 0) {
				...
				if (cep_nonzero_cnt > 10) {
					...
					if (chip->p922x_chg_status.cep_info <
					    to_cep_info(chip->p922x_chg_status.CEP_value))
						chip->p922x_chg_status.cep_info =
							to_cep_info(chip->p922x_chg_status.CEP_value);
				}
#endif
				break;
	...
	case WPC_CHG_STATUS_WAIT_IOUT_READY:
		...
		if (chip->p922x_chg_status.ftm_mode) {
			...
			if (chip->p922x_chg_status.adapter_type == ADAPTER_TYPE_VOOC || g_oplus_chip->soc >= 90 ||
			    g_oplus_chip->temperature < g_oplus_chip->limits.little_cool_bat_decidegc) {
				chip->p922x_chg_status.charge_status = WPC_CHG_STATUS_CHARGER_FASTCHG_INIT;
			} else {
				cep_nonzero_cnt = 0;
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			temp_value = p922x_increase_vout_to_target(chip, g_oplus_chip->batt_volt * 4 + 300,
		...
		if (chargepump_check_dwp_status() == -1) {
			...
			for (i = 0; i < CHARGEPUMP_DETECT_CNT; i++) {
				...
				if ((chip->p922x_chg_status.vout < (batt_volt * 4 - 500)) ||
					...
					chargepump_disable();
					chip->p922x_chg_status.charge_status =
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			...
			if (chip->p922x_chg_status.iout > iout_max_value) {
				...
				if (chip->p922x_chg_status.charge_voltage > WPC_CHARGE_VOLTAGE_CHGPUMP_MIN) {
					temp_value = chip->p922x_chg_status.iout - iout_max_value;
					if (chip->p922x_chg_status.iout > 2100) {
						p922x_set_rx_charge_voltage(
							chip, chip->p922x_chg_status.charge_voltage - 200);
					} else if (temp_value > 50) {
						p922x_set_rx_charge_voltage(
							chip, chip->p922x_chg_status.charge_voltage - 100);
					} else {
						p922x_set_rx_charge_voltage(chip,
									    chip->p922x_chg_status.charge_voltage - 20);
					}
				...
				if (chip->p922x_chg_status.charge_voltage < WPC_CHARGE_VOLTAGE_CHGPUMP_MAX) {
					...
					if ((temp_value > 100) && (chip->p922x_chg_status.iout < 1800)) {
						...
						p922x_set_rx_charge_voltage(chip,
									    chip->p922x_chg_status.charge_voltage + 50);
					} else {
						p922x_set_rx_charge_voltage(chip,
									    chip->p922x_chg_status.charge_voltage + 20);
					}
	...
	case WPC_CHG_STATUS_DECREASE_IOUT_TO_200MA:
		...
		if ((p922x_get_CEP_flag(chip) == 0) || chip->p922x_chg_status.wpc_skewing_proc) {
			if (chip->p922x_chg_status.iout > chip->p922x_chg_status.wpc_chg_param.curr_cp_to_charger) {
				if (chip->p922x_chg_status.charge_voltage > WPC_CHARGE_VOLTAGE_CHGPUMP_MIN) {
					temp_value = chip->p922x_chg_status.iout -
						     chip->p922x_chg_status.wpc_chg_param.curr_cp_to_charger;
					if (chip->p922x_chg_status.iout > 2100) {
						p922x_set_rx_charge_voltage(
							chip, chip->p922x_chg_status.charge_voltage - 200);
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_TO_12V:
		if ((p922x_get_CEP_flag(chip) == 0) || chip->p922x_chg_status.wpc_skewing_proc) {
			temp_value = p922x_decrease_vout_to_target(chip, WPC_CHARGE_VOLTAGE_CHGPUMP_TO_CHARGER,
	...
	case WPC_CHG_STATUS_INCREASE_CURRENT_FOR_CHARGER:
		...
		temp_value = p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_current,
						       chip->p922x_chg_status.fastchg_current_limit, 500);
		if (temp_value != 0) {
	...
	case WPC_CHG_STATUS_ADJUST_VOL_AFTER_INC_CURRENT:
		...
		if (chip->p922x_chg_status.iout >
		    (chip->p922x_chg_status.iout_stated_current + WPC_CHARGE_CURRENT_OFFSET)) {
			chg_err("<~WPC~>  IDT Iout > %dmA!\n",
				(chip->p922x_chg_status.iout_stated_current + WPC_CHARGE_CURRENT_OFFSET));
			temp_value = p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_voltage,
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		if (g_oplus_chip->temperature >= 420) {
			...
			temp_value = p922x_decrease_vc_by_step(chip, chip->p922x_chg_status.charge_current,
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			...
			if (chip->p922x_chg_status.iout > iout_max_value) {
				...
				if (chip->p922x_chg_status.charge_voltage >= WPC_CHARGE_VOLTAGE_FASTCHG_MAX) {
					temp_value =
						p922x_decrease_vc_by_step(chip, chip->p922x_chg_status.charge_current,
									  WPC_CHARGE_CURRENT_DEFAULT,
					...
					temp_value =
						p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_voltage,
									  WPC_CHARGE_VOLTAGE_FASTCHG_MAX,
				...
				if (chip->p922x_chg_status.charge_voltage > WPC_CHARGE_VOLTAGE_FASTCHG_MIN) {
					temp_value =
						p922x_decrease_vc_by_step(chip, chip->p922x_chg_status.charge_voltage,
					...
					temp_value =
						p922x_increase_vc_by_step(chip, chip->p922x_chg_status.charge_current,
									  chip->p922x_chg_status.fastchg_current_limit,
									  WPC_CHARGE_CURRENT_CHANGE_STEP_50MA);
					if (temp_value != 0) {
						p922x_set_rx_charge_current(chip, temp_value);
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_FOR_RESTART:
		...
		temp_value = p922x_decrease_vout_to_target(chip, g_oplus_chip->batt_volt * 4 + 300,
							   WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
		if (temp_value == 0) {
			...
			if (val_buf[0] == 0x4F) {
				...
				case P9237_RESPONE_LED_BRIGHTNESS:
					if ((tx_command == tx_command_r) && (tx_data == tx_data_r)) {
						...
						if (chip->p922x_chg_status.send_message ==
	...
	if (p922x_get_idt_con_val() == 1) {
		...
		if (p922x_chip->p922x_chg_status.charge_online == true) {
			...
			if (p922x_test_charging_status() >= 1) {
				...
				stop_timer = 0;
				p922x_set_vbat_en_val(1);
				schedule_delayed_work(&chip->p922x_test_work,
...
static void p922x_idt_int_eint_register(struct oplus_p922x_ic *chip)
{
	...
	retval = request_irq(chip->idt_int_irq, irq_idt_event_int_handler, IRQF_TRIGGER_FALLING, "p922x_idt_int",
...
static void p922x_idt_con_eint_register(struct oplus_p922x_ic *chip)
{
	...
	retval = request_irq(chip->idt_con_irq, irq_idt_connect_int_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
			     "p922x_con_int", chip); //0X01:rising edge,0x02:falling edge
	if (retval < 0) {
...
static void p922x_self_reset_process(struct work_struct *work)
{
	...
	if (p922x_get_vbat_en_val() == 1) {
		...
		p922x_set_vbat_en_val(0);
		if (chip->p922x_chg_status.wpc_self_reset) {
			schedule_delayed_work(&chip->p922x_self_reset_work,
...
static int p922x_wpc_get_max_wireless_power(void)
{
	...
	max_wireless_power = adapter_wireless_power > chip->p922x_chg_status.wireless_power ?
				     chip->p922x_chg_status.wireless_power :
...
static int p922x_wireless_get_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int p922x_wireless_set_prop(struct power_supply *psy, enum power_supply_property psp,
