...
static int cool_down_svooc[] = {
	0, 500, 500, 1200, 1200, 1500, 1500, 2000, 2000, 2500, 2500, 3000, 3000,
};
...
static bool oplus_get_chg_smem_info(struct oplus_nu1619_ic *chip)
{
	...
	if (IS_ERR(smem_addr)) {
		...
		if (smem_data == ERR_PTR(-EPROBE_DEFER)) {
			...
			memcpy(chip->nu1619_chg_status.random_num, &smem_data->wls_auth_data.random_num,
			       WLS_AUTH_RANDOM_LEN);
			memcpy(chip->nu1619_chg_status.rx_encode_num, &smem_data->wls_auth_data.encode_num,
			       WLS_AUTH_ENCODE_LEN);
...
static int nu1619_decrease_vout_to_target(struct oplus_nu1619_ic *chip, int vout_target, int vout_min)
{
	...
	chg_err("<~WPC~> vout_setting[%d], vout_sample[%d],target_voltage[%d]\n",
		chip->nu1619_chg_status.charge_voltage, chip->nu1619_chg_status.vout, vout_target);

	if (chip->nu1619_chg_status.vout > vout_target) {
		if (chip->nu1619_chg_status.charge_voltage > vout_target) {
			temp_value = nu1619_decrease_vc_by_step(chip, chip->nu1619_chg_status.charge_voltage,
								vout_target, WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = nu1619_decrease_vc_by_step(chip, chip->nu1619_chg_status.charge_voltage, vout_min,
...
static int nu1619_increase_vout_to_target(struct oplus_nu1619_ic *chip, int vout_target, int vout_max)
{
	...
	if (chip->nu1619_chg_status.vout < vout_target) {
		if (chip->nu1619_chg_status.charge_voltage < vout_target) {
			...
			temp_value = nu1619_increase_vc_by_step(chip, chip->nu1619_chg_status.charge_voltage, vout_max,
								WPC_CHARGE_VOLTAGE_CHANGE_STEP_100MV);
		}
...
static void nu1619_charger_init(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP ||
		...
		if (is_ext_chg_ops()) {
			g_oplus_chip->chg_ops->input_current_write_without_aicl(
				chip->nu1619_chg_status.epp_current_step);
		} else {
		...
		if (is_ext_chg_ops()) {
			g_oplus_chip->chg_ops->input_current_write_without_aicl(
				chip->nu1619_chg_status.BPP_current_limit);
		} else {
...
static void oplus_wpc_update_track_info(struct oplus_nu1619_ic *chip, char *crux_info)
{
	int index = 0;
	...
	index += snprintf(&crux_info[index], OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  ...
			  chg_status->tx_version, chg_status->rx_version, chg_status->boot_version,
			  chg_status->adapter_type, chg_status->dock_version, chg_status->fastchg_ing, chg_status->vout,
			  chg_status->iout, chg_status->rx_temperature, chg_status->wpc_dischg_status,
			  chg_status->break_count, chg_status->wpc_chg_err, chg_status->highest_temp,
			  chg_status->max_iout, chg_status->min_cool_down, chg_status->min_skewing_current,
			  chg_status->wls_auth_fail, chg_status->work_silent_mode);
	pr_info("%s\n", crux_info);
}
...
static int oplus_wpc_track_upload_trx_general_info(struct oplus_nu1619_ic *chip, char *trx_crux_info,
						   bool usb_present_once)
{
	int index = 0;

	memset(chip->trx_info_load_trigger.crux_info, 0, sizeof(chip->trx_info_load_trigger.crux_info));
	index += snprintf(
		&(chip->trx_info_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
		"$$total_time@@%d",
		(chip->nu1619_chg_status.trx_transfer_end_time - chip->nu1619_chg_status.trx_transfer_start_time) /
			WPC_TRX_INFO_THD_1MIN);

	index += snprintf(&(chip->trx_info_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
...
void nu1619_set_rtx_function(bool is_on)
{
	...
	if (is_on) {
		...
		chg_err("trx_online=%d, start_time=%d, end_time=%d, trx_usb_present_once\n",
			chip->nu1619_chg_status.tx_online, chip->nu1619_chg_status.trx_transfer_start_time,
			chip->nu1619_chg_status.trx_transfer_end_time, chip->nu1619_chg_status.trx_usb_present_once);
		if (chip->nu1619_chg_status.tx_online && chip->nu1619_chg_status.trx_transfer_start_time &&
		    (chip->nu1619_chg_status.trx_transfer_end_time - chip->nu1619_chg_status.trx_transfer_start_time >
...
5V_en[%d], EXT2_otg_en[%d], EXT1_otg_en[%d], internal_5V_ldo_en[%d], wls_pg[%d]\n",
		nu1619_get_vt_sleep_val(), nu1619_get_idt_con_val(), nu1619_get_idt_int_val(), oplus_get_idt_en_val(),
		nu1619_get_booster_en_val(), chargepump_get_chargepump_en_val(), oplus_get_wrx_en_val(),
		oplus_get_wrx_otg_en_val(), nu1619_get_cp_ldo_5v_val(), nu1619_get_ext2_wireless_otg_en_val(),
		nu1619_get_ext1_wired_otg_en_val(), nu1619_get_vbat_en_val(), oplus_get_wls_pg_value());
...
[Chg Vol: %d] [Chg Curr: %d] [Curr limit: %d] [ffc: %d] [silent: %d] [Rx temp: %d]\n",
		nu1619_chip->nu1619_chg_status.charge_status, nu1619_chip->nu1619_chg_status.adapter_type,
		nu1619_chip->nu1619_chg_status.charge_type, nu1619_chip->nu1619_chg_status.dock_version,
		nu1619_chip->nu1619_chg_status.charge_voltage, nu1619_chip->nu1619_chg_status.charge_current,
		nu1619_chip->nu1619_chg_status.fastchg_current_limit, nu1619_chip->nu1619_chg_status.wpc_ffc_charge,
		nu1619_chip->nu1619_chg_status.work_silent_mode, nu1619_chip->nu1619_chg_status.rx_temperature);

	chg_err("<~WPC~> [Temp: %d] [Soc: %d] [Chg curr: %d] [Batt vol: %d]\
[Batt vol max: %d] [Batt vol min: %d]\n",
		g_oplus_chip->temperature, g_oplus_chip->soc, g_oplus_chip->icharging, g_oplus_chip->batt_volt,
		g_oplus_chip->batt_volt_max, g_oplus_chip->batt_volt_min);
}
...
static bool nu1619_req_checksum_and_fw_version(struct oplus_nu1619_ic *chip, char *boot_check_sum, char *rx_check_sum,
					       char *tx_check_sum, char *boot_ver, char *rx_ver, char *tx_ver)
{
	char read_buffer1[4];
	char read_buffer2[5];
...
static bool nu1619_check_firmware_version(struct oplus_nu1619_ic *chip)
{
	...
	nu1619_req_checksum_and_fw_version(chip, &boot_checksum, &rx_checksum, &tx_checksum, &boot_version, &rx_version,
					   &tx_version);
	chg_err("check_sum: 0x%x,0x%x,0x%x  Version:0x%x,0x%x,0x%x \n", boot_checksum, rx_checksum, tx_checksum,
...
static int nu1619_check_idt_fw_update(struct oplus_nu1619_ic *chip)
{
	...
	if (g_oplus_chip->charger_exist == 1 || g_oplus_chip->charger_volt > 2500 || g_oplus_chip->ui_soc < 15) {
...
READ_IOUT_RETRY:
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		if ((nu1619_get_CEP_flag(chip) == 0) &&
		    (abs(chip->nu1619_chg_status.charge_voltage - vout_value) > 2000)) {
...
static void nu1619_fastcharge_current_adjust_40w(struct oplus_nu1619_ic *chip, bool is_init)
{
	...
	if (g_oplus_chip->temperature < g_oplus_chip->limits.little_cool_bat_decidegc ||
	...
	if (chip->nu1619_chg_status.wpc_reach_stable_charge) {
		if (!chip->nu1619_chg_status.wpc_reach_4370mv && (g_oplus_chip->batt_volt >= 4370)) {
			...
			if (fasctchg_current[chip->nu1619_chg_status.fastcharge_level] >
				...
				if ((g_oplus_chip->icharging < 0) &&
				    ((-1 * g_oplus_chip->icharging) < WPC_CHARGE_CURRENT_FFC_TO_CV)) {
...
{
	if ((chip->nu1619_chg_status.wpc_reach_4370mv == false) && (g_oplus_chip->batt_volt <= 4370) &&
...
static int oplus_wpc_parse_target_ichg_table(struct oplus_nu1619_ic *chip)
{
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("ffc_table[%d,%d,%d]\n", ffc_table[i].vbat, ffc_table[i].target_ichg,
			ffc_table[i].input_current);
	}
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("svooc_table[%d,%d,%d]\n", svooc_table[i].vbat, svooc_table[i].target_ichg,
			svooc_table[i].input_current);
	}
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("vooc_table[%d,%d,%d]\n", vooc_table[i].vbat, vooc_table[i].target_ichg,
			vooc_table[i].input_current);
	}
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("epp_table[%d,%d,%d]\n", epp_table[i].vbat, epp_table[i].target_ichg,
			epp_table[i].input_current);
	}
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("bpp_table[%d,%d,%d]\n", bpp_table[i].vbat, bpp_table[i].target_ichg,
			bpp_table[i].input_current);
	}
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_nu1619_ic *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,iout_ma", &chip->nu1619_chg_status.wpc_chg_param.iout_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param.iout_ma = 1000;
	}
	chg_err("iout_ma[%d]\n", chip->nu1619_chg_status.wpc_chg_param.iout_ma);

	rc = of_property_read_u32(node, "qcom,bpp_input_ma", &chip->nu1619_chg_status.wpc_chg_param.bpp_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,epp_input_ma", &chip->nu1619_chg_status.wpc_chg_param.epp_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,epp_8w_input_ma", &chip->nu1619_chg_status.wpc_chg_param.epp_8w_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_input_ma", &chip->nu1619_chg_status.wpc_chg_param.vooc_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_iout_ma", &chip->nu1619_chg_status.wpc_chg_param.vooc_iout_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_input_ma", &chip->nu1619_chg_status.wpc_chg_param.svooc_input_ma);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,wireless_max_chg_vol_mv", &chip->nu1619_chg_status.max_charge_voltage);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,support_airsvooc", &chip->nu1619_chg_status.support_airsvooc);
	if (rc) {
		chip->nu1619_chg_status.support_airsvooc = 1;
	}

	rc = of_property_read_u32(node, "qcom,phone_id", &chip->nu1619_chg_status.phone_id);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,wireless_power", &chip->nu1619_chg_status.wireless_power);
	if (rc) {
	...
	if (rc)
		...
		chip->nu1619_chg_status.wpc_chg_param.wireless_vooc_min_temp =
			-1 * chip->nu1619_chg_status.wpc_chg_param.wireless_vooc_min_temp;
	...
	if (rc)
		...
		chip->nu1619_chg_status.wpc_chg_param.wireless_svooc_min_temp =
			-1 * chip->nu1619_chg_status.wpc_chg_param.wireless_svooc_min_temp;
...
static void nu1619_charge_check_normal_temp_region(struct oplus_nu1619_ic *chip)
{
	...
	if (now_tbatt != BATTERY_STATUS__LITTLE_COOL_TEMP && now_tbatt != BATTERY_STATUS__NORMAL) {
		chip->nu1619_chg_status.normal_temp_region = NORMAL_TEMP_REGION_UNKNOW;
...
static int nu1916_charge_get_skewing_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if (cep_zero_cnt > 10) {
		...
		if (chip->nu1619_chg_status.normal_temp_region == NORMAL_TEMP_REGION1 && pre_wpc_skewing_proc == true) {
			pre_wpc_skewing_proc = false;
...
{
	if (chip->nu1619_chg_status.wpc_reach_4370mv == false && g_oplus_chip->batt_volt >= P922X_CC2CV_CHG_THD_HI) {
	...
	if (chip->nu1619_chg_status.wpc_ffc_charge == true) {
		if (g_oplus_chip->temperature < WPC_CHARGE_FFC_TEMP_MIN ||
		    g_oplus_chip->temperature > WPC_CHARGE_FFC_TEMP_MAX ||
		    chip->nu1619_chg_status.wpc_reach_4500mv == true ||
		    chip->nu1619_chg_status.work_silent_mode == true || chip->nu1619_chg_status.call_mode == true ||
		...
		if ((chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_SVOOC_50W ||
		    ...
		    chip->nu1619_chg_status.work_silent_mode == false && chip->nu1619_chg_status.call_mode == false &&
		    chip->nu1619_chg_status.wpc_reach_4370mv == false &&
		    chip->nu1619_chg_status.fastcharge_level <= FASTCHARGE_LEVEL_4 && g_oplus_chip->hmac == true) {
...
static int nu1619_charge_get_ffc_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if (ffc_level == FASTCHARGE_LEVEL_NUM || non_ffc_level == FASTCHARGE_LEVEL_NUM ||
	    g_oplus_chip->temperature < WPC_FASTCHG_TEMP_MIN || g_oplus_chip->temperature > WPC_FASTCHG_TEMP_MAX ||
	...
	if (chip->nu1619_chg_status.wpc_ffc_charge == true) {
		...
		if (non_ffc_level == FASTCHARGE_LEVEL_UNKNOW) {
			...
			if (pre_adapter_type == ADAPTER_TYPE_SVOOC_50W || pre_adapter_type == ADAPTER_TYPE_SVOOC ||
			    chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_THIRD_PARTY)
				non_ffc_level = FASTCHARGE_LEVEL_1;
			else if (pre_adapter_type == ADAPTER_TYPE_VOOC)
				non_ffc_level = FASTCHARGE_LEVEL_5;
			else if (pre_adapter_type == ADAPTER_TYPE_EPP && pre_rx_runing_mode == RX_RUNNING_MODE_EPP_15W)
...
static int nu1916_charge_get_cool_down_target_ichg(struct oplus_nu1619_ic *chip)
{
	...
	case ADAPTER_TYPE_SVOOC:
	case ADAPTER_TYPE_SVOOC_50W:
		if (cool_down >= ARRAY_SIZE(cool_down_svooc) && ARRAY_SIZE(cool_down_svooc) > 1)
			cool_down = ARRAY_SIZE(cool_down_svooc) - 1;
		cool_down_target_ichg = cool_down_svooc[cool_down];
		break;
	case ADAPTER_TYPE_VOOC:
		if (cool_down >= ARRAY_SIZE(cool_down_vooc) && ARRAY_SIZE(cool_down_vooc) > 1)
			cool_down = ARRAY_SIZE(cool_down_vooc) - 1;
		cool_down_target_ichg = cool_down_vooc[cool_down];
		break;
	case ADAPTER_TYPE_EPP:
		if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP_15W) {
			if (cool_down >= ARRAY_SIZE(cool_down_epp_15w) && ARRAY_SIZE(cool_down_epp_15w) > 1)
...
static bool nu1916_charge_switch_to_low_vout(struct oplus_nu1619_ic *chip)
{
	...
	if (oplus_chg_get_tbatt_status() == BATTERY_STATUS__COLD_TEMP ||
	    ...
	    chip->nu1619_chg_status.fastcharge_level >= FASTCHARGE_LEVEL_5 ||
	    chip->nu1619_chg_status.skewing_fastcharge_level >= FASTCHARGE_LEVEL_5 ||
	    chip->nu1619_chg_status.wpc_chg_param.target_ichg <=
		    chip->nu1619_chg_status.wpc_chg_param.vooc_target_ichg_little_cold_fastchg_ma ||
	    chip->nu1619_chg_status.work_silent_mode == true || chip->nu1619_chg_status.call_mode == true ||
...
static void nu1619_charge_set_target_ichg(struct oplus_nu1619_ic *chip)
{
	...
	if ((chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_SVOOC_50W ||
	     ...
	     chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_THIRD_PARTY) &&
	    (chip->nu1619_chg_status.work_silent_mode == true || chip->nu1619_chg_status.call_mode == true)) {
		if (chip->nu1619_chg_status.wpc_chg_param.target_ichg >
		    chip->nu1619_chg_status.wpc_chg_param.vooc_target_ichg_little_cold_fastchg_ma)
			chip->nu1619_chg_status.wpc_chg_param.target_ichg =
				chip->nu1619_chg_status.wpc_chg_param.vooc_target_ichg_little_cold_fastchg_ma;
	}
	...
	case ADAPTER_TYPE_SVOOC_50W:
		if ((chip->nu1619_chg_status.dock_version == DOCK_THIRD) &&
		    (chip->nu1619_chg_status.adapter_power == ADAPTER_POWER_THIRD_20W)) {
	...
	default:
		if (chip->nu1619_chg_status.iout_stated_current == bpp_table[0].input_current &&
		    chip->nu1619_chg_status.iout < bpp_table[0].input_current - 100 && bpp_table[0].input_current > 0) {
			if (g_oplus_chip->chg_ops && g_oplus_chip->chg_ops->rerun_wls_aicl &&
			    chip->nu1619_chg_status.rerun_wls_aicl_count < 20) {
				g_oplus_chip->chg_ops->rerun_wls_aicl();
				chip->nu1619_chg_status.rerun_wls_aicl_count++;
	...
	chg_err("<~WPC~> t_iin-iout[%d-%d]: t-cool-cv_ichg[%d-%d-%d], input[%d]; %sffc-skew_ichg[%d-%d], input[%d-%d]; vbatt[%d]\n",
		chip->nu1619_chg_status.target_iin, chip->nu1619_chg_status.iout_stated_current,
		chip->nu1619_chg_status.wpc_chg_param.target_ichg, cool_down_target_ichg, cv_target_ichg, input_current,
		(chip->nu1619_chg_status.wpc_ffc_charge == true) ? "" : "non-",
		fasctchg_current[chip->nu1619_chg_status.fastcharge_level],
		fasctchg_current[chip->nu1619_chg_status.skewing_fastcharge_level], ffc_input_current,
		skewing_input_current, vbatt);
}
...
{
	chip->nu1619_chg_status.highest_temp = max(chip->nu1619_chg_status.highest_temp, g_oplus_chip->temperature);

	chip->nu1619_chg_status.max_iout = max(chip->nu1619_chg_status.max_iout, chip->nu1619_chg_status.iout);
...
int nu1619_charge_set_max_current_by_tbatt(struct oplus_nu1619_ic *chip)
{
	...
	case BATTERY_STATUS__NORMAL:
		...
		chip->nu1619_chg_status.BPP_fastchg_current_ma =
			chip->nu1619_chg_status.wpc_chg_param.bpp_temp_normal_fastchg_ma;
		switch (chip->nu1619_chg_status.adapter_type) {
		case ADAPTER_TYPE_VOOC:
	...
	if (pre_tbatt != now_tbatt) {
		if (pre_tbatt == BATTERY_STATUS__REMOVED || pre_tbatt == BATTERY_STATUS__LOW_TEMP ||
...
static int oplus_wpc_set_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if (current_limit != chip->nu1619_chg_status.wpc_chg_param.pre_input_ma && current_limit != 0) {
		chg_err("<~WPC~> current_limit %d, pre_input_ma[%d], adapter_type[%d]\n", current_limit,
			chip->nu1619_chg_status.wpc_chg_param.pre_input_ma, chip->nu1619_chg_status.adapter_type);
...
int nu1619_charge_set_max_current_by_adapter_power(struct oplus_nu1619_ic *chip)
{
	if (chip->nu1619_chg_status.dock_version != DOCK_THIRD) {
		...
		case ADAPTER_POWER_65W:
			if (nu1619_test_charging_status() == 4) {
				if (chip->nu1619_chg_status.fastchg_current_limit >
				    fasctchg_current[FASTCHARGE_LEVEL_2])
					chip->nu1619_chg_status.fastchg_current_limit =
						fasctchg_current[FASTCHARGE_LEVEL_2];
			}
...
static int nu1619_charge_status_process(struct oplus_nu1619_ic *chip)
{
...
#ifndef FASTCHG_TEST_BY_TIME
	if ((g_oplus_chip->temperature < g_oplus_chip->limits.cold_bat_decidegc) ||
	    (g_oplus_chip->temperature > g_oplus_chip->limits.hot_bat_decidegc)) {
		...
		if ((chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_VOOC ||
			...
			if (g_oplus_chip->batt_volt_max > 4100) {
				...
				freq_thr = chip->nu1619_chg_status.freq_threshold +
					   (g_oplus_chip->batt_volt_max - 3700) / 200;
			} else {
				freq_thr = chip->nu1619_chg_status.freq_threshold;
			}
	...
	case WPC_CHG_STATUS_READY_FOR_EPP:
		...
		if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP_15W) {
			if ((chip->nu1619_chg_status.epp_current_limit >=
				     chip->nu1619_chg_status.wpc_chg_param.epp_temp_warm_input_ma &&
			     oplus_chg_get_tbatt_status() == BATTERY_STATUS__WARM_TEMP) ||
			    chip->nu1619_chg_status.epp_current_limit >=
				    chip->nu1619_chg_status.wpc_chg_param.epp_15w_input_ma) {
	...
	case WPC_CHG_STATUS_EPP_WORKING:
		if (chip->nu1619_chg_status.engineering_mode == true &&
		    ...
		    chip->nu1619_chg_status.send_message == P9221_CMD_NULL && chip->nu1619_chg_status.rx_power == 0) {
			msleep(5000);
		...
		if (chip->nu1619_chg_status.charge_type == WPC_CHARGE_TYPE_FAST) {
			if (nu1619_get_CEP_flag(chip) != 0) {
				...
				if (cep_nonzero_cnt > 10) {
					...
					if (chip->nu1619_chg_status.cep_info <
					    to_cep_info(chip->nu1619_chg_status.CEP_value))
						chip->nu1619_chg_status.cep_info =
							to_cep_info(chip->nu1619_chg_status.CEP_value);
				}
#endif
				break;
	...
	case WPC_CHG_STATUS_WAIT_IOUT_READY:
		...
		if (chip->nu1619_chg_status.ftm_mode) {
			...
			if (chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_VOOC ||
			    (g_oplus_chip->wpc_no_chargerpump &&
			     (chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_SVOOC ||
			      chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_THIRD_PARTY)) ||
			    g_oplus_chip->soc >= 90 ||
			    g_oplus_chip->temperature < g_oplus_chip->limits.little_cool_bat_decidegc) {
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (nu1619_get_CEP_flag(chip) == 0) {
			temp_value = nu1619_increase_vout_to_target(chip, g_oplus_chip->batt_volt * 4 + 300,
								    WPC_CHARGE_VOLTAGE_CHGPUMP_MAX);
			if (temp_value != 0) {
				nu1619_reset_CEP_flag(chip);
			} else {
		...
		if (chargepump_check_dwp_status() == -1) {
			...
			for (i = 0; i < CHARGEPUMP_DETECT_CNT; i++) {
				...
				if ((chip->nu1619_chg_status.vout < (batt_volt * 4 - 500)) ||
					...
					chargepump_disable();
					chip->nu1619_chg_status.charge_status =
		...
		if (nu1619_get_CEP_flag(chip) == 0) {
			...
			if (chip->nu1619_chg_status.iout > iout_max_value) {
				...
				if (chip->nu1619_chg_status.charge_voltage > WPC_CHARGE_VOLTAGE_CHGPUMP_MIN) {
					temp_value = chip->nu1619_chg_status.iout - iout_max_value;
					if (chip->nu1619_chg_status.iout > 2100) {
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage - 200);
					} else if (temp_value > 50) {
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage - 100);
					} else {
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage - 20);
					}
				...
				if (chip->nu1619_chg_status.charge_voltage < WPC_CHARGE_VOLTAGE_CHGPUMP_MAX) {
					...
					if ((temp_value > 100) && (chip->nu1619_chg_status.iout < 2200)) {
						...
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage + 50);
					} else {
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage + 20);
					}
	...
	case WPC_CHG_STATUS_DECREASE_IOUT_TO_200MA:
		...
		if ((nu1619_get_CEP_flag(chip) == 0) || chip->nu1619_chg_status.wpc_skewing_proc) {
			if (chip->nu1619_chg_status.iout > chip->nu1619_chg_status.wpc_chg_param.curr_cp_to_charger) {
				if (chip->nu1619_chg_status.charge_voltage > WPC_CHARGE_VOLTAGE_CHGPUMP_MIN) {
					temp_value = chip->nu1619_chg_status.iout -
						     chip->nu1619_chg_status.wpc_chg_param.curr_cp_to_charger;
					if (chip->nu1619_chg_status.iout > 2100) {
						nu1619_set_rx_charge_voltage(
							chip, chip->nu1619_chg_status.charge_voltage - 200);
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_TO_12V:
		if ((nu1619_get_CEP_flag(chip) == 0) || chip->nu1619_chg_status.wpc_skewing_proc) {
			temp_value = nu1619_decrease_vout_to_target(chip, WPC_CHARGE_VOLTAGE_CHGPUMP_TO_CHARGER,
								    WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
			if (temp_value == 0) {
				if (is_ext_chg_ops()) {
	...
	case WPC_CHG_STATUS_INCREASE_CURRENT_FOR_CHARGER:
		...
		temp_value = nu1619_increase_vc_by_step(chip, chip->nu1619_chg_status.charge_current,
							chip->nu1619_chg_status.fastchg_current_limit, 500);
		if (temp_value != 0) {
	...
	case WPC_CHG_STATUS_ADJUST_VOL_AFTER_INC_CURRENT:
		...
		if (chip->nu1619_chg_status.iout >
		    (chip->nu1619_chg_status.iout_stated_current + WPC_CHARGE_CURRENT_OFFSET)) {
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		if (chip->nu1619_chg_status.engineering_mode == true && chip->nu1619_chg_status.iout >= 950 &&
			...
			if ((nu1619_get_CEP_flag(chip) == 0) && (chip->nu1619_chg_status.rx_power == 0) &&
			    (chip->nu1619_chg_status.dock_version != DOCK_THIRD)) {
				chip->nu1619_chg_status.rx_power = -1;
				chip->nu1619_chg_status.send_message = P9221_CMD_GET_RXTX_POWER;
				chip->nu1619_chg_status.send_msg_cnt = 10;
		...
		if (chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_SVOOC ||
			...
			if (nu1916_charge_switch_to_low_vout(chip) == true) {
				...
				if (chip->nu1619_chg_status.charge_voltage <
				    chip->nu1619_chg_status.max_charge_voltage) {
					temp_value =
						nu1619_increase_vc_by_step(chip, chip->nu1619_chg_status.charge_voltage,
									   chip->nu1619_chg_status.max_charge_voltage,
									   WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
					if (temp_value != 0) {
						nu1619_set_rx_charge_voltage(chip, temp_value);
	...
	case WPC_CHG_STATUS_READY_FOR_FTM:
		...
		if (nu1619_get_CEP_flag(chip) == 0) {
			temp_value =
				nu1619_increase_vc_by_step(chip, chip->nu1619_chg_status.charge_voltage,
							   WPC_CHARGE_VOLTAGE_FTM, WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
			if (temp_value != 0) {
				nu1619_set_rx_charge_voltage(chip, temp_value);
				nu1619_reset_CEP_flag(chip);
			} else {
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_FOR_RESTART:
		...
		temp_value = nu1619_decrease_vout_to_target(chip, g_oplus_chip->batt_volt * 4 + 300,
							    WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
		if (temp_value == 0) {
...
static int oplus_wpc_track_upload_trx_err_info(struct oplus_nu1619_ic *chip, char *trx_crux_info, u8 trx_err)
{
	int index = 0;
	...
	memset(chip->trx_err_load_trigger.crux_info, 0, sizeof(chip->trx_err_load_trigger.crux_info));
	index += snprintf(&(chip->trx_err_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_reason@@%s", trx_err_reason);
	if (trx_crux_info && strlen(trx_crux_info))
		index += snprintf(&(chip->trx_err_load_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				  "%s", trx_crux_info);
...
*/
		...
		if (P922X_RTX_READY & regdata[0]) {
			...
			if (!chip->nu1619_chg_status.trx_usb_present_once)
				chip->nu1619_chg_status.trx_usb_present_once = oplus_get_wired_chg_present();
	...
	if (chip->nu1619_chg_status.tx_online != pre_tx_online) {
		...
		if (pre_tx_online && !chip->nu1619_chg_status.tx_online) {
			...
			chip->nu1619_chg_status.trx_usb_present_once = false;
			chip->nu1619_chg_status.trx_transfer_start_time = oplus_wpc_get_local_time_s();
		}
		pre_tx_online = chip->nu1619_chg_status.tx_online;
		wpc_battery_update();
	}
			...
			if (val_buf[0] == 0x4F || val_buf[0] == 0x5F) {
				...
				case P9237_RESPONE_LED_BRIGHTNESS:
					if ((tx_command == tx_command_r) && (tx_data == tx_data_r)) {
						...
						if (chip->nu1619_chg_status.send_message ==
						    P9221_CMD_SET_LED_BRIGHTNESS) {
							chip->nu1619_chg_status.send_message = P9221_CMD_NULL;
						}
					}
					break;
				case P9237_RESPONE_RXTX_POWER:
					if (val_buf[0] == 0x5F) {
						...
						chg_err("<~WPC~> rx_power[%d], tx_power[%d]\n",
							chip->nu1619_chg_status.rx_power,
							chip->nu1619_chg_status.tx_power);
				...
				case P9237_RESPONE_SEND_3RD_DECODE_DATA:
					...
					chg_err("<~WPC~><~VRY~> tx encode number: %02x %02x %02x %02x %02x %02x %02x %02x\n",
						chip->nu1619_chg_status.tx_encode_num[0],
						chip->nu1619_chg_status.tx_encode_num[1],
						chip->nu1619_chg_status.tx_encode_num[2],
						chip->nu1619_chg_status.tx_encode_num[3],
						chip->nu1619_chg_status.tx_encode_num[4],
						chip->nu1619_chg_status.tx_encode_num[5],
						chip->nu1619_chg_status.tx_encode_num[6],
						chip->nu1619_chg_status.tx_encode_num[7]);

					if ((chip->nu1619_chg_status.tx_encode_num[0] ==
					     chip->nu1619_chg_status.rx_encode_num[0]) &&
					    (chip->nu1619_chg_status.tx_encode_num[1] ==
					     chip->nu1619_chg_status.rx_encode_num[1]) &&
					    (chip->nu1619_chg_status.tx_encode_num[2] ==
					     chip->nu1619_chg_status.rx_encode_num[2]) &&
					    (chip->nu1619_chg_status.tx_encode_num[3] ==
					     chip->nu1619_chg_status.rx_encode_num[3]) &&
					    (chip->nu1619_chg_status.tx_encode_num[4] ==
					     chip->nu1619_chg_status.rx_encode_num[4]) &&
					    (chip->nu1619_chg_status.tx_encode_num[5] ==
					     chip->nu1619_chg_status.rx_encode_num[5]) &&
					    (chip->nu1619_chg_status.tx_encode_num[6] ==
					     chip->nu1619_chg_status.rx_encode_num[6]) &&
					    (chip->nu1619_chg_status.tx_encode_num[7] ==
					     chip->nu1619_chg_status.rx_encode_num[7])) {
				...
				case P9237_RESPONE_VENDOR_ID:
					if ((tx_command == tx_command_r) && (tx_data == tx_data_r)) {
						...
						chg_err("<~WPC~><~VRY~> random datas: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[0],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[1],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[2],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[3],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[4],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[5],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[6],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[7],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[8],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[9],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[10],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[11],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[12],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[13],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[14],
							chip->nu1619_chg_status.aes_verfith_data.aes_random_num[15]);
						chg_err("<~WPC~><~VRY~> rx encode datas: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[0],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[1],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[2],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[3],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[4],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[5],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[6],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[7],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[8],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[9],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[10],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[11],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[12],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[13],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[14],
							chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[15]);
				...
				case P9221_CMD_RESPONE_GET_AES_DATA6:
					...
					for (i = 0; i < WLS_AUTH_AES_ENCODE_LEN; i++)
						pr_err("tx send to ap  tx_encode_num[%d]:0x%02x\n", i,
						       (chip->nu1619_chg_status.tx_encode_num)[i]);

					for (i = 0; i < WLS_AUTH_AES_ENCODE_LEN; i++)
						pr_err("ap send to tx rx_encode_num[%d]:0x%02x ", i,
						       (chip->nu1619_chg_status.aes_verfith_data.aes_encode_num)[i]);

					for (i = 0; i < WLS_AUTH_AES_DATA_LEN; i++) {
						if (chip->nu1619_chg_status.tx_encode_num[i] !=
						    chip->nu1619_chg_status.aes_verfith_data.aes_encode_num[i]) {
							chg_err("<~WPC~><~VRY~> VERIFY FAIL! RETRY[%d]\n",
								chip->nu1619_chg_status.dock_verify_retry);
							if (chip->nu1619_chg_status.dock_verify_retry == 0) {
								...
								chip->nu1619_chg_status.charge_status =
									WPC_CHG_STATUS_VERIFY_FAIL;
								chip->nu1619_chg_status.dock_verify_status =
									DOCK_VERIFY_FAIL;
							} else {
								chip->nu1619_chg_status.send_message = P9221_CMD_NULL;
								chip->nu1619_chg_status.charge_status =
...
static void nu1619_idt_event_int_func(struct work_struct *work)
{
	...
	if (nu1619_chip->nu1619_chg_status.charge_online == true || nu1619_wpc_get_otg_charging() == true) {
...
static void nu1619_idt_connect_int_func(struct work_struct *work)
{
...
#ifndef FASTCHG_TEST_BY_TIME
	if (oplus_get_wired_chg_present() == true && (chip->wireless_mode == WIRELESS_MODE_NULL) &&
	...
	if (nu1619_chip->booster_en_gpio <= 0 && oplus_get_wired_otg_online() == true &&
	    nu1619_get_idt_con_val() == 1) {
	...
	if (nu1619_get_idt_con_val() == 1) {
		...
		if (nu1619_chip->nu1619_chg_status.charge_online == false) {
			...
			oplus_chg_track_check_wls_charging_break(nu1619_chip->nu1619_chg_status.charge_online);
			...
			schedule_delayed_work(&chip->charger_start_work,
		...
		if (nu1619_chip->nu1619_chg_status.charge_online == true) {
			...
			schedule_delayed_work(&chip->charger_disconnect_work,
			...
			if (nu1619_test_charging_status() >= 1) {
				...
				stop_timer = 0;
				nu1619_set_vt_sleep_val(1);
				schedule_delayed_work(&chip->nu1619_test_work,
...
static void nu1619_idt_int_eint_register(struct oplus_nu1619_ic *chip)
{
	...
	retval =
		request_irq(chip->idt_int_irq, irq_idt_event_int_handler, IRQF_TRIGGER_FALLING, "nu1619_idt_int", chip);
	if (retval < 0) {
...
static void nu1619_idt_con_eint_register(struct oplus_nu1619_ic *chip)
{
	...
	retval = request_irq(chip->idt_con_irq, irq_idt_connect_int_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
			     "nu1619_con_int", chip);
	if (retval < 0) {
...
void nu1619_set_ext1_wired_otg_en_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->ext1_wired_otg_en_active) ||
	    IS_ERR_OR_NULL(chip->ext1_wired_otg_en_sleep) || IS_ERR_OR_NULL(chip->ext1_wired_otg_en_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
...
void nu1619_set_ext2_wireless_otg_en_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->ext2_wireless_otg_en_active) ||
	    IS_ERR_OR_NULL(chip->ext2_wireless_otg_en_sleep) || IS_ERR_OR_NULL(chip->ext2_wireless_otg_en_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
...
static int nu1619_ext3_wireless_otg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->ext3_wireless_otg_en_active = pinctrl_lookup_state(chip->pinctrl, "ext3_wireless_otg_en_active");
	if (IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_active)) {
		chg_err("get ext3_wireless_otg_en_active fail\n");
		return -EINVAL;
	}

	chip->ext3_wireless_otg_en_sleep = pinctrl_lookup_state(chip->pinctrl, "ext3_wireless_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_sleep)) {
		chg_err("get ext3_wireless_otg_en_sleep fail\n");
		return -EINVAL;
	}

	chip->ext3_wireless_otg_en_default = pinctrl_lookup_state(chip->pinctrl, "ext3_wireless_otg_en_default");
	...
	pinctrl_select_state(chip->pinctrl, chip->ext3_wireless_otg_en_active);
...
void nu1619_set_ext3_wireless_otg_en_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_active) ||
	    IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_sleep) || IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
	...
	if (value) {
		...
		pinctrl_select_state(chip->pinctrl, chip->ext3_wireless_otg_en_default);
	} else {
		gpio_direction_output(chip->ext3_wireless_otg_en_gpio, 0);
		pinctrl_select_state(chip->pinctrl, chip->ext3_wireless_otg_en_sleep);
	}
...
static int nu1619_vt_sleep_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->vt_sleep_active = pinctrl_lookup_state(chip->pinctrl, "vt_sleep_active");
	if (IS_ERR_OR_NULL(chip->vt_sleep_active)) {
		chg_err("get vt_sleep_active fail\n");
		return -EINVAL;
	}

	chip->vt_sleep_sleep = pinctrl_lookup_state(chip->pinctrl, "vt_sleep_sleep");
	if (IS_ERR_OR_NULL(chip->vt_sleep_sleep)) {
		chg_err("get vt_sleep_sleep fail\n");
		return -EINVAL;
	}

	chip->vt_sleep_default = pinctrl_lookup_state(chip->pinctrl, "vt_sleep_default");
	...
	if (oplus_get_wired_chg_present() == true) {
		...
		pinctrl_select_state(chip->pinctrl, chip->vt_sleep_active);
	} else {
		gpio_direction_output(chip->vt_sleep_gpio, 0);
		pinctrl_select_state(chip->pinctrl, chip->vt_sleep_sleep);
	}
...
void nu1619_set_vt_sleep_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->vt_sleep_active) ||
	    IS_ERR_OR_NULL(chip->vt_sleep_sleep) || IS_ERR_OR_NULL(chip->vt_sleep_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
	...
	if (value) {
		gpio_direction_output(chip->vt_sleep_gpio, 1);
		pinctrl_select_state(chip->pinctrl, chip->vt_sleep_default);
	} else {
		gpio_direction_output(chip->vt_sleep_gpio, 0);
		pinctrl_select_state(chip->pinctrl, chip->vt_sleep_sleep);
	}
...
void nu1619_set_cp_ldo_5v_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->cp_ldo_5v_active) ||
	    IS_ERR_OR_NULL(chip->cp_ldo_5v_sleep) || IS_ERR_OR_NULL(chip->cp_ldo_5v_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
...
static int oplus_wrx_en_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: gpio value[%d]!\n", __func__, gpio_get_value(chip->wrx_en_gpio));
...
static int oplus_wrx_otg_en_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: gpio value[%d]!\n", __func__, gpio_get_value(chip->wrx_otg_en_gpio));
	return 0;
}
...
void oplus_set_wrx_otg_en_value(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->wrx_otg_en_active) ||
	...
	if (value == 1) {
		...
		pinctrl_select_state(chip->pinctrl, chip->wrx_otg_en_active);
	} else {
		gpio_direction_output(chip->wrx_otg_en_gpio, 0);
		pinctrl_select_state(chip->pinctrl, chip->wrx_otg_en_sleep);
	}
...
static int oplus_wls_pg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: gpio value[%d]!\n", __func__, gpio_get_value(chip->wls_pg_gpio));
	return 0;
}
...
static void oplus_set_wls_pg_value(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->wls_pg_active) ||
...
static void nu1619_task_work_process(struct work_struct *work)
{
	...
	if (nu1619_chip->nu1619_chg_status.charge_online) {
		...
		if (nu1619_get_vrect_iout(chip) != 0 && nu1619_chip->nu1619_chg_status.charge_online) {
...
static void nu1619_self_reset_process(struct work_struct *work)
{
	...
	if (nu1619_get_vt_sleep_val() == 1) {
		...
		nu1619_set_vt_sleep_val(0);
		if (chip->nu1619_chg_status.wpc_self_reset) {
			schedule_delayed_work(&chip->nu1619_self_reset_work,
					      round_jiffies_relative(msecs_to_jiffies(4500)));
		}
	} else {
...
static void oplus_chg_wls_get_third_part_verity_data_work_v1(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, wls_get_third_part_verity_data_work_v1);

	oplus_chg_set_mutual_cmd(CMD_WLS_THIRD_PART_AUTH, sizeof(chip->nu1619_chg_status.vendor_id),
				 &(chip->nu1619_chg_status.vendor_id));
}
...
static ssize_t proc_wireless_rx_voltage_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	char vol_string[8];
	int len = 0;
	len = snprintf(vol_string, 8, "%d", nu1619_chip->nu1619_chg_status.charge_voltage);
...
static bool nu1619_upgrade_firmware(struct oplus_nu1619_ic *chip, u8 *fw_buf, int fw_size, char area)
{
	u16 addr = 0;
	u8 addr_h = 0;
	u8 addr_l = 0;
	...
	for (i = 0; i < fw_size; i += 4) {
		...
		if ((read_buf[0] == (0xff & (~fw_data[i + 0]))) && (read_buf[1] == (0xff & (~fw_data[i + 1]))) &&
		    (read_buf[2] == (0xff & (~fw_data[i + 2]))) && (read_buf[3] == (0xff & (~fw_data[i + 3])))) {
			pass_count++;
		} else {
			j++;
			if (j >= 50) {
...
CCC:
	...
	chg_err(" fw_data version=0x%x, 0x%x, 0x%x, 0x%x\n", 0xff & (~fw_data[fw_size - 4]),
		0xff & (~fw_data[fw_size - 3]), 0xff & (~fw_data[fw_size - 2]), 0xff & (~fw_data[fw_size - 1]));
...
start:
	...
	case UPGRADE_START:
		...
		if (fw_head->magic[0] == 0x02 && fw_head->magic[1] == 0x00 && fw_head->magic[2] == 0x03 &&
			...
			if (copy_from_user(fw_buf, buf + sizeof(struct idt_fw_head),
					   count - sizeof(struct idt_fw_head))) {
				chg_err("copy from user error\n");
				return -EFAULT;
			}
...
static int init_wireless_charge_proc(struct oplus_nu1619_ic *chip)
{
	...
	prEntry_tmp = proc_create_data("rx_power", 0664, prEntry_da, &proc_wireless_rx_power_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}

	prEntry_tmp = proc_create_data("tx_power", 0664, prEntry_da, &proc_wireless_tx_power_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}

	prEntry_tmp = proc_create_data("rx_version", 0664, prEntry_da, &proc_wireless_rx_version_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}

	prEntry_tmp = proc_create_data("wired_otg_online", 0664, prEntry_da, &proc_wired_otg_online_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(dwork, struct oplus_nu1619_ic, status_keep_clean_work);
	struct power_supply *wls_psy;
...
static enum power_supply_property nu1619_wireless_props[] = {
	POWER_SUPPLY_PROP_PRESENT,	  POWER_SUPPLY_PROP_ONLINE,	    POWER_SUPPLY_PROP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_VOLTAGE_MAX,	  POWER_SUPPLY_PROP_CURRENT_NOW,    POWER_SUPPLY_PROP_CURRENT_MAX,
	POWER_SUPPLY_PROP_REAL_TYPE,	  POWER_SUPPLY_PROP_TX_VOLTAGE_NOW, POWER_SUPPLY_PROP_TX_CURRENT_NOW,
	POWER_SUPPLY_PROP_CP_VOLTAGE_NOW, POWER_SUPPLY_PROP_CP_CURRENT_NOW, POWER_SUPPLY_PROP_WIRELESS_MODE,
	POWER_SUPPLY_PROP_WIRELESS_TYPE,  POWER_SUPPLY_PROP_CEP_INFO,
};

static int nu1619_wireless_get_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int nu1619_wireless_set_prop(struct power_supply *psy, enum power_supply_property psp,
				    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CEP_INFO:
		if (val->intval == 0) {
			chg_err("clear cep info = %d, wpc_skewing_proc = %d\n", nu1619_chip->nu1619_chg_status.cep_info,
				nu1619_chip->nu1619_chg_status.wpc_skewing_proc);
...
static int nu1619_wireless_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc;
...
static int nu1619_wpc_get_max_wireless_power_ower(void)
{
	...
	max_wireless_power = adapter_wireless_power > chip->nu1619_chg_status.wireless_power ?
				     chip->nu1619_chg_status.wireless_power :
				     adapter_wireless_power;
...
static void nu1619_wpc_track_trx_info_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(dwork, struct oplus_nu1619_ic, trx_info_load_trigger_work);
...
static void nu1619_wpc_track_trx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(dwork, struct oplus_nu1619_ic, trx_err_load_trigger_work);
...
static int nu1619_wpc_track_init(struct oplus_nu1619_ic *chip)
{
	...
	INIT_DELAYED_WORK(&chip->trx_err_load_trigger_work, nu1619_wpc_track_trx_err_load_trigger_work);
	INIT_DELAYED_WORK(&chip->trx_info_load_trigger_work, nu1619_wpc_track_trx_info_load_trigger_work);

	return rc;
}

static int oplus_chg_wls_aes_mutual_notifier_call_v1(struct notifier_block *nb, unsigned long val, void *v)
{
	int i;
	struct oplus_nu1619_ic *chip;
	...
	if (aes_auth_result) {
		...
		memcpy(chip->nu1619_chg_status.aes_verfith_data.aes_random_num, aes_auth_result->aes_random_num,
		       sizeof(aes_auth_result->aes_random_num));
		for (i = 0; i < WLS_AUTH_AES_ENCODE_LEN; i++)
			chg_err("aes_random_num_v1[%d]:0x%02x\n", i,
				(chip->nu1619_chg_status.aes_verfith_data.aes_random_num)[i]);

		memcpy(chip->nu1619_chg_status.aes_verfith_data.aes_encode_num, aes_auth_result->aes_encode_num,
...
static int nu1619_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_nu1619_ic), GFP_KERNEL);
	if (!chip) {
