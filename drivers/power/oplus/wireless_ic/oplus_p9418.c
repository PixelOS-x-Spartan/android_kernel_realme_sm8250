...
void p9418_check_point_function(struct work_struct *work)
{
	...
	if (chip->pen_support_track) {
		...
		if ((g_ble_timeout_cnt != 0) || (g_verify_failed_cnt != 0) ||
		    (chip->debug_pen_match_state_trigger == true)) {
...
static int p9418_load_bootloader(struct oplus_p9418_ic *chip)
{
	...
	rc = p9418_write_reg_multi_byte(chip, 0x0800, MTPBootloader9415,
...
static int p9418_load_fw(struct oplus_p9418_ic *chip, unsigned char *fw_data, int CodeLength)
{
	...
	rc = p9418_write_reg_multi_byte(chip, 0x400, fw_data, ((CodeLength + 8 + 15) / 16) * 16);
	if (rc != 0) {
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_p9418_ic *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,pen_support_track", &chip->pen_support_track);
	if (rc) {
...
void p9418_ept_type_detect_func(struct oplus_p9418_ic *chip)
{
	...
	if (rc) {
		...
		if ((recv_data[1] & P9418_RTX_ERR_TX_POCP) || (recv_data[1] & P9418_RTX_ERR_TX_OTP) ||
		    (recv_data[1] & P9418_RTX_ERR_TX_FOD) || (recv_data[1] & P9418_RTX_ERR_TX_LVP) ||
		    (recv_data[1] & P9418_RTX_ERR_TX_OVP) || (recv_data[1] & P9418_RTX_ERR_TX_OCP)) {
...
static void p9418_commu_data_process(struct oplus_p9418_ic *chip)
{
	...
	chg_err("int ATO 0x2C: %02x %02x %02x %02x\n", chip->int_flag_data[0], chip->int_flag_data[1],
		chip->int_flag_data[2], chip->int_flag_data[3]);
...
static int p9418_wireless_get_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int p9418_wireless_set_prop(struct power_supply *psy, enum power_supply_property psp,
...
static int p9418_wireless_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc = 0;
...
static int p9418_track_debugfs_init(struct oplus_p9418_ic *chip)
{
	...
	debugfs_create_u32("debug_pen_match_state_trigger", 0644, debugfs_p9418,
			   &(chip->debug_pen_match_state_trigger));
...
static void p9418_track_pen_match_state_info(struct oplus_p9418_ic *chip, int err_type)
{
	...
	index += snprintf(&(chip->pen_match_state_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "p9418");
	index += snprintf(&(chip->pen_match_state_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_PEN_MATCH_ERR);
	...
	index += snprintf(&(chip->pen_match_state_trigger.crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$pen_timeout@@%d"
			  "$$pen_verify_fail@@%d"
			  "$$pen_mac_addr@@0x%x%x%x%x%x%x",
			  g_ble_timeout_cnt, g_verify_failed_cnt, g_ble_mac_addr_info[5], g_ble_mac_addr_info[4],
			  g_ble_mac_addr_info[3], g_ble_mac_addr_info[2], g_ble_mac_addr_info[1],
			  g_ble_mac_addr_info[0]);
...
static int p9418_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chg_err(" call \n");
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_p9418_ic), GFP_KERNEL);
	if (!chip) {
