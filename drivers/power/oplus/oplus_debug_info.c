...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		queue_delayed_work(oplus_chg_debug_info.oplus_chg_debug_wq, &oplus_chg_debug_info.send_info_dwork,
				   msecs_to_jiffies(SEND_INFO_DELAY));
	} else {
		//soc jump
		...
		//slow check
		if (send_info_flag !=
		    SEND_INFO_FLAG_IRQ) { //plug out with mcu error 0x54  do not clear charger slow flag
		...
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_NON_AUTH,
						 OPLUS_NOTIFY_SC8571_ERROR); /*ic error*/
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_NON_AUTH,
						 OPLUS_NOTIFY_SC85x7_ERROR); /*ic error*/
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_CHG_BATT_RECHG,
						 OPLUS_NOTIFY_CHG_BATT_RECHG); /*add for rechg counts*/
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_WIRELESS_BOOTUP,
						 OPLUS_NOTIFY_WIRELESS_STOP_TX); /*add for wireless chg*/
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_WIRELESS_BOOTUP,
						 OPLUS_NOTIFY_WIRELESS_WIRELESS_CHG_END); /*add for wireless chg*/
		oplus_chg_debug_mask_notify_flag(OPLUS_NOTIFY_BCC_ANODE_POTENTIAL_OVER_TIME,
		...
		if (oplus_switching_support_parallel_chg() == 1) {
			oplus_chg_debug_mask_notify_flag(
				OPLUS_NOTIFY_PARALLEL_LIMITIC_ERROR,
		...
		memset(oplus_chg_debug_info.sc85x7_error_reason, 0, sizeof(oplus_chg_debug_info.sc85x7_error_reason));
			...
			if (!ret) {
				...
				if (oplus_pps_get_support_type() == PPS_SUPPORT_2CP ||
					...
					queue_delayed_work(oplus_chg_debug_info.oplus_chg_debug_wq,
							   &oplus_chg_debug_info.send_info_dwork, 0);
			}
		}
#else
		printk(KERN_ERR "%s\n", oplus_chg_debug_msg);
